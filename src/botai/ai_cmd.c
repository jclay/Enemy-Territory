...
void BotMatch_HelpAccompany(bot_state_t* bs, bot_match_t* match) {
  ...
  sizeof(netname)); client = ClientFromName(netname); other = qfalse;
          }
          else {
          ...
          if (bs->teamgoal.entitynum < 0) {
                  ...
                  if (match->subtype & ST_NEARITEM) {
                          ...
                          trap_BotMatchVariable(match, ITEM, itemname,
  sizeof(itemname));
                          //
                          if (!BotGetMessageTeamGoal(bs, itemname,
  &bs->teamgoal)) {
                                  //BotAI_BotInitialChat(bs, "cannotfind",
  itemname, NULL);
                                  //trap_BotEnterChat(bs->cs, bs->client,
  CHAT_TEAM); return;
                          }
                  }
          }
          //
          if (bs->teamgoal.entitynum < 0) {
                  if (other) BotAI_BotInitialChat(bs, "whereis", teammate,
  NULL); else BotAI_BotInitialChat(bs, "whereareyou", netname, NULL);
                  trap_BotEnterChat(bs->cs, bs->client, CHAT_TEAM);
                  return;
          }
          //the team mate
          ...
          if (match->type == MSG_HELP) {
                  ...
                  if (!bs->teamgoal_time) bs->teamgoal_time = trap_AAS_Time() +
  TEAM_HELP_TIME;
          }
          else {
                  bs->ltgtype = LTG_TEAMACCOMPANY;
                  if (!bs->teamgoal_time) bs->teamgoal_time = trap_AAS_Time() +
  TEAM_ACCOMPANY_TIME; bs->arrive_time = 0;
          }
  #ifdef DEBUG
          BotPrintTeamGoal(bs);
  #endif //DEBUG
...
void BotMatch_Camp(bot_state_t* bs, bot_match_t* match) {
  /*
          ...
          if (match->subtype & ST_THERE) {
                  ...
                  //if info is valid (in PVS)
                  if (entinfo.valid) {
                          areanum = BotPointAreaNum(entinfo.number,
  entinfo.origin); if (areanum && trap_AAS_AreaReachability(areanum)) {
                                  //NOTE: just cheat and assume the bot knows
  where the person is
                                  //if (BotEntityVisible(bs->entitynum, bs->eye,
  bs->viewangles, 360, client)) { bs->teamgoal.entitynum = client;
                                          bs->teamgoal.areanum = areanum;
                                          VectorCopy(entinfo.origin,
  bs->teamgoal.origin); VectorSet(bs->teamgoal.mins, -8, -8, -8);
                                          VectorSet(bs->teamgoal.maxs, 8, 8, 8);
                                  //}
                          }
                  }
                  ...
                  if (bs->teamgoal.entitynum < 0) {
                          BotAI_BotInitialChat(bs, "whereareyou", netname,
  NULL); trap_BotEnterChat(bs->cs, bs->client, CHAT_TEAM); return;
                  }
          }
...
void BotMatch_WhereAreYou(bot_state_t* bs, bot_match_t* match) {
  ...
  bot_goal_t goal;
  char* nearbyitems[] = {"Shotgun",         "Grenade Launcher",
                         "Rocket Launcher", "Plasmagun",
                         "Railgun",         "Lightning Gun",
                         "BFG10K",          "Quad Damage",
                         "Regeneration",    "Battle Suit",
                         "Speed",           "Invisibility",
                         "Flight",          "Armor",
                         "Heavy Armor",     "Red Flag",
                         "Blue Flag",       NULL};
  //
  if (!TeamPlayIsOn()) {
    return;
...
int BotMatchMessage(bot_state_t* bs, char* message) {
  ...
  if (!trap_BotFindMatch(
          message, &match,
          MTCONTEXT_ENTERGAME | MTCONTEXT_INITIALTEAMCHAT | MTCONTEXT_CTF)) {
    return qfalse;
  }
