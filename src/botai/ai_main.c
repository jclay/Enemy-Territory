...
void BotChangeViewAngles(bot_state_t* bs, float thinktime) {
  ...
  if (bs->enemy >= 0) {
    factor = trap_Characteristic_BFloat(bs->character,
                                        CHARACTERISTIC_VIEW_FACTOR, 0.01, 1);
...
==============
*/
void BotInputToUserCommand(bot_state_t* bs,
                           bot_input_t* bi,
                           usercmd_t* ucmd,
                           int delta_angles[3],
                           int time) {
  ...
  // bi->dir[2], bi->speed); the duration for the user command in milli seconds
  ...
  if (BotSinglePlayer() && !(bi->actionflags & ACTION_CROUCH)) {
          ...
          //
          classInfo = BG_GetPlayerClassInfo( bs->sess.sessionTeam,
  bs->sess.playerType ); if (classInfo) { animIndex = BG_GetAnimScriptAnimation(
  bs->client, classInfo, (ucmd->buttons & BUTTON_WALKING) ? ANIM_MT_WALK :
  ANIM_MT_RUN ); if (animIndex >= 0) { anim = BG_GetAnimationForIndex(
  classInfo, animIndex ); if (anim->moveSpeed > 0) { bi->speed *=
  (float)anim->moveSpeed / g_speed.value; if (bi->speed > 1.0f) bi->speed
  = 1.0f;
                          }
                  }
          }
  ...
  */
  ...
  if (value > 0) {
    if (ucmd->forwardmove > value) {
      ucmd->rightmove = (int)((float)ucmd->rightmove *
                              ((float)value / (float)ucmd->forwardmove));
    ...
    if (ucmd->rightmove > value) {
      ucmd->forwardmove = (int)((float)ucmd->forwardmove *
                                ((float)value / (float)ucmd->rightmove));
  ...
  // ucmd.rightmove, ucmd.upmove); Com_Printf("ucmd->serverTime = %d\n",
  // ucmd->serverTime);
}
...
int BotTravelFlagsForClient(int client) {
  ...
  //
  if ((level.doorAllowTeams & TEAM_ALLIES) &&
      cl->sess.sessionTeam == TEAM_ALLIES) {
    tfl |= TFL_TEAM_ALLIES;
    if ((level.doorAllowTeams & ALLOW_DISGUISED_CVOPS) &&
        cl->ps.powerups[PW_OPS_DISGUISED]) {
...
int BotAI(int client, float thinktime) {
  ...
  level.clients[bs->client].sess.latchPlayerType =
      bs->mpClass;  // FIXME: better place for this?
...
void BotGetInitialAttributes(bot_state_t* bs) {
  if (G_IsSinglePlayerGame()) {
    ...
    bs->attribs[BOT_REACTION_TIME] = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_REACTIONTIME, 0, 1);
    bs->attribs[BOT_AIM_ACCURACY] = trap_Characteristic_BFloat(
...
const char* g_botDefaultKeys[] = {"BOT",
                                  "BOT_REACTION_TIME",
                                  "BOT_AIM_ACCURACY",
                                  "BOT_WIMP_FACTOR",
                                  "SETSPEEDCOEFFICIENT",
...
// Written: 1/11/2003
//
void ParseBotDefaultAttributes(char* fileName) {
  // Local Variables ////////////////////////////////////////////////////////
        ...
        switch (j) {
          case e_BOT_NAME:
            ...
            if (foundBot) {
              ...
              if (fieldCount < MAX_BOT_DEFAULT_KEYS) {
                Com_Printf(
                    "File %s has bot %s without enough fields\n", fileName,
                    g_botDefaultValues[g_botDefaultValueCount].m_botName);
          ...
          case e_BOT_REACTION_TIME:
            // Read entity's name
            token = COM_ParseExt(p, qtrue);
            g_botDefaultValues[g_botDefaultValueCount].m_reactionTime =
                atof(token);
          ...
          case e_BOT_AIM_ACCURACY:
            ...
            token = COM_ParseExt(p, qtrue);
            g_botDefaultValues[g_botDefaultValueCount].m_aimAccuracy =
                atof(token);
          ...
          case e_BOT_WIMP_FACTOR:
            // Read entity's classname
            token = COM_ParseExt(p, qtrue);
            g_botDefaultValues[g_botDefaultValueCount].m_wimpFactor =
                atof(token);
          ...
          case e_SETSPEEDCOEFFICIENT:
            // Read entity's classname
            token = COM_ParseExt(p, qtrue);
            g_botDefaultValues[g_botDefaultValueCount].m_speedCoefficient =
                atof(token);
          ...
          case e_SETFIELDOFVIEW:
            // Read entity's classname
            token = COM_ParseExt(p, qtrue);
            g_botDefaultValues[g_botDefaultValueCount].m_scriptedFieldOfView =
                atof(token);
          ...
          case e_SETHEARINGRANGE:
            // Read entity's classname
            token = COM_ParseExt(p, qtrue);
            g_botDefaultValues[g_botDefaultValueCount].m_scriptedHearingRange =
                atof(token);
          ...
          case e_SETCLOSEHEARINGRANGE:
            // Read entity's classname
            token = COM_ParseExt(p, qtrue);
            g_botDefaultValues[g_botDefaultValueCount].m_closeHearingRange =
                atof(token);
          ...
          case e_SETVISIONRANGE:
            // Read entity's classname
            token = COM_ParseExt(p, qtrue);
            g_botDefaultValues[g_botDefaultValueCount].m_visionRange =
                atof(token);
          ...
          case e_SETFARSEEINGRANGE:
            // Read entity's classname
            token = COM_ParseExt(p, qtrue);
            g_botDefaultValues[g_botDefaultValueCount].m_farSeeingRange =
                atof(token);
          ...
          case e_SETMOVEMENTAUTONOMY:
            ...
            g_botDefaultValues[g_botDefaultValueCount].m_movementAutonomy =
                level;
...
// Written: 1/11/2003
//
void BotSetCharacterAttributes(bot_state_t* bs,
...
// Written: 1/11/2003
//
void BotSetUpCharacter(bot_state_t* bs) {
  // Local Variables ////////////////////////////////////////////////////////
  int i;
  ///////////////////////////////////////////////////////////////////////////
...
int BotAISetupClient(int client, struct bot_settings_s* settings) {
  ...
  level.clients[bs->client].sess.latchPlayerType =
      bs->mpClass;  // FIXME: better place for this?
  level.clients[bs->client].sess.latchPlayerWeapon =
      bs->mpWeapon;  // FIXME: better place for this?
  level.clients[bs->client].sess.latchPlayerWeapon2 =
      0;  // FIXME: better place for this?
  //
...
int BotAIThinkFrame(int time) {
  ...
  for (i = lastbot + 1, botcount = 0; botcount < MAX_CLIENTS; i++, botcount++) {
    ...
    botstates[i].botthink_residual +=
        elapsed_time +
        (rand() % (bot_thinktime.integer /
                   4));  // randomize the times a bit so they are more likely to
                         // disperse amongst frames

    if (botstates[i].botthink_residual >=
        thinktime *
            ((VectorLengthSquared(botstates[i].cur_ps.velocity) < SQR(10))
                 ? 2
                 : 1)) {
      botstates[i].botthink_residual -=
          thinktime *
          ((VectorLengthSquared(botstates[i].cur_ps.velocity) < SQR(10)) ? 2
                                                                         : 1);
  ...
  /*	if( bot_profile.integer == 1 ) {
                  ...
                  G_Printf( "BotAIThinkFrame: %4i total (%4i em, %4i fg, %4i en)
     thinkcount: %i\n", totalProfileTime, botTime_EmergencyGoals,
     botTime_FindGoals, botTime_FindEnemy, thinkcount );
          }*/
...
int BotAIStartFrame(int time) {
  ...
  // let bots do their thinking (only if this is dedicated, or the local client
  // hasnt processed bot thinks in a while)
  if (bot_enable.integer &&
      (g_dedicated.integer || (level.lastClientBotThink < level.time - 200))) {
    BotAIThinkFrame(level.time);
  }
#endif  // NO_BOT_SUPPORT

  // Since there is no interpolation on the client-side anymore, there is no
  // need to move the bot at a set time-interval anymore
  /*
...
gentity_t *BotSpawnGameEntity(void) {
        ...
        //
        botGameEntities[numBotGameEntities].s.number = MAX_GENTITIES +
numBotGameEntities; botGameEntities[numBotGameEntities].inuse = qtrue; return
&botGameEntities[numBotGameEntities++];
}

// TAT 11/12/2002 - we're going to want to check that something is from this
list
//		first thing when we spawn it
//		And this returns a ptr to the new entity or to the old one if it
isn't from our list gentity_t *BotCheckBotGameEntity( gentity_t *ent )
{
        int i;
        gentity_t *botent;
        int num;
        ...
        for (i=0; botGameEntityNames[i]; i++) {
                if (!Q_stricmp( botGameEntityNames[i], ent->classname )) {
                        ...
                        if (!botent) {
                                G_Error( "BotCheckBotGameEntity: exceeded
NUM_BOTGAMEENTITIES (%i)", NUM_BOTGAMEENTITIES );
                        }
                        num = botent->s.number;
...
char* botStaticEntityStrings[NUM_BOTSTATICENTITY] = {
    "team_WOLF_checkpoint",   "trigger_flagonly", "misc_mg42",
    "trigger_objective_info", "team_CTF_redflag", "team_CTF_blueflag",
    "func_explosive",         "func_door",        "func_door_rotating",
    "func_constructible",     "trigger_multiple", "trigger_flagonly_multiple",
    "bot_landmine_area",      "bot_attractor",    "bot_sniper_spot",
...
================
*/
gentity_t* BotFindNextStaticEntity(gentity_t* start,
...
void G_SetAASBlockingEntity(gentity_t* ent, int blocking) {
  ...
  // if we are not blocking now, and we were previously, always use the same box
  // for removing the blocking areas
  if (!(blocking & 1)) {
    if (ent->AASblocking & 1) {
      // turn off old blocking areas
      trap_AAS_SetAASBlockingEntity(ent->AASblocking_mins,
                                    ent->AASblocking_maxs, qfalse);
  ...
  // if we are currently blocking, and we are being asked to block again, but
  // with a different box, turn off previous blocking
  if (blocking & 1 && ent->AASblocking & 1 &&
      (!VectorCompare(ent->r.absmin, ent->AASblocking_mins) ||
       !VectorCompare(ent->r.absmax, ent->AASblocking_maxs))) {
    // turn off old blocking areas
    trap_AAS_SetAASBlockingEntity(ent->AASblocking_mins, ent->AASblocking_maxs,
                                  qfalse);
  }

  // if we are blocking now and before, and the bounds are the same, ignore
  if ((blocking == ent->AASblocking) &&
      (VectorCompare(ent->r.absmin, ent->AASblocking_mins) &&
       VectorCompare(ent->r.absmax, ent->AASblocking_maxs))) {
    return;
  }
...
BotRecordTeamDeath
  ...
  Allows the AAS to make dangerous routes more costly, therefore bots will avoid
them when possible
===================
*/
  ...
  if (bs->inuse) {
    ...
    if (bs->leader > -1) {
      trap_Cvar_Set("bot_debug_curAINode",
                    va("%s: leader = %i tagent = %i", bs->ainodeText,
                       bs->leader, bs->leader_tagent));
    } else {
...

    {
      ...
      animModelInfo_t* animModelInfo =
          BG_GetCharacterForPlayerstate(ps)->animModelInfo;
      trap_Cvar_Set(
          "bot_debug_anim",
          va("leg-%s torso-%s",
    ...
    trap_Cvar_Set("bot_debug_pos", (va("(%f,%f,%f)", bs->origin[0],
                                       bs->origin[1], bs->origin[2])));
    ...
    // TAT 12/9/2002 - Throwing some extra info into the cover spot display
    {
      ...
      Com_sprintf(buf, sizeof(buf), "%i(%s)  Enemy = %i", bs->seekCoverSpot,
                  coverSpot ? coverSpot->name : "", bs->enemy);
