...
// NOTE: not using a cvar which can be updated because the game should be
// reloaded anyway
...
qboolean BotCarryingFlag(int client) {
  ...
  if (level.clients[client].ps.powerups[PW_REDFLAG] ||
      level.clients[client].ps.powerups[PW_BLUEFLAG]) {
    return qtrue;
  }
...
==================
*/
int BotFirstReachabilityArea(int entnum,
                             vec3_t origin,
                             int* areas,
                             int numareas,
                             qboolean distCheck) {
  ...
  for (i = 0; i < numareas; i++) {
    ...
    if (trap_AAS_AreaReachability(areas[i])) {
      ...
      if (distCheck) {
        ...
        if (dist < bestDist) {
          trap_Trace(&tr, origin, mins, maxs, center, -1,
                     MASK_PLAYERSOLID & ~CONTENTS_BODY);
          if (tr.fraction > .99f ||
              tr.startsolid) {  // if we start in solid, ignore trace test
            best = areas[i];
            bestDist = dist;
            // if (dist < 128) {
            //	return best;
...
#define BOTAREA_JIGGLE_DIST 32
  ...
  if (ent && ent->client && ent->client->ps.pm_flags & PMF_LADDER) {
    ...
    // trace a line from below us, upwards, finding the first area the line
    // touches
    VectorCopy(origin, start);
    VectorCopy(origin, end);
    if ((entnum >= 0) && g_entities[entnum].inuse &&
    ...
    if (numareas > 0) {
      bestarea =
          BotFirstReachabilityArea(entnum, origin, areas, numareas, qfalse);
    }
    if (bestarea) {
      goto done;
    }
    ...
    if (numareas > 0) {
      bestarea =
          BotFirstReachabilityArea(entnum, origin, areas, numareas, qtrue);
    }
    if (bestarea) {
      goto done;
    }
  ...
  if ((entnum >= 0) && g_entities[entnum].inuse && g_entities[entnum].client) {
    numareas = trap_AAS_BBoxAreas(g_entities[entnum].r.absmin,
                                  g_entities[entnum].r.absmax, areas, 50);
    if (numareas > 0) {
      bestarea =
          BotFirstReachabilityArea(entnum, origin, areas, numareas, qtrue);
    }
  ...
  //@TODO.  The following code seems to often cause bogus areanums to be
  //returned.  They are offset
...
int BotReachableBBoxAreaNum(bot_state_t* bs, vec3_t absmin, vec3_t absmax) {
  ...
  //
  // find the area that is reachable from bs, and closest to the center of the
  // box
  ...
  for (i = 0; i < numareas; i++) {
    if (trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, sorted[i],
                                          bs->tfl)) {
      return sorted[i];
    }
  }
...
static qboolean sDoNearbyGoalCheck(bot_state_t* bs,
                                   vec3_t loc,
                                   gentity_t* target) {
  int t, areanum;
  bot_goal_t goal;
...
#define MAX_NEARBY_TIME 1500
  ...
  t = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, areanum,
                                        bs->tfl);
...
qboolean BotGotEnoughAmmoForWeapon(bot_state_t* bs, int weapon) {
  ...
  // TODO!! check some kind of weapon list that holds the minimum requirements
  // for each weapon
  switch (weapon) {
    default:
      return (qboolean)((clip >= GetAmmoTableData(weapon)->uses) ||
                        (ammo >= GetAmmoTableData(weapon)->uses));  //----(SA)
  }
}
...
qboolean G_WeaponCharged(playerState_t* ps,
                         team_t team,
                         int weapon,
                         int* skill) {
  switch (weapon) {
...
qboolean BotWeaponCharged(bot_state_t* bs, int weapon) {
  return G_WeaponCharged(&bs->cur_ps, bs->sess.sessionTeam, weapon,
                         bs->sess.skill);
}
...
BotWeaponOnlyUseIfInInRange
  ...
  returns qtrue if the given weapon should only be used if the enemy is within
range (binocs, panzer, etc)
==============
*/
  ...
  switch (weapon) {
    case WP_KNIFE:
      ...
      if (level.warmupTime > level.time &&
          !((bs->client + level.warmupTime / 100) % 5)) {
        return 2.0;
      }
      return 0.2;
    ...
    case WP_FG42SCOPE:
      if ((!moving && bs->enemy > -1) &&
          (bs->inventory[ENEMY_HORIZONTAL_DIST] > 300)) {
        return 1.0;
      }
      return 0.1;
    case WP_FLAMETHROWER:
      if ((!moving || bs->enemy > -1) &&
          bs->inventory[ENEMY_HORIZONTAL_DIST] < 800) {
        return 1.0;
      }
      return 0.1;
    case WP_PANZERFAUST:
      if (!moving || bs->enemy > -1) {
        ...
        if (bs->inventory[INVENTORY_HEALTH] < 15 ||
            bs->inventory[ENEMY_HORIZONTAL_DIST] > 400) {
          return 1.0;
        }
      }
      return 0.1;
    ...
    case WP_GRENADE_PINEAPPLE:
      ...
      if (bs->enemy > -1 && bs->inventory[ENEMY_HORIZONTAL_DIST] < 300) {
        if (BotHealthScale(bs->client) < 0.3 &&
            (bs->enemy < 0 || !BotCarryingFlag(bs->enemy))) {
          return 2.0 *
                 (1.0 - (float)bs->inventory[INVENTORY_HEALTH] /
      ...
      break;
    case WP_SMOKE_MARKER:
      if (bs->sess.playerType == PC_FIELDOPS && bs->enemy > -1 &&
          (bs->inventory[ENEMY_HORIZONTAL_DIST] < 400)) {
      ...
      if (!moving) {
        if (bs->sess.playerType == PC_FIELDOPS) {
          if (bs->enemy > -1 && (bs->inventory[ENEMY_HORIZONTAL_DIST] < 400)) {
            if (BG_GetSkyHeightAtPoint(BotGetOrigin(bs->enemy)) ==
                MAX_MAP_SIZE) {
              return 0.f;
            }
            return 1.f;
...
int BotBestFightWeapon(bot_state_t* bs) {
  ...
  for (i = 0; i < WP_NUM_WEAPONS; i++) {
    if (COM_BitCheck(bs->cur_ps.weapons, i)) {
      ...
      //
      // get the wantScale for this weapon given the current circumstances (0.0
      // - 1.0)
...
void BotChooseWeapon(bot_state_t* bs) {
  ...
  if (bs->cur_ps.weaponstate == WEAPON_RAISING ||
      bs->cur_ps.weaponstate == WEAPON_DROPPING || bs->cur_ps.weaponDelay) {
...
Bot cycles to next weapon in inventory, and will use it until told to cycle
again
...
void BotCycleWeapon(bot_state_t* bs) {
  ...
  for (i = curWeapon + 1; i != curWeapon; i++) {
    ...
    if (i >= WP_NUM_WEAPONS) {
      i = 0;
      // we have an endless loop here, when we have no weapons, our current
      // weapon is 0, and we never hit the ending condition in the loop
...
void BotUpdateInventory(bot_state_t* bs) {
  ...
  if (bs->target_goal.entitynum != -1) {
    bs->inventory[GOAL_TRAVELTIME] = trap_AAS_AreaTravelTimeToGoalArea(
        bs->areanum, bs->origin, bs->target_goal.areanum, bs->tfl);
  } else {
    bs->inventory[GOAL_TRAVELTIME] = 0;
  }
  /*	bs->inventory[INVENTORY_TELEPORTER] =
     bs->cur_ps.stats[STAT_HOLDABLE_ITEM] == MODELINDEX_TELEPORTER;
          bs->inventory[INVENTORY_MEDKIT] = bs->cur_ps.stats[STAT_HOLDABLE_ITEM]
     == MODELINDEX_MEDKIT; bs->inventory[INVENTORY_REDFLAG] =
     bs->cur_ps.powerups[PW_REDFLAG] != 0; bs->inventory[INVENTORY_BLUEFLAG] =
     bs->cur_ps.powerups[PW_BLUEFLAG] != 0;*/
}
...
qboolean EntityInLimbo(aas_entityinfo_t* entinfo) {
  ...
  return (g_entities[entinfo->number].client->ps.pm_flags & PMF_LIMBO) ? qtrue
                                                                       : qfalse;
}
...
int BotCanAndWantsToRocketJump(bot_state_t* bs) {
  ...
  /*
     ...
     CHARACTERISTIC_WEAPONJUMPING, 0, 1); if (rocketjumper < 0.5) return qfalse;
          return qtrue;
  */
}
...
void BotGoCamp(bot_state_t* bs, bot_goal_t* goal) {
  /*
          ...
          camper = trap_Characteristic_BFloat(bs->character,
     CHARACTERISTIC_CAMPER, 0, 1); if (camper > 0.99) bs->teamgoal_time = 99999;
...
/*int BotWantsToCamp(bot_state_t *bs) {
        ...
        camper = trap_Characteristic_BFloat(bs->character,
CHARACTERISTIC_CAMPER, 0, 1); if (camper < 0.1) return qfalse;
        ...
        //if camped recently
        if (bs->camp_time > trap_AAS_Time() - 60 + 300 * (1-camper)) return
qfalse;
        //
        if (random() > camper) {
                bs->camp_time = trap_AAS_Time();
        ...
        //the bot should have at least have the rocket launcher, the railgun or
the bfg10k with some ammo
...
trap_BotGetNextCampSpotGoal(cs, &goal)) { traveltime =
trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, goal.areanum,
bs->tfl); if (traveltime && traveltime < besttraveltime) { besttraveltime =
traveltime; memcpy(&bestgoal, &goal, sizeof(bot_goal_t));
                }
        }
  ...
  for (n = 0; n < 10; n++) {
    ...
    if (len > 200) {
      ...
      if (!trace.startsolid) {
        ...
        if (!(pc & CONTENTS_LAVA)) {  //----(SA)	modified since slime is
                                      //no longer deadly
                                      //				if (!(pc & (CONTENTS_LAVA |
                                      //CONTENTS_SLIME))) {
...
bot_moveresult_t BotAttackMove(bot_state_t* bs, int tfl) {
  ...
  jumper = 0.05 * trap_Characteristic_BFloat(bs->character,
  ...
  /*	// if using mobile mg42, go prone if possible
          if (bs->weaponnum == WP_MOBILE_MG42) {
                  if (dist > 1024.0f) {
                          ...
                          oldviewheight =
     level.clients[bs->client].ps.viewheight;
                          level.clients[bs->client].ps.viewheight =
     PRONE_VIEWHEIGHT; if (BotVisibleFromPos( bs->origin, bs->client,
     BotGetOrigin( bs->enemy ), bs->enemy, qtrue )) { trap_EA_Prone( bs->client
     ); level.clients[bs->client].ps.viewheight = oldviewheight; return
     moveresult; } else {
                                  // just crouch
                                  trap_EA_Crouch( bs->client );
                                  level.clients[bs->client].ps.viewheight =
     oldviewheight; return moveresult;
                          }
                  }
          }
  */
  ...
  if (bs->attackcrouch_time < trap_AAS_Time() - 1) {
    ...
    else if (bs->attackcrouch_time < trap_AAS_Time() - 1 &&
             random() < croucher) {
  ...
  /*	//if the bot is stupid
          if (attack_skill <= 0.4) {
                  ...
                  if (dist > attack_dist + attack_range) {
                          if (trap_BotMoveInDirection(bs->ms, forward, 400,
     movetype)) return moveresult;
                  }
                  if (dist < attack_dist - attack_range) {
                          if (trap_BotMoveInDirection(bs->ms, backward, 400,
     movetype)) return moveresult;
                  }
                  return moveresult;
          }
  ...
  for (i = 0; i < 2; i++) {
    ...
    /*		//randomly go back a little
                    if (random() > 0.9) {
                            ...
                            //walk forward or backward to get at the ideal
       attack distance if (dist > attack_dist + attack_range)
       VectorAdd(sideward, forward, sideward); else if (dist < attack_dist -
       attack_range) VectorAdd(sideward, backward, sideward);
                    }
    */                                                                                                                                                                                                                                                                                                                                                                  //
		// check this direction for trigger_hurt
    VectorMA(bs->origin, 80, sideward, end);
    trap_Trace(&tr, bs->origin, vec3_origin, vec3_origin, end, bs->client,
               CONTENTS_TRIGGER);
    if (tr.fraction < 1.0 &&
        !Q_stricmp(g_entities[tr.entityNum].classname, "trigger_hurt")) {
    ...
    if (trap_AAS_PredictClientMovement(
            &move, bs->client, bs->origin, -1, qtrue, sideward, vec3_origin, 10,
            0, 0.1, SE_GAP | SE_LEAVEGROUND | SE_STUCK, -1, qfalse)) {
      // if this is a bad movement, flip the direction, and wait util next frame
      // when we can check that that move is ok
...
qboolean BotEntInvisibleBySmokeBomb(vec3_t start, vec3_t end) {
  ...
  while ((ent = G_FindSmokeBomb(ent))) {
    ...
    // CG_SpawnSmokeSprite().
    smokeCenter[2] += 32;
    // smoke sprite has a maximum radius of 640/2. and it takes a while for it
    // to reach that size, so adjust the radius accordingly.
    smokeRadius = MAX_SMOKE_RADIUS * ((level.time - ent->grenadeExplodeTime) /
                                      MAX_SMOKE_RADIUS_TIME);
    ...
    if (DistanceFromLineSquared(smokeCenter, start, end) <
...
==================
*/
float BotEntityVisible(int viewer,
                       vec3_t eye,
                       vec3_t viewangles,
                       float fov,
                       int ent,
  ...
  // if the entity is using an mg42, then move the trace upwards to avoid the
  // gun
  ...
  // RF, if they are far away, and we arent using a sniper rifle, then only do 1
  // check
  checkCount = 3;
  if ((botstates[viewer].inuse && BotCanSnipe(&botstates[viewer], qtrue)) &&
  ...
  for (i = 0; i < checkCount; i++) {
    //
    contents_mask =
        MASK_SHOT & ~(CONTENTS_CORPSE);  // CONTENTS_SOLID|CONTENTS_PLAYERCLIP;
    passent = viewer;
    hitent = ent;
    ...
    if (trace.fraction >= .99f || trace.ent == hitent ||
        ((entinfo.flags & EF_TAGCONNECT) &&
         (g_entities[trace.ent].nextTrain == g_entities[ent].tagParent))) {
      ...
      vis = 1 /
            ((fogdist * fogdist * 0.001) < 1 ? 1 : (fogdist * fogdist * 0.001));

      // if entering water visibility is reduced
      vis *= waterfactor;

      // xkan 11/25/2002 - are there smoke (from smoke bombs) blocking the
      // sight?
...
================
*/
qboolean BotVisibleFromPos(vec3_t srcorigin,
                           int srcnum,
                           vec3_t destorigin,
                           int destent,
                           qboolean dummy) {
...
================
*/
qboolean BotCheckAttackAtPos(int entnum,
                             int enemy,
                             vec3_t pos,
                             qboolean ducking,
...
qboolean BotGetMovementAutonomyPos(bot_state_t* bs, vec3_t pos) {
  ...
  if (bs->script.frameFlags & BSFFL_MOVETOTARGET) {
    if (bs->target_goal.entitynum == bs->script.entityNum) {
      ...
      // VectorCopy( pos, bs->script.movementAutonomyPos );		// use
      // this as the new autonomy pos VectorCopy( pos, bs->movementAutonomyPos
      // );		// use this as the new autonomy pos
...
float BotGetRawMovementAutonomyRange(bot_state_t* bs) {
  ...
  //		TAT 12/16/2002 - Don't use this range in SP here - if we want
  //the follow distance, we check it explicitly
  ...
  if (level > BMA_HIGH) {
    ...
    G_Printf(
        "BotGetMovementAutonomyRange(): autonomy range less than "
        "BMA_NOVALUE\n");
    return 0;  // should never happen
  }
...
float BotGetMovementAutonomyRange(bot_state_t* bs, bot_goal_t* goal) {
  ...
  if (goal && goal->urgency >= BGU_MEDIUM) {
    range = (range * (1.0 + 0.25 * (goal->urgency - BGU_LOW))) +
            256 * (goal->urgency - BGU_LOW);
  }
...
// Start - TAT 9/18/2002
//	What distance should a bot be from its leader during a follow order?
//Based on autonomy
float BotGetFollowAutonomyDist(bot_state_t* bs) {
  int level;

  // The autonomy distances have nothing to do with how far a unit should be for
  // following
...
qboolean BotPointWithinMovementAutonomy(bot_state_t* bs,
  ...
  //
  // RF, had to remove, alcoves were causing this to fail, when they shouldn't
  // be
...
qboolean BotGoalWithinMovementAutonomy(bot_state_t* bs,
  ...
  for (i = 0, ignoreTrav = bs->ignoreGoals; i < MAX_IGNORE_GOALS;
       i++, ignoreTrav++) {
...
==================
*/
qboolean BotGoalForEntity(bot_state_t* bs,
                          int entityNum,
                          bot_goal_t* goal,
                          int urgency) {
  vec3_t p;
  ...
  if (ent) {
    ...
    if (VectorLengthSquared(ent->r.absmax) &&
        (/*ent->s.eType == ET_MOVER ||*/ ent->s.eType == ET_GENERAL)) {
...
// Start - TAT 8/26/2002
// NOTE:  The following 3 funcs to create and destroy the bot indicator objects
// should probably live somewhere else
//		but I don't have time to figure out where
#define BOTINDICATORSET_POSTDELAY_TIME \
  1000  // can only set a waypoint once every 1 seconds
...
// Gordon: 25/11/02: removing alot of the statics on these funcs, need them
// elsewhere, rename? Move a point back towards the player a bit, and down to
// the floor
void sAdjustPointTowardsPlayer(vec3_t playerLoc, vec3_t endPos, qboolean shouldLoop, vec3_t outPos /* Gordon: vec3_t is an array, no need for the pointer (was vec3_t*) */) {
  ...
  do {
    ...
    // Drop the point to the floor, we don't want the bots trying to climb the
    // walls or anything
    //		Started with some code from BotDropToFloor

    // We're going to do a trace from our goal point to a new location that is
    // way below the old one
  ...
  } while (shouldLoop && (timesThrough < 10) &&
           sAngleBetweenVectors(floorNormal, trace.plane.normal) >= M_PI / 4);
}
...
BotCheckMovementAutonomy
  ...
  if we wander outside out movement autonomy, it better be for a bloody good
reason

        returns qtrue if we have moved outside autonomy range, and need to get
back
=====================
*/
qboolean BotCheckMovementAutonomy(bot_state_t* bs, bot_goal_t* goal) {
  ...
  if (!VectorCompare(vec3_origin, goal->origin) &&
      !BotPointWithinMovementAutonomy(bs, goal, goal->origin)) {
    return qtrue;
  }
...
void BotIgnoreGoal(bot_state_t* bs, bot_goal_t* goal, int duration) {
  ...
  for (i = 0, ignoreTrav = bs->ignoreGoals; i < MAX_IGNORE_GOALS;
    ...
    if (ignoreTrav->expireTime >= level.time) {
      if ((ignoreTrav->expireTime < level.time + duration) &&
          (!ignoreOldest ||
           (ignoreOldest->expireTime < ignoreTrav->expireTime))) {
...
BotDangerousGoal
  ...
  returns qtrue if the given goal is in a dangerous area (dynamite, grenade,
etc)
==================
*/
...
qboolean BotDangerousGoal(bot_state_t* bs, bot_goal_t* goal) {
  ...
  if (dangerEntsCount < MAX_DANGER_ENTS) {
    if (level.time -
            level.clients[bs->client].lastConstructibleBlockingWarnTime <
        5000) {
      dangerEnts[dangerEntsCount] =
          level.clients[bs->client].lastConstructibleBlockingWarnEnt;
      dangerEntsCount++;
    }
  }
    ...
    switch (trav->s.eType) {
      ...
      case ET_PLAYER:
        if (trav->client) {
          ...
          if (!(trav->client->ps.weapon == WP_PANZERFAUST &&
                trav->client->ps.weaponDelay)) {
            continue;
          }
        }
        break;
  ...
  for (i = 0; i < dangerEntsCount; i++) {
    ...
    switch (trav->s.eType) {
      case ET_MISSILE:
        switch (trav->methodOfDeath) {
          ...
          case MOD_GRENADE_LAUNCHER:
            if (trav->nextthink >
                level.time + (2000 + 8 * trav->splashRadius)) {
              continue;
            }
            if (!G_PredictMissile(
                    trav, trav->nextthink - level.time, bangPos,
    ...
    if (trav->s.eType == ET_CONSTRUCTIBLE ||
...
==================
*/
/*int BotFindLeadersEnemy(bot_state_t *bs, int curenemy, aas_entityinfo_t
*entinfo)
{
        //
        // if we dont have a curenemy, and our leader has an enemy, we should
know about it if (curenemy < 0 && bs->leader_tagent >= 0 &&
botstates[bs->leader_tagent].inuse) { if (botstates[bs->leader_tagent].enemy >=
0) { BotEntityInfo(botstates[bs->leader_tagent].enemy, entinfo); if
(!EntityIsDead(entinfo)) {
                                // if we can see our leader
                                if (BotEntityVisible( bs->client, bs->eye,
bs->viewangles, 360, bs->leader_tagent, BotGetOrigin(bs->leader_tagent) ) ) {
                                        //found an enemy
                                        bs->enemy =
botstates[bs->leader_tagent].enemy;
                                        // if they aren't visible, and we dont
want to chase, then ignore them if (!BotEntityVisible(bs->entitynum, bs->eye,
bs->viewangles, 360, bs->enemy, NULL) && !BotWantsToChase(bs)) { bs->enemy = -1;
                                        } else {
                                                if (curenemy >= 0)
                                                        bs->enemysight_time =
trap_AAS_Time() - 2; else bs->enemysight_time = trap_AAS_Time();
//						bs->enemysuicide = qfalse;
                                                bs->enemydeath_time = 0;
                                                bs->last_findenemy_enemy =
bs->enemy; if (!bs->last_enemysight || (bs->last_enemysight < level.time -
10000)) { Bot_ScriptEvent( bs->client, "enemysight",
!(g_entities[bs->enemy].r.svFlags & SVF_BOT) ?
g_entities[bs->enemy].client->pers.netname : g_entities[bs->enemy].scriptName );
                                                }
                                                bs->last_enemysight =
level.time; return qtrue;
                                        }
                                }
                        }
                }
...
=====================*/
qboolean ChangeBotAlertState(bot_state_t* bs,
  ...
  if (newAlertState != bs->alertState &&
      ...
      // if we are relaxing, make sure we don't relax too quickly, this would
      // prevent model from twitching in some weird situations
      // (alert->combat->alert->combat in rapid fire fashion).
      && (newAlertState > bs->alertState ||
          level.time - bs->alertStateChangeTime > 2000)) {
    bs->alertState = newAlertState;
    bs->alertStateChangeTime = level.time;
    bs->alertStateSetTime = level.time;
    return qtrue;
...
void BotUpdateAlertStateSquadSensingInfo(bot_state_t* bs,
...
Return a modifier for how loud a bot is depending on the surface they're moving
on
...
// Written: 10/31/2002
//
qboolean BotIsValidTarget(bot_state_t* bs, int target, int curenemy) {
  // Local Variables ////////////////////////////////////////////////////////
  ...
  // If we are not mounted on an MG42, or sniping, then ignore clients that are
  // not in a valid area
...
==================
*/
int BotFindEnemyMP(bot_state_t* bs,
                   int curenemy,
  ...
  BotSortClientsByDistance(bs->origin, distanceSorted,
                           COM_BitCheck(bs->cur_ps.weapons, WP_PANZERFAUST));
  ...
  for (i = 0; i < level.numConnectedClients; i++) {
    ...
    // if the enemy is dead, Gordon: and not in limbo, still want to kill em
    // totally
    ...
    // this is a complete check, which takes into account all forms of view
    // restriction
    ...
    if (COM_BitCheck(bs->cur_ps.weapons, WP_PANZERFAUST) &&
        (g_entities[j].client->ps.eFlags & EF_MG42_ACTIVE)) {
      dist /= 3;
    }

    // If this is enemy is attempting to disarm dynamite/build, they take
    // preference!
    if (g_entities[j].client->ps.weapon == WP_PLIERS &&
        g_entities[j].client->touchingTOI) {
      dist /= 3;
    }
    ...
    /*		//if the enemy is quite far away, not shooting and the bot is
       not damaged if (curenemy < 0 && dist > 200 && !healthdecrease &&
       !EntityIsShooting(&entinfo))
                    {
                            ...
                            if
       (!InFieldOfVision(g_entities[j].client->ps.viewangles, 120, angles)) {
    ...
    */
    // Gordon: with our aggresive botclipping this is a LOT of places, so still
    // attack them, NOTE: perhaps change this to check if not valid area, then
    // see if we can hit with long range weapon, if not, THEN ignore if we are
    // not mounted on an MG42, or sniping, then ignore clients that are not in a
    // valid area
    /*		if (!(bs->cur_ps.eFlags & EF_MG42_ACTIVE) && !(bs->flags &
       BFL_SNIPING))
                    {
                            // if they are not in a valid area
                            if (!BotGetArea( j ))
    ...
    // check if the enemy visibility.  This does a trace to make sure you're
    // actually visible
    vis =
        BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, fov, j, NULL);
    ...
    if (curenemy >= 0 || bs->last_fire > level.time - 2000 ||
        bs->last_enemysight > level.time - 2000) {
    ...
    /*		if (!bs->last_enemysight || (bs->last_enemysight < level.time -
       10000))
                    {
                            Bot_ScriptEvent( bs->client, "enemysight",
       !(g_entities[bs->enemy].r.svFlags & SVF_BOT) ?
       g_entities[bs->enemy].client->pers.netname :
...
float BotGetReactionTime(bot_state_t* bs) {
  ...
  return bs->attribs
      [BOT_REACTION_TIME];  // * trap_Characteristic_BFloat(bs->character,
                            // CHARACTERISTIC_REACTIONTIME, 0, 1);
}

float swayrand(float x, float y) {
  return sin(level.time / 1000.0 * x * M_PI * 2) *
         cos(level.time / 1000.0 * y * M_PI * 2);
}
...
void BotAimAtEnemy(bot_state_t* bs) {
  ...
  if (g_entities[bs->client].s.eFlags & EF_MG42_ACTIVE) {
    ...
    if (bs->weaponnum == WP_GRENADE_LAUNCHER ||
        bs->weaponnum == WP_GRENADE_PINEAPPLE) {
    ...
    if (bs->weaponnum == WP_FLAMETHROWER) {
      aim_accuracy = trap_Characteristic_BFloat(
          bs->character, CHARACTERISTIC_AIM_ACCURACY_FLAMETHROWER, 0, 1);
    }
  ...
  if (enemyvisible) {
      ...
      if (!(dist > 100 && VectorLengthSquared(dir) < SQR(32))) {
        ...
        if (aim_skill > 0.8 &&
          ...
          trap_AAS_PredictClientMovement(
              ...
              0, dist * 10 / wi.speed, 0.1, 0, 0, qfalse);
    ...
    if ((bs->weaponnum != WP_FLAMETHROWER) &&
      ...
      if (entinfo.origin[2] < bs->origin[2] + 16) {
        ...
        BotAI_Trace(&trace, bestorigin, NULL, NULL, end, entinfo.number,
                    MASK_SHOT);
        //
    ...
    if (aim_skill > 0.5) {
      ...
      //			if (wi.number == WP_BFG ||	//----(SA)
      //removing old weapon references
      if (wi.number == WP_PANZERFAUST || wi.number == WP_GRENADE_LAUNCHER ||
        ...
        if (trap_BotPredictVisiblePosition(bs->lastenemyorigin,
                                           bs->lastenemyareanum, &goal, bs->tfl,
                                           target)) {
  ...
  bs->ideal_viewangles[PITCH] +=
      6 * wi.vspread * swayrand(0.36f, 0.45f) * (1 - aim_accuracy);
  bs->ideal_viewangles[PITCH] = AngleMod(bs->ideal_viewangles[PITCH]);
  bs->ideal_viewangles[YAW] +=
      6 * wi.hspread * swayrand(0.654f, 0.54f) * (1 - aim_accuracy);
  ...
  if (BotScopedWeapon(bs->weaponnum)) {
    ...
    swayang[PITCH] = ZOOM_PITCH_AMPLITUDE * sin(phase) *
    ...
    aim_skill = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_AIM_SKILL_SNIPERRIFLE, 0, 1);
  ...
  } else if (!(bs->cur_ps.eFlags & EF_MG42_ACTIVE) &&
  ...
  if (!(bs->cur_ps.eFlags & EF_MG42_ACTIVE) && aim_accuracy > 0.95 &&
      bs->enemysight_time < trap_AAS_Time() - 2) {
    // set the view angles directly
    if (bs->ideal_viewangles[PITCH] > 180) {
      bs->ideal_viewangles[PITCH] -= 360;
    }
    if (AngleNormalize180(bs->ideal_viewangles[YAW] - bs->viewangles[YAW]) <
        25) {
...
BotCheckAttack
  ...
  returns qfalse if the enemy is unreachable. qtrue if it is possible to attack
the enemy (this doesnt always mean the trigger is pulled).
==================
*/
qboolean BotCheckAttack(bot_state_t* bs) {
  ...
  if ((bs->weaponnum == WP_GARAND_SCOPE || bs->weaponnum == WP_K43_SCOPE) &&
      (level.time - bs->last_fire) < 800) {
    // pause for a bit
    return qtrue;
  }

  {
    ...
    if (SQR(range) <
        VectorDistanceSquared(bs->origin, BotGetOrigin(bs->enemy))) {
      // cant reach them
      return qfalse;
    }
  }
  ...
  if ((bs->last_botcheckattack_weapon == bs->weaponnum) &&
  ...
  reactiontime = BotGetReactionTime(bs);  // trap_Characteristic_BFloat(bs->character,
                                          // CHARACTERISTIC_REACTIONTIME, 0, 1);
  aimskill =
      trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL, 0, 1);
  ...
  if ((bs->cur_ps.weapon == WP_GRENADE_LAUNCHER ||
       bs->cur_ps.weapon == WP_GRENADE_PINEAPPLE) &&
  ...
  BotAI_Trace(&trace, start, mins, maxs, end, bs->entitynum,
              MASK_SHOT);  //----(SA) should this maybe check the weapon type
                           //and adjust the clipflag?  it seems like this is
                           //probably fine as-is, but I thought I'd note it.
  ...
  if (wi.proj.damagetype & DAMAGETYPE_RADIAL) {
    // if they are carrying the flag, and are close to the destination, fire
    // away
    if (BotCarryingFlag(bs->enemy)) {
      ...
      if (trav) {
        ...
        if (VectorDistanceSquared(g_entities[bs->enemy].r.currentOrigin, end) <
            SQR(800)) {
          goto passed;
        }
      }
    ...
    for (i = 0; i < level.maxclients; i++) {
      ...
      if (!CanDamage(BotGetEntity(i), end) &&
          !CanDamage(BotGetEntity(i), bs->origin)) {
        continue;
      }
      // if they are within range
      if ((dist = VectorDistance(g_entities[i].r.currentOrigin, trace.endpos)) <
          wi.proj.radius) {
      ...
      } else if ((dist = 4.0 * VectorDistance(g_entities[i].r.currentOrigin,
                                              bs->origin)) < wi.proj.radius) {
        // dist /= 4.0;
        // points = (wi.proj.damage * (dist / wi.proj.radius));
        // if (points > 10) {
        if (BotSameTeam(bs, i)) {
...
void CheckButtons(void)
{
        ...
        for (ent = entities; ent; ent = ent->next)
        {
                ...
                if (!strcmp(classname, "func_button"))
                {
                        ...
                        VectorClear(angles);
                        AAS_BSPModelMinsMaxsOrigin(modelindex - 1, angles, mins,
maxs, NULL);
                        //get the lip of the button
                        lip = AAS_FloatForBSPEpairKey(ent, "lip");
                        if (!lip) lip = 4;
                        //get the move direction from the angle
                        VectorSet(angles, 0, AAS_FloatForBSPEpairKey(ent,
"angle"), 0); AAS_SetMovedir(angles, movedir);
                        //button size
                        ...
                        dist = fabs(movedir[0]) * size[0] + fabs(movedir[1]) *
size[1] + fabs(movedir[2]) * size[2];// - lip; dist *= 0.5;
                        //
                        ...
                        if (health)
                        {
                                ...
                                AAS_DrawPermanentCross(goalorigin, 4,
LINECOLOR_BLUE); } //end if else
                        {
                                //add bounding box size to the dist
                                AAS_PresenceTypeBoundingBox(PRESENCE_CROUCH,
bboxmins, bboxmaxs); for (i = 0; i < 3; i++)
                                {
                                        if (movedir[i] < 0) dist +=
fabs(movedir[i]) * fabs(bboxmaxs[i]); else dist += fabs(movedir[i]) *
fabs(bboxmins[i]); } //end for
                                //calculate the goal origin
                                VectorMA(origin, -dist, movedir, goalorigin);
                                //
...
100); trace = AAS_TraceClientBBox(start, end, PRESENCE_CROUCH, -1); if
(!trace.startsolid)
                                {
...
LINECOLOR_YELLOW);
                                ...
                                AAS_DrawPermanentCross(start, 4,
LINECOLOR_BLUE); VectorAdd(maxs, origin, start); AAS_DrawPermanentCross(start,
4, LINECOLOR_BLUE); } //end else if (++numbuttons > 5) return; } //end if }
//end for } //end of the function CheckButtons
*/
...
int BotEntityToActivate(int entitynum) {
  ...
  // RF, disabled this until I can figure out a way of getting this all working
  // with the current code
  return 0;

  /*	BotEntityInfo(entitynum, &entinfo);
          Com_sprintf(model, sizeof( model ), "*%d", entinfo.modelindex);
          for (ent = trap_AAS_NextBSPEntity(0); ent; ent =
  trap_AAS_NextBSPEntity(ent)) { if (!trap_AAS_ValueForBSPEpairKey(ent, "model",
  tmpmodel, sizeof(tmpmodel))) continue; if (!strcmp(model, tmpmodel)) break;
          }
          if (!ent) {
                  BotAI_Print(PRT_ERROR, "BotEntityToActivate: no entity found
  with model %s\n", model); return 0;
          }
          trap_AAS_ValueForBSPEpairKey(ent, "classname", classname,
  sizeof(classname)); if (!classname) { BotAI_Print(PRT_ERROR,
  ...
  sizeof(targetname[0]))) { #ifdef OBSTACLEDEBUG BotAI_Print(PRT_ERROR,
  "BotEntityToActivate: entity with model \"%s\" has no targetname\n", model);
  #endif //OBSTACLEDEBUG
                  return 0;
          }
          cur_entities[0] = trap_AAS_NextBSPEntity(0);
          for (i = 0; i >= 0 && i < 10;) {
                  for (ent = cur_entities[i]; ent; ent =
  trap_AAS_NextBSPEntity(ent)) { if (!trap_AAS_ValueForBSPEpairKey(ent,
  "target", target, sizeof(target))) continue; if (!strcmp(targetname[i],
  target)) { cur_entities[i] = trap_AAS_NextBSPEntity(ent); break;
                          }
                  }
                  if (!ent) {
                          BotAI_Print(PRT_ERROR, "BotEntityToActivate: no entity
  with target \"%s\"\n", targetname[i]); i--; continue;
                  }
                  if (!trap_AAS_ValueForBSPEpairKey(ent, "classname", classname,
  sizeof(classname))) { BotAI_Print(PRT_ERROR, "BotEntityToActivate: entity with
  target \"%s\" has no classname\n", targetname[i]); continue;
                  }
                  if (!strcmp(classname, "func_button")) {
                          //BSP button model
                          return ent;
          ...
          //BotAI_Print(PRT_ERROR, "BotEntityToActivate: unknown activator with
  classname \"%s\"\n", classname); return 0;*/
}
...
void BotAIBlocked(bot_state_t* bs, bot_moveresult_t* moveresult, int activate) {
  ...
  if ((entinfo.number < level.maxclients) && (bs->blockentTime < level.time)) {
    // RF, in multiplayer, only ignore if this is our leader and we are
    // currently following them
    if (((bs->ainode == AINode_MP_DefendTarget) &&
    ...
    if (bs->target_goal.flags & GFL_LEADER) {
      if ((BotPointWithinMovementAutonomy(bs, &bs->target_goal, bs->origin)) &&
          (VectorDistanceSquared(bs->origin,
                                 g_entities[bs->leader].r.currentOrigin) <
           SQR(512))) {
        VectorCopy(bs->origin, bs->target_goal.origin);
        bs->target_goal.areanum = bs->areanum;
        return;
  ...
  if (entinfo.modelindex > 0 && entinfo.modelindex <= max_bspmodelindex &&
    ...
    // the blocking entity
    if (((Q_stricmp(g_entities[entinfo.number].classname, "func_door") == 0) ||
         (Q_stricmp(g_entities[entinfo.number].classname,
                    "func_door_rotating") == 0))) {
      if ((g_entities[entinfo.number].moverState == MOVER_POS1) ||
          (g_entities[entinfo.number].moverState == MOVER_POS1ROTATE)) {
        oldMoverState = g_entities[entinfo.number].moverState;
        G_TryDoor(BotGetEntity(entinfo.number), BotGetEntity(bs->client),
                  BotGetEntity(bs->client));
        if (g_entities[entinfo.number].moverState != oldMoverState) {
          // wait for it to finish opening
          return;
        }
      } else if ((g_entities[entinfo.number].moverState == MOVER_POS2) ||
...
#ifdef OBSTACLEDEBUG
//		ClientName(bs->client, netname, sizeof(netname));
//		BotAI_Print(PRT_MESSAGE, "%s: I've got no brain cells for
//activating entities\n", netname);
#endif /* OBSTACLEDEBUG */
  ...
  //
  //	if (moveresult->flags & MOVERESULT_ONTOPOFOBSTACLE) movetype =
  //MOVE_JUMP; 	else
  movetype = MOVE_WALK;
  ...
  //MASK_PLAYERSOLID); 	if (bsptrace.fraction >= 1) movetype = MOVE_CROUCH; get
  // the sideward vector
...
void BotCheckEvents(bot_state_t* bs, entityState_t* state) {
  ...
  //
  // this sucks, we're accessing the gentity_t directly but there's no other
  // fast way to do it right now
...
void BotCheckAir(bot_state_t* bs) {
  /*	if (bs->inventory[INVENTORY_ENVIRONMENTSUIT] <= 0) {
     ...
     (CONTENTS_WATER|CONTENTS_SLIME|CONTENTS_LAVA)) { return;
                  }
          }*/
}
...
#define AISTATE_RELAXING_FOOTSTEP \
  4000  // time it takes to relax from foot step hearing
...
void BotUpdateReconInfo(bot_state_t* bs) {
  ...
  // want to update if we've been hit above regardless, since that's worse than
  // just spotting an enemy
  //		but now if it's not all clear, we don't need to do any further
  //updates
  ...
  for (i = 0; i < level.maxclients; i++) {
    ...
    if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 120,
                         entinfo.number, NULL)) {
...
==================
*/
void BotDeathmatchAIFirstCalled(bot_state_t* bs) {
...
void BotPowThink(bot_state_t* bs) {
...
void BotDeathmatchAI(bot_state_t* bs, float thinktime) {
  ...
  if ((level.warmupTime > level.time) &&
      (bs->lastClassCheck < level.time - 1000)) {  // check for a better class
  ...
  /*	if (!bs->scriptedSleep) {
                  ...
                  for (i = 0; i < MAX_NODESWITCHES && bs->ainode; i++) {
                          if(bot_profile.integer == 4 || bot_profile.integer ==
     5) { qboolean ret; int t = trap_Milliseconds(); const char* txt =
     bs->ainodeText;
  ...
  if (i > 0) {
    //		Bot_ScriptLog_Entry( bs, qfalse, ">> NODE CHANGE <<",
    //va("%s\r\n", bs->ainodeText) );
  }
  ...
  if (i >= MAX_NODESWITCHES) {
    ...
    BotAI_Print(PRT_ERROR, "%s at %1.1f switched more than %d AI nodes\n", name,
                trap_AAS_Time(), MAX_NODESWITCHES);
// #endif // #ifndef DONT_PRINT_REPEATED_AI_ERRORS
#endif
  }
  ...
  if (g_gametype.integer >= GT_WOLF) {
    if (rand() % 2 == 0) {
      ...
      if (winner == (bs->sess.sessionTeam - 1)) {
        // if (rand()%3) {
        BotSendVoiceChat(bs, "Cheer", SAY_ALL, 1000 + rand() % 5000,
                         BOT_SHOWTEXT, qfalse);
        //} else {
        //	BotSendVoiceChat( bs, "GoodGame", SAY_ALL, 1000 + rand()%3000,
        //BOT_SHOWTEXT, qfalse );
        //}
      } else if (bs->sess.sessionTeam) {
        // if (rand()%2) {
        BotSendVoiceChat(bs, "Negative", SAY_ALL, 1000 + rand() % 5000,
                         BOT_SHOWTEXT, qfalse);
        //} else {
        //	BotSendVoiceChat( bs, "GoodGame", SAY_ALL, 1000 + rand()%3000,
        //BOT_SHOWTEXT, qfalse );
...
void BotRecordTeamChange(int client) {
  ...
  for (i = 0; i < level.maxclients; i++) {
    ...
    if (!BotSinglePlayer() && !BotCoop()) {
      BotVoiceChatAfterIdleTime(bs->client, "Hi", SAY_TEAM,
                                1000 + rand() % 6000, BOT_SHOWTEXT, 7000,
                                qfalse);
    }
  }
}
...
void BotRecordKill(int client, int enemy) {
  ...
  if (BotSameTeam(bs, enemy)) {
    ...
    // Don't use excess voices in Single Player
    if (!BotSinglePlayer() && !BotCoop()) {
      BotVoiceChatAfterIdleTime(bs->client, "Sorry", SAY_TEAM,
                                1000 + rand() % 4000, qfalse,
  ...
  if (!G_IsSinglePlayerGame()) {
    ...
    if (bs->shorterm_kill_count > 2) {
      BotSendVoiceChat(bs, "Yeah", SAY_ALL, 1000 + rand() % 1000, qfalse,
                       qfalse);
    } else if (bs->shorterm_kill_count > 1) {
      BotVoiceChatAfterIdleTime(bs->client, "EnemyWeak", SAY_TEAM,
                                1000 + rand() % 1000, qfalse, 3000, qfalse);
    }
  }
}
...
void BotRecordPain(int client, int enemy, int mod) {
  ...
  if (enemy < level.maxclients && BotSameTeam(bs, enemy)) {
    // friendly fire!
    // Don't use excess voices in Single Player
    BotVoiceChatAfterIdleTime(bs->client, "HoldYourFire", SAY_TEAM,
                              1000 + rand() % 1000, qfalse,
                              3000 + rand() % 2000, qfalse);
    return;
  }
  ...
  if (bs->defendgoal.entitynum >= level.maxclients) {
    ...
    //  Don't use excess voices in Single Player
    if (!BotSinglePlayer() && !BotCoop()) {
      BotVoiceChatAfterIdleTime(bs->client, "TakingFire", SAY_TEAM,
                                1000 + rand() % 1000, qfalse,
...
void BotRecordDeath(int client, int enemy) {
  ...
  if (bs->defendgoal.entitynum > level.maxclients) {
    ...
    //
    // we are near the goal
    BotVoiceChatAfterIdleTime(bs->client, "Incoming", SAY_TEAM,
                              1000 + rand() % 1000, qfalse, 6000, qfalse);
  }
}
...
BotGetAimAccuracySkill
        Get the bot's aim accuracy and skill accounting for distance from
leader, weapon, pose (standing vs. crouching vs. prone)
===================
*/
void BotGetAimAccuracySkill(bot_state_t* bs,
  ...
  if (bs->weaponnum == WP_FLAMETHROWER) {
    // aim_accuracy = trap_Characteristic_BFloat(bs->character,
    // CHARACTERISTIC_AIM_ACCURACY_FLAMETHROWER, 0, 1);
  }
  ...
  aim_accuracy =
      aim_accuracy * (1.0f - (NO_LEADER_MAX_AIM_PENALTY * noLeaderPenalty));
...
int BotBestSniperSpot(bot_state_t* bs) {
  ...
  bestTime = 99999;
  while (
      (trav = BotFindNextStaticEntity(trav, BOTSTATICENTITY_BOT_SNIPERSPOT))) {
    // is it disabled?
...
int BotBestLandmineSpotingSpot(bot_state_t* bs) {
  ...
  bestSpot = NULL;
  bestTime = 99999;
  while ((trav = BotFindNextStaticEntity(
              trav, BOTSTATICENTITY_BOT_LANDMINESPOTINGSPOT))) {
    // is it disabled?
    if (trav->aiInactive & (1 << bs->sess.sessionTeam)) {
      continue;
    }
...
int BotBestMG42Spot(bot_state_t* bs, qboolean force) {
  ...
  while ((mg42 = BotFindNextStaticEntity(mg42, BOTSTATICENTITY_MG42))) {
    ...
    if (trav->melee->active) {
      // if the person using it is from the other team, then we should still
      // head for it, to try and take it from them
      if (mg42->r.ownerNum < level.maxclients &&
          (g_entities[mg42->r.ownerNum].client->sess.sessionTeam !=
           bs->sess.sessionTeam)) {
        // errr..
      } else {
        continue;
    ...
    // NOTE: if it's active, and we got to here, then it must be an enemy, so
    // "ignore" the ignoreTime
    if (!force && !mg42->active && trav->botIgnoreTime &&
        (trav->botIgnoreTime > level.time - 1000)) {
      continue;
    }
    // get the travel time to the goal
    areanum = BotPointAreaNum(trav->s.number, trav->s.origin);
...
void BotDropFlag(bot_state_t* bs) {
  ...
  if (item) {
    ...
    flag->s.modelindex2 =
        ent->s.otherEntityNum2;  // JPW NERVE FIXME set player->otherentitynum2
                                 // with old modelindex2 from flag and restore
                                 // here
...
void BotCalculateMg42Spots(void) {
  ...
  // Start	TAT	9/24/2002
  //		This isn't working for constructible MG42s, because our traces
  //are hitting the constructible markers 		so we need to temporarily unlink all
  //the linked constructible markers, and then we'll link 		them again after we're
  //done, so they don't show up in the traces
  ...
  while ((trav = BotFindNextStaticEntity(trav, BOTSTATICENTITY_MG42))) {
    ...
    // if the count is hugely in favor of one side, then it must belong to the
    // other side
...
void BotDefaultNode(bot_state_t* bs) {
  ...
  if (!bs->areanum || bs->sess.sessionTeam >= TEAM_SPECTATOR ||
      !bs->sess.sessionTeam) {
    AIEnter_MP_Stand(bs);
...
#define PLAYER_PERFORMANCE_COMMENT_FREQENCY \
  5  // the number of recent shots we will comment on
#define ACCURACY_CHANGE_THRESHOLD \
  0.2  // how big should the change (in either direction) be to consider
       // commenting
...
void BotCheckVoiceChats(bot_state_t* bs) {
  if (VectorLengthSquared(bs->cur_ps.velocity) < SQR(10)) {
    ...
    if (bs->sess.playerType != PC_FIELDOPS && ClientNeedsAmmo(bs->client)) {
      BotVoiceChatAfterIdleTime(bs->client, "NeedAmmo", SAY_TEAM,
                                2000 + rand() % 10000, qfalse,
    ...
    if ((bs->sess.playerType == PC_MEDIC &&
         BotHealthScale(bs->client) <= 0.0) ||
        (bs->sess.playerType != PC_MEDIC &&
         BotHealthScale(bs->client) <= 0.2)) {
      BotVoiceChatAfterIdleTime(bs->client, "Medic", SAY_TEAM,
                                2000 + rand() % 10000, qfalse,
                                30000 + rand() % 10000, qfalse);
    }

    // if we have received health, then thank someone
    if (bs->sess.playerType != PC_MEDIC && bs->last_checkvoice_health > 0 &&
        bs->cur_ps.stats[STAT_HEALTH] > bs->last_checkvoice_health) {
      BotVoiceChatAfterIdleTime(bs->client, "Thanks", SAY_TEAM,
                                500 + rand() % 1000, qfalse,
  ...
  if (bs->enemy >= 0) {
    ...
    if (bs->enemy >= 0) {
      ...
      if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360,
        ...
        // TAT 12/6/2002 - don't use the knife here (only can successfully use
        // it in BattleChase)
        if (bs->weaponnum != WP_KNIFE) {
          // aim at the enemy
          BotAimAtEnemySP(bs);
...
void BotShareLastAttacked(bot_state_t* bs) {
  ...
  ent = BotGetEntity(bs->client);
  //
  for (i = 0, ptbs = botstates, trav = g_entities; i < level.maxclients;
       i++, ptbs++, trav++) {
    if (!ptbs->inuse) {
      continue;
    }
...
qboolean BotSeekCover(bot_state_t* bs) {
  ...
  //
  // limit calls
  // if (bs->lastSeekCover >= level.time - 500) return qfalse;	// dont hog
  // calls if (lastcall == level.time) return qfalse; lastcall = level.time;
  ...
  //
  area = trap_AAS_NearestHideArea(
      bs->client, bs->origin, bs->areanum, bs->enemy,
...
#define BOT_GETORIGIN_NUMEYES 9
  ...
  // TAT 11/12/2002 - using Ryan's botentity system, so might have an entity num
  // out of range
  ...
  } else if (g_entities[entnum].s.eType == ET_TRIGGER_MULTIPLE ||
             g_entities[entnum].s.eType == ET_MOVER) {
...
==============
*/
int BotReduceListByTravelTime(int* list,
                              int numList,
                              vec3_t destpos,
                              int destarea,
                              int traveltime) {
  ...
  for (i = 0; i < numList; i++) {
    ...
    t = trap_AAS_AreaTravelTimeToGoalArea(areanum, BotGetOrigin(list[i]),
                                          destarea, lbs->tfl);
    if (!t) {
      continue;
    }
...
int BotTravelTimeToEntity(bot_state_t* bs, int entnum) {
  ...
  return trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, area,
                                           bs->tfl);
}
...
===============
*/
qboolean BotDirectMoveToGoal(bot_state_t* bs,
                             bot_goal_t* goal,
                             bot_moveresult_t* moveresult) {
  vec3_t dir;
  aas_clientmove_t move;
  ...
  //	trap_Trace( &tr, bs->origin, vec3_origin, vec3_origin, goal->origin,
  //bs->client, MASK_PLAYERSOLID & ~CONTENTS_BODY ); 	if (tr.fraction < 1.0)
  //return qfalse;
  //
  ...
  //
  if (trap_AAS_PredictClientMovement(
          &move, bs->client, bs->origin, goal->entitynum, qfalse, dir,
          goal->origin, -1, 40, 0.05,
          SE_ENTERAREA | SE_HITGROUNDDAMAGE | SE_HITENT | SE_HITGROUNDAREA |
              SE_STUCK | SE_GAP,
          goal->areanum,
#ifdef _DEBUG
          qtrue))
...
#endif
  {
    ...
    switch (move.stopevent) {
      ...
      case SE_HITGROUNDAREA:
        ...
        if (dist < 200) {
          trap_EA_Move(bs->client, dir,
                       400 - (320.0f * (128.0f - dist) / 128.0f));
        } else {
          trap_EA_Move(bs->client, dir, 400);
        }
        // check against other players/bots
        //		TAT 2/3/2003 - you can be blocked by stuff that isn't a
        //player - changing trace mask to include other stuff
        trap_Trace(&tr, bs->origin, bs->cur_ps.mins, bs->cur_ps.maxs,
                   goal->origin, bs->client, MASK_SHOT /*CONTENTS_BODY*/);
        if (tr.fraction < .99f &&
            VectorDistanceSquared(bs->origin, tr.endpos) < SQR(30) &&
            tr.entityNum != ENTITYNUM_WORLD) {  // best not be worldspawn
          // blocked, avoid them
          moveresult->blocked = qtrue;
...
qboolean BotGetReachableEntityArea(bot_state_t* bs,
  ...
  if (VectorDistanceSquared(ent->r.absmin, ent->r.absmax) > (16 * 16)) {
    ...
    if (oldest) {
      oldestTime = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                                     oldest, bs->tfl);
    }
    ...
    oldestTime =
        trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, i, bs->tfl);
  }
  ...
  if (ent->r.contents & CONTENTS_TRIGGER) {
    ...
    if (!trap_EntityContactCapsule(mins, maxs, ent)) {
      ...
      vec[2] -= 512;
      // trace to the ground
      trap_Trace(&tr, center, bs->cur_ps.mins, bs->cur_ps.maxs, vec, -1,
                 MASK_PLAYERSOLID & ~CONTENTS_BODY);
...
BotIsConstructible
  ...
  returns qtrue if the contructible attached to the given target_objective_info
can be built
====================
*/
...
BotCanSnipe
  ...
  returns WP_NONE if we cant snipe, otherwise returns the weapon we can snipe
with
=================
*/
...
float BotHealthScale(int entnum) {
  return g_entities[entnum].health /
         (float)g_entities[entnum].client->ps.stats[STAT_MAX_HEALTH];
}
...
qboolean EnemyIsCloseEnoughToFight(bot_state_t* bs) {
  ...
  // If we've got the knife, the enemy isn't visible, or the enemy is too far,
  // we can't use battle fight.
  if ((bs->weaponnum == WP_KNIFE) ||
      !BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, bs->enemy,
                        NULL) ||
      (VectorDistanceSquared(bs->origin, entinfo.origin) >
       SQR(kBOT_CHASE_RANGE))) {
    return qfalse;
  }
...
// TAT 11/21/2002
//		This is silly - almost all the single player ai nodes do the
//same thing 		Find an enemy and try to attack it
...
void BotUpdateViewAngles(bot_state_t* bs,
                         bot_goal_t* goal,
                         bot_moveresult_t moveresult) {
  vec3_t target;
  vec3_t dir;
  ...
  if (bs->enemy < 0 && !(bs->flags & BFL_IDEALVIEWSET)) {
    ...
    } else if (trap_BotMovementViewTarget(bs->ms, &goal, bs->tfl, 300,
                                          target)) {
...
int BotBestTargetWeapon(bot_state_t* bs, int targetNum) {
  ...
  // this function currently only supports the following types of target
  // entities
  ...
  if (!(validWeapons[0] & bs->cur_ps.weapons[0]) &&
      !(validWeapons[1] & bs->cur_ps.weapons[0])) {
    return WP_NONE;
  }
...
gentity_t* BotGetVisibleDamagableScriptMover(bot_state_t* bs) {
  ...
  for (i = MAX_CLIENTS, trav = g_entities + MAX_CLIENTS; i < level.num_entities;
       i++, trav++) {
    if (!trav->inuse) {
      continue;
    }
    ...
    if (!BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, i,
                          NULL)) {
      continue;
    }
    ...
    /*		// it's an enemy mover, can we see it?
                    ...
                    trap_Trace( &tr, bs->eye, vec3_origin, vec3_origin,
       BotGetOrigin( i ), -1, MASK_PLAYERSOLID & ~(CONTENTS_BODY) ); if
       (tr.entityNum != i) { continue;
                    }*/
...
void BotCountLandMines(void) {
  ...
  // reset counts
  trav = NULL;
  while ((trav = BotFindNextStaticEntity(trav,
  ...
  while ((mine = G_FindLandmine(mine))) {
    // doesn't matter if it's not armed, so that we dont drop too many landmines
    // at once
    ...
    trav = NULL;
    while ((trav = BotFindNextStaticEntity(
                trav, BOTSTATICENTITY_BOT_LANDMINE_AREA))) {
      // are we within range?
      if (PointInBounds(org, trav->r.absmin, trav->r.absmax)) {
        ...
        // dont break here or else if we have overlapping areas, all mines in
        // the overlapping section will only count to the first area
      }
    }
  }
...
void BotPickupWeapon(int client, int weaponnum, qboolean alreadyHave) {
  ...
  for (i = 0; i < level.numConnectedClients; i++) {
    // send the noweapon command with who as the 1st param, and 0 meaning we
    // don't have no weapon
