...
qboolean BotMP_CheckClassActions(bot_state_t* bs) {
  ...
  hasLeader = qfalse;
  if (bs->leader > -1) {
    if (BotCarryingFlag(bs->leader) ||
        !(g_entities[bs->leader].r.svFlags & SVF_BOT)) {
      hasLeader = qtrue;
    }
  }

  /*	if( bs->script.frameFlags & BSFFL_MOVETOTARGET ) {
                  hasLeader = qtrue;	// dont go so far if we're trying to get
     to a marker
          }*/
  ...
  if (BotClass_MedicCheckRevives(bs, (hasLeader ? 300 : 600), &target, qtrue)) {
    if (target.entitynum != bs->target_goal.entitynum ||
  ...
  if (BotCarryingFlag(bs->client)) {
    return qfalse;  // only revive if we have flag (Gordon: or skip other checks
                    // if we have flag to make more sense...)
  }
...
qboolean BotMP_CheckEmergencyGoals(bot_state_t* bs) {
  ...
  // if a bot finds itself in a void, it should use DrirectMove() to get out of
  // it
  if (!bs->areanum) {
  ...
  if (g_cheats.integer && bot_debug.integer == BOT_DEBUG_FOLLOW_PLAYER) {
    ...
    if (BotGoalForEntity(bs, bs->leader, &target, BGU_LOW)) {
      if (bs->target_goal.entitynum == target.entitynum &&
    ...
    // DEBUG MODE: bot_debug = 12 (dont look for goals)
             ...
             bot_debug.integer == BOT_DEBUG_FOLLOW_PLAYER + 1) {
    return qfalse;
  }
  ...
  if (!BotIsPOW(bs)) {
    ...
    if (BotCheckMovementAutonomy(bs, &bs->target_goal)) {
      if (bs->target_goal.entitynum == bs->leader &&
          bs->ainode == AINode_MP_DefendTarget) {
        return qfalse;
      }
      if (bs->ainode == AINode_MP_MoveToAutonomyRange) {
    ...
    if (BotFindDroppedFlag(&trav)) {
      ...
      if (!(trav->r.ownerNum == bs->client &&
            trav->botIgnoreTime > level.time)) {
        // if we are already going for it
        if (bs->target_goal.entitynum == trav->s.number &&
        ...
        if (BotGoalForEntity(NULL, trav->s.number, &target, BGU_HIGH)) {
          ...
          //
          if (VectorDistanceSquared(target.origin, bs->origin) <
                  (1600 * 1600) ||
              trap_InPVS(target.origin, bs->origin)) {
            if (trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                                  target.areanum, bs->tfl)) {
              bs->target_goal = target;
  ...
  // if we have a visible & damagable script_mover target, then we should try to
  // blow it up
  ...
  while ((trav = BotFindNextStaticEntity(trav, BOTSTATICENTITY_CHECKPOINT))) {
    // if the opposition team controls this checkpoint, or it hasnt been
    // captured yet
    if (trav->count ==
  ...
  // in GT_WOLF mode on defense, if the flag is not at base, then all units
  // should help get it back
  if (level.captureFlagMode) {
    ...
    if (bs->sess.sessionTeam != level.attackingTeam) {
      ...
      if (!BotFlagAtBase(bs->sess.sessionTeam, &flag)) {
        ...
        if (trav) {
          //
          if (BotNumTeamMatesWithTarget(bs, trav->s.number, NULL, 0) <
              (int)ceil(0.5 * numTeammates)) {
            //
            ...
            numList =
                trap_AAS_BBoxAreas(trav->r.absmin, trav->r.absmax, list, 32);
            if (numList) {
              oldestTime = -1;
              bestDist = -1;
              for (i = 0; i < numList; i++) {
                ...
                t = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                                      list[i], bs->tfl);
                if (t > 0) {
                  // choose the reachable area closest to the center of the
                  // info_objective brush
           ...
           bs->origin, target.areanum, bs->tfl); if (t) {
                           // if we are already heading there, continue
                           if (bs->target_goal.entitynum == target.entitynum &&
           bs->ainode == AINode_MP_TouchTarget) return qfalse; bs->target_goal =
           target; BotFindSparseDefendArea(bs, &bs->target_goal);
                           AIEnter_MP_DefendTarget(bs);
                           return qtrue;
                   }
           }*/
        }
        // keep sending some troops to the flag carrier, so we have the most
        // direct route covered
        flag = BotGetEnemyFlagCarrier(bs);
        if (flag &&
            (VectorDistanceSquared(flag->r.currentOrigin, bs->eye) <
                 (1024 * 1024) ||
             trap_InPVS(flag->r.currentOrigin, bs->eye) ||
             BotNumTeamMatesWithTarget(bs, flag->s.number, NULL, 0) < 1)) {
          // HACK, err... lets just take a snapshot of where they are now, and
          // go down there
    ...
    } else  // if we are close to the flag, get it!
    {
      ...
      if (((bs->leader == -1) ||
          ...
          (BotFlagAtBase(
               level.attackingTeam == TEAM_AXIS ? TEAM_ALLIES : TEAM_AXIS,
               &flag) == qtrue)) {
        qboolean getflag = qtrue;
        //
        // check special circumstances in which we should not go for the flag
        if (bs->ainode == AINode_MP_MedicRevive ||
            bs->ainode == AINode_MP_MedicGiveHealth) {
          // if we are close to them, continue
          if (trap_InPVS(
                  bs->origin,
                  g_entities[bs->target_goal.entitynum].r.currentOrigin) &&
  ...
  // NOTE: check that we havent just spawned, since this can cause congestion at
  // the start
  if ((g_entities[bs->client].client->respawnTime < level.time - 12000) &&
      g_entities[bs->client].awaitingHelpTime > level.time + 200) {
    if (bs->enemy < 0 && bs->stand_time < trap_AAS_Time() &&
        !BotCarryingFlag(bs->client)) {
      AIEnter_MP_Stand(bs);
  ...
  if (BotClass_MedicCheckRevives(bs, 300, &target, qtrue)) {
    if (!BotCarryingFlag(bs->client) ||
        ((bs->enemy == -1) && (trap_AAS_AreaTravelTimeToGoalArea(
                                   bs->areanum, bs->origin,
                                   bs->target_goal.areanum, bs->tfl) > 600))) {
      if (target.entitynum == bs->target_goal.entitynum &&
          bs->ainode != AINode_MP_MedicRevive) {
        return qfalse;
      }
  ...
  if ((COM_BitCheck(bs->cur_ps.weapons, WP_PANZERFAUST))) {
    if ((BotWeaponWantScale(bs, WP_PANZERFAUST) > 0)) {
      ...
      while ((trav = BotFindNextStaticEntity(trav, BOTSTATICENTITY_MG42))) {
        ...
        trap_Trace(&tr, bs->eye, NULL, NULL, center, bs->client,
                   MASK_MISSILESHOT);
  ...
  // defense engineers should look for dynamite placed nearby objectives to
  // defuse
  {
    int list[10], numList;
    //
      ...
      for (i = 0; i < numList; i++) {
        ...
        if (bs->sess.playerType != PC_ENGINEER) {
          if (BotCanSnipe(bs, qtrue)) {
            if (level.explosiveTargets[bs->sess.sessionTeam == TEAM_AXIS ? 0
                                                                         : 1] !=
                    BotGetTargetExplosives(bs->sess.sessionTeam, NULL, 0,
                                           qfalse) ||
                BotNumTeamMatesWithTarget(bs, list[i], NULL, 0) >=
                    (int)(floor(0.3 * numTeammates / numList))) {
              continue;
            }
          }
        }

        // if this dynamite has been laid by our team, and we are an engineer,
        // no need to defend it
        if (bs->sess.playerType == PC_ENGINEER &&
            ((trav->s.teamNum % 4) == bs->sess.sessionTeam)) {
          continue;
        }

        // if (trav->missionLevel > level.time) continue;
        if (BotNumTeamMatesWithTarget(bs, trav->s.number, NULL, 0) >
            (int)floor((float)(numTeammates / numList))) {
          continue;
        }
        VectorAdd(trav->r.absmin, trav->r.absmax, center);
        VectorScale(center, 0.5, center);
        if ((areanum = BotPointAreaNum(trav->s.number, center))) {
          // if (areanum = BotReachableBBoxAreaNum( bs, trav->r.absmin,
          // trav->r.absmax )) {
          if (trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                                areanum, bs->tfl)) {
            // make this our goal
            ...
            VectorCopy(center, target.origin);
            //
            // if we are an engineer, and this is hostile dynamite, we should
            // defuse it, otherwise defend it
            if (bs->sess.playerType == PC_ENGINEER &&
                ((trav->s.teamNum % 4) != bs->sess.sessionTeam)) {
              if (BotGoalWithinMovementAutonomy(bs, &target, BGU_HIGH)) {
                // if we are already heading there, continue
              ...
              // only "defend" the dynamite if they are yet to break through any
              // of the defenses
            } else if ((BotNumTeamMatesWithTargetAndCloser(
                            bs, target.entitynum, target.areanum, NULL, 0, -1) <
                        (int)(ceil(0.3 * numTeammates / numList)))) {
              // if we are already heading there, continue
              if (bs->target_goal.entitynum &&
                  bs->target_goal.entitynum == trav->s.number &&
                  bs->ainode == AINode_MP_DefendTarget) {
...
int BotMP_FindGoal_BuildGoalList(bot_state_t* bs,
  ...
  if (leader != -1 && g_entities[leader].r.svFlags & SVF_BOT) {
    ...
    pGoals[goalNum].priority =
        g_entities[leader].goalPriority[bs->sess.sessionTeam - TEAM_AXIS];
    pGoals[goalNum].defaultPriority =
        botgoalPriorities_Class[bs->sess.playerType][pGoals[goalNum].type];
  ...
  trav = NULL;
  while ((
      trav = BotFindNextStaticEntity(trav, BOTSTATICENTITY_TRIGGER_MULTIPLE))) {
    // is it disabled?
    ...
    if (!trav->r.linked || (trav->nextthink > level.time + 1000)) {
      continue;  // if it's thinking, then it's not active (doesnt respond to
                 // touch)
    }
    ...
    pGoals[goalNum].priority =
        trav->goalPriority[bs->sess.sessionTeam - TEAM_AXIS];
    pGoals[goalNum].defaultPriority =
        botgoalPriorities_Class[bs->sess.playerType][pGoals[goalNum].type];
  ...
  if (bs->sess.playerType == PC_ENGINEER) {
    ...
    trav = NULL;
    while ((
        trav = BotFindNextStaticEntity(trav, BOTSTATICENTITY_OBJECTIVE_INFO))) {
      // is it disabled?
      if (BFG_CHECKDISABLED(trav, bs)) {
        continue;
      }
    ...
    if (BotWeaponCharged(bs, WP_DYNAMITE)) {
      numTargets =
          BotGetTargetExplosives(bs->sess.sessionTeam, tlist, 10, qfalse);
      for (k = 0; k < numTargets; k++) {
        ...
        if (ent->s.eType == ET_OID_TRIGGER) {
          ...
          if (ent->target_ent) {
            pGoals[goalNum].priority =
                ent->target_ent->goalPriority[bs->sess.sessionTeam - TEAM_AXIS];
          } else {
            // Gordon: this shouldn't happen really....
            pGoals[goalNum].priority =
                ent->goalPriority[bs->sess.sessionTeam - TEAM_AXIS];
          }
        } else {
        ...
        if (trav->melee) {
          ...
          if ((ent->botIgnoreTime < level.time)) {
            ...
            pGoals[goalNum].priority =
                trav->goalPriority[bs->sess.sessionTeam - TEAM_AXIS];
            pGoals[goalNum].defaultPriority =
                botgoalPriorities_Class[bs->sess.playerType]
                                       [pGoals[goalNum].type];
    ...
    if (G_CountTeamLandmines(bs->sess.sessionTeam) < MAX_TEAM_LANDMINES) {
      ...
      while ((trav = BotFindNextStaticEntity(
        ...
        if ((trav->spawnflags & 1) && (bs->sess.sessionTeam != TEAM_AXIS)) {
          continue;
        } else if ((trav->spawnflags & 2) &&
                   (bs->sess.sessionTeam != TEAM_ALLIES)) {
          continue;
        }
  ...
  trav = NULL;
  while (
      (trav = BotFindNextStaticEntity(trav, BOTSTATICENTITY_BOT_ATTRACTOR))) {
    if (BFG_CHECKDISABLED(trav, bs)) {
      continue;
    }
  ...
  if (BotCanSnipe(bs, qtrue)) {
    ...
    if (c != -1) {
      ...
      pGoals[goalNum].priority =
          g_entities[c].goalPriority[bs->sess.sessionTeam - TEAM_AXIS];
      pGoals[goalNum].defaultPriority =
          botgoalPriorities_Class[bs->sess.playerType][pGoals[goalNum].type];
  ...
  if (bs->sess.playerType == PC_COVERTOPS) {
    ...
    numTargets =
        BotGetTargetsForSatchelCharge(bs->sess.sessionTeam, tlist, 10, qfalse);
    for (k = 0; k < numTargets; k++) {
      // see if this is a worthy goal
      ent = &g_entities[tlist[k]];
...
int QDECL BotMP_FindGoals_Sort_Standard(const void* a, const void* b) {
  ...
  if (botgoalPriorities_Standard[bg1->type] >
      botgoalPriorities_Standard[bg2->type]) {
    return -1;
  } else if (botgoalPriorities_Standard[bg2->type] >
             botgoalPriorities_Standard[bg1->type]) {
    return 1;
  }
...
int QDECL BotMP_FindGoals_Sort_Engineer(const void* a, const void* b) {
  ...
  if (botgoalPriorities_Engineer[bg1->type] >
      botgoalPriorities_Engineer[bg2->type]) {
    return -1;
  } else if (botgoalPriorities_Engineer[bg2->type] >
             botgoalPriorities_Engineer[bg1->type]) {
    return 1;
  }
...
int QDECL BotMP_FindGoals_Sort_CovertOps(const void* a, const void* b) {
  ...
  if (botgoalPriorities_CovertOps[bg1->type] >
      botgoalPriorities_CovertOps[bg2->type]) {
    return -1;
  } else if (botgoalPriorities_CovertOps[bg2->type] >
             botgoalPriorities_CovertOps[bg1->type]) {
    return 1;
  }
...
#define BFG_RETURN_FAILURE_AND_IGNORE \
  bg->ignore = qtrue;                 \
  BFG_RETURN_FAILURE
...
botMPpg_t BotMP_FindGoal_ProcessGoal(bot_state_t* bs,
                                     botgoalFind_t* bg,
  ...
  switch (bg->type) {
    ...
    case BFG_CONSTRUCT:
      ...
      if (bs->target_goal.entitynum == BFG_GOAL_ENTITYNUM &&
          bs->ainode == AINode_MP_ConstructibleTarget) {
        BFG_RETURN_ALREADY;  // already going for it
      }

      // Gordon: if this is a new goal, check pliers are half-full before
      // heading off (optimist ;))
      if (bs->cur_ps.classWeaponTime >
          (level.time -
           (level.engineerChargeTime[bs->sess.sessionTeam - 1] * 0.5f))) {
        BFG_RETURN_FAILURE_AND_IGNORE;
      }
    ...
    case BFG_TRIGGER:
      ...
      // is it for us only?
      if (((bg->ent->spawnflags & 1) && (bs->sess.sessionTeam == TEAM_AXIS)) ||
          ((bg->ent->spawnflags & 2) &&
           (bs->sess.sessionTeam == TEAM_ALLIES))) {
        if (bs->target_goal.entitynum == target_goal->entitynum &&
            bs->ainode == AINode_MP_TouchTarget) {
    ...
    case BFG_DESTRUCTION_EXPLOSIVE:
      ...
      // find the best goal area
      numList =
          trap_AAS_BBoxAreas(bg->ent->r.absmin, bg->ent->r.absmax, list, 32);
      if (numList) {
        int oldestTime = -1;
        int bestDist = -1;
        int oldest = 0;
        ...
        if (oldestTime > 0) {
          ...
          t = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, oldest,
                                                bs->tfl);
          if (t) {
    ...
    case BFG_DESTRUCTION_SATCHEL:
      ...
      if (numList) {
        ...
        for (i = 0; i < numList; i++) {
          ...
          if ((VectorLengthSquared(vec) <= SQR(G_GetWeaponDamage(weap))) &&
              CanDamage(bg->ent, center)) {
            t = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                                  list[i], bs->tfl);
            if (t) {
              oldestTime = t;
    ...
    case BFG_MINE:
      ...
      do {
        ...
        VectorCopy(vec, target_goal->origin);
               ...
               ((target_goal->areanum < 0) ||
                !PointInBounds(vec, bg->ent->r.absmin, bg->ent->r.absmax)));
    ...
    case BFG_ATTRACTOR:
      if (BotGoalForEntity(bs, BFG_GOAL_ENTITYNUM, target_goal, BGU_MAXIMUM)) {
        // get the distance, halve it if there is noone one going for this
        // attractor
        t = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                              target_goal->areanum, bs->tfl);
        if (t) {
...
void BotMP_FindGoal_PostProcessGoal(bot_state_t* bs,
                                    botgoalFind_t* bg,
                                    bot_goal_t* goal) {
  bs->target_goal = *goal;
...
qboolean BotMP_AlreadyDoing_FastOut(bot_state_t* bs, botgoalFind_t* bg) {
  // return qtrue if the given goal, which we are already doing, is ok to
  // continue doing, without looking for other goals
...
/*		if(bot_profile.integer == 1) {
                        ...
                        G_Printf( "Findgoal: %s: %i\n", BG_ClassnameForNumber(
   bs->sess.playerType ), t );
                }*/
#endif  // _DEBUG
    return qfalse;
  }
  ...
  qsort(findGoals, numGoals, sizeof(botgoalFind_t),
        botmp_sortFuncs[bs->cur_ps.stats[STAT_PLAYER_CLASS]]);
...
/*		if(bot_profile.integer == 1) {
    ...
    switch (res) {
      case BPG_FAILED:
        continue;
      case BPG_ALREADYDOING:  // we should check for range also if we are
                              // already doing it, otherwise they may all get
                              // stuck at the same goal
...
/*					if(bot_profile.integer == 1) {
   ...
   BG_ClassnameForNumber( bs->sess.playerType ), t );
      ...
      case BPG_NEWGOAL:
        // Possible optimization here: record the travel time within each bot's
        // AI node, so we can use that time inside
        // BotNumTeamMatesWithTargetAndCloser() rather than calculating the time
        // for all bots
        findGoals[i].numPlayers = numCloser =
            BotNumTeamMatesWithTargetAndCloser(
                bs, goal.entitynum, goal.areanum, NULL, 0,
        ...
        if (bestGoal_numCloser >= 0) {
          ...
          if (numCloser == bestGoal_numCloser) {
            ...
            if (findGoals[i].defaultPriority <
                findGoals[bestGoal_InList].defaultPriority) {
              continue;
            }
            ...
            if ((findGoals[i].defaultPriority ==
                 findGoals[bestGoal_InList].defaultPriority) &&
                (findGoals[i].priority <=
                 findGoals[bestGoal_InList].priority)) {
              continue;
            }
          }
        }
...
/*			if(bot_profile.integer == 1) {
  ...
  if (bestGoal_numCloser >= 0) {
    switch (bestGoal_result) {
      case BPG_NEWGOAL:
        BotMP_FindGoal_PostProcessGoal(bs, &findGoals[bestGoal_InList],
                                       &bestGoal);
        return qtrue;
...
qboolean BotMP_FindGoal(bot_state_t* bs) {
  ...
  if (bs->leader > -1) {
    // we have a leader, follow them
    if (BotGoalForEntity(bs, bs->leader, &secondary, BGU_LOW) ||
        !(g_entities[bs->leader].r.svFlags & SVF_BOT)) {
      secondary.flags = GFL_LEADER;

      t = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                            secondary.areanum, bs->tfl);
  ...
  if (bs->sess.playerType == PC_ENGINEER) {
    ...
    for (pass = 0; pass < 2; pass++) {
      ...
      while ((trav = BotFindNextStaticEntity(trav,
        ...
        if ((pass == 0) && BotNumTeamMatesWithTargetByClass(
                               bs, trav->s.number, NULL, 0, PC_ENGINEER)) {
        ...
        // we can build it
        //
        // if we are already heading for this goal, stay with it, otherwise look
        // only for closest targets
        //
        if (bs->target_goal.entitynum == trav->s.number &&
            bs->ainode == AINode_MP_ConstructibleTarget) {
          return qfalse;
        ...
        // if (bs->cur_ps.classWeaponTime + t*10 < (level.time -
        // level.lieutenantChargeTime[bs->sess.sessionTeam-1])) continue;
        //
        bestTarget = target;
        closestTime = t;
      }
...

  {
    ...
    while ((trav = BotFindNextStaticEntity(trav,
      ...
      targetPriority = 2;
      //
      // if there are too many going for this goal, then ignore it if we are the
      // furthest away
      if ((numList = BotNumTeamMatesWithTargetAndCloser(
               bs, trav->s.number, target.areanum, list, MAX_CLIENTS, -1)) >
          (int)floor(0.3 * numTeammates)) {
        distances = BotSortPlayersByTraveltime(BotGetArea(trav->s.number), list,
                                               numList);
    ...
    if (closestTime) {
      ...
      if (((trav->spawnflags & 1) && (bs->sess.sessionTeam == TEAM_AXIS)) ||
        ...
        if (bs->target_goal.entitynum == bestTarget.entitynum &&
            bs->ainode == AINode_MP_TouchTarget) {
          return qfalse;
        }
        bs->target_goal = bestTarget;
  ...
  if (bs->sess.playerType == PC_ENGINEER) {
    ...
    for (pass = 0; pass < 2; pass++) {
      ...
      while ((trav = BotFindNextStaticEntity(trav,
        ...
        if (BotNumTeamMatesWithTargetByClass(bs, trav->s.number, NULL, 0,
                                             PC_ENGINEER) > pass) {
          continue;  // someone else is going for it
        }
        if (!BotGetReachableEntityArea(bs, trav->s.number, &target)) {
  ...
  if (bs->sess.playerType == PC_ENGINEER) {
    ...
    if (numTargets) {
      ...
      for (k = 0; k < numTargets; k++) {
        ...
        // is it disabled?
        // Gordon: forest script was setup to use the entity itself, NOT the toi
        // (toi for building, entity itself for destruction)
        //				if (ent->aiInactive &
        //(1<<bs->sess.sessionTeam)) continue; 				if (ent->parent &&
        //(ent->parent->aiInactive & (1<<bs->sess.sessionTeam))) continue;
        // is the target disabled?
        //				if (ent->target_ent &&
        //(ent->target_ent->aiInactive & (1<<bs->sess.sessionTeam))) continue;
        //
        ...
        if (ent->s.eType == ET_OID_TRIGGER) {
          ...
          if (numList) {
            ...
            if (oldestTime > 0) {
              // now trace from this area towards the func_explosive
              trap_AAS_AreaCenter(oldest, center);
              // trap_Trace( &tr, center, bs->cur_ps.mins, bs->cur_ps.maxs,
              // brushPos, -1, MASK_PLAYERSOLID & ~CONTENTS_BODY ); VectorCopy(
              // tr.endpos, center );
              VectorCopy(center, end);
              end[2] -= 512;
              trap_Trace(&tr, center, bs->cur_ps.mins, bs->cur_ps.maxs, end, -1,
          ...
          if (numList) {
            ...
            for (i = 0; i < numList; i++) {
              ...
              if ((VectorLengthSquared(vec) <=
                   SQR(G_GetWeaponDamage(WP_DYNAMITE))) &&
  ...
  if (bs->sess.playerType == PC_ENGINEER) {
    ...
    while ((trav = BotFindNextStaticEntity(trav, BOTSTATICENTITY_MG42))) {
      ...
      if (trav->health <= 0) {
        if (trav->melee) {
          ent = trav->melee;
          if ((ent->aiTeam == bs->sess.sessionTeam) &&
              (ent->botIgnoreTime < level.time) &&
              (BotNumTeamMatesWithTargetByClass(bs, trav->s.number, NULL, -1,
                                                PC_ENGINEER) == 0)) {
            // go fix it?
  ...
  // PLANT LAND MINES
  if (bs->sess.playerType == PC_ENGINEER &&
      (G_CountTeamLandmines(bs->sess.sessionTeam) < MAX_TEAM_LANDMINES)) {
    // look for a land mine area that isn't full
    closestTime = 0;
    trav = NULL;
    while ((trav = BotFindNextStaticEntity(
      ...
      // FIXME: djbob needs to add the etype of the landmine hint to the filter
      // is it for us?
      if ((trav->spawnflags & 1) && (bs->sess.sessionTeam != TEAM_AXIS)) {
        continue;
      }
    ...
    if (closestTime) {
      ...
      do {
        ...
        VectorCopy(vec, bestTarget.origin);
               ...
               ((bestTarget.areanum < 0) ||
                !PointInBounds(vec, trav->r.absmin, trav->r.absmax)));
      //
      if (i < 5) {  // we found a valid spot
        bs->target_goal = bestTarget;
        AIEnter_MP_PlantMine(bs);
    ...
    if (closestTime) {
      ...
      if (((trav->spawnflags & 1) && (bs->sess.sessionTeam == TEAM_AXIS)) ||
        ...
        if (targetPriority > 1) {
          ...
          AIEnter_MP_TouchTarget(bs);
          return qtrue;
        } else {  // enough people going for it, so only go for it if there's
                  // nothing else to do
          targetTime = closestTime;
          gotTarget = -1;
          defendTarget = qfalse;
  ...
  if (!gotTarget) {
    // Defend checkpoints if they are owned by us. Make sure we disperse troops
    // amongst checkpoints, and actively pursue checkpoints not owned by us more
    // aggressively.
    ...
    while ((trav = BotFindNextStaticEntity(trav, BOTSTATICENTITY_CHECKPOINT))) {
      ...
      if (trav->count ==
        ...
        if (BotNumTeamMatesWithTarget(bs, trav->s.number, NULL, 0) < c) {
          ...
          VectorScale(center, 0.5, center);
          secondary.areanum =
              BotReachableBBoxAreaNum(bs, trav->r.absmin, trav->r.absmax);
          ...
          if (t && (!gotTarget || t < targetTime)) {
            ...
            bestTarget = secondary;
            if (t < 500 &&
                BotNumTeamMatesWithTarget(bs, trav->s.number, NULL, 0) < 1) {
        ...
        if (!level.captureFlagMode &&
            BotNumTeamMatesWithTarget(bs, trav->s.number, NULL, 0) <
                (c > 1 ? c / 2 : c)) {
          // GO GET IT
          // do we have a route to the enemy flag?
          secondary.entitynum = trav->s.number;
  ...
  /*	if (gotTarget != qtrue) {
     ...
     continue;
                          //
                          // only guard objectives that are attached to
     constructibles or destructibles
                          //
                          useObjective = qfalse;
                          //
                          if (trav->target_ent) {
                                  if (trav->target_ent->s.eType ==
     ET_CONSTRUCTIBLE) {
                                          // has it been built?
                                          // all constructibles are considered
     important, whether built or not, unless they are disabled in scripting
                                          //if (BotIsConstructible(
     bs->sess.sessionTeam, trav->s.number )) { useObjective = qtrue;
                                          //}
                                  } else if (trav->target_ent->s.eType ==
     ET_EXPLOSIVE) {
                                          // is it still valid?
                          ...
                          //
                          i++;
                          if ((k = BotNumTeamMatesWithTarget( bs,
     trav->s.number, NULL, 0 )) < c) { if (!BotGetReachableEntityArea( bs,
     trav->s.number, &secondary )) continue;	// not reachable c = k; oldest =
     trav->s.number;
                          }
                  }
                  if (i) {
     ...
     continue;
                          //
                          if (BotNumTeamMatesWithTarget( bs, trav->s.number,
     NULL, 0 ) >= (int)ceil(f*numTeammates)) { if (bestPriority >= 2) continue;
     // we already have an important goal targetPriority = 1; } else {
                                  targetPriority = 2;
                          }
                          //
                          // only guard objectives that are attached to
     ...
     ET_EXPLOSIVE) {
                                  ...
                                  //
                                  if (!useObjective) continue;	// ignore this
     objective
                                  //
                                  // GO GET IT
                                  // do we have a route to the objective?
                                  if (!BotGetReachableEntityArea( bs,
     trav->s.number, &secondary )) continue;	// not reachable
                                  //
                                  t =
     trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
     secondary.areanum, bs->tfl); if (t) { if (!gotTarget || (targetPriority >
     bestPriority) || (oldest != -1 ? trav->s.number == oldest : t <
     targetTime)) { if (BotGoalWithinMovementAutonomy( bs, &secondary, BGU_LOW
     )) { targetTime = t; gotTarget = qtrue; defendTarget = qtrue; bestTarget =
     secondary; bestPriority = targetPriority;
                                                  }
                                          }
                                  }
                          }
    ...
    if (bs->sess.sessionTeam == level.attackingTeam) {
      ...
      if (BotCarryingFlag(bs->client)) {  // we have it!!
        ...
        if (ent) {
          ...
          if (bestDist > 0) {
            if (bs->target_goal.entitynum == ent->s.number &&
                bs->ainode == AINode_MP_TouchTarget &&
            ...
            if (BotGoalWithinMovementAutonomy(bs, &target, BGU_LOW)) {
              ...
              bs->target_goal = target;
              //
              // if we are close to the static flag, then look for an alternate
              // route
              BotFlagAtBase(
                  level.attackingTeam == TEAM_AXIS ? TEAM_ALLIES : TEAM_AXIS,
                  &ent);
              if (ent && VectorDistanceSquared(
                             bs->origin, ent->r.currentOrigin) < (384 * 384)) {
                // check for an alternate route
                if ((t > 1000) && (numList = trap_AAS_AlternativeRouteGoals(
                                       bs->origin, bestAreaCenter, bs->tfl,
                                       altroutegoals, 40, 0))) {
                  // pick one at random
                  i = rand() % numList;
      ...
      if (BotFlagAtBase(
        ...
        t = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                              target.areanum, bs->tfl);
        if (t && ((t < 500) || !gotTarget || defendTarget ||
                  ((t / 2) < targetTime))) {
          //
          if (BotGoalWithinMovementAutonomy(bs, &target, BGU_MEDIUM)) {
            //
            ...
            //
            // check for an alternate route
            if (t > 600 && (numList = trap_AAS_AlternativeRouteGoals(
                                bs->origin, bs->target_goal.origin, bs->tfl,
                                altroutegoals, 40, 0))) {
              // pick one at random
              i = rand() % numList;
              // make this the altrouetgoal
        ...
        if ( ( target.entitynum > -1 && target.entitynum != bs->client ) &&
          ...
          // TODO: if there are too many defenders, and we are the furthest
          // away, stop defending
          // !!!
          if ((numList = BotNumTeamMatesWithTarget(bs, target.entitynum, list,
                                                   MAX_CLIENTS)) >=
              BOT_FLAG_CARRIER_DEFENDERS) {
            ourDist = VectorDistanceSquared(
                bs->origin, g_entities[target.entitynum].r.currentOrigin);
            // if (!trap_InPVS( bs->origin,
            // g_entities[target.entitynum].r.currentOrigin )) ourDist += 2048;
            distances = BotSortPlayersByDistance(
                g_entities[target.entitynum].r.currentOrigin, list, numList);
        ...
        if (target.entitynum != bs->client && ent &&
            (BotNumTeamMatesWithTarget(bs, ent->s.number, NULL, 0) <
             (int)ceil(0.3 * numTeammates))) {
          VectorAdd(ent->r.absmin, ent->r.absmax, brushPos);
          VectorScale(brushPos, 0.5, brushPos);
          // find the best goal area
          numList = trap_AAS_BBoxAreas(ent->r.absmin, ent->r.absmax, list, 32);
        ...
        BotFlagAtBase(
            level.attackingTeam == TEAM_AXIS ? TEAM_ALLIES : TEAM_AXIS, &ent);
        // do we have a route to the enemy flag?
        BotClearGoal(&target);
        target.entitynum = ent->s.number;
        VectorCopy(ent->r.currentOrigin, center);
      ...
      // note: mg42 only operate if flag is at base
      if ((bs->sess.playerType == PC_SOLDIER) &&
          (BotFlagAtBase(bs->sess.sessionTeam, &ent) == qtrue)) {
        // already sniping
        if (bs->ainode == AINode_MP_MG42Mount ||
            bs->ainode == AINode_MP_MG42Scan) {
          return qfalse;
        }
        //
      ...
      if (!gotTarget ||
          ...
          (level.explosiveTargets[bs->sess.sessionTeam == TEAM_AXIS ? 0 : 1] -
               BotGetTargetExplosives(bs->sess.sessionTeam, NULL, 0, qtrue) >
           0)) {
        ...
        if (BotFlagAtBase(bs->sess.sessionTeam, &ent) == qtrue) {
            ...
            if (t && (!gotTarget || defendTarget || t < targetTime)) {
              if (BotGoalWithinMovementAutonomy(bs, &target, BGU_MEDIUM)) {
                ...
                if (BotGoalWithinMovementAutonomy(bs, &target, BGU_MEDIUM)) {
                  ...
                  // we are on defense
                  BotVoiceChatAfterIdleTime(bs->client, "OnDefense", SAY_TEAM,
                                            1000, qfalse, 10000 + rand() % 5000,
                                            qfalse);
                  bs->target_goal = target;
                  AIEnter_MP_DefendTarget(bs);
                  return qtrue;
          ...
          ent = BotFindNextStaticEntity(NULL, BOTSTATICENTITY_FLAGONLY);
          if (!ent) {
            ent = BotFindNextStaticEntity(NULL,
          ...
          if ( ent /*&& (BotNumTeamMatesWithTarget( bs, ent->s.number, NULL, 0 ) < (int)ceil(0.5*numTeammates))*/ ) {
            ...
            // find the best goal area
            numList =
                trap_AAS_BBoxAreas(ent->r.absmin, ent->r.absmax, list, 32);
            if (numList) {
              oldestTime = -1;
              for (i = 0; i < numList; i++) {
                if (!trap_AAS_AreaReachability(list[i])) {
