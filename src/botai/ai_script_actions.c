...
qboolean Bot_ScriptAction_SetSpeed(bot_state_t* bs, char* params) {
  ...
  VectorAdd(g_entities[bs->entitynum].client->ps.velocity, speed,
            g_entities[bs->entitynum].client->ps.velocity);
...
void Bot_ScriptError(bot_state_t* bs, char* fmt, ...) {
  ...
  //
  G_Error("BotScript (line %i): %s", bs->script.status.currentItem->lineNum,
          text);
  //
  va_end(ap);
}
...
Bot_ScriptAction_SetAccumToPlayerCount
  ...
  syntax: SetAccumToPlayerCount <accum_index> [[<condition> <value>]
[<condition> <value>] ...]

  condition list: team [axis/allies], class [medic/engineer/etc], weapon
[flamethrower/sniperrifle/etc], within_range <targetname> <distance>
=================
*/
qboolean Bot_ScriptAction_SetAccumToPlayerCount(bot_state_t* bs, char* params) {
  ...
  if (!token || !token[0] || (token[0] < '0') || (token[0] > '9')) {
    Bot_ScriptError(
        bs, "accum buffer index expected, %s found: SetAccumToPlayerCount %s",
        token, params);
  }
  accum = atoi(token);
  if (accum < 0 || accum >= MAX_SCRIPT_ACCUM_BUFFERS) {
    Bot_ScriptError(bs,
                    "accum buffer index out of range, %s found (range is 0 - "
                    "%i): SetAccumToPlayerCount %s",
  ...
  while (qtrue) {
    ...
    if (token[0] != '/') {
      Bot_ScriptError(
          bs,
          "condition identifier expected, %s found: SetAccumToPlayerCount %s",
          token, params);
    }
    //
    if (!Q_stricmp(token, "/team")) {
      ...
      if (!token || !token[0] || token[0] == '/') {
        Bot_ScriptError(
            bs, "unexpected end of command: SetAccumToPlayerCount %s", params);
      }
      //
      if (!Q_stricmp(token, "axis")) {
        ...
        Bot_ScriptError(bs, "unknown team \"%s\": SetAccumToPlayerCount %s",
                        token, params);
      }
      // eliminate players
      for (i = 0; i < level.maxclients; i++) {
      ...
      if (!Q_stricmp(token, "/class")) {
        ...
        if (!token || !token[0] || token[0] == '/') {
          Bot_ScriptError(bs,
                          "unexpected end of command: SetAccumToPlayerCount %s",
                          params);
        }
        //
        val = Team_ClassForString(token);
        if (val < 0) {
          Bot_ScriptError(bs, "unknown class \"%s\": SetAccumToPlayerCount %s",
                          token, params);
        }
        // eliminate players
        for (i = 0; i < level.maxclients; i++) {
        ...
        if (!Q_stricmp(token, "/weapon")) {
          ...
          while (qtrue) {
            ...
            if (!token || !token[0] || token[0] == '/') {
              Bot_ScriptError(
                  bs, "unexpected end of command: SetAccumToPlayerCount %s",
                  params);
            }
            //
            if ((item = BG_FindItem(token))) {
              if (!item->giTag) {
                Bot_ScriptError(
                    bs, "unknown weapon \"%s\": SetAccumToPlayerCount %s",
                    token, params);
              }
              COM_BitSet(weapons, item->giTag);
            } else {
              Bot_ScriptError(bs,
                              "unknown weapon \"%s\": SetAccumToPlayerCount %s",
                              token, params);
            }
            //
            pStrBackup = pStr;
          ...
          for (i = 0; i < level.maxclients; i++) {
            ...
            if (!(g_entities[i].client->ps.weapons[0] & weapons[0]) &&
          ...
          if (!Q_stricmp(token, "/within_range")) {
            ...
            if (!ent) {
              Bot_ScriptError(
                  bs, "unknown spawn point \"%s\": SetAccumToPlayerCount %s",
                  token, params);
            }
            // range
            token = COM_ParseExt(&pStr, qfalse);
            if (!token || !token[0] || token[0] == '/') {
              Bot_ScriptError(
                  bs, "range expected, not found: SetAccumToPlayerCount %s",
                  params);
            }
            //
            // eliminate players
            for (i = 0; i < level.maxclients; i++) {
              ...
              if (VectorDistanceSquared(g_entities[i].r.currentOrigin,
                                        ent->s.origin) > SQR(atof(token))) {
                validPlayers[i] = 0;
                count--;
              }
            }
...
qboolean Bot_ScriptAction_Accum(bot_state_t* bs, char* params) {
  ...
  if (bufferIndex >= MAX_SCRIPT_ACCUM_BUFFERS) {
    Bot_ScriptError(bs, "accum: buffer is outside range (0 - %i)",
  ...
  if (!Q_stricmp(lastToken, "inc")) {
    ...
    if (bs->script.accumBuffer[bufferIndex] < atoi(token)) {
      ...
      bs->script.status.stackHead =
          bs->script.data->events[bs->script.status.eventIndex].stack.numItems;
    }
    ...
    if (bs->script.accumBuffer[bufferIndex] > atoi(token)) {
      ...
      bs->script.status.stackHead =
          bs->script.data->events[bs->script.status.eventIndex].stack.numItems;
             ...
             !Q_stricmp(lastToken, "abort_if_not_equals")) {
    if (!token[0]) {
      Bot_ScriptError(bs, "accum: %s requires a parameter", lastToken);
    }
    if (bs->script.accumBuffer[bufferIndex] != atoi(token)) {
...
qboolean Bot_ScriptAction_MoveToMarker(bot_state_t* bs, char* params) {
  ...
  //		Why did we need it?
  // Gordon: 27/11/02: dont want to teleport dead players, can move further
  // down, but i see no reason a general early out wouldn't be as good why does
  // it crash?
  /*
  // Gordon: 6/11/02
  ...
  */
  ...
  // TAT 8/20/2002
  //		If we have received a player command that overrides our movement
  //scripts, 		we should end the script
  ...
  while ((token = COM_ParseExt(&pString, qfalse)) && token[0]) {
    if (!Q_stricmp(token, "/WALKING")) {
      ...
      TeleportPlayer(&g_entities[bs->entitynum], target->origin,
                     g_entities[bs->entitynum].client->ps.viewangles);
      return qtrue;
      // Gordon: end
    }
  }
  // START	Mad Doctor I changes, 8/19/2002
  // The check was too constrictive and failed too often.  I may have made it
  // too loose, but it seems to work. if we have passed or are close enough to
  // the marker, then return qtrue Gordon: added custom radius ability, default
  // is 24 as Ian had set here
  if (VectorDistanceSquared(bs->origin, target->origin) < SQR(radius)) {
    ...
    return qtrue;
  } else if ((bs->script.status.stackChangeTime < level.time - 500) &&
...
qboolean Bot_ScriptAction_Trigger(bot_state_t* bs, char* params) {
  ...
  // Changes to fix bugs caused if you used "trigger Foo FooAction" within bot
  // Foo's scripts instead of using "trigger self FooAction".
  if ((!Q_stricmp(name, "self")) || (!Q_stricmp(name, ent->scriptName))) {
    ...
    for (i = 0; i < level.num_entities; i++, trent++) {
      ...
      if (!(trent->r.svFlags & SVF_BOT)) {
        ...
        oldId = bs->script.status.id;
        Bot_ScriptEvent(bs->client, "trigger", trigger);
        // if the script changed, return false so we don't muck with it's
        // variables
    ...
    // Did we find a bot?
    if (found) {
      // We found one, and triggered the action, so keep going on with our
      // script
      return qtrue;
...
qboolean Bot_ScriptAction_Logging(bot_state_t* bs, char* params) {
  ...
  if (!Q_stricmp(params, "ON")) {
    ...
    Q_strncpyz(
        filename,
        va("BotLog_%s_[%i]_[%4i_%2i_%2i]_[%2i_%2i_%2i].txt",
           g_entities[bs->client].aiName, bs->client, 1900 + localTime->tm_year,
           1 + localTime->tm_mon, 1 + localTime->tm_mday, localTime->tm_hour,
    ...
    Bot_ScriptError(bs, "Logging has unknown parameter (%s), expected ON/OFF",
                    params);
  }
...
char* botAttributeStrings[] = {
    ...
    "BOT_REACTION_TIME", "BOT_AIM_ACCURACY",

    "BOT_WIMP_FACTOR", NULL};
...
qboolean Bot_ScriptAction_MountMG42(bot_state_t* bs, char* params) {
  ...
  if (!mg42Spot) {
    Bot_ScriptError(
        bs, "MountMG42: (internal error) mg42 (\"%s\") has invalid mg42_spot",
        params);
  }
  //
...
qboolean Bot_ScriptAction_SetWeapon(bot_state_t* bs, char* params) {
  ...
  weapon = Bot_GetWeaponForClassAndTeam(
      playerClass, g_entities[bs->client].client->sess.sessionTeam, params);
  if (weapon == -1) {
    // can't use this weapon
    Bot_ScriptError(
        bs, "Bot %s on team %s cannot use weapon %s\n",
        g_entities[bs->client].aiName,
        (g_entities[bs->client].client->sess.sessionTeam == TEAM_AXIS)
            ? "Axis"
            : "Allies",
        params);
  }
  ...
  if (weapon != WP_NONE) {
    //		AddWeaponToPlayer( g_entities[bs->client].client, weapon, 1, 0,
    //qtrue );
...
qboolean Bot_ScriptAction_SetClass(bot_state_t* bs, char* params) {
  ...
  if (!Q_stricmp(params, "ANY")) {
    ...
    val = PC_ENGINEER;
                 ...
                 params,
                 "lieutenant")) {  // FIXME: remove this from missionpack? once
...
qboolean Bot_ScriptAction_GlobalAccum(bot_state_t* bs, char* params) {
  ...
  bufferIndex = atoi(token);
  if (bufferIndex >= MAX_SCRIPT_ACCUM_BUFFERS) {
    Bot_ScriptError(bs, "globalAccum: buffer is outside range (0 - %i)",
                    MAX_SCRIPT_ACCUM_BUFFERS);
  }
...
qboolean Bot_ScriptAction_Cvar(bot_state_t* bs, char* params) {
  ...
  if (!Q_stricmp(lastToken, "inc")) {
    ...
    if (cvarValue == atoi(token)) {
      ...
      //			Bot_ScriptError( bs, "trigger has unknown name:
      //%s\n", name );
...
qboolean Bot_ScriptAction_SetMovementAutonomy(bot_state_t* bs, char* params) {
  ...
  if (mlevel < 0) {
    Bot_ScriptError(bs, "SetMovementAutonomy: unknown parameter \"%s\"",
                    params);
  }
  ...
  //
  if (bs->leader < 0) {
    VectorCopy(level.clients[bs->client].ps.origin,
               bs->script.movementAutonomyPos);
...
qboolean Bot_ScriptAction_MovementAutonomy(bot_state_t* bs, char* params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  operand =
      va("%s", token);  // RF, this is a cheap and nasty way of saving memory
  //
  if (!operand[0]) {
  ...
  if (maLevel < 0) {
    Bot_ScriptError(bs, "SetMovementAutonomy: unknown movementAutonomy \"%s\"",
                    params);
  }
  //
  // apply the function
...
qboolean Bot_ScriptAction_PrintAccum(bot_state_t* bs, char* params) {
  ...
  if ((bufferIndex < 0) || (bufferIndex >= MAX_SCRIPT_ACCUM_BUFFERS)) {
    G_Error("Bot_ScriptAction_PrintAccum: buffer is outside range (0 - %i)",
            MAX_SCRIPT_ACCUM_BUFFERS);
  }

  ent = BotGetEntity(bs->client);

  G_Printf("(BotScript)  %s: Accum[%i] = %d\n", ent->scriptName, bufferIndex,
           ent->scriptAccumBuffer[bufferIndex]);
...
qboolean Bot_ScriptAction_PrintGlobalAccum(gentity_t* ent, char* params) {
  ...
  if (!params || !params[0]) {
    G_Error(
        "Bot_ScriptAction_PrintGlobalAccum: syntax: PrintGlobalAccum "
        "<globalAccumNumber>\n");
  }
  ...
  G_Printf("(BotScript) GlobalAccum[%i] = %d\n", bufferIndex,
           level.globalAccumBuffer[bufferIndex]);
...
Bot_ScriptAction_SetFireRate
   ...
   Sets the fire rate. fire rate 1 is normal (continuous) firing. fire rate 0
means do not fire at all. fire rate in between makes the bot fire a few bullets,
wait a little bit, and then fire a few more bullets.
=======================
*/
...
Bot_ScriptAction_SetFireCycleTime
   ...
   Sets the minimum/maximum fire cycle time. Actual fire cycle time is a random
number between minimum cycle time and maximum cycle time.

   during a fire cycle, the bot will fire for a duration equal to fireRate *
cycleTime, and hold fire for the rest of the cycle (which is equal to
(1-fireRate)*cycleTime.)
...
qboolean Bot_ScriptAction_SetFireCycleTime(bot_state_t* bs, char* params) {
  ...
  if (!params || !params[0]) {
    G_Error(
        "Bot_ScriptAction_SetFireCycleTime: syntax: SetFireCycleTime <minimum "
        "time in msec> <maximum time in msec>\n");
  }
...
qboolean Bot_ScriptAction_SetVisionRange(bot_state_t* bs, char* params) {
  ...
  if (!params || !params[0]) {
    G_Error(
        "Bot_ScriptAction_SetVisionRange: syntax: SetVisionRange <range>\n");
  }
...
qboolean Bot_ScriptAction_SetFarSeeingRange(bot_state_t* bs, char* params) {
  ...
  if (!params || !params[0]) {
    G_Error(
        "Bot_ScriptAction_SetFarSeeingRange: syntax: SetFarSeeingRange "
        "<range>\n");
  }
...
qboolean Bot_ScriptAction_SetCloseHearingRange(bot_state_t* bs, char* params) {
  ...
  if (!params || !params[0]) {
    G_Error(
        "Bot_ScriptAction_SetCloseHearingRange: syntax: SetCloseHearingRange "
        "<range>\n");
  }
...
// TAT 2/4/2003 - just force an update of current bot selection - used when who
// is selectable has changed
extern void UpdateSelectedBots(gentity_t* ent);

// TAT 11/16/2002 - Set the selected weapon of the bot - does NOT change which
// weapons the bot is holding
qboolean Bot_ScriptAction_SetActiveWeapon(bot_state_t* bs, char* params) {
  ...
  if (!COM_BitCheck(bs->cur_ps.weapons, weapon)) {
    Bot_ScriptError(
        bs, "Bot %s on team %s doesn't have weapon %s\n",
        g_entities[bs->client].aiName,
        (g_entities[bs->client].client->sess.sessionTeam == TEAM_AXIS)
            ? "Axis"
            : "Allies",
...
qboolean Bot_ScriptAction_FireAtTarget(bot_state_t* bs, char* params) {
  ...
  if (!ent) {
    ...
    if (!ent) {
      Bot_ScriptError(
          bs,
          "AI Scripting: fireattarget cannot find targetname/aiName \"%s\"\n",
          token);
    }
  }
  ...
  if (bs->weaponnum == WP_MORTAR_SET) {
    /*		vec_t x, y, u, d, b, g, a;
                    ...
                    bs->ideal_viewangles[PITCH] = (AngleMod(RAD2DEG(a)- 180) +
       60);*/
  ...
  } else  // if (bs->weaponnum != WP_MORTAR_SET)
  {
    for (i = 0; i < 2; i++) {
      diff = abs(
          AngleDifference(bs->cur_ps.viewangles[i], bs->ideal_viewangles[i]));
  ...
  //		TAT 1/29/2003 - the token in question was parsed right above
  //this 		so can't just move this up to the top 		plus, if we don't pass in a
  //duration, that means fire once, which that check above for no token will
  //handle, 		but we WILL fire once at least
...
//		Doesn't change the current weapon loadout (which weapons we
//carry)
qboolean Bot_ScriptAction_SetAmmoAmount(bot_state_t* bs, char* params) {
  ...
  if (!params || !params[0]) {
    Bot_ScriptError(bs,
                    "Bot_ScriptAction_SetAmmoAmmount: syntax: SetAmmoAmount "
                    "<weaponname> <number>");
  }
  ...
  weapon = Bot_GetWeaponForClassAndTeam(
      g_entities[bs->client].client->sess.playerType,
  ...
  if (!COM_BitCheck(bs->cur_ps.weapons, weapon)) {
    Bot_ScriptError(
        bs, "Bot_ScriptAction_SetAmmoAmount: Bot %s does not have weapon %s",
  ...
  if (clipOnly) {
    g_entities[bs->client].client->ps.ammoclip[BG_FindAmmoForWeapon(weapon)] =
        amount;
  } else {
    g_entities[bs->client].client->ps.ammo[BG_FindAmmoForWeapon(weapon)] =
        amount;
  }
...
qboolean Bot_ScriptAction_SetCivilian(bot_state_t* bs, char* params) {
  ...
  if (!params || !params[0]) {
    Bot_ScriptError(
        bs, "Bot_ScriptAction_SetCivilian: syntax: SetCivilian <Yes/No>");
  }
