...
int BotNumTeamMates(bot_state_t* bs, int* list, int maxList) {
  ...
  for (i = 0; i < level.numConnectedClients; i++) {
    ...
    if (list) {
      ...
      // calling without a list gives the full count no matter what maxList is
      // set to
      numplayers++;
    }
  }
...
==================
*/
int BotNumTeamMatesWithTarget(bot_state_t* bs,
                              int targetEntity,
                              int* list,
                              int maxList) {
  int i, j, numplayers;
...
==================
*/
int BotNumTeamMatesWithTargetByClass(bot_state_t* bs,
                                     int targetEntity,
                                     int* list,
                                     int maxList,
                                     int playerType) {
...
==================
*/
int BotNumTeamMatesWithTargetAndCloser(bot_state_t* bs,
                                       int targetEntity,
                                       int targetArea,
                                       int* list,
                                       int maxList,
  ...
  ourTime = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                              targetArea, bs->tfl);
  ...
  for (i = 0; i < level.numConnectedClients; i++) {
    ...
    if (playerType >= 0 &&
        g_entities[j].client->sess.playerType != playerType) {
      continue;
    }
    ...
    // trap_AAS_AreaTravelTimeToGoalArea( botstates[j].target_goal.areanum,
    // botstates[j].origin, targetArea, bs->tfl );
    if (!t) {
      continue;
    }
    if (t > ourTime) {
...
float* BotSortPlayersByDistance(vec3_t target, int* list, int numList) {
  ...
  for (i = 0; i < numList; i++) {
    distances[i] =
        VectorDistanceSquared(g_entities[list[i]].r.currentOrigin, target);
  }
...
float* BotSortPlayersByTraveltime(int areanum, int* list, int numList) {
  ...
  for (i = 0; i < numList; i++) {
    ...
    if (lbs && lbs->inuse) {
      distances[i] = (float)trap_AAS_AreaTravelTimeToGoalArea(
          BotGetArea(list[i]), BotGetOrigin(list[i]), areanum, lbs->tfl);
    } else {
      distances[i] = 0;
    }
  }
  ...
  for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
    ...
    if (!BotSameTeam(bs, i)) {
      if (g_entities[i].client->ps.powerups[PW_BLUEFLAG] ||
          g_entities[i].client->ps.powerups[PW_REDFLAG]) {
...
int BotFlagAtBase(int team, gentity_t** returnEnt) {
  gentity_t* ent;
  botStaticEntityEnum_t flags[2] = {BOTSTATICENTITY_CTF_REDFLAG,
...
int BotClientTravelTimeToGoal(int client, bot_goal_t* goal) {
  ...
  return trap_AAS_AreaTravelTimeToGoalArea(areanum, ps.origin, goal->areanum,
                                           BotTravelFlagsForClient(client));
}
...
int BotFindSparseDefendArea(bot_state_t* bs, bot_goal_t* goal, qboolean force) {
  ...
  int avoidEnts[] = {BOTSTATICENTITY_FUNC_DOOR,
                     BOTSTATICENTITY_FUNC_DOOR_ROTATING, -1};
  vec3_t avoidPos[128];
  int numAvoidPos;
  ...
  numTPos = 0;
  for (i = 0; i < numTeam; i++) {
    if (!g_entities[teammates[i]].inuse ||
  ...
  if (goal->entitynum >= 0) {
    VectorAdd(g_entities[goal->entitynum].r.absmin,
              g_entities[goal->entitynum].r.absmax, targetPos);
  ...
  if (!BotSinglePlayer() && !BotCoop() && goal->entitynum >= 0 &&
      goal->entitynum < MAX_CLIENTS && BotCarryingFlag(goal->entitynum)) {
    if (!(g_entities[goal->entitynum].r.svFlags & SVF_BOT)) {
      ...
      if (trav) {
        ...
        // find the flag itself, if we are closer the that, then pick a spot
        // further away from the flag
        flagEnt = NULL;
        ...
        if (flagEnt) {
          ...
          t = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                                flag.areanum, bs->tfl);
          if (t) {
            t2 = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                                   flagGoal.areanum, bs->tfl);
            if (t < t2) {
              // we should find a spot further away from the flag itself, rather
              // than closer to the destination. this takes into account the
              // player taking an alternate route, so that the bots lead the
              // player still, rather than trailing behind while they think they
              // are leading the player.
          ...
          numList =
              trap_AAS_BBoxAreas(trav->r.absmin, trav->r.absmax, list, 20);
          bestDist = -1;
          if (numList) {
            oldestTime = -1;
            for (i = 0; i < numList; i++) {
              ...
              t = trap_AAS_AreaTravelTimeToGoalArea(goal->areanum, goal->origin,
                                                    list[i], bs->tfl);
              if (t > 0) {
  ...
  if (!BotSinglePlayer() && !BotCoop()) {
    ...
    /*		// if we are following a leader, we need to work out where to
       position ourselves from them if (goal->entitynum >= 0 && goal->entitynum
       < MAX_CLIENTS) { useoffset = qtrue;
                            //
                            trav = BotGetEntity( goal->entitynum );
                            // are they moving?
                            if (VectorLengthSquared( trav->client->ps.velocity )
       > SQR(10)) { VectorNormalize2( trav->client->ps.velocity, offsetVec );
  ...
  //
  numareas = trap_AAS_ListAreasInRange(
      targetPos, goal->areanum, maxrange,
      bs->tfl & ~(TFL_BARRIERJUMP | TFL_LADDER | TFL_JUMP), (float**)waypoints,
      64);
  //
  if (!numareas) {
    return 0;
  ...
  //
  // find the area with the greatest distance from all other teammates, and the
  // goal
  bestDist = -999999;
  closest = 99999;
  for (i = 0; i < numareas; i++) {
    ...
    dist += closest * 6;  // weight this higher than the goal distance, since we
                          // really want to space each other out
    //
    ...
    if (hasflag) {
      ...
      t = trap_AAS_AreaTravelTimeToGoalArea(area, waypoints[i],
                                            flagGoal.areanum, bs->tfl);
      //
      if (!t) {
        continue;
    ...
    if (dist > bestDist) {
      area = trap_AAS_PointAreaNum(waypoints[i]);
      if (area) {
        t = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, area,
                                              bs->tfl);
        if (t) {
          bestDist = dist;
  ...
  while (
    ...
    if ((targ = trav->target_ent)) {
      if (targ->s.eType == ET_EXPLOSIVE) {
        ...
        if ((targ->parent->spawnflags & AXIS_OBJECTIVE) &&
            (team == TEAM_AXIS)) {
          continue;
        } else if ((targ->parent->spawnflags & ALLIED_OBJECTIVE) &&
                   (team == TEAM_ALLIES)) {
          continue;
        }
        ...
        targ = G_ConstructionForTeam(
            trav, team == TEAM_AXIS ? TEAM_ALLIES : TEAM_AXIS);
...
==================
*/
int BotGetTargetsForSatchelCharge(team_t team,
                                  int* list,
                                  int listSize,
  ...
  if ((pCache->time == level.time) && (pCache->listSize == listSize) &&
  ...
  count = 0;
  trav = NULL;
  for (trav = G_FindSatchelChargeTargetForTeam(NULL, team); trav;
    ...
    if (!ignoreSatchelCharge) {
      ...
      for (dyn = G_FindSatchels(NULL); dyn; dyn = G_FindSatchels(dyn)) {
        ...
        if ((VectorLengthSquared(vec) <= SQR(dyn->splashRadius)) &&
...
==================
*/
int BotGetTargetExplosives(team_t team,
                           int* list,
                           int listSize,
  ...
  count = 0;
  trav = NULL;
  for (trav = G_FindDynamiteTargetForTeam(NULL, team); trav;
    ...
    if (!ignoreDynamite) {
      ...
      for (dyn = G_FindDynamite(NULL); dyn; dyn = G_FindDynamite(dyn)) {
        ...
        if ((VectorLengthSquared(vec) <= SQR(dyn->splashRadius)) &&
            CanDamage(trav, dyn->r.currentOrigin)) {
          // Gordon: checking damage makes no sense in this case, as these are
          // one hit wonders
          if (listSize) {
            if (list && count >= listSize) {
              continue;
    ...
    for (team = TEAM_AXIS; team <= TEAM_ALLIES; team++) {
      ...
      if (target) {
        if (target->s.eType == ET_EXPLOSIVE) {
          ...
          if (BG_BBoxCollision(dyn->r.absmin, dyn->r.absmax,
                               target->parent->r.absmin,
...
==================
*/
int BotGetConstructibles(team_t team,
                         int* list,
                         int listSize,
...
qboolean BotCheckNeedEngineer(bot_state_t* bs, team_t team) {
  ...
  if (*lastTime && *lastTime <= level.time && *lastTime > level.time - 10000) {
    return qfalse;  // only check every 10 seconds, to prevent everyone from
                    // changing class at once
  }
  *lastTime = level.time;
  ...
  if (BotCarryingFlag(bs->client) ||
      (bs->leader > -1 &&
       BotCarryingFlag(bs->leader))) {  // dont abandon a leader that needs us
    return qfalse;
  }
  ...
  if (!BotGetTargetExplosives(team, NULL, 0, qfalse) &&
      !BotGetConstructibles(team, NULL, 0, qtrue)) {
    return qfalse;
  }
...
int BotSuggestClass(bot_state_t* bs, team_t team) {
  ...
  int classPriority[NUM_PLAYER_CLASSES] = {PC_ENGINEER, PC_SOLDIER, PC_MEDIC,
                                           PC_FIELDOPS, PC_COVERTOPS};
  //	char	userinfo[MAX_INFO_STRING], *str;
  float bestDiff, diff;
  int bestClass;
  qboolean needEngineers = qfalse;
  int lastMg42Death =
      (team == TEAM_ALLIES ? level.alliesMG42Counter : level.axisMG42Counter);
  ...
  if (BotGetTargetExplosives(team, NULL, 0, qtrue) ||
      BotGetConstructibles(team, NULL, 0, qtrue)) {
    needEngineers = qtrue;
  } else {
  ...
  numRequired[PC_COVERTOPS] = numRequired[PC_FIELDOPS] = numRequired[PC_MEDIC] =
      (numTeamMembers / 6.f) > 1 ? (numTeamMembers / 6.f) : 1;

  //
  // special cases
  if (lastMg42Death && ((level.time - lastMg42Death) < (30 * 1000))) {
    // use panzers to clear mg42 nests
    numRequired[PC_SOLDIER] = (numTeamMembers - 3 > numRequired[PC_SOLDIER]
                                   ? numTeamMembers - 3
                                   : numRequired[PC_SOLDIER]);
    numRequired[PC_COVERTOPS] = numRequired[PC_FIELDOPS] =
        numRequired[PC_MEDIC] = 1;
  }
  ...
  for (i = 0; i < NUM_PLAYER_CLASSES; i++) {
    ...
    diff = BotNumTeamClasses(team, classPriority[i], bs->client) /
           (float)numRequired[classPriority[i]];
    if (bestDiff > diff) {
  ...
  // Gordon: FIXME: balanace it? and why not engineer, they ARE useful outside
  // of constructing stuff etc... not important, so just pick at random
...
int BotSuggestWeapon(bot_state_t* bs, team_t team) {
  ...
  /*	trap_GetUserinfo( bs->client, userinfo, sizeof(userinfo) );
          if ((str = Info_ValueForKey( userinfo, "pWeapon" )) && Q_stricmp(str,
     "ANY") && (i = atoi(str))) { return i;
          }*/
  ...
  if (level.captureFlagMode) {
    ...
    if ((bs->mpClass == PC_COVERTOPS) && (rand() % 3) &&
    ...
    if ((bs->mpClass == PC_SOLDIER) && (rand() % 3) &&
        (level.time < level.startTime + 30000) &&
        (bs->mpTeam == level.attackingTeam)) {
      // return 4; // panzer
      return WP_PANZERFAUST;
    }
    // if GT_WOLF, and checkpoint is owned by opposition, then no point in
    // having snipers
    trav = NULL;
    ...
    case PC_SOLDIER:
      ...
      switch (r) {
        ...
        case 2:
          ...
          // case 3:	return WP_MORTAR;		// they dont understand
          // this yet
        case 0:
          switch (bs->mpTeam) {
            case TEAM_AXIS:
...
void BotCTFOrders_BothFlagsNotAtBase(bot_state_t* bs) {
  ...
  switch (bs->numteammates) {
    ...
    case 3: {
      ...
      BotSayTeamOrder(bs, other);
      // tell the one furthest from the the base not carrying the flag to get
      // the enemy flag
...
qboolean BotClass_CovertOpsCheckDisguises(bot_state_t* bs,
                                          int maxTravel,
                                          bot_goal_t* goal) {
  gentity_t* trav;
  int t, area, best = -1, bestTravel,
               bestArea = -1;  // Arnout: bestArea was not initialized
  bot_goal_t target;
  int list[32], numList;
  vec3_t loc;
  ...
  while ((trav = G_Find(trav, FOFS(classname), "corpse"))) {
    ...
    numList = BotNumTeamMatesWithTargetByClass(bs, trav->s.number, list, 32,
                                               PC_COVERTOPS);
    if (numList) {
      numList =
          BotReduceListByTravelTime(list, numList, BotGetOrigin(trav->s.number),
                                    BotGetArea(trav->s.number),
                                    BotTravelTimeToEntity(bs, trav->s.number));
    }
    if (numList) {
      continue;
    }
...
==============
*/
qboolean BotClass_MedicCheckRevives(bot_state_t* bs,
                                    int maxtravel,
                                    bot_goal_t* goal,
                                    qboolean lookForBots) {
  gentity_t* trav;
  int i, t, area, best = -1, bestTravel,
                  bestArea = -1;  // Arnout: bestArea was not initialized
  ...
  for (i = 0; i < numTeammates; i++) {
    ...
    if ((numList = BotNumTeamMatesWithTargetByClass(bs, teammates[i], list, 32,
                                                    PC_MEDIC))) {
      if (BotReduceListByTravelTime(list, numList, BotGetOrigin(teammates[i]),
                                    BotGetArea(teammates[i]),
    ...
    // check the route to them
    area = BotGetArea(
        trav->s.number);  // trap_AAS_PointAreaNum(trav->r.currentOrigin);
    if (!area) {
      continue;
    }
...
==============
*/
qboolean BotClass_MedicCheckGiveHealth(bot_state_t* bs,
                                       int maxTravelTime,
                                       bot_goal_t* goal) {
  gentity_t* trav;
  int i, area, best = -1,
  ...
  for (i = 0; i < numTeammates; i++) {
    ...
    time = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, area,
                                             bs->tfl);
    if (!time) {
      continue;
    }
...
==============
*/
qboolean BotClass_LtCheckGiveAmmo(bot_state_t* bs,
                                  int maxTravelTime,
                                  bot_goal_t* goal) {
  gentity_t* trav;
  int i, area = 0, best = -1,
...
char* vchat_idstr[] = {"HealMe", "NeedAmmo", "NeedBackup", "DynamitePlanted",
                       "GreatShot", "Hi", "Thanks", "FollowMe", "DropFlag",
                       "AggressionLow", "AggressionMed", "AggressionHigh",
                       "AggressionCycle", "AutonomyLow", "AutonomyMed",
                       "AutonomyHigh", "AutonomyCycle",

                       // Start - TAT 8/21/2002
                       // corresponding strings from the .menu file that trigger
                       // the vchat events below
                       "HoldPosition",
                       // End - TAT 8/21/2002
                       ...
                       "Prone",
                       // END		xkan, 9/13/2002
                       "Crouch", "Stand", "PositionCycle",

                       "FireteamA", "FireteamB", "FireteamC",

                       "ReviveMe", "HealTeam", "TeamAmmo",
                       // End - TAT 9/23/2002

                       "CycleWeapon", "BestWeapon",
...
===============
*/
void BotRecordVoiceChat(int client,
                        int destclient,
                        const char* id,
                        int mode,
                        qboolean noResponse) {
  ...
  for (i = 0; i < MAX_VCHATS; i++, trav++) {
    ...
    switch (trav->id) {
      case VCHAT_THANKS:
        ...
        // your welcome!
        BotVoiceChatAfterIdleTime(bs->client, "Welcome", SAY_TEAM,
                                  1000 + rand() % 1000, BOT_SHOWTEXT, 3000,
                                  qfalse);
        break;
      case VCHAT_GREATSHOT:
        ...
        // thanks!
        BotSendVoiceChat(bs, "Thanks", SAY_ALL, 1000 + rand() % 1000,
                         BOT_SHOWTEXT, qfalse);
        break;
      case VCHAT_HI:
        clear = qtrue;
        ...
        if (other->client->sess.sessionTeam) {
          ...
          BotVoiceChatAfterIdleTime(bs->client, "Hi", SAY_ALL,
                                    1000 + rand() % 6000, BOT_SHOWTEXT, 7000,
                                    qfalse);
        }
        break;
...
void BotDelayedVoiceChat(gentity_t* ent) {
  ...
  if (g_entities[ent->r.ownerNum].inuse &&
      ent->count2 == g_entities[ent->r.ownerNum].client->pers.connectTime) {
    bs->last_voice_chat = level.time;
    G_Voice(BotGetEntity(ent->r.ownerNum), NULL, ent->missionLevel,
            ent->spawnitem, ent->count);
  }
...
===============
*/
void BotSendVoiceChat(bot_state_t* bs,
                      const char* id,
                      int mode,
                      int delay,
                      qboolean voiceonly,
...
===============
*/
void BotVoiceChatAfterIdleTime(int client,
                               const char* id,
                               int mode,
                               int delay,
                               qboolean voiceonly,
...
// Written: 10/31/2002
//
void AI_Team_Init(AI_Team_t* thisOne) {
...
*/
void BotVoiceChatAfterTeamIdleTime(int client,
                                   const char* id,
                                   int mode,
                                   int delay,
                                   qboolean voiceonly,
  ...
  if (g_aiTeam[bs->sess.sessionTeam].last_voice_chat &&
...
void BotSpawnSpecialEntities(void) {
  ...
  while (1) {
    ...
    if (com_token[0] != '{') {
      G_Error("BotSpawnSpecialEntities: (%s) found %s when expecting {",
              filename, com_token);
    }
    ...
    while (1) {
      ...
      com_token = COM_Parse(&pStr);
      if (!com_token || !com_token[0]) {
        G_Error("BotSpawnSpecialEntities: (%s) EOF without closing brace",
                filename);
      }
      ...
      if (com_token[0] == '}') {
        G_Error("BotSpawnSpecialEntities: (%s) closing brace without data",
                filename);
      }
...
int BotGetLeader(bot_state_t* bs, qboolean onlyRequested) {
  ...
  if (bs->leader > -1 && (bs->script.frameFlags & BSFFL_FOLLOW_LEADER) &&
      (bs->leader == bs->script.entityNum)) {
    return bs->leader;
  }
  ...
  // if we have a scripting enforced movement autonomy, stay put, unless we are
  // given a new scripted leader
  ...
  for (i = 0; i < level.numConnectedClients; i++, trav++) {
    ...
    /*		if (!requested && (trav->client->sess.playerType != PC_SOLDIER))
       {
                            // follow engineers only if they have a critical
       task if (trav->client->sess.playerType == PC_ENGINEER && obs) { if
       (!(obs->ainode == AINode_MP_DynamiteTarget || obs->ainode ==
       AINode_MP_ConstructibleTarget)) { continue; } else { requested = qtrue;
                                    }
                            } else {
                                    continue;
                            }
    ...
    if ((ourDist = VectorDistanceSquared(trav->r.currentOrigin, bs->origin)) >
        (MAX_BOTLEADER_DIST * MAX_BOTLEADER_DIST)) {
      continue;
    }

    if (requested && (!trap_InPVS(trav->r.currentOrigin, bs->origin) ||
                      (ourDist > (1024 * 1024)))) {
      continue;
    }

    if ((numList = BotNumTeamMatesWithTarget(bs, i, list, MAX_CLIENTS)) >=
        2 + (requested ? 2 : 0)) {
      // if there are too many defenders, and we are the furthest away, dont
      // defend
      distances =
          BotSortPlayersByDistance(trav->r.currentOrigin, list, numList);
    ...
    if (!BotGoalForEntity(bs, i, &goal, BGU_MEDIUM) && !requested) {
      continue;  // NOTE: BotGoalForEntity() must be called first, so that if it
                 // fails, but we requested it, it still goes through with valid
                 // goal information
    }
    ...
    if (requested) {
      j = rand() % 2;
      if (j == 0) {
        BotVoiceChatAfterIdleTime(bs->client, "WhereTo", SAY_TEAM,
                                  1000 + rand() % 3000, BOT_SHOWTEXT,
