...
void AIEnter_MP_Stand(bot_state_t* bs) {
  ...
  bs->respawn_time =
      trap_AAS_Time() + 20;  // after this long just standing around, suicide
...
int AINode_MP_Stand(bot_state_t* bs) {
  ...
  if (bs->stand_time < trap_AAS_Time()) {
    ...
    if (bs->ainode != AINode_MP_Stand) {
      ...
      trap_EA_Move(bs->client, tv(crandom(), crandom(), crandom()),
                   100 + random() * 200);
    } else {
...
int AINode_MP_Respawn(bot_state_t* bs) {
  ...
  //	if (ent->missionLevel < level.time) {	// else medic is heading to us
  //to revive
    ...
    if (!BotIsDead(bs)) {
      ...
      if (bs->flags & BFL_MISCFLAG) {
        ...
        if (lastCall > level.time || lastCall < level.time - 2000) {
          ...
          switch (bs->mpClass) {
            case PC_SOLDIER:
              BotVoiceChatAfterIdleTime(bs->client, "IamSoldier", SAY_TEAM,
                                        1000 + rand() % 5000, BOT_SHOWTEXT,
                                        20000, qfalse);
              break;
            case PC_MEDIC:
              BotVoiceChatAfterIdleTime(bs->client, "IamMedic", SAY_TEAM,
                                        1000 + rand() % 5000, BOT_SHOWTEXT,
                                        20000, qfalse);
              break;
            case PC_FIELDOPS:
              BotVoiceChatAfterIdleTime(bs->client, "IamLieutenant", SAY_TEAM,
                                        1000 + rand() % 5000, BOT_SHOWTEXT,
                                        20000, qfalse);
              break;
            case PC_ENGINEER:
              BotVoiceChatAfterIdleTime(bs->client, "IamEngineer", SAY_TEAM,
                                        1000 + rand() % 5000, BOT_SHOWTEXT,
                                        20000, qfalse);
              break;
          }
        ...
        if (rand() % 2) {
          BotVoiceChatAfterIdleTime(bs->client, "LetsGo", SAY_TEAM,
                                    1000 + rand() % 2000, qfalse, 20000,
                                    qfalse);
        }
      }
      //
...
int AINode_MP_Seek_NBG(bot_state_t* bs) {
  ...
  if (!ent->inuse) {
    ...
    if (/*trap_BotTouchingGoal( bs->origin, &goal ) ||*/ (ent->r.svFlags &
                                                          SVF_NOCLIENT)) {
      bs->ignore_specialgoal_time = 0;
      BotDefaultNode(bs);
      return qfalse;
...
void AIEnter_MP_AvoidDanger(bot_state_t* bs) {
  ...
  // if this is dynamite
  if (g_entities[bs->avoid_goal.entitynum].s.eType == ET_MISSILE &&
      g_entities[bs->avoid_goal.entitynum].methodOfDeath == MOD_DYNAMITE) {
    if (!(rand() % 3)) {
      BotVoiceChatAfterIdleTime(bs->client, "FireInTheHole", SAY_TEAM, 500,
                                qfalse, 3000, qfalse);
    }
  }

  bs->flags &= ~BFL_MISCFLAG;
  if (!(bestarea = trap_AAS_AvoidDangerArea(
            bs->origin, bs->areanum, bs->avoid_goal.origin,
            BotPointAreaNum(-1, bs->avoid_goal.origin),
            bs->avoid_goal.number + 100,
            bs->tfl))) {  // no hiding spot, ignore it
    bs->flags |= BFL_MISCFLAG;
  } else {
...
int AINode_MP_AvoidDanger(bot_state_t* bs) {
  ...
  if (!trav->inuse) {
    ...
    if ((g_entities[bs->client].client->lastConstructibleBlockingWarnEnt !=
         goal.entitynum) ||
        ((level.time -
          g_entities[bs->client].client->lastConstructibleBlockingWarnTime) >
         5000)) {
      bs->ignore_specialgoal_time = 0;
      BotDefaultNode(bs);
      return qfalse;
  ...
  // if the thing we're avoiding is a landmine, then we don't really need to run
  // away from it, just take a few steps don't need to move, is this a landmine?
  if ((trav->methodOfDeath == MOD_LANDMINE) &&
  ...
  if (trav->client) {
    ...
    if (trav->s.eType == ET_MISSILE && trav->s.weapon == WP_DYNAMITE) {
      ...
      if (!G_PredictMissile(trav, trav->nextthink - level.time, target,
    ...
    if ((bs->last_avoiddangerarea < level.time - 200) &&
        (VectorDistanceSquared(target, goal.origin) <
         (SQR(trav->splashRadius + 100)))) {
      bs->last_avoiddangerarea = level.time + rand() % 200;
      if (!(bestarea = trap_AAS_AvoidDangerArea(
                bs->origin, bs->areanum, target, BotPointAreaNum(-1, target),
                trav->splashRadius + 100, bs->tfl))) {
  ...
  // if waiting for something
    ...
    if (bs->enemy < 0 && !(bs->flags & BFL_IDEALVIEWSET)) {
      if (!moved) {
        VectorSubtract(g_entities[goal.entitynum].r.currentOrigin, bs->origin,
                       dir);
        vectoangles(dir, bs->ideal_viewangles);
...
int AINode_MP_GiveAmmo(bot_state_t* bs) {
  ...
  if (!ClientNeedsAmmo(trav->s.number)) {
    if (VectorDistanceSquared(bs->origin, goal.origin) < SQR(72)) {
      ...
      if (trav->r.svFlags & SVF_BOT) {
        BotVoiceChatAfterIdleTime(trav->s.number, "Thanks", SAY_TEAM,
                                  1000 + rand() % 2000, qfalse,
  ...
  bs->target_goal.areanum =
      BotPointAreaNum(trav->s.number, trav->r.currentOrigin);
  if (!bs->target_goal.areanum) {
    BotDefaultNode(bs);
    return qfalse;
  }
  goal = bs->target_goal;
  if (VectorDistanceSquared(bs->origin, goal.origin) < SQR(100) &&
      BotEntityVisible(bs->client, bs->eye, bs->viewangles, 360, trav->s.number,
                       NULL)) {
  ...
  // reload?
  if ((bs->last_fire != level.time) &&
      (bs->cur_ps.ammoclip[BG_FindClipForWeapon(bs->cur_ps.weapon)] <
...
int AINode_MP_MedicRevive(bot_state_t* bs) {
  ...
  if (g_entities[goal.entitynum].botIgnoreHealthTime >= level.time) {
    ...
    //
    // if there are too many defenders, and we are the furthest away, stop
    // defending
    if ((numList = BotNumTeamMatesWithTarget(bs, goal.entitynum, list,
                                             MAX_CLIENTS)) > 0) {
      ourDist = VectorDistanceSquared(
          bs->origin, g_entities[goal.entitynum].r.currentOrigin);
      if (!trap_InPVS(bs->origin, g_entities[goal.entitynum].r.currentOrigin)) {
        ourDist += (2048 * 2048);
      }
      distances = BotSortPlayersByDistance(
          g_entities[goal.entitynum].r.currentOrigin, list, numList);
  ...
  range = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                            goal.areanum, bs->tfl);
  //
...
int AINode_MP_PanzerTarget(bot_state_t* bs) {
  ...
  if ((bs->cur_ps.weapon == bs->weaponnum) &&
      ...
      (AngleDifference(bs->ideal_viewangles[PITCH], bs->viewangles[PITCH]) <
       0.5)) {
    trap_EA_Attack(bs->client);
  }
  //
...
int AINode_MP_FixMG42(bot_state_t* bs) {
  ...
  if (BotIsDead(bs)) {
    ent->botIgnoreTime =
        level.time + 5000;  // other bots should avoid this spot
    AIEnter_MP_Respawn(bs);
    return qfalse;
  }
  // check for emergency targets (flags, etc)
  ...
  if (VectorLengthSquared(dir) < SQR(64)) {
    trap_Trace(&tr, ent->s.origin, NULL, NULL, ent->s.origin, bs->client,
               MASK_PLAYERSOLID);
  ...
  if (VectorLengthSquared(dir) > SQR(8)) {
    ...
    if (moveresult.flags & (MOVERESULT_MOVEMENTVIEWSET |
      ...
      VectorCopy(moveresult.ideal_viewangles, bs->ideal_viewangles);
    } else if (moveresult.flags &
               MOVERESULT_WAITING) {  // if waiting for something
      if (random() < bs->thinktime * 0.8) {
        BotRoamGoal(bs, target);
        VectorSubtract(target, bs->origin, dir);
        vectoangles(dir, bs->ideal_viewangles);
        ...
        if (bs->enemy < 0 && !(bs->flags & BFL_IDEALVIEWSET)) {
          if (moveresult.flags & MOVERESULT_DIRECTMOVE) {
            ...
            vectoangles(dir, bs->ideal_viewangles);
                         ...
                         moveresult
                             .movedir)) {  // FIXME: look at cluster portals?
            vectoangles(moveresult.movedir, bs->ideal_viewangles);
          } else if (random() < bs->thinktime * 0.8) {
            BotRoamGoal(bs, target);
            VectorSubtract(target, bs->origin, dir);
    ...
    if (bs->enemy >= 0) {
      ...
      if (bs->enemy >= 0) {
        if (VectorDistanceSquared(bs->origin, bs->enemyorigin) < SQR(512)) {
          ...
          if (VectorDistanceSquared(bs->origin,
  ...
  } else if ((bs->last_fire != level.time) &&
             (bs->cur_ps.ammoclip[BG_FindClipForWeapon(bs->cur_ps.weapon)] <
              (int)(0.8 * GetAmmoTableData(bs->cur_ps.weapon)->maxclip)) &&
             bs->cur_ps
                 .ammo[BG_FindAmmoForWeapon(bs->cur_ps.weapon)]) {  // reload?
    trap_EA_Reload(bs->client);
  }
...
int AINode_MP_Battle_MobileMG42(bot_state_t* bs) {
  ...
  if (level.captureFlagMode &&
      BotFlagAtBase(bs->sess.sessionTeam, NULL) == qfalse) {
    if (BotFindSpecialGoals(bs)) {
      return qfalse;
    }
  }
  ...
  if (tookDamage > 20 || g_entities[bs->client].health < 40 ||
      bs->arrive_time < level.time - 5000) {
    if (BotFindSpecialGoals(bs)) {
      return qfalse;
    }
  }
  ...
  if (!(bs->cur_ps.eFlags & EF_PRONE) &&
      (fabs(AngleDifference(bs->ideal_viewangles[YAW], bs->viewangles[YAW])) >
       5.0f)) {
    return qtrue;
  }
  ...
  if (bs->enemy < 0) {
    // if we still have no enemy, then after a slight pause, allow us to escape
    // prone mode if we find an
  ...
  if (bs->enemy >= 0) {
    ...
    if (bs->enemy >= 0) {
      ...
      if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 120,
        ...
        if (VectorDistanceSquared(
                bs->origin, g_entities[bs->enemy].r.currentOrigin) < SQR(400)) {
          AIEnter_MP_Battle_Fight(bs);
          return qfalse;
        }
        //
    ...
    // NOTE: remember last visible enemy positions, so we can stay on them
    // longer than usual
...
*/      } else {
  // use mg42 angles
  VectorCopy(level.clients[bs->client].pmext.mountedWeaponAngles,
             bs->ideal_viewangles);
...
int AINode_MP_MG42Scan(bot_state_t* bs) {
  ...
  //
  ent->botIgnoreTime =
      0;  // set this now, only reset it to ignore if we want to ignore it
  //
  ...
  */                                                                                                                                                                      // look for something better to do
  ...
  if (bs->arrive_time < level.time - 40000) {
    ...
    ent->botIgnoreTime =
        level.time + 20000;  // we should ignore it for a while now
    if (BotFindSpecialGoals(bs)) {
      return qfalse;
    }
  }
  ...
  if (bs->enemy >= 0) {
    ...
    if (bs->enemy >= 0) {
      ...
      if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 120,
                           bs->enemy, NULL)) {
        if ((fabs(AngleDifference(ang[PITCH], mg42->s.angles[PITCH])) >=
             mg42->varc) ||
            (fabs(AngleDifference(ang[YAW], mg42->s.angles[YAW])) >=
             mg42->harc)) {
          ent->botIgnoreTime = level.time + 5000;
          AIEnter_MP_Battle_Fight(bs);
          return qfalse;
        ...
        // if they are real close, abort mg42 mode
        if (VectorDistanceSquared(
                bs->origin, g_entities[bs->enemy].r.currentOrigin) < SQR(512)) {
          ent->botIgnoreTime = level.time + 5000;
          AIEnter_MP_Battle_Fight(bs);
          return qfalse;
        }
...
int AINode_MP_MG42Mount(bot_state_t* bs) {
  ...
  if (ent->melee->health <= 0 || (ent->melee->entstate != STATE_DEFAULT) ||
      ent->melee->active) {
    bs->ainode = NULL;
    bs->ainodeText = "NULL";
    return qtrue;
  ...
  */                                                                                            // look for something better to do
  ...
  if (bs->arrive_time < level.time - 40000) {
    ent->botIgnoreTime =
        level.time + 15000;  // other bots should avoid this spot
    if (BotFindSpecialGoals(bs)) {
      return qfalse;
    }
  }
  ...
  // is the destination blocked?
  if (VectorLengthSquared(dir) < SQR(64)) {
    trap_Trace(&tr, ent->r.currentOrigin, NULL, NULL, ent->r.currentOrigin,
  ...
  if (VectorDistanceSquared(bs->origin, goal.origin) < SQR(64)) {
    ...
    CalcMuzzlePointForActivate(&g_entities[bs->client], forward, right, up,
                               offset);
    VectorSubtract(ent->melee->r.currentOrigin, offset, dir);
    VectorNormalize(dir);
    vectoangles(dir, bs->ideal_viewangles);
    if (bs->arrive_time < level.time - 500) {
      // randomize the angles a bit to solve some times when a direct sight
      // isn't "enough"
      if (rand() % 4 == 0) {
...
int AINode_MP_ScanForLandmines(bot_state_t* bs) {
  ...
  if (VectorLengthSquared(dir) < SQR(32)) {
    ...
    if (bs->altenemy <= 0) {
      ...
      // we're going to modify the point we're looking at, based on the current
      // counter number
      ...
      bs->altenemy = AngleNormalize360(
          RAD2DEG(2 * tan(DETECT_RADIUS / VectorDistance(bs->eye, target))));
      ...
      // we're doing a sweep - on the 2nd pass we sweep the reverse direction of
      // the other 2 passes
...
int AINode_MP_SniperSpot(bot_state_t* bs) {
  ...
  if (VectorLengthSquared(dir) < SQR(32)) {
    ...
    if (bs->enemy >= 0) {
      ...
      if (bs->enemy >= 0) {
        if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 50,
                             bs->enemy, NULL)) {
          // if they are real close, abort sniper mode
          if (VectorDistanceSquared(bs->origin,
                                    g_entities[bs->enemy].r.currentOrigin) <
          ...
          // attack the enemy if possible
          if (bs->weaponnum == bs->cur_ps.weapon &&
              BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 5,
                               bs->enemy, NULL)) {
            BotCheckAttack(bs);
          }
        } else {
      ...
      if (bs->enemyposition_time > trap_AAS_Time() - 5.0) {
        ...
        // use same angles
        //
      } else if (!(rand() % (1 + BotGetNumVisibleSniperSpots(bs))) &&
      ...
      } else if (rand() % 2 && bs->enemyposition_time &&
                 (bs->enemyposition_time > trap_AAS_Time() - 60.0)) {
        // look at last enemy pos
        VectorSubtract(bs->enemyorigin, bs->origin, dir);
        // if it's not too far away, use non-zoomed view
        if (VectorNormalize(dir) < 3000) {
          // switch to non-zoomed view
          if (bs->weaponnum >= WP_BEGINSECONDARY &&
              bs->weaponnum <= WP_LASTSECONDARY) {
  ...
  // reload?
  if ((level.time - bs->last_fire > 2000) &&
      (bs->cur_ps.ammoclip[BG_FindClipForWeapon(bs->cur_ps.weapon)] <
       (int)(0.8 * GetAmmoTableData(bs->cur_ps.weapon)->maxclip)) &&
      bs->cur_ps.ammo[BG_FindAmmoForWeapon(bs->cur_ps.weapon)]) {
    trap_EA_Reload(bs->client);
  ...
  */
      ...
      if (!g_entities[bs->leader].inuse || !g_entities[bs->leader].client ||
        ...
        if (g_entities[bs->leader].health <= 0 &&
          ...
          return qfalse;
                      ...
                      g_entities[bs->leader].client->ps.velocity) < SQR(10)) &&
                 (VectorLengthSquared(bs->cur_ps.velocity) < SQR(10))) {
        if (!(g_entities[bs->leader].r.svFlags & SVF_BOT)) {
          BotVoiceChatAfterIdleTime(bs->client, "WhereTo", SAY_BUDDY,
                                    1000 + rand() % 3000, BOT_SHOWTEXT, 12000,
                                    qfalse);
        }
      }
      //
      if (!g_entities[bs->leader].inuse || g_entities[bs->leader].health <= 0 ||
          VectorDistanceSquared(g_entities[bs->leader].r.currentOrigin,
  ...
  if (BotFindEnemyMP(bs, -1, qfalse)) {
    ...
    if (BotCarryingFlag(bs->enemy) ||
        (!(goal.flags & GFL_LEADER) && (bs->flags & BFL_MISCFLAG))) {
      if (!BotCarryingFlag(bs->enemy) &&
          VectorLengthSquared(bs->cur_ps.velocity) && BotWantsToRetreat(bs)) {
        // keep the current long term goal and retreat
      } else {
        trap_BotResetLastAvoidReach(bs->ms);
        // go fight
  ...
  switch (ent->s.eType) {
    ...
    case ET_TRAP:
      if (!Q_stricmp(ent->classname, "team_WOLF_checkpoint")) {
        if (ent->count ==
            (bs->sess.sessionTeam == TEAM_AXIS ? TEAM_ALLIES : TEAM_AXIS)) {
  ...
  // if we are defending a player, and they have moved, get a new sparse defend
  // area
  if (ent->client && (bs->target_goal.number < level.time - 300) &&
    ...
    if (BotCarryingFlag(bs->target_goal.entitynum)) {
      if ((numList = BotNumTeamMatesWithTarget(bs, goal.entitynum, list,
                                               MAX_CLIENTS)) >
          BOT_FLAG_CARRIER_DEFENDERS) {
        ourDist = VectorDistanceSquared(
            bs->origin, g_entities[goal.entitynum].r.currentOrigin);
        if (!trap_InPVS(bs->origin,
    ...
    bs->target_goal.areanum =
        BotPointAreaNum(ent->s.number, ent->r.currentOrigin);
  ...
  //
  // remember the last position we couldnt see the DESTINATION from, so we can
  // look there once we arrive
  ...
  if (!(bs->target_goal.flags & GFL_DEFEND_CLOSE)) {
    ...
    if (!move && trap_InPVS(bs->origin, goal.origin)) {
      trace_t tr;
      trap_Trace(&tr, bs->origin, NULL, NULL, goal.origin, -1,
                 MASK_SHOT & ~(CONTENTS_BODY | CONTENTS_CORPSE));
    ...
    if (!move && VectorDistanceSquared(bs->origin, goal.origin) >
                     ((goal.flags & GFL_LEADER) ? SQR(80) : SQR(32))) {
      move = qtrue;
    }
  }
  ...
  if (move) {
    ...
    if (bs->blockentTime > level.time - 500 &&
        bs->blockent < level.maxclients) {
    ...
    if (moveresult.flags & (MOVERESULT_MOVEMENTVIEWSET |
      ...
      if (bs->enemy >= 0) {
        ...
        if (bs->enemy >= 0) {
          ...
          if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360,
            ...
            if (!BotCarryingFlag(bs->target_goal.entitynum) &&
                ((bs->weaponnum == WP_LUGER) || (bs->weaponnum == WP_COLT) ||
                 (bs->weaponnum == WP_MOBILE_MG42))) {
              AIEnter_MP_Battle_Fight(bs);
              return qfalse;
            } else {
      ...
      if (bs->enemy < 0 && !(bs->flags & BFL_IDEALVIEWSET)) {
        ...
        if (bs->sess.playerType == PC_MEDIC &&
            BotHealthScale(bs->client) < 1.0) {
          // switch to regen and pump away
          bs->weaponnum = WP_MEDKIT;
          trap_EA_SelectWeapon(bs->client, bs->weaponnum);
          if (bs->cur_ps.weapon == WP_MEDKIT &&
              BotWeaponCharged(bs, WP_MEDKIT)) {
            trap_EA_Attack(bs->client);
          }
        }
      }
    ...
    gentity_t* objtarg;
    // NOTE: dont bother looking for enemies, since we have reached the
    // destination, so we are free to pursue enemies now
    //
    ...
    if ((ent->s.eType == ET_OID_TRIGGER) && (objtarg = ent->target_ent)) {
      if (objtarg->spawnflags & 64) {
        if (ent->lastHintCheckTime < level.time && rand() % 2) {
          ...
          //
          numList =
              BotGetTargetExplosives(bs->sess.sessionTeam, list, 10, qfalse);
          for (i = 0; i < numList; i++) {
          ...
          if (numList && i < numList) {
            BotVoiceChatAfterIdleTime(bs->client, "NeedEngineer", SAY_TEAM,
                                      500 + rand() % 4000, qfalse,
    ...
    if (bs->enemy >= 0) {
      ...
      if (goal.entitynum > -1 && goal.entitynum < level.maxclients &&
        ...
        if (!flaggoal) {
          flaggoal =
              BotFindNextStaticEntity(NULL, BOTSTATICENTITY_FLAGONLY_MULTIPLE);
        }
        if (flaggoal) {
          if (BotGoalForEntity(NULL, flaggoal->s.number, &fgoal, BGU_HIGH)) {
            if (trap_BotMovementViewTarget(bs->ms, &fgoal, bs->tfl, 300,
                                           target)) {
              VectorSubtract(target, bs->origin, dir);
              vectoangles(dir, bs->ideal_viewangles);
              setdir = qtrue;
...
int AINode_MP_TouchTarget(bot_state_t* bs) {
  ...
  goal = bs->target_goal;
  if (bs->alt_goal.number && bs->alt_goal.entitynum == goal.entitynum &&
      bs->alt_goal.number == goal.number) {
    if ((bs->alt_goal.number < level.time - 12000) ||
  ...
  */
  ...
  if (ent->s.eType == ET_TRAP &&
    ...
    if (ent->s.eType == ET_TRIGGER_MULTIPLE) {
      ...
      // if the current destination is no longer touching the brush, get a new
      // position
  ...
  //
  // if we have an enemy, then we should look for an alternate route away from
  // them
  if ((BotCarryingFlag(bs->client)) && (goal.number < level.time - 8000) &&
      ...
      (trap_AAS_AreaTravelTimeToGoalArea(
           bs->areanum, bs->origin, bs->target_goal.areanum, bs->tfl) > 1500)) {
    if (bs->enemy > -1) {
      ...
      if (DotProduct(edir, bs->cur_ps.velocity) > 0) {
        ...
        if ((numList = trap_AAS_AlternativeRouteGoals(
                 bs->origin, bs->target_goal.origin, bs->tfl, altroutegoals, 40,
                 0))) {
          for (i = 0; i < numList; i++) {
            BotClearGoal(&tempgoal);
            tempgoal.areanum = altroutegoals[i].areanum;
            VectorCopy(altroutegoals[i].origin, tempgoal.origin);
            if (qtrue) {  // trap_BotMovementViewTarget(bs->ms, &goal, bs->tfl,
                          // 300, target)) {
              // VectorSubtract(target, bs->origin, dir);
              ...
              if (DotProduct(dir, edir) < 0) {  // moving away from the enemy
                ...
                trap_AAS_AreaWaypoint(altroutegoals[i].areanum,
                                      bs->alt_goal.origin);
                bs->alt_goal.areanum =
                    trap_AAS_PointAreaNum(bs->alt_goal.origin);
                break;
              }
            }
          }
      ...
      int i, t;
      bot_state_t* tbs;
      // if we have defenders, stop every now and then to let them get infront
      // of us
      if ((bs->stand_time < level.time - 3000) &&
          (trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                             bs->target_goal.areanum,
                                             bs->tfl) > 600) &&
          (numList =
               BotNumTeamMatesWithTarget(bs, bs->client, list, MAX_CLIENTS))) {
        // if one of them has a goal infront of us, and they are not near it, we
        // should pause for a bit
        for (i = 0; i < numList; i++) {
          tbs = &botstates[list[i]];
          //
          if ((VectorDistanceSquared(tbs->origin, tbs->defendgoal.origin) <
               SQR(1500)) &&
              (VectorDistanceSquared(tbs->origin, bs->origin) < SQR(1024)) &&
              (trap_InPVS(tbs->origin, bs->origin)) &&
              (trap_AAS_AreaTravelTimeToGoalArea(tbs->defendgoal.areanum,
                                                 tbs->defendgoal.origin,
                                                 goal.areanum, bs->tfl) <
               trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                                 bs->target_goal.areanum,
                                                 bs->tfl)) &&
              (trap_AAS_AreaTravelTimeToGoalArea(tbs->areanum, tbs->origin,
                                                 goal.areanum, bs->tfl) -
               trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                                 goal.areanum, bs->tfl))) {
            t = trap_AAS_AreaTravelTimeToGoalArea(
                tbs->areanum, tbs->origin, tbs->defendgoal.areanum, bs->tfl);
            if (t > 300) {
              t = 300;
            }
            if (t > 100) {
  ...
  // if waiting for something
    ...
    if (bs->enemy >= 0) {
      ...
      if (bs->enemy >= 0) {
        ...
        if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360,
                             bs->enemy, NULL)) {
          /*					// if we are not going for an
             urgent goal, go into battle mode if (!BotCarryingFlag(bs->client)
             && bs->ignore_specialgoal_time < level.time && BotWantsToChase(bs)
             && (VectorDistanceSquared( goal.origin, bs->origin ) > SQR(384)) &&
                                                          (VectorDistanceSquared(
             bs->enemyorigin, bs->origin ) < SQR(2048)) &&
                                                          Q_stricmp(g_entities[goal.entitynum].classname,
             "team_CTF_redflag") &&
                                                          Q_stricmp(g_entities[goal.entitynum].classname,
             "team_CTF_blueflag")) { AIEnter_MP_Battle_Fight(bs); return qfalse;
                                                  }
          */
          // choose the best weapon to fight with
          BotChooseWeapon(bs);
...
int AINode_MP_SatchelChargeTarget(bot_state_t* bs) {
  ...
  // if there are 2 bots going for this goal, then we should abort if we are
  // further away
  goalDist = VectorDistanceSquared(bs->origin, goal.origin);
  if ((numList = BotNumTeamMatesWithTargetByClass(bs, goal.entitynum, list, 10,
                                                  PC_ENGINEER))) {
    if (goalDist > SQR(256)) {
      goalDist =
          SQR(256);  // only abort if one of our teammates is close to the goal
    }
    for (i = 0; i < numList; i++) {
      if (botstates[list[i]].ainode == AINode_MP_SatchelChargeTarget) {
        if (VectorDistanceSquared(botstates[list[i]].origin, goal.origin) <
            goalDist) {
          // make sure other bots dont head for this target also
  ...
  if (goalDist < SQR(32)) {
    ...
    if (!trav) {
      ...
      if (BotWeaponCharged(bs, WP_SATCHEL)) {
        // VOICE: cover me!
        BotVoiceChatAfterIdleTime(bs->client, "CoverMe", SAY_TEAM,
                                  500 + rand() % 1000, qfalse, 10000, qfalse);
        // make sure other bots dont head for this target also
        g_entities[bs->target_goal.entitynum].lastHintCheckTime =
            level.time + 2000;
        // crouch down
        trap_EA_Crouch(bs->client);
        // look downwards
        ...
        trap_EA_SelectWeapon(bs->client, bs->weaponnum);
        // hold fire
        if (!VectorLengthSquared(bs->velocity) &&
            fabs(bs->viewangles[PITCH] - bs->ideal_viewangles[PITCH]) < 2 &&
            bs->cur_ps.weapon == WP_SATCHEL && !bs->cur_ps.grenadeTimeLeft) {
          trap_EA_Attack(bs->client);
        }
        return qtrue;
      }
      ...
      /*			// check for emergency targets (flags, etc)
                              ...
                              // check for dangerous elements
                              if (BotDangerousGoal(bs, &goal)) {
                                      AIEnter_MP_AvoidDanger(bs);	// avoid
         this danger until it passes return qfalse;
                              }
  ...
  if (goalDist < SQR(32)) {
    ...
    if (!trav) {
      ...
      if (BotWeaponCharged(bs, WP_DYNAMITE)) {
        ...
        trap_EA_SelectWeapon(bs->client, bs->weaponnum);
        // hold fire
        if (rand() % 2 && !VectorLengthSquared(bs->velocity) &&
            fabs(bs->viewangles[PITCH] - bs->ideal_viewangles[PITCH]) < 2 &&
            bs->cur_ps.weapon == WP_DYNAMITE && !bs->cur_ps.grenadeTimeLeft) {
          trap_EA_Attack(bs->client);
        }
        return qtrue;
      }
      ...
      // if we are still in dynamite mode, then we may be trying to plant more
      // dynamite at same place
      bs->toggleHidingTime = level.time;
...
int AINode_MP_ConstructibleTarget(bot_state_t* bs) {
  ...
  if ((numList = BotNumTeamMatesWithTarget(bs, goal.entitynum, list, 10)) >
      botgoalMaxCloser[BFG_CONSTRUCT]) {
    goalDist = VectorDistanceSquared(bs->origin, goal.origin);
    if (goalDist > SQR(1024)) {
      goalDist =
          SQR(1024);  // only abort if one of our teammates is close to the goal
    }
  ...
  if (ent->client->touchingTOI && ent->client->touchingTOI == trav) {
    // if the construction has changed level, we should reset the construction
    // flag so we wait for full power again
    if ((bs->flags & BFL_MISCFLAG) && constructible->count2 &&
        !constructible->s.angles2[0]) {
      bs->flags &= ~BFL_MISCFLAG;
    }

    // if we haven't started yet, make sure we wait until we have reasonable
    // power before starting
    if (!(bs->flags & BFL_MISCFLAG)) {
      if (bs->cur_ps.classWeaponTime >
          (level.time -
           level.engineerChargeTime[bs->sess.sessionTeam - 1] / 4)) {
  ...
  goalDist = VectorDistanceSquared(bs->origin, goal.origin);
  if (moveresult.failure ||
      (VectorLengthSquared(bs->cur_ps.velocity) < SQR(3) &&
       goalDist < (128 * 128))) {  // allow for boxes at construction site that
                                   // may get in the way
    // reset the avoid reach, otherwise bot is stuck in current area
    trap_BotResetAvoidReach(bs->ms);
    // BotAI_Print(PRT_MESSAGE, "movement failure %d\n", moveresult.traveltype);
...
int AINode_MP_PlantMine(bot_state_t* bs) {
  ...
  if (VectorDistanceSquared(bs->origin, goal.origin) < SQR(32)) {
    // VOICE: cover me!
    BotVoiceChatAfterIdleTime(bs->client, "CoverMe", SAY_TEAM,
                              500 + rand() % 1000, qfalse, 60000, qfalse);
    // crouch down
    trap_EA_Crouch(bs->client);
    // look downwards
    bs->ideal_viewangles[PITCH] = 70;
    ...
    while ((trav = G_FindLandmine(trav))) {
      ...
      if (VectorDistanceSquared(bs->target_goal.origin, trav->r.currentOrigin) >
          SQR(100)) {
        continue;
      }
    ...
    if (!trav) {
      ...
      if (!VectorLengthSquared(bs->velocity) &&
          fabs(bs->viewangles[PITCH] - bs->ideal_viewangles[PITCH]) < 2 &&
          bs->cur_ps.weapon == bs->weaponnum && !bs->cur_ps.grenadeTimeLeft) {
        trap_EA_Attack(bs->client);
      }
    } else if (trav->s.teamNum >= 4) {  // the dynamite is not armed
      // switch to pliers and arm the dynamite
...
void AIEnter_MP_DisarmDynamite(bot_state_t* bs) {
  // VOICE: cover me!
  BotVoiceChatAfterIdleTime(bs->client, "CoverMe", SAY_TEAM,
                            500 + rand() % 1000, qfalse, 4000, qfalse);
  bs->target_goal.flags &= ~GFL_NOSLOWAPPROACH;
  // choose the best weapon to fight with
  BotChooseWeapon(bs);
  bs->flags &= ~BFL_MISCFLAG;
...
int AINode_MP_DisarmDynamite(bot_state_t* bs) {
  ...
  goalDist = VectorDistanceSquared(bs->origin, goal.origin);
  /*	if (numList = BotNumTeamMatesWithTarget( bs, goal.entitynum, list, 10 ))
     { for (i=0; i<numList; i++) { if (list[i] == bs->client) continue; if
     (botstates[list[i]].ainode == AINode_MP_DisarmDynamite) { if
     (VectorDistanceSquared( botstates[list[i]].origin, goal.origin ) <
     goalDist) {
                                          // make sure other bots dont head for
     this target also bs->ignore_specialgoal_time = 0;
                                          g_entities[goal.entitynum].missionLevel
     = level.time + 5000; BotDefaultNode( bs ); return qfalse;
                                  }
                          }
                  }
          }
  */                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                //
  ...
  if (!trav->inuse || trav->s.eType != ET_MISSILE ||
      trav->s.weapon != WP_DYNAMITE) {
    // it's gone, so reset goal
    bs->ignore_specialgoal_time = 0;
    BotDefaultNode(bs);
    return qfalse;
...
int AINode_MP_Battle_Fight(bot_state_t* bs) {
  ...
  if (bs->weaponnum == WP_MOBILE_MG42) {
    if (VectorDistanceSquared(bs->origin, BotGetOrigin(bs->enemy)) > SQR(700)) {
      ...
      if (tr.fraction < 1.f) {
        ...
        if (fabs(AngleDifference(mountedWeaponAngles[PITCH], ang[PITCH])) <
            15.f) {
          int oldviewheight;
          ...
          if (BotVisibleFromPos(bs->origin, bs->client, BotGetOrigin(bs->enemy),
                                bs->enemy, qtrue)) {
  ...
  areanum = BotGetArea(
      bs->enemy);  // BotPointAreaNum(entinfo.number, entinfo.origin);
  if (areanum) {
  ...
  if (BotCarryingFlag(bs->enemy) &&
      VectorDistanceSquared(bs->origin, bs->lastenemyorigin) > SQR(72)) {
    if (trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                          bs->lastenemyareanum, bs->tfl)) {
...
int AINode_MP_Battle_Chase(bot_state_t* bs) {
  ...
  if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, bs->enemy,
    ...
    // RF, if we are in the air, wait until we land (we may be climbing a
    // ladder)
    if (bs->cur_ps.groundEntityNum != ENTITYNUM_NONE ||
        bs->cur_ps.velocity[2] < 0) {
      // get real close to flag carrier
      if ((bs->weaponnum != WP_KNIFE) &&
          ((!BotCarryingFlag(bs->enemy) &&
            (enemydist < SQR(BotWeaponRange(bs, bs->weaponnum)))) ||
           (enemydist < SQR(64)))) {
  ...
  if (!BotCarryingFlag(bs->enemy)) {
    ...
    oldEnemy = bs->enemy;
    if ((bs->weaponnum != WP_KNIFE) && BotFindEnemyMP(bs, -1, qfalse) &&
        (bs->enemy != oldEnemy)) {
      AIEnter_MP_Battle_Fight(bs);
      return qfalse;
    }
  ...
  if (moveresult.flags & (MOVERESULT_MOVEMENTVIEWSET | MOVERESULT_MOVEMENTVIEW |
    ...
    // forget about our current enemy temporarily, so we can shoot at other
    // while pursuing the flag carrier
    oldenemy = bs->enemy;
    ...
    //
    if ((bs->altenemy < 0 || !BotCarryingFlag(oldenemy)) &&
...
int AINode_MP_NavigateFromVoid(bot_state_t* bs) {
  ...
  if (bs->flags & BFL_MISCFLAG) {
    // we are currently heading in a direction, verify to make sure it's still
    // valid
    if (bs->last_attackShout < level.time - 250) {
      VectorCopy(bs->lastLeaderPosition, dir);
      success = qfalse;
      if (trap_AAS_PredictClientMovement(
              &move, bs->client, bs->origin, -1, qfalse, dir, vec3_origin,
              checkFrames, 0, checkFrametime,
              SE_HITGROUNDDAMAGE | SE_HITGROUNDAREA | SE_STUCK | SE_GAP, -1,
              qfalse)) {
    ...
    for (i = 0; i < randomChecksPerSlice; i++) {
      ...
      if (success) {
        ...
        switch (move.stopevent) {
          ...
          default:
            // Gordon: OPT: bleh....
            if (bestDist < 0 || VectorDistanceSquared(bs->origin, move.endpos) <
                                    SQR(bestDist)) {
