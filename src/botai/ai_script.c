...
// action functions need to be declared here so they can be accessed in the
// scriptAction table
...
// Mad Doctor I, 10/23/2002.  When an enemy is this close, you can sense them
// outside FOV.
...
// TAT 12/6/2002 - script autonomy is used to determine if bots will break their
// scripted actions to attack or chase
...
static bot_script_stack_action_t botScriptActions[] = {
    ...
    // Mad Doctor I, 10/23/2002.  When an enemy is this close, you can sense
    // them outside FOV.
    ...
    // xkan, 10/23/2002. Rate of slowdown/interruption to normally continuous
    // firing
    ...
    // TAT 11/16/2002 - Set the selected weapon of the bot - does NOT change
    // which weapons the bot is holding
    {"SetActiveWeapon", Bot_ScriptAction_SetActiveWeapon},

    // Gordon: just need this temporarily to give the player some messages,
    // should remove later
    ...
    // TAT 12/6/2002 - script autonomy is used to determine if bots will break
    // their scripted actions to attack or chase
...
static bot_script_event_define_t botScriptEvents[] = {
    ...
    {"trigger",
     Bot_EventMatch_StringEqual},         // something has triggered us (always
                                          // followed by an identifier)
    {"pain", Bot_EventMatch_IntInRange},  // we've been hurt
    {"death", NULL},                      // RIP
    {"activate", Bot_EventMatch_StringEqual},   // "param" has just activated us
    {"objective", Bot_EventMatch_StringEqual},  // something has occured
                                                // involving the objective
...
STATIC SCRIPT DATA STRUCTURES
  ...
  Holds parsed information for each scripted character. Each bot that enters the
game chooses their scripting data from one of the characters below.
=============================
*/
...
===============
*/
qboolean Bot_EventMatch_StringEqual(bot_script_event_t* event,
                                    char* eventParm) {
  if (!event->params || !event->params[0] ||
      (eventParm && !Q_stricmp(event->params, eventParm))) {
    return qtrue;
  } else {
    return qfalse;
  }
...
void Bot_ScriptLoad(void) {
  /*
          ...
          trap_Cvar_VariableStringBuffer( "bot_scriptName", filename,
     sizeof(filename) ); if (strlen( filename ) > 0) { trap_Cvar_Register(
     &mapname, "bot_scriptName", "", CVAR_ROM ); } else { trap_Cvar_Register(
     &mapname, "mapname", "", CVAR_SERVERINFO | CVAR_ROM );
          }
...
void Bot_ScriptParseAllCharacters() {
  ...
  while (1) {
    ...
    if (token[0] == '{' || token[0] == '}') {
      G_Error(
          "Bot_ScriptParse(), Error (line %d): entry identifier expected, '%s' "
          "found.\n",
    ...
    if (numScriptCharacters == MAX_BOT_SCRIPT_CHARACTERS) {
      G_Error(
          "Bot_ScriptParse(), Error (line %d): MAX_BOT_SCRIPT_CHARACTERS "
          "exceeded (%i), too many bot script characters\n",
    ...
    if (!token[0]) {
      G_Error(
          "Bot_ScriptParse(), Error (line %d): name expected, end of line "
          "found.\n",
          1 + COM_GetCurrentParseLine());
    }
    if (token[0] == '{' || token[0] == '}') {
      G_Error(
          "Bot_ScriptParse(), Error (line %d): name expected, '%s' found.\n",
          1 + COM_GetCurrentParseLine(), token);
    }
    // allocate the name
    ...
    while ((token = COM_ParseExt(&pScript, qfalse)) && token[0]) {
      if (strlen(params) + strlen(token) >= sizeof(params)) {
        G_Error(
            "Bot_ScriptParse(), Error (line %d): parameters exceed maximum "
            "size\n",
            1 + COM_GetCurrentParseLine());
      }
      if (strlen(params) > 0) {
...
void Bot_ScriptParse(bot_script_data_t* bsd, char** text) {
  ...
  while (1) {
    ...
    if (token[0] == '}') {
      bracketLevel--;
      if (!bracketLevel) {  // we have already parsed the given bot, so get out
                            // of here
        break;
      }
      if (bracketLevel < 0) {
        G_Error(
            "Bot_ScriptParse(), Error (line %d): '%s' found, name expected\n",
            1 + COM_GetCurrentParseLine(), token);
      }
    } else if (token[0] == '{') {
      bracketLevel++;
      if (bracketLevel > 1) {
        G_Error(
            "Bot_ScriptParse(), Error (line %d): '%s' found, event name "
            "expected\n",
            1 + COM_GetCurrentParseLine(), token);
      }
      ...
      if (numEventItems >= BOT_MAX_SCRIPT_EVENTS) {
        G_Error(
            "Bot_ScriptParse(), Error (line %d): BOT_MAX_SCRIPT_EVENTS reached "
            "(%d)\n",
      ...
      while ((token = COM_Parse(text)) && (token[0] != '{')) {
        if (!token[0]) {
          G_Error(
              "Bot_ScriptParse(), Error (line %d): '}' expected, end of script "
              "found.\n",
              1 + COM_GetCurrentParseLine());
        }
      ...
      if (strlen(params)) {  // copy the params into the event
        ...
        Q_strncpyz(curEvent->params, params,
                   BOT_SIZE_STRING_POOL - strPoolCount);
        if ((strPoolCount += strlen(params) + 1) >= BOT_SIZE_STRING_POOL) {
          G_Error(
              "Bot_ScriptParse(), Error (line %d): string pool size exceeded "
              "(MAX = %i)\n",
      ...
      while ((token = COM_Parse(text)) && (token[0] != '}')) {
        ...
        for (i = 0; token[0]; i++) {
          ...
          // Special case: playsound's need to be cached on startup to prevent
          // in-game pauses
          if (i == 0) {
          ...
          if (strrchr(token, ' ')) {  // need to wrap this param in quotes since
        ...
        if (strlen(params)) {  // copy the params into the event
          ...
          Q_strncpyz(items[numItems].params, params,
                     BOT_SIZE_STRING_POOL - strPoolCount);
          if ((strPoolCount += strlen(params) + 1) >= BOT_SIZE_STRING_POOL) {
            G_Error(
                "Bot_ScriptParse(), Error (line %d): string pool size exceeded "
        ...
        if (numItems >= BOT_MAX_SCRIPT_ITEMS) {
          G_Error(
              "Bot_ScriptParse(), Error (line %d): script exceeded "
              "BOT_MAX_SCRIPT_ITEMS (%d)\n",
...
qboolean Bot_ScriptInitBot(int entnum) {
  ...
  for (i = 0; i < numScriptCharacters; i++, bsgd++) {
    ...
    while (qtrue) {
      ...
      if (token[0] != '/') {
        G_Error(
            "BotScript, line %i: condition identifier expected, '%s' found\n",
            bsgd->lineNum, token);
      }
      //
      if (!Q_stricmp(token, "/team")) {
        ...
        if (!token || !token[0] || token[0] == '/') {
          G_Error("BotScript, line %i: unexpected end of /team parameter",
                  bsgd->lineNum);
        }
        //
        if (!Q_stricmp(token, "axis")) {
          ...
          G_Error("BotScript, line %i: unknown team \"%s\"", bsgd->lineNum,
                  token);
        }
        // eliminate player
        if (bs->mpTeam != val) {
        ...
        if (!Q_stricmp(token, "/class")) {
          token = COM_ParseExt(&p, qfalse);
          if (!token || !token[0] || token[0] == '/') {
            G_Error("BotScript, line %i: unexpected end of /class parameter",
                    bsgd->lineNum);
          }
          //
          val = Team_ClassForString(token);
          if (val < 0) {
            G_Error("BotScript, line %i: unknown class \"%s\"", bsgd->lineNum,
                    token);
          }
          if (bs->mpClass != val) {
            break;
          ...
          if (!Q_stricmp(token, "/weapon")) {
            ...
            while (qtrue) {
              ...
              if (!token || !token[0] || token[0] == '/') {
                G_Error(
                    "BotScript, line %i: unexpected end of /weapon parameter",
                    bsgd->lineNum);
              }
              //
              if ((item = BG_FindItem(token))) {
                if (!item->giTag) {
                  G_Error("BotScript, line %i: unknown weapon \"%s\"",
                          bsgd->lineNum, token);
                }
            ...
            if (!(ent->client->ps.weapons[0] & weapons[0]) &&
                !(ent->client->ps.weapons[1] & weapons[1])) {
              break;
            }
          } else
            //
            if (!Q_stricmp(token, "/within_range")) {
              ...
              if (!token || !token[0] || token[0] == '/') {
                G_Error(
                    "BotScript, line %i: unexpected end of /within_range "
                    "parameter",
                    bsgd->lineNum);
              }
              trav = G_FindByTargetname(NULL, token);
              if (!trav) {
                G_Error("BotScript, line %i: unknown spawn point \"%s\"",
                        bsgd->lineNum, token);
              }
              // range
              token = COM_ParseExt(&p, qfalse);
              if (!token || !token[0] || token[0] == '/') {
                G_Error("BotScript, line %i: range expected, not found",
                        bsgd->lineNum);
              }
              //
              // eliminate players
              if (VectorDistanceSquared(ent->r.currentOrigin, trav->s.origin) >
                  SQR(atof(token))) {
                break;
              }
            }
...
void Bot_ScriptChange(bot_state_t* bs, int newScriptNum) {
  ...
  Bot_ScriptLog_Entry(
      bs, qfalse,
      Bot_LineText(bs->script.data->events[bs->script.status.eventIndex].text),
      "** NEW EVENT **\r\n");

  // try and run the script, if it doesn't finish, then abort the current script
  // (discard backup)
  if (Bot_ScriptRun(bs, qtrue)) {
    ...
    if (statusBackup.eventIndex > -1) {
      Bot_ScriptLog_Entry(
          bs, qfalse,
          Bot_LineText(bs->script.data->events[statusBackup.eventIndex].text),
          "**RESUMED**\r\n");
    }
  } else {
    // still running, previous script is terminated
    if (statusBackup.eventIndex > -1 &&
        statusBackup.eventIndex != bs->script.status.eventIndex) {
      Bot_ScriptLog_Entry(
          bs, qfalse,
          Bot_LineText(bs->script.data->events[statusBackup.eventIndex].text),
          "**TERMINATED**\r\n");
  ...
  if (g_scriptDebug.integer) {
    if (g_entities[entityNum].scriptName) {
      G_Printf("%i : (%s) GScript event: %s %s\n", level.time,
               g_entities[entityNum].scriptName, eventStr,
               params ? params : "");
    } else {
    ...
    if (bs->script.data->events[i].eventNum == eventNum) {
      if ((!bs->script.data->events[i].params) ||
          (!botScriptEvents[eventNum].eventMatch ||
           botScriptEvents[eventNum].eventMatch(&bs->script.data->events[i],
                                                params))) {
...
qboolean Bot_ScriptRun(bot_state_t* bs, qboolean force) {
  ...
  while (bs->script.status.stackHead < stack->numItems) {
    item = &bs->script.data
                ->items[stack->startIndex + bs->script.status.stackHead];
    ...
    if (bs->script.flags & BSFL_FIRST_CALL) {
      Bot_ScriptLog_Entry(
          bs, qtrue,
          Bot_LineText(
              bs->script.data->events[bs->script.status.eventIndex].text),
          "");
    }
    //
    ...
    if (G_IsSinglePlayerGame()) {
      if (bot_debug.integer) {
        trap_SendServerCommand(0, va("botdebugprint %i \"Line: %i %s %s\"",
                                     bs->client, Bot_Script_GetCurrentLine(bs),
...
void Bot_ScriptLog_Entry(bot_state_t* bs,
  ...
  if (showDetails && (Bot_Script_GetCurrentLine(bs) > -1)) {
    ...
    Q_strncpyz(text, va("(line %i:", Bot_Script_GetCurrentLine(bs)),
               sizeof(text));
    trap_FS_Write(text, strlen(text), f);
    // text
