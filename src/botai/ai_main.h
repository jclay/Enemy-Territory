...
#define BSFL_FORCED_MOVEMENT_AUTONOMY \
  8  // scripting has enforced movement autonomy
#define BSFL_FORCED_WEAPON_AUTONOMY \
  16  // scripting has enforced weapon autonomy
// START	xkan, 8/22/2002
// flags that make bots crouch/prone through scripts
#define BSFL_CROUCH 32
#define BSFL_PRONE 64
// #define BSFL_TALK					128		// flag
// to make bot play talk animation
//  END		xkan, 8/22/2002
...
//	TAT 12/5/2002
//		So the follow behavior is different, depending on what our
//leader is doing 		Possible follow modes:
typedef enum {
...
typedef struct {
  ...
  bot_script_event_t
      events[BOT_MAX_SCRIPT_EVENTS];  // contains a list of actions to perform
                                      // for each event type
} bot_script_data_t;
//
typedef struct {
...
typedef struct BotDefaultAttributes_s {
  ...
  // When an enemy is this close; we can hear them even outside our field of
  // view
  float m_closeHearingRange;
  // How far can this bot see?
  float m_visionRange;
  // This range is range to which we can see enemies; even if they are too far
  // to attack
...
typedef enum {
  ...
  BOT_COMMAND_GIVEHEALTH,   // making the medic give health command an explicit
                            // state
  BOT_COMMAND_GIVEAMMO,     // similarly, making lieutenant give ammo command
                            // explicit as well
  BOT_COMMAND_TEAM_HEALTH_AMMO,  // devote yourself to giving health or ammo to
                                 // the whole team - dependent on your class
  BOT_COMMAND_DETECTMINES,  // covert op looks for mines in a specified area
  BOT_COMMAND_DISARM,       // engineer disarm mine or dynamite
  BOT_COMMAND_ATTACK,  // an attack order, means we shouldn't break off from
                       // attacking our current target as easily
...
typedef struct {
  ...
  vec3_t autonomyPos;  // avoid it only if our autonomy position is the same as
                       // this
  int expireTime;
} botIgnoreGoal_t;
...
// TAT 1/8/2003 - we store this many of our previous seek cover seqs - used for
// retreating
...
typedef struct bot_state_s {
  ...
  //	int enemysuicide;
  ////true when the enemy of the bot suicides
  int setupcount;  // true when the bot has just been setup

  int num_deaths;  // number of time this bot died
  int num_kills;   // number of kills of this bot
  //	int revenge_enemy;
  ////the revenge enemy 	int revenge_kills;
  ////number of kills the enemy made
  ...
  //	float activatemessage_time;
  ////time to show activate message

  //	float defendaway_range;
  ////max travel time away from defend area 	float rushbaseaway_time;
  ////time away from rushing to the base
  ...
  ////last time the team mate was NOT visible
  //
  ...
  //  //time backing up towards team mate
  //
  char teamleader[32];  // netname of the team leader
  //	float askteamleader_time;
  ////time asked for team leader 	float becometeamleader_time;
  ////time the bot will become the team leader 	float teamgiveorders_time;
  ////time to give team orders
  ...
  ////netname of the team mate the bot uses for relative positioning 	float
  //formation_angle;							//angle
  //relative to the formation team mate 	vec3_t formation_dir;
  ////the direction the formation is moving in 	vec3_t formation_origin;
  ...
  int last_voice_chat;
  qboolean
      alreadySaidOutOfAmmo;  // if we just ran out of ammo, have we said it?
  //
  int last_kill_time;
  ...
  int last_heardFootSteps;  // xkan - the last time we heard people moving
                            // around
  int last_attackShout;     // xkan - the last time we shout attack/kill/etc.
  int next_nearbygoal;
  int mobileMG42ProneTime;

  int nextRetreatCheck;  // when should we check to see if we should retreat to
                         // the player -- TAT 12/12/2002
  int lastRetreatTime;   // the last time we retreated
  float retreatLikeliness;  // chance that we will retreat
  qboolean shouldRetreat;   // should we retreat to the player?		-- TAT
                            // 12/12/2002
  ...
  // TAT 12/13/2002 - Need to keep track of when the player last moved, so
  // storing their position,
  //		and we'll update the timer whenever it changes - only relevant
  //in FollowLeader
  ...
  int alertStateChangeTime;  // the time alert state was changed to the current
                             // state.
  int alertStateSetTime;  // the time we attempt to set alert state to the same
                          // state.
  ...
  // Is this bot selectable - it might be "hidden" from the player	-- TAT
  // 10/28/2002
  qboolean isBotSelectable;

  // TAT changes 8/20/2002
  //	Have we received a player command such that we should override any
  //movement scripts?
  ...
  // TAT 9/23/2002
  // keep track of recon info for a bot - have we seen an enemy, are we under
  // attack
  int reconInfo;

  int returnToLeader;   //	Return to following this leader when our current
                        //goal is completed	-- TAT 9/27/2002
  int commandedWeapon;  // if this bot has been commanded to use a particular
                        // weapon - this is it	-- TAT 11/14/2002

  // TAT 11/12/2002
  //		For Listen up/go go go
  //		We need to store info about the next command to execute
  // Do we have a queued action? what type? 0 means no action, 1 means location,
  // 2 means vchat
  int haveQueuedCmd;
  int queuedAction;  // the botaction index - also could be used to store voice
                     // chat id
  ...
  scriptAutonomy_t
      scriptAutonomy;  // how much are we allowed to break from our scripted
                       // commands?		-- TAT 12/5/2002

  followModes_t followMode;  // we don't want to switch between followmodes too
                             // frequently, so store the mode	-- TAT 12/5/2002
  int nextFollowModeTime;    // nest time to update the followMode	-- TAT
                             // 12/5/2002
  ...
  //
  float fireRate;  // the ratio of actual firing speed vs. maximum(normal)
                   // firing speed
  int fireRateCheckTime;  // the next time we should check for fire rate -xkan
  int minFireRateCycleTime;  // during a fire rate cycle, bot will fire for a
                             // duration of fireRate * cylcleTime
  int maxFireRateCycleTime;  // and will not fire for the rest of the cycle -
...
typedef qboolean (*fCoverSpotCheck)(bot_state_t* bs,
                                    int leader,
                                    vec3_t leaderPos,
...
// We need to support more than 64 bot scripts.  Note that this doesn't mean
// more than 64 simultaneous bots, but separate bot scritps.

#define MAX_BOT_SCRIPT_CHARACTERS 256

extern bot_script_global_data_t
    botCharacterScriptData[MAX_BOT_SCRIPT_CHARACTERS];
...
void Bot_ScriptLog_Entry(bot_state_t* bs,
