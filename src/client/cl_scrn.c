...
void SCR_DrawScreenField(stereoFrame_t stereoFrame) {
  ...
  /*	if ( cls.state != CA_ACTIVE ) {
                  if ( cls.glconfig.vidWidth * 480 > cls.glconfig.vidHeight *
     640 ) { re.SetColor( g_color_table[0] ); re.DrawStretchPic( 0, 0,
     cls.glconfig.vidWidth, cls.glconfig.vidHeight, 0, 0, 0, 0, cls.whiteShader
     ); re.SetColor( NULL );
                  }
          }*/
  ...
  if (!VM_Call(uivm, UI_IS_FULLSCREEN)) {
    switch (cls.state) {
      ...
      case CA_CONNECTED:
        ...
        break;
        //			// Ridah, if the cgame is valid, fall through to
        //there 			if (!cls.cgameStarted || !com_sv_running->integer) {
        //				// connecting clients will only show the
        //connection dialog 				VM_Call( uivm, UI_DRAW_CONNECT_SCREEN, qfalse );
        //				break;
        //			}
      case CA_LOADING:
      case CA_PRIMED:
