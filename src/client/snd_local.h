...
typedef struct loopSound_s {
  ...
  int vol;
  qboolean
      loudUnderWater;  // (SA) set if this sound should be played at full vol
                       // even when under water (under water loop sound for ex.)
...
typedef struct {
  ...
  int s_soundStarted;
  int s_soundMute;  // 0 - not muted, 1 - muted, 2 - no new sounds, but play out
                    // remaining sounds (so they can die if necessary)

  vec3_t entityPositions[MAX_GENTITIES];

  char nextMusicTrack[MAX_QPATH];  // extracted from CS_MUSIC_QUEUE //----(SA)
                                   // added
...
#define MAX_STREAMING_SOUNDS \
  12  // need to keep it low, or the rawsamples will get too big
#define MAX_RAW_SAMPLES 16384

extern streamingSound_t streamingSounds[MAX_STREAMING_SOUNDS];
extern int s_rawend[MAX_STREAMING_SOUNDS];
extern portable_samplepair_t s_rawsamples[MAX_STREAMING_SOUNDS]
                                         [MAX_RAW_SAMPLES];
