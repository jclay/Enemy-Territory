...
char* svc_strings[256] = {
    ...
    "svc_nop",           "svc_gamestate", "svc_configstring", "svc_baseline",
...
==================
*/
void CL_ParsePacketEntities(msg_t* msg,
  ...
  if (newSnap.deltaNum <= 0) {
    ...
    if (clc.demorecording) {
      ...
      if (cl_autorecord
        ...
        Com_sprintf(name, sizeof(name), "demos/%s_%i_%i.dm_%d", period,
                    time.tm_mday, time.tm_mon + 1, PROTOCOL_VERSION);
...
void CL_SystemInfoChanged(void) {
  ...
  // use this new serverId show_bug.cgi?id=475 in some cases, outdated cp
  // commands might get sent with this news serverId
...
void CL_ParseGamestate(msg_t* msg) {
  ...
  if (cl_connectedToPureServer && !FS_VerifyOfficialPaks()) {
    Com_Error(ERR_DROP,
              ...
              "and make sure it has been updated to the latest version.");
  }
...
void CL_ParseDownload(msg_t* msg) {
    ...
    if (!clc.bWWWDl) {
      ...
      if (clc.downloadFlags & (1 << DL_FLAG_URL)) {
        ...
        CL_AddReliableCommand(
            "wwwdl bbl8r");  // not sure if that's the right msg
      ...
      Q_strncpyz(cls.downloadTempName,
                 FS_BuildOSPath(Cvar_VariableString("fs_homepath"),
      ...
      if (!DL_BeginDownload(cls.downloadTempName, cls.downloadName,
        ...
        // that the server has sent us still leave a flag so that CL_WWWDownload
        // is inactive we count on server sending us a gamestate to start up
        // clean again
        CL_AddReliableCommand("wwwdl fail");
        clc.bWWWDlAborting = qtrue;
  ...
  if (!size) {  // A zero length block means EOF
    ...
    *cls.downloadTempName = *cls.downloadName = 0;
    ...
    // start loading right away.  If we take a while to load, the server is
    // happily trying to send us that last block over and over. Write it twice
    // to help make sure we acknowledge the download
    CL_WritePacket();
    CL_WritePacket();
...
void CL_ParseBinaryMessage(msg_t* msg) {
  ...
  CL_CGameBinaryMessageReceived(&msg->data[msg->readcount], size,
                                cl.snap.serverTime);
}
