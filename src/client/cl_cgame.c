...
extern qboolean getCameraInfo(int camNum,
                              int time,
                              vec3_t* origin,
                              vec3_t* angles,
                              float* fov);
...
qboolean CL_GetParseEntityState(int parseEntityNumber, entityState_t* state) {
  ...
  if (parseEntityNumber >= cl.parseEntitiesNum) {
    Com_Error(ERR_DROP, "CL_GetParseEntityState: %i >= %i", parseEntityNumber,
              cl.parseEntitiesNum);
  }
...
==============
*/
void CL_SetUserCmdValue(int userCmdValue,
                        int flags,
...
rescan:
  ...
  if (!strcmp(cmd, "cs")) {
    CL_ConfigstringModified();
    // reparse the string, because CL_ConfigstringModified may have done another
    // Cmd_TokenizeString()
    Cmd_TokenizeString(s);
    return qtrue;
  }
  ...
  if (!strcmp(
          cmd,
          "popup")) {  // direct server to client popup request, bypassing cgame
...
====================
*/
void CL_CGameBinaryMessageReceived(const char* buf,
                                   int buflen,
                                   int serverTime) {
...
void CL_CM_LoadMap(const char* mapname) {
  ...
  // DHM - Nerve :: If we are not running the server, then set expected usage
  // here
  if (!com_sv_running->integer) {
    CL_SetExpectedHunkUsage(mapname);
  } else {
    // TTimo
    // catch here when a local server is started to avoid outdated
    // com_errorDiagnoseIP
...
int CL_CgameSystemCalls(int* args) {
  switch (args[0]) {
    ...
    case CG_R_PROJECTDECAL:
      re.ProjectDecal(args[1], args[2], VMA(3), VMA(4), VMA(5), args[6],
                      args[7]);
      return 0;
    ...
    case CG_S_STARTBACKGROUNDTRACK:
      S_StartBackgroundTrack(VMA(1), VMA(2),
                             args[3]);  //----(SA)	added fadeup time
      return 0;
    case CG_S_FADESTREAMINGSOUND:
      S_FadeStreamingSound(
          VMF(1), args[2],
          args[3]);  //----(SA)	added music/all-streaming options
      return 0;
    ...
    case CG_R_ADDLIGHTTOSCENE:
      ...
      //		re.AddAdditiveLightToScene( VMA(1), VMF(2), VMF(3),
      //VMF(4), VMF(5) ); 		return 0;
    case CG_R_ADDCORONATOSCENE:
      re.AddCoronaToScene(VMA(1), VMF(2), VMF(3), VMF(4), VMF(5), args[6],
                          args[7]);
      return 0;
    case CG_R_SETFOG:
    ...
    case CG_R_DRAWROTATEDPIC:
      re.DrawRotatedPic(VMF(1), VMF(2), VMF(3), VMF(4), VMF(5), VMF(6), VMF(7),
                        VMF(8), args[9], VMF(10));
      return 0;
    case CG_R_DRAWSTRETCHPIC_GRADIENT:
      re.DrawStretchPicGradient(VMF(1), VMF(2), VMF(3), VMF(4), VMF(5), VMF(6),
                                VMF(7), VMF(8), args[9], VMA(10), args[11]);
      return 0;
...
CL_UpdateLevelHunkUsage
  ...
  This updates the "hunkusage.dat" file with the current map and it's hunk usage
count

  This is used for level loading, so we can show a percentage bar dependant on
the amount of hunk memory allocated so far

  This will be slightly inaccurate if some settings like sound quality are
changed, but these things should only account for a small variation (hopefully)
====================
*/
  ...
  if (len >=
    ...
    while ((token = COM_Parse(&buftrav)) != NULL && token[0]) {
      if (!Q_stricmp(token, cl.mapname)) {
        ...
        if (token && token[0]) {
          if (atoi(token) ==
              memusage) {  // if it is the same, abort this process
            Z_Free(buf);
            Z_Free(outbuf);
            return;
          }
...
void CL_InitCGame(void) {
  ...
  // bani - added clc.demoplaying, since some mods need this at init time, and
  // drawactiveframe is too late for them
  VM_Call(cgvm, CG_INIT, clc.serverMessageSequence,
          clc.lastExecutedServerCommand, clc.clientNum, clc.demoplaying);
