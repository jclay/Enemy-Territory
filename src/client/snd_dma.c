...
void S_ClearSounds(qboolean clearStreaming,
                   qboolean clearMusic);  //----(SA)	modified
...
cvar_t* s_separation;
cvar_t* s_doppler;
cvar_t* s_mute;  // (SA) for DM so he can 'toggle' sound on/off without
                 // disturbing volume levels
cvar_t*
    s_defaultsound;  // (SA) added to silence the default beep sound if desired
...
// RF, store the volumes, since now they get adjusted at time of painting, so we
// can extract talking data first
...
#ifdef _DEBUG
  if (*(channel_t**)snd.freelist > &s_channels[MAX_CHANNELS] ||
      *(channel_t**)snd.freelist < &s_channels[0]) {  // DAJ	extra check
    Com_DPrintf("s_channel OUT OF BOUNDS\n");
    return NULL;
  }
#endif
...
void S_SpatializeOrigin(vec3_t origin,
  ...
  //	dist_mult = range*0.00000064f;		// default range of 1250 gives
  //.0008
  ...
  if (dma.channels == 1 ||
      noAttenuation) {  // no attenuation = no spatialization
    rscale = 1.0;
    lscale = 1.0;
  } else {
...
Entchannel 0 will never override a playing sound
  ...
  flags:  (currently apply only to non-looping sounds)
        SND_NORMAL			    0	- (default) allow sound to be
cut off only by the same sound on this channel SND_OKTOCUT
0x001	- allow sound to be cut off by any following sounds on this channel
        SND_REQUESTCUT		0x002	- allow sound to be cut off by following
sounds on this channel only for sounds who request cutoff SND_CUTOFF
0x004	- cut off sounds on this channel that are marked 'SND_REQUESTCUT'
        SND_CUTOFF_ALL		0x008	- cut off all sounds on this channel
====================
*/
void S_ThreadStartSoundEx(vec3_t origin,
                          int entityNum,
                          int entchannel,
...
void S_StartSoundEx(vec3_t origin,
                    int entityNum,
                    int entchannel,
                    sfxHandle_t sfxHandle,
                    int flags,
  ...
  // RF, we have lots of NULL sounds using up valuable channels, so just ignore
  // them
  /*	if( !sfxHandle && entchannel != CHAN_WEAPON ) {	// let null weapon
     sounds try to play.  they kill any weapon sounds playing when a guy dies
                  Com_Printf( "^1WARNING: NULL sfx handle\n" );
                  return;
          }*/

  // RF, make the call now, or else we could override following streaming sounds
  // in the same frame, due to the delay
...
void S_ThreadStartSoundEx(vec3_t origin,
  ...
  if (entityNum >= 0) {
    for (i = 1; i < MAX_STREAMING_SOUNDS; i++) {  // track 0 is music/cinematics
      ...
      // check to see if this character currently has another sound streaming on
      // the same channel
      if ((entchannel != CHAN_AUTO) && (streamingSounds[i].entnum >= 0) &&
          (streamingSounds[i].channel == entchannel) &&
          (streamingSounds[i].entnum == entityNum)) {
  ...
  for (i = 0; i < MAX_CHANNELS; i++) {
    if (s_channels[i].entnum == entityNum && s_channels[i].thesfx &&
      ...
      // TAT 11/2/2002 - don't do this, then if 2 bots try to talk at the same
      // time, the 1st voice goes away.
      //		multiple people are allowed to talk at the same time

      // xkan, 11/21/2002 - restore this for all non-zero clients, for client 0,
      // we sometimes do get (and want) multiple sounds on him.
      //
      /*if(entityNum != 0 &&
         entityNum < MAX_CLIENTS && s_channels[i].entchannel != CHAN_AUTO &&
      s_channels[i].entchannel != CHAN_WEAPON) { S_ChannelFree(&s_channels[i]);
              continue;
      }*/
  ...
  for (i = 0; i < MAX_CHANNELS; i++) {
    if (s_channels[i].entnum == entityNum &&
        s_channels[i].entchannel == entchannel && entchannel != CHAN_AUTO) {
...
#ifdef _DEBUG
  if (ch > &s_channels[MAX_CHANNELS] ||
      ch < &s_channels[0]) {  // DAJ	extra check
    Com_DPrintf("s_channel OUT OF BOUNDS\n");
    Sys_LeaveCriticalSection(crit);
    return;
  }
  ...
  if (ch->fixed_origin) {
    S_SpatializeOrigin(ch->origin, ch->master_vol, &ch->leftvol, &ch->rightvol,
                       SOUND_RANGE_DEFAULT, flags & SND_NO_ATTENUATION);
  } else {
    S_SpatializeOrigin(snd.entityPositions[ch->entnum], ch->master_vol,
                       &ch->leftvol, &ch->rightvol, SOUND_RANGE_DEFAULT,
                       flags & SND_NO_ATTENUATION);
  }
...
void S_ClearSoundBuffer(qboolean killStreaming) {
  ...
  S_ClearSounds(killStreaming,
                qtrue);  // do this now since you might not be allowed to in a
                         // sec (no multi-threaeded)
}
...
void S_StopAllSounds(void) {
  ...
  for (i = 1; i < MAX_STREAMING_SOUNDS;
       i++) {  // DAJ numStreamingSounds can get bigger than the MAX array size
    streamingSounds[i].kill = 1;
  }
  Sys_LeaveCriticalSection(crit);
...
void S_AddLoopingSound(const vec3_t origin,
  ...
  // ydnar: allow looped sounds to start when initially triggered, rather than
  // in the middle of the sample
  snd.loopSounds[snd.numLoopSounds].startSample =
      sfx->soundLength
  ...
  snd.loopSounds[snd.numLoopSounds].startSample =
      sfx->soundLength
          ? s_paintedtime -
                ((int)(dma.speed * (cl.serverTime - soundTime) / 1000.0f))
          : 0;
  ...
  snd.loopSounds[snd.numLoopSounds].vol =
      (int)((float)volume * snd.volCurrent);  //----(SA)	modified
  ...
  if (s_doppler->integer && VectorLengthSquared(velocity) > 0.0) {
    ...
    lena = DistanceSquared(snd.entityPositions[listener_number],
                           snd.loopSounds[snd.numLoopSounds].origin);
    VectorAdd(snd.loopSounds[snd.numLoopSounds].origin,
              snd.loopSounds[snd.numLoopSounds].velocity, out);
    ...
    if ((snd.loopSounds[snd.numLoopSounds].framenum + 1) != cls.framecount) {
      ...
      snd.loopSounds[snd.numLoopSounds].oldDopplerScale =
          snd.loopSounds[snd.numLoopSounds].dopplerScale;
    }
    snd.loopSounds[snd.numLoopSounds].dopplerScale = lenb / (lena * 100);
    if (snd.loopSounds[snd.numLoopSounds].dopplerScale <= 1.0) {
      snd.loopSounds[snd.numLoopSounds].doppler =
...
void S_AddRealLoopingSound(const vec3_t origin,
  ...
  snd.loopSounds[ snd.numLoopSounds ].startSample = sfx->soundLength
          ?  s_paintedtime - ((int) (dma.speed * (cl.serverTime - soundTime) /
  1000.0f)) : 0;

  snd.loopSounds[ snd.numLoopSounds ].startSample = soundTime %
  sfx->soundLength;*/
...
void S_AddLoopSounds(void) {
  ...
  for (i = 0; i < snd.numLoopSounds; i++) {
    ...
    for (j = (i + 1); j < MAX_GENTITIES; j++) {
      ...
      if (loop2->kill) {
        S_SpatializeOrigin(loop2->origin, 127, &left, &right, loop2->range,
                           qfalse);  // 3d
      } else {
        S_SpatializeOrigin(loop2->origin, 90, &left, &right, loop2->range,
                           qfalse);  // sphere
      }
...
=================
*/
// DAJ void S_ByteSwapRawSamples( int samples, int width, int s_channels, const
// byte *data ) {
  ...
  if (s_channels == 2 && width == 2) {
    if (scale == 1.0) {  // optimized case
      for (i = 0; i < samples; i++) {
        ...
        s_rawsamples[streamingIndex][dst].left =
            ((short*)data)[i * 2] * intVolumeL;
        s_rawsamples[streamingIndex][dst].right =
            ((short*)data)[i * 2 + 1] * intVolumeR;
      }
    } else {
      for (i = 0;; i++) {
        src = i * scale;
        ...
        s_rawend[streamingIndex]++;
        s_rawsamples[streamingIndex][dst].left =
            ((short*)data)[src * 2] * intVolumeL;
        s_rawsamples[streamingIndex][dst].right =
            ((short*)data)[src * 2 + 1] * intVolumeR;
      }
    }
    ...
    for (i = 0;; i++) {
      ...
      s_rawend[streamingIndex]++;
      s_rawsamples[streamingIndex][dst].left =
          ((char*)data)[src * 2] * intVolumeL;
      s_rawsamples[streamingIndex][dst].right =
          ((char*)data)[src * 2 + 1] * intVolumeR;
    }
    ...
    for (i = 0;; i++) {
      ...
      s_rawend[streamingIndex]++;
      s_rawsamples[streamingIndex][dst].left =
          (((byte*)data)[src] - 128) * intVolumeL;
      s_rawsamples[streamingIndex][dst].right =
          (((byte*)data)[src] - 128) * intVolumeR;
    }
  }
...
============
*/
void S_Respatialize(int entityNum,
                    const vec3_t head,
                    vec3_t axis[3],
                    int inwater) {
  if (!snd.s_soundStarted || (snd.s_soundMute == 1)) {
  ...
  for (i = 0; i < MAX_CHANNELS; i++, ch++) {
    ...
    if (ch->entnum == listener_number) {
      ...
      S_SpatializeOrigin(origin, ch->master_vol, &ch->leftvol, &ch->rightvol,
                         SOUND_RANGE_DEFAULT, ch->flags & SND_NO_ATTENUATION);
    }
  }
}
...
int S_CheckForQueuedMusic(void) {
  ...
  if (snd.nextMusicTrackType == QUEUED_PLAY_ONCE_SILENT) {
    ...
    S_StartBackgroundTrack(
        nextMusicVA, ss->name,
        0);  // play once, then go back to looping what's currently playing
    ...
    for (i = 0; i < MAX_CHANNELS; i++, ch++) {
      if (ch->thesfx && (ch->leftvol || ch->rightvol)) {
        Com_Printf("%i %i %s\n", ch->leftvol, ch->rightvol,
                   ch->thesfx->soundName);  // <- this is not thread safe
        total++;
      }
    }
...
void S_ClearSounds(qboolean clearStreaming, qboolean clearMusic) {
  ...
  // RF, moved this up so streaming sounds dont get updated with the music,
  // below, and leave us with a snippet off streaming sounds after we reload
  if (clearStreaming) {  // we don't want to stop guys with long dialogue from
                         // getting cut off by a file read
    ...
    // RF, we should also kill all channels, since we are killing streaming
    // sounds anyway (fixes siren in forest playing after a map_restart/loadgame
    ch = s_channels;
    for (i = 0; i < MAX_CHANNELS; i++, ch++) {
      if (ch->thesfx) {
        S_ChannelFree(ch);
  ...
  if (!clearMusic) {
    S_UpdateStreamingSounds();  //----(SA)	added so music will get updated
                                //if not cleared
  } else {
    // music cleanup
  ...
  if (clearStreaming && clearMusic) {
    ...
    // NERVE - SMF - clear out channels so they don't finish playing when audio
    // restarts
    S_ChannelSetup();
  } else {
    Sys_LeaveCriticalSection(crit);
...
void S_UpdateThread(void) {
  ...
  if (snd.s_clearSoundBuffer) {
    S_ClearSounds(qtrue,
                  (qboolean)(snd.s_clearSoundBuffer >= 4));  //----(SA)	modified
...
void S_Update_Mix(void) {
  ...
  // RF, this isn't used anymore, since it was causing timing problems with
  // streaming sounds, since the starting of the sound is delayed, it could
  // cause streaming sounds to be cutoff, when the steaming sound was issued
  // after this sound
  /*
          for(i=0;i<snd.tart;i++) {
                  if (snd.pushPop[i].fixedOrigin) {
                          S_ThreadStartSoundEx(snd.pushPop[i].origin,
     snd.pushPop[i].entityNum, snd.pushPop[i].entityChannel, snd.pushPop[i].sfx,
     snd.pushPop[i].flags, pushPop[i].volume ); } else {
                          S_ThreadStartSoundEx(NULL, snd.pushPop[i].entityNum,
     snd.pushPop[i].entityChannel, snd.pushPop[i].sfx, snd.pushPop[i].flags,
     pushPop[i].volume );
                  }
          }
  ...
  */
  ...
  if (s_soundtime < snd.volTime2) {    // still has fading to do
    if (s_soundtime > snd.volTime1) {  // has started fading
      snd.volFadeFrac = ((float)(s_soundtime - snd.volTime1) /
                         (float)(snd.volTime2 - snd.volTime1));
      snd.volCurrent = ((1.0 - snd.volFadeFrac) * snd.volStart +
                        snd.volFadeFrac * snd.volTarget);
...
void S_QueueMusic_f(void) {
  ...
  // NOTE: could actually use this to touch the file now so there's not a hit
  // when the queue'd music is played?
...
void S_Music_f(void) {
  ...
  if (c == 2) {
    ...
    Q_strncpyz(streamingSounds[0].loop, Cmd_Argv(2),
               sizeof(streamingSounds[0].loop));
  } else {
...
void S_StreamingSound_f(void) {
  ...
  if (c == 2) {
    ...
    S_StartStreamingSound(Cmd_Argv(1), 0, atoi(Cmd_Argv(2)), atoi(Cmd_Argv(3)),
                          atoi(Cmd_Argv(4)));
  } else {
...
======================
*/
void S_StartBackgroundTrack(const char* intro,
                            const char* loop,
                            int fadeupTime) {
  int len;
  char dump[16];
  ...
  //----(SA)	added
  if (fadeupTime <
      0) {  // queue, don't play until music fades to 0 or is stopped
    ...
    if (intro && strlen(intro)) {
      ...
      if (fadeupTime == -2) {
        Cvar_Set(
            "s_currentMusic",
            intro);  //----(SA)	so the savegame will have the right music
      }
      ...
      snd.nextMusicTrack[0] =
          0;  // clear out the next track so things go on as they are
      snd.nextMusicTrackType = 0;  // be quiet at the next opportunity

      // clear out looping sound in current music so that it'll stop when it's
      // done
      if (ss && ss->loop) {
  ...
  Cvar_Set("s_currentMusic",
           "");  //----(SA)	so the savegame will have the right music

  if (!Q_stricmp(loop, "onetimeonly")) {  // don't change the loop if you're
                                          // playing a single hit
  ...
  if (s_debugMusic->integer) {
    Com_Printf("MUSIC: StartBgTrack:\n   playing %s\n   looping %s %d\n", intro,
               loopMusic, fadeupTime);
  }

  Cvar_Set(
      "s_currentMusic",
      loopMusic);  //----(SA)	so the savegame will have the right music
  ...
  if (fadeupTime) {
    ...
    ss->fadeEnd =
        s_soundtime + (((float)(ss->info.rate) / 1000.0f) * fadeupTime);
    //		ss->fadeStart		= s_paintedtime;
    //		ss->fadeEnd			= s_paintedtime +
    //(((float)(ss->info.rate)/1000.0f ) * fadeupTime);
...
void S_FadeAllSounds(float targetVol, int time, qboolean stopsounds) {
  // TAT 11/15/2002
  //		Because of strange timing issues, sometimes we try to fade up
  //before the fade down completed 		If that's the case, just force an immediate
  //stop to all sounds
...
void S_FadeStreamingSound(float targetVol, int time, int ssNum) {
  ...
  if (ss->fadeStart) {
    if (ss->fadeEnd <= s_soundtime) {
      ...
      ss->fadeStartVol = ((float)(s_soundtime - ss->fadeStart) /
                          (float)(ss->fadeEnd - ss->fadeStart));
    }
    //			ss->fadeStartVol = ( (float)(s_paintedtime -
    //ss->fadeStart)/(float)(ss->fadeEnd - ss->fadeStart) );
  }
  ...
  //	ss->fadeStart		= s_paintedtime;
  //	ss->fadeEnd			= s_paintedtime +
  //(((float)(ss->info.rate)/1000.0f ) * time);
...
float S_GetStreamingFade(streamingSound_t* ss) {
  ...
  newfrac = (float)(s_soundtime - ss->fadeStart) /
            (float)(ss->fadeEnd - ss->fadeStart);
  //	newfrac = (float)(s_paintedtime - ss->fadeStart)/(float)(ss->fadeEnd -
  //ss->fadeStart);
...
======================
*/
float S_StartStreamingSound(const char* intro,
                            const char* loop,
                            int entnum,
                            int channel,
                            int attenuation) {
  ...
  if (!crit || !snd.s_soundStarted || snd.s_soundMute ||
      cls.state != CA_ACTIVE) {
    return 0;
  }
  ...
  Com_DPrintf("S_StartStreamingSound( %s, %s, %i, %i, %i )\n", intro, loop,
              entnum, channel, attenuation);

  // look for a free track, but first check for overriding a currently playing
  // sound for this entity
  ss = NULL;
  if (entnum >= 0) {
    for (i = 1; i < MAX_STREAMING_SOUNDS; i++) {  // track 0 is music/cinematics
      if (!streamingSounds[i].file) {
      ...
      // check to see if this character currently has another sound streaming on
      // the same channel
      if ((channel != CHAN_AUTO) && (streamingSounds[i].entnum >= 0) &&
          (streamingSounds[i].channel == channel) &&
...
void S_UpdateStreamingSounds(void) {
  ...
  soundMixAheadTime = s_soundtime;  // + (int)(0.35 * dma.speed);	// allow
                                    // for talking animations

  snd.s_soundPainted = qtrue;

  for (i = 0, ss = streamingSounds, re = s_rawend, rp = s_rawpainted;
       i < MAX_STREAMING_SOUNDS; i++, ss++, re++, rp++) {
    if (ss->kill && ss->file) {
      fileHandle_t file;
      file = ss->file;
      ss->file = 0;
      ...
      if (i == 0 || ss->kill == 2) {  //  kill whole channel /now/
                                      //				memset( &s_rawsamples[i], 0,
        //MAX_RAW_SAMPLES*sizeof(portable_samplepair_t) );
        *re = 0;  // reset rawend
      }
      ss->kill = 0;
      continue;
    ...
    *rp = qfalse;
    ...
    if (*re < soundMixAheadTime) {  // RF, read a bit ahead of time to allow for
                                    // talking animations
    ...
    while (*re < soundMixAheadTime + MAX_RAW_SAMPLES) {
      ...
      // byte swap if needed
      S_ByteSwapRawSamples(fileSamples, ss->info.width, ss->info.channels,
                           (short*)raw);
      ...
      if (s_mute->value) {  //----(SA)	sound is muted.  process to maintain
                            //timing, but play at 0 volume
        streamingVol = 0;
      }
      ...
      if (i == 0) {  // music
        ...
        if (ss->entnum >= 0 && ss->attenuation) {
          int r, l;
          S_SpatializeOrigin(snd.entityPositions[ss->entnum],
                             s_volume->value * 255.0f, &l, &r,
                             SOUND_RANGE_DEFAULT, qfalse);
      ...
      // add to raw buffer
      S_RawSamples(fileSamples, ss->info.rate, ss->info.width,
                   ss->info.channels, raw, lvol, rvol, i);
      ...
      if (!ss->samples) {  // at the end of the sound
        ...
        if (i == 0 && snd.nextMusicTrackType) {  // queued music is queued
          if (ss->file) {
            ...
            numStreamingSounds--;
            //						memset(
            //&s_rawsamples[i], 0, MAX_RAW_SAMPLES*sizeof(portable_samplepair_t)
            //);	// really clear it
            s_rawend[i] = 0;  // reset rawend
          }
          /*
                                                  nextMusicVA = va("%s",
             snd.nextMusicTrack); if(snd.nextMusicTrackType == QUEUED_PLAY_ONCE)
             { S_StartBackgroundTrack( nextMusicVA, ss->name, 0);
             // play once, then go back to looping what's currently playing }
             else {	// QUEUED_PLAY_LOOPED S_StartBackgroundTrack(
             nextMusicVA, nextMusicVA, 0);	// take over
                                                  }
                                                  snd.nextMusicTrack[0] = 0;
             // clear out music queue
          */
          break;  // this is now the music ss->file, no need to re-start next
                  // time through
        } else {
          // loop
          if (ss->loop && ss->loop[0]) {
            if (ss->looped) {
              char dump[16];
              Sys_StreamSeek(ss->file, 0,
                             FS_SEEK_SET);  // just go back to the beginning
              ...
              ss->looped = qtrue;  // this is now the music ss->file, no need to
                                   // re-start next time through
              break;
            }
            ...
            if (i == 0) {
              Cvar_Set("s_currentMusic",
                       "");  //----(SA)	so the savegame know's it's supposed to
                             //be quiet
