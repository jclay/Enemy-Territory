...
void IN_Notebook(void) {
  ...
  // VM_Call( uivm, UI_SET_ACTIVE_MENU, UIMENU_NOTEBOOK);	// startup
  // notebook
  // }
}
...
void CL_KeyMove(usercmd_t* cmd) {
  ...
  if (com_frameTime - cl.doubleTap.lastdoubleTap >
      cl_doubletapdelay->integer + 150 +
          cls.frametime) {  // double tap only once every 500 msecs (add
...
void CL_JoystickMove(usercmd_t* cmd) {
  ...
  if (!kb[KB_STRAFE].active) {
    cl.viewangles[YAW] +=
        anglespeed * cl_yawspeed->value * cl.joystickAxis[AXIS_SIDE];
  } else {
    cmd->rightmove = ClampChar(cmd->rightmove + cl.joystickAxis[AXIS_SIDE]);
  }
#endif
  if (kb[KB_MLOOK].active) {
    cl.viewangles[PITCH] +=
        anglespeed * cl_pitchspeed->value * cl.joystickAxis[AXIS_FORWARD];
  } else {
    cmd->forwardmove =
...
void CL_MouseMove(usercmd_t* cmd) {
  ...
  /*	NERVE - SMF - this has moved to CG_CalcFov to fix zoomed-in/out
     transition movement bug if ( cl.snap.ps.stats[STAT_ZOOMED_VIEW] ) {
...
void CL_WritePacket(void) {
  ...
  // duplicates typically several unacknowledged cp or userinfo commands stacked
  // up
...
void CL_InitInput(void) {
  //	Cmd_AddCommand ("centerview",IN_CenterView);	// this is an exploit
  //nowadays
