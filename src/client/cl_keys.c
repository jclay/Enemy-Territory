...
===================
*/
void Field_VariableSizeDraw(field_t* edit,
                            int x,
                            int y,
                            int width,
                            int size,
...
void Field_BigDraw(field_t* edit,
                   int x,
                   int y,
                   int width,
                   qboolean showCursor) {
...
void Field_KeyDownEvent(field_t* edit, int key) {
  ...
  if (key == K_HOME || key == K_KP_HOME ||
      (tolower(key) == 'a' && keys[K_CTRL].down)) {
    edit->cursor = 0;
    return;
  }

  if (key == K_END || key == K_KP_END ||
      (tolower(key) == 'e' && keys[K_CTRL].down)) {
    edit->cursor = len;
    return;
  }
...
static void CompleteCommand(void) {
  ...
  if (!con.acLength) {
    ...
    if (completionString[0] == '\\' || completionString[0] == '/') {
      ...
      // Q_strncpyz( completionString, completionString+1,
      // sizeof(completionString) );
      memmove(completionString, completionString + 1,
              sizeof(completionString) - 1);
    }
...
void Console_Key(int key) {
  ...
  if (key == K_ENTER || key == K_KP_ENTER) {
    ...
    if (cls.state != CA_ACTIVE && g_consoleField.buffer[0] != '\\' &&
      ...
      Com_sprintf(g_consoleField.buffer, sizeof(g_consoleField.buffer), "\\%s",
                  temp);
...
void CL_KeyEvent(int key, qboolean down, unsigned time) {
  ...
  if (cl.cameraMode) {
    if (!(cls.keyCatchers &
      ...
      // in cutscenes we need to handle keys specially (pausing not allowed in
      // camera mode)
      if ((key == K_ESCAPE || key == K_SPACE || key == K_ENTER) && down) {
  ...
  if (cl_bypassMouseInput && cl_bypassMouseInput->integer) {
    ...
    } else if ((cls.keyCatchers & KEYCATCH_UI && !UI_checkKeyExec(key)) ||
               (cls.keyCatchers & KEYCATCH_CGAME &&
                !CL_CGameCheckKeyExec(key))) {
      bypassMenu = qtrue;
    }
  }
