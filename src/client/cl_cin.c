...
//-----------------------------------------------------------------------------
long RllDecodeMonoToMono(unsigned char* from,
                         short* to,
                         unsigned int size,
                         char signedOutput,
                         unsigned short flag) {
...
// Returns:		Number of samples placed in output buffer
//-----------------------------------------------------------------------------
long RllDecodeMonoToStereo(unsigned char* from,
                           short* to,
                           unsigned int size,
                           char signedOutput,
                           unsigned short flag) {
...
// Returns:		Number of samples placed in output buffer
//-----------------------------------------------------------------------------
long RllDecodeStereoToStereo(unsigned char* from,
                             short* to,
                             unsigned int size,
                             char signedOutput,
                             unsigned short flag) {
...
// Returns:		Number of samples placed in output buffer
//-----------------------------------------------------------------------------
long RllDecodeStereoToMono(unsigned char* from,
                           short* to,
                           unsigned int size,
                           char signedOutput,
                           unsigned short flag) {
        ...
        for (i = 0; i < 4; i++) {
          ...
          switch (code) {  // code in top two bits of code
            ...
            case 0xc000:  // 2x2 vq code
              ...
              data++;
              blit2_32((byte*)&vq2[(*data) * 8], status[index] + spl * 2 + 8,
                       spl);
              data++;
              break;
...
static void recurseQuad(long startX,
                        long startY,
                        long quadSize,
                        long xOff,
                        long yOff) {
...
static void readQuadInfo(byte* qData) {
  ...
  cinTable[currentHandle].t[0] = (0 - (unsigned int)cin.linbuf) +
                                 (unsigned int)cin.linbuf +
                                 cinTable[currentHandle].screenDelta;
  cinTable[currentHandle].t[1] =
      (0 - ((unsigned int)cin.linbuf + cinTable[currentHandle].screenDelta)) +
      (unsigned int)cin.linbuf;
...
static void RoQInterrupt(void) {
  ...
  Sys_StreamedRead(cin.file, cinTable[currentHandle].RoQFrameSize + 8, 1,
...
redump:
  ...
  //	r = Sys_StreamedRead( cin.file, cinTable[currentHandle].RoQFrameSize+8,
  //1, cinTable[currentHandle].iFile );
...
static void RoQShutdown(void) {
  ...
  if (cinTable[currentHandle].alterGameState) {
    ...
    if (s[0]) {
      ...
      ) { Cvar_Set( "nextmap", "cinematic sdlogo.roq" );	// FIXME: sd
      logo Cvar_Set( "com_logosPlaying", "0" ); } else {*/
      Cvar_Set("nextmap", "");
      //}
    }
    CL_handle = -1;
...
==================
*/
int CIN_PlayCinematic(const char* arg,
                      int x,
                      int y,
                      int w,
                      int h,
  ...
  cinTable[currentHandle].ROQSize = FS_FOpenFileRead(
      cinTable[currentHandle].fileName, &cinTable[currentHandle].iFile, qtrue);
...
void CIN_UploadCinematic(int handle) {
  if (handle >= 0 && handle < MAX_VIDEO_HANDLES) {
    ...
    re.UploadCinematic(256, 256, 256, 256, cinTable[handle].buf, handle,
