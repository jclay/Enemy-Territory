...
===================
*/
void S_SetVoiceAmplitudeFrom16(const sfx_t* sc,
                               int sampleOffset,
                               int count,
                               int entnum) {
  ...
  //  adjust the sfx_count according to the frametime (scale down for longer
  //  frametimes)
...
===================
*/
void S_SetVoiceAmplitudeFromADPCM(const sfx_t* sc,
                                  int sampleOffset,
                                  int count,
                                  int entnum) {
  int data, i, sfx_count;
...
===================
*/
void S_SetVoiceAmplitudeFromWavelet(const sfx_t* sc,
                                    int sampleOffset,
                                    int count,
                                    int entnum) {
  int data, i, sfx_count;
...
===================
*/
void S_SetVoiceAmplitudeFromMuLaw(const sfx_t* sc,
                                  int sampleOffset,
                                  int count,
                                  int entnum) {
  int data, i, sfx_count;
...
===================
*/
static void S_PaintChannelFrom16(channel_t* ch,
                                 const sfx_t* sc,
                                 int count,
                                 int sampleOffset,
                                 int bufferOffset) {
...
===================
*/
void S_PaintChannelFromWavelet(channel_t* ch,
                               sfx_t* sc,
                               int count,
                               int sampleOffset,
                               int bufferOffset) {
...
===================
*/
void S_PaintChannelFromADPCM(channel_t* ch,
                             sfx_t* sc,
                             int count,
                             int sampleOffset,
                             int bufferOffset) {
...
===================
*/
void S_PaintChannelFromMuLaw(channel_t* ch,
                             sfx_t* sc,
                             int count,
                             int sampleOffset,
                             int bufferOffset) {
  ...
  while (s_paintedtime < endtime) {
    ...
    for (si = 0, ss = streamingSounds; si < MAX_STREAMING_SOUNDS; si++, ss++) {
      ...
      if (s_rawend[si] >= s_paintedtime) {
        ...
        // DAJ				fsir =
        // ((float)s_rawVolume[si].left/255.0f); DAJ
        // fsil = ((float)s_rawVolume[si].right/255.0f);
        for (i = s_paintedtime; i < stop; i++) {
          s = i & (MAX_RAW_SAMPLES - 1);
          // DAJ
          // paintbuffer[i-s_paintedtime].left +=
          // (int)((float)s_rawsamples[si][s].left * fsir); DAJ
          // paintbuffer[i-s_paintedtime].right +=
          // (int)((float)s_rawsamples[si][s].right * fsil); DAJ even faster
          paintbuffer[i - s_paintedtime].left +=
              (s_rawsamples[si][s].left * s_rawVolume[si].left) >> 8;
          paintbuffer[i - s_paintedtime].right +=
              (s_rawsamples[si][s].right * s_rawVolume[si].right) >> 8;
        }
#ifdef TALKANIM
        if (firstPass && ss->channel == CHAN_VOICE &&
            ss->entnum < MAX_CLIENTS) {
          int talkcnt, talktime;
          int sfx_count, vstop;
          int data;

          // we need to go into the future, since the interpolated behaviour of
          // the facial animation creates lag in the time it takes to display
          // the current facial frame
          talktime = s_paintedtime +
                     (int)(TALK_FUTURE_SEC * (float)s_khz->integer * 1000);
          vstop =
              (talktime + 100 < s_rawend[si]) ? talktime + 100 : s_rawend[si];
          talkcnt = 1;
          sfx_count = 0;
    ...
    for (i = 0; i < MAX_CHANNELS; i++, ch++) {
      if (ch->startSample == START_SAMPLE_IMMEDIATE || !ch->thesfx ||
      ...
      if (count > 0) {
        ...
        if (firstPass && ch->entchannel == CHAN_VOICE &&
            ch->entnum < MAX_CLIENTS) {
          int talkofs, talkcnt, talktime;
          // we need to go into the future, since the interpolated behaviour of
          // the facial animation creates lag in the time it takes to display
          // the current facial frame
          talktime =
              ltime + (int)(TALK_FUTURE_SEC * (float)s_khz->integer * 1000);
    ...
    for (i = 0; i < numLoopChannels; i++, ch++) {
      ...
      do {
        ...
        //%	sampleOffset = (ltime - ch->startSample) % sc->soundLength;
        //// ydnar
        sampleOffset =
            (ltime /*- ch->startSample*/) % sc->soundLength;  // ydnar
        ...
        if (count > 0) {
          ...
          if (firstPass && ch->entchannel == CHAN_VOICE &&
              ch->entnum < MAX_CLIENTS) {
            int talkofs, talkcnt, talktime;
            // we need to go into the future, since the interpolated behaviour
            // of the facial animation creates lag in the time it takes to
            // display the current facial frame
            talktime =
                ltime + (int)(TALK_FUTURE_SEC * (float)s_khz->integer * 1000);
            ...
            if (talkofs + talkcnt < sc->soundLength) {
              if (sc->soundCompressionMethod == 1) {
                ...
                S_SetVoiceAmplitudeFromWavelet(sc, talkofs, talkcnt,
                                               ch->entnum);
              } else if (sc->soundCompressionMethod == 3) {
                S_SetVoiceAmplitudeFromMuLaw(sc, talkofs, talkcnt, ch->entnum);
              } else {
