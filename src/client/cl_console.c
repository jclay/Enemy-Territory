...
void Con_Init(void) {
  ...
  con_notifytime = Cvar_Get("con_notifytime", "7",
                            0);  // JPW NERVE increased per id req for obits
...
void Con_DrawNotify(void) {
  ...
  for (i = con.current - NUM_CON_TIMES + 1; i <= con.current; i++) {
    ...
    for (x = 0; x < con.linewidth; x++) {
      ...
      SCR_DrawSmallChar(
          cl_conXOffset->integer + con.xadjust + (x + 1) * SMALLCHAR_WIDTH, v,
          text[x] & 0xff);
    }
