...
cvar_t* cl_shownet = NULL;  // NERVE - SMF - This is referenced in msg.c and we
                            // need to make sure it is NULL
...
// TTimo : moved from char* to array (was getting the char* from va(), broke on
// big downloads)
...
void CL_Record_f(void) {
  ...
  //	Com_Printf (S_COLOR_YELLOW "WARNING: You should set
...
void CL_PlayDemo_f(void) {
  ...
  //	CL_FlushMemory();	//----(SA)	MEM NOTE: in missionpack, this
  //is moved to CL_DownloadsComplete
...
void CL_Disconnect(qboolean showMainMenu) {
  ...
  // show_bug.cgi?id=589
  // don't try a restart if uivm is NULL, as we might be in the middle of a
  // restart already
...
void CL_RequestAuthorization(void) {
  ...
  if (Cvar_VariableValue("fs_restrict")) {
    ...
    // only grab the alphanumeric values from the cdkey, to avoid any dashes or
    // spaces
    j = 0;
    l = strlen(cl_cdkey);
    if (l > 32) {
...
void CL_Vid_Restart_f(void) {
  // RF, don't show percent bar, since the memory usage will just sit at the
  // same level anyway
...
void CL_DownloadsComplete(void) {
  ...
  if (autoupdateStarted) {
    if (autoupdateFilename && (strlen(autoupdateFilename) > 4)) {
      ...
      fn = va("%s/%s", FS_ShiftStr(AUTOUPDATE_DIR, AUTOUPDATE_DIR_SHIFT),
              autoupdateFilename);
#else
      fs_write_path = Cvar_VariableString("fs_homepath");
      fn = FS_BuildOSPath(fs_write_path,
                          FS_ShiftStr(AUTOUPDATE_DIR, AUTOUPDATE_DIR_SHIFT),
                          autoupdateFilename);
#ifndef __MACOS__
      Sys_Chmod(fn, S_IXUSR);
#endif
#endif
      // will either exit with a successful process spawn, or will Com_Error
      // ERR_DROP so we need to clear the disconnected download data if needed
      if (cls.bWWWDlDisconnected) {
  ...
  if (cls.downloadRestart) {
    ...
    FS_Restart(clc.checksumFeed);  // We possibly downloaded a pak, restart the
                                   // file system to load it
  ...
  // TTimo: I wonder if that happens - it should not but I suspect it could
  // happen if a download fails in the middle or is aborted
...
void CL_BeginDownload(const char* localName, const char* remoteName) {
  ...
  Com_sprintf(cls.downloadTempName, sizeof(cls.downloadTempName), "%s.tmp",
              localName);
  ...
  if (autoupdateStarted &&
    ...
    if (strlen(cl_updatefiles->string) > 4) {
      ...
      Q_strncpyz(clc.downloadList,
                 va("@%s/%s@%s/%s", dir, cl_updatefiles->string, dir,
                    cl_updatefiles->string),
                 MAX_INFO_STRING);
      cls.state = CA_CONNECTED;
      CL_NextDownload();
      return;
    ...
    // whatever autodownlad configuration, store missing files in a cvar, use
    ...
    if (cl_allowDownload->integer &&
        FS_ComparePaks(clc.downloadList, sizeof(clc.downloadList), qtrue)) {
...
void CL_DisconnectPacket(netadr_t from) {
  ...
  // malicious spoof) NOTE TTimo: there used to be a  clc.lastPacketTime =
  // cls.realtime; line in CL_PacketEvent before calling CL_ConnectionLessPacket
  // therefore .. packets never got through this check, clients never
  // disconnected switched the clc.lastPacketTime = cls.realtime to happen after
  // the connectionless packets have been processed you still can't spoof
  // disconnects, cause legal netchan packets will maintain realtime -
  // lastPacketTime below the threshold
...
void CL_PrintPacket(netadr_t from, msg_t* msg) {
  ...
  if (!Q_stricmpn(s, "[err_dialog]", 12)) {
    ...
    // Cvar_Set("com_errorMessage", CL_TranslateStringBuf(
...
void CL_ServersResponsePacket(netadr_t from, msg_t* msg) {
  ...
  for (i = 0; i < numservers && count < max; i++) {
    // build net address
    // serverInfo_t *server = (cls.masterNum == 0) ? &cls.globalServers[count] :
...
void CL_ConnectionlessPacket(netadr_t from, msg_t* msg) {
  ...
  if (!Q_stricmp(c, "connectResponse")) {
    ...
    // DHM - Nerve :: If we have completed a connection to the Auto-Update
    // server...
  ...
  // NERVE - SMF - bugfix, make this compare first n chars so it doesnt bail if
  // token is parsed incorrectly echo request from server
...
void CL_CheckUserinfo(void) {
  ...
  if (cvar_modifiedFlags & CVAR_USERINFO) {
    cvar_modifiedFlags &= ~CVAR_USERINFO;
    CL_AddReliableCommand(
        va("userinfo \"%s\"", Cvar_InfoString(CVAR_USERINFO)));
  }
}
...
void CL_WWWDownload(void) {
  ...
  if (ret == DL_DONE) {
    ...
    *cls.downloadTempName = *cls.downloadName = 0;
    Cvar_Set("cl_downloadName", "");
    if (cls.bWWWDlDisconnected) {
      // for an auto-update in disconnected mode, we'll be spawning the setup in
      // CL_DownloadsComplete
      if (!autoupdateStarted) {
        // reconnect to the server, which might send us to a new disconnected
        // download
    ...
    if (cls.bWWWDlDisconnected) {
      ...
      // for a reply but in this case we can't get anything from server if we
      // just reconnect it's likely we'll get the same disconnected download
      // message, and error out again this may happen for a regular dl or an
      // auto update
      const char* error =
          va("Download failure while getting '%s'\n",
             cls.downloadName);  // get the msg before clearing structs
      cls.bWWWDlDisconnected =
          qfalse;  // need clearing structs before ERR_DROP, or it goes into
                   // endless reload
      CL_ClearStaticDownload();
...
qboolean CL_WWWBadChecksum(const char* pakname) {
  if (strstr(clc.redirectedList, va("@%s", pakname))) {
    Com_Printf(
        "WARNING: file %s obtained through download redirect has wrong "
        "checksum\n",
        pakname);
    Com_Printf(
...
#define CACHE_HIT_RATIO \
  0.75  // if hit on this percentage of maps, it'll get cached
...
void CL_InitRenderer(void) {
  ...
  cls.consoleShader =
      re.RegisterShader("console-16bit");  // JPW NERVE shader works with 16bit
...
qboolean CL_NextUpdateServer(void) {
  ...
  Com_DPrintf("%i.%i.%i.%i:%i\n", cls.autoupdateServer.ip[0],
              cls.autoupdateServer.ip[1], cls.autoupdateServer.ip[2],
              cls.autoupdateServer.ip[3], BigShort(cls.autoupdateServer.port));
...
void CL_Init(void) {
  ...
  cl_bypassMouseInput =
      Cvar_Get("cl_bypassMouseInput", "0",
               0);  // CVAR_ROM );			// NERVE - SMF

  cl_doubletapdelay =
      Cvar_Get("cl_doubletapdelay", "350", CVAR_ARCHIVE);  // Arnout: double tap
  ...
  Cvar_Get("rate", "5000",
           CVAR_USERINFO | CVAR_ARCHIVE);  // NERVE - SMF - changed from 3000
  Cvar_Get("snaps", "20", CVAR_USERINFO | CVAR_ARCHIVE);
  //	Cvar_Get ("model", "american", CVAR_USERINFO | CVAR_ARCHIVE );	// temp
  //until we have an skeletal american model 	Arnout - no need // Cvar_Get
  //("model", "multi", CVAR_USERINFO | CVAR_ARCHIVE ); 	Arnout - no need //
  //Cvar_Get ("head", "default", CVAR_USERINFO | CVAR_ARCHIVE ); 	Arnout - no
  //need // Cvar_Get ("color", "4", CVAR_USERINFO | CVAR_ARCHIVE ); 	Arnout - no
  //need // Cvar_Get ("handicap", "0", CVAR_USERINFO | CVAR_ARCHIVE ); 	Cvar_Get
  //("sex", "male", CVAR_USERINFO | CVAR_ARCHIVE );
  ...
  Cmd_AddCommand("SaveTranslations",
                 CL_SaveTranslations_f);  // NERVE - SMF - localization
  Cmd_AddCommand("SaveNewTranslations",
                 CL_SaveNewTranslations_f);  // NERVE - SMF - localization
  Cmd_AddCommand("LoadTranslations",
                 CL_LoadTranslations_f);  // NERVE - SMF - localization
#endif
  // NERVE - SMF - don't do this in multiplayer
  // RF, add this command so clients can't bind a key to send client damage
  // commands to the server
  //	Cmd_AddCommand ("cld", CL_ClientDamageCommand );

  //	Cmd_AddCommand ( "startSingleplayer", CL_startSingleplayer_f );
  //// NERVE - SMF 	fretn - unused 	Cmd_AddCommand ( "buyNow", CL_buyNow_f );
  //// NERVE - SMF 	Cmd_AddCommand ( "singlePlayLink", CL_singlePlayLink_f );
  //// NERVE - SMF
  ...
  if (server) {
    if (info) {
      ...
      server->allowAnonymous =
          atoi(Info_ValueForKey(info, "sv_allowAnonymous"));
      server->friendlyFire =
          atoi(Info_ValueForKey(info, "friendlyFire"));  // NERVE - SMF
      server->maxlives =
          atoi(Info_ValueForKey(info, "maxlives"));  // NERVE - SMF
      server->needpass =
          atoi(Info_ValueForKey(info, "needpass"));  // NERVE - SMF
      server->punkbuster =
          atoi(Info_ValueForKey(info, "punkbuster"));  // DHM - Nerve
...
static void CL_SetServerInfoByAddress(netadr_t from,
                                      const char* info,
                                      int ping) {
  int i;
...
=======================
*/
static trans_t* LookupTrans(char* original,
                            char* translated[MAX_LANGUAGES],
                            qboolean isLoading) {
  ...
  // see if we want to save out the translation table everytime a string is
  // added
...
void CL_SaveTransTable(const char* fileName, qboolean newOnly) {
  ...
  if (cl.corruptedTranslationFile) {
    Com_Printf(S_COLOR_YELLOW
               "WARNING: Cannot save corrupted translation file. Please reload "
               "first.");
    return;
  }
  ...
  Com_Printf(
      "Saved translation table.\nTotal = %i, Translated = %i, Untranslated = "
      "%i, aveblen = %2.2f, maxblen = %i\n",
      transnum + untransnum, transnum, untransnum,
      (float)avebucketlen / bucketnum, maxbucketlen);
...
void CL_LoadTransTable(const char* fileName) {
  ...
  do {
    ...
    if (!CL_CheckTranslationString(original, translated[LANGUAGE_FRENCH])) {
      Com_Printf(S_COLOR_YELLOW
                 "WARNING: Translation formatting doesn't match up with "
                 "English version!\n");
      aborted = qtrue;
      break;
    }
