...
#define LIMBOCHAT_WIDTH \
  140  // NERVE - SMF - NOTE TTimo buffer size indicator, not related to screen
       // bbox
...
typedef struct {
  ...
  qboolean bWWWDlAborting;  // disable the CL_WWWDownload until server gets us a
...
typedef struct {
  ...
  char servername[MAX_OSPATH];  // name of server from original connect (used by
                                // reconnect)
  ...
  // for clear up
  qboolean bWWWDlDisconnected;  // keep going with the download after server
                                // disconnect
...
void SCR_FillRect(float x,
                  float y,
                  float width,
                  float height,
                  const float* color);
void SCR_DrawPic(float x,
                 float y,
                 float width,
                 float height,
                 qhandle_t hShader);
void SCR_DrawNamedPic(float x,
                      float y,
                      float width,
                      float height,
                      const char* picname);

void SCR_DrawBigString(int x,
                       int y,
                       const char* s,
                       float alpha);  // draws a string with embedded color
                                      // control characters with fade
void SCR_DrawBigStringColor(
    int x,
    int y,
    const char* s,
    vec4_t color);  // ignores embedded color control characters
void SCR_DrawSmallStringExt(int x,
                            int y,
                            const char* string,
                            float* setColor,
                            qboolean forceColor);
