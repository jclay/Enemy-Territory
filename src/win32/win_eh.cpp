...
#define CASE(seCode)                                                 \
  case EXCEPTION_##seCode:                                           \
    Com_sprintf(minibuffer, sizeof(minibuffer),                      \
                "Exception %s (0x%.8x) at address 0x%.8x.", #seCode, \
                EXCEPTION_##seCode,                                  \
...
class CWolfException {
 public:
  ...
  void BuildErrorMessage(char* buffer, int size) {
    ...
    switch (m_seCode) {
      case EXCEPTION_ACCESS_VIOLATION:
        Com_sprintf(minibuffer, sizeof(minibuffer),
                    "Exception ACCESS_VIOLATION (0x%.8x) at address 0x%.8x "
                    "trying to %s address 0x%.8x.",
                    EXCEPTION_ACCESS_VIOLATION,
                    m_exPointers->ExceptionRecord->ExceptionAddress,
                    m_exPointers->ExceptionRecord->ExceptionInformation[0]
                        ? "write"
                        : "read",
  ...
  void BuildRegisters(char* buffer, int size) {
    ...
    Com_sprintf(
        minibuffer, sizeof(minibuffer),
        "Registers      : EAX=%.8x CS=%.4x EIP=%.8x EFLGS=%.8x\r\n"
        "               : EBX=%.8x SS=%.4x ESP=%.8x EBP=%.8x\r\n"
        "               : ECX=%.8x DS=%.4x ESI=%.8x FS=%.4x\r\n"
        "               : EDX=%.8x ES=%.4x EDI=%.8x GS=%.4x\r\n",
        m_exPointers->ContextRecord->Eax, m_exPointers->ContextRecord->SegCs,
        m_exPointers->ContextRecord->Eip, m_exPointers->ContextRecord->EFlags,
        m_exPointers->ContextRecord->Ebx, m_exPointers->ContextRecord->SegSs,
        m_exPointers->ContextRecord->Esp, m_exPointers->ContextRecord->Ebp,
        m_exPointers->ContextRecord->Ecx, m_exPointers->ContextRecord->SegDs,
        m_exPointers->ContextRecord->Esi, m_exPointers->ContextRecord->SegFs,
        m_exPointers->ContextRecord->Edx, m_exPointers->ContextRecord->SegEs,
        m_exPointers->ContextRecord->Edi, m_exPointers->ContextRecord->SegGs);
