...
cvar_t*
    r_allowSoftwareGL;     // don't abort out if the pixelformat claims software
cvar_t* r_maskMinidriver;  // allow a different dll name to be treated as if it
                           // were opengl32.dll
...
// fretn - comes from linux_glimp.c msvc really needs this (well ... mine does
// \o/ )
...
static int GLW_ChoosePFD(HDC hDC, PIXELFORMATDESCRIPTOR* pPFD) {
  ...
  if (glConfig.driverType > GLDRV_ICD) {
    maxPFD = qwglDescribePixelFormat(hDC, 1, sizeof(PIXELFORMATDESCRIPTOR),
                                     &pfds[0]);
  } else {
    maxPFD =
        DescribePixelFormat(hDC, 1, sizeof(PIXELFORMATDESCRIPTOR), &pfds[0]);
  }
  if (maxPFD > MAX_PFDS) {
  ...
  for (i = 1; i <= maxPFD; i++) {
    ...
    if (bestMatch) {
      // check stereo
      if ((pfds[i].dwFlags & PFD_STEREO) &&
          (!(pfds[bestMatch].dwFlags & PFD_STEREO)) &&
...
static qboolean GLW_InitDriver(const char* drivername, int colorbits) {
  ...
  static PIXELFORMATDESCRIPTOR
      pfd;  // save between frames since 'tr' gets cleared
...
static qboolean GLW_CreateWindow(const char* drivername,
                                 int width,
                                 int height,
                                 int colorbits,
                                 qboolean cdsFullscreen) {
  ...
  if (!g_wv.hWnd) {
    ...
    g_wv.hWnd = CreateWindowEx(exstyle, WINDOW_CLASS_NAME,
                               //"Wolfenstein",
                               "Enemy Territory", stylebits, x, y, w, h, NULL,
                               NULL, g_wv.hInstance, NULL);
...
static rserr_t GLW_SetMode(const char* drivername,
  ...
  if (cdsFullscreen) {
    ...
    //
      ...
      if ((cdsRet = ChangeDisplaySettings(&dm, CDS_FULLSCREEN)) ==
        ...
        //
        // the exact mode failed, so scan EnumDisplaySettings for the next
        // largest mode
        //
        DEVMODE devmode;
        int modeNum;
  ...
  //
  // success, now check display frequency, although this won't be valid on
  // Voodoo(2)
  //
  memset(&dm, 0, sizeof(dm));
  dm.dmSize = sizeof(dm);
...
static void GLW_InitExtensions(void) {
  ...
  /* RF, disabled this section, since this method produces very ugly results on
  nvidia hardware else if ( strstr( glConfig.extensions_string, "GL_S3_s3tc" ) )
  {
          if ( r_ext_compressed_textures->integer )
          {
  ...
  */
  ...
  if (strstr(glConfig.extensions_string, "GL_ARB_multitexture")) {
    if (r_ext_multitexture->integer) {
      qglMultiTexCoord2fARB = (PFNGLMULTITEXCOORD2FARBPROC)qwglGetProcAddress(
          "glMultiTexCoord2fARB");
      qglActiveTextureARB =
          (PFNGLACTIVETEXTUREARBPROC)qwglGetProcAddress("glActiveTextureARB");
      qglClientActiveTextureARB =
          (PFNGLCLIENTACTIVETEXTUREARBPROC)qwglGetProcAddress(
  ...
  if (strstr(glConfig.extensions_string, "GL_EXT_compiled_vertex_array") &&
    ...
    if (r_ext_compiled_vertex_array->integer) {
      ...
      qglLockArraysEXT =
          (void(APIENTRY*)(int, int))qwglGetProcAddress("glLockArraysEXT");
      qglUnlockArraysEXT =
          (void(APIENTRY*)(void))qwglGetProcAddress("glUnlockArraysEXT");
  ...
  if (strstr(glConfig.extensions_string, "WGL_3DFX_gamma_control")) {
    if (!r_ignorehwgamma->integer && r_ext_gamma_control->integer) {
      qwglGetDeviceGammaRamp3DFX = (BOOL(WINAPI*)(
          HDC, LPVOID))qwglGetProcAddress("wglGetDeviceGammaRamp3DFX");
      qwglSetDeviceGammaRamp3DFX = (BOOL(WINAPI*)(
          HDC, LPVOID))qwglGetProcAddress("wglSetDeviceGammaRamp3DFX");
  ...
  if (Q_stristr(glConfig.extensions_string,
                "GL_EXT_texture_filter_anisotropic")) {
...
static void GLW_GenDefaultLists(void) {
  ...
  if (fontbase_init) {
    Com_DPrintf(
        "ERROR: GLW_GenDefaultLists: font base is already marked "
        "initialized\n");
  }
...
// FIXME: newer 3Dfx drivers means this can go away
    ...
    if (!GLW_StartDriverAndSetMode(drivername, r_mode->integer,
                                   r_colorbits->integer, cdsFullscreen)) {
      // if we're on a 24/32-bit desktop and we're going fullscreen on an ICD,
      // try it again but with a 16-bit desktop
      if (glConfig.driverType == GLDRV_ICD) {
        if (r_colorbits->integer != 16 || cdsFullscreen != qtrue ||
            r_mode->integer != 3) {
...
void GLimp_Init(void) {
  ...
  Q_strncpyz(glConfig.vendor_string, qglGetString(GL_VENDOR),
             sizeof(glConfig.vendor_string));
  Q_strncpyz(glConfig.renderer_string, qglGetString(GL_RENDERER),
             sizeof(glConfig.renderer_string));
  Q_strncpyz(glConfig.version_string, qglGetString(GL_VERSION),
             sizeof(glConfig.version_string));
  Q_strncpyz(glConfig.extensions_string, qglGetString(GL_EXTENSIONS),
             sizeof(glConfig.extensions_string));
  // TTimo - safe check
  if (strlen(qglGetString(GL_EXTENSIONS)) >=
      sizeof(glConfig.extensions_string)) {
    Com_Printf(
        S_COLOR_YELLOW
        "WARNNING: GL extensions string too long (%d), truncated to %d\n",
        strlen(qglGetString(GL_EXTENSIONS)),
        sizeof(glConfig.extensions_string));
  }
  ...
  if (Q_stricmp(lastValidRenderer->string, glConfig.renderer_string)) {
    ...
    if (strstr(buf, "voodoo graphics/1 tmu/2 mb")) {
      ...
      //----(SA)	FIXME: RETURN TO DEFAULT  Another id build change for
      //DK/DM
  ...
  // VOODOO GRAPHICS w/ 2MB
    ...
    ri.Printf(PRINT_WARNING,
              "WARNING: Rage Pro hardware is unsupported. Rendering errors may "
              "occur.\n");
  } else if (strstr(buf, "rage 128")) {
  } else if (strstr(buf, "permedia2")) {
    glConfig.hardwareType = GLHW_PERMEDIA2;
    ri.Printf(PRINT_WARNING,
              "WARNING: Permedia hardware is unsupported. Rendering errors may "
              "occur.\n");
  } else if (strstr(buf, "riva 128")) {
    glConfig.hardwareType = GLHW_RIVA128;
    ri.Printf(PRINT_WARNING,
              "WARNING: Riva 128 hardware is unsupported. Rendering errors may "
              "occur.\n");
  } else if (strstr(buf, "matrox")) {
  } else if (strstr(buf, "riva tnt ")) {
  }

  if (strstr(buf, "geforce3") || strstr(buf, "geforce4 ti") ||
      strstr(buf, "geforce fx 5600") || strstr(buf, "geforce fx 5800") ||
      strstr(buf, "radeon 8500") || strstr(buf, "radeon 9000") ||
      strstr(buf, "radeon 9500") || strstr(buf, "radeon 9600") ||
      strstr(buf, "radeon 9700") || strstr(buf, "radeon 9800") ||
      strstr(buf, "nv20") || strstr(buf, "nv30")) {
