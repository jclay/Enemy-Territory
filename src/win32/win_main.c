...
void Sys_StartProcess(char* exeName, qboolean doexit) {
  ...
  if (!CreateProcess(NULL, va("%s\\%s", szPathOrig, exeName), NULL, NULL, FALSE,
                     0, NULL, NULL, &si, &pi)) {
    // couldn't start it, popup error box
    Com_Error(ERR_DROP, "Could not start process: '%s\\%s' ", szPathOrig,
              exeName);
    return;
  }
  // jpw
...
// this time fqpath will be empty if dll not loaded, otherwise will hold fully
// qualified path of dll module loaded fqpath buffersize must be at least
// MAX_QPATH+1 bytes long
void* QDECL Sys_LoadDll(const char* name,
  ...
  *fqpath = 0;  // added 2/15/02 by T.Ray
  ...
  dllEntry = (void(QDECL*)(int(QDECL*)(int, ...)))GetProcAddress(libHandle,
                                                                 "dllEntry");
...
void Sys_StreamFillBuffer(int i) {
  ...
  if (stream.sIO[i].active && !stream.sIO[i].eof) {
    count = stream.sIO[i].bufferSize -
            (stream.sIO[i].threadPosition - stream.sIO[i].streamPosition);
    if (!count) {
      return;
    }
    ...
    // r = FS_ReadDirect( stream.sIO[i].buffer + bufferPoint, readCount,
    // stream.sIO[i].file );
    r = FS_Read2(stream.sIO[i].buffer + bufferPoint, readCount,
                 stream.sIO[i].file);
    if (r != readCount) {
...
void Sys_InitStreamThread(void) {
  ...
  stream.musicThreadHandle =
      CreateThread(NULL,  // LPSECURITY_ATTRIBUTES lpsa,
                   0,     // DWORD cbStack,
                   (LPTHREAD_START_ROUTINE)
                       Sys_MusicThread,  // LPTHREAD_START_ROUTINE lpStartAddr,
                   0,                    // LPVOID lpvThreadParm,
                   0,                    //   DWORD fdwCreate,
