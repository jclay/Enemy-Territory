...
// in VC6 you will also need to add the include path and lib path in Tools >
// Options > Directories
#include <iphlpapi.h>
#include <iptypes.h>
...
void Sys_SendPacket(int length, const void* data, netadr_t to) {
  ...
  if (usingSocks && to.type == NA_IP) {
    ...
    *(short*)&socksBuf[8] = ((struct sockaddr_in*)&addr)->sin_port;
    ...
    ret = sendto(net_socket, socksBuf, length + 10, 0, &socksRelayAddr,
...
void NET_GetInterfaces(void) {
  ...
  if (!pAdapterInfo) {
    Com_Error(ERR_FATAL, "NET_GetInterfaces: Couldn't malloc( %d )",
  ...
  if (GetAdaptersInfo(pAdapterInfo, &ulOutBufLen) == ERROR_BUFFER_OVERFLOW) {
    ...
    if (!pAdapterInfo) {
      Com_Error(ERR_FATAL, "NET_GetInterfaces: Couldn't malloc( %ld )",
                ulOutBufLen);
    }
  }
  ...
  if ((dwRetVal = GetAdaptersInfo(pAdapterInfo, &ulOutBufLen)) == NO_ERROR) {
    ...
    while (pAdapter) {
      ...
      //pAdapter->AdapterName ); 			Com_Printf( "Adapter Desc: %s\n",
      //pAdapter->Description );
      pIPAddrString = &pAdapter->IpAddressList;
      while (pIPAddrString) {
        unsigned long ip_a, ip_m;
        //				Com_Printf( "IP Address: %s\n",
        //pIPAddrString->IpAddress.String ); 				Com_Printf( "IP Mask: %s\n",
