...
static DIOBJECTDATAFORMAT rgodf[] = {
    ...
    {
        0,
        FIELD_OFFSET(MYDATA, bButtonE),
        0x80000000 | DIDFT_BUTTON | DIDFT_ANYINSTANCE,
        0,
    },
    {
        0,
        FIELD_OFFSET(MYDATA, bButtonF),
        0x80000000 | DIDFT_BUTTON | DIDFT_ANYINSTANCE,
        0,
    },
    {
        0,
        FIELD_OFFSET(MYDATA, bButtonG),
        0x80000000 | DIDFT_BUTTON | DIDFT_ANYINSTANCE,
        0,
    },
    {
        0,
        FIELD_OFFSET(MYDATA, bButtonH),
        0x80000000 | DIDFT_BUTTON | DIDFT_ANYINSTANCE,
        0,
    },
};
...
qboolean IN_InitDInput(void) {
...
#ifdef __GNUC__
  hResult =
      DirectInputCreate(g_wv.hInstance, DIRECTINPUT_VERSION, &g_pdi, NULL);
#else
  hResult = DirectInput8Create(g_wv.hInstance, DIRECTINPUT_VERSION,
                               &IID_IDirectInput8A, &g_pdi, NULL);
#endif
  ...
  hResult = IDirectInputDevice_SetCooperativeLevel(
      g_pMouse, g_wv.hWnd, DISCL_EXCLUSIVE | DISCL_FOREGROUND);
  ...
  hResult =
      IDirectInputDevice_SetProperty(g_pMouse, DIPROP_BUFFERSIZE, &dipdw.diph);
      ...
      switch (od.dwOfs) {
        ...
        case DIMOFS_Z:
          ...
          if ((int)od.dwData > 0) {
            ...
            Sys_QueEvent(g_wv.sysMsgTime, SE_KEY, K_MWHEELDOWN, qfalse, 0,
                         NULL);
          }
  ...
  if (directInput) {
    ...
    if (g_pMouse) {
      IDirectInputDevice_SetCooperativeLevel(
          g_pMouse, g_wv.hWnd, DISCL_NONEXCLUSIVE | DISCL_BACKGROUND);
...
void IN_StartupJoystick(void) {
  ...
  if ((mmr = joyGetDevCaps(joy.id, &joy.jc, sizeof(joy.jc))) !=
      JOYERR_NOERROR) {
...
int joyDirectionKeys[16] = {K_LEFTARROW, K_RIGHTARROW, K_UPARROW, K_DOWNARROW,
                            K_JOY16,     K_JOY17,      K_JOY18,   K_JOY19,
                            K_JOY20,     K_JOY21,      K_JOY22,   K_JOY23,

                            K_JOY24,     K_JOY25,      K_JOY26,   K_JOY27};
...
void IN_JoyMove(void) {
  ...
  if (in_debugJoystick->integer) {
    Com_Printf("%8x %5i %5.2f %5.2f %5.2f %5.2f %6i %6i\n", joy.ji.dwButtons,
               joy.ji.dwPOV, JoyToF(joy.ji.dwXpos), JoyToF(joy.ji.dwYpos),
  ...
  for (i = 0; i < 16; i++) {
    if ((povstate & (1 << i)) && !(joy.oldpovstate & (1 << i))) {
      Sys_QueEvent(g_wv.sysMsgTime, SE_KEY, joyDirectionKeys[i], qtrue, 0,
                   NULL);
    }

    if (!(povstate & (1 << i)) && (joy.oldpovstate & (1 << i))) {
      Sys_QueEvent(g_wv.sysMsgTime, SE_KEY, joyDirectionKeys[i], qfalse, 0,
                   NULL);
    }
  }
...
static void IN_StartupMIDI(void) {
  ...
  if (midiInOpen(&s_midiInfo.hMidiIn, in_mididevice->integer,
                 (unsigned long)MidiInProc, (unsigned long)NULL,
                 CALLBACK_FUNCTION) != MMSYSERR_NOERROR) {
    Com_Printf("WARNING: could not open MIDI device %d: '%s'\n",
               in_mididevice->integer,
               s_midiInfo.caps[(int)in_mididevice->value]);
    return;
  }
