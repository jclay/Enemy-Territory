...
void R_ImageList_f(void) {
  ...
  for (i = 0; i < tr.numImages; i++) {
    ...
    ri.Printf(PRINT_ALL, "%4i: %4i %4i  %s   %d   %5d ", i, image->uploadWidth,
...
================
*/
static void ResampleTexture(unsigned* in,
                            int inwidth,
                            int inheight,
                            unsigned* out,
                            int outwidth,
...
================
*/
void R_LightScaleTexture(unsigned* in,
                         int inwidth,
                         int inheight,
                         qboolean only_gamma) {
  if (only_gamma) {
...
static void R_MipMap2(unsigned* in, int inWidth, int inHeight) {
  ...
  for (i = 0; i < outHeight; i++) {
    for (j = 0; j < outWidth; j++) {
      ...
      for (k = 0; k < 4; k++) {
        total = 1 * ((byte*)&in[((i * 2 - 1) & inHeightMask) * inWidth +
                                ((j * 2 - 1) & inWidthMask)])[k] +
...
static void Upload32(unsigned* data,
  ...
  /*	if (r_rmse->value) {
                  while (R_RMSE((byte *)data, width, height) < r_rmse->value) {
                          rmse_saved +=
     (height*width*4)-((width>>1)*(height>>1)*4); resampledBuffer =
     R_GetImageBuffer( (width>>1) * (height>>1) * 4, BUFFER_RESAMPLED );
                          ResampleTexture (data, width, height, resampledBuffer,
     width>>1, height>>1); data = resampledBuffer; width = width>>1; height =
     height>>1; ri.Printf (PRINT_ALL, "r_rmse of %f has saved %dkb\n",
  ...
  if (scaled_width != width || scaled_height != height) {
    ...
    // scaled_height * 4 );
    resampledBuffer =
        R_GetImageBuffer(scaled_width * scaled_height * 4, BUFFER_RESAMPLED);
  ...
  scaledBuffer = R_GetImageBuffer(
      sizeof(unsigned) * scaled_width * scaled_height, BUFFER_SCALED);
...
image_t* R_CreateImage(const char* name,
  ...
  if (!noCompress &&
      strstr(name, "weapons")) {  // don't compress view weapon skins
    noCompress = qtrue;
  }
...
#if __MACOS__
  // LBO 2/8/05. Work around apparent bug in OSX. Some mipmap textures draw
  // incorrectly when texture compression is enabled. Examples include brick
  // edging on fueldump level appearing bluish-green from a distance.
  else if (mipmap) {
    noCompress = qtrue;
  }
#endif
  ...
  // ydnar: not exactly sure why this mechanism is here at all, but it's
  // generating bad texture names (not that the rest of the code is a saint, but
  // hey...)
...
// developer
    ...
    strcpy(altname, name);    //
    len = strlen(altname);    //
    altname[len - 3] = toupper(
        altname[len - 3]);  // and try upper case extension for unix systems
...
#define CHECKPOWEROF2
  ...
  image = R_CreateImage((char*)name, pic, width, height, mipmap, allowPicmip,
                        glWrapClampMode);
...
static void R_CreateDlightImage(void) {
  ...
  tr.dlightImage = R_CreateImage("*dlight", (byte*)data, DLIGHT_SIZE,
...
static void R_CreateFogImage(void) {
  ...
  // ydnar: new, linear fog texture generating algo for GL_CLAMP_TO_EDGE
  // (OpenGL 1.2+)

  // S is distance, T is depth
  for (x = 0; x < FOG_S; x++) {
    for (y = 0; y < FOG_T; y++) {
      alpha = 270 * ((float)x / FOG_S) *
              ((float)y / FOG_T);  // need slop room for fp round to 0
      if (alpha < 0) {
        alpha = 0;
      } else if (alpha > 255) {
        alpha = 255;
      ...
      // ensure edge/corner cases are fully transparent (at 0,0) or fully opaque
      // (at 1,N where N is 0-1.0)
      if (x == 0) {
        alpha = 0;
      } else if (x == (FOG_S - 1)) {
        alpha = 255;
  ...
  // what we want.
  tr.fogImage = R_CreateImage("*fog", (byte*)data, FOG_S, FOG_T, qfalse, qfalse,
                              GL_CLAMP);
...
static void R_CreateDefaultImage(void) {
  ...
  tr.defaultImage = R_CreateImage("*default", (byte*)data, DEFAULT_SIZE,
...
void R_CreateBuiltinImages(void) {
  ...
  tr.whiteImage =
      R_CreateImage("*white", (byte*)data, 8, 8, qfalse, qfalse, GL_REPEAT);
  ...
  for (x = 0; x < 32; x++) {
    ...
    tr.scratchImage[x] = R_CreateImage("*scratch", (byte*)data, DEFAULT_SIZE,
...
void R_InitImages(void) {
  ...
  // Ridah, load the cache media, if they were loaded previously, they'll be
  // restored from the backupImages
  R_LoadCacheImages();
  // done.
}
...
RE_GetShaderFromModel
        ...
        'withlightmap' set to '0' will create a new shader that is a copy of the
one found on the model, without the lighmap stage, if the shader has a lightmap
stage

        NOTE: only works for bmodels right now.  Could modify for other models
(md3's etc.)
==============
*/
qhandle_t RE_GetShaderFromModel(qhandle_t modelid,
                                int surfnum,
                                int withlightmap) {
  model_t* model;
  bmodel_t* bmodel;
  ...
  if (model) {
    ...
    if (bmodel && bmodel->firstSurface) {
      if (surfnum >= bmodel->numSurfaces) {  // if it's out of range, return the
                                             // first surface
        surfnum = 0;
      }

      surf = bmodel->firstSurface + surfnum;
      // RF, check for null shader (can happen on func_explosive's with botclips
      // attached)
      if (!surf->shader) {
        return 0;
      }
...
qhandle_t RE_RegisterSkin(const char* name) {
  ...
  //----(SA)	moved things around slightly to fix the problem where you
  //restart 			a map that has ai characters who had invalid skin names entered 			in
  //thier "skin" or "head" field
  ...
  while (text_p && *text_p) {
    ...
    if (!Q_stricmpn(token, "md3_", 4)) {
      ...
      model = skin->models[skin->numModels] =
          ri.Hunk_Alloc(sizeof(*skin->models[0]), h_low);
...
// Ridah, utility for automatically cropping and numbering a bunch of images in
// a directory
/*
=============
SaveTGA
...
qboolean R_CropImage(char* name,
                     byte** pic,
                     int border,
                     int* width,
                     int* height,
...
===============
*/
void R_CropAndNumberImagesInDirectory(char* dir,
                                      char* ext,
                                      int maxWidth,
                                      int maxHeight,
                                      int withAlpha) {}
...
R_CacheImageAlloc
  ...
  this will only get called to allocate the image_t structures, not that actual
image pixels
===============
*/
void* R_CacheImageAlloc(int size) {
  if (r_cache->integer && r_cacheShaders->integer) {
    ...
    return malloc(
        size);  // ri.Z_Malloc causes load times about twice as long?... Gordon
...
R_TouchImage
  ...
  remove this image from the backupHashTable and make sure it doesn't get
overwritten
===============
*/
qboolean R_TouchImage(image_t* inImage) {
  ...
  if (inImage == tr.dlightImage || inImage == tr.whiteImage ||
      inImage == tr.defaultImage ||
      inImage->imgName[0] ==
          '*') {  // can't use lightmaps since they might have the same name,
                  // but different maps will have different actual lightmap
                  // pixels
    return qfalse;
  }
...
R_PurgeBackupImages
  ...
  Can specify the number of Images to purge this call (used for background
purging)
===============
*/
