...
void R_Fog(glfog_t* curfog) {
  ...
  //	}
  //	if(curfog->color[0] != setfog.color[0] || curfog->color[1] !=
  //setfog.color[1] || curfog->color[2] != setfog.color[2] ||
  //!setfog.registered) {
  ...
  if (r_zfar->value) {  // (SA) allow override for helping level designers test
                        // fog distances
  ...
  qglClearColor(curfog->color[0], curfog->color[1], curfog->color[2],
                curfog->color[3]);
}
...
void R_FogOn(void) {
  ...
  if (backEnd.refdef.rdflags &
      RDF_SKYBOXPORTAL) {  // don't force world fog on portal sky
...
R_SetFog
  ...
  if fogvar == FOG_CMD_SWITCHFOG {
        ...
        rgb = color
        density is density, and is used to derive the values of 'mode',
'drawsky', and 'clearscreen'
  }
==============
*/
void R_SetFog(int fogvar,
              int var1,
              int var2,
              float r,
              float g,
...
void R_LocalPointToWorld(vec3_t local, vec3_t world) {
  world[0] = local[0] * tr.orientation.axis[0][0] +
             local[1] * tr.orientation.axis[1][0] +
             local[2] * tr.orientation.axis[2][0] + tr.orientation.origin[0];
  world[1] = local[0] * tr.orientation.axis[0][1] +
             local[1] * tr.orientation.axis[1][1] +
             local[2] * tr.orientation.axis[2][1] + tr.orientation.origin[1];
  world[2] = local[0] * tr.orientation.axis[0][2] +
             local[1] * tr.orientation.axis[1][2] +
             local[2] * tr.orientation.axis[2][2] + tr.orientation.origin[2];
}
...
void R_SetFrameFog(void) {
  ...
  if (tr.world->globalFogTransEndTime) {
    if (tr.world->globalFogTransEndTime >= tr.refdef.time) {
      ...
      fadeTime =
          tr.world->globalFogTransEndTime - tr.world->globalFogTransStartTime;
      lerpPos = (float)(tr.refdef.time - tr.world->globalFogTransStartTime) /
                (float)fadeTime;
      if (lerpPos > 1) {
        lerpPos = 1;
      }
      ...
      tr.world->fogs[tr.world->globalFog].shader->fogParms.color[0] =
          ...
          ((tr.world->globalTransEndFog[0] - tr.world->globalTransStartFog[0]) *
           lerpPos);
      tr.world->fogs[tr.world->globalFog].shader->fogParms.color[1] =
          tr.world->globalTransStartFog[1] +
          ((tr.world->globalTransEndFog[1] - tr.world->globalTransStartFog[1]) *
           lerpPos);
      tr.world->fogs[tr.world->globalFog].shader->fogParms.color[2] =
          tr.world->globalTransStartFog[2] +
          ((tr.world->globalTransEndFog[2] - tr.world->globalTransStartFog[2]) *
           lerpPos);
      ...
      tr.world->fogs[tr.world->globalFog].shader->fogParms.depthForOpaque =
          ...
          ((tr.world->globalTransEndFog[3] - tr.world->globalTransStartFog[3]) *
           lerpPos);
      ...
      // transition complete
      VectorCopy(tr.world->globalTransEndFog,
                 tr.world->fogs[tr.world->globalFog].shader->fogParms.color);
      tr.world->fogs[tr.world->globalFog].shader->fogParms.colorInt =
          ColorBytes4(tr.world->globalTransEndFog[0] * tr.identityLight,
                      tr.world->globalTransEndFog[1] * tr.identityLight,
                      tr.world->globalTransEndFog[2] * tr.identityLight, 1.0);
      tr.world->fogs[tr.world->globalFog].shader->fogParms.depthForOpaque =
          tr.world->globalTransEndFog[3];
      tr.world->fogs[tr.world->globalFog].shader->fogParms.tcScale =
          1.0f /
          (tr.world->fogs[tr.world->globalFog].shader->fogParms.depthForOpaque *
           8);
  ...
  if (glfogsettings[FOG_TARGET].finishTime &&
    ...
    // transitioning like fog modes
      ...
      lerpPos = (float)(tr.refdef.time - glfogsettings[FOG_TARGET].startTime) /
                (float)fadeTime;
      if (lerpPos > 1) {
        lerpPos = 1;
      }
      ...
      // if either fog in the transition clears the screen, clear the background
      // this frame to avoid hall of mirrors
    ...
    // potential FIXME: since this is the most common occurance, diff first and
    // only set changes
    memcpy(&glfogsettings[FOG_CURRENT], &glfogsettings[FOG_TARGET],
           sizeof(glfog_t));
  }

  // shorten the far clip if the fog opaque distance is closer than the
  // procedural farcip dist
  ...
  if (r_speeds->integer == 5) {
    if (glfogsettings[FOG_CURRENT].mode == GL_LINEAR) {
      ...
      ri.Printf(PRINT_ALL,
                "density fog - den: %0.4f  calc zFar: %0.1f  fog zFar: %0.1f\n",
                glfogsettings[FOG_CURRENT].density, tr.viewParms.zFar,
                glfogsettings[FOG_CURRENT].end);
    }
  }
...
static void SetFarClip(void) {
  ...
  //----(SA)	this lets you use r_zfar from the command line to experiment
  //with different 			distances, but setting it back to 0 uses the map (or
  //procedurally generated) default
  if (r_zfar->value) {
...
void R_SetupFrustum(void) {
  ...
  VectorMA(tr.viewParms.frustum[0].normal, xc, tr.viewParms.orientation.axis[1],
           tr.viewParms.frustum[0].normal);
  ...
  VectorMA(tr.viewParms.frustum[2].normal, xc, tr.viewParms.orientation.axis[2],
           tr.viewParms.frustum[2].normal);
  ...
  for (i = 0; i < 4; i++) {
    ...
    tr.viewParms.frustum[i].dist = DotProduct(tr.viewParms.orientation.origin,
                                              tr.viewParms.frustum[i].normal);
    SetPlaneSignbits(&tr.viewParms.frustum[i]);
  }

  // ydnar: farplane (testing! use farplane for real)
  VectorScale(tr.viewParms.orientation.axis[0], -1,
              tr.viewParms.frustum[4].normal);
  tr.viewParms.frustum[4].dist = DotProduct(tr.viewParms.orientation.origin,
                                            tr.viewParms.frustum[4].normal) -
                                 tr.viewParms.zFar;
...
void R_SetupProjection(void) {
  ...
  // but if a player wants to play with fov 160, then they can deal with
  // z-fighting assume fov 90 = scale 1, fov 180 = scale 1/16th
  if (tr.refdef.fov_x > 90.0f) {
    zNear /= ((tr.refdef.fov_x - 90.0f) * 0.09f + 1.0f);
  }

  if (r_zfar->value) {
    zFar = r_zfar->value;  // (SA) allow override for helping level designers
                           // test fog distances
  } else {
  ...
  for (i = 0; i < tr.refdef.num_entities; i++) {
    ...
    if (e->e.oldframe) {
      ...
      if (e->e.frame) {
        ...
        RotatePointAroundVector(camera->axis[1], camera->axis[0], transformed,
                                d);
...
static qboolean SurfIsOffscreen(const drawSurf_t* drawSurf,
  ...
  if (glConfig.smpActive) {  // FIXME!  we can't do
                             // RB_BeginSurface/RB_EndSurface stuff with smp!
    return qfalse;
  }

  R_RotateForViewer();

  R_DecomposeSort(drawSurf->sort, &entityNum, &shader, &fogNum, &frontFace,
                  &dlighted);
  ...
  for (i = 0; i < tess.numVertexes; i++) {
    ...
    R_TransformModelToClip(tess.xyz[i].v, tr.orientation.modelMatrix,
  ...
  for (i = 0; i < tess.numIndexes; i += 3) {
    ...
    VectorSubtract(tess.xyz[tess.indexes[i]].v, tr.viewParms.orientation.origin,
                   normal);
...
void R_AddDrawSurf(surfaceType_t* surface,
  ...
  tr.refdef.drawSurfs[index].sort =
      ...
      (fogNum << QSORT_FOGNUM_SHIFT) | (frontFace << QSORT_FRONTFACE_SHIFT) |
      dlightMap;
...
void R_DecomposeSort(unsigned sort,
  ...
  *entityNum =
      (sort >> QSORT_ENTITYNUM_SHIFT) &
      (MAX_GENTITIES - 1);  // (SA) uppded entity count for Wolf to 11 bits
  ...
  for (tr.currentEntityNum = 0; tr.currentEntityNum < tr.refdef.num_entities;
    ...
    switch (ent->e.reType) {
      ...
      case RT_RAIL_RINGS:
        ...
        // primary view.  We can't just do this check for all entities, because
        // md3 entities may still want to cast shadows from them
...
void R_GenerateDrawSurfs(void) {
  ...
  if (tr.refdef.rdflags & RDF_NOWORLDMODEL || tr.world == NULL) {
    VectorSet(tr.viewParms.visBounds[0], MIN_WORLD_COORD, MIN_WORLD_COORD,
              MIN_WORLD_COORD);
    VectorSet(tr.viewParms.visBounds[1], MAX_WORLD_COORD, MAX_WORLD_COORD,
              MAX_WORLD_COORD);
  } else {
...
================
*/
void R_DebugText(const vec3_t org,
                 float r,
                 float g,
                 float b,
                 const char* text,
                 qboolean neverOcclude) {
  if (neverOcclude) {
...
void R_DebugGraphics(void) {
  ...
  R_FogOff();  // moved this in here to keep from /always/ doing the fog state
               // change
