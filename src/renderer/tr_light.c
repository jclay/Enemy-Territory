...
void R_CullDlights(void) {
  ...
  for (i = 0, dl = tr.refdef.dlights; i < tr.refdef.num_dlights; i++, dl++) {
    if ((dl->flags & REF_DIRECTED_DLIGHT) ||
        R_CullPointAndRadius(dl->origin, dl->radius) != CULL_OUT) {
      numDlights = i + 1;
      dlightBits |= (1 << i);
    }
  }
...
void R_DlightBmodel(bmodel_t* bmodel) {
  ...
  // (SA) isn't this dangerous to do to an enumerated type? (setting it to an
  // int)
  //		meaning, shouldn't ->needDlights be changed to an int rather
  //than a qbool?
  ...
  for (i = 0; i < bmodel->numSurfaces; i++) {
    ...
    if (*surf->data == SF_FACE) {
      ...
      //			((srfTriangles_t *)surf->data)->dlightBits[
      //tr.smpFrame ] = mask;
...
static void R_SetupEntityLightingGrid(trRefEntity_t* ent) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    //%		ri.Printf( PRINT_ALL, "lat: %3d lng: %3d dir: %2.3f %2.3f
    //%2.3f\n", %			data[ 7 ], data[ 8 ], normal[ 0 ],
    //normal[ 1 ], normal[ 2 ] );
  }
...
void R_SetupEntityLighting(const trRefdef_t* refdef, trRefEntity_t* ent) {
  ...
  if (tr.world && tr.world->lightGridData &&
      (!(refdef->rdflags & RDF_NOWORLDMODEL) ||
       ((refdef->rdflags & RDF_NOWORLDMODEL) &&
        (ent->e.renderfx & RF_LIGHTING_ORIGIN)))) {
    R_SetupEntityLightingGrid(ent);
  } else {
    //%	ent->ambientLight[0] = ent->ambientLight[1] = ent->ambientLight[2] =
    //tr.identityLight * 150; %	ent->directedLight[0] = ent->directedLight[1] =
    //ent->directedLight[2] = tr.identityLight * 150; %	VectorCopy(
    //tr.sunDirection, ent->lightDir );
  ...
  if (ent->e.entityNum < MAX_CLIENTS && (refdef->rdflags & RDF_SNOOPERVIEW)) {
    VectorSet(ent->ambientLight, 245, 245,
              245);  // allow a little room for flicker from directed light
  }
  ...
  for (i = 0; i < refdef->num_dlights; i++) {
    ...
    if (dl->shader) {  //----(SA)	if the dlight has a diff shader
                       //specified, you don't know what it does, so don't let it
                       //affect entities lighting
      continue;
    }
