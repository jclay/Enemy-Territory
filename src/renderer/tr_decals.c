...
static qboolean MakeTextureMatrix(vec4_t texMat[2],
                                  vec4_t projection,
                                  decalVert_t* a,
                                  decalVert_t* b,
                                  decalVert_t* c) {
...
omnidirectional omnidirectional decals use points[ 0 ] as center and projection[
3 ] as radius pass in lifeTime < 0 for a temporary mark
*/
...
void RE_ProjectDecal(qhandle_t hShader,
  ...
  if (numPoints == 1) {
    ...
    /* set corner */
    VectorSet(xyz, points[0][0] - radius, points[0][1] - radius,
              points[0][2] + radius);
    ...
    /* setup decal points */
    VectorSet(dv[0].xyz, points[0][0] - radius, points[0][1] - radius,
              points[0][2] + radius);
    VectorSet(dv[1].xyz, points[0][0] - radius, points[0][1] + radius,
              points[0][2] + radius);
    VectorSet(dv[2].xyz, points[0][0] + radius, points[0][1] + radius,
              points[0][2] + radius);
    VectorSet(dv[3].xyz, points[0][0] + radius, points[0][1] - radius,
              points[0][2] + radius);
  } else {
    ...
    /* make texture matrix */
    if (!MakeTextureMatrix(temp.texMat[0], projection, &dv[0], &dv[1],
                           &dv[2])) {
      return;
    }
  }
  ...
  for (i = 0; i < numPoints; i++) {
    VectorMA(dv[i].xyz, projection[3], projection, xyz);
    if (!PlaneFromPoints(temp.planes[i + 2], dv[(i + 1) % numPoints].xyz,
                         dv[i].xyz, xyz)) {
      return;
    }
  }
...
void R_AddModelShadow(refEntity_t* ent) {
  ...
  /* shadows? */
  if (!r_drawentities->integer || r_shadows->integer != 1 ||
...
void R_TransformDecalProjector(decalProjector_t* in,
  ...
  *((int*)out->color) = *((int*)in->color);
  ...
  /* translate bounding box and sphere (note: rotated projector bounding box
   * will be invalid!) */
  ...
  //%	ri.Printf( PRINT_ALL, "plane 0: %f %f %f in dist: %f out dist: %f z:
  //%f\n", %		out->planes[ 0 ][ 0 ], out->planes[ 0 ][ 1 ],
  //out->planes[ 0 ][ 2 ], %		in->planes[ 0 ][ 3 ], out->planes[ 0 ][
  //3 ], origin[ 2 ] );
  ...
  for (m = 0; m < 3; m++) {
    for (i = 0; i < 2; i++) {
      ...
      out->texMat[m][i][3] =
          in->texMat[m][i][3] + DotProduct(in->texMat[m][i], origin);
    }
  }
}
...
qboolean R_TestDecalBoundingBox(decalProjector_t* dp,
                                vec3_t mins,
                                vec3_t maxs) {
...
static void ProjectDecalOntoWinding(decalProjector_t* dp,
                                    int numPoints,
  ...
  {
    ChopWindingBehindPlane(numPoints, points[pingPong], &numPoints,
                           points[!pingPong], dp->planes[i], 0.0f);
    pingPong ^= 1;
    if (numPoints < 3) {
      return;
    }
...
static void ProjectDecalOntoTriangles(decalProjector_t* dp,
                                      msurface_t* surf,
                                      bmodel_t* bmodel) {
  int i;
  srfTriangles_t* srf;
...
static void ProjectDecalOntoGrid(decalProjector_t* dp,
                                 msurface_t* surf,
                                 bmodel_t* bmodel) {
  int x, y;
  srfGridMesh_t* srf;
...
void R_ProjectDecalOntoSurface(decalProjector_t* dp,
  ...
  if (!R_TestDecalBoundingSphere(dp, gen->origin,
                                 (gen->radius * gen->radius))) {
    return;
  }
...
void R_AddDecalSurface(decal_t* decal) {
  ...
  /* early outs */
  if (decal->shader == NULL || decal->parent->viewCount != tr.viewCount ||
