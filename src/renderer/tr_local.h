...
typedef struct {
  ...
  qboolean isDetail;
  qboolean isFogged;  // used only for shaders that have fog disabled, so we can
                      // enable it for individual stages
} shaderStage_t;
...
typedef struct shader_s {
  ...
  vec4_t distanceCull;  // ydnar: opaque alpha range for foliage (inner, outer,
                        // alpha threshold, 1/(outer-inner))
...
typedef struct corona_s {
  ...
  int id;
  qboolean visible;  // still send the corona request, even if not visible, for
                     // proper fading
} corona_t;
...
// ydnar: foliage surfaces are autogenerated from models into geometry lists by
// q3map2
...
typedef struct {
  ...
  vec4_t
      globalOriginalFog;  // Arnout: to be able to restore original global fog
...
typedef struct {
  ...
  shader_t* sunShader;
  shader_t*
      sunflareShader[6];  //----(SA) for the camera lens flare effect for sun
...
extern cvar_t* r_trisColor;     // enables modifying of the wireframe colour (in
                                // 0xRRGGBB[AA] format, alpha defaults to FF)
...
float R_ProcessLightmap(byte** pic,
                        int in_padding,
                        int width,
                        int height,
                        byte** pic_out);  // Arnout
...
void R_RemapShader(const char* oldShader,
...
#define RB_CHECKOVERFLOW(v, i)                         \
  if (tess.numVertexes + (v) >= tess.maxShaderVerts || \
...
void RB_AddFlare(
    ...
    vec3_t normal,
    int id,
    qboolean visible);  //----(SA)	added scale.  added id.  added visible
...
srfGridMesh_t* R_GridInsertColumn(srfGridMesh_t* grid,
                                  int column,
                                  int row,
                                  vec3_t point,
                                  float loderror);
...
int R_MarkFragments(int orientation,
                    const vec3_t* points,
...
void RE_ProjectDecal(qhandle_t hShader,
                     int numPoints,
                     vec3_t* points,
                     vec4_t projection,
                     vec4_t color,
...
void R_TransformDecalProjector(decalProjector_t* in,
                               vec3_t axis[3],
                               vec3_t origin,
                               decalProjector_t* out);
qboolean R_TestDecalBoundingBox(decalProjector_t* dp, vec3_t mins, vec3_t maxs);
qboolean R_TestDecalBoundingSphere(decalProjector_t* dp,
                                   vec3_t center,
                                   float radius2);

void R_ProjectDecalOntoSurface(decalProjector_t* dp,
                               msurface_t* surf,
                               bmodel_t* bmodel);
...
void RE_AddPolyToScene(qhandle_t hShader,
                       int numVerts,
                       const polyVert_t* verts);
// Ridah
void RE_AddPolysToScene(qhandle_t hShader,
                        int numVerts,
                        const polyVert_t* verts,
                        int numPolys);
// done.
...
void RE_AddLightToScene(const vec3_t org,
                        float radius,
                        float intensity,
                        float r,
                        float g,
                        ...
                        int flags);
//----(SA)
void RE_AddCoronaToScene(const vec3_t org,
                         float r,
                         float g,
                         float b,
                         float scale,
...
void RE_RotatedPic(float x,
                   ...
                   qhandle_t hShader,
                   float angle);  // NERVE - SMF
void RE_StretchPicGradient(float x,
                           float y,
                           float w,
                           float h,
                           float s1,
...
void RE_SetGlobalFog(qboolean restore,
                     int duration,
                     float r,
                     float g,
                     float b,
...
#define R_CacheShaderAlloc(name, size) \
  R_CacheShaderAllocExt(name, size, __FILE__, __LINE__)
#define R_CacheShaderFree(name, ptr) \
  R_CacheShaderFreeExt(name, ptr, __FILE__, __LINE__)
...
// NOTE: MDC_MAX_ERROR is effectively the compression level. the lower this
// value, the higher the accuracy, but with lower compression ratios.
#define MDC_MAX_ERROR \
  0.1  // if any compressed vert is off by more than this from the
       // actual vert, make this a baseframe

#define MDC_DIST_SCALE 0.05  // lower for more accuracy, but less range

// note: we are locked in at 8 or less bits since changing to byte-encoded
// normals
...
extern void R_SetFog(int fogvar,
                     int var1,
                     int var2,
                     float r,
                     float g,
