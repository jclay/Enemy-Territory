...
** GL_BindMultitexture
*/
void GL_BindMultitexture(image_t* image0,
                         GLuint env0,
                         image_t* image1,
                         GLuint env1) {
  int texnum0, texnum1;
...
void RB_BeginDrawingView(void) {
  ...
  //		clearBits = GL_DEPTH_BUFFER_BIT;	// (SA) always just
  //clear depth for menus
  //	}
  // ydnar: global q3 fog volume
    ...
    if (backEnd.refdef.rdflags &
        RDF_SKYBOXPORTAL) {  // portal scene, clear whatever is necessary

      clearBits |= GL_DEPTH_BUFFER_BIT;

      if (r_fastsky->integer ||
          backEnd.refdef.rdflags & RDF_NOWORLDMODEL) {  // fastsky: clear color

        // try clearing first with the portal sky fog color, then the world fog
        // color, then finally a default
        clearBits |= GL_COLOR_BUFFER_BIT;
        if (glfogsettings[FOG_PORTALVIEW].registered) {
          qglClearColor(glfogsettings[FOG_PORTALVIEW].color[0],
                        glfogsettings[FOG_PORTALVIEW].color[1],
                        glfogsettings[FOG_PORTALVIEW].color[2],
                        glfogsettings[FOG_PORTALVIEW].color[3]);
        } else if (glfogNum > FOG_NONE &&
                   glfogsettings[FOG_CURRENT].registered) {
          qglClearColor(glfogsettings[FOG_CURRENT].color[0],
                        glfogsettings[FOG_CURRENT].color[1],
                        glfogsettings[FOG_CURRENT].color[2],
                        glfogsettings[FOG_CURRENT].color[3]);
        } else {
          //					qglClearColor ( 1.0, 0.0,
          //0.0, 1.0 );	// red clear for testing portal sky clear
        ...
        if (glfogsettings[FOG_PORTALVIEW].registered) {
          ...
          if (glfogsettings[FOG_PORTALVIEW]
                  .clearscreen) {  // portal fog requests a screen clear
                                   // (distance fog rather than quake sky)
            clearBits |= GL_COLOR_BUFFER_BIT;
          }
        }
      }
    } else {  // world scene with portal sky, don't clear any buffers, just set
              // the fog color if there is one

      clearBits |= GL_DEPTH_BUFFER_BIT;  // this will go when I get the portal
                                         // sky rendering way out in the zbuffer
                                         // (or not writing to zbuffer at all)
      ...
      if (glfogNum > FOG_NONE && glfogsettings[FOG_CURRENT].registered) {
        if (backEnd.refdef.rdflags & RDF_UNDERWATER) {
          if (glfogsettings[FOG_CURRENT].mode == GL_LINEAR) {
            clearBits |= GL_COLOR_BUFFER_BIT;
                         ...
                         ->integer)) {  // portal skies have been manually
                                        // turned off, clear bg color
          clearBits |= GL_COLOR_BUFFER_BIT;
        }
        ...
        qglClearColor(glfogsettings[FOG_CURRENT].color[0],
                      ...
                      glfogsettings[FOG_CURRENT].color[3]);
                       ...
                       ->integer)) {  // ydnar: portal skies have been manually
                                      // turned off, clear bg color
        clearBits |= GL_COLOR_BUFFER_BIT;
        qglClearColor(0.5, 0.5, 0.5, 1.0);
      }
    ...
    // NERVE - SMF - we don't want to clear the buffer when no world model is
    // specified
    ...
    // -NERVE - SMF
      ...
      if (glfogsettings[FOG_CURRENT]
              .registered) {  // try to clear fastsky with current fog color
        qglClearColor(glfogsettings[FOG_CURRENT].color[0],
                      glfogsettings[FOG_CURRENT].color[1],
                      glfogsettings[FOG_CURRENT].color[2],
                      glfogsettings[FOG_CURRENT].color[3]);
        ...
        //// blue clear for testing world sky clear
        qglClearColor(0.05, 0.05, 0.05,
                      1.0);  // JPW NERVE changed per id req was 0.5s
      }
    } else {  // world scene, no portal sky, not fastsky, clear color if fog
              // says to, otherwise, just set the clearcolor
      if (glfogsettings[FOG_CURRENT]
              .registered) {  // try to clear fastsky with current fog color
        qglClearColor(glfogsettings[FOG_CURRENT].color[0],
                      glfogsettings[FOG_CURRENT].color[1],
        ...
        if (glfogsettings[FOG_CURRENT]
                .clearscreen) {  // world fog requests a screen clear (distance
                                 // fog rather than quake sky)
          clearBits |= GL_COLOR_BUFFER_BIT;
        }
      }
    }
  ...
  if (backEnd.viewParms.isPortal) {
    ...
    plane2[3] =
        DotProduct(plane, backEnd.viewParms.orientation.origin) - plane[3];
...
void RB_RenderDrawSurfList(drawSurf_t* drawSurfs, int numDrawSurfs) {
  ...
  for (i = 0, drawSurf = drawSurfs; i < numDrawSurfs; i++, drawSurf++) {
    ...
    // seperate entities merged into a single batch, like smoke and blood puff
    // sprites
    ...
    if (entityNum != oldEntityNum) {
      ...
      if (entityNum != ENTITYNUM_WORLD) {
        ...
        backEnd.refdef.floatTime =
            originalTime;  // - backEnd.currentEntity->e.shaderTime; // JPW
                           // NERVE pulled this to match q3ta
...
void RE_StretchRaw(int x,
  ...
  if (cols != tr.scratchImage[client]->width ||
    ...
    tr.scratchImage[client]->height = tr.scratchImage[client]->uploadHeight =
        rows;
    qglTexImage2D(GL_TEXTURE_2D, 0, 3, cols, rows, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                  data);
...
const void* RB_StretchPicGradient(const void* data) {
  ...
  //		*(int *)tess.vertexColors[ numVerts + 1 ].v =
  //		*(int *)tess.vertexColors[ numVerts + 2 ].v =
  //		*(int *)tess.vertexColors[ numVerts + 3 ].v = *(int
  //*)backEnd.color2D;
...
const void* RB_RenderToTexture(const void* data) {
  ...
  qglCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, cmd->x, cmd->y, cmd->w, cmd->h,
                    0);
  //	qglCopyTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, cmd->x, cmd->y, cmd->w,
  //cmd->h );
