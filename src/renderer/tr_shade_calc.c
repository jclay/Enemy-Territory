...
#define WAVEVALUE(table, base, amplitude, phase, freq)                     \
  ((base) +                                                                \
   table[myftol((((phase) + tess.shaderTime * (freq)) * FUNCTABLE_SIZE)) & \
         FUNCTABLE_MASK] *                                                 \
       (amplitude))
...
void RB_CalcDeformVertexes(deformStage_t* ds) {
  ...
  if (ds->deformationWave.frequency < 0) {
    ...
    VectorScale(worldUp,
                0.4 + 0.6 * Q_fabs(backEnd.currentEntity->e.fireRiseDir[2]),
                worldUp);

    ds->deformationWave.frequency *= -1;
    if (ds->deformationWave.frequency >
        999) {  // hack for negative Z deformation (ack)
      inverse = qtrue;
...
void RB_CalcMoveVertexes(deformStage_t* ds) {
  ...
  scale =
      WAVEVALUE(table, ds->deformationWave.base, ds->deformationWave.amplitude,
...
void RB_CalcWaveColor(const waveForm_t* wf, unsigned char* dstColors) {
  ...
  if (wf->func == GF_NOISE) {
    glow =
        wf->base +
        R_NoiseGet4f(0, 0, 0, (tess.shaderTime + wf->phase) * wf->frequency) *
            wf->amplitude;
  } else {
...
void RB_CalcFogTexCoords(float* st) {
  ...
  // if the brush model containing the fog volume wasn't in the scene, then
  // don't bother rendering the fog
  ...
  //%	VectorSubtract( backEnd.orientation.viewOrigin, bmodel->origin[
  //backEnd.smpFrame ], viewOrigin );
  ...
  fogSurface[3] =
      fog->surface[3] +
      DotProduct(fogSurface, bmodel->orientation[backEnd.smpFrame].origin);
  ...
  if (fog->originalBrushNumber >= 0) {
    ...
    if (fog->hasSurface) {
      fogDepthVector[0] = fogSurface[0] * backEnd.orientation.axis[0][0] +
                          fogSurface[1] * backEnd.orientation.axis[0][1] +
                          fogSurface[2] * backEnd.orientation.axis[0][2];
      fogDepthVector[1] = fogSurface[0] * backEnd.orientation.axis[1][0] +
                          fogSurface[1] * backEnd.orientation.axis[1][1] +
                          fogSurface[2] * backEnd.orientation.axis[1][2];
      fogDepthVector[2] = fogSurface[0] * backEnd.orientation.axis[2][0] +
                          fogSurface[1] * backEnd.orientation.axis[2][1] +
                          fogSurface[2] * backEnd.orientation.axis[2][2];
      fogDepthVector[3] =
          -fogSurface[3] + DotProduct(backEnd.orientation.origin, fogSurface);
...
#undef DEBUG_ENVMAPPING
  ...
  for (i = 0; i < tess.numVertexes; i++, v += 4, normal += 4, st += 2) {
    ...
    reflectedTransformed[0] = reflected[0] * backEnd.orientation.axis[0][0] +
                              reflected[1] * backEnd.orientation.axis[1][0] +
                              reflected[2] * backEnd.orientation.axis[2][0];
    reflectedTransformed[1] = reflected[0] * backEnd.orientation.axis[0][1] +
                              reflected[1] * backEnd.orientation.axis[1][1] +
                              reflected[2] * backEnd.orientation.axis[2][1];
    reflectedTransformed[2] = reflected[0] * backEnd.orientation.axis[0][2] +
                              reflected[1] * backEnd.orientation.axis[1][2] +
    ...
    qglVertex3f(v[0] + reflected[0] * 2, v[1] + reflected[1] * 2,
                v[2] + reflected[2] * 2);
#endif
  }
...
void RB_CalcTurbulentTexCoords(const waveForm_t* wf, float* st) {
  ...
  for (i = 0; i < tess.numVertexes; i++, st += 2) {
    ...
    st[0] = s + tr.sinTable[((int)(((tess.xyz[i].v[0] + tess.xyz[i].v[2]) *
                                        1.0 / 128 * 0.125 +
                                    now) *
                                   FUNCTABLE_SIZE)) &
                            (FUNCTABLE_MASK)] *
                    wf->amplitude;
    st[1] =
        t + tr.sinTable[((int)((tess.xyz[i].v[1] * 1.0 / 128 * 0.125 + now) *
                               FUNCTABLE_SIZE)) &
                        (FUNCTABLE_MASK)] *
                wf->amplitude;
  }
...
// saves about 1-2ms per frame on my machine with 64 x 1000 triangle models in
// scene
