...
// Ridah
static qboolean R_LoadMDC(model_t* mod,
                          int lod,
                          void* buffer,
                          const char* mod_name);
// done.
static qboolean R_LoadMD3(model_t* mod,
                          int lod,
                          void* buffer,
                          const char* name);
  ...
  if (buf != NULL) {
    ...
    if (shadowBits != NULL) {
      *shadowBits = '\0';
      ...
      sscanf(shadowBits, "%f %f %f %f %f %f", &mod->shadowParms[0],
             &mod->shadowParms[1], &mod->shadowParms[2], &mod->shadowParms[3],
             &mod->shadowParms[4], &mod->shadowParms[5]);
    }
    ri.FS_FreeFile(buf);
  }
}
...
qhandle_t RE_RegisterModel(const char* name) {
  ...
  if (!name || !name[0]) {
    // Ridah, disabled this, we can see models that can't be found because they
    // won't be there
  ...
  if (strstr(name, ".mds") || strstr(name, ".mdm") ||
      strstr(name, ".mdx")) {  // try loading skeletal file
    loaded = qfalse;
  ...
  if (numLoaded) {
    ...
    for (lod--; lod >= 0; lod--) {
      mod->numLods++;
      // Ridah, mesh compression
      //	this check for mod->md3[0] could leave mod->md3[0] == 0x0000000
      //if r_lodbias is set to anything except '0' 	which causes trouble in
      //tr_mesh.c in R_AddMD3Surfaces() and other locations since it checks
      //md3[0] 	for various things.
...
unsigned char R_MDC_GetAnorm(const vec3_t dir) {
  ...
  for (i = next_start; i < next_end; i++) {
    ...
    if (next_start) {
      if (r_anormals[i][2] > dir[2]) {
        break;  // we've gone passed the dir[2], so we can't possibly find a
                // better match now
      }
    } else {
      if (r_anormals[i][2] < dir[2]) {
  ...
  for (i = best_start_i[2], group_val = r_anormals[i][2];
       i < NUMMDCVERTEXNORMALS; i++) {
...
=================
*/
qboolean R_MDC_EncodeXyzCompressed(const vec3_t vec,
                                   const vec3_t normal,
  ...
  for (i = 0; i < 3; i++) {
    ...
    retval.ofsVec +=
        (((int)Q_fabs((vec[i] + MDC_DIST_SCALE * 0.5) * (1.0 / MDC_DIST_SCALE) +
                      MDC_MAX_OFS))
         << (i * MDC_BITS_PER_AXIS));
  }
...
static qboolean R_LoadMDC(model_t* mod,
  ...
  if (version != MDC_VERSION) {
    ri.Printf(PRINT_WARNING,
              "R_LoadMDC: %s has wrong version (%i should be %i)\n", mod_name,
              version, MDC_VERSION);
    return qfalse;
  }
  ...
  // swap all the frames
  frame = (md3Frame_t*)((byte*)mod->model.mdc[lod] +
                        mod->model.mdc[lod]->ofsFrames);
  ...
  if (LittleLong(1) != 1) {
    for (i = 0;
         i < mod->model.mdc[lod]->numTags * mod->model.mdc[lod]->numFrames;
         i++, tag++) {
      for (j = 0; j < 3; j++) {
  ...
  surf = (mdcSurface_t*)((byte*)mod->model.mdc[lod] +
                         mod->model.mdc[lod]->ofsSurfaces);
  for (i = 0; i < mod->model.mdc[lod]->numSurfaces; i++) {
    ...
    if (surf->numVerts > tess.maxShaderVerts) {
      ri.Error(ERR_DROP,
               "R_LoadMDC: %s has more than %i verts on a surface (%i)",
               mod_name, tess.maxShaderVerts, surf->numVerts);
    }
    if (surf->numTriangles * 3 > tess.maxShaderIndicies) {
      ri.Error(ERR_DROP,
               "R_LoadMDC: %s has more than %i triangles on a surface (%i)",
...
static qboolean R_LoadMD3(model_t* mod,
  ...
  // swap all the frames
  frame = (md3Frame_t*)((byte*)mod->model.md3[lod] +
                        mod->model.md3[lod]->ofsFrames);
  ...
  for (i = 0; i < mod->model.md3[lod]->numTags * mod->model.md3[lod]->numFrames;
       i++, tag++) {
    for (j = 0; j < 3; j++) {
  ...
  // swap all the surfaces
  surf = (md3Surface_t*)((byte*)mod->model.md3[lod] +
                         mod->model.md3[lod]->ofsSurfaces);
...
static qboolean R_LoadMDS(model_t* mod, void* buffer, const char* mod_name) {
  ...
  if (version != MDS_VERSION) {
    ri.Printf(PRINT_WARNING,
              "R_LoadMDS: %s has wrong version (%i should be %i)\n", mod_name,
              version, MDS_VERSION);
    return qfalse;
  }
  ...
  if (LittleLong(1) != 1) {
    ...
    frameSize = (int)(sizeof(mdsFrame_t) - sizeof(mdsBoneFrameCompressed_t) +
                      mds->numBones * sizeof(mdsBoneFrameCompressed_t));
    for (i = 0; i < mds->numFrames; i++, frame++) {
      ...
      for (j = 0;
           j < mds->numBones * sizeof(mdsBoneFrameCompressed_t) / sizeof(short);
           j++) {
    ...
    for (i = 0; i < mds->numBones; i++, bi++) {
      bi = (mdsBoneInfo_t*)((byte*)mds + mds->ofsBones +
                            i * sizeof(mdsBoneInfo_t));
      LL(bi->parent);
  ...
  for (i = 0; i < mds->numSurfaces; i++) {
    ...
    if (surf->numVerts > tess.maxShaderVerts) {
      ri.Error(ERR_DROP,
               "R_LoadMDS: %s has more than %i verts on a surface (%i)",
               mod_name, tess.maxShaderVerts, surf->numVerts);
    }
    if (surf->numTriangles * 3 > tess.maxShaderIndicies) {
      ri.Error(ERR_DROP,
               "R_LoadMDS: %s has more than %i triangles on a surface (%i)",
               mod_name, tess.maxShaderIndicies / 3, surf->numTriangles);
    }
    ...
    if (LittleLong(1) != 1) {
      ...
      for (j = 0; j < surf->numVerts; j++) {
        ...
        if (v->numWeights == 2) {
          ...
          if (VectorLength(v->weights[0].offset) <
              VectorLength(v->weights[1].offset)) {
            v->fixedParent = 0;
          } else {
            v->fixedParent = 1;
          }
...
static qboolean R_LoadMDM(model_t* mod, void* buffer, const char* mod_name) {
  ...
  if (version != MDM_VERSION) {
    ri.Printf(PRINT_WARNING,
              "R_LoadMDM: %s has wrong version (%i should be %i)\n", mod_name,
              version, MDM_VERSION);
    return qfalse;
  }
  ...
  if (LittleLong(1) != 1) {
    ...
    for ( i = 0 ; i < mdm->numFrames ; i++, frame++) {
            frame = (mdmFrame_t *) ( (byte *)mdm + mdm->ofsFrames + i *
    frameSize ); frame->radius = LittleFloat( frame->radius ); for ( j = 0 ; j <
    3 ; j++ ) { frame->bounds[0][j] = LittleFloat( frame->bounds[0][j] );
                    frame->bounds[1][j] = LittleFloat( frame->bounds[1][j] );
                    frame->localOrigin[j] = LittleFloat( frame->localOrigin[j]
    ); frame->parentOffset[j] = LittleFloat( frame->parentOffset[j] );
            }
  ...
  for (i = 0; i < mdm->numSurfaces; i++) {
    ...
    if (surf->numVerts > tess.maxShaderVerts) {
      ri.Error(ERR_DROP,
               "R_LoadMDM: %s has more than %i verts on a surface (%i)",
               mod_name, tess.maxShaderVerts, surf->numVerts);
    }
    if (surf->numTriangles * 3 > tess.maxShaderIndicies) {
      ri.Error(ERR_DROP,
               "R_LoadMDM: %s has more than %i triangles on a surface (%i)",
               mod_name, tess.maxShaderIndicies / 3, surf->numTriangles);
    }
...
static qboolean R_LoadMDX(model_t* mod, void* buffer, const char* mod_name) {
  ...
  if (version != MDX_VERSION) {
    ri.Printf(PRINT_WARNING,
              "R_LoadMDX: %s has wrong version (%i should be %i)\n", mod_name,
              version, MDX_VERSION);
    return qfalse;
  }
  ...
  if (LittleLong(1) != 1) {
    ...
    for (i = 0; i < mdx->numFrames; i++) {
      ...
      bframe = (short*)((byte*)mdx + mdx->ofsFrames + i * frameSize +
                        ((i + 1) * sizeof(mdxFrame_t)));
      for (j = 0;
           j < mdx->numBones * sizeof(mdxBoneFrameCompressed_t) / sizeof(short);
           j++) {
    ...
    for (i = 0; i < mdx->numBones; i++) {
      bi = (mdxBoneInfo_t*)((byte*)mdx + mdx->ofsBones +
                            i * sizeof(mdxBoneInfo_t));
      LL(bi->parent);
      bi->torsoWeight = LittleFloat(bi->torsoWeight);
      bi->parentDist = LittleFloat(bi->parentDist);
      LL(bi->flags);
...
static int R_GetTag(byte* mod,
  ...
  for (i = 0; i < md3->numTags; i++, tag++) {
    if ((i >= startTagIndex) && !strcmp(tag->name, tagName)) {
      // if we are looking for an indexed tag, wait until we find the correct
      // number of matches
      // if (startTagIndex) {
      //	startTagIndex--;
      //	continue;
...
================
*/
static int R_GetMDCTag(byte* mod,
                       int frame,
                       const char* tagName,
                       int startTagIndex,
                       mdcTag_t** outTag) {
...
/*
// TTimo: unused
static int R_GetMDSTag( byte *mod, const char *tagName, int startTagIndex,
mdsTag_t **outTag ) { mdsTag_t		*tag; int i; mdsHeader_t
*mds;
        ...
        for ( i = 0 ; i < mds->numTags ; i++ ) {
                ...
                tag = (mdsTag_t *) ((byte *)tag + sizeof(mdsTag_t) -
sizeof(mdsBoneFrameCompressed_t) + mds->numFrames *
sizeof(mdsBoneFrameCompressed_t) );
        }
...
R_LerpTag
  ...
  returns the index of the tag it found, for cycling through tags with the same
name
================
*/
int R_LerpTag(orientation_t* tag,
  ...
  /*
          // if the tagName has a space in it, then it is passing through the
     starting tag number if (ch = strrchr(tagName, ' ')) { *ch = 0; ch++;
                  startIndex = atoi(ch);
          }
  */
  ...
  if (model->type == MOD_MESH) {
    // old MD3 style
    retval = R_GetTag((byte*)model->model.md3[0], startFrame, tagName,
                      startIndex, &start);
    retval = R_GetTag((byte*)model->model.md3[0], endFrame, tagName, startIndex,
                      &end);
    ...
    retval =
        R_MDM_GetBoneTag(tag, model->model.mdm, startIndex, refent, tagNameIn);
    ...
    retval = R_GetMDCTag((byte*)model->model.mdc[0], startFrame, tagName,
                         startIndex, &cstart);
    retval = R_GetMDCTag((byte*)model->model.mdc[0], endFrame, tagName,
                         startIndex, &cend);
...
void R_TagInfo_f(void) {
  ...
  /*
          ...
          R_LerpTag( &tag, handle, frame, frame, 0.0, (const char
     *)ri.Cmd_Argv(2) );
...
void R_ModelBounds(qhandle_t handle, vec3_t mins, vec3_t maxs) {
  ...
  switch (model->type) {
    ...
    case MOD_MESH:
      ...
      return;
    case MOD_MDC:
      frame = (md3Frame_t*)((byte*)model->model.mdc[0] +
                            model->model.mdc[0]->ofsFrames);
...
void* R_Hunk_Begin(void) {
  ...
  if (!membase) {
    ...
    // it is possible that we have been allocated already, in case we don't do
    // anything
    membase = malloc(maxsize);
    // TTimo NOTE: initially, I was doing the memset even if we had an existing
    // membase but this breaks some shaders (i.e. /map mp_beach, then go back to
    // the main menu .. some shaders are missing) I assume the shader missing is
    // because we don't clear memory either on win32 meaning even on win32 we
    // are using memory that is still reserved but was uncommited .. it works
    // out of pure luck
...
#endif
  // on non win32 OS, we keep the allocated chunk as is, just start again to
  // curzise = 0
...
// TODO: convert the Hunk_Alloc's in the model loading to malloc's, so we don't
// have to move so much memory around during transitions
...
void R_CacheModelFree(void* ptr) {
  if (r_cache->integer && r_cacheModels->integer) {
    // TTimo: it's in the hunk, leave it there, next R_Hunk_Begin will clear it
    // all
  } else {
...
void R_PurgeModels(int count) {
  ...
  // note: we can only do this since we only use the virtual memory for the
  // model caching!
  R_Hunk_Reset();
}
        ...
        case MOD_MESH:
          for (j = MD3_MAX_LODS - 1; j >= 0; j--) {
            if (j < mod->numLods && mod->model.md3[j]) {
              if ((j == MD3_MAX_LODS - 1) ||
                  (mod->model.md3[j] != mod->model.md3[j + 1])) {
                modBack->model.md3[j] =
                    R_CacheModelAlloc(mod->model.md3[j]->ofsEnd);
                memcpy(modBack->model.md3[j], mod->model.md3[j],
                       mod->model.md3[j]->ofsEnd);
              } else {
        ...
        case MOD_MDC:
          for (j = MD3_MAX_LODS - 1; j >= 0; j--) {
            if (j < mod->numLods && mod->model.mdc[j]) {
              if ((j == MD3_MAX_LODS - 1) ||
                  (mod->model.mdc[j] != mod->model.mdc[j + 1])) {
                modBack->model.mdc[j] =
                    R_CacheModelAlloc(mod->model.mdc[j]->ofsEnd);
                memcpy(modBack->model.mdc[j], mod->model.mdc[j],
                       mod->model.mdc[j]->ofsEnd);
              } else {
          ...
          for (j = MD3_MAX_LODS - 1; j >= 0; j--) {
            if (j < mod->numLods && mod->model.md3[j]) {
              if ((j == MD3_MAX_LODS - 1) ||
                  (mod->model.md3[j] != mod->model.md3[j + 1])) {
                newmod->model.md3[j] =
                    ri.Hunk_Alloc(mod->model.md3[j]->ofsEnd, h_low);
                memcpy(newmod->model.md3[j], mod->model.md3[j],
                       mod->model.md3[j]->ofsEnd);
          ...
          for (j = MD3_MAX_LODS - 1; j >= 0; j--) {
            if (j < mod->numLods && mod->model.mdc[j]) {
              if ((j == MD3_MAX_LODS - 1) ||
                  (mod->model.mdc[j] != mod->model.mdc[j + 1])) {
                newmod->model.mdc[j] =
                    ri.Hunk_Alloc(mod->model.mdc[j]->ofsEnd, h_low);
                memcpy(newmod->model.mdc[j], mod->model.mdc[j],
                       mod->model.mdc[j]->ofsEnd);
...
void R_LoadCacheModels(void) {
  ...
  // don't load the cache list in between level loads, only on startup, or after
  // a vid_restart
