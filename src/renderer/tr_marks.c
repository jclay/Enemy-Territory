...
// Ridah, just make these global to prevent having to add more paramaters, which
// add overhead
...
void R_AddMarkFragments(int numClipPoints,
  ...
  for (i = 0; i < numPlanes; i++) {
    R_ChopPolyBehindPlane(numClipPoints, clipPoints[pingPong], &numClipPoints,
                          clipPoints[!pingPong], normals[i], dists[i], 0.5);
    pingPong ^= 1;
    if (numClipPoints == 0) {
      break;
    }
  ...
  */
  ...
  for (i = 0; i < numClipPoints; i++) {
    VectorCopy(clipPoints[pingPong][i],
               (float*)pointBuffer + 5 * (*returnedPoints + i));
  }
...
=================
*/
int R_OldMarkFragments(int numPoints,
                       const vec3_t* points,
    ...
    if (*surfaces[i] == SF_GRID) {
      ...
      for (m = 0; m < cv->height - 1; m++) {
        for (n = 0; n < cv->width - 1; n++) {
          ...
          VectorMA(clipPoints[0][1], MARKER_OFFSET, dv[cv->width].normal,
                   clipPoints[0][1]);
          ...
          if (DotProduct(normal, projectionDir) < -0.1) {
            ...
            R_AddMarkFragments(numClipPoints, clipPoints, numPlanes, normals,
                               dists, maxPoints, pointBuffer, maxFragments,
          ...
          VectorMA(clipPoints[0][2], MARKER_OFFSET, dv[cv->width + 1].normal,
                   clipPoints[0][2]);
          // check the normal of this triangle
          VectorSubtract(clipPoints[0][0], clipPoints[0][1], v1);
          VectorSubtract(clipPoints[0][2], clipPoints[0][1], v2);
      ...
      */
      ...
      for (k = 0; k < cts->numIndexes; k += 3) {
        for (j = 0; j < 3; j++) {
          ...
          VectorMA(v, MARKER_OFFSET, cts->verts[indexes[k + j]].normal,
                   clipPoints[0][j]);
        }
        // add the fragments of this face
        R_AddMarkFragments(3, clipPoints, numPlanes, normals, dists, maxPoints,
...
int R_MarkFragments(int orientation,
  ...
  // vec3_t			bestCenter;	// center point projected onto
  // the closest surface
  float texCoordScale;
  // float			dot;
  int numPoints = 4;  // Ridah, we were only ever passing in 4, so I made this
                      // local and used the parameter for the orientation
  ...
  if (maxFragments < 0) {
    ...
    // return R_OldMarkFragments( numPoints, points, projection, maxPoints,
    // pointBuffer, maxFragments, fragmentBuffer );
    oldMapping = qtrue;
  }
  ...
  for (i = 0; i < numPoints; i++) {
    ...
    // make sure we get all the leafs (also the one(s) in front of the hit
    // surface)
    VectorMA(points[i], -20 * (1.0 + (float)oldMapping * (radius / 20.0) * 4),
             projectionDir, temp);
    AddPointToBounds(temp, mins, maxs);
  }
  ...
  // add near and far clipping planes for projection
  VectorCopy(projectionDir, normals[numPoints]);
  dists[numPoints] = DotProduct(normals[numPoints], points[0]) -
                     radius * (1 + oldMapping * 10);
  VectorCopy(projectionDir, normals[numPoints + 1]);
  VectorInverse(normals[numPoints + 1]);
  dists[numPoints + 1] = DotProduct(normals[numPoints + 1], points[0]) -
                         radius * (1 + oldMapping * 10);
  numPlanes = numPoints + 2;

  numsurfaces = 0;
  R_BoxSurfaces_r(tr.world->nodes, mins, maxs, surfaces, 4096, &numsurfaces,
                  projectionDir);
  // assert(numsurfaces <= 64);
  // assert(numsurfaces != 64);
  ...
  // find the closest surface to center the decal there, and wrap around other
  // surfaces
  if (!oldMapping) {
    /*
                    for ( i = 0 ; i < numsurfaces ; i++ ) {
                            if (*surfaces[i] == SF_FACE) {
                                    surf = ( srfSurfaceFace_t * ) surfaces[i];
                                    // Ridah, check if this is the closest
       surface dot = DotProduct( center, surf->plane.normal ); dot -=
       surf->plane.dist; if (!bestdist) { if (dot < 0) bestdist = Q_fabs(dot) +
       1000;	// avoid this surface, since the point is behind it else
                                                    bestdist = dot;
                                            VectorCopy( surf->plane.normal,
       bestnormal ); VectorMA( center, -dot, surf->plane.normal, bestCenter );
  ...
  for (i = 0; i < numsurfaces; i++) {
    if (*surfaces[i] == SF_GRID) {
      ...
      int oldNumPoints;
      float epsilon = 0.5;
      // duplicated so we don't mess with the original clips for the curved
      // surfaces
      ...
      if (!oldMapping) {
        // Ridah, create a new clip box such that this decal surface is mapped
        // onto the current surface without distortion. To find the center of
        // the new clip box, we project the center of the original impact center
        // out along the projection vector, onto the current surface
        ...
        if (surf->plane.type == PLANE_NON_PLANAR) {
          ...
          // if the impact point is behind the surface, subtract the projection,
          // otherwise add it
        ...
        for (j = 0; j < 3; j++) {
          originalPoints[0][j] =
              newCenter[j] - radius * axis[1][j] - radius * axis[2][j];
          originalPoints[1][j] =
              newCenter[j] + radius * axis[1][j] - radius * axis[2][j];
          originalPoints[2][j] =
              newCenter[j] + radius * axis[1][j] + radius * axis[2][j];
        ...
        for (j = 0; j < 4; j++) {
          ...
          VectorSubtract(originalPoints[(j + 1) % numPoints], originalPoints[j],
                         v1);
        ...
        for (k = 0; k < surf->numIndices; k += 3) {
          ...
          // add the fragments of this face
          R_AddMarkFragments(3, clipPoints, numPlanes, lnormals, ldists,
                             maxPoints, pointBuffer, maxFragments,
                             fragmentBuffer, &returnedPoints,
                             &returnedFragments, lmins, lmaxs);
        ...
        // check the normal of this face
        // Arnout: wolf code was disabled, but differs from q3 code (maybe wolf
        // one was broken and fixed in q3?):
        /*				if (DotProduct(surf->plane.normal,
           projectionDir) > -0.5) { continue;
                                        }*/
    ...
    // Arnout: projection on models (mainly for terrain though)
      ...
      if (!oldMapping) {
        ...
        ldists[numPoints+1] = DotProduct(lnormals[numPoints+1], points[0]) -
        radius;*/
