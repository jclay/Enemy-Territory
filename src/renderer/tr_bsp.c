...
float R_ProcessLightmap(byte** pic,
  ...
  if (r_lightmap->integer > 1) {  // color code by intensity as development tool
                                  // (FIXME: check range)
...
static void R_LoadLightmaps(lump_t* l) {
  ...
  for (i = 0; i < tr.numLightmaps; i++) {
    ...
    intensity =
        R_ProcessLightmap(&buf_p, 3, LIGHTMAP_SIZE, LIGHTMAP_SIZE, &image_p);
    if (intensity > maxIntensity) {
      maxIntensity = intensity;
    }

    tr.lightmaps[i] = R_CreateImage(va("*lightmap%d", i), image, LIGHTMAP_SIZE,
                                    LIGHTMAP_SIZE, qfalse, qfalse, GL_CLAMP);
  }
...
static void SphereFromBounds(vec3_t mins,
                             vec3_t maxs,
                             vec3_t origin,
                             float* radius) {
  vec3_t temp;
...
#else
static void ParseTriSurf(dsurface_t* ds,
                         drawVert_t* verts,
                         msurface_t* surf,
                         int* indexes) {
  srfTriangles_t* tri;
...
static void ParseFoliage(dsurface_t* ds,
                         drawVert_t* verts,
                         msurface_t* surf,
                         int* indexes) {
  ...
  size = sizeof(*foliage) +
         numVerts *
             (sizeof(foliage->xyz[0]) + sizeof(foliage->normal[0]) +
              sizeof(foliage->texCoords[0]) + sizeof(foliage->lmTexCoords[0])) +
  ...
  foliage->instances =
      (foliageInstance_t*)(foliage->indexes + foliage->numIndexes);
  ...
  for (i = 0; i < numInstances; i++) {
    ...
    AddPointToBounds(boundsTranslated[0], foliage->bounds[0],
                     foliage->bounds[1]);
    AddPointToBounds(boundsTranslated[1], foliage->bounds[0],
                     foliage->bounds[1]);
...
static void ParseFlare(dsurface_t* ds,
                       drawVert_t* verts,
                       msurface_t* surf,
                       int* indexes) {
  srfFlare_t* flare;
...
int R_MergedWidthPoints(srfGridMesh_t* grid, int offset) {
  ...
  for (i = 1; i < grid->width - 1; i++) {
    for (j = i + 1; j < grid->width - 1; j++) {
      if (Q_fabs(grid->verts[i + offset].xyz[0] -
                 grid->verts[j + offset].xyz[0]) > .1) {
        continue;
      }
      if (Q_fabs(grid->verts[i + offset].xyz[1] -
                 grid->verts[j + offset].xyz[1]) > .1) {
        continue;
      }
      if (Q_fabs(grid->verts[i + offset].xyz[2] -
...
int R_MergedHeightPoints(srfGridMesh_t* grid, int offset) {
  ...
  for (i = 1; i < grid->height - 1; i++) {
    for (j = i + 1; j < grid->height - 1; j++) {
      if (Q_fabs(grid->verts[grid->width * i + offset].xyz[0] -
                 grid->verts[grid->width * j + offset].xyz[0]) > .1) {
        continue;
      }
      if (Q_fabs(grid->verts[grid->width * i + offset].xyz[1] -
                 grid->verts[grid->width * j + offset].xyz[1]) > .1) {
        continue;
      }
      if (Q_fabs(grid->verts[grid->width * i + offset].xyz[2] -
      ...
      for (k = 1; k < grid1->width - 1; k++) {
        for (m = 0; m < 2; m++) {
          ...
          for (l = 1; l < grid2->width - 1; l++) {
            //
            if (Q_fabs(grid1->verts[k + offset1].xyz[0] -
                       grid2->verts[l + offset2].xyz[0]) > .1) {
              continue;
            }
            if (Q_fabs(grid1->verts[k + offset1].xyz[1] -
                       grid2->verts[l + offset2].xyz[1]) > .1) {
              continue;
            }
            if (Q_fabs(grid1->verts[k + offset1].xyz[2] -
    ...
    for (n = 0; n < 2; n++) {
      ...
      for (k = 1; k < grid1->height - 1; k++) {
        for (m = 0; m < 2; m++) {
          ...
          for (l = 1; l < grid2->width - 1; l++) {
            //
            if (Q_fabs(grid1->verts[grid1->width * k + offset1].xyz[0] -
                       grid2->verts[l + offset2].xyz[0]) > .1) {
              continue;
            }
            if (Q_fabs(grid1->verts[grid1->width * k + offset1].xyz[1] -
                       grid2->verts[l + offset2].xyz[1]) > .1) {
              continue;
            }
            if (Q_fabs(grid1->verts[grid1->width * k + offset1].xyz[2] -
                       grid2->verts[l + offset2].xyz[2]) > .1) {
              continue;
            }
            // ok the points are equal and should have the same lod error
    ...
    for (k = 0; k < grid1->width - 2; k += 2) {
      for (m = 0; m < 2; m++) {
        ...
        for (l = 0; l < grid2->width - 1; l++) {
          ...
          v2 = grid2->verts[l + 1 + offset2].xyz;
          if (Q_fabs(v1[0] - v2[0]) < .01 && Q_fabs(v1[1] - v2[1]) < .01 &&
              Q_fabs(v1[2] - v2[2]) < .01) {
            continue;
          }
          //
          // ri.Printf( PRINT_ALL, "found highest LoD crack between two
          // patches\n" );
...
void R_MovePatchSurfacesToHunk(void) {
  ...
  for (i = 0; i < s_worldData.numsurfaces; i++) {
    ...
    //
    size =
        (grid->width * grid->height - 1) * sizeof(drawVert_t) + sizeof(*grid);
...
static void R_LoadSurfaces(lump_t* surfs, lump_t* verts, lump_t* indexLump) {
  ...
  // look for memory for each surface, we allocate a big block and just chew it
  // up as we go
  R_InitSurfMemory();
  ...
  if (node->contents != -1) {
    ...
    if (node->nummarksurfaces > 0) {
      ...
      while (c--) {
        ...
        if (gen->surfaceType != SF_FACE && gen->surfaceType != SF_GRID &&
...
static void R_LoadNodesAndLeafs(lump_t* nodeLump, lump_t* leafLump) {
  ...
  s_worldData.skyNodes =
      ri.Hunk_Alloc(WORLD_MAX_SKY_NODES * sizeof(*s_worldData.skyNodes), h_low);
    ...
    if (out->originalBrushNumber == -1) {
      ...
      for (j = 0; j < s_worldData.numBModels; j++) {
        if (out->originalBrushNumber >= s_worldData.bmodels[j].firstBrush &&
            out->originalBrushNumber < (s_worldData.bmodels[j].firstBrush +
                                        s_worldData.bmodels[j].numBrushes)) {
          out->modelNum = j;
          break;
        }
      }
...
void RE_LoadWorldMap(const char* name) {
  ...
  tr.sunShader =
      0;  // clear sunshader so it's not there if the level doesn't specify it

  // inalidate fogs (likely to be re-initialized to new values by the current
  // map)
  ...
  //&header->lumps[LUMP_BRUSHSIDES] ); %	ri.Cmd_ExecuteText( EXEC_NOW,
  //"updatescreen\n" );
  R_LoadSurfaces(&header->lumps[LUMP_SURFACES], &header->lumps[LUMP_DRAWVERTS],
