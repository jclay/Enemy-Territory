...
//
// !!! NOTE: Any changes made here must be duplicated in tr_cmesh.c for MDC
// support
...
int R_ComputeLOD(trRefEntity_t* ent) {
  ...
  if (tr.currentModel->numLods < 2) {
    ...
    frame = (md3Frame_t*)(((unsigned char*)tr.currentModel->model.md3[0]) +
                          tr.currentModel->model.md3[0]->ofsFrames);
...
void R_AddMD3Surfaces(trRefEntity_t* ent) {
  ...
  if ((ent->e.frame >= tr.currentModel->model.md3[lod]->numFrames) ||
    ...
    ri.Printf(PRINT_DEVELOPER,
              "R_AddMD3Surfaces: no such frame %d to %d for '%s' (%d)\n",
