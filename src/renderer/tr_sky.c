...
*/
static void MakeSkyVec(float s,
                       float t,
                       int axis,
                       float outSt[2],
                       vec3_t outXYZ) {
  ...
  if (glfogsettings[FOG_SKY].registered) {  // (SA) trying this...
    ///		boxSize = backEnd.viewParms.zFar / 1.75;		// div
    ///sqrt(3)
...
static void DrawSkySide(struct image_s* image,
                        const int mins[2],
                        const int maxs[2]) {
  int s, t;
  ...
  for (t = mins[1] + HALF_SKY_SUBDIVISIONS; t < maxs[1] + HALF_SKY_SUBDIVISIONS;
       t++) {
...
static void DrawSkySideInner(struct image_s* image,
                             const int mins[2],
                             const int maxs[2]) {
  int s, t;
...
static void DrawSkyBoxInner(shader_t* shader) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    DrawSkySideInner(shader->sky.innerbox[sky_texorder[i]], sky_mins_subd,
                     sky_maxs_subd);
  }
}

static void FillCloudySkySide(const int mins[2],
                              const int maxs[2],
                              qboolean addIndexes) {
  int s, t;
...
==============
RB_DrawSun
        (SA) FIXME: sun should render behind clouds, so passing dark areas cover
it up
==============
*/
void RB_DrawSun(void) {
  ...
  qglTranslatef(backEnd.viewParms.orientation.origin[0],
  ...
  */
  ...
  if (r_drawSun->integer > 1) {  // draw flare effect
    ...
    //				If we decide to use the flare business I will
    ///definatly/ improve all this
    ...
    // (SA) FIXME: todo: flare effect should render last (on top of everything
    // else) and only when sun is in view (sun moving out of camera past degree
    // n should start to cause flare dimming until view angle to sun is off by
    // angle n + x.
...
void RB_StageIteratorSky(void) {
  ...
  if (tess.shader->sky.innerbox[0] &&
      tess.shader->sky.innerbox[0] != tr.defaultImage) {
    qglColor3f(tr.identityLight, tr.identityLight, tr.identityLight);
