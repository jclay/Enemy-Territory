...
void RE_AddPolysToScene(qhandle_t hShader,
  ...
  for (j = 0; j < numPolys; j++) {
    if (r_numpolyverts + numVerts > max_polyverts || r_numpolys >= max_polys) {
      //			ri.Printf( PRINT_WARNING, "WARNING:
      //RE_AddPolysToScene: MAX_POLYS or MAX_POLYVERTS reached\n");
      return;
    }
...
void RE_AddLightToScene(const vec3_t org,
  ...
  // early out
  if (!tr.registered || r_numdlights >= MAX_DLIGHTS || radius <= 0 ||
      intensity <= 0) {
    return;
  }
  ...
  dl->radiusInverseCubed =
      dl->radiusInverseCubed * dl->radiusInverseCubed * dl->radiusInverseCubed;
...
// Temp storage for saving view paramters.  Drawing the animated head in the
// corner was creaming important view info.
