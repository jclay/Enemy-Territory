...
typedef struct flare_s {
  ...
  qboolean cgvisible;  // for coronas, the client determines current visibility,
                       // but it's still inserted so it will fade out properly
...
void RB_AddFlare(
    ...
    vec3_t normal,
    int id,
    qboolean cgvisible) {  //----(SA)	added scale. added id.  added visible
  int i;
  flare_t *f, *oldest;
  vec3_t local;
  float d;
  ...
  for (f = r_activeFlares; f; f = f->next) {
    //		if ( f->surface == surface && f->frameSceneNum ==
    //backEnd.viewParms.frameSceneNum && f->inPortal ==
...
void RB_AddDlightFlares(void) {
  ...
  for (i = 0; i < backEnd.refdef.num_dlights; i++, l++) {
    ...
    RB_AddFlare((void*)l, j, l->origin, l->color, 1.0f, NULL, id++,
                qtrue);  //----(SA)	also set scale
  }
}
...
void RB_AddCoronaFlares(void) {
  ...
  if (!(tr.world)) {  // (SA) possible currently at the player model selection
                      // menu
    return;
  }
  ...
  for (i = 0; i < backEnd.refdef.num_coronas; i++, cor++) {
    ...
    for (j = 1; j < tr.world->numfogs; j++) {
      fog = &tr.world->fogs[j];
      for (k = 0; k < 3; k++) {
        if (cor->origin[k] < fog->bounds[0][k] ||
            cor->origin[k] > fog->bounds[1][k]) {
          break;
        }
      }
      if (k == 3) {
    ...
    RB_AddFlare((void*)cor, j, cor->origin, cor->color, cor->scale, NULL,
                cor->id, cor->visible);
  }
}
...
void RB_TestFlare(flare_t* f) {
  ...
  //	qglReadPixels( f->windowX, f->windowY, 1, 1, GL_DEPTH_COMPONENT,
  //GL_FLOAT, &depth ); 	screenZ = backEnd.viewParms.projectionMatrix[14] / 		( (
  //2*depth - 1 ) * backEnd.viewParms.projectionMatrix[11] -
  //backEnd.viewParms.projectionMatrix[10] );

  //----(SA)	24 was way to low tolerance.  It gave Dan problems with free
  //standing light fixtures
  //----(SA)	I will monitor to see if changing this screws up any other
  //situations
  ...
  if (visible) {
    ...
    fade = 1.0f -
           ((backEnd.refdef.time - f->fadeTime) / 1000.0f) * r_flareFade->value;
  }
...
void RB_RenderFlare(flare_t* f) {
  ...
  //			additive blends and have them fog correctly with our
  //distance fog. 		/when/ we fix the blend problems with distance fog, this
  //should 		be changed back to additive since there's nearly no hit for that 		but
  //the alpha blend is noticably slower.

  //	VectorScale( f->color, f->drawIntensity*tr.identityLight, color );
  VectorScale(f->color, tr.identityLight,
              color);  //----(SA)	mod for alpha blend rather than additive
  ...
  size = backEnd.viewParms.viewportWidth *
         ((r_flareSize->value * f->scale) / 640.0 + 8 / -f->eyeZ);
  ...
  tess.vertexColors[tess.numVertexes].v[3] =
      f->drawIntensity *
      255;  //----(SA)	mod for alpha blend rather than additive
            //	tess.vertexColors[tess.numVertexes].v[3] = 255;		//----(SA)
            //mod for alpha blend rather than additive
  tess.numVertexes++;
