...
//-----------------------------------------------------------------------------
// Static Vars, ugly but easiest (and fastest) means of seperating
// RB_SurfaceAnim and R_CalcBones
...
static mdxBoneFrame_t bones[MDX_MAX_BONES], rawBones[MDX_MAX_BONES],
    oldBones[MDX_MAX_BONES];
static char validBones[MDX_MAX_BONES];
static char newBones[MDX_MAX_BONES];
static mdxBoneFrame_t *bonePtr, *bone, *parentBone;
static mdxBoneFrameCompressed_t *cBonePtr, *cTBonePtr, *cOldBonePtr,
    *cOldTBonePtr, *cBoneList, *cOldBoneList, *cBoneListTorso,
    *cOldBoneListTorso;
...
static vec3_t angles, tangles, torsoParentOffset,
    torsoAxis[3];  //, tmpAxis[3];	// rain - unused
...
static int R_CullModel(trRefEntity_t* ent) {
  ...
  newFrame =
      (mdxFrame_t*)((byte*)newFrameHeader + newFrameHeader->ofsFrames +
                    ent->e.frame * (int)(sizeof(mdxBoneFrameCompressed_t)) *
                        newFrameHeader->numBones +
                    ent->e.frame * sizeof(mdxFrame_t));
  oldFrame =
      (mdxFrame_t*)((byte*)oldFrameHeader + oldFrameHeader->ofsFrames +
                    ent->e.oldframe * (int)(sizeof(mdxBoneFrameCompressed_t)) *
                        oldFrameHeader->numBones +
                    ent->e.oldframe * sizeof(mdxFrame_t));

  // cull bounding sphere ONLY if this is not an upscaled entity
  if (!ent->e.nonNormalizedAxes) {
    if (ent->e.frame == ent->e.oldframe &&
        ent->e.frameModel == ent->e.oldframeModel) {
      switch (
...
=================
*/
static float R_CalcMDMLod(refEntity_t* refent,
                          vec3_t origin,
                          float radius,
                          float modelBias,
                          float modelScale) {
  ...
  // compute projected bounding sphere and use that as a criteria for selecting
  // LOD
  ...
  if (projectedRadius != 0) {
    ...
    lodScale = r_lodscale->value;  // fudge factor since MDS uses a much
                                   // smoother method of LOD
...
static int R_ComputeFogNum(trRefEntity_t* ent) {
  ...
  // compute frame pointers
  mdxFrame =
      (mdxFrame_t*)((byte*)header + header->ofsFrames +
                    ent->e.frame * (int)(sizeof(mdxBoneFrameCompressed_t)) *
                        header->numBones +
                    ent->e.frame * sizeof(mdxFrame_t));
...
void R_MDM_AddAnimSurfaces(trRefEntity_t* ent) {
  ...
  for (i = 0; i < header->numSurfaces; i++) {
    if (ent->e.customShader) {
      ...
      if (shader ==
          tr.defaultShader) {  // blink reference in skin was not found
...
__inline void LocalMatrixTransformVectorTranslate(vec3_t in,
                                                  vec3_t mat[3],
                                                  vec3_t tr,
                                                  vec3_t out) {
...
__inline void LocalScaledMatrixTransformVector(vec3_t in,
                                               float s,
                                               vec3_t mat[3],
                                               vec3_t out) {
  out[0] = (1.0f - s) * in[0] +
  ...
  out[1] = (1.0f - s) * in[1] +
           s * (in[0] * mat[1][0] + in[1] * mat[1][1] + in[2] * mat[1][2]);
  out[2] = (1.0f - s) * in[2] +
           s * (in[0] * mat[2][0] + in[1] * mat[2][1] + in[2] * mat[2][2]);
}

__inline void LocalScaledMatrixTransformVectorTranslate(vec3_t in,
                                                        float s,
                                                        vec3_t mat[3],
                                                        vec3_t tr,
                                                        vec3_t out) {
  ...
  out[1] = (1.0f - s) * in[1] + s * (in[0] * mat[1][0] + in[1] * mat[1][1] +
                                     in[2] * mat[1][2] + tr[1]);
  out[2] = (1.0f - s) * in[2] + s * (in[0] * mat[2][0] + in[1] * mat[2][1] +
                                     in[2] * mat[2][2] + tr[2]);
}

__inline void LocalScaledMatrixTransformVectorFullTranslate(vec3_t in,
                                                            float s,
                                                            vec3_t mat[3],
                                                            vec3_t tr,
                                                            vec3_t out) {
...
__inline void LocalAddScaledMatrixTransformVectorFullTranslate(vec3_t in,
                                                               float s,
                                                               vec3_t mat[3],
                                                               vec3_t tr,
                                                               vec3_t out) {
  ...
  out[1] +=
      s * (in[0] * mat[1][0] + in[1] * mat[1][1] + in[2] * mat[1][2]) + tr[1];
  out[2] +=
      s * (in[0] * mat[2][0] + in[1] * mat[2][1] + in[2] * mat[2][2]) + tr[2];
}

__inline void LocalAddScaledMatrixTransformVectorTranslate(vec3_t in,
                                                           float s,
                                                           vec3_t mat[3],
                                                           vec3_t tr,
                                                           vec3_t out) {
...
__inline void LocalAddScaledMatrixTransformVector(vec3_t in,
                                                  float s,
                                                  vec3_t mat[3],
                                                  vec3_t out) {
...
#define ANGLES_SHORT_TO_FLOAT(pf, sh) \
  {                                   \
    *(pf++) = SHORT2ANGLE(*(sh++));   \
    *(pf++) = SHORT2ANGLE(*(sh++));   \
    *(pf++) = SHORT2ANGLE(*(sh++));   \
...
#define COS_TABLE(i)                                              \
  tr.sinTable[(((i) >> FUNCTABLE_SHIFT) + (FUNCTABLE_SIZE / 4)) & \
              FUNCTABLE_MASK];
...
__inline void Matrix4Multiply(const vec4_t a[4],
                              const vec4_t b[4],
                              vec4_t dst[4]) {
  ...
  dst[0][2] = a[0][0] * b[0][2] + a[0][1] * b[1][2] + a[0][2] * b[2][2] +
              a[0][3] * b[3][2];
  dst[0][3] = a[0][0] * b[0][3] + a[0][1] * b[1][3] + a[0][2] * b[2][3] +
              a[0][3] * b[3][3];
  ...
  dst[1][2] = a[1][0] * b[0][2] + a[1][1] * b[1][2] + a[1][2] * b[2][2] +
              a[1][3] * b[3][2];
  dst[1][3] = a[1][0] * b[0][3] + a[1][1] * b[1][3] + a[1][2] * b[2][3] +
              a[1][3] * b[3][3];
  ...
  dst[2][2] = a[2][0] * b[0][2] + a[2][1] * b[1][2] + a[2][2] * b[2][2] +
              a[2][3] * b[3][2];
  dst[2][3] = a[2][0] * b[0][3] + a[2][1] * b[1][3] + a[2][2] * b[2][3] +
              a[2][3] * b[3][3];

  dst[3][0] = a[3][0] * b[0][0] + a[3][1] * b[1][0] + a[3][2] * b[2][0] +
              a[3][3] * b[3][0];
  dst[3][1] = a[3][0] * b[0][1] + a[3][1] * b[1][1] + a[3][2] * b[2][1] +
              a[3][3] * b[3][1];
  dst[3][2] = a[3][0] * b[0][2] + a[3][1] * b[1][2] + a[3][2] * b[2][2] +
              a[3][3] * b[3][2];
  dst[3][3] = a[3][0] * b[0][3] + a[3][1] * b[1][3] + a[3][2] * b[2][3] +
              a[3][3] * b[3][3];
}
...
__inline void Matrix4MultiplyInto3x3AndTranslation(/*const*/ vec4_t a[4],
  ...
  dst[0][2] = a[0][0] * b[0][2] + a[0][1] * b[1][2] + a[0][2] * b[2][2] +
              a[0][3] * b[3][2];
  t[0] = a[0][0] * b[0][3] + a[0][1] * b[1][3] + a[0][2] * b[2][3] +
         a[0][3] * b[3][3];
  ...
  dst[1][2] = a[1][0] * b[0][2] + a[1][1] * b[1][2] + a[1][2] * b[2][2] +
              a[1][3] * b[3][2];
  t[1] = a[1][0] * b[0][3] + a[1][1] * b[1][3] + a[1][2] * b[2][3] +
         a[1][3] * b[3][3];
  ...
  dst[2][2] = a[2][0] * b[0][2] + a[2][1] * b[1][2] + a[2][2] * b[2][2] +
              a[2][3] * b[3][2];
  t[2] = a[2][0] * b[0][3] + a[2][1] * b[1][3] + a[2][2] * b[2][3] +
         a[2][3] * b[3][3];
}
...
__inline void Matrix4FromScaledAxis(const vec3_t axis[3],
                                    const float scale,
                                    vec4_t dst[4]) {
  int i, j;
...
// matrix TTimo: const usage would require an explicit cast, non ANSI C see
// unix/const-arg.c
...
__inline void Matrix4TransformVector(const vec4_t m[4],
                                     const vec3_t src,
                                     vec3_t dst) {
...
==============
*/
static void R_CalcBone(const int torsoParent,
  ...
  // we can assume the parent has already been uncompressed for this frame +
  // lerp
  ...
  if (parentBone) {
    if (fullTorso) {
      ...
      *(pf++) = 0;
      ...
      VectorMA(parentBone->translation, thisBoneInfo->parentDist, vec,
               bonePtr->translation);
    } else {
#ifndef YD_INGLES
...
==============
*/
static void R_CalcBoneLerp(const int torsoParent,
                           const refEntity_t* refent,
                           int boneNum) {
  int j;
...
// rotation (take into account 170 to -170 lerps, which need to take the
// shortest route)
#ifndef YD_INGLES
  ...
  if (parentBone) {
    ...
    *(pf++) = 0;
    ...
    // translation
    if (!fullTorso &&
        isTorso) {  // partial legs/torso, need to lerp according to torsoWeight
    ...
    VectorMA(parentBone->translation, thisBoneInfo->parentDist, dir,
             bonePtr->translation);

  } else {  // just interpolate the frame positions

    bonePtr->translation[0] = frontlerp * frame->parentOffset[0] +
                              backlerp * oldFrame->parentOffset[0];
    bonePtr->translation[1] = frontlerp * frame->parentOffset[1] +
                              backlerp * oldFrame->parentOffset[1];
    bonePtr->translation[2] = frontlerp * frame->parentOffset[2] +
                              backlerp * oldFrame->parentOffset[2];
  }
  //
  if (boneNum == torsoParent) {  // this is the torsoParent
    VectorCopy(bonePtr->translation, torsoParentOffset);
...
R_BonesStillValid
        ...
        FIXME: optimization opportunity here, profile which values change most
often and check for those first to get early outs

        Other way we could do this is doing a random memory probe, which in
worst case scenario ends up being the memcmp? - BAD as only a few values are
used
...
static qboolean R_BonesStillValid(const refEntity_t* refent) {
  if (lastBoneEntity.hModel != refent->hModel) {
    ...
    return qfalse;
  } else if (!VectorCompare(lastBoneEntity.torsoAxis[0],
                            refent->torsoAxis[0]) ||
             !VectorCompare(lastBoneEntity.torsoAxis[1],
                            refent->torsoAxis[1]) ||
             !VectorCompare(lastBoneEntity.torsoAxis[2],
                            refent->torsoAxis[2])) {
    return qfalse;
  }
...
==============
*/
static void R_CalcBones(const refEntity_t* refent,
                        int* boneList,
                        int numBones) {
  int i;
  int* boneRefs;
  float torsoWeight;
  mdxHeader_t* mdxFrameHeader =
      R_GetModelByHandle(refent->frameModel)->model.mdx;
  mdxHeader_t* mdxOldFrameHeader =
      R_GetModelByHandle(refent->oldframeModel)->model.mdx;
  mdxHeader_t* mdxTorsoFrameHeader =
      R_GetModelByHandle(refent->torsoFrameModel)->model.mdx;
  mdxHeader_t* mdxOldTorsoFrameHeader =
      R_GetModelByHandle(refent->oldTorsoFrameModel)->model.mdx;

  if (!mdxFrameHeader || !mdxOldFrameHeader || !mdxTorsoFrameHeader ||
      !mdxOldTorsoFrameHeader) {
    return;
  }

  //
  // if the entity has changed since the last time the bones were built, reset
  // them
  //
  if (!R_BonesStillValid(refent)) {
    ...
    if (r_bonesDebug->integer == 4 && totalrt) {
      ri.Printf(PRINT_ALL, "Lod %.2f  verts %4d/%4d  tris %4d/%4d  (%.2f%%)\n",
                lodScale, totalrv, totalv, totalrt, totalt,
  ...
  if (refent->oldframe == refent->frame &&
      refent->oldframeModel == refent->frameModel) {
    backlerp = 0;
    frontlerp = 1;
  } else {
  ...
  frame =
      (mdxFrame_t*)((byte*)mdxFrameHeader + mdxFrameHeader->ofsFrames +
                    refent->frame * (int)(sizeof(mdxBoneFrameCompressed_t)) *
                        mdxFrameHeader->numBones +
                    refent->frame * sizeof(mdxFrame_t));
  torsoFrame = (mdxFrame_t*)((byte*)mdxTorsoFrameHeader +
                             mdxTorsoFrameHeader->ofsFrames +
                             refent->torsoFrame *
                                 (int)(sizeof(mdxBoneFrameCompressed_t)) *
                                 mdxTorsoFrameHeader->numBones +
                             refent->torsoFrame * sizeof(mdxFrame_t));
  oldFrame =
      (mdxFrame_t*)((byte*)mdxOldFrameHeader + mdxOldFrameHeader->ofsFrames +
                    refent->oldframe * (int)(sizeof(mdxBoneFrameCompressed_t)) *
                        mdxOldFrameHeader->numBones +
                    refent->oldframe * sizeof(mdxFrame_t));
  oldTorsoFrame = (mdxFrame_t*)((byte*)mdxOldTorsoFrameHeader +
                                mdxOldTorsoFrameHeader->ofsFrames +
                                refent->oldTorsoFrame *
                                    (int)(sizeof(mdxBoneFrameCompressed_t)) *
                                    mdxOldTorsoFrameHeader->numBones +
                                refent->oldTorsoFrame * sizeof(mdxFrame_t));

  //
  // lerp all the needed bones (torsoParent is always the first bone in the
  // list)
  //
  frameSize = (int)sizeof(mdxBoneFrameCompressed_t) * mdxFrameHeader->numBones;

  cBoneList =
      (mdxBoneFrameCompressed_t*)((byte*)mdxFrameHeader +
                                  mdxFrameHeader->ofsFrames +
                                  (refent->frame + 1) * sizeof(mdxFrame_t) +
                                  refent->frame * frameSize);
  cBoneListTorso = (mdxBoneFrameCompressed_t*)((byte*)mdxTorsoFrameHeader +
                                               mdxTorsoFrameHeader->ofsFrames +
                                               (refent->torsoFrame + 1) *
                                                   sizeof(mdxFrame_t) +
  ...
  if (!backlerp && !torsoBacklerp) {
    for (i = 0; i < numBones; i++, boneRefs++) {
      ...
      if ((boneInfo[*boneRefs].parent >= 0) &&
          (!validBones[boneInfo[*boneRefs].parent] &&
           !newBones[boneInfo[*boneRefs].parent])) {
        R_CalcBone(mdxFrameHeader->torsoParent, refent,
                   boneInfo[*boneRefs].parent);
      }
    ...
    cOldBoneList = (mdxBoneFrameCompressed_t*)((byte*)mdxOldFrameHeader +
                                               mdxOldFrameHeader->ofsFrames +
                                               (refent->oldframe + 1) *
                                                   sizeof(mdxFrame_t) +
                                               refent->oldframe * frameSize);
    cOldBoneListTorso =
        (mdxBoneFrameCompressed_t*)((byte*)mdxOldTorsoFrameHeader +
                                    mdxOldTorsoFrameHeader->ofsFrames +
                                    (refent->oldTorsoFrame + 1) *
                                        sizeof(mdxFrame_t) +
    ...
    for (i = 0; i < numBones; i++, boneRefs++) {
      ...
      if ((boneInfo[*boneRefs].parent >= 0) &&
          (!validBones[boneInfo[*boneRefs].parent] &&
           !newBones[boneInfo[*boneRefs].parent])) {
        R_CalcBoneLerp(mdxFrameHeader->torsoParent, refent,
                       boneInfo[*boneRefs].parent);
      }
  ...
  for (i = 0; i < numBones; i++, boneRefs++) {
    ...
    if (thisBoneInfo->torsoWeight > 0) {
      ...
      // 2nd translation for rotation relative to bone around torso parent
      // offset
      ...
      if (torsoWeight != thisBoneInfo->torsoWeight) {
        Matrix4FromScaledAxisPlusTranslation(
            torsoAxis, thisBoneInfo->torsoWeight, torsoParentOffset, m2);
      ...
      /*} else {	// tag's require special handling
              ...
              LocalScaledMatrixTransformVector( bonePtr->matrix[0],
      thisBoneInfo->torsoWeight, torsoAxis, tmpAxis[0] );
              LocalScaledMatrixTransformVector( bonePtr->matrix[1],
      thisBoneInfo->torsoWeight, torsoAxis, tmpAxis[1] );
              LocalScaledMatrixTransformVector( bonePtr->matrix[2],
      thisBoneInfo->torsoWeight, torsoAxis, tmpAxis[2] ); memcpy(
      bonePtr->matrix, tmpAxis, sizeof(tmpAxis) );

              // rotate the translation around the torsoParent
              VectorSubtract( bonePtr->translation, torsoParentOffset, t );
              LocalScaledMatrixTransformVector( t, thisBoneInfo->torsoWeight,
      torsoAxis, bonePtr->translation ); VectorAdd( bonePtr->translation,
      torsoParentOffset, bonePtr->translation );
...
void RB_MDM_SurfaceAnim(mdmSurface_t* surface) {
  ...
  lodScale =
      R_CalcMDMLod(refent, vec, lodRadius, header->lodBias, header->lodScale);
  ...
  //----(SA)	modification to allow dead skeletal bodies to go below minlod
  //(experiment)
  if (refent->reFlags & REFLAG_DEAD_LOD) {
    if (lodScale <
        0.35) {  // allow dead to lod down to 35% (even if below surf->minLod)
                 // (%35 is arbitrary and probably not good generally.  worked
                 // for the blackguard/infantry as a test though)
      lodScale = 0.35;
    }
  ...
  for (j = 0; j < render_count; j++, tempVert += 4, tempNormal += 4) {
    ...
    for (k = 0; k < v->numWeights; k++, w++) {
      ...
      LocalAddScaledMatrixTransformVectorTranslate(
          w->offset, w->boneWeight, bone->matrix, bone->translation, tempVert);
    }
  ...
  if (r_bonesDebug->integer) {
    ...
    if (r_bonesDebug->integer < 3 || r_bonesDebug->integer == 5 ||
        r_bonesDebug->integer == 8 || r_bonesDebug->integer == 9) {
      ...
      for (i = 0; i < surface->numBoneReferences; i++, boneRefs++) {
        ...
        if (r_bonesDebug->integer != 9) {
          ...
          for (j = 0; j < 3; j++) {
            ...
            VectorMA(bonePtr->translation,
                     (r_bonesDebug->integer == 8 ? 1.5 : 5), bonePtr->matrix[j],
                     vec);
            qglVertex3fv(vec);
          }
          qglEnd();
      ...
      if (r_bonesDebug->integer == 8) {
        ...
        for (i = 0; i < surface->numBoneReferences; i++, boneRefs++) {
          ...
          mdxBoneInfo_t* mdxBoneInfo =
              (mdxBoneInfo_t*)((byte*)mdxHeader + mdxHeader->ofsBones +
                               *boneRefs * sizeof(mdxBoneInfo_t));
          bonePtr = &bones[*boneRefs];
          ...
          R_DebugText(
              vec, 1.f, 1.f, 1.f, mdxBoneInfo->name,
              qfalse);  // qfalse, as there is no reason to set depthrange again
        }
        ...
        if (surface == (mdmSurface_t*)((byte*)header + header->ofsSurfaces)) {
          ...
          for (i = 0; i < header->numTags; i++) {
            ...
            LocalAddScaledMatrixTransformVectorTranslate(
                pTag->offset, 1.f, tagBone->matrix, tagBone->translation,
                outTag.origin);
            for (j = 0; j < 3; j++) {
              LocalMatrixTransformVector(pTag->axis[j], bone->matrix,
                                         outTag.axis[j]);
            }
            ...
            R_DebugText(vec, 1.f, 1.f, 1.f, pTag->name,
                        qfalse);  // qfalse, as there is no reason to set
                                  // depthrange again
    ...
    if (r_bonesDebug->integer >= 3 && r_bonesDebug->integer <= 6) {
      ...
      if (r_bonesDebug->integer == 3) {
        ri.Printf(
            PRINT_ALL, "Lod %.2f  verts %4d/%4d  tris %4d/%4d  (%.2f%%)\n",
            lodScale, render_count, surface->numVerts, render_indexes / 3,
  ...
  /*	if( r_showmodelbounds->integer ) {
                  ...
                  mdxHeader_t	*mdxHeader = R_GetModelByHandle(
     refent->frameModel )->model.mdx; mdxFrame_t	*mdxFrame = (mdxFrame_t
     *)((byte *)mdxHeader + mdxHeader->ofsFrames + refent->frame * (int) (
     sizeof( mdxBoneFrameCompressed_t ) ) * mdxHeader->numBones + refent->frame
     * sizeof(mdxFrame_t));
                  ...
                  VectorSubtract( mdxFrame->bounds[0], mdxFrame->bounds[1],
     diff);
...
===============
*/
int R_MDM_GetBoneTag(orientation_t* outTag,
                     mdmHeader_t* mdm,
                     int startTagIndex,
                     const refEntity_t* refent,
                     const char* tagName) {
  ...
  VectorClear(outTag->origin);
  LocalAddScaledMatrixTransformVectorTranslate(
      pTag->offset, 1.f, bone->matrix, bone->translation, outTag->origin);
  for (j = 0; j < 3; j++) {
