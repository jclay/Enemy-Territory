...
static int c_begins;
static void R_DrawStripElements(int numIndexes,
...
static void R_BindAnimatedImage(textureBundle_t* bundle) {
  ...
  index =
      myftol(tess.shaderTime * bundle->imageAnimationSpeed * FUNCTABLE_SIZE);
...
static void DrawTris(shaderCommands_t* input) {
  ...
  if (*s == '0' && (*(s + 1) == 'x' || *(s + 1) == 'X')) {
    ...
    if (Q_IsHexColorString(s)) {
      ...
      trisColor[1] =
          ((float)(gethex(*(s + 2)) * 16 + gethex(*(s + 3)))) / 255.00;
      trisColor[2] =
          ((float)(gethex(*(s + 4)) * 16 + gethex(*(s + 5)))) / 255.00;

      if (Q_HexColorStringHasAlpha(s)) {
        trisColor[3] =
            ((float)(gethex(*(s + 6)) * 16 + gethex(*(s + 7)))) / 255.00;
      }
    }
  } else {
    int i;
...
static void DrawNormals(shaderCommands_t* input) {
  ...
  if (r_shownormals->integer == 2) {
    ...
    qglColor3f(ent->ambientLight[0] / 255, ent->ambientLight[1] / 255,
               ent->ambientLight[2] / 255);
    qglPointSize(5);
    qglBegin(GL_POINTS);
    qglVertex3fv(temp);
    qglEnd();
    ...
    if (fabs(VectorLengthSquared(ent->lightDir) - 1.0f) > 0.2f) {
      ...
      qglColor3f(ent->directedLight[0] / 255, ent->directedLight[1] / 255,
                 ent->directedLight[2] / 255);
    }
    qglLineWidth(3);
    qglBegin(GL_LINES);
    qglVertex3fv(temp);
  ...
  // ydnar: normals drawing
    ...
    for (i = 0; i < input->numVertexes; i++) {
      ...
      VectorMA(input->xyz[i].v, r_normallength->value, input->normal[i].v,
               temp);
      qglVertex3fv(temp);
    }
    qglEnd();
...
static void RB_FogPass(void) {
  ...
  if (tr.refdef.rdflags & RDF_SNOOPERVIEW || tess.shader->noFog ||
      !r_wolffog->integer) {
    return;
  }
...
static void ComputeColors(shaderStage_t* pStage) {
  ...
  switch (pStage->rgbGen) {
    ...
    case CGEN_EXACT_VERTEX:
      memcpy(tess.svars.colors, tess.vertexColors,
             tess.numVertexes * sizeof(tess.vertexColors[0].v));
      break;
    case CGEN_CONST:
      for (i = 0; i < tess.numVertexes; i++) {
      ...
      if (tr.identityLight == 1) {
        ...
        for (i = 0; i < tess.numVertexes; i++) {
          tess.svars.colors[i][0] =
              tess.vertexColors[i].v[0] * tr.identityLight;
          tess.svars.colors[i][1] =
              tess.vertexColors[i].v[1] * tr.identityLight;
          tess.svars.colors[i][2] =
              tess.vertexColors[i].v[2] * tr.identityLight;
    ...
    case CGEN_ONE_MINUS_VERTEX:
      if (tr.identityLight == 1) {
        ...
        for (i = 0; i < tess.numVertexes; i++) {
          tess.svars.colors[i][0] =
              (255 - tess.vertexColors[i].v[0]) * tr.identityLight;
          tess.svars.colors[i][1] =
              (255 - tess.vertexColors[i].v[1]) * tr.identityLight;
          tess.svars.colors[i][2] =
              (255 - tess.vertexColors[i].v[2]) * tr.identityLight;
        }
      }
      break;
    case CGEN_FOG: {
  ...
  switch (pStage->alphaGen) {
    ...
    case AGEN_NORMALZFADE: {
      ...
      if (backEnd.currentEntity->e
              .hModel) {  // world surfaces dont have an axis
        VectorRotate(backEnd.currentEntity->e.fireRiseDir,
                     backEnd.currentEntity->e.axis, worldUp);
      } else {
      ...
      for (i = 0; i < tess.numVertexes; i++) {
        ...
        if (zombieEffect) {
          alpha =
              (float)backEnd.currentEntity->e.shaderRGBA[3] * (dot + 1.0) / 2.0;
          alpha += (2.0 * (float)backEnd.currentEntity->e.shaderRGBA[3]) *
                   (1.0 - (dot + 1.0) / 2.0);
          if (alpha > 255.0) {
            alpha = 255.0;
          } else if (alpha < 0.0) {
        ...
        if (dot < highest) {
          if (dot > lowest) {
            if (dot < lowest + range / 2) {
              alpha = ((float)pStage->constantColor[3] *
                       ((dot - lowest) / (range / 2)));
            } else {
              alpha = ((float)pStage->constantColor[3] *
                       (1.0 - ((dot - lowest - range / 2) / (range / 2))));
            }
            if (alpha > 255.0) {
              alpha = 255.0;
            } else if (alpha < 0.0) {
...
static void ComputeTexCoords(shaderStage_t* pStage) {
  ...
  for (b = 0; b < NUM_TEXTURE_BUNDLES; b++) {
    ...
    switch (pStage->bundle[b].tcGen) {
      case TCGEN_IDENTITY:
        memset(tess.svars.texcoords[b], 0,
      ...
      case TCGEN_VECTOR:
        for (i = 0; i < tess.numVertexes; i++) {
          tess.svars.texcoords[b][i][0] =
              DotProduct(tess.xyz[i].v, pStage->bundle[b].tcGenVectors[0]);
          tess.svars.texcoords[b][i][1] =
              DotProduct(tess.xyz[i].v, pStage->bundle[b].tcGenVectors[1]);
        }
        break;
      case TCGEN_FOG:
    ...
    if (pStage->bundle[1].image[0] != 0) {
      ...
      if (fadeStart) {
        ...
        if (fadeStart > tr.refdef.time) {  // has not started to fade yet
          ...
          alphaval =
              (float)(fadeEnd - tr.refdef.time) / (float)(fadeEnd - fadeStart);
...
void RB_EndSurface(void) {
  ...
  if (input->indexes[input->maxShaderIndicies - 1] != 0) {
    ri.Error(ERR_DROP, "RB_EndSurface() - input->maxShaderIndicies(%i) hit",
             input->maxShaderIndicies);
  }
  if (input->xyz[input->maxShaderVerts - 1].v[0] != 0) {
    ri.Error(ERR_DROP, "RB_EndSurface() - input->maxShaderVerts(%i) hit",
             input->maxShaderVerts);
  }
