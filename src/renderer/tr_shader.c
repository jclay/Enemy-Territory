...
void R_RemapShader(const char* shaderName,
...
static qboolean ParseStage(shaderStage_t* stage, char** text) {
  ...
  while (1) {
    ...
    //
    // check special case for map16/map32/mapcomp/mapnocomp (compression
    // enabled)
    if (!Q_stricmp(token,
    ...
    if (!Q_stricmp(token, "map")) {
      ...
      //----(SA)	fixes startup error and allows polygon shadows to work
      //again
      ...
      //----(SA) end
        ...
        stage->bundle[0].image[0] = R_FindImageFile(
            token, !shader.noMipMaps, !shader.noPicMip, GL_REPEAT, qfalse);
    ...
    //
      ...
      stage->bundle[0].image[0] = R_FindImageFile(
          token, !shader.noMipMaps, !shader.noPicMip, GL_CLAMP, qfalse);
      if (!stage->bundle[0].image[0]) {
        ri.Printf(
            PRINT_WARNING,
            "WARNING: R_FindImageFile could not find '%s' in shader '%s'\n",
    ...
    //
      ...
      if (!token[0]) {
        ri.Printf(PRINT_WARNING,
                  "WARNING: missing parameter for 'lightmap' keyword in shader "
                  "'%s'\n",
                  shader.name);
        return qfalse;
      }
      ...
      //----(SA) end
        ...
        stage->bundle[0].image[0] =
            R_FindImageFile(token, qfalse, qfalse, GL_CLAMP, qtrue);
        if (!stage->bundle[0].image[0]) {
          ri.Printf(
              PRINT_WARNING,
              "WARNING: R_FindImageFile could not find '%s' in shader '%s'\n",
  ...
  if (strcmp(token, "-")) {
    for (i = 0; i < 6; i++) {
      ...
      shader.sky.outerbox[i] =
          R_FindImageFile((char*)pathname, qtrue, qtrue, GL_CLAMP, qfalse);
  ...
  if (strcmp(token, "-")) {
    for (i = 0; i < 6; i++) {
      ...
      shader.sky.innerbox[i] =
          R_FindImageFile((char*)pathname, qtrue, qtrue, GL_REPEAT, qfalse);
...
infoParm_t infoParms[] = {
    ...
    //----(SA)	modified
    {"clipmissile", 1, 0,
     CONTENTS_MISSILECLIP},  // impact only specific weapons (rl, gl)
                             //----(SA)	end
    ...
    {"metalsteps", 0, SURF_METAL,
     0},  // retain bw compatibility with Q3A metal shaders... (SA)
...
static qboolean ParseShader(char** text) {
  ...
  while (1) {
    ...
    // fogParms
      ...
      shader.fogParms.depthForOpaque = shader.fogParms.depthForOpaque < 1
                                           ? 1
    ...
    // skyfogvars <(r,g,b)> <dist>
      ...
      if (atof(token) > 1) {
        ri.Printf(PRINT_WARNING,
                  "WARNING: last value for skyfogvars is 'density' which needs "
                  "to be 0.0-1.0\n");
        continue;
      }
    ...
    //----(SA)	added
                 ...
                 token,
                 "lightgridmulamb")) {  // ambient multiplier for lightgrid
      token = COM_ParseExt(text, qfalse);
      if (!token[0]) {
        ri.Printf(
            PRINT_WARNING,
            "WARNING: missing value for 'lightgrid ambient multiplier'\n");
        continue;
      }
      if (atof(token) > 0) {
        tr.lightGridMulAmbient = atof(token);
                          ...
                          "lightgridmuldir")) {  // directional multiplier for
                                                 // lightgrid
      token = COM_ParseExt(text, qfalse);
      if (!token[0]) {
        ri.Printf(
            PRINT_WARNING,
            "WARNING: missing value for 'lightgrid directional multiplier'\n");
        continue;
      }
      if (atof(token) > 0) {
    ...
    //----(SA)	end
      ...
      //----(SA)	right now allow one water color per map.  I'm sure this
      //will need 			to change at some point, but I'm not sure how to track fog
      //parameters 			on a "per-water volume" basis yet.

      if (fogvar == 0) {  // '0' specifies "use the map values for everything
                          // except the fog color
                          // TODO
      } else if (fogvar > 1) {  // distance "linear" fog
        R_SetFog(FOG_WATER, 0, fogvar, watercolor[0], watercolor[1],
                 watercolor[2], 1.1);
    ...
    // fogvars
      ...
      //----(SA)	NOTE:	fogFar > 1 means the shader is setting the
      //farclip, < 1 means setting 					density (so old maps or maps that just need
      //softening fog don't have to care about farclip)
    ...
    // ydnar: distancecull <opaque distance> <transparent distance> <alpha
    // threshold>
      ...
      if (shader.distanceCull[1] - shader.distanceCull[0] > 0) {
        ...
        shader.distanceCull[3] =
            1.0 / (shader.distanceCull[1] - shader.distanceCull[0]);
      } else {
          ...
          for (i = 0, drawSurf = ds_cmd->drawSurfs; i < ds_cmd->numDrawSurfs;
            ...
            if (sortedIndex >= newShader) {
              ...
              drawSurf->sort = (sortedIndex << QSORT_SHADERNUM_SHIFT) |
                               ...
                               (frontFace << QSORT_FRONTFACE_SHIFT) | dlightMap;
            }
...
static shader_t* GeneratePermanentShader(void) {
  ...
  newShader = R_CacheShaderAlloc(
      shader.lightmapIndex < 0
          ? va("%s lm: %i", shader.name, shader.lightmapIndex)
          : NULL,
      sizeof(shader_t));
  ...
  *newShader = shader;
  ...
  if (shader.sort <=
      SS_SEE_THROUGH) {  // ydnar: was SS_DECAL, this allows grates to be fogged
    ...
    *newShader->stages[i] = stages[i];
    ...
    for (b = 0; b < NUM_TEXTURE_BUNDLES; b++) {
      if (!newShader->stages[i]->bundle[b].numTexMods) {
        // make sure unalloc'd texMods aren't pointing to some random point in
        // memory
...
static void SetImplicitShaderStages(image_t* image) {
  ...
  switch (shader.lightmapIndex) {
    ...
    default:
      ...
      if (implicitStateBits &
          (GLS_ATEST_BITS | GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) {
        stages[0].bundle[0].image[0] = image;
        stages[0].active = qtrue;
        stages[0].rgbGen = CGEN_IDENTITY;
        stages[0].stateBits = implicitStateBits;
      ...
      // otherwise do standard lightmap + texture
        ...
        stages[1].rgbGen = CGEN_IDENTITY;
        stages[1].stateBits =
            GLS_DEFAULT | GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ZERO;
      }
      break;
  }
    ...
    if (pStage->isDetail && !r_detailTextures->integer) {
      if (stage < (MAX_SHADER_STAGES - 1)) {
        memmove(pStage, pStage + 1,
                sizeof(*pStage) * (MAX_SHADER_STAGES - stage - 1));
...
qboolean RE_LoadDynamicShader(const char* shadername, const char* shadertext) {
  ...
  //	ri.Printf( PRINT_ALL, "Loaded dynamic shader [%s] with shadertext
  ...
  if (dshader) {
    ...
    while (dptr) {
      if (!dptr->shadertext || !strlen(dptr->shadertext)) {
        ri.Printf(PRINT_WARNING,
                  "WARNING: dynamic shader %s(%d) has no shadertext\n",
                  shadername, i);
      } else {
        q = dptr->shadertext;
        ...
        if ((token[0] != 0) && !Q_stricmp(token, shadername)) {
          ...
          //dynamic shader [%s] with shadertext [%s]\n", shadername,
          //dptr->shadertext );
          return q;
        }
      }
...
// Ridah, modified this so we don't keep trying to load an invalid lightmap
// shader
#if 0
  ...
  image = R_FindImageFile(fileName, !shader.noMipMaps, !shader.noPicMip,
                          mipRawImage ? GL_REPEAT : GL_CLAMP, qfalse);
  if (!image) {
...
qhandle_t RE_RegisterShaderFromImage(const char* name,
                                     int lightmapIndex,
                                     image_t* image,
                                     qboolean mipRawImage) {
  int i, hash;
...
static void BuildShaderChecksumLookup(void) {
  ...
  while (1) {
    ...
    // Gordon: NOTE this is WRONG, need to either unget the {, or as i'm gonna
    // do, assume the shader section follows, if it doesnt, it's b0rked anyway
    /*		if (!Q_stricmp( token, "{" )) {
    ...
    if (!shaderChecksumLookup[checksum].pStr) {
      ...
      newStrPtr =
          &shaderStringPointerList
              [numShaderStringPointers++];  // ri.Hunk_Alloc( sizeof(
                                            // shaderStringPointer_t ), h_low );
      newStrPtr->pStr = pOld;
...
static void ScanAndLoadShaderFiles(void) {
  ...
  // Gordon: optimised to not use strcat/strlen which can be VERY slow for the
  // large strings we're using here
  p = s_shaderText;
...
void* R_CacheShaderAllocExt(const char* name,
                            int size,
                            const char* file,
                            int line) {
  if (r_cache->integer && r_cacheShaders->integer) {
    ...
    //		if( name ) {
    //			Com_Printf( "Zone Malloc from %s: size %i: pointer %p:
...
void R_CacheShaderFreeExt(const char* name,
                          void* ptr,
                          const char* file,
                          int line) {
  if (r_cache->integer && r_cacheShaders->integer) {
    ...
    //		if( name ) {
    //			Com_Printf( "Zone Free from %s: pointer %p: %i in
                  ...
                  for ( j = 0 ; j < (*sh)->numUnfoggedPasses ; j++ ) {
                          ...
                          for ( b = 0 ; b < NUM_TEXTURE_BUNDLES ; b++ ) {
                                  if ((*sh)->stages[j]->bundle[b].texMods)
                                          R_CacheShaderFree( NULL,
  (*sh)->stages[j]->bundle[b].texMods );
                          }
                          R_CacheShaderFree( NULL, (*sh)->stages[j] );
                  }
                  R_CacheShaderFree( (*sh)->lightmapIndex ? va( "%s lm: %i",
  (*sh)->name, (*sh)->lightmapIndex) : NULL, *sh ); *sh = NULL;
  ...
  for (i = 0; i < sh->numUnfoggedPasses; i++) {
    if (sh->stages[i] && sh->stages[i]->active) {
      for (b = 0; b < NUM_TEXTURE_BUNDLES; b++) {
        ...
        // for (j=0; sh->stages[i]->bundle[b].image[j] && j <
        // MAX_IMAGE_ANIMATIONS; j++) {
        for (j = 0;
             j < MAX_IMAGE_ANIMATIONS && sh->stages[i]->bundle[b].image[j];
             j++) {
  ...
  for (i = 0; i < sizeof(backupHashTable) / sizeof(backupHashTable[0]); i++) {
    ...
    while (sh) {
      if (sh->lightmapIndex >= 0 || !R_ShaderCanBeCached(sh)) {
        ...
        R_CacheShaderFree(sh->lightmapIndex < 0
                              ? va("%s lm: %i", sh->name, sh->lightmapIndex)
                              : NULL,
                          sh);
...
void R_BackupShaders(void) {
  ...
  //]->name, backupShaders[i]->lightmapIndex );
  //		}
  //	}
...
static qboolean R_RegisterShaderImages(shader_t* sh) {
  ...
  for (i = 0; i < sh->numUnfoggedPasses; i++) {
    if (sh->stages[i] && sh->stages[i]->active) {
      for (b = 0; b < NUM_TEXTURE_BUNDLES; b++) {
        for (j = 0;
             sh->stages[i]->bundle[b].image[j] && j < MAX_IMAGE_ANIMATIONS;
             j++) {
...
shader_t* R_FindCachedShader(const char* name, int lightmapIndex, int hash) {
  ...
  while (sh) {
    if (sh->lightmapIndex == lightmapIndex && !Q_stricmp(sh->name, name)) {
      if (tr.numShaders == MAX_SHADERS) {
        ri.Printf(PRINT_WARNING,
                  "WARNING: R_FindCachedShader - MAX_SHADERS hit\n");
        return NULL;
      }
      ...
      //			Com_Printf( "Removing %s from the cache: lm:
