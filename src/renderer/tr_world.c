...
================
*/
static qboolean R_CullSurface(surfaceType_t* surface,
                              shader_t* shader,
                              int* frontFace) {
  srfGeneric_t* gen;
  int cull;
  ...
  *frontFace = 0;
  ...
  if (gen->plane.type != PLANE_NON_PLANAR && r_facePlaneCull->integer) {
    d = DotProduct(tr.orientation.viewOrigin, gen->plane.normal) -
        gen->plane.dist;
    if (d > 0.0f) {
      *frontFace = 1;
    }
...
static int R_DlightSurface(msurface_t* surface, int dlightBits) {
  ...
  // ydnar: made surface dlighting generic, inline with q3map2 surface
  // classification
...
static void R_AddWorldSurface(msurface_t* surf,
  ...
  if (decalBits) {
    ...
    for (i = 0; i < tr.refdef.numDecalProjectors; i++) {
      if (decalBits & (1 << i)) {
        R_ProjectDecalOntoSurface(&tr.refdef.decalProjectors[i], surf,
                                  tr.currentBModel);
      }
    }
  }
...
void R_AddBrushModelSurfaces(trRefEntity_t* ent) {
  ...
  for (i = 0; i < tr.refdef.numDecalProjectors; i++) {
    ...
    // transform entity bbox (fixme: rotated entities have invalid bounding
    // boxes)
    ...
    if (R_TestDecalBoundingBox(&tr.refdef.decalProjectors[i], mins, maxs)) {
      R_TransformDecalProjector(&tr.refdef.decalProjectors[i],
                                tr.orientation.axis, tr.orientation.origin,
    ...
    if (dlightBits) {  //%	&& node->contents != -1 )
      for (i = 0; i < tr.refdef.num_dlights; i++) {
        if (dlightBits & (1 << i)) {
          ...
          dl = &tr.refdef.dlights[i];
          if (node->surfMins[0] >= (dl->origin[0] + dl->radius) ||
              node->surfMaxs[0] <= (dl->origin[0] - dl->radius) ||
              node->surfMins[1] >= (dl->origin[1] + dl->radius) ||
              node->surfMaxs[1] <= (dl->origin[1] - dl->radius) ||
              node->surfMins[2] >= (dl->origin[2] + dl->radius) ||
              node->surfMaxs[2] <= (dl->origin[2] - dl->radius)) {
            dlightBits &= ~(1 << i);
          }
        }
      }
    ...
    if (decalBits) {
      for (i = 0; i < tr.refdef.numDecalProjectors; i++) {
        if (decalBits & (1 << i)) {
          ...
          if (tr.refdef.decalProjectors[i].shader == NULL ||
              !R_TestDecalBoundingBox(&tr.refdef.decalProjectors[i],
                                      node->surfMins, node->surfMaxs)) {
            decalBits &= ~(1 << i);
          }
        }
      }
...
void R_AddWorldSurfaces(void) {
  ...
  if (tr.refdef.rdflags & RDF_SKYBOXPORTAL && tr.world->numSkyNodes > 0) {
    ...
    for (i = 0, node = tr.world->skyNodes; i < tr.world->numSkyNodes;
         i++, node++)
      R_AddLeafSurfaces(*node, tr.refdef.dlightBits,
                        0);  // no decals on skybox nodes
  } else {
