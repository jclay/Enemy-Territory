...
static mdsBoneFrame_t bones[MDS_MAX_BONES], rawBones[MDS_MAX_BONES],
    oldBones[MDS_MAX_BONES];
static char validBones[MDS_MAX_BONES];
static char newBones[MDS_MAX_BONES];
static mdsBoneFrame_t *bonePtr, *bone, *parentBone;
static mdsBoneFrameCompressed_t *cBonePtr, *cTBonePtr, *cOldBonePtr,
...
static int R_CullModel(mdsHeader_t* header, trRefEntity_t* ent) {
  ...
  frameSize = (int)(sizeof(mdsFrame_t) - sizeof(mdsBoneFrameCompressed_t) +
                    header->numBones * sizeof(mdsBoneFrameCompressed_t));

  // compute frame pointers
  newFrame = (mdsFrame_t*)((byte*)header + header->ofsFrames +
                           ent->e.frame * frameSize);
  oldFrame = (mdsFrame_t*)((byte*)header + header->ofsFrames +
                           ent->e.oldframe * frameSize);
...
=================
*/
float R_CalcMDSLod(refEntity_t* refent,
                   vec3_t origin,
                   float radius,
                   float modelBias,
                   float modelScale) {
...
static int R_ComputeFogNum(mdsHeader_t* header, trRefEntity_t* ent) {
  ...
  mdsFrame =
      (mdsFrame_t*)((byte*)header + header->ofsFrames +
                    (sizeof(mdsFrame_t) + sizeof(mdsBoneFrameCompressed_t) *
                                              (header->numBones - 1)) *
                        ent->e.frame);
...
static __inline void LocalMatrixTransformVector(vec3_t in,
                                                vec3_t mat[3],
                                                vec3_t out) {
...
static __inline void LocalMatrixTransformVectorTranslate(vec3_t in,
                                                         vec3_t mat[3],
                                                         vec3_t tr,
                                                         vec3_t out) {
...
static __inline void LocalScaledMatrixTransformVector(vec3_t in,
                                                      float s,
                                                      vec3_t mat[3],
                                                      vec3_t out) {
  out[0] = (1.0f - s) * in[0] +
...
static __inline void LocalScaledMatrixTransformVectorTranslate(vec3_t in,
                                                               float s,
                                                               vec3_t mat[3],
                                                               vec3_t tr,
                                                               vec3_t out) {
...
static __inline void LocalScaledMatrixTransformVectorFullTranslate(
    vec3_t in,
    float s,
    vec3_t mat[3],
    vec3_t tr,
...
static __inline void LocalAddScaledMatrixTransformVectorFullTranslate(
    vec3_t in,
    float s,
    vec3_t mat[3],
    vec3_t tr,
...
static __inline void LocalAddScaledMatrixTransformVectorTranslate(vec3_t in,
                                                                  float s,
                                                                  vec3_t mat[3],
                                                                  vec3_t tr,
                                                                  vec3_t out) {
...
static __inline void LocalAddScaledMatrixTransformVector(vec3_t in,
                                                         float s,
                                                         vec3_t mat[3],
                                                         vec3_t out) {
...
static __inline void LocalVectorMA(vec3_t org,
                                   float dist,
                                   vec3_t vec,
                                   vec3_t out) {
...
static __inline void SLerp_Normal(vec3_t from,
                                  vec3_t to,
                                  float tt,
                                  vec3_t out) {
  float ft = 1.0 - tt;
...
static __inline void Matrix4Multiply(const vec4_t a[4],
                                     const vec4_t b[4],
                                     vec4_t dst[4]) {
...
static __inline void Matrix4MultiplyInto3x3AndTranslation(/*const*/ vec4_t a[4],
                                                          /*const*/ vec4_t b[4],
                                                          vec3_t dst[3],
                                                          vec3_t t) {
  dst[0][0] = a[0][0] * b[0][0] + a[0][1] * b[1][0] + a[0][2] * b[2][0] +
...
static __inline void Matrix4Transpose(const vec4_t matrix[4],
                                      vec4_t transpose[4]) {
  int i, j;
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
...
static __inline void Matrix4FromScaledAxis(const vec3_t axis[3],
                                           const float scale,
                                           vec4_t dst[4]) {
  int i, j;
...
// TTimo: const usage would require an explicit cast, non ANSI C
// see unix/const-arg.c
static __inline void Matrix4FromAxisPlusTranslation(/*const*/ vec3_t axis[3],
                                                    const vec3_t t,
                                                    vec4_t dst[4]) {
  int i, j;
  for (i = 0; i < 3; i++) {
...
// unix/const-arg.c
static __inline void Matrix4FromScaledAxisPlusTranslation(
    /*const*/ vec3_t axis[3],
    const float scale,
    const vec3_t t,
    vec4_t dst[4]) {
...
static __inline void Matrix4TransformVector(const vec4_t m[4],
                                            const vec3_t src,
                                            vec3_t dst) {
...
static __inline void Matrix3Transpose(const vec3_t matrix[3],
                                      vec3_t transpose[3]) {
  int i, j;
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 3; j++) {
...
void R_CalcBone(mdsHeader_t* header, const refEntity_t* refent, int boneNum) {
  ...
  if (parentBone) {
    ...
    if (fullTorso) {
      ...
      LocalVectorMA(parentBone->translation, thisBoneInfo->parentDist, vec,
                    bonePtr->translation);
    } else {
...
==============
*/
void R_CalcBoneLerp(mdsHeader_t* header,
  ...
  if (parentBone) {
    ...
    *(pf++) = 0;
    ...
    LocalVectorMA(parentBone->translation, thisBoneInfo->parentDist, dir,
                  bonePtr->translation);
...
==============
*/
void R_CalcBones(mdsHeader_t* header,
                 const refEntity_t* refent,
                 int* boneList,
                 int numBones) {
  int i;
  ...
  frameSize = (int)(sizeof(mdsFrame_t) +
                    (header->numBones - 1) * sizeof(mdsBoneFrameCompressed_t));
    ...
    if (thisBoneInfo->torsoWeight > 0) {
      ...
      if (!(thisBoneInfo->flags & BONEFLAG_TAG)) {
        ...
        LocalScaledMatrixTransformVector(bonePtr->matrix[0],
                                         thisBoneInfo->torsoWeight, torsoAxis,
                                         tmpAxis[0]);
        ...
        LocalScaledMatrixTransformVector(t, thisBoneInfo->torsoWeight,
                                         torsoAxis, bonePtr->translation);
        VectorAdd(bonePtr->translation, torsoParentOffset,
                  bonePtr->translation);
      }
    }
  }
...
void RB_SurfaceAnim(mdsSurface_t* surface) {
  ...
  R_CalcBones(header, (const refEntity_t*)refent, boneList,
              surface->numBoneReferences);
  ...
  lodScale =
      R_CalcMDSLod(refent, vec, lodRadius, header->lodBias, header->lodScale);
...
===============
*/
void R_RecursiveBoneListAdd(int bi,
                            int* boneList,
                            int* numBones,
                            mdsBoneInfo_t* boneInfoList) {
  if (boneInfoList[bi].parent >= 0) {
    R_RecursiveBoneListAdd(boneInfoList[bi].parent, boneList, numBones,
                           boneInfoList);
  }
...
===============
*/
int R_GetBoneTag(orientation_t* outTag,
                 mdsHeader_t* mds,
                 int startTagIndex,
                 const refEntity_t* refent,
                 const char* tagName) {
  ...
  // now build the list of bones we need to calc to get this tag's bone
  // information
