...
void RB_CheckOverflow(int verts, int indexes) {
  if (tess.numVertexes + verts < tess.maxShaderVerts &&
      tess.numIndexes + indexes < tess.maxShaderIndicies) {
    return;
  }
...
==============
*/
void RB_AddQuadStampFadingCornersExt(vec3_t origin,
                                     vec3_t left,
                                     vec3_t up,
                                     byte* color,
                                     float s1,
  ...
  tess.normal[ndx].v[0] = tess.normal[ndx + 1].v[0] =
      tess.normal[ndx + 2].v[0] = tess.normal[ndx + 3].v[0] =
          tess.normal[ndx + 4].v[0] = normal[0];
  tess.normal[ndx].v[1] = tess.normal[ndx + 1].v[1] =
      tess.normal[ndx + 2].v[1] = tess.normal[ndx + 3].v[1] =
          tess.normal[ndx + 4].v[1] = normal[1];
  tess.normal[ndx].v[2] = tess.normal[ndx + 1].v[2] =
      tess.normal[ndx + 2].v[2] = tess.normal[ndx + 3].v[2] =
  ...
  tess.texCoords0[ndx + 4].v[0] = tess.texCoords1[ndx + 4].v[0] =
      (s1 + s2) / 2.0;
  tess.texCoords0[ndx + 4].v[1] = tess.texCoords1[ndx + 4].v[1] =
      (t1 + t2) / 2.0;
...
void RB_AddQuadStampExt(vec3_t origin,
  ...
  tess.normal[ndx].v[0] = tess.normal[ndx + 1].v[0] =
      tess.normal[ndx + 2].v[0] = tess.normal[ndx + 3].v[0] = normal[0];
  tess.normal[ndx].v[1] = tess.normal[ndx + 1].v[1] =
      tess.normal[ndx + 2].v[1] = tess.normal[ndx + 3].v[1] = normal[1];
  tess.normal[ndx].v[2] = tess.normal[ndx + 1].v[2] =
      tess.normal[ndx + 2].v[2] = tess.normal[ndx + 3].v[2] = normal[2];
...
void RB_SurfaceTriangles(srfTriangles_t* srf) {
  ...
  if (needsNormal) {
    for (i = 0; i < srf->numVerts; i++, dv++, xyz += 4, normal += 4,
        texCoords0 += 2, texCoords1 += 2, color += 4) {
      xyz[0] = dv->xyz[0];
      xyz[1] = dv->xyz[1];
      xyz[2] = dv->xyz[2];
...
void RB_SurfaceFoliage(srfFoliage_t* srf) {
  ...
  if (distanceCull[1] > 0) {
    ...
    // alpha = (distanceCull[ 1 ] - VectorLength( delta ) + srf->radius) *
    // distanceCull[ 3 ];
    z = fovScale * (DotProduct(srf->origin, distanceVector) +
                    distanceVector[3] - srf->radius);
  ...
  for (o = 0; o < srf->numInstances; o++, instance++) {
    ...
    if (distanceCull[1] > 0.0f) {
      ...
      z = fovScale *
          (DotProduct(instance->origin, distanceVector) + distanceVector[3]);
      ...
      for (i = 0; i < 5; i++) {
        dist =
            DotProduct(instance->origin, backEnd.viewParms.frustum[i].normal) -
            backEnd.viewParms.frustum[i].dist;
        if (dist < -64.0) {
          break;
        }
      }
    ...
    // Com_Printf( "Color: %d %d %d %d\n", srf->colors[ o ][ 0 ], srf->colors[ o
    // ][ 1 ], srf->colors[ o ][ 2 ], alpha );
    ...
    // copy indexes
    memcpy(&tess.indexes[tess.numIndexes], srf->indexes,
           numIndexes * sizeof(srf->indexes[0]));
    ...
    if (tess.shader->needsNormal) {
      memcpy(&tess.normal[tess.numVertexes].v, srf->normal,
             numVerts * sizeof(srf->xyz[0]));
    }
    memcpy(&tess.texCoords0[tess.numVertexes], srf->texCoords,
           numVerts * sizeof(srf->texCoords[0]));
    memcpy(&tess.texCoords1[tess.numVertexes], srf->lmTexCoords,
           numVerts * sizeof(srf->lmTexCoords[0]));
...
static void DoRailCore(const vec3_t start,
                       const vec3_t end,
                       const vec3_t up,
                       float len,
                       float spanWidth) {
  ...
  tess.vertexColors[tess.numVertexes].v[0] =
      backEnd.currentEntity->e.shaderRGBA[0];
  tess.vertexColors[tess.numVertexes].v[1] =
      backEnd.currentEntity->e.shaderRGBA[1];
  tess.vertexColors[tess.numVertexes].v[2] =
      backEnd.currentEntity->e.shaderRGBA[2];
  tess.vertexColors[tess.numVertexes].v[3] =
...
static void DoRailDiscs(int numSegs,
                        const vec3_t start,
                        const vec3_t dir,
                        const vec3_t right,
                        const vec3_t up) {
...
static void LerpMeshVertexes(md3Surface_t* surf, float backlerp) {
  ...
  if (backlerp == 0) {
    ...
    //
    for (vertNum = 0; vertNum < numVerts;
         vertNum++, newXyz += 4, newNormals += 4, outXyz += 4, outNormal += 4) {
    ...
    for (vertNum = 0; vertNum < numVerts; vertNum++, oldXyz += 4, newXyz += 4,
        oldNormals += 4, newNormals += 4, outXyz += 4, outNormal += 4) {
...
// ydnar: ok :)
...
#else
      lat = myftol((((oldNormals[0] >> 8) & 0xFF) * (FUNCTABLE_SIZE / 256) *
                    newNormalScale) +
                   (((oldNormals[0] >> 8) & 0xFF) * (FUNCTABLE_SIZE / 256) *
                    oldNormalScale));
      lng = myftol(
          ((oldNormals[0] & 0xFF) * (FUNCTABLE_SIZE / 256) * newNormalScale) +
    ...
    //%	VectorArrayNormalize((vec4_t *)tess.normal[tess.numVertexes].v,
    //numVerts);
  }
}
...
void R_LatLongToNormal(vec3_t outNormal, short latLong) {
  ...
  outNormal[0] = tr.sinTable[(lat + (FUNCTABLE_SIZE / 4)) & FUNCTABLE_MASK] *
                 tr.sinTable[lng];
  outNormal[1] = tr.sinTable[lat] * tr.sinTable[lng];
  outNormal[2] = tr.sinTable[(lng + (FUNCTABLE_SIZE / 4)) & FUNCTABLE_MASK];
}
  ...
  if (backlerp == 0) {
    ...
    for (vertNum = 0; vertNum < numVerts;
      ...
      if (hasComp && *newComp >= 0) {
        ...
        //%	VectorCopy( r_anormals[ (lng & (0xF << 4)) | ((lat >> 4) & 0xF)
        //], outNormal );
      }
    }
  } else {
    ...
    if (hasComp) {
      ...
      if (*oldComp >= 0) {
        oldXyzComp =
            (mdcXyzCompressed_t*)((byte*)surf + surf->ofsXyzCompressed) +
    ...
    for (vertNum = 0; vertNum < numVerts; vertNum++, oldXyz += 4, newXyz += 4,
      ...
      // add the compressed ofsVec
      if (hasComp && *newComp >= 0) {
        R_MDC_DecodeXyzCompressed(newXyzComp->ofsVec, newOfsVec,
                                  uncompressedNewNormal);
        newXyzComp++;
      ...
      if (hasComp && *oldComp >= 0) {
        R_MDC_DecodeXyzCompressed(oldXyzComp->ofsVec, oldOfsVec,
                                  uncompressedOldNormal);
        oldXyzComp++;
