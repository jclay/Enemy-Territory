...
#define RF_HILIGHT \
  0x000100  // more than RF_MINLIGHT.  For when an object is "Highlighted"
...
typedef enum {
  ...
  RT_BEAM,
  RT_RAIL_CORE,
  RT_RAIL_CORE_TAPER,  // a modified core that creates a properly texture mapped
                       // core that's wider at one end
  RT_RAIL_RINGS,
  RT_LIGHTNING,
...
typedef struct {
  ...
  vec3_t
      torsoAxis[3];  // rotation vectors for torso section of skeletal animation
...
typedef enum {
  ...
  FOG_SKY,         //	1	fog values to apply to the sky when using density fog
  ...
  FOG_MAP,      //	4	use fog parameter specified using the "fogvars" in the
                //sky shader
  FOG_WATER,    //	5	used when underwater
  FOG_SERVER,   //	6	the server has set my fog (probably a
                //target_fog) (keep synch in sv_ccmds.c !!!)
  FOG_CURRENT,  //	7	stores the current values when a transition
                //starts
  FOG_LAST,     //	8	stores the current values when a transition starts
  FOG_TARGET,   //	9	the values it's transitioning to.

  FOG_CMD_SWITCHFOG,  // 10	transition to the fog specified in the second
                      // parameter of R_SetFog(...) (keep synch in sv_ccmds.c
                      // !!!)
