...
panel_button_text_t debriefTitleFont = {
    0.3f, 0.3f, {1.f, 1.f, 1.f, 0.8f}, 0, ITEM_ALIGN_CENTER, DB_MASTER_FONT,
};

panel_button_text_t debriefHeadingFont = {
    0.24f, 0.24f, {1.f, 1.f, 1.f, 0.8f}, 0, 0, DB_MASTER_FONT,
};

panel_button_text_t debriefListFont = {
    0.20f, 0.22f, {1.f, 1.f, 1.f, 0.8f}, 0, 0, DB_MASTER_FONT,
};

panel_button_text_t debriefPlayerHeadingSmallerFont = {
    0.2f, 0.2f, {0.6f, 0.6f, 0.6f, 1.f}, 0, 0, DB_MASTER_FONT,
};
...
panel_button_t* debriefPanelButtons[] = {&debriefTitleWindow,
...
panel_button_text_t teamDebriefBigTitle = {
    0.32f, 0.32f, {1.f, 1.f, 1.f, 0.8f}, 0, 0, DB_MASTER_FONT,
};

panel_button_text_t teamDebriefTitleSmall = {
    0.24f, 0.24f, {1.f, 1.f, 1.f, 0.8f}, 0, ITEM_ALIGN_CENTER, DB_MASTER_FONT,
};

panel_button_text_t teamDebriefTitle = {
    0.28f, 0.28f, {1.f, 1.f, 1.f, 0.8f}, 0, 0, DB_MASTER_FONT,
};
...
panel_button_t* teamDebriefPanelButtons[] = {&debriefTitleWindow,
...
panel_button_text_t chatPanelButtonFont = {
    0.20f, 0.20f, {1.f, 1.f, 1.f, 0.8f}, 0, ITEM_ALIGN_CENTER, DB_MASTER_FONT,
};

panel_button_text_t chatPanelButtonFontRed = {
    0.20f, 0.20f, {1.f, 0.f, 0.f, 0.8f}, 0, ITEM_ALIGN_CENTER, DB_MASTER_FONT,
};
...
void CG_Debriefing_ChatEdit_Draw(panel_button_t* button) {
  ...
  } while (CG_Text_Width_Ext(buffer + offset, button->font->scalex, 0,
  ...
  CG_Text_Paint_Ext(button->rect.x, button->rect.y + button->rect.h,
                    button->font->scalex, button->font->scaley,
                    button->font->colour, cs, 0, 0, button->font->style,
                    button->font->font);
}
...
void CG_Debriefing_ChatBox_Draw(panel_button_t* button) {
  ...
  if (cgs.teamLastChatPos != cgs.teamChatPos) {
    ...
    for (i = cgs.teamLastChatPos; i < cgs.teamChatPos; i++) {
      len = CG_Text_Width_Ext(cgs.teamChatMsgs[i % chatHeight], 0.2f, 0,
    ...
    for (i = cgs.teamChatPos - 1; i >= cgs.teamLastChatPos; i--) {
      ...
      CG_DrawPic(button->rect.x,
                 button->rect.y - (cgs.teamChatPos - i) * lineHeight, chatWidth,
                 lineHeight, cgs.media.teamStatusBar);

      trap_R_SetColor(NULL);

      if (cgs.teamChatMsgTeams[i % chatHeight] == TEAM_AXIS) {
        CG_DrawPic(button->rect.x,
                   button->rect.y - (cgs.teamChatPos - i - 1) * lineHeight - 8,
                   12, 10, cgs.media.axisFlag);
      } else if (cgs.teamChatMsgTeams[i % chatHeight] == TEAM_ALLIES) {
        CG_DrawPic(button->rect.x,
                   button->rect.y - (cgs.teamChatPos - i - 1) * lineHeight - 8,
                   12, 10, cgs.media.alliedFlag);
      }

      CG_Text_Paint_Ext(button->rect.x + 12,
                        button->rect.y - (cgs.teamChatPos - i - 1) * lineHeight,
                        0.2f, 0.2f, colorWhite,
                        cgs.teamChatMsgs[i % chatHeight], 0, 0, 0,
                        &cgs.media.limboFont2);
    }
  }
}

panel_button_t* chatPanelButtons[] = {&chatPanelWindow,
                                      &chatPanelText,
...
void CG_Debriefing_Startup(void) {
  ...
  if (atoi(buf) == -1) {
    ...
    trap_S_StartLocalSound(
        trap_S_RegisterSound("sound/music/allies_win.wav", qtrue),
        CHAN_LOCAL_SOUND);
  } else {
    trap_S_StartLocalSound(
        trap_S_RegisterSound("sound/music/axis_win.wav", qtrue),
        CHAN_LOCAL_SOUND);
  }
...
void CG_DebriefingPlayerWeaponStats_Draw(panel_button_t* button) {
  ...
  for (i = 0; i < 7 && pos != -1;
       i++, pos = CG_Debriefing_GetNextWeaponStat(pos)) {
    CG_Text_Paint_Ext(18, y, button->font->scalex, button->font->scaley,
                      button->font->colour, aWeaponInfo[pos].pszName, 0, 0, 0,
                      button->font->font);

    CG_Text_Paint_Ext(
        78, y, button->font->scalex, button->font->scaley, button->font->colour,
        va("%i", cgs.dbWeaponStats[pos].numShots), 0, 0, 0, button->font->font);

    CG_Text_Paint_Ext(118, y, button->font->scalex, button->font->scaley,
                      button->font->colour,
                      va("%i", cgs.dbWeaponStats[pos].numHits), 0, 0, 0,
                      button->font->font);

    CG_Text_Paint_Ext(148, y, button->font->scalex, button->font->scaley,
                      button->font->colour,
                      va("%i", cgs.dbWeaponStats[pos].numKills), 0, 0, 0,
                      button->font->font);
...
void CG_DebriefingTitle_Draw(panel_button_t* button) {
  ...
  CG_Text_Paint_Ext(x, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour, s, 0, 0, 0,
                    button->font->font);

  s = va("%i seconds to next map",
         max(60 - (cg.time - cgs.intermissionStartTime) / 1000, 0));
  w = CG_Text_Width_Ext(s, button->font->scalex, 0, button->font->font);
...
void CG_DebriefingPlayerList_Draw(panel_button_t* button) {
  ...
  for (i = 0; i + cgs.dbPlayerListOffset < MAX_CLIENTS && i < 24; i++) {
    clientInfo_t* ci =
        &cgs.clientinfo[cgs.dbSortedClients[i + cgs.dbPlayerListOffset]];
    if (!ci->infoValid) {
      break;
    }

    for (j = 0; j < MAX_CLIENTS; j++) {
      if (cg.scores[j].client ==
          cgs.dbSortedClients[i + cgs.dbPlayerListOffset]) {
        score = &cg.scores[j];
        break;
      }
    }
    ...
    if (cgs.dbSelectedClient ==
        cgs.dbSortedClients[i + cgs.dbPlayerListOffset]) {
      vec4_t clr = {1.f, 1.f, 1.f, 0.3f};
      CG_FillRect(button->rect.x, y - 10, 640 - 10 - 8 - 16 - button->rect.x,
                  12, clr);
    }

    CG_Text_Paint_Ext(DB_RANK_X, y, button->font->scalex, button->font->scaley,
                      button->font->colour,
                      CG_Debriefing_RankNameForClientInfo(ci), 0, 0, 0,
                      button->font->font);

    CG_Text_Paint_Ext(DB_NAME_X, y, button->font->scalex, button->font->scaley,
                      button->font->colour, ci->name, 0, 28, 0,
                      button->font->font);

    /*		x = DB_MEDALS_X;
                    for( j = 0; j < SK_NUM_SKILLS; j++ ) {
                            if( ci->medals[j] ) {
                                    CG_DrawPic( x, y - 9, 10, 10,
       cgs.media.medals[j] );
                            }
                            x += 12;
                    }*/

    CG_Text_Paint_Ext(DB_TIME_X, y, button->font->scalex, button->font->scaley,
                      button->font->colour, va("%i", score->time), 0, 0, 0,
                      button->font->font);

    CG_Text_Paint_Ext(DB_XP_X, y, button->font->scalex, button->font->scaley,
                      button->font->colour, va("%i", ci->score), 0, 0, 0,
                      button->font->font);

    if (cgs.dbPlayerKillsDeathsRecieved) {
      CG_Text_Paint_Ext(DB_KILLS_X, y, button->font->scalex,
                        button->font->scaley, button->font->colour,
                        va("%i", ci->kills), 0, 0, 0, button->font->font);
      CG_Text_Paint_Ext(DB_DEATHS_X, y, button->font->scalex,
                        button->font->scaley, button->font->colour,
                        va("%i", ci->deaths), 0, 0, 0, button->font->font);
    } else {
      CG_Text_Paint_Ext(DB_KILLS_X, y, button->font->scalex,
                        button->font->scaley, button->font->colour, "-", 0, 0,
                        0, button->font->font);
      CG_Text_Paint_Ext(DB_DEATHS_X, y, button->font->scalex,
                        button->font->scaley, button->font->colour, "-", 0, 0,
                        0, button->font->font);
...
const char* CG_Debriefing_FullRankNameForClientInfo(clientInfo_t* ci) {
  ...
  return ci->team == TEAM_AXIS ? rankNames_Axis[ci->rank]
...
const char* CG_Debriefing_RankNameForClientInfo(clientInfo_t* ci) {
  ...
  return ci->team == TEAM_AXIS ? miniRankNames_Axis[ci->rank]
...
void CG_Debriefing_Scrollbar_Draw(panel_button_t* button) {
  ...
  CG_FillRect(button->rect.x, button->rect.y, button->rect.w, button->rect.h,
              clr2);
  CG_DrawRect_FixedBorder(button->rect.x, button->rect.y, button->rect.w,
                          button->rect.h, 1, colorMdGrey);
...
void CG_Debriefing_PlayerSkills_Draw(panel_button_t* button) {
  ...
  CG_Text_Paint_Ext(button->rect.x, button->rect.y - 2, button->font->scalex,
                    button->font->scaley, button->font->colour,
                    skillNames[button->data[0]], 0, 0, ITEM_TEXTSTYLE_SHADOWED,
                    button->font->font);

  x = button->rect.x;
  CG_DrawPic(x, button->rect.y, button->rect.w, button->rect.h,
             cgs.media.skillPics[button->data[0]]);

  x += button->rect.w + 2;
  for (i = ci->skill[button->data[0]]; i > 0; i--) {
    CG_DrawPicST(x, button->rect.y, button->rect.w, button->rect.h, 0, 0, 1.f,
                 0.5f, cgs.media.limboStar_roll);
...
void CG_Debriefing_PlayerACC_Draw(panel_button_t* button) {
  ...
  CG_Text_Paint_Ext(button->rect.x - w, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour, "ACC:", 0, 0,
                    ITEM_TEXTSTYLE_SHADOWED, button->font->font);

  CG_Text_Paint_Ext(button->rect.x, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour,
                    va("%i%%", ci->totalWeapAcc), 0, 0, ITEM_TEXTSTYLE_SHADOWED,
                    button->font->font);
}
...
void CG_Debriefing_PlayerXP_Draw(panel_button_t* button) {
  ...
  CG_Text_Paint_Ext(button->rect.x - w, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour, "XP:", 0, 0,
                    ITEM_TEXTSTYLE_SHADOWED, button->font->font);

  CG_Text_Paint_Ext(button->rect.x, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour,
                    va("%i", ci->score), 0, 0, ITEM_TEXTSTYLE_SHADOWED,
                    button->font->font);
}
...
void CG_Debriefing_PlayerTime_Draw(panel_button_t* button) {
  ...
  CG_Text_Paint_Ext(button->rect.x - w, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour, "Time:", 0, 0,
                    ITEM_TEXTSTYLE_SHADOWED, button->font->font);

  CG_Text_Paint_Ext(button->rect.x, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour,
                    va("%i", score->time), 0, 0, ITEM_TEXTSTYLE_SHADOWED,
                    button->font->font);
}
...
void CG_Debriefing_PlayerMedals_Draw(panel_button_t* button) {
  ...
  w = CG_Text_Width_Ext("Medals: ", button->font->scalex, 0,
                        button->font->font);
  CG_Text_Paint_Ext(button->rect.x - w, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour, "Medals:", 0, 0,
                    ITEM_TEXTSTYLE_SHADOWED, button->font->font);
...
void CG_Debriefing_PlayerRank_Draw(panel_button_t* button) {
  ...
  CG_Text_Paint_Ext(button->rect.x - w, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour, "Rank:", 0, 0,
                    ITEM_TEXTSTYLE_SHADOWED, button->font->font);

  CG_Text_Paint_Ext(button->rect.x, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour,
                    CG_Debriefing_FullRankNameForClientInfo(ci), 0, 0,
                    ITEM_TEXTSTYLE_SHADOWED, button->font->font);
}
...
void CG_Debriefing_PlayerName_Draw(panel_button_t* button) {
  ...
  CG_Text_Paint_Ext(button->rect.x, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour, ci->name, 0, 0,
                    ITEM_TEXTSTYLE_SHADOWED, button->font->font);
}
...
void CG_TeamDebriefingMapList_Draw(panel_button_t* button) {
  ...
  for (i = 0; i + cgs.tdbMapListOffset <= MAX_MAPS_PER_CAMPAIGN && i < 4; i++) {
    ...
    if (i + cgs.tdbMapListOffset == 0) {
      ...
      CG_Text_Paint_Ext(
          button->rect.x, y, button->font->scalex, button->font->scaley,
          button->font->colour,
          cgs.campaignData.arenas[i + cgs.tdbMapListOffset - 1].longname, 0, 0,
          0, button->font->font);
    }
  ...
  if (cg_gameType.integer == GT_WOLF_CAMPAIGN) {
    for (i = 0; i < cgs.campaignData.mapCount; i++) {
      ...
      for (j = 0; j < SK_NUM_SKILLS; j++) {
        ...
        cnt += team == TEAM_AXIS ? cgs.tdbAxisMapsXP[j][i]
                                 : cgs.tdbAlliedMapsXP[j][i];
      }
    }
    ...
    for (j = 0; j < SK_NUM_SKILLS; j++) {
      ...
      cnt += team == TEAM_AXIS ? cgs.tdbAxisMapsXP[j][0]
                               : cgs.tdbAlliedMapsXP[j][0];
    }
  }
...
void CG_TeamDebriefingTeamXP_Draw(panel_button_t* button) {
  ...
  CG_Text_Paint_Ext(button->rect.x, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour,
                    va("%s XP: %i", team == TEAM_AXIS ? "Axis" : "Allies", xp),
...
void CG_TeamDebriefingTeamSkillXP_Draw(panel_button_t* button) {
  ...
  if (button->data[1] == SK_NUM_SKILLS) {
    ...
    xp =
        CG_TeamDebriefing_CalcXP(team, cgs.tdbSelectedMap - 1, button->data[1]);
  }

  //	const char* text = va( "%s: ", skillNames[ button->data[1] ] );

  //	float w = CG_Text_Width_Ext( text, button->font->scalex, 0,
  //button->font->font );

  //	CG_Text_Paint_Ext( button->rect.x - w, button->rect.y,
  //button->font->scalex, button->font->scaley, button->font->colour, text, 0,
  //0, 0, button->font->font );

  CG_Text_Paint_Ext(button->rect.x, button->rect.y, button->font->scalex,
                    button->font->scaley, button->font->colour, va("%i", xp), 0,
                    0, 0, button->font->font);
}
  ...
  if (cgs.tdbSelectedMap == 0) {
    ...
    CG_DrawPic(button->rect.x, button->rect.y, button->rect.w, button->rect.h,
               trap_R_RegisterShaderNoMip(
                   va("levelshots/%s_cc.tga",
                      cgs.campaignData.mapnames[cgs.tdbSelectedMap - 1])));
  }
}
...
void CG_PanelButtonsRender_Button_Ext(rectDef_t* r, const char* text) {
  ...
  if (text) {
    ...
    CG_Text_Paint_Ext(r->x + ((r->w + 2) - w) * 0.5f, r->y + 11, 0.19f, 0.19f,
                      hilight ? clrTxt_hi : clrTxt, text, 0, 0, 0,
                      &cgs.media.limboFont2);
  }
}
...
void CG_PanelButtonsRender_Window_Ext(rectDef_t* r,
                                      const char* text,
                                      int align,
                                      int innerheight,
                                      float fontscale,
  ...
  if (text) {
    ...
    CG_Text_Paint_Ext(x, r->y + yofs, fontscale, fontscale, clrTxtBck, text, 0,
...
void CG_Debreifing2_MissionTitle_Draw(panel_button_t* button) {
  ...
  if (cg_gameType.integer == GT_WOLF_STOPWATCH) {
    ...
    s = va("CAMPAIGN STATUS: %s", CG_Debreifing2_WinStringForTeam(
                                      CG_Debriefing_FindOveralWinningTeam()));
    w = CG_Text_Width_Ext(s, 0.25f, 0, &cgs.media.limboFont1);
    CG_Text_Paint_Ext(button->rect.x + (button->rect.w - w) * 0.5f,
                      button->rect.y + 16, 0.25f, 0.25f, clrTxtBck, s, 0, 0, 0,
  ...
  s = va("%i SECS TO NEXT MAP",
         max(60 - (cg.time - cgs.intermissionStartTime) / 1000, 0));

  w = CG_Text_Width_Ext(s, 0.25f, 0, &cgs.media.limboFont1);
  x = button->rect.x + button->rect.w - w - 4;

  CG_Text_Paint_Ext(x, button->rect.y + 16, 0.25f, 0.25f, clrTxtBck, s, 0, 0, 0,
                    &cgs.media.limboFont1);
}
...
const char* awardNames[NUM_ENDGAME_AWARDS] = {
    ...
    "Welcome Newbie! Award",  // dont get this if any other award given or > 100
                              // xp (this map)
};
...
void CG_Debreifing2_Awards_Draw(panel_button_t* button) {
  ...
  for (i = 0; i < NUM_ENDGAME_AWARDS; i++) {
    ...
    CG_DrawPic(button->rect.x + 6, y + 2, 18, 12,
               cgs.dbAwardTeams[i] == TEAM_AXIS ? cgs.media.axisFlag
                                                : cgs.media.alliedFlag);

    CG_Text_Paint_Ext(button->rect.x + 28, y + 11, 0.19f, 0.19f, clrTxtBck,
                      awardNames[i], 0, 0, 0, &cgs.media.limboFont2);

    CG_Text_Paint_Ext(button->rect.x + 28 + 180, y + 11, 0.19f, 0.19f,
                      clrTxtBck, cgs.dbAwardNames[i], 0, 0, 0,
                      &cgs.media.limboFont2);
    y += 16;
  }
}
...
void CG_Debreifing2_Maps_Draw(panel_button_t* button) {
  ...
  if (cg_gameType.integer == GT_WOLF_CAMPAIGN) {
    ...
    CG_Text_Paint_Ext(button->rect.x + 4, button->rect.y + 11, 0.19f, 0.19f,
                      clrTxtBck,
                      va("Campaign: %s", cgs.campaignData.campaignName), 0, 0,
    ...
    for (i = 0; i < cgs.campaignData.mapCount; i++) {
      ...
      CG_Text_Paint_Ext(
          button->rect.x + 8, y + 11, 0.19f, 0.19f, clrTxtBck,
          va("%i. %s", i + 1, cgs.campaignData.arenas[i].longname), 0, 0, 0,
          &cgs.media.limboFont2);

      if (i <= cgs.currentCampaignMap) {
        str = CG_Debreifing2_WinStringForTeam(
            CG_Debriefing_FindWinningTeamForPos(i + 1));

        w = CG_Text_Width_Ext(str, 0.2f, 0, &cgs.media.limboFont2);
        CG_Text_Paint_Ext(button->rect.x + button->rect.w - w - 8, y + 11,
                          0.19f, 0.19f, clrTxtBck, str, 0, 0, 0,
                          &cgs.media.limboFont2);
      }
...
void CG_Debreifing2_Mission_Draw(panel_button_t* button) {
  if (cg_gameType.integer == GT_WOLF_CAMPAIGN) {
    ...
    if (cgs.campaignData.mapTC[0][0] && cgs.campaignData.mapTC[1][0]) {
      ...
      CG_DrawPicST(button->rect.x, button->rect.y, button->rect.w,
                   button->rect.h, cgs.campaignData.mapTC[0][0] / 1024.f,
                   cgs.campaignData.mapTC[0][1] / 1024.f,
                   cgs.campaignData.mapTC[1][0] / 1024.f,
                   cgs.campaignData.mapTC[1][1] / 1024.f,
      ...
      for (i = cgs.campaignData.mapCount - 1; i >= 0; i--) {
        ...
        x = button->rect.x + ((cgs.campaignData.arenas[i].mappos[0] -
                               cgs.campaignData.mapTC[0][0]) /
                              650.f * button->rect.w);
        y = button->rect.y + ((cgs.campaignData.arenas[i].mappos[1] -
                               cgs.campaignData.mapTC[0][1]) /
                              650.f * button->rect.h);

        w = CG_Text_Width_Ext(cgs.campaignData.arenas[i].longname, 0.2f, 0,
                              &cgs.media.limboFont2);
        ...
        switch (CG_Debriefing_FindWinningTeamForPos(i + 1)) {
          case TEAM_AXIS:
            CG_DrawPic(x - 12, y - 12, 24, 24,
                       trap_R_RegisterShaderNoMip("gfx/loading/pin_axis"));
            break;
          case TEAM_ALLIES:
            CG_DrawPic(x - 12, y - 12, 24, 24,
                       trap_R_RegisterShaderNoMip("gfx/loading/pin_allied"));
            break;
          default:
            CG_DrawPic(x - 12, y - 12, 24, 24,
        ...
        if (x + 10 + w > button->rect.x + button->rect.w) {
          CG_Text_Paint_Ext(x - w - 10, y + 3, 0.2f, 0.2f, colorWhite,
                            cgs.campaignData.arenas[i].longname, 0, 0, 0,
                            &cgs.media.limboFont2);
        } else {
          CG_Text_Paint_Ext(x + 10, y + 3, 0.2f, 0.2f, colorWhite,
                            cgs.campaignData.arenas[i].longname, 0, 0, 0,
                            &cgs.media.limboFont2);
        }
      }
  ...
  if (cgs.arenaData.mappos[0] && cgs.arenaData.mappos[1]) {
    ...
    CG_DrawPicST(button->rect.x, button->rect.y, button->rect.w, button->rect.h,
                 tl[0] / 1024.f, tl[1] / 1024.f, br[0] / 1024.f, br[1] / 1024.f,
                 trap_R_RegisterShaderNoMip("gfx/loading/camp_map"));

    x = button->rect.x +
        ((cgs.arenaData.mappos[0] - tl[0]) / 650.f * button->rect.w);
    y = button->rect.y +
        ((cgs.arenaData.mappos[1] - tl[1]) / 650.f * button->rect.h);

    w = CG_Text_Width_Ext(cgs.arenaData.longname, 0.2f, 0,
                          &cgs.media.limboFont2);
    ...
    if (x + 10 + w > button->rect.x + button->rect.w) {
      CG_Text_Paint_Ext(x - w - 10, y + 3, 0.2f, 0.2f, colorWhite,
                        cgs.arenaData.longname, 0, 0, 0, &cgs.media.limboFont2);
    } else {
      CG_Text_Paint_Ext(x + 10, y + 3, 0.2f, 0.2f, colorWhite,
                        cgs.arenaData.longname, 0, 0, 0, &cgs.media.limboFont2);
    }
  ...
  for (j = 0; j < 2; j++) {
    for (i = 0; i <= SK_NUM_SKILLS; i++) {
      ...
      if (*str) {
        ...
        CG_Text_Paint_Ext(button->rect.x + 100 + skillPositions[i] - (w * 0.5f),
                          button->rect.y + (j * 11), 0.2f, 0.2f, clrTxtBck, str,
...
void CG_Debriefing2TeamSkillXP_Draw(panel_button_t* button) {
  ...
  switch (team) {
    case TEAM_AXIS:
      CG_Text_Paint_Ext(button->rect.x, button->rect.y + 11, scale, scale,
                        clrTxtBck, "Axis", 0, 0, 0, &cgs.media.limboFont2);
      break;
    default:
      CG_Text_Paint_Ext(button->rect.x, button->rect.y + 11, scale, scale,
                        clrTxtBck, "Allies", 0, 0, 0, &cgs.media.limboFont2);
      break;
  }
  ...
  for (i = 0; i <= SK_NUM_SKILLS; i++) {
    ...
    CG_Text_Paint_Ext(button->rect.x + 100 + skillPositions[i] - (w * 0.5f),
                      button->rect.y + 11, scale, scale, clrTxtBck, str, 0, 0,
                      0, &cgs.media.limboFont2);
  }
}
