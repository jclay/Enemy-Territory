...
qboolean CG_FindCampaignInFile(char* filename,
                               char* campaignShortName,
                               cg_campaignInfo_t* info) {
  int handle;
  pc_token_t token;
  //	char* dummy;
  ...
  while (trap_PC_ReadToken(handle, &token)) {
    if (*token.string == '}') {
      ...
      if (!trap_PC_ReadToken(
              handle,
              &token)) {  // don't do a stringparse due to memory constraints
        trap_Print(
      ...
      Q_strncpyz(info->campaignDescription, token.string,
                 sizeof(info->campaignDescription));
...
qboolean CG_FindArenaInfo(char* filename, char* mapname, arenaInfo_t* info) {
  ...
  while (trap_PC_ReadToken(handle, &token)) {
    if (*token.string == '}') {
      if (found) {
        //				info->image =
        //trap_R_RegisterShaderNoMip(va("levelshots/%s", mapname));
        trap_PC_FreeSource(handle);
        return qtrue;
      }
      found = qfalse;
...
void CG_LocateCampaign(void) {
  /*	int			numdirs;
     ...
     ); dirptr  = dirlist; for (i = 0; i < numdirs; i++, dirptr += dirlen+1) {
     ...
     { found = qtrue; break;
          ...
          for(i = 0; i < cgs.campaignData.mapCount; i++ ) {
                  Com_sprintf( filename, sizeof(filename), "scripts/%s.arena",
     cgs.campaignData.mapnames[i] );
                  // Gordon: horrible hack, but i dont plan to parse EVERY
     .arena to get a map briefing... if(	!CG_FindArenaInfo(
     "scripts/wolfmp.arena", cgs.campaignData.mapnames[i],
     &cgs.campaignData.arenas[i] ) && !CG_FindArenaInfo( "scripts/wolfxp.arena",
     cgs.campaignData.mapnames[i], &cgs.campaignData.arenas[i] ) &&
                          !CG_FindArenaInfo( filename,
     cgs.campaignData.mapnames[i], &cgs.campaignData.arenas[i] )) { return;
                  }
          }
  ...
  for (i = 0; i < numdirs; i++, dirptr += dirlen + 1) {
    ...
    if (CG_FindCampaignInFile(filename, cgs.currentCampaign,
                              &cgs.campaignData)) {
      found = qtrue;
      break;
    }
  ...
  for (i = 0; i < cgs.campaignData.mapCount; i++) {
    Com_sprintf(filename, sizeof(filename), "scripts/%s.arena",
                cgs.campaignData.mapnames[i]);
    // Gordon: horrible hack, but i dont plan to parse EVERY .arena to get a map
    // briefing...
    if (/*!CG_FindArenaInfo( "scripts/wolfmp.arena",
           cgs.campaignData.mapnames[i], &cgs.campaignData.arenas[i] ) &&
           !CG_FindArenaInfo( "scripts/wolfxp.arena",
           cgs.campaignData.mapnames[i], &cgs.campaignData.arenas[i] ) &&*/
        !CG_FindArenaInfo(filename, cgs.campaignData.mapnames[i],
                          &cgs.campaignData.arenas[i])) {
      return;
    }
  }
