  ...
  switch (key) {
    ...
    case K_UPARROW:
      if (milli > cgs.thirdpersonUpdate) {
        ...
        range -= ((range >= 4 * DEMO_RANGEDELTA) ? DEMO_RANGEDELTA
...
void CG_GameStatsDraw() {
  if (cgs.gamestats.show == SHOW_OFF) {
    ...
    y += 1 + tSpacing;
    CG_Text_Paint_Ext(x, y, hScale2, hScaleY2, hdrColor, "Rank", 0.0f, 0,
                      hStyle2, hFont2);
    x += 82;
    CG_Text_Paint_Ext(x, y, hScale2, hScaleY2, hdrColor, "XP", 0.0f, 0, hStyle2,
                      hFont2);
    ...
    y += 1;
    y += tSpacing;
    CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, gs->strRank, 0.0f, 0,
                      tStyle, tFont);
    ...
    y += 1 + tSpacing;
    CG_Text_Paint_Ext(x, y, hScale2, hScaleY2, hdrColor, "Skills", 0.0f, 0,
                      hStyle2, hFont2);
    x += 84;
    CG_Text_Paint_Ext(x, y, hScale2, hScaleY2, hdrColor, "Level", 0.0f, 0,
                      hStyle2, hFont2);
    x += 40;
    CG_Text_Paint_Ext(x, y, hScale2, hScaleY2, hdrColor, "XP / Next Level",
                      0.0f, 0, hStyle2, hFont2);
    if (cgs.gametype == GT_WOLF_CAMPAIGN) {
      x += 86;
      CG_Text_Paint_Ext(x, y, hScale2, hScaleY2, hdrColor, "Medals", 0.0f, 0,
                        hStyle2, hFont2);
    }
    ...
    if (gs->cSkills == 0) {
      y += tSpacing;
      CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, "No skills acquired!",
                        0.0f, 0, tStyle, tFont);
    } else {
      for (i = 0; i < gs->cSkills; i++) {
        y += tSpacing;
        CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, gs->strSkillz[i], 0.0f,
                          0, tStyle, tFont);
      }
    }
  }
...
void CG_DemoHelpDraw() {
  if (cg.demohelpWindow == SHOW_OFF) {
    ...
    const char* help[] = {
        "^nTAB       ^mscores",         "^nF1-F5     ^mavidemo record",
        "^nF11-F12   ^mscreenshot",     NULL,
        "^nKP_DOWN   ^mslow down (--)", "^nKP_LEFT   ^mslow down (-)",
        "^nKP_UP     ^mspeed up (++)",  "^nKP_RIGHT  ^mspeed up (+)",
        "^nSPACE     ^mnormal speed",   NULL,
        "^nENTER     ^mExternal view",  "^nLFT/RGHT  ^mChange angle",
    ...
    for (i = 0; i < sizeof(help) / sizeof(char*); i++) {
      y += tSpacing;
      if (help[i] != NULL) {
        CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, (char*)help[i], 0.0f, 0,
                          tStyle, tFont);
      }
    }
