...
void CG_ParseServerinfo(void) {
  ...
  cg_gameType.integer = cgs.gametype =
      atoi(Info_ValueForKey(info, "g_gametype"));
  ...
  // prolly should parse all CS_SERVERINFO keys automagically, but I don't want
  // to break anything that might be improperly set for wolf SP, so I'm just
  // parsing MP relevant stuff here
  ...
  cgs.weaponRestrictions =
      atoi(Info_ValueForKey(info, "g_heavyWeaponRestriction")) * 0.01f;

  cgs.minclients = atoi(Info_ValueForKey(
      info,
      "g_minGameClients"));  // NERVE - SMF -- OSP: overloaded for ready counts

  // TTimo - make this available for ingame_callvote
  trap_Cvar_Set("cg_ui_voteFlags", ((authLevel.integer == RL_NONE)
                                        ? Info_ValueForKey(info, "voteFlags")
                                        : "0"));
}
...
static void CG_ParseWarmup(void) {
  ...
  if (warmup == 0 && cg.warmup) {
    ...
    //		if(cg_announcer.integer > 0) trap_S_StartLocalSound(
...
static void CG_ConfigStringModified(void) {
  ...
  if (num == CS_MUSIC) {
    ...
    CG_ParseServerVersionInfo(
        str);  // OSP - set versioning info for older demo playback
    ...
    if (str[0] != '*') {  // player specific sounds don't register here
      ...
      if (!strstr(str, ".wav")) {
        ...
        cgs.gameSounds[num - CS_SOUNDS] =
            trap_S_RegisterSound(str, qfalse);  // FIXME: add a compress flag?
      }
    }
    ...
    if (!BG_FindCharacter(str)) {
      ...
      if (!CG_RegisterCharacter(str, cgs.gameCharacters[num - CS_CHARACTERS])) {
        CG_Error(
            "ERROR: CG_ConfigStringModified: failed to load character file "
            "'%s'\n",
            str);
      }
    }
...
static void CG_MapRestart(void) {
  ...
  cg.centerPrintTime =
      0;  // reset centerprint counter so previous messages don't re-appear
  cg.itemPickupTime =
      0;  // reset item pickup counter so previous messages don't re-appear
...
int CG_ParseVoiceChats(const char* filename,
  ...
  // TAT - 10/28/2002 - if you really want to be able to load multiple files,
  // you should take out the loop
  //		above that clears out all the commands "voiceChats[i].id[0] =
  //0;" 		We don't even want the MP voice chats in SP, so no need anyway
...
=================
*/
int CG_GetVoiceChat(voiceChatList_t* voiceChatList,
                    const char* id,
                    sfxHandle_t* snd,
                    qhandle_t* sprite,
                    char** chat) {
...
void CG_AddBufferedVoiceChat(bufferedVoiceChat_t* vchat) {
  // JPW NERVE new system doesn't buffer but overwrites vchats FIXME put this on
  // a cvar to choose which to use
...
=================
*/
void CG_VoiceChatLocal(int mode,
                       qboolean voiceOnly,
                       int clientNum,
                       int color,
                       const char* cmd,
    ...
    if (mode == SAY_TEAM || !cg_teamChatsOnly.integer) {
      ...
      if (mode == SAY_TEAM) {
        Com_sprintf(vchat.message, sizeof(vchat.message),
                    "(%s)%c%c(%s): %c%c%s", ci->name, Q_COLOR_ESCAPE,
        ...
        Com_sprintf(vchat.message, sizeof(vchat.message), "%s%c%c: %c%c%s",
                    ci->name, Q_COLOR_ESCAPE, COLOR_YELLOW, Q_COLOR_ESCAPE,
                    color, CG_TranslateString(chat));
      }
...
const char* CG_LocalizeServerCommand(const char* buf) {
  ...
  for (i = 0; *s; i++, s++) {
    // TTimo:
    // line was: if ( *s == '[' && !Q_strncmp( s, "[lon]", 5 ) || !Q_strncmp( s,
    // "[lof]", 5 ) ) {
  ...
  while (iWeap) {
    ...
    if (ts->cWeapons < WS_MAX * 2) {
      ...
      Q_strncpyz(
          ...
          va("%-12s %5.1f %4d/%-4d %5d  %s", aWeaponInfo[iWeap - 1].pszName,
...
void CG_dumpStats(void) {
  ...
  //		3. we'll just continue to parse the remaining ws entries and
  //dump them to the log 		   before the scores result would ever hit us.. thus, we
  //still keep proper ordering :)
  if (fDoScores) {
...
static void CG_ServerCommand(void) {
  ...
  if (!strcmp(cmd, "rockandroll")) {  // map loaded, game is ready to begin.
    ...
    // trap_UI_Popup("pregame");				// start pregame
    // menu trap_Cvar_Set("cg_norender", "1");	// don't render the world until
    // the player clicks in and the 'playerstart' func has been called (g_main
    // in G_UpdateCvars() ~ilne 949)
  ...
  if (!Q_stricmp(cmd,
                 "mu_stop")) {  // has optional parameter for fade-down time
  ...
  // ydnar: bug 267: server sends this command when it's about to kill the
  // current server, before the client can reconnect
  if (!Q_stricmp(cmd, "spawnserver")) {
    ...
    // fade out over the course of 5 seconds, should be enough (nuking: atvi bug
    // 3793)
