...
void CG_CheckAmmo(void) {
  ...
  if (!weapons[0] && !weapons[1]) {  // (SA) we start out with no weapons, so
                                     // don't make a click on startup
    return;
  }
  ...
  for (i = 0; i < WP_NUM_WEAPONS; i++) {
    ...
    switch (i) {
      ...
      default:
        ...
        //			default:
        //				total +=
        //cg.snap->ps.ammo[BG_FindAmmoForWeapon(i)] * 200; 				break;
    }
...
void CG_DamageFeedback(int yawByte, int pitchByte, int damage) {
  ...
  for (slot = 0; slot < MAX_VIEWDAMAGE; slot++) {
    if (cg.viewDamage[slot].damageTime + cg.viewDamage[slot].damageDuration <
        cg.time) {
      break;
    }
  }
...
void CG_Respawn(qboolean revived) {
  ...
  cg.predictedPlayerState.weapAnim =
      ...
      PM_IdleAnimForWeapon(cg.snap->ps.weapon);  // reset weapon animations
  cg.predictedPlayerState.weaponstate =
      WEAPON_READY;  // hmm, set this?  what to?
...
void CG_CheckLocalSounds(playerState_t* ps, playerState_t* ops) {
  ...
  if (cgs.timelimit > 0 && cgs.gamestate == GS_PLAYING) {
    ...
    if (cgs.timelimit > 2 && !(cg.timelimitWarnings & 2) &&
...
void CG_TransitionPlayerState(playerState_t* ps, playerState_t* ops) {
  ...
  if (cg.mvTotalClients > 0) {
    if (ps->clientNum != ops->clientNum) {
      ...
      cg.predictedPlayerEntity.voiceChatSpriteTime =
          0;  // CHECKME: should we do this here?
  ...
  if (ps->persistant[PERS_SPAWN_COUNT] != ops->persistant[PERS_SPAWN_COUNT]) {
    CG_Respawn(ps->persistant[PERS_REVIVE_COUNT] !=
                       ops->persistant[PERS_REVIVE_COUNT]
                   ? qtrue
                   : qfalse);
  }
  ...
  if (ps->eFlags & EF_PRONE_MOVING) {
    ...
    if (!(ops->eFlags & EF_PRONE_MOVING)) {
      // ydnar: this screws up auto-switching when dynamite planted or grenade
      // thrown/out of ammo
