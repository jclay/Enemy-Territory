...
markPoly_t* CG_AllocMark(int endTime) {
  ...
  // Ridah, TODO: sort this, so the list is always sorted by longest duration ->
  // shortest duration, this way the shortest duration mark will always get
  // overwritten first
  // for (trav = cg_activeMarkPolys.nextMark; (trav->duration + trav->time >
  // endTime) && (trav != cg_activeMarkPolys.prevMark) ; lastTrav = trav, trav++
  // ) {
...
// #define	MARK_TOTAL_TIME		20000	// (SA) made this a cvar:
// cg_markTime  (we could cap the time or remove marks quicker if too long a
// time starts to cause new marks to not appear)
...
#ifdef YDNAR_DECAL_MARKS
{
  ...
  for (i = 0; i < 3; i++) {
    /* old */
    //%	points[ 0 ][ i ] = pushedOrigin[ i ] - radius * axis[ 1 ][ i ] - radius
    //* axis[ 2 ][ i ]; %	points[ 1 ][ i ] = pushedOrigin[ i ] + radius *
    //axis[ 1 ][ i ] - radius * axis[ 2 ][ i ]; %	points[ 2 ][ i ] =
    //pushedOrigin[ i ] + radius * axis[ 1 ][ i ] + radius * axis[ 2 ][ i ]; %
    //points[ 3 ][ i ] = pushedOrigin[ i ] - radius * axis[ 1 ][ i ] + radius *
    //axis[ 2 ][ i ];
...
#else
{
  ...
  vec5_t
      markPoints[MAX_MARK_POINTS];  // Ridah, made it vec5_t so it includes S/T
  vec3_t projection;
  ...
  numFragments = trap_CM_MarkFragments(
      (int)orientation, (void*)originalPoints, projection, MAX_MARK_POINTS,
      (float*)&markPoints[0], MAX_MARK_FRAGMENTS * multMaxFragments,
      markFragments);
