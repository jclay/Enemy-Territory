...
void CG_Init(int serverMessageNum,
...
cvarTable_t cvarTable[] = {
    ...
    //	{ &cg_draw2D, "cg_draw2D", "1", CVAR_CHEAT }, // JPW NERVE changed per
    //atvi req to prevent sniper rifle zoom cheats
    ...
    {&cg_centertime, "cg_centertime", "5",
     CVAR_CHEAT},  // DHM - Nerve :: changed from 3 to 5
    ...
    {&cg_synchronousClients, "g_synchronousClients", "0",
     CVAR_SYSTEMINFO | CVAR_CHEAT},  // communicated by systeminfo
#endif                               // ALLOW_GSYNC
...
void CG_RegisterCvars(void) {
  ...
  // Gordon: um, here, why?
  CG_setClientFlags();
  BG_setCrosshair(cg_crosshairColor.string, cg.xhairColor,
                  cg_crosshairAlpha.value, "cg_crosshairColor");
  BG_setCrosshair(cg_crosshairColorAlt.string, cg.xhairColorAlt,
    ...
    if (cv->vmCvar) {
      ...
      if (cv->modificationCount != cv->vmCvar->modificationCount) {
        ...
        if (cv->vmCvar == &cg_autoAction || cv->vmCvar == &cg_autoReload ||
          ...
          fSetFlags = qtrue;
        } else if (cv->vmCvar == &cg_crosshairColor ||
                   cv->vmCvar == &cg_crosshairAlpha) {
          BG_setCrosshair(cg_crosshairColor.string, cg.xhairColor,
                          cg_crosshairAlpha.value, "cg_crosshairColor");
        } else if (cv->vmCvar == &cg_crosshairColorAlt ||
...
void CG_nameCleanFilename(const char* pszIn,
                          char* pszOut,
...
char* CG_generateFilename(void) {
  qtime_t ct;
  //	int index = (cg.snap == NULL || (cg.snap->ps.pm_flags & PMF_LIMBO)) ?
  ...
  trap_RealTime(&ct);
  //	CG_nameCleanFilename(Info_ValueForKey(pszPlayerInfo, "n"), strCleanName,
  //sizeof(strCleanName));
  ...
  while (1) {
    ...
    if (!Q_stricmp(token.string, "wm_mapdescription")) {
      ...
      if (!trap_PC_ReadToken(handle, &token2)) {
        CG_Printf(
            "^1ERROR: bad objdata line : description parameter required\n");
        break;
      }
...
void CG_SetupDlightstyles(void) {
  ...
  for (i = 1; i < MAX_DLIGHT_CONFIGSTRINGS; i++) {
    ...
    for (j = 0; j < strlen(cent->dl_stylestring); j++) {
      cent->dl_stylestring[j] +=
          cent->dl_atten;  // adjust character for attenuation/amplification
...
static void CG_RegisterSounds(void) {
  ...
  for (i = 1; i < MAX_SOUNDS; i++) {
    ...
    if (!strstr(soundName, ".wav")) {
      ...
      cgs.gameSounds[i] = trap_S_RegisterSound(
          soundName, qfalse);  // FIXME: allow option to compress?
    }
  }
...
");
  ...
  //	cgs.media.waypointBotShader		= trap_R_RegisterShaderNoMip(
  //"sprites/botorder" );
  //	cgs.media.waypointBotQueuedShader=trap_R_RegisterShaderNoMip(
  //"sprites/botqueuedorder" ); 	cgs.media.waypointCompassAttackShader	=
  //trap_R_RegisterShaderNoMip( "sprites/waypoint_attack_compass" );
  //	cgs.media.waypointCompassDefendShader	= trap_R_RegisterShaderNoMip(
  //"sprites/waypoint_defend_compass" ); 	cgs.media.waypointCompassRegroupShader
  //= trap_R_RegisterShaderNoMip( "sprites/waypoint_regroup_compass" );
  //	cgs.media.commandCentreWoodShader		=
  //trap_R_RegisterShaderNoMip( "ui/assets2/commandMap" );
  if (cgs.ccLayers) {
    for (i = 0; i < cgs.ccLayers; i++) {
      cgs.media.commandCentreMapShader[i] = trap_R_RegisterShaderNoMip(
          va("levelshots/%s_%i_cc.tga", cgs.rawmapname, i));
      cgs.media.commandCentreMapShaderTrans[i] = trap_R_RegisterShaderNoMip(
          va("levelshots/%s_%i_cc_trans", cgs.rawmapname, i));
      cgs.media.commandCentreAutomapShader[i] = trap_R_RegisterShaderNoMip(
          va("levelshots/%s_%i_cc_automap", cgs.rawmapname, i));
    }
  } else {
    cgs.media.commandCentreMapShader[0] =
        trap_R_RegisterShaderNoMip(va("levelshots/%s_cc.tga", cgs.rawmapname));
    cgs.media.commandCentreMapShaderTrans[0] = trap_R_RegisterShaderNoMip(
        va("levelshots/%s_cc_trans", cgs.rawmapname));
    cgs.media.commandCentreAutomapShader[0] = trap_R_RegisterShaderNoMip(
  ...
  // "textures/sfx/construction" ); cgs.media.genericConstructionShaderModel =
  // trap_R_RegisterShader( "textures/sfx/construction_model" );
  ...
  */
  ...
  trap_R_RegisterShaderNoMip( "ui/assets2/ingame/bot_action_disarmdynamite" );
  ...
  trap_R_RegisterShaderNoMip( "ui/assets2/ingame/bot_action_grenadelaunch" );
  ...
  trap_R_RegisterShaderNoMip( "ui/assets2/ingame/bot_action_mountedattack" );
  ...
  //----(SA)	okay, new stuff to intialize rather than doing it at level load
  //time (or "give all" time) 			(I'm certainly not against being efficient here,
  //but I'm tired of the rocket launcher effect only registering 			sometimes and
  //want it to work for sure for this demo)
  ...
  cgs.media.wakeMarkShaderAnim =
      trap_R_RegisterShaderNoMip("wakeAnim");  // (SA)
  ...
  // TAT 12/23/2002 - as a safety check, let's not let the number of models
  // exceed MAX_MODELS
  if (cgs.numInlineModels > MAX_MODELS) {
    CG_Error("CG_RegisterGraphics: Too many inline models: %i\n",
             cgs.numInlineModels);
    // CG_Printf( S_COLOR_RED "WARNING: CG_RegisterGraphics: Too many inline
  ...
  for (i = 1; i < MAX_CHARACTERS; i++) {
    ...
    if (!BG_FindCharacter(characterName)) {
      ...
      if (!CG_RegisterCharacter(characterName, cgs.gameCharacters[i])) {
        CG_Error(
            "ERROR: CG_RegisterGraphics: failed to load character file '%s'\n",
            characterName);
      }
    }
  }
  ...
  // cgs.media.limboCursor_off =
  // trap_R_RegisterShaderNoMip( "gfx/limbo/et_cursor_off"	);
...
void CG_QueueMusic(void) {
  ...
  trap_S_StartBackgroundTrack(
      parm, "", -2);  // '-2' for 'queue looping track' (QUEUED_PLAY_LOOPED)
}
...
static qboolean CG_OwnerDrawHandleKey(int ownerDraw,
                                      int flags,
                                      float* special,
                                      int key) {
  return qfalse;
...
static clientInfo_t* CG_InfoFromScoreIndex(int index,
                                           int team,
                                           int* scoreIndex) {
  int i, count;
...
static const char* CG_FeederItemText(float feederID,
                                     int index,
                                     int column,
                                     qhandle_t* handle,
                                     int* numhandles) {
...
void CG_Text_PaintWithCursor(float x,
                             float y,
                             float scale,
                             vec4_t color,
                             const char* text,
...
static int CG_PlayCinematic(const char* name,
                            float x,
                            float y,
                            float w,
                            float h) {
