...
void trap_R_ProjectDecal(qhandle_t hShader,
                         ...
                         int lifeTime,
                         int fadeTime) {
  syscall(CG_R_PROJECTDECAL, hShader, numPoints, points, projection, color,
          lifeTime, fadeTime);
}
...
void trap_S_AddLoopingSound(const vec3_t origin,
                            ...
                            int volume,
                            int soundTime) {
  syscall(CG_S_ADDLOOPINGSOUND, origin, velocity, 1250, sfx, volume,
...
void trap_S_AddRealLoopingSound(const vec3_t origin,
                                ...
                                int volume,
                                int soundTime) {
  syscall(CG_S_ADDREALLOOPINGSOUND, origin, velocity, range, sfx, volume,
          soundTime);
}
...
{ CG_DrawInformation(); return syscall( CG_S_REGISTERSOUND, sample, compressed
);
}*/
...
void trap_S_FadeBackgroundTrack(
    float targetvol,
    int time,
    int num) {  // yes, i know.  fadebackground coming in, fadestreaming going
                // out.  will have to see where functionality leads...
  syscall(
      CG_S_FADESTREAMINGSOUND, PASSFLOAT(targetvol), time,
      num);  // 'num' is '0' if it's music, '1' if it's "all streaming sounds"
}
...
int trap_S_StartStreamingSound(const char* intro,
                               ...
                               int channel,
                               int attenuation) {
  return syscall(CG_S_STARTSTREAMINGSOUND, intro, loop, entnum, channel,
                 attenuation);
}
...
//r, float g, float b, int overdraw ) { %		syscall(
...
void trap_R_AddCoronaToScene(const vec3_t org,
                             ...
                             int id,
                             qboolean visible) {
  syscall(CG_R_ADDCORONATOSCENE, org, PASSFLOAT(r), PASSFLOAT(g), PASSFLOAT(b),
          PASSFLOAT(scale), id, visible);
}
//----(SA)
...
void trap_R_SetFog(int fogvar,
                   ...
                   float b,
                   float density) {
  syscall(CG_R_SETFOG, fogvar, var1, var2, PASSFLOAT(r), PASSFLOAT(g),
...
void trap_R_SetGlobalFog(qboolean restore,
  ...
  syscall(CG_R_SETGLOBALFOG, restore, duration, PASSFLOAT(r), PASSFLOAT(g),
...
void trap_R_DrawRotatedPic(float x,
                           ...
                           qhandle_t hShader,
                           float angle) {
  syscall(CG_R_DRAWROTATEDPIC, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w),
          PASSFLOAT(h), PASSFLOAT(s1), PASSFLOAT(t1), PASSFLOAT(s2),
...
void trap_R_DrawStretchPicGradient(float x,
                                   ...
                                   int gradientType) {
  syscall(CG_R_DRAWSTRETCHPIC_GRADIENT, PASSFLOAT(x), PASSFLOAT(y),
          PASSFLOAT(w), PASSFLOAT(h), PASSFLOAT(s1), PASSFLOAT(t1),
          PASSFLOAT(s2), PASSFLOAT(t2), hShader, gradientColor, gradientType);
}
      ...
      if (curSnapshotNumber < cg.latestSnapshotNum) {
        *snapshotNumber = cg.latestSnapshotNum + 1;
        curSnapshotNumber =
            ...
            2;  // skip one ahead and we're good to go on the next frame
        skiponeget = qtrue;
      } else {
...
void trap_SetUserCmdValue(int stateValue,
                          ...
                          int mpIdentClient) {
  syscall(CG_SETUSERCMDVALUE, stateValue, flags, PASSFLOAT(sensitivityScale),
          mpIdentClient);
}
