...
void CG_DrawPlayerWeaponIcon(rectDef_t* rect,
  ...
  if (cg.predictedPlayerState
          .grenadeTimeLeft) {  // grenades and dynamite set this
...
void CG_DrawCursorhint(rectDef_t* rect) {
  ...
  switch (cg.cursorHintIcon) {
    ...
    case HINT_CHAIR:
      ...
      // only show 'pickupable' if you're not armed, or are armed with a single
      // handed weapon
...
void CG_DrawWeapStability(rectDef_t* rect) {
  ...
  if (cg_drawSpreadScale.integer == 1 &&
    ...
    // cg_drawSpreadScale of '1' means only draw for scoped weapons, '2' means
    // draw all the time (for debugging)
    return;
  }
...
void CG_DrawWeapHeat(rectDef_t* rect, int align) {
  ...
  flags |= 1;   // BAR_LEFT			- this is hardcoded now, but
                // will be decided by the menu script
