...
void CG_BuildSolidList(void) {
  ...
  for (i = 0; i < snap->numEntities; i++) {
    ...
    if (cent->nextState.solid) {
      /*			if(cg_fastSolids.integer) { // Gordon:
         "optimization" (disabling until i fix it) vec3_t vec; float len;
         ...
         ) { VectorAdd( cgs.inlineModelMidpoints[ cent->currentState.modelindex
         ], cent->lerpOrigin, vec ); VectorSubtract( vec,
         cg.predictedPlayerEntity.lerpOrigin, vec ); } else { VectorSubtract(
         cent->lerpOrigin, cg.predictedPlayerEntity.lerpOrigin, vec );
                                      }
                                      if((len = DotProduct( vec, vec )) < (512 *
         512)) { cg_solidEntities[cg_numSolidEntities] = cent;
                                              cg_numSolidEntities++;
                                              continue;
                                      }
                              } else*/
...
====================
*/
static void CG_ClipMoveToEntities(const vec3_t start,
                                  const vec3_t mins,
                                  const vec3_t maxs,
                                  const vec3_t end,
                                  int skipNumber,
...
static void CG_ClipMoveToEntities_FT(const vec3_t start,
                                     const vec3_t mins,
                                     const vec3_t maxs,
                                     const vec3_t end,
                                     int skipNumber,
...
void CG_Trace_World(trace_t* result,
                    const vec3_t start,
                    const vec3_t mins,
                    const vec3_t maxs,
                    const vec3_t end,
...
================
*/
void CG_TraceCapsule(trace_t* result,
                     const vec3_t start,
                     const vec3_t mins,
                     const vec3_t maxs,
                     const vec3_t end,
...
void CG_TraceCapsule_World(trace_t* result,
                           const vec3_t start,
                           const vec3_t mins,
                           const vec3_t maxs,
                           const vec3_t end,
...
int CG_PointContents(const vec3_t point, int passEntityNum) {
  ...
  for (i = 0; i < cg_numSolidEntities; i++) {
    ...
    // Gordon: again, need to use the projected water position to allow for
    // moving entity based water.
...
static void CG_TouchItem(centity_t* cent) {
  ...
  if (!BG_CanItemBeGrabbed(&cent->currentState, &cg.predictedPlayerState,
                           cgs.clientinfo[cg.snap->ps.clientNum].skill,
  ...
  if (cgs.gamestate != GS_PLAYING && item->giType != IT_WEAPON &&
      item->giType != IT_AMMO && item->giType != IT_HEALTH) {
    return;
  }
  ...
  BG_AddPredictableEventToPlayerstate(
      EV_ITEM_PICKUP, cent->currentState.modelindex, &cg.predictedPlayerState);
...
static void CG_TouchTriggerPrediction(void) {
  ...
  spectator = ((cg.predictedPlayerState.pm_type == PM_SPECTATOR) ||
               (cg.predictedPlayerState.pm_flags & PMF_LIMBO));  // JPW NERVE
...
void CG_PredictPlayerState(void) {
  ...
  if (cg_pmove.ps->pm_type == PM_DEAD) {
    ...
    cg_pmove.ps->eFlags |=
        EF_DEAD;  // DHM-Nerve added:: EF_DEAD is checked for in Pmove
                  // functions, but wasn't being set until after Pmove
  ...
  for (cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++) {
    ...
    //		memcpy( &pmext, &cg.pmext, sizeof(pmoveExt_t) );	// grab
    //data, we only want the final result
  ...
  if (cg.time > cg.cameraShakeTime) {
    ...
    cg.predictedPlayerState.origin[2] +=
        sin(M_PI * 8 * 13 + cg.cameraShakePhase) * x * 6.0f *
        cg.cameraShakeScale;

    cg.predictedPlayerState.origin[1] +=
        sin(M_PI * 17 * x + cg.cameraShakePhase) * x * 6.0f *
        cg.cameraShakeScale;

    cg.predictedPlayerState.origin[0] +=
        cos(M_PI * 7 * x + cg.cameraShakePhase) * x * 6.0f *
        cg.cameraShakeScale;
  }
}
