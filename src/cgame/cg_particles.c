...
void CG_AddParticleToScene(cparticle_t* p, vec3_t org, float alpha) {
  ...
  if (p->type == P_WEATHER || p->type == P_WEATHER_TURBULENT ||
    ...
    if (p->type != P_WEATHER_FLURRY) {
      if (p->type == P_BUBBLE || p->type == P_BUBBLE_TURBULENT) {
        if (org[2] > p->end) {
          p->time = cg.time;
          VectorCopy(org, p->org);  // Ridah, fixes rare snow flakes that
  ...
  } else if (p->type == P_SMOKE ||
             p->type ==
                 P_SMOKE_IMPACT) {  // create a front rotating facing polygon

    if (p->type == P_SMOKE_IMPACT &&
        VectorDistanceSquared(cg.snap->ps.origin, org) > SQR(1024)) {
      return;
    }
  ...
  // Ridah
    ...
    if (p->type == P_DLIGHT_ANIM) {
      ...
      trap_R_AddLightToScene(org,
                             320,  //%	1.5 * (width > height ? width : height),
  ...
  if (!p->pshader) {
    // (SA) temp commented out for DM again.  FIXME: TODO: this needs to be
    // addressed
...
void CG_AddParticles(void) {
  ...
  for (p = active_particles; p; p = next) {
    ...
    if (p->type == P_SMOKE || p->type == P_ANIM || p->type == P_DLIGHT_ANIM ||
        p->type == P_BLEED || p->type == P_SMOKE_IMPACT) {
...
void CG_ParticleBubble(qhandle_t pshader,
                       vec3_t origin,
                       vec3_t origin2,
                       int turb,
                       float range,
...
void CG_ParticleImpactSmokePuffExtended(qhandle_t pshader,
                                        vec3_t origin,
                                        int lifetime,
                                        int vel,
                                        int acc,
  ...
  // xkan, 1/10/2003 - changed calculation to prevent division by 0 for small
  // size
  p->width = size * (1.0 + random() * 0.5);  // rand()%(int)(size * .5f) + size;
  p->height =
      size * (1.0 + random() * 0.5);  // rand()%(int)(size * .5f) + size;
...
void CG_Particle_OilParticle(qhandle_t pshader,
                             ...
                             vec3_t dir,
                             int ptime,
                             int snum) {  // snum is parent ent number?
  cparticle_t* p;
