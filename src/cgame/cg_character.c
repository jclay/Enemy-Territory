...
static void CG_CalcMoveSpeeds(bg_character_t* character) {
  ...
  for (i = 0; i < character->animModelInfo->numAnimations; i++) {
    ...
    for (j = 0; j < anim->numFrames; j++) {
      ...
      refent.torsoFrameModel = refent.oldTorsoFrameModel = refent.frameModel =
          refent.oldframeModel = anim->mdxFile;

      // for each foot
      for (k = 0; k < 2; k++) {
        if (trap_R_LerpTag(&o[k], &refent, tags[k], 0) < 0) {
          CG_Error(
              "CG_CalcMoveSpeeds: unable to find tag %s, cannot calculate "
              "movespeed",
              tags[k]);
        }
      }
...
static qboolean CG_ParseAnimationFiles(bg_character_t* character,
  ...
  // set the name of the animationGroup and animationScript in the animModelInfo
  // structure
  ...
  // parse the text
  BG_AnimParseAnimScript(character->animModelInfo, &cgs.animScriptData,
                         animationScript, bigTextBuffer);
...
CG_CheckForExistingAnimModelInfo
  ...
  If this player model has already been parsed, then use the existing
information. Otherwise, set the modelInfo pointer to the first free slot.
...
static qboolean CG_CheckForExistingAnimModelInfo(
  ...
  for (i = 0, trav = cgs.animScriptData.modelInfo; i < MAX_ANIMSCRIPT_MODELS;
       i++, trav++) {
    if (*trav->animationGroup && *trav->animationScript) {
      if (!Q_stricmp(trav->animationGroup, animationGroup) &&
  ...
  // qfalse signifies that we need to parse the information from the script
  // files
  return qfalse;
}
...
==============
*/
static qboolean CG_RegisterAcc(const char* modelName,
                               int* model,
                               const char* skinname,
                               qhandle_t* skin) {
  char filename[MAX_QPATH];
...
static acc_t cg_accessories[] = {
    {"md3_beltr", ACC_BELT_LEFT}, {"md3_beltl", ACC_BELT_RIGHT},
    {"md3_belt", ACC_BELT},       {"md3_back", ACC_BACK},
    {"md3_weapon", ACC_WEAPON},   {"md3_weapon2", ACC_WEAPON2},
};

static int cg_numAccessories =
    sizeof(cg_accessories) / sizeof(cg_accessories[0]);
...
static int cg_numHeadAccessories =
    sizeof(cg_headAccessories) / sizeof(cg_headAccessories[0]);
...
qboolean CG_RegisterCharacter(const char* characterFile,
  ...
  if (!(character->mesh = trap_R_RegisterModel(characterDef.mesh))) {
    CG_Printf(S_COLOR_YELLOW
              "WARNING: failed to register mesh '%s' referenced from '%s'\n",
  ...
  if (!(character->skin = trap_R_RegisterSkin(filename))) {
    CG_Printf(S_COLOR_YELLOW
              "WARNING: failed to register skin '%s' referenced from '%s'\n",
              filename, characterFile);
  } else {
    for (i = 0; i < cg_numAccessories; i++) {
      if (trap_R_GetSkinModel(character->skin, cg_accessories[i].type,
                              accessoryname)) {
        if (!CG_RegisterAcc(accessoryname,
                            &character->accModels[cg_accessories[i].index],
                            characterDef.skin,
                            &character->accSkins[cg_accessories[i].index])) {
          CG_Printf(S_COLOR_YELLOW
                    "WARNING: failed to register accessory '%s' referenced "
                    "from '%s'->'%s'\n",
    ...
    for (i = 0; i < cg_numHeadAccessories; i++) {
      if (trap_R_GetSkinModel(character->skin, cg_headAccessories[i].type,
                              accessoryname)) {
        if (!CG_RegisterAcc(
                accessoryname,
                &character->accModels[cg_headAccessories[i].index],
                characterDef.skin,
  ...
  if (*characterDef.undressedCorpseModel) {
    ...
    if (!(character->undressedCorpseModel =
              trap_R_RegisterModel(characterDef.undressedCorpseModel))) {
      CG_Printf(S_COLOR_YELLOW
                "WARNING: failed to register undressed corpse model '%s' "
                "referenced from '%s'\n",
    ...
    if (!(character->undressedCorpseSkin = trap_R_RegisterSkin(filename))) {
      CG_Printf(S_COLOR_YELLOW
                "WARNING: failed to register undressed corpse skin '%s' "
                "referenced from '%s'\n",
                filename, characterFile);
    }
  }
  ...
  if (*characterDef.hudhead) {
    ...
    if (!(character->hudhead = trap_R_RegisterModel(characterDef.hudhead))) {
      CG_Printf(S_COLOR_YELLOW
                "WARNING: failed to register hud head model '%s' referenced "
                "from '%s'\n",
                characterDef.hudhead, characterFile);
    }

    if (*characterDef.hudheadskin &&
        !(character->hudheadskin =
              trap_R_RegisterSkin(characterDef.hudheadskin))) {
      CG_Printf(S_COLOR_YELLOW
                "WARNING: failed to register hud head skin '%s' referenced "
                "from '%s'\n",
                characterDef.hudheadskin, characterFile);
    }

    if (*characterDef.hudheadanims) {
      if (!CG_ParseHudHeadConfig(characterDef.hudheadanims,
                                 character->hudheadanimations)) {
        CG_Printf(S_COLOR_YELLOW
                  "WARNING: failed to register hud head animations '%s' "
                  "referenced from '%s'\n",
                  characterDef.hudheadanims, characterFile);
      }
    } else {
      CG_Printf(S_COLOR_YELLOW
                "WARNING: no hud head animations supplied in '%s'\n",
                characterFile);
    }
  }

  // Parse Animation Files
  if (!CG_CheckForExistingAnimModelInfo(characterDef.animationGroup,
                                        characterDef.animationScript,
                                        &character->animModelInfo)) {
    if (!CG_ParseAnimationFiles(character, characterDef.animationGroup,
                                characterDef.animationScript)) {
      CG_Printf(
          S_COLOR_YELLOW
          "WARNING: failed to load animation files referenced from '%s'\n",
          characterFile);
      return qfalse;
    }
  }
  ...
  if (cent && cent->currentState.eType == ET_CORPSE) {
    if (cent->currentState.onFireStart >= 0) {
      ...
      if (cent->currentState.modelindex < 4) {
        return BG_GetCharacter(cent->currentState.modelindex,
                               cent->currentState.modelindex2);
      } else {
        return BG_GetCharacter(cent->currentState.modelindex - 4,
                               cent->currentState.modelindex2);
      }
    }
  }
...
bg_character_t* CG_CharacterForPlayerstate(playerState_t* ps) {
  ...
  return BG_GetCharacter(cgs.clientinfo[ps->clientNum].team,
                         cgs.clientinfo[ps->clientNum].cls);
}
...
void CG_RegisterPlayerClasses(void) {
  ...
  for (team = TEAM_AXIS; team <= TEAM_ALLIES; team++) {
    for (cls = PC_SOLDIER; cls < NUM_PLAYER_CLASSES; cls++) {
      ...
      Q_strncpyz(character->characterFile, classInfo->characterFile,
                 sizeof(character->characterFile));

      if (!CG_RegisterCharacter(character->characterFile, character)) {
        CG_Error(
            "ERROR: CG_RegisterPlayerClasses: failed to load character file "
            "'%s' for the %s %s\n",
      ...
      if (!(classInfo->icon =
                trap_R_RegisterShaderNoMip(classInfo->iconName))) {
        CG_Printf(S_COLOR_YELLOW
                  "WARNING: failed to load class icon '%s' for the %s %s\n",
                  classInfo->iconName, (team == TEAM_AXIS ? "Axis" : "Allied"),
                  BG_ClassnameForNumber(classInfo->classNum));
      }

      if (!(classInfo->arrow =
                trap_R_RegisterShaderNoMip(classInfo->iconArrow))) {
        CG_Printf(S_COLOR_YELLOW
                  "WARNING: failed to load icon arrow '%s' for the %s %s\n",
