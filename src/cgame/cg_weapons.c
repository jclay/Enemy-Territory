...
static void CG_StartWeaponAnim(int anim) {
  ...
  cg.predictedPlayerState.weapAnim =
      ((cg.predictedPlayerState.weapAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) |
      anim;
}
...
void CG_MachineGunEjectBrassNew(centity_t* cent) {
  ...
  if (CG_PointContents(re->origin, -1) &
      (CONTENTS_WATER |
       CONTENTS_SLIME)) {  //----(SA)	modified since slime is no longer deadly
  ...
  le->angles.trBase[0] =
      (rand() & 31) +
      60;  // bullets should come out horizontal not vertical JPW NERVE
...
void CG_MachineGunEjectBrass(centity_t* cent) {
  ...
  // JPW NERVE new brass handling behavior because the SP stuff just doesn't cut
  // it for MP
  if (cent->currentState.eFlags & EF_MG42_ACTIVE ||
    ...
    offset[1] = -4;
    offset[2] = 28;
    velocity[0] = -20 + 40 * crandom();   // JPW NERVE -- more reasonable brass
                                          // ballistics for a machinegun
    velocity[1] = -150 + 40 * crandom();  // JPW NERVE
    velocity[2] = 100 + 50 * crandom();   // JPW NERVE
    re->hModel = cgs.media.machinegunBrassModel;
    le->angles.trBase[0] = 90;  // rand()&31; // JPW NERVE belt-fed rounds
                                // should come out horizontal
    ...
    le->angles.trBase[0] =
        (rand() & 15) +
        82;  // bullets should come out horizontal not vertical JPW NERVE
    le->angles.trBase[1] = rand() & 255;  // random spin from extractor
    le->angles.trBase[2] = rand() & 31;
    le->angles.trDelta[0] = 2;
    le->angles.trDelta[1] = 1;
...
static void CG_PanzerFaustEjectBrass(centity_t* cent) {
  ...
  le->endTime = le->startTime + (cg_brassTime.integer * 8) +
  ...
  le->angles.trBase[1] =
      cent->currentState.apos.trBase[1];  // rotate to match the player
  ...
  le->leFlags = LEF_TUMBLE |
                LEF_SMOKING;  // (SA) probably doesn't need to be 'tumble' since
                              // it doesn't really rotate much when flying
...
void CG_PyroSmokeTrail(centity_t* ent, const weaponInfo_t* wi) {
  ...
  if (ent->currentState.weapon == WP_LANDMINE) {
    if (ent->currentState.teamNum < 8) {
      ...
      if (!ent->miscTime) {
        ...
        // Arnout: play the armed sound - weird place to do it but saves us
        // sending an event
        trap_S_StartSound(NULL, ent->currentState.number, CHAN_WEAPON,
  ...
  BG_EvaluateTrajectory(&es->pos, ent->trailTime, lastPos, qfalse,
                        es->effect2Time);
  lastContents = CG_PointContents(lastPos, -1);

  ent->trailTime = cg.time;

  /* smoke pyro works fine in water (well, it's dye in real life, might wanna
     change this in-game) if ( contents & ( CONTENTS_WATER | CONTENTS_SLIME |
     CONTENTS_LAVA ) ) return;
  */
  ...
  for (; t <= ent->trailTime; t += step) {
    ...
    //			CG_ParticleExplosion( "expblue", lastPos, vec3_origin,
    //100 + (int)(rnd*400), 4, 4 );	// fire "flare"

    // use the optimized local entity add
    //		le->leType = LE_SCALE_FADE;
    /* this one works
                    if (rand()%4)
                            CG_ParticleExplosion( "blacksmokeanim", origin, dir,
       2800+(int)(random()*1500), 15, 45+(int)(rnd*90) );	// smoke
       blacksmokeanim else CG_ParticleExplosion( "expblue", lastPos,
       vec3_origin, 100 + (int)(rnd*400), 4, 4 );	// fire "flare"
    */
  }
}
// jpw
...
void CG_RocketTrail(centity_t* ent, const weaponInfo_t* wi) {
  ...
  if ((ent->currentState.eType != ET_RAMJET) &&
    ...
    if (ent->currentState.eType == ET_FLAMEBARREL) {
      if ((rand() % 100) > 50) {
        CG_ParticleExplosion("twiltb2", lastPos, vec3_origin,
                             100 + (int)(rnd * 400), 5, 7 + (int)(rnd * 10),
                             qfalse);  // fire
      }
      CG_ParticleExplosion("blacksmokeanim", lastPos, vec3_origin,
                           800 + (int)(rnd * 1500), 5, 12 + (int)(rnd * 30),
                           qfalse);  // smoke
    } else if (ent->currentState.eType == ET_FP_PARTS) {
      if ((rand() % 100) > 50) {
        CG_ParticleExplosion("twiltb2", lastPos, vec3_origin,
      ...
      duration = 100;
      CG_ParticleExplosion("twiltb2", lastPos, vec3_origin,
                           duration + (int)(rnd * 100), 5, 5 + (int)(rnd * 10),
                           qfalse);  // fire
      CG_ParticleExplosion("blacksmokeanim", lastPos, vec3_origin,
                           400 + (int)(rnd * 750), 12, 24 + (int)(rnd * 30),
                           qfalse);  // smoke
    } else if (ent->currentState.eType == ET_FIRE_COLUMN ||
               ent->currentState.eType == ET_FIRE_COLUMN_SMOKE) {
      int duration;
      int sizeStart;
      int sizeEnd;
      ...
      CG_ParticleExplosion("twiltb2", lastPos, vec3_origin,
                           duration + (int)(rnd * 400), sizeStart,
                           sizeEnd + (int)(rnd * 10), qfalse);  // fire

      if (ent->currentState.eType == ET_FIRE_COLUMN_SMOKE &&
          (rand() % 100) > 50) {
        CG_ParticleExplosion("blacksmokeanim", lastPos, vec3_origin,
                             800 + (int)(rnd * 1500), 5, 12 + (int)(rnd * 30),
                             qfalse);  // smoke
      ...
      // CG_ParticleExplosion( "twiltb", lastPos, vec3_origin,
      // 300+(int)(rnd*100), 4, 14+(int)(rnd*8) );	// fire
      CG_ParticleExplosion("blacksmokeanim", lastPos, vec3_origin,
                           800 + (int)(rnd * 1500), 5, 12 + (int)(rnd * 30),
                           qfalse);  // smoke
    }
...
static void CG_DynamiteTrail(centity_t* ent, const weaponInfo_t* wi) {
  ...
  if (ent->currentState.teamNum < 4) {
    ...
    //%	trap_R_AddLightToScene( origin, 10 + 300 * mult, 1.f, 1.f, 0,
    //REF_FORCE_DLIGHT);
...
v[2][2]; VectorAdd( cent->lerpOrigin, xoffset, origin );

        VectorSet( up, 0, 0, 64 );

        smoke = CG_SmokePuff( origin, up, 32, 1, 1, 1, 0.33f, 700, cg.time, 0,
...
void CG_RailTrail2(clientInfo_t* ci, vec3_t start, vec3_t end) {
  ...
  //	// still allow different colors so we can tell AI shots from player
  //shots, etc.
  /*	if(ci) {
...
==============
*/
void CG_RailTrail(clientInfo_t* ci,
                  vec3_t start,
                  vec3_t end,
                  int type) {  //----(SA)	added 'type'
    ...
    if (newfmt) {
      ...
      if (atoi(token)) {
        wi->weapAnimations[i].moveSpeed |=
            (1 << W_MAX_PARTS);  // set the bit one higher than can be set by
                                 // the barrel bits
      }
      token = COM_Parse(
          &text_p);  // barrel hide bits (so objects can be flagged to not be
                     // drawn during all sequences (a reloading hand that comes
                     // in from off screen for that one animation for example)
      if (!token) {
        break;
      }
      wi->weapAnimations[i].moveSpeed |=
...
static qboolean CG_RW_ParseWeaponLinkPart(int handle,
                                          weaponInfo_t* weaponInfo,
                                          modelViewType_t viewType) {
  pc_token_t token;
  ...
  while (1) {
    ...
    if (!Q_stricmp(token.string, "tag")) {
      if (!PC_String_ParseNoAlloc(handle, partModel->tagName,
                                  sizeof(partModel->tagName))) {
...
static qboolean CG_RW_ParseWeaponLink(int handle,
                                      weaponInfo_t* weaponInfo,
                                      modelViewType_t viewType) {
  pc_token_t token;
...
static qboolean CG_RW_ParseViewType(int handle,
                                    weaponInfo_t* weaponInfo,
                                    modelViewType_t viewType) {
  pc_token_t token;
  char filename[MAX_QPATH];
  ...
  while (1) {
    ...
    if (!Q_stricmp(token.string, "model")) {
      if (!PC_String_ParseNoAlloc(handle, filename, sizeof(filename))) {
        ...
        weaponInfo->weaponModel[viewType].model =
      ...
      if (!PC_String_ParseNoAlloc(handle, filename, sizeof(filename))) {
        ...
        weaponInfo->weaponModel[viewType].skin[0] =
      ...
      if (!PC_String_ParseNoAlloc(handle, filename, sizeof(filename))) {
        ...
        weaponInfo->weaponModel[viewType].skin[TEAM_AXIS] =
            trap_R_RegisterSkin(filename);
      }
    } else if (!Q_stricmp(token.string, "alliedSkin")) {
      if (!PC_String_ParseNoAlloc(handle, filename, sizeof(filename))) {
        ...
        weaponInfo->weaponModel[viewType].skin[TEAM_ALLIES] =
            trap_R_RegisterSkin(filename);
      }
    ...
    if (!Q_stricmp(token.string, "standModel")) {
      ...
      if (!PC_String_ParseNoAlloc(handle, filename, sizeof(filename))) {
        ...
        if (!CG_ParseWeaponConfig(filename, weaponInfo)) {
          //					CG_Error( "Couldn't register
          //weapon %i (failed to parse %s)", weaponNum, filename );
        }
      }
      ...
      /*} else if( !Q_stricmp( token.string, "ammoIcon" ) ) {
         ...
         else { weaponInfo->ammoIcon = trap_R_RegisterShader( filename );
              }*/
...
static qboolean CG_RegisterWeaponFromWeaponFile(const char* filename,
  ...
  if (!trap_PC_ReadToken(handle, &token) ||
      Q_stricmp(token.string, "weaponDef")) {
  ...
  switch (weaponNum) {
    ...
    default:
      CG_Printf(S_COLOR_RED
                "WARNING: trying to register weapon %i but there is no weapon "
                "file entry for it.\n",
                weaponNum);
      return;
  }

  if (!CG_RegisterWeaponFromWeaponFile(va("weapons/%s", filename),
                                       weaponInfo)) {
    CG_Printf(S_COLOR_RED
              "WARNING: failed to register media for weapon %i from %s\n",
              weaponNum, filename);
  }
}
...
void CG_RegisterItemVisuals(int itemNum) {
  ...
  if (item->icon) {
    ...
    if (item->giType == IT_HOLDABLE) {
      // (SA) register alternate icons (since holdables can have multiple uses,
      // they might have different icons to represent how many uses are left)
      for (i = 1; i < MAX_ITEM_ICONS; i++)
        itemInfo->icons[i] =
            trap_R_RegisterShader(va("%s%i", item->icon, i + 1));
    }
  }

  itemInfo->registered =
      qtrue;  //----(SA)	moved this down after the registerweapon()
}
...
==============
*/
qboolean CG_GetPartFramesFromWeap(centity_t* cent,
                                  refEntity_t* part,
                                  refEntity_t* parent,
                                  int partid,
                                  weaponInfo_t* wi) {
  ...
  for (i = 0; i < (cent->pe.weap.animationNumber & ~ANIM_TOGGLEBIT); i++) {
    if (wi->weapAnimations[i].moveSpeed &
        (1 << partid)) {  // this part has animation for this sequence
...
===============
*/
static void CG_SetWeapLerpFrameAnimation(weaponInfo_t* wi,
                                         lerpFrame_t* lf,
                                         int newAnimation) {
  animation_t* anim;
...
===============
*/
void CG_ClearWeapLerpFrame(weaponInfo_t* wi,
                           lerpFrame_t* lf,
                           int animationNumber) {
...
===============
*/
static void CG_RunWeapLerpFrame(clientInfo_t* ci,
                                weaponInfo_t* wi,
                                lerpFrame_t* lf,
                                int newAnimation,
                                float speedScale) {
  ...
  if (!lf->animation) {
    ...
    if ((newAnimation & ~ANIM_TOGGLEBIT) ==
        PM_RaiseAnimForWeapon(cg.snap->ps.nextWeapon)) {
      CG_ClearWeapLerpFrame(
          wi, lf, newAnimation);  // clear when switching to raise (since it
                                  // should be out of view anyway)
    } else {
...
//----(SA)	modified.  this is now client-side only (server does not dictate
//weapon animation info)
static void CG_WeaponAnimation(playerState_t* ps,
                               weaponInfo_t* weapon,
                               int* weapOld,
                               int* weap,
                               float* weapBackLerp) {
...
static void CG_CalculateWeaponPosition(vec3_t origin, vec3_t angles) {
  ...
  if (cg.predictedPlayerState.eFlags & EF_PRONE_MOVING) {
    ...
    if (pronemovingtime > 0) {  // div by 0
      float factor =
          pronemovingtime > 200 ? 1.f : 1.f / (200.f / (float)pronemovingtime);
    ...
    if (pronenomovingtime < 200) {
      float factor = pronenomovingtime == 0
                         ? 1.f
                         : 1.f - (1.f / (200.f / (float)pronenomovingtime));
      VectorMA(origin, factor * -20, cg.refdef_current->viewaxis[0], origin);
      VectorMA(origin, factor * 3, cg.refdef_current->viewaxis[1], origin);
    }
  }
  ...
  if (cg.predictedPlayerState.leanf != 0) {
    ...
    // this gives you some impression that the weapon stays in relatively the
    // same position while you lean, so you appear to 'peek' over the weapon
...
void CG_AddPlayerWeapon(refEntity_t* parent,
  ...
  if (weaponNum == WP_FG42SCOPE || weaponNum == WP_GARAND_SCOPE ||
  ...
  if (cent->currentState.eFlags & EF_MOUNTEDTANK) {
    ...
    if (cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME) {
      ...
      VectorCopy(cg_entities[cg_entities[cent->currentState.number].tagParent]
                     .mountedMG42Flash.origin,
                 flash.origin);
      AxisCopy(cg_entities[cg_entities[cent->currentState.number].tagParent]
                   .mountedMG42Flash.axis,
               flash.axis);
  ...
  if ((!ps || cg.renderingThirdPerson) &&
      cent->currentState.eFlags & EF_PRONE_MOVING) {
    return;
  }
    ...
    if (isPlayer) {
      akimboFire = BG_AkimboFireSequence(
          ...
          cg.predictedPlayerState
              .ammoclip[BG_FindClipForWeapon(BG_AkimboSidearm(weaponNum))]);
    } else if (ps) {
      akimboFire = BG_AkimboFireSequence(
          weaponNum, ps->ammoclip[BG_FindClipForWeapon(weaponNum)],
          ps->ammoclip[BG_FindClipForWeapon(BG_AkimboSidearm(weaponNum))]);
    }
    // Gordon: FIXME: alternate for other clients, store flip-flop on cent or
    // smuffin
  }
  ...
  if (!ps) {
    ...
    if ((cent->currentState.eFlags & EF_FIRING) && weapon->firingSound) {
      ...
      // down
      trap_S_AddLoopingSound(cent->lerpOrigin, vec3_origin, weapon->firingSound,
                             255, 0);
  ...
  /*	playerScaled = (qboolean)(cgs.clientinfo[ cent->currentState.clientNum
     ].playermodelScale[0] != 0); if(!ps && playerScaled) {	// don't
     "un-scale" weap up in 1st person for(i=0;i<3;i++) {	// scale weapon
     back up so it doesn't pick up the adjusted scale of the character models.
                                                          // this will affect
     any parts attached to the gun as well (barrel/bolt/flash/brass/etc.)
                          VectorScale( gun.axis[i], 1.0/(cgs.clientinfo[
     cent->currentState.clientNum ].playermodelScale[i]), gun.axis[i]);
                  }

          }*/

  if (ps) {
    drawpart = CG_GetPartFramesFromWeap(
        cent, &gun, parent, W_MAX_PARTS,
        weapon);  // W_MAX_PARTS specifies this as the primary view model
  } else {
    drawpart = qtrue;
  }
  ...
  if (drawpart) {
    if (weaponNum == WP_AMMO) {
      if (ps) {
        ...
        if (cgs.clientinfo[cent->currentState.clientNum].skill[SK_SIGNALS] >=
            1) {
          gun.customShader = weapon->modModels[0];
        }
      }
    ...
    if (!ps) {
      if (weaponNum == WP_MEDIC_SYRINGE) {
        if (cgs.clientinfo[cent->currentState.clientNum].skill[SK_FIRST_AID] >=
            3) {
          gun.customShader = weapon->modModels[0];
        }
      }
  ...
  if ((!ps || cg.renderingThirdPerson) &&
      (weaponNum == WP_AKIMBO_COLT || weaponNum == WP_AKIMBO_SILENCEDCOLT ||
  ...
  //%		trap_R_AddLightToScene( gun.origin, 512, 1.5, 1.0, 1.0, 1.0, 0,
  //0 );
  ...
  // if a barrel should be attached for third person, add it in the (!ps)
  // section below
      ...
      if (weaponNum == WP_MORTAR_SET) {
        if (i == W_PART_3) {
          if (ps && !cg.renderingThirdPerson &&
            ...
            angles[ROLL] =
                .8f * AngleNormalize180(cg.pmext.mountedWeaponAngles[YAW] -
                                        ps->viewangles[YAW]);
            spunpart = qtrue;
          }
      ...
      if (barrel.hModel) {
        if (spunpart) {
          CG_PositionRotatedEntityOnTag(
              &barrel, parent, weapon->partModels[W_FP_MODEL][i].tagName);
        } else {
        ...
        if (weaponNum == WP_MORTAR_SET && (i == W_PART_1 || i == W_PART_2)) {
          if (ps && !cg.renderingThirdPerson &&
              cg.predictedPlayerState.weaponstate != WEAPON_RAISING) {
            VectorMA(barrel.origin, .5f * angles[PITCH],
                     cg.refdef_current->viewaxis[0], barrel.origin);
          }
        }

        if (drawpart) {
          if ((ps->persistant[PERS_TEAM] == TEAM_AXIS ||
               (ps->persistant[PERS_TEAM] == TEAM_ALLIES &&
                cent->currentState.powerups & (1 << PW_OPS_DISGUISED))) &&
          ...
          if (weaponNum == WP_SATCHEL_DET && i == W_PART_1) {
            ...
            if (cg.satchelCharge) {
              rangeSquared =
                  DistanceSquared(cg.satchelCharge->lerpOrigin,
            ...
            CG_PositionRotatedEntityOnTag(&satchelDetPart, &barrel,
                                          "tag_needle");
  ...
  if (isPlayer && !cg.renderingThirdPerson) {  // (SA) for now just do it on the
    ...
    if (weaponNum == WP_CARBINE || weaponNum == WP_KAR98 ||
      ...
      if ((cg.snap->ps.ammo[BG_FindAmmoForWeapon(WP_GPG40)] ||
        ...
        if (anim == PM_AltSwitchFromForWeapon(weaponNum) ||
  ...
  // 3rd person attachements
    ...
    if (weaponNum == WP_M7 ||
      ...
      // the grenade - have to always enabled it, no means of telling if another
      // person has a grenade loaded or not atm :/
      // if( cg.snap->ps.weaponstate != WEAPON_FIRING && cg.snap->ps.weaponstate
      // != WEAPON_RELOADING ) {
...
#define BARREL_SMOKE_TIME 1000
    ...
    if (weaponNum == WP_MORTAR_SET) {
      if (ps && !cg.renderingThirdPerson &&
          cg.time - cent->muzzleFlashTime < 800) {
        CG_ParticleImpactSmokePuffExtended(cgs.media.smokeParticleShader,
                                           flash.origin, 700, 16, 20, 30, .12f,
                                           4.f);
      }
    }
  ...
  if (ps || cg.renderingThirdPerson || !isPlayer) {
    if (weaponNum == WP_STEN) {
      if (cg.time - cent->muzzleFlashTime < 100) {
        ...
        //(cgs.media.smokeParticleShader, flash.origin);
        CG_ParticleImpactSmokePuffExtended(cgs.media.smokeParticleShader,
  ...
  if (flash.hModel) {
    if (weaponNum != WP_FLAMETHROWER) {  // Ridah, hide the flash also for now
      // RF, changed this so the muzzle flash stays onscreen for long enough to
      // be seen
      if (cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME) {
        // if (firing) {	// Ridah
        trap_R_AddRefEntityToScene(&flash);
...
void CG_AddViewWeapon(playerState_t* ps) {
  ...
  if (ps->eFlags & EF_MOUNTEDTANK) {
    ...
    if (cg_entities[cg_entities[cg_entities[ps->clientNum].tagParent]
                        .tankparent]
            .currentState.density &
        8) {  // should we use a browning?
    ...
    gunoff[0] = 20;
    if (cg.time - cg.predictedPlayerEntity.muzzleFlashTime <
        MUZZLE_FLASH_TIME) {
      gunoff[0] += random() * 2.f;
    }
    VectorMA(hand.origin, gunoff[0], cg.refdef_current->viewaxis[0],
             hand.origin);
    VectorMA(hand.origin, -10, cg.refdef_current->viewaxis[1], hand.origin);
    VectorMA(hand.origin, (-8 + fovOffset), cg.refdef_current->viewaxis[2],
             hand.origin);

    CG_AddWeaponWithPowerups(&hand,
                             cg.predictedPlayerEntity.currentState.powerups, ps,
    ...
    if (cg.time - cg.predictedPlayerEntity.overheatTime < 3000) {
      if (!(rand() % 3)) {
        float alpha;
        alpha =
            1.0f - ((float)(cg.time - cg.predictedPlayerEntity.overheatTime) /
                    3000.0f);
        alpha *= 0.25f;  // .25 max alpha
        CG_ParticleImpactSmokePuffExtended(cgs.media.smokeParticleShader,
                                           cg.tankflashorg, 1000, 8, 20, 30,
                                           alpha, 8.f);
      }
    }
  ...
  if (ps->weapon > WP_NONE) {
    ...
    VectorMA(hand.origin, gunoff[0], cg.refdef_current->viewaxis[0],
             hand.origin);
    VectorMA(hand.origin, gunoff[1], cg.refdef_current->viewaxis[1],
             hand.origin);
    VectorMA(hand.origin, (gunoff[2] + fovOffset),
             cg.refdef_current->viewaxis[2], hand.origin);
    ...
    if (cg_gun_frame.integer) {
      ...
      CG_WeaponAnimation(ps, weapon, &hand.oldframe, &hand.frame,
                         &hand.backlerp);  //----(SA)	changed
    }
...
qboolean CG_WeaponSelectable(int i) {
  ...
  // if holding a melee weapon (chair/shield/etc.) only allow single-handed
  // weapons
...
==============
getNextWeapInBank
        Pass in a bank and cycle and this will return the next valid weapon
higher in the cycle. if the weap passed in is above highest in a cycle
(MAX_WEAPS_IN_BANK), this will safely loop around
==============
*/
static int getNextWeapInBank(int bank, int cycle) {
  ...
  if (weapBanksMultiPlayer[bank][cycle]) {  // return next weapon in bank if
                                            // there is one
...
getPrevWeapInBank
        Pass in a bank and cycle and this will return the next valid weapon
lower in the cycle. if the weap passed in is the lowest in a cycle (0), this
will loop around to the top (MAX_WEAPS_IN_BANK-1) and start down from there
looking for a valid weapon position
==============
*/
...
==============
getNextBankWeap
        Pass in a bank and cycle and this will return the next valid weapon in a
higher bank. sameBankPosition: if there's a weapon in the next bank at the same
cycle, return that	(colt returns thompson for example) rather than the
lowest weapon
==============
*/
...
getPrevBankWeap
        Pass in a bank and cycle and this will return the next valid weapon in a
lower bank. sameBankPosition: if there's a weapon in the prev bank at the same
cycle, return that	(thompson returns colt for example) rather than the
highest weapon
==============
*/
...
getEquivWeapon
        ...
        Passing the weapnum of the mp40 returns the id of the thompson, and
likewise passing the weapnum of the thompson returns the id of the mp40. No
equivalent available will return the weapnum passed in.
==============
*/
int getEquivWeapon(int weapnum) {
  int num = weapnum;
...
void CG_SetSniperZoom(int lastweap, int newweap) {
  ...
  switch (lastweap) {
    case WP_FG42SCOPE:
      //			cg.zoomedScope	= 1;	// TODO: add to
      //zoomTable 			cg.zoomTime		= cg.time;
      break;
    case WP_GARAND_SCOPE:
      //			cg.zoomedScope	= 500;	// TODO: add to
      //zoomTable 			cg.zoomTime		= cg.time;
      break;
    case WP_K43_SCOPE:
      //			cg.zoomedScope	= 500;	// TODO: add to
  ...
  switch (newweap) {
    ...
    case WP_FG42SCOPE:
      cg.zoomval = cg_zoomDefaultSniper
                       .value;  // JPW NERVE changed from defaultFG per atvi req
...
void CG_FinishWeaponChange(int lastweap, int newweap) {
  ...
  switch (newweap) {
    ...
    case WP_GPG40:
      ...
      break;
      //	case WP_MEDIC_SYRINGE:
      //		if((cg.pmext.silencedSideArm & 4) && lastweap !=
      //WP_MEDIC_ADRENALINE) { 			newweap = WP_MEDIC_ADRENALINE; 			cg.weaponSelect =
      //newweap;
      //		}
      //		break;
      //	case WP_MEDIC_ADRENALINE:
      //		if(!(cg.pmext.silencedSideArm & 4) && lastweap !=
      //WP_MEDIC_SYRINGE) { 			newweap = WP_MEDIC_SYRINGE; 			cg.weaponSelect =
      //newweap;
      //		}
      //		break;
  }
...
void CG_AltWeapon_f(void) {
  ...
  // Arnout: don't allow another weapon switch when we're still swapping the
  // gpg40, to prevent animation breaking
  if ((cg.snap->ps.weaponstate == WEAPON_RAISING ||
       cg.snap->ps.weaponstate == WEAPON_DROPPING) &&
      ((original == WP_GPG40 || num == WP_GPG40 || original == WP_M7 ||
        num == WP_M7) ||
       (original == WP_SILENCER || num == WP_SILENCER ||
        original == WP_SILENCED_COLT || num == WP_SILENCED_COLT) ||
       (original == WP_AKIMBO_SILENCEDCOLT || num == WP_AKIMBO_SILENCEDCOLT ||
...
CG_NextWeap
  ...
  switchBanks - curweap is the last in a bank, 'qtrue' means go to the next
available bank, 'qfalse' means loop to the head of the bank
==============
*/
void CG_NextWeap(qboolean switchBanks) {
  ...
  if (curweap >= WP_BEGINSECONDARY && curweap <= WP_LASTSECONDARY) {
    ...
    if (CG_WeaponSelectable(
            num)) {  // the parent was selectable, drop back to that
  ...
  if (cg_cycleAllWeaps.integer || !switchBanks) {
    for (i = 0; i < MAX_WEAPS_IN_BANK_MP; i++) {
      ...
      CG_WeaponIndex(num, NULL,
                     &newcycle);  // get cycle of new weapon.  if it's lower
                                  // than the original, then it cycled around
  ...
  if (nextbank) {
    for (i = 0; i < MAX_WEAP_BANKS_MP; i++) {
      if (cg_cycleAllWeaps.integer) {
        num = getNextBankWeap(bank + i, cycle,
                              qfalse);  // cycling all weaps always starts the
                                        // next bank at the bottom
      } else {
      ...
      if (CG_WeaponSelectable(num)) {  // first entry in bank was selectable, no
                                       // need to scan the bank
        break;
      } else {
        qboolean found = qfalse;
        switch (num) {
...
void CG_PrevWeap(qboolean switchBanks) {
  ...
  if (cg_cycleAllWeaps.integer || !switchBanks) {
    for (i = cycle; i >= 0; i--) {
      ...
      CG_WeaponIndex(num, NULL,
                     &newcycle);  // get cycle of new weapon.  if it's greater
                                  // than the original, then it cycled around
  ...
  if (prevbank) {
    for (i = 0; i < MAX_WEAP_BANKS_MP; i++) {
      if (cg_cycleAllWeaps.integer) {
        num = getPrevBankWeap(bank - i, cycle,
                              qfalse);  // cycling all weaps always starts the
                                        // next bank at the bottom
      } else {
...
void CG_LastWeaponUsed_f(void) {
  ...
  if (cg.weaponSelect == WP_MORTAR_SET ||
...
void CG_NextWeaponInBank_f(void) {
  ...
  // this cvar is an option that lets the player use his weapon switching keys
  // (probably the mousewheel) for zooming (binocs/snooper/sniper/etc.)
  if (cg.zoomval) {
...
void CG_NextWeapon_f(void) {
  ...
  //				present, but would discard the reload.  So the
  //when you switched 				back you'd have to start the reload over.  This seems bad,
  //however 				the delay for the current reload is already in effect, so you'd lose
  //				the reload time twice.  (the first pause for the
  //current weapon reload, 				and the pause when you have to reload again 'cause
  //you canceled this one)
...
void CG_WeaponBank_f(void) {
  ...
  CG_WeaponIndex(curweap, &curbank,
                 &curcycle);  // get bank/cycle of current weapon
  ...
  if ((cg.snap->ps.weaponstate == WEAPON_RAISING ||
       cg.snap->ps.weaponstate == WEAPON_DROPPING) &&
      ((curweap == WP_GPG40 || num == WP_GPG40 || curweap == WP_M7 ||
        num == WP_M7) ||
       (curweap == WP_SILENCER || num == WP_SILENCER ||
        curweap == WP_SILENCED_COLT || num == WP_SILENCED_COLT) ||
...
void CG_Weapon_f(void) {
  ...
  // JPW NERVE
  // weapon bind should execute weaponbank instead -- for splitting out class
  // weapons, per Id request
  ...
  /*	cg.weaponSelectTime = cg.time;	// flash the current weapon icon
          ...
          CG_WeaponIndex(curweap, &bank, &cycle);		// get
     bank/cycle of current weapon banked = CG_WeaponIndex(num, &newbank,
     &newcycle);		// get bank/cycle of requested weapon
          ...
          if(bank != newbank)
                  cycle = newcycle - 1;	//	drop down one from the requested
     weap's cycle so it will
                                                                  //	try to
     initially cycle up to the requested weapon
...
void CG_OutOfAmmoChange(qboolean allowforceswitch) {
  ...
  if (cg.weaponSelect == WP_PLIERS ||
      (cg.weaponSelect == WP_SATCHEL_DET &&
       cg.predictedPlayerState.ammo[WP_SATCHEL_DET])) {
    return;
  }
  ...
  if (allowforceswitch) {
    ...
    if (cg.weaponSelect == WP_PANZERFAUST) {
      ...
      for (i = 0; i < MAX_WEAPS_IN_BANK_MP; i++) {
        if (CG_WeaponSelectable(
                weapBanksMultiPlayer[4][i])) {  // find a grenade
    ...
    if (cg.weaponSelect >= WP_BEGINSECONDARY &&
        cg.weaponSelect <= WP_LASTSECONDARY) {
      cg.weaponSelect = equiv = getAltWeapon(
          cg.weaponSelect);  // base any further changes on the parent
      if (CG_WeaponSelectable(
              equiv)) {  // the parent was selectable, drop back to that
        CG_FinishWeaponChange(cg.predictedPlayerState.weapon,
                              cg.weaponSelect);  //----(SA)
        return;
      }
  ...
  // didn't have available alternative or equivalent, try another weap in the
  // bank
  CG_WeaponIndex(cg.weaponSelect, &bank,
                 &cycle);  // get bank/cycle of current weapon

  // JPW NERVE -- more useful weapon changes -- check if rifle or pistol is
  // still working, and use that if available
  for (i = 0; i < MAX_WEAPS_IN_BANK_MP; i++) {
...
==============
CG_MortarEFX
        Right now mostly copied directly from Raf's MG42 FX, but with the
optional addtion of smoke
==============
*/
void CG_MortarEFX(centity_t* cent) {
  ...
  if (cent->currentState.density & 1) {
    // smoke
    CG_ParticleImpactSmokePuff(cgs.media.smokePuffShader,
                               cent->currentState.origin);
  }

  if (cent->currentState.density & 2) {
    // light
    //%	trap_R_AddLightToScene( cent->currentState.origin, 200 +
    //(rand()&31), 1.0, 1.0, 1.0, 0 );
    trap_R_AddLightToScene(cent->currentState.origin, 256,
                           0.75 + 8.0 / (rand() & 31), 1.0, 1.0, 1.0, 0, 0);
...
void CG_FireWeapon(centity_t* cent) {
  ...
  // Arnout: quick hack for EF_MOUNTEDTANK, need to change this - likely it
  // needs to use viewlocked as well
  if (cent->currentState.eFlags & EF_MOUNTEDTANK) {
    if (cg_entities
  ...
  if (BG_PlayerMounted(cent->currentState.eFlags)) {
    if (cent->currentState.eFlags & EF_AAGUN_ACTIVE) {
      ...
      //cent->currentState.number, CHAN_WEAPON, cgs.media.hflakWeaponSnd );
    } else {
      trap_S_StartSound(NULL, cent->currentState.number, CHAN_WEAPON,
                        cgs.media.hWeaponSnd);
    }
  ...
  if (ent->weapon == WP_FLAMETHROWER) {
    if (cent->pe.lightningFiring) {
      return;
             ...
             ent->weapon == WP_DYNAMITE || ent->weapon == WP_SMOKE_MARKER ||
             ent->weapon == WP_LANDMINE || ent->weapon == WP_SATCHEL ||
  ...
  if (!(cent->currentState.eFlags &
        EF_ZOOMING)) {  // JPW NERVE -- don't play sounds or eject brass if
                        // zoomed in
    // play a sound
    for (c = 0; c < 4; c++) {
      if (!firesound[c]) {
    ...
    if (c > 0) {
      ...
      if (firesound[c]) {
        ...
        if (fireEchosound && fireEchosound[c]) {  // check for echo
          ...
          if (gdist > 512 && gdist < 4096) {  // temp dist.  TODO: use numbers
                                              // that are weapon specific
            // use gorg as the new sound origin
            VectorMA(cg.refdef_current->vieworg, 64, norm,
                     gorg);  // sound-on-a-stick
            trap_S_StartSoundEx(gorg, ent->number, CHAN_WEAPON,
...
=================
*/
void CG_AddSparks(vec3_t origin,
                  vec3_t dir,
                  int speed,
                  int duration,
                  int count,
...
=================
*/
void CG_AddBulletParticles(vec3_t origin,
                           vec3_t dir,
                           int speed,
                           int duration,
                           int count,
  ...
  /*
          ...
          for (i=0; i<count/3; i++) {
                  ...
                  VectorSet( velocity, dir[0] + crandom()*randScale, dir[1] +
  crandom()*randScale, dir[2] + crandom()*randScale ); VectorScale( velocity,
  (float)speed*3, velocity );

                  le->leType = LE_SPARK;
                  le->startTime = cg.time;
                  le->endTime = le->startTime + duration - (int)(0.5 * random()
  * duration); le->lastTrailTime = cg.time;
                  ...
                  VectorCopy( origin, le->pos.trBase );
                  VectorMA( le->pos.trBase, 2 + random()*4, dir, le->pos.trBase
  ); VectorCopy( velocity, le->pos.trDelta ); le->pos.trTime = cg.time;
...
=================
*/
void CG_AddDirtBulletParticles(vec3_t origin,
                               vec3_t dir,
                               int speed,
                               int duration,
                               int count,
  ...
  CG_ParticleDirtBulletDebris_Core(
      pos, velocity, duration, width, height, alpha,
      shader);  // 600 + rand()%300 ); // keep central one
  for (i = 0; i < count; i++) {
    VectorSet(velocity, dir[0] * crandom() * speed * randScale,
              dir[1] * crandom() * speed * randScale,
              dir[2] * random() * speed);
    CG_ParticleDirtBulletDebris_Core(pos, velocity,
                                     duration + (rand() % (duration >> 1)),
                                     width, height, alpha, shader);
  }
}
...
=================
*/
void CG_AddDebris(vec3_t origin,
                  vec3_t dir,
                  int speed,
                  int duration,
                  int count) {
  ...
  for (i = 0; i < count; i++) {
    ...
    VectorSet(
        unitvel, dir[0] + crandom() * 0.9, dir[1] + crandom() * 0.9,
        fabs(dir[2]) > 0.5 ? dir[2] * (0.2 + 0.8 * random()) : random() * 0.6);
    VectorScale(unitvel, (float)speed + (float)speed * 0.5 * crandom(),
                velocity);

    le->leType = LE_DEBRIS;
    le->startTime = cg.time;
    le->endTime =
        le->startTime + duration + (int)((float)duration * 0.8 * crandom());
    le->lastTrailTime = cg.time;
...
==============
*/
void CG_WaterRipple(qhandle_t shader,
                    vec3_t loc,
                    vec3_t dir,
                    int size,
                    int lifetime) {
...
=================
*/
void CG_MissileHitWall(int weapon,
                       int clientNum,
                       vec3_t origin,
                       vec3_t dir,
                       int surfFlags) {  //	(SA) modified to send
  ...
  switch (weapon) {
    case WP_KNIFE:
      i = rand() % 4;
      if (!surfFlags) {
        sfx = cgs.media.sfx_knifehit[4];  // different values for different
                                          // types (stone/metal/wood/etc.)
    ...
    case WP_K43_SCOPE:
      ...
      r = (rand() & 3) + 1;  // JPW NERVE increased spark frequency so players
                             // can tell where rounds are coming from in MP
      ...
      if (!clientNum) {
        // RF, why is this here? we need sparks if clientNum = 0, used for
        // warzombie
        ...
        if ((surfFlags & SURF_GRASS || surfFlags & SURF_GRAVEL ||
             surfFlags & SURF_SNOW)) {  // JPW NERVE added SURF_SNOW

          // some debris particles
          // JPW NERVE added surf_snow
          if (surfFlags & SURF_SNOW) {
            CG_AddDirtBulletParticles(origin, dir, 190, 900, 5, 0.25, 80, 32,
                                      0.5, cgs.media.dirtParticle2Shader);
          } else {
            CG_AddDirtBulletParticles(origin, dir, 190, 900, 5, 0.5, 80, 16,
                                      0.5, cgs.media.dirtParticle1Shader);
          }
        } else {
        ...
        // (SA) needed to do the CG_WaterRipple using a localent since I needed
        // the timer reset on the shader for each shot
      ...
      // (SA) small modification.  only do this for non-rifles (so you can see
      // your shots hitting when you're zooming with a rifle scope)
      if (weapon == WP_FG42SCOPE || weapon == WP_GARAND_SCOPE ||
        ...
        if (clientNum) {
          ...
          alphaFade = qtrue;  // max made the bullet mark alpha (he'll make
                              // everything in the game out of 1024 textures,
                              // all with alpha blend funcs yet...)
...
#define MAX_IMPACT_SOUNDS 5
          if (surfFlags & SURF_METAL || surfFlags & SURF_ROOF) {
            ...
            alphaFade = qtrue;
            radius +=
                0.4f;  // experimenting with different mark sizes per surface
    ...
    case WP_MAPMORTAR:
      ...
      if (CG_PointContents(origin, 0) & CONTENTS_WATER) {
        ...
        CG_AddDirtBulletParticles(trace.endpos, dir, 900, 1800, 15, 0.5, 350,
                                  128, 0.125, cgs.media.dirtParticle2Shader);
      } else {
        VectorCopy(origin, tmpv);
        tmpv[2] += 20;
        ...
        if (trace.surfaceFlags & SURF_GRASS ||
          ...
          CG_AddDirtBulletParticles(origin, dir, 600, 2000, 10, 0.5, 275, 125,
                                    0.25, cgs.media.dirtParticle1Shader);
        }
    ...
    case WP_TRIPMINE:
      ...
      if (CG_PointContents(origin, 0) & CONTENTS_WATER) {
        ...
        for (i = 0; i < 4; i++) {  // JPW random vector based on plane normal so
                                   // explosions move away from walls/dirt/etc
          for (j = 0; j < 3; j++) {
    ...
    case WP_MORTAR_SET:
      ...
      if (CG_PointContents(origin, 0) & CONTENTS_WATER) {
        ...
        CG_AddDirtBulletParticles(trace.endpos, dir, 400, 900, 15, 0.5, 256,
                                  128, 0.125, cgs.media.dirtParticle2Shader);
      } else {
        VectorCopy(origin, tmpv);
        tmpv[2] += 20;
  ...
  if (sfx2) {  // distant sounds for weapons with a broadcast fire sound (so you
               // /always/ hear dynamite explosions)
    vec3_t porg, gorg, norm;  // player/gun origin
    float gdist;
    ...
    if (gdist > 1200 && gdist < 8000) {  // 1200 is max cam shakey dist (2*600)
      ...
      VectorMA(cg.refdef_current->vieworg, sfx2range, norm,
               gorg);  // JPW NERVE non-distance falloff makes more sense;
                       // sfx2range was gdist*0.2
      // sfx2range is variable to give us minimum volume control different
      // explosion sizes (see mortar, panzerfaust, and grenade)
  ...
  if (mark == cgs.media.burnMarkShader) {
    ...
    CG_ImpactMark(mark, markOrigin, projection, radius, random() * 360.0f, 1.0f,
                  1.0f, 1.0f, 1.0f, markDuration);
  }
}
...
==============
*/
void CG_MissileHitWallSmall(int weapon,
                            int clientNum,
                            vec3_t origin,
                            vec3_t dir) {
  qhandle_t mod;
...
=================
*/
void CG_MissileHitPlayer(centity_t* cent,
                         int weapon,
                         vec3_t origin,
                         vec3_t dir,
                         int entityNum) {
  ...
  switch (weapon) {
    ...
    case WP_PANZERFAUST:
      CG_MissileHitWall(weapon, 0, origin, dir,
                        0);  // JPW NERVE like the old one
      break;
    case WP_KNIFE:
      CG_MissileHitWall(
          weapon, 0, origin, dir,
          1);  // JPW NERVE this one makes the hitting fleshy sound. whee
      break;
    default:
      break;
  }
...
void CG_SpawnTracer(int sourceEnt, vec3_t pstart, vec3_t pend) {
  ...
  if (sourceEnt < cgs.maxclients) {
    ...
    if (!(cg_entities[sourceEnt].currentState.eFlags & EF_MG42_ACTIVE ||
          cg_entities[sourceEnt].currentState.eFlags &
              EF_AAGUN_ACTIVE)) {  // not MG42
  ...
  // subtract the length of the tracer from the end point, so we dont go through
  // the end point
...
qboolean CG_CalcMuzzlePoint(int entityNum, vec3_t muzzle) {
  ...
  if (entityNum == cg.snap->ps.clientNum) {
    ...
    if (cg.snap->ps.eFlags & EF_MG42_ACTIVE) {
      ...
      VectorMA(mg42->currentState.pos.trBase, 40, forward,
               muzzle);  // wsa -36, made 40 to be in sync with the actual
                         // muzzleflash drawing
      ...
      VectorCopy(
          aagun->lerpOrigin,
          muzzle);  // Gordon: modelindex2 will already have been incremented on
                    // the server, so work out what it WAS then
      BG_AdjustAAGunMuzzleForBarrel(muzzle, forward, right, up,
                                    (aagun->currentState.modelindex2 + 3) % 4);
  ...
  //----(SA)	removed check.  is this still necessary?  (this way works for
  //ai's firing mg42)  should I check for mg42? 	if ( !cent->currentValid ) {
  //		return qfalse;
  //	}
  //----(SA)	end
...
// ydnar: this is silly--the entity is a mg42 barrel, so just use itself
    ...
    for (num = 0; num < cg.snap->numEntities; num++) {
      ...
      if (aagun->currentState.eType == ET_AAGUN &&
        ...
        VectorCopy(
            aagun->lerpOrigin,
            muzzle);  // Gordon: modelindex2 will already have been incremented
                      // on the server, so work out what it WAS then
...
void CG_Bullet(vec3_t end,
  ...
  // Arnout: snap tracers for MG42 to viewangle of client when antilag is
  // enabled
  if (cgs.antilag && otherEntNum2 == cg.snap->ps.clientNum &&
    ...
    VectorCopy(
        cg_entities[cg.snap->ps.viewlocked_entNum].currentState.pos.trBase,
        muzzle);
  ...
  if (flesh) {
    ...
    // all this to come up with a decent center-body displacement of bullet
    // impact point
    ...
    if (headshot && cg_blood.integer) {
      for (i = 0; i < 5; i++) {
        ...
        le = CG_SmokePuff(origin, tmpv, 5 + rnd * 10, 1, rnd * 0.8, rnd * 0.8,
                          0.5, 500 + (rand() % 800), cg.time, 0, 0,
      ...
      for (i = 0; i < 10; i++) {
        ...
        tmpv2[2] = 0;
        VectorAdd(tmpv, tmpv2, tmpv);
        le = CG_SmokePuff(origin, tmpv, 5 + rnd * 10, rnd * 0.3f + 0.5f,
    ...
    // HACK, if this is not us getting hit, make it quieter // JPW NERVE pulled
    // hack, we like loud impact sounds for MP
    if (fleshEntityNum == cg.snap->ps.clientNum) {
      // CG_SoundPlayIndexedScript( cgs.media.bulletHitFleshScript, NULL,
      // fleshEntityNum );
      trap_S_StartSound(
          NULL, fleshEntityNum, CHAN_BODY,
          cgs.media.sfx_bullet_stonehit[rand() % MAX_IMPACT_SOUNDS]);
    } else {
      // CG_SoundPlayIndexedScript( cgs.media.bulletHitFleshScript,
    ...
    // if we haven't dropped a blood spat in a while, check if this is a good
    // scenario
    if (cg_blood.integer &&
...
//qfalse, cg_bloodTime.integer * 1000 );
    ...
    // Gordon: all bullet weapons have the same fx, and this stops pvs issues
    // causing grenade explosions
    int fromweap =
        WP_MP40;  // cg_entities[sourceEntityNum].currentState.weapon;
    ...
    if (CG_CalcMuzzlePoint(sourceEntityNum, start) ||
      ...
      if (waterfraction) {
        ...
        /*			vec3_t	start2;
                        VectorSubtract( end, start, dir );
                                VectorNormalize( dir );*/
        /*			VectorMA( end, -4, dir, start2 );	// back
           off a little so it doesn't start in solid VectorMA( end, 64, dir, dir
           );*/
        ...
        // CG_MissileHitWall( fromweap, 1, end, normal, trace.surfaceFlags);
        // // smoke puff	//	(SA) modified to send missilehitwall
        // surface parameters

        //%	CG_MissileHitWall( fromweap, 1, trace.endpos,
        //trace.plane.normal, trace.surfaceFlags);	// smoke puff	//
