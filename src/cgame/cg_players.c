...
qboolean CG_EntOnFire(centity_t* cent) {
  if (cent->currentState.number == cg.snap->ps.clientNum) {
    // TAT 11/15/2002 - the player is always starting out on fire, which is
    // easily seen in cinematics
    //		so make sure onFireStart is not 0
    return (cg.snap->ps.onFireStart && (cg.snap->ps.onFireStart < cg.time) &&
...
qboolean CG_IsCrouchingAnim(animModelInfo_t* animModelInfo, int animNum) {
  ...
  //
  if (anim->movetype & ((1 << ANIM_MT_IDLECR) | (1 << ANIM_MT_WALKCR) |
                        (1 << ANIM_MT_WALKCRBK))) {
    return qtrue;
  }
  //
...
void CG_NewClientInfo(int clientNum) {
  ...
  // Gordon: grabbing some older stuff, if it's a new client, tinfo will update
  // within one second anyway, otherwise you get the health thing flashing red
  // NOTE: why are we bothering to do all this setting up of a new clientInfo_t
  // anyway? it was all for deffered clients iirc, which we dont have
  ...
  if (clientNum == cg.clientNum) {
    ...
    if (newInfo.rank > cgs.clientinfo[cg.clientNum].rank) {
      CG_SoundPlaySoundScript(cgs.clientinfo[cg.clientNum].team == TEAM_ALLIES
                                  ? rankSoundNames_Allies[newInfo.rank]
                                  : rankSoundNames_Axis[newInfo.rank],
                              NULL, -1, qtrue);

      CG_AddPMItemBig(PM_RANK,
                      va("Promoted to rank %s!",
                         cgs.clientinfo[cg.clientNum].team == TEAM_AXIS
                             ? rankNames_Axis[newInfo.rank]
                             : rankNames_Allies[newInfo.rank]),
    ...
    for (i = 0; i < SK_NUM_SKILLS; i++) {
      if (newInfo.skill[i] > cgs.clientinfo[cg.clientNum].skill[i]) {
        ...
        CG_AddPMItemBig(PM_SKILL,
                        va("Increased %s skill to level %i!", skillNames[i],
                           newInfo.skill[i]),
                        cgs.media.skillPics[i]);

        CG_PriorityCenterPrint(va("You have been rewarded with %s",
                                  cg_skillRewards[i][newInfo.skill[i] - 1]),
                               SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.20),
                               SMALLCHAR_WIDTH, 99999);
      }
    }
    ...
    if (newInfo.refStatus != ci->refStatus) {
      if (newInfo.refStatus <= RL_NONE) {
        ...
        CG_Printf(
            "[cgnotify]^3*** You have been stripped of your referee status! "
            "***\n");

      } else {
        trap_Cvar_Set("cg_ui_voteFlags", "0");
        CG_Printf(
            "[cgnotify]^2*** You have been authorized \"%s\" status ***\n",
...
bg_playerclass_t* CG_PlayerClassForClientinfo(clientInfo_t* ci,
  ...
  if (cent && cent->currentState.eType == ET_CORPSE) {
    return BG_GetPlayerClassInfo(cent->currentState.modelindex,
...
===============
*/
static void CG_SetLerpFrameAnimation(centity_t* cent,
                                     clientInfo_t* ci,
                                     lerpFrame_t* lf,
                                     int newAnimation) {
  animation_t* anim;
  ...
  if (cg_debugAnim.integer == 1) {
    CG_Printf("Anim: %i, %s\n", newAnimation,
              character->animModelInfo->animations[newAnimation]->name);
  }
}
...
===============
*/
void CG_RunLerpFrame(centity_t* cent,
                     clientInfo_t* ci,
                     lerpFrame_t* lf,
                     int newAnimation,
                     float speedScale) {
...
===============
*/
static void CG_ClearLerpFrame(centity_t* cent,
                              clientInfo_t* ci,
                              lerpFrame_t* lf,
                              int animationNumber) {
...
===============
*/
void CG_SetLerpFrameAnimationRate(centity_t* cent,
                                  clientInfo_t* ci,
                                  lerpFrame_t* lf,
                                  int newAnimation) {
  ...
  if (!(anim->flags & ANIMFL_FIRINGANIM) || (lf != &cent->pe.torso)) {
    if ((lf == &cent->pe.legs) &&
      ...
      if (anim->moveSpeed ||
          (anim->movetype &
           ((1 << ANIM_MT_TURNLEFT) |
            (1
             << ANIM_MT_TURNRIGHT)))) {  // if unknown movetype, go there faster
    ...
    if (oldanim && oldanim->animBlend) {  // transitionMin < lf->frameTime +
                                          // oldanim->animBlend) {
...
===============
*/
void CG_RunLerpFrameRate(clientInfo_t* ci,
                         lerpFrame_t* lf,
                         int newAnimation,
                         centity_t* cent,
                         int recursion) {
...
#define ANIM_SPEEDMAX_HIGH 20
  ...
  if (cent->currentState.eFlags & EF_FORCE_END_FRAME
      // xkan, 12/27/2002 - In SP, corpse also stays at the last frame (of the
      // death animation) so that the death animation can end up in different
      // positions and the body will stay in that position
  ...
  if (anim->moveSpeed && lf->oldFrameSnapshotTime) {
    ...
    if (cg.latestSnapshotTime != lf->oldFrameSnapshotTime && cg.nextSnap) {
      if (cent->currentState.number == cg.snap->ps.clientNum) {
        ...
        moveSpeed = Distance(cent->lerpOrigin, lf->oldFramePos) /
                    ((float)(cg.time - lf->oldFrameTime) / 1000.0);
      } else {
        if (isLadderAnim) {  // only use Z axis for speed
  ...
  if (cg.time >= lf->frameTime) {
    ...
    if (lf->animSpeedScale <
        0.25) {  // if it's too slow, then a really slow spped, combined with a
                 // sudden take-off, can leave them playing a really slow frame
                 // while they a moving really fast
      ...
      if (!(anim->flags & ANIMFL_LADDERANIM)) {
        ...
        if (anim->moveSpeed > ANIM_SPEEDMAX_LOW) {
          ...
          lf->animSpeedScale =
              ANIM_SCALEMAX_HIGH -
              (ANIM_SCALEMAX_HIGH - ANIM_SCALEMAX_LOW) *
                  (float)(anim->moveSpeed - ANIM_SPEEDMAX_HIGH) /
    ...
    if (!lf->animSpeedScale) {
      ...
      lf->frameTime +=
          anim->frameLerp;  // don't wait too long before starting to move again
    } else if (lf->oldAnimationNumber != lf->animationNumber &&
               (!anim->moveSpeed || lf->oldFrame < anim->firstFrame ||
                lf->oldFrame >=
                    anim->firstFrame +
                        anim->numFrames)) {  // Ridah, added this so walking
                                             // frames don't always get reset to
                                             // 0, which can happen in the
                                             // middle of a walking anim, which
                                             // looks wierd
      lf->frameTime = lf->animationTime;     // initial lerp
    ...
    } else if ((lf == &cent->pe.legs) && otherAnim &&
               !(anim->flags & ANIMFL_FIRINGANIM) &&
               ((lf->animationNumber & ~ANIM_TOGGLEBIT) ==
                (cent->pe.torso.animationNumber & ~ANIM_TOGGLEBIT)) &&
               (!anim->moveSpeed)) {
      // legs should synch with torso
      f = cent->pe.torso.frame - otherAnim->firstFrame;
      if (f >= anim->numFrames || f < 0) {
        f = 0;  // wait at the start for the legs to catch up (assuming they are
                // still in an old anim)
      }
      lf->frameTime = cent->pe.torso.frameTime;
    } else if ((lf == &cent->pe.torso) && otherAnim &&
               !(anim->flags & ANIMFL_FIRINGANIM) &&
               ((lf->animationNumber & ~ANIM_TOGGLEBIT) ==
                (cent->pe.legs.animationNumber & ~ANIM_TOGGLEBIT)) &&
               (otherAnim->moveSpeed)) {
      ...
      lf->frameTime = lf->oldFrameTime + (int)((float)anim->frameLerp *
                                               (1.0 / lf->animSpeedScale));
      if (lf->frameTime < cg.time) {
        lf->frameTime = cg.time;
      }

      // check for skipping frames (eg. death anims play in slo-mo if low
      // framerate)
      if (anim->flags & ANIMFL_REVERSED) {
        ...
        if (cg.time > lf->frameTime && !anim->moveSpeed) {
          f = (lf->frame - anim->firstFrame) + 1 +
              (cg.time - lf->frameTime) / anim->frameLerp;
        } else {
          f = (lf->frame - anim->firstFrame) + 1;
        }
      }
    ...
    if (cg.time > lf->frameTime) {
      // Ridah, run the frame again until we move ahead of the current time,
      // fixes walking speeds for zombie
  ...
  // Gordon: BIG hack, occaisionaly (VERY occaisionally), the frametime gets
  // totally wacked
...
===============
*/
void CG_ClearLerpFrameRate(centity_t* cent,
                           clientInfo_t* ci,
                           lerpFrame_t* lf,
                           int animationNumber) {
  lf->frameTime = lf->oldFrameTime = cg.time;
...
static void CG_PlayerAnimation(centity_t* cent, refEntity_t* body) {
  ...
  if (cg_noPlayerAnims.integer) {
    ...
    body->frameModel = body->oldframeModel = body->torsoFrameModel =
        body->oldTorsoFrameModel =
  ...
  if (!(cent->currentState.eFlags & EF_DEAD) && cent->pe.legs.yawing) {
    // CG_Printf("turn: %i\n", cg.time );
    tempIndex = BG_GetAnimScriptAnimation(
        clientNum, character->animModelInfo, cent->currentState.aiState,
        (cent->pe.legs.yawing == SWING_RIGHT ? ANIM_MT_TURNRIGHT
                                             : ANIM_MT_TURNLEFT));
    if (tempIndex > -1) {
      animIndex = tempIndex;
    }
  ...
  CG_RunLerpFrameRate(ci, &cent->pe.torso, cent->currentState.torsoAnim, cent,
                      0);
...
==================
*/
static void CG_SwingAngles(float destination,
...
static void CG_PlayerAngles(centity_t* cent,
  ...
  } else if (BG_GetConditionValue(cent->currentState.clientNum,
                                  ANIM_COND_FIRING, qtrue)) {
    cent->pe.torso.yawing = qtrue;    // always center
    cent->pe.torso.pitching = qtrue;  // always center
  }
  ...
  if (cent->currentState.eFlags & EF_DEAD ||
    ...
    if (!(cent->currentState.eFlags & EF_FIRING)) {
      torsoAngles[YAW] =
          headAngles[YAW] + 0.35 * cent->currentState.angles2[YAW];
      clampTolerance = 90;
    } else {                               // must be firing
    ...
    // torso
    CG_SwingAngles(torsoAngles[YAW], 25, clampTolerance, cg_swingSpeed.value,
                   &cent->pe.torso.yawAngle, &cent->pe.torso.yawing);

    // if the legs are yawing (facing heading direction), allow them to rotate a
    // bit, so we don't keep calling the legs_turn animation while an AI is
    // firing, and therefore his angles will be randomizing according to their
    // accuracy

    clampTolerance = 150;

    if (BG_GetConditionBitFlag(ci->clientNum, ANIM_COND_MOVETYPE,
                               ANIM_MT_IDLE)) {
      cent->pe.legs.yawing = qfalse;  // set it if they really need to swing
      CG_SwingAngles(legsAngles[YAW], 20, clampTolerance,
                     0.5 * cg_swingSpeed.value, &cent->pe.legs.yawAngle,
                     &cent->pe.legs.yawing);
    } else if (strstr(BG_GetAnimString(character->animModelInfo, legsSet),
                      "strafe")) {
      // FIXME: what is this strstr hack??
      // if	( BG_GetConditionValue( ci->clientNum, ANIM_COND_MOVETYPE,
      // qfalse ) & ((1<<ANIM_MT_STRAFERIGHT)|(1<<ANIM_MT_STRAFELEFT)) )
      cent->pe.legs.yawing = qfalse;  // set it if they really need to swing
      legsAngles[YAW] = headAngles[YAW];
      CG_SwingAngles(legsAngles[YAW], 0, clampTolerance, cg_swingSpeed.value,
                     &cent->pe.legs.yawAngle, &cent->pe.legs.yawing);
    } else if (cent->pe.legs.yawing) {
      CG_SwingAngles(legsAngles[YAW], 0, clampTolerance, cg_swingSpeed.value,
                     &cent->pe.legs.yawAngle, &cent->pe.legs.yawing);
    } else {
      CG_SwingAngles(legsAngles[YAW], 40, clampTolerance, cg_swingSpeed.value,
                     &cent->pe.legs.yawAngle, &cent->pe.legs.yawing);
    }
  ...
  // CG_SwingAngles( dest, 15, 30, 0.1, &cent->pe.torso.pitchAngle,
  // &cent->pe.torso.pitching ); torsoAngles[PITCH] = cent->pe.torso.pitchAngle;
...
static void CG_BreathPuffs(centity_t* cent, refEntity_t* head) {
  ...
  CG_SmokePuff(morg, forward, 4, 1, 1, 1, 0.5f, 2000, cg.time, cg.time + 400, 0,
...
/*static void CG_TrailItem( centity_t *cent, qhandle_t hModel ) {
        ...
        if ( cent->currentState.number == cg.snap->ps.clientNum &&
!cg.renderingThirdPerson ) return;
...
===============
*/
static void CG_PlayerFloatSprite(centity_t* cent,
                                 qhandle_t shader,
                                 int height) {
  int rf;
  refEntity_t ent;
...
static qboolean CG_PlayerShadow(centity_t* cent, float* shadowPlane) {
  ...
  shadowPart_t shadowParts[] = {{"tag_footleft", 10, 4, 1.0, 0},
  ...
  *shadowPlane = trace.endpos[2] + 1;
  ...
  if (!(cent->currentState.eFlags & EF_ZOOMING) && (dist > SHADOW_MIN_DIST)) {
    ...
    // ydnar: add a bit of height so foot shadows don't clip into sloped
    // geometry as much
    origin[2] += 18.0f;
    ...
    // ydnar: decal remix
    //%	CG_ImpactMark( cgs.media.shadowTorsoShader, trace.endpos,
    //trace.plane.normal, %		0, alpha,alpha,alpha,1, qfalse, 16,
    //qtrue, -1 );
  ...
  if (dist < SHADOW_MAX_DIST) {  // show more detail
    ...
    for (tagIndex = 0; shadowParts[tagIndex].tagname; tagIndex++) {
      ...
      for (subIndex = 0;
           (subIndex = CG_GetOriginForTag(cent, &cent->pe.bodyRefEnt,
                                          shadowParts[tagIndex].tagname,
                                          subIndex, origin, axis)) >= 0;
           subIndex++) {
        // project it onto the shadow plane
        if (origin[2] < *shadowPlane) {
          origin[2] = *shadowPlane;
        ...
        // ydnar: decal remix
        //%	CG_ImpactMark( shadowParts[tagIndex].shader, origin,
        //trace.plane.normal, %		angles[YAW]/*cent->pe.legs.yawAngle*/,
        //alpha,alpha,alpha,1, qfalse, shadowParts[tagIndex].size, qtrue, -1 );

        //%	CG_ImpactMark( shadowParts[ tagIndex ].shader, origin, up,
        //%			cent->lerpAngles[ YAW ], 1.0f, 1.0f, 1.0f, 1.0f,
        //qfalse, shadowParts[ tagIndex ].size, qtrue, -1 );
  ...
  if (onFire) {
    ...
    if (ent->hModel == cent->pe.bodyRefEnt.hModel) {
      trap_S_AddLoopingSound(ent->origin, vec3_origin,
                             cgs.media.flameCrackSound, (int)(255.0 * alpha),
                             0);
    }
  }
...
CG_AnimPlayerConditions
        ...
        predict, or calculate condition for this entity, if it is not the local
client
===============
*/
void CG_AnimPlayerConditions(bg_character_t* character, centity_t* cent) {
  ...
  if (es->eFlags & EF_ZOOMING) {
    BG_UpdateConditionValue(es->clientNum, ANIM_COND_WEAPON, WP_BINOCULARS,
                            qtrue);
  } else {
  ...
  if ((es->eFlags & EF_MG42_ACTIVE) || (es->eFlags & EF_MOUNTEDTANK)) {
    BG_UpdateConditionValue(es->clientNum, ANIM_COND_MOUNTED, MOUNTED_MG42,
                            qtrue);
  } else if (es->eFlags & EF_AAGUN_ACTIVE) {
    BG_UpdateConditionValue(es->clientNum, ANIM_COND_MOUNTED, MOUNTED_AAGUN,
                            qtrue);
  } else {
    BG_UpdateConditionValue(es->clientNum, ANIM_COND_MOUNTED, MOUNTED_UNUSED,
                            qtrue);
  }

  // UNDERHAND
  BG_UpdateConditionValue(es->clientNum, ANIM_COND_UNDERHAND,
                          cent->lerpAngles[0] > 0, qtrue);
...
void CG_Player(centity_t* cent) {
  ...
  if (cent->currentState.eFlags & EF_MOUNTEDTANK) {
    VectorCopy(cg_entities[cg_entities[cent->currentState.clientNum].tagParent]
                   .mountedMG42Player.origin,
               playerOrigin);
  } else if (cent->currentState.eFlags & EF_MG42_ACTIVE ||
             cent->currentState.eFlags &
                 EF_AAGUN_ACTIVE) {  // Arnout: see if we're attached to a gun
    centity_t* mg42;
    int num;
  ...
  CG_AddRefEntityWithPowerups(&body, cent->currentState.powerups, ci->team,
...
// ydnar debug
  ...
  }
  {
          ...
          if( cg.predictedPlayerState.eFlags & EF_PRONE ) {
                  maxs[2] = maxs[2] - (cg.predictedPlayerState.standViewHeight -
  PRONE_VIEWHEIGHT + 8); } else if( cg.predictedPlayerState.pm_flags &
  PMF_DUCKED ) { maxs[2] = cg.predictedPlayerState.crouchMaxZ;
          }
  ...
  if (cent->currentState.eType != ET_CORPSE &&
    ...
    CG_HudHeadAnimation(character, &cent->pe.head, &head.oldframe, &head.frame,
                        &head.backlerp, anim);
  } else {
    head.frame = 0;
    head.oldframe = 0;
    ...
    if (!(cent->currentState.eType == ET_CORPSE &&
      ...
      switch (i) {
        ...
        case ACC_MOUTH3:  // hat3
          ...
          // weapon and weapon2
          // these are used by characters who have permanent weapons attached to
          // their character in the skin
...
extern void CG_ClearWeapLerpFrame(clientInfo_t* ci,
                                  lerpFrame_t* lf,
                                  int animationNumber);
...
void CG_ResetPlayerEntity(centity_t* cent) {
  ...
  //	cent->errorTime = -99999;		// guarantee no error decay
  //added 	cent->extrapolated = qfalse;

  if (!(cent->currentState.eFlags & EF_DEAD)) {
    CG_ClearLerpFrameRate(cent, &cgs.clientinfo[cent->currentState.clientNum],
                          &cent->pe.legs, cent->currentState.legsAnim);
    CG_ClearLerpFrame(cent, &cgs.clientinfo[cent->currentState.clientNum],
...
void CG_GetBleedOrigin(vec3_t head_origin,
                       vec3_t body_origin,
                       int fleshEntityNum) {
  clientInfo_t* ci;
  refEntity_t body;
...
==================
*/
static void CG_SwingAngles_Limbo(float destination,
                                 float swingTolerance,
                                 float clampTolerance,
                                 float speed,
                                 float* angle,
...
===============
*/
void CG_PlayerAngles_Limbo(playerInfo_t* pi,
                           vec3_t legs[3],
                           vec3_t torso[3],
                           vec3_t head[3]) {
  vec3_t legsAngles, torsoAngles, headAngles;
  ...
  CG_SwingAngles_Limbo(dest, 15, 30, 0.1, &pi->torso.pitchAngle,
                       &pi->torso.pitching);
...
animation_t* CG_GetLimboAnimation(playerInfo_t* pi, const char* name) {
  ...
  return character->animModelInfo
      ->animations[0];  // safe fallback so we never end up without an animation
...
void CG_DrawPlayer_Limbo(float x,
                         float y,
                         float w,
                         float h,
                         playerInfo_t* pi,
  ...
  // FIXME - this is stupid - should all be character driven - NO CODE HACKS FOR
  // SPECIFIC THINGS THAT CAN BE DONE CLEANLY
  ...
  if (cg.time >= pi->torso.frameTime) {
    ...
    pi->torso.oldFrameModel = pi->torso.frameModel =
        pi->torso.animation->mdxFile;

    while (cg.time >= pi->torso.frameTime) {
      pi->torso.frameTime += (pi->torso.animation->duration /
                              (float)(pi->torso.animation->numFrames));
      pi->torso.frame++;

      if (pi->torso.frame >=
          pi->torso.animation->firstFrame + pi->torso.animation->numFrames) {
  ...
  if (pi->torso.frameTime == pi->torso.oldFrameTime) {
    ...
    pi->torso.backlerp =
        1.0 - (float)(cg.time - pi->torso.oldFrameTime) /
                  (pi->torso.frameTime - pi->torso.oldFrameTime);
  }
  ...
  if (cg.time >= pi->legs.frameTime) {
    ...
    while (cg.time >= pi->legs.frameTime) {
      pi->legs.frameTime += (pi->legs.animation->duration /
                             (float)(pi->legs.animation->numFrames));
      pi->legs.frame++;

      if (pi->legs.frame >=
          pi->legs.animation->firstFrame + pi->legs.animation->numFrames) {
  ...
  if (pi->legs.frameTime == pi->legs.oldFrameTime) {
    ...
    pi->legs.backlerp = 1.0 - (float)(cg.time - pi->legs.oldFrameTime) /
                                  (pi->legs.frameTime - pi->legs.oldFrameTime);
  }
...
void CG_MenuSetAnimation(playerInfo_t* pi,
                         const char* legsAnim,
  ...
  if (force) {
    ...
    pi->legs.frameTime +=
        200;  // Give them some time to lerp between animations
...
void CG_MenuPendingAnimation(playerInfo_t* pi,
                             const char* legsAnim,
                             const char* torsoAnim,
                             int delay) {
  ...
  if (!pi->numPendingAnimations) {
    ...
    pi->pendingAnimations[pi->numPendingAnimations].pendingAnimationTime =
        pi->pendingAnimations[pi->numPendingAnimations - 1]
            .pendingAnimationTime +
        delay;
  }
...
void CG_MenuCheckPendingAnimation(playerInfo_t* pi) {
  ...
  if (pi->pendingAnimations[0].pendingAnimationTime &&
      pi->pendingAnimations[0].pendingAnimationTime < cg.time) {
    CG_MenuSetAnimation(pi, pi->pendingAnimations[0].pendingLegsAnim,
                        pi->pendingAnimations[0].pendingTorsoAnim, qfalse,
                        qfalse);

    for (i = 0; i < 3; i++) {
      memcpy(&pi->pendingAnimations[i], &pi->pendingAnimations[i + 1],
             sizeof(pendingAnimation_t));
    }
...
void CG_SetHudHeadLerpFrameAnimation(bg_character_t* ch,
                                     lerpFrame_t* lf,
                                     int newAnimation) {
  animation_t* anim;
...
void CG_ClearHudHeadLerpFrame(bg_character_t* ch,
                              lerpFrame_t* lf,
                              int animationNumber) {
  lf->frameTime = lf->oldFrameTime = cg.time;
...
void CG_RunHudHeadLerpFrame(bg_character_t* ch,
                            lerpFrame_t* lf,
                            int newAnimation,
                            float speedScale) {
  int f;
...
void CG_HudHeadAnimation(bg_character_t* ch,
                         lerpFrame_t* lf,
                         int* oldframe,
                         int* frame,
                         float* backlerp,
