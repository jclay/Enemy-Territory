...
// spawned, but as they grow, they can be merged so that less overdraw occurs
typedef struct flameChunk_s {
  struct flameChunk_s *nextGlobal,
      *prevGlobal;  // next junction in the global list it is in (free or used)
  ...
  qboolean inuse;
  qboolean dead;  // set when a chunk is effectively inactive, but waiting to be
                  // freed
  ...
  float sizeRand;
  float sizeRate;  // rate per ms, variable according to speed (larger if moving
                   // slower)
  vec3_t baseOrg;
  int baseOrgTime;
  vec3_t velDir;
  float velSpeed;  // flame chunks should start with a fast velocity, then slow
                   // down if there is nothing behind them pushing them along
  float rollAngle;
...
typedef struct {
  //	float fireVolume;	// not needed, since we add individual loop
  //sources for the flame, so it gets spacialized
  float blowVolume;
  float streamVolume;
} flameSoundStatus_t;
...
void CG_FlameGetMuzzlePoint(vec3_t org,
                            vec3_t fwd,
                            vec3_t right,
                            vec3_t up,
                            vec3_t outPos);

// these must be globals, since they cannot expand or contract, since that might
// result in them getting
//	stuck in geometry. therefore when a chunk hits a surface, we should
//deflect it away from the surface 	slightly, rather than running along it, so
//that as the chink grows, the sprites don't sink into the 	wall too much.
...
#define FLAME_START_MAX_SIZE \
  140.0  // when the flame is spawned, it should endevour to reach this size
...
#define FLAME_LENGTH \
  (FLAMETHROWER_RANGE + 50.0)  // NOTE: only modify the range, since this should
                               // always reflect that range
...
===============
*/
void CG_FlameLerpVec(const vec3_t oldV,
                     const vec3_t newV,
                     float backLerp,
                     vec3_t outV) {
...
void CG_FireFlameChunks(centity_t* cent,
  ...
  // for any other character or in 3rd person view, use entity angles for
  // friction
  ...
  // if this entity was firing last frame, interpolate the angles as we spawn
  // the chunks that fired over the last frame
  if ((centInfo->lastClientFrame == cent->currentState.frame) &&
    ...
    while (t <= cg.time) {
      ...
      f->sizeMax = speedScale *
                   (FLAME_START_MAX_SIZE + f->sizeRand * (firing ? 1.0 : 0.0));
      f->sizeRand = 0;
  ...
  while (f) {
          ...
          if (f->lastFriction < cg.time - 50) {
                  ...
                  if (dot >= 0.99) {
                          dot -= 0.99;
                          dot *= (1.0/(1.0-0.99));
                          CG_FlameAdjustSpeed( f, 0.5 * frametime *
  FLAME_FRICTION_PER_SEC * pow(dot,4) );
                  }
          }
...
flameChunk_t* CG_SpawnFlameChunk(flameChunk_t* headFlameChunk) {
  ...
  f->nextFlameChunk = headFlameChunk;  // if headJunc is NULL, then we'll just
...
*/
void CG_FlameCalcOrg(flameChunk_t* f, int time, vec3_t outOrg) {
  VectorMA(f->baseOrg, f->velSpeed * ((float)(time - f->baseOrgTime) / 1000),
           f->velDir, outOrg);
  // outOrg[2] -= f->gravity * ((float)(time - f->timeStart)/1000.0) *
  // ((float)(time - f->timeStart)/1000.0);
}
...
void CG_MoveFlameChunk(flameChunk_t* f) {
  ...
  if (f->velSpeed > 1 && f->lastFrictionTake < cg.time - 50) {
    CG_FlameAdjustSpeed(f, -((float)(cg.time - f->lastFrictionTake) / 1000.0) *
                               FLAME_FRICTION_PER_SEC);
    f->lastFrictionTake = cg.time;
  }
  ...
  if (f->size < f->sizeMax) {
    if ((cg.time - f->timeStart) < f->blueLife) {
      f->sizeRate = GET_FLAME_BLUE_SIZE_SPEED(
          FLAME_START_MAX_SIZE);  // use a constant so the blue flame doesn't
                                  // distort
    } else {
  ...
  while (f->velSpeed > 1 && f->baseOrgTime != cg.time) {
    ...
    CG_Trace(&trace, sOrg, flameChunkMins, flameChunkMaxs, newOrigin,
             f->ownerCent, MASK_SHOT | MASK_WATER);  // JPW NERVE water fixes
...
#ifdef _DEBUG  // just in case we forget about it, but it should be disabled at
...
void CG_AddFlameSpriteToScene(flameChunk_t* f, float lifeFrac, float alpha) {
  ...
  pPolyBuffer = CG_PB_FindFreePolyBuffer(
      cg_fxflags & 1 ? getTestShader() : flameShaders[frameNum], 4, 6);
  ...
  memcpy(pPolyBuffer->color[pPolyBuffer->numVerts + 1],
         pPolyBuffer->color[pPolyBuffer->numVerts + 0],
         sizeof(pPolyBuffer->color[0]));
  memcpy(pPolyBuffer->color[pPolyBuffer->numVerts + 2],
         pPolyBuffer->color[pPolyBuffer->numVerts + 0],
         sizeof(pPolyBuffer->color[0]));
  memcpy(pPolyBuffer->color[pPolyBuffer->numVerts + 3],
         pPolyBuffer->color[pPolyBuffer->numVerts + 0],
         sizeof(pPolyBuffer->color[0]));

  // find the projected distance from the eye to the projection of the flame
  // origin onto the view direction vector
  VectorMA(cg.refdef_current->vieworg, 1024, cg.refdef_current->viewaxis[0],
           p2);
  ...
  if ((rotatingFlames) &&
      (!(cg_fxflags & 1))) {  // JPW NERVE no rotate for alt flame shaders
  ...
  pPolyBuffer->indicies[pPolyBuffer->numIndicies + 0] =
      pPolyBuffer->numVerts + 0;
  pPolyBuffer->indicies[pPolyBuffer->numIndicies + 1] =
      pPolyBuffer->numVerts + 1;
  pPolyBuffer->indicies[pPolyBuffer->numIndicies + 2] =
      pPolyBuffer->numVerts + 2;

  pPolyBuffer->indicies[pPolyBuffer->numIndicies + 3] =
      pPolyBuffer->numVerts + 2;
  pPolyBuffer->indicies[pPolyBuffer->numIndicies + 4] =
      pPolyBuffer->numVerts + 3;
  pPolyBuffer->indicies[pPolyBuffer->numIndicies + 5] =
      pPolyBuffer->numVerts + 0;
...
void CG_AddFlameToScene(flameChunk_t* fHead) {
  ...
  while (f) {
    ...
    vdist = Distance(cg.refdef_current->vieworg,
                     f->org);  // NOTE: this needs to be here or the flameSound
                               // code further below won't work
      ...
      if (!skip) {
        ...
        if (f->blueLife > lived * (f->ignitionOnly ? 3.0 : 3.0)) {
          ...
          blueTrailHead = CG_AddTrailJunc(
              ...
              f->size *
                  (f->ignitionOnly /*&& (cg.snap->ps.clientNum != f->ownerCent
                                      || cg_thirdPerson.integer)*/
                       ? 2.0
                       : 1.0),
        ...
        if (!f->ignitionOnly) {
          ...
          if (!f->nextFlameChunk) {
            alpha = 0;
                     ...
                     bscale * FLAME_BLUE_FADEIN_TIME(f->blueLife)) {
            alpha = FLAME_BLUE_MAX_ALPHA *
                    ((lived / 1.3) /
                     (bscale * FLAME_BLUE_FADEIN_TIME(f->blueLife)));
          } else if (lived / 1.3 <
                     (f->blueLife - FLAME_BLUE_FADEOUT_TIME(f->blueLife))) {
            alpha = FLAME_BLUE_MAX_ALPHA;
          } else {
            alpha =
                FLAME_BLUE_MAX_ALPHA *
                (1.0 - ((lived / 1.3 -
                         (f->blueLife - FLAME_BLUE_FADEOUT_TIME(f->blueLife))) /
...
#define FLAME_SPRITE_START_BLUE_SCALE 0.2
    ...
    if (!f->ignitionOnly &&
      ...
      while (fNext && !droppedTrail) {
        if ((Distance(f->org, fNext->org) <
             ((0.1 + 0.9 * f->lifeFrac) * f->size * 0.35)) &&
  ...
  if (fHead->ignitionOnly) {
    ...
    //%	trap_R_AddLightToScene( lightOrg, 90 + lightSize, 0, 0, alpha, 0 +
    //isClientFlame * (fHead->ownerCent == cg.snap->ps.clientNum) );
    trap_R_AddLightToScene(lightOrg, 80, alpha, 0.2, 0.21, 0.5, 0, 0);
  } else if (isClientFlame || (fHead->ownerCent == cg.snap->ps.clientNum)) {
    //%	trap_R_AddLightToScene( lightOrg, 90 + lightSize, 1.000000*alpha,
...
CG_GenerateShaders
  ...
  A util to create a bunch of shaders in a unique shader file, which represent
an animation
=============
*/
void CG_GenerateShaders(char* filename,
                        char* shaderName,
                        char* dir,
                        int numFrames,
                        char* srcBlend,
  ...
  for (i = 0; i < numFrames; i++) {
    ...
    if (compressedVersionAvailable) {
      Com_sprintf(
          str, sizeof(str),
          "%s%i\n{\n\tnofog%s\n\tallowCompress\n\tcull none\n\t{\n\t\tmapcomp "
          "sprites/%s_lg/spr%i%i%i.tga\n\t\tmapnocomp "
          "sprites/%s/spr%i%i%i.tga\n\t\tblendFunc %s %s\n%s\t}\n}\n",
          shaderName, i + 1, nomipmap ? "\n\tnomipmaps" : "", dir, b, c, d, dir,
          b, c, d, srcBlend, dstBlend, extras);
    } else {
      Com_sprintf(
          str, sizeof(str),
          "%s%i\n{\n\tnofog%s\n\tallowCompress\n\tcull none\n\t{\n\t\tmap "
...
void CG_InitFlameChunks(void) {
  ...
  CG_GenerateShaders("scripts/flamethrower.shader", "flamethrowerFire",
                     FLAME_SPRITE_DIR, NUM_FLAME_SPRITES, FLAME_BLEND_SRC,
                     FLAME_BLEND_DST, "", qtrue, qtrue);

  CG_GenerateShaders("scripts/blacksmokeanim.shader", "blacksmokeanim",
                     "explode1", 23, "GL_ZERO", "GL_ONE_MINUS_SRC_ALPHA",
                     "\t\talphaGen const 0.2\n", qfalse, qfalse);

  CG_GenerateShaders("scripts/viewflames.shader", "viewFlashFire", "clnfire",
                     16, "GL_ONE", "GL_ONE",
                     "\t\talphaGen vertex\n\t\trgbGen vertex\n", qtrue, qtrue);

  CG_GenerateShaders("scripts/twiltb.shader", "twiltb", "twiltb", 42,
                     "GL_SRC_ALPHA", "GL_ONE_MINUS_SRC_COLOR", "", qtrue,
                     qfalse);

  CG_GenerateShaders("scripts/twiltb2.shader", "twiltb2", "twiltb2", 45,
                     "GL_ONE", "GL_ONE_MINUS_SRC_COLOR", "", qtrue, qfalse);
  /*
          CG_GenerateShaders( "scripts/expblue.shader",
                                                  ...
                                                  qfalse, qfalse );
  */
  CG_GenerateShaders("scripts/firest.shader", "firest", "firest", 36, "GL_ONE",
                     "GL_ONE_MINUS_SRC_COLOR", "", qtrue, qfalse);

  CG_GenerateShaders("scripts/explode1.shader", "explode1", "explode1", 23,
                     "GL_ONE", "GL_ONE_MINUS_SRC_COLOR", "", qtrue, qfalse);

  CG_GenerateShaders("scripts/funnel.shader", "funnel", "funnel", 21, "GL_ONE",
                     "GL_ONE_MINUS_SRC_COLOR", "", qfalse, qfalse);
#endif
...
void CG_AddFlameChunks(void) {
  ...
  while (f) {
    if (!f->dead) {
      if (cg.time > f->timeEnd) {
        ...
        CG_MoveFlameChunk(f);
        f->lifeFrac = (float)(cg.time - f->timeStart) /
                      (float)(f->timeEnd - f->timeStart);
      }
    }
    f = f->nextGlobal;
  }
  ...
  while (f) {
    ...
    if (f->dead) {
      ...
      CG_FreeFlameChunk(f);
    } else if (!f->ignitionOnly ||
               (centFlameInfo[f->ownerCent].lastFlameChunk ==
                f)) {  // don't draw the ignition flame after we start firing
  ...
  while (f) {
    ...
    if (centFlameInfo[f->ownerCent].lastSoundUpdate != cg.time) {
      ...
      if (centFlameStatus[f->ownerCent].blowVolume * 255.0 > MIN_BLOW_VOLUME) {
        trap_S_AddLoopingSound(
            f->org, vec3_origin, cgs.media.flameBlowSound,
            (int)(255.0 * centFlameStatus[f->ownerCent].blowVolume),
            0);  // JPW NERVE
      } else {
        trap_S_AddLoopingSound(f->org, vec3_origin, cgs.media.flameBlowSound,
                               MIN_BLOW_VOLUME, 0);  // JPW NERVE
      }

      if (centFlameStatus[f->ownerCent].streamVolume) {
        trap_S_AddLoopingSound(
            f->org, vec3_origin, cgs.media.flameStreamSound,
            (int)(255.0 * centFlameStatus[f->ownerCent].streamVolume),
            0);  // JPW NERVE
      }
    ...
    for (trav = f; trav; trav = trav->nextFlameChunk) {
      ...
      if (trav->blueLife + 100 < (cg.time - trav->timeStart)) {
        trap_S_AddLoopingSound(
            trav->org, vec3_origin, cgs.media.flameSound,
