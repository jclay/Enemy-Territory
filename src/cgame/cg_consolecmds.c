 ...
 *
 * desc:		text commands typed in at the local console, or executed
 * by a key binding
 *
 */
...
static void CG_StatsDown_f(void) {
  if (!cg.demoPlayback) {
    int i = (cg.mvTotalClients > 0) ? (cg.mvCurrentActive->mvInfo & MV_PID)
    ...
    if (cgs.gamestats.show == SHOW_SHUTDOWN &&
        cg.time < cgs.gamestats.fadeTime) {
      cgs.gamestats.fadeTime =
          2 * cg.time + STATS_FADE_TIME - cgs.gamestats.fadeTime;
...
void CG_topshotsDown_f(void) {
  if (!cg.demoPlayback) {
    if (cgs.topshots.show == SHOW_SHUTDOWN && cg.time < cgs.topshots.fadeTime) {
      cgs.topshots.fadeTime =
          2 * cg.time + STATS_FADE_TIME - cgs.topshots.fadeTime;
...
int CG_LoadCamera(const char* name) {
  int i;
  for (i = 1; i < MAX_CAMERAS;
       i++) {  // start at '1' since '0' is always taken by the cutscene camera
    if (!cameraInuse[i]) {
...
void CG_StartCamera(const char* name, qboolean startBlack) {
  ...
  // if ( cg.predictedPlayerState.stats[STAT_HEALTH] <= 0 )	// don't allow
  // camera to start if you're dead 	return;
...
static void CG_VoiceChat_f(void) {
  ...
  // NOTE - This cg.snap will be the person you are following, but its just for
  // intermission test
  if (cg.snap && (cg.snap->ps.pm_type != PM_INTERMISSION)) {
    if (cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR ||
        cgs.clientinfo[cg.clientNum].team == TEAM_FREE) {
...
static void CG_AutomapExpandDown_f(void) {
  if (!cgs.autoMapExpanded) {
    ...
    if (cg.time - cgs.autoMapExpandTime < 250.f) {
      cgs.autoMapExpandTime =
          cg.time - (250.f - (cg.time - cgs.autoMapExpandTime));
    } else {
...
// OSP
const char* aMonths[12] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
...
void CG_dumpStats_f(void) {
  if (cgs.dumpStatsTime < cg.time) {
    ...
    trap_SendClientCommand((cg.mvTotalClients < 1) ? "weaponstats"
                                                   : "statsall");
  }
}
...
static void CG_EditSpeakers_f(void) {
  if (cg.editingSpeakers) {
    ...
    const char* s =
        Info_ValueForKey(CG_ConfigString(CS_SYSTEMINFO), "sv_cheats");
    if (s[0] != '1') {
...
static void CG_DumpSpeaker_f(void) {
  /*	char sscrfilename[MAX_QPATH];
          ...
          {
                  CG_Printf( "Usage: dumpspeaker <soundfile> (
     <wait=value>|<random=value> )\n" ); return;
          }
     ...
     \"target_speaker\"\n\"origin\" \"%i %i %i\"\n\"noise\" \"%s\"\n", (int)
     cg.snap->ps.origin[0], (int) cg.snap->ps.origin[1], (int)
     cg.snap->ps.origin[2], soundfile );

          if( wait ) {
                  Q_strcat( soundfile, sizeof(soundfile), va( "\"wait\"
     \"%i\"\n", wait ) );
          }
          if( random ) {
                  Q_strcat( soundfile, sizeof(soundfile), va( "\"random\"
     \"%i\"\n", random ) );
          }
          ...
          CG_Printf( "Entity dumped to '%s' (%i %i %i).\n", sscrfilename,
                             (int) cg.snap->ps.origin[0], (int)
     cg.snap->ps.origin[1], (int) cg.snap->ps.origin[2] );*/
  ...
  if (tr.fraction < 1.f) {
    ...
    VectorMA(speaker.origin, -4, cg.refdef_current->viewaxis[0],
             speaker.origin);
  } else {
    VectorCopy(tr.endpos, speaker.origin);
  }
