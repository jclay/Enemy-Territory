...
void CG_Letterbox(float xsize, float ysize, qboolean center) {
  ...
  if (cg_letterbox.integer) {
    ...
    ysize = lbheight;
    //		if(letterbox_frac != 0) {
    //			letterbox_frac -= 0.01f;	// (SA) TODO: make non
    //fps dependant 			if(letterbox_frac < 0) 				letterbox_frac = 0; 			ysize += (lbdiff
    //* letterbox_frac);
    //		}
    //	} else {
    //		if(letterbox_frac != 1) {
    //			letterbox_frac += 0.01f;	// (SA) TODO: make non
    //fps dependant 			if(letterbox_frac > 1) 				letterbox_frac = 1; 			ysize = lbheight
    //+ (lbdiff * letterbox_frac);
    //		}
  }
  ...
  for (t = cg.frametime; t > 0; t -= STEP) {
    ...
    for (i = 0; i < 3; i++) {
      if (cg.kickAVel[i] || cg.kickAngles[i]) {
        ...
        if (cg.kickAngles[i] &&
            (cg.kickAngles[i] < 0) !=
                (kickChange < 0)) {  // slower when returning to center
          kickChange *= 0.06;
        }
        ...
        if (!cg.kickAngles[i] ||
          ...
          if (!cg.kickAngles[i] && frametime) {
            ...
            cg.kickAVel[i] = 0;  // force Avel to return us to center rather
                                 // than keep going outside range
          }
...
static void CG_OffsetFirstPersonView(void) {
  ...
  speed * 15 : cg.bobfracsin * cg_bobyaw.value * speed * 15; if( useLastValidBob
  ) { if( cg.lastvalidBobcycle & 1 ) delta = -delta; } else if (cg.bobcycle & 1)
                  delta = -delta;
...
void CG_ZoomIn_f(void) {
  ...
  if (cg_entities[cg.snap->ps.clientNum].currentState.weapon ==
    ...
    CG_AdjustZoomVal(
        -(cg_zoomStepSniper.value),
        ZOOM_SNIPER);  // JPW NERVE per atvi request all use same vals to match
                       // menu (was zoomStepBinoc, ZOOM_BINOC);
  }
}
    ...
    if (cg.predictedPlayerState.weapon == WP_FG42SCOPE) {
      cg.zoomval =
          (cg.zoomval == 0)
              ...
              : cg.zoomval;  // JPW NERVE was DefaultFG, changed per atvi req
  ...
  if (cg.predictedPlayerState.eFlags & EF_ZOOMING) {
    ...
    cg.zoomval =
        cg_zoomDefaultSniper
            .value;  // JPW NERVE was DefaultBinoc, changed per atvi req
  } else {
    if (cg.zoomedBinoc) {
      ...
      if (cg.weaponSelect == WP_FG42SCOPE) {
        cg.zoomval =
            cg_zoomDefaultSniper
                .value;  // JPW NERVE was DefaultFG, changed per atvi req
      ...
      // bani - we now sanity check to make sure we can't zoom non-zoomable
      // weapons zinx - fix for #423 - don't sanity check while following
      if (!((cg.snap->ps.pm_flags & PMF_FOLLOW) || cg.demoPlayback)) {
  ...
  if (cg.snap->ps.pm_type == PM_FREEZE ||
    ...
    if (cg.zoomval) {
      cg.zoomSensitivity =
          0.6 *
          (cg.zoomval /
           90.f);  // NERVE - SMF - changed to get less sensitive as you zoom in
...
static void CG_DamageBlendBlob(void) {
  ...
  // Gordon: no damage blend blobs if in limbo or spectator, and in the limbo
  // menu
  if ((cg.snap->ps.pm_flags & PMF_LIMBO ||
...
static void CG_DrawScreenFade(void) {
  /* moved over to cg_draw.c
     ...
     need to use trap_Milliseconds() here since the cg.time gets modified upon
     reloading lastTime = time; if (elapsed < 500 && elapsed > 0) { if
     (cgs.fadeAlphaCurrent > cgs.fadeAlpha) { cgs.fadeAlphaCurrent -=
...
int CG_CalcViewValues(void) {
  ...
  // sizeof(cg.refdef_current->text[0]) ); Q_strncpyz( cg.refdef.text[1], "19",
  // sizeof(cg.refdef_current->text[1]) );
  ...
  if (cg.cameraMode) {
    ...
    if (trap_getCameraInfo(CAM_PRIMARY, cg.time, &origin, &angles, &fov)) {
      ...
      angles[ROLL] = 0;
      angles[PITCH] =
          -angles[PITCH];  // (SA) compensate for reversed pitch (this makes the
                           // game match the editor, however I'm guessing the
                           // real fix is to be done there)
...
void CG_ParseSkyBox(void) {
  ...
  token = CG_MustParse(
      &cstr,
      "CG_ParseSkyBox: error parsing skybox configstring.  No fog state\n");
...
void CG_ParseTagConnect(int tagNum) {
  char *token, *pString = (char*)CG_ConfigString(
...
void CG_DrawActiveFrame(int serverTime,
  ...
  // (FIXME: this is a hack for the time being since a scripted "selectweapon"
  // does not hit the first snap, the server weapon set in cg_playerstate.c line
  // 219 doesn't do the trick)
  ...
  // let the client system know what our weapon, holdable item and zoom settings
  // are
