...
(SA) boy, it would be nice to have an acceleration vector for this as well.
                big velocity vector with a negative acceleration for
deceleration, etc. (breath could then come out of a guys mouth at the rate he's
...
=================
*/
void CG_AddBloodTrails(vec3_t origin,
                       vec3_t dir,
                       int speed,
                       int duration,
                       int count,
  ...
  for (i = 0; i < count; i++) {
    ...
    le->startTime = cg.time;
    le->endTime =
        le->startTime + duration;  // DHM - Nerve :: (removed) - (int)(0.5 *
                                   // random() * duration);
...
#define BLOOD_SPURT_COUNT 4
  ...
  if (cg_reloading.integer) {
    // to dangerous, since we call playerangles() in here, which calls the
    // animation system, which might not be setup yet
    return;
  }
...
==================
*/
void CG_LaunchGib(centity_t* cent,
                  vec3_t origin,
                  vec3_t angles,
                  vec3_t velocity,
                  qhandle_t hModel,
  ...
  le->angles.trDelta[1] =
      (100 + (rand() & 500)) -
      300;  // (SA) this is the safe one right now (yaw)  turn the others up
            // when I have tumbling things landing properly
...
void CG_LoseHat(centity_t* cent, vec3_t dir) {
  ...
  tagIndex = CG_GetOriginForTag(cent, &cent->pe.headRefEnt, "tag_mouth", 0,
                                origin, NULL);
...

  {
    ...
    // (SA) FIXME: origin of hat md3 is offset from center.  need to center the
    // origin when you toss it
    le->pos.trType = TR_GRAVITY;
    VectorCopy(origin, le->pos.trBase);
    VectorCopy(velocity, le->pos.trDelta);
    le->pos.trTime = cg.time;
    ...
    le->angles.trDelta[2] =
        400;  // (SA) this is set with a very particular value to try to get it
...
void CG_GibPlayer(centity_t* cent, vec3_t playerOrigin, vec3_t gdir) {
  ...
  char* gibTags[] = {// tags in the legs
                     "tag_footright", "tag_footleft", "tag_legright",
                     "tag_legleft", "tag_torso",

                     // tags in the torso
                     "tag_chest", "tag_armright", "tag_armleft", "tag_head",
                     NULL};
...
void CG_ProjectedSpotLight(vec3_t start, vec3_t dir) {
  ...
  //%	CG_ImpactMark( cgs.media.spotLightShader, tr.endpos, projection, 0,
...
==============
CG_Spotlight
        segs:	number of sides on tube. - 999 is a valid value and just means,
'cap to max' (MAX_SPOT_SEGS) or use lod scheme range:	effective range of beam
        startWidth: will be optimized for '0' as a value (true cone) to not use
quads and not cap the start circle

        -- flags --
        SL_NOTRACE			- don't do a trace check for shortening
the beam, always draw at full 'range' length SL_TRACEWORLDONLY	- go through
everything but the world SL_NODLIGHT			- don't put a dlight at
the end SL_NOSTARTCAP		- dont' cap the start circle SL_LOCKTRACETORANGE
- only trace out as far as the specified range (rather than to max spot range)
        SL_NOFLARE			- don't draw a flare when the light is
pointing at the camera SL_NOIMPACT			- don't draw the impact
mark on hit surfaces SL_LOCKUV			- lock the texture coordinates
at the 'true' length of the requested beam. SL_NOCORE			- don't
draw the center 'core' beam
  ...
  I know, this is a bit kooky right now.  It evolved big, but now that I know
what it should do, it'll get crunched down to a bunch of table driven stuff.
once it works, I'll make it work well...
...
void CG_Spotlight(centity_t* cent,
  ...
  vec3_t v1, v2;
  vec3_t startvec,
      endvec;  // the vectors to rotate around lightDir to create the circles
  vec3_t conevec;
  ...
  if (flags & SL_LOCKTRACETORANGE) {
    ...
    VectorMA(start, MAX_SPOT_RANGE, lightDir,
             traceEnd);  // trace all the way out to max dist
  }
  ...
  if (flags & SL_NOTRACE) {
    ...
    //		CG_Trace( &tr, start, NULL, NULL, traceEnd, -1, MASK_ALL
    //&~(CONTENTS_MONSTERCLIP|CONTENTS_AREAPORTAL|CONTENTS_CLUSTERPORTAL));
  }
  ...
  VectorMA(startvec, beamLen, conevec,
           endvec);  // this applies the offset of the start diameter before
                     // finding the end points

  VectorScale(lightDir, beamLen, endCenter);
  VectorSubtract(endCenter, endvec,
                 coreverts[3].xyz);  // get a vector of the radius out at the
                                     // end for the core to use
...
#define CORESCALE 0.6f
  ...
  if (!(flags & SL_NOCORE)) {
    ...
    VectorMA(endCenter, -coreEndRadius * CORESCALE, coreright,
             coreverts[2].xyz);
  ...
  for (i = 0; i <= segs; i++) {
    RotatePointAroundVector(start_points[i], lightDir, startvec,
                            (360.0f / (float)segs) * i);
    VectorAdd(start_points[i], start, start_points[i]);

    RotatePointAroundVector(end_points[i], lightDir, endvec,
                            (360.0f / (float)segs) * i);
  ...
  if (!(flags & SL_NOIMPACT)) {
    if (hitDist) {
      ...
      //%	CG_ImpactMark( cgs.media.spotLightShader, tr.endpos, proj, 0,
      //colorNorm[0], colorNorm[1], colorNorm[2], alpha, qfalse, radius, qtrue,
      //-1 );

      VectorCopy(lightDir, projection);
      projection[3] = radius * 2.0f;
      CG_ImpactMark(cgs.media.spotLightShader, tr.endpos, projection, radius,
                    colorNorm[0], colorNorm[1], colorNorm[2], 1.0f, 1.0f, -1);
    }
  }
  ...
  if (!(flags & SL_NOFLARE)) {
    ...
    // it's facing the camera, find out how closely and trace to see if the
    // source can be seen

    deg = RAD2DEG(M_PI - acos(dot));
    if (deg <=
        35) {  // start flare a bit before the camera gets inside the cylinder
      lightInEyes = qtrue;
      flarescale = 1 - (deg / 35);
    }
    ...
    if (lightInEyes) {  // the dot check succeeded, now do a trace
      CG_Trace(&tr, start, NULL, NULL, camloc, -1,
               MASK_ALL & ~(CONTENTS_MONSTERCLIP | CONTENTS_AREAPORTAL |
    ...
    if (lightInEyes) {
      ...
      trap_R_AddCoronaToScene(start, colorNorm[0], colorNorm[1], colorNorm[2],
                              coronasize, cent->currentState.number, qtrue);
    } else {
      // even though it's off, still need to add it, but turned off so it can
      // fade in/out properly
      trap_R_AddCoronaToScene(start, colorNorm[0], colorNorm[1], colorNorm[2],
                              0, cent->currentState.number, qfalse);
    }
  }
}
...
void CG_RumbleEfx(float pitch, float yaw) {
  ...
  // crandom()*yawRandom()" but that seems to skew the effect either to the left
  // or to the right for long streches of time. The idea here is to keep it
  // skewed for short period of time and then switches to the other direction -
  // switch the sign of recoil[YAW] when random() < 0.05 and keep the sign
  // otherwise. This seems better at balancing out the effect.
...
typedef struct smokesprite_s {
  ...
  struct smokesprite_s*
      prev;  // this one is only valid for alloced smokesprites
...
static smokesprite_t*
    firstfreesmokesprite;  // pointer to the first free smokepuff in the
                           // SmokeSprites pool
static smokesprite_t*
    lastusedsmokesprite;  // pointer to the last used smokepuff
...
static smokesprite_t* DeAllocSmokeSprite(smokesprite_t* dealloc) {
  ...
  if (dealloc->next) {
    ...
    if (lastusedsmokesprite) {  // incase that there was no previous particle
                                // (happens when there is only one particle and
...
static qboolean CG_SmokeSpritePhysics(smokesprite_t* smokesprite,
  ...
  // FIXME: use mins and max with smoke sprite  minimum radius and then expand
  // to max possible distance or real current sprite size? would definately look
  // nice I think
  // VectorSet( maxs, .3f * smokesprite->size, .3f * smokesprite->size, .3f *
  // smokesprite->size ); VectorNegate( maxs, mins ); CG_Trace( &tr, oldpos,
  // mins, maxs, smokesprite->pos, -1, CONTENTS_SOLID );
...
void CG_RenderSmokeGrenadeSmoke(centity_t* cent, const weaponInfo_t* weapon) {
  ...
  if (cent->currentState.effect1Time > 16) {
    ...
    //		numSpritesForRadius = cent->currentState.effect1Time / ((((640.f
    //- 16.f)/16)/1000.f) * cg.frametime); 		numNewSpritesNeeded =
    //numSpritesForRadius - cent->miscTime;

    //		CG_Printf( "numSpritesForRadius: %i / numNewSpritesNeeded: %i /
    //cent->miscTime: %i\n", numSpritesForRadius, numNewSpritesNeeded,
    //cent->miscTime );
    ...
    if (!spritesNeeded) {
      ...
      if (!CG_SpawnSmokeSprite(cent, 0.f)) {
        // try again, just in case, so we don't get lots of gaps and remain
        // quite constant
      ...
      for (dtime = spritesNeeded * spawnrate; dtime > 0; dtime -= spawnrate) {
        // this is theoretically fine, till the smokegrenade ends up in a solid
        // while( !CG_SpawnSmokeSprite( cent, lerp * cg.frametime *
        // SMOKEBOMB_SMOKEVELOCITY ) );

        // this is better
        if (!CG_SpawnSmokeSprite(
                cent, lerp * cg.frametime * SMOKEBOMB_SMOKEVELOCITY)) {
          // try again, just in case, so we don't get lots of gaps and remain
          // quite constant
