 ...
 * desc:	The entire cgame module is unloaded and reloaded on each level
 ...
 the client side. *			If you absolutely need something stored,
 it can either be kept *			by the server in the server
 stored userinfos, or stashed in a cvar.
...
typedef struct {
  ...
  float x;                           // Target x-coordinate
            //    negative values will align the window from the right minus the
            //    (window width + offset(x))
  float y;  // Target y-coordinate
            //    negative values will align the window from the bottom minus
            //    the (window height + offset(y))
} cg_window_t;
...
typedef struct centity_s {
  ...
  //error from this time 	vec3_t			errorOrigin; 	vec3_t
  //errorAngles;

  //	qboolean		extrapolated;	// false if origin / angles is
  //an interpolation
  vec3_t rawOrigin;
  vec3_t rawAngles;
  ...
  vec3_t lastLerpAngles;  // (SA) for remembering the last position when a state
                          // changes
  vec3_t lastLerpOrigin;  // Gordon: Added for linked trains player adjust
                          // prediction
  ...
  // Ridah
  vec3_t fireRiseDir;  // if standing still this will be up, otherwise it'll
                       // point away from movement dir
  ...
  vec3_t highlightOrigin;  // center of the geometry.  for things like corona
                           // placement on treasure
...
typedef struct localEntity_s {
  ...
  struct localEntity_s*
      chain;  // used for grouping entities (like for flamethrower junctions)
...
typedef struct clientInfo_s {
  ...
  // Gordon: caching fireteam pointer here, better than trying to work it out
  // all the time
...
typedef struct weaponInfo_s {
  ...
  qhandle_t
      standModel;  // not drawn.  tags used for positioning weapons for pickup
  ...
  sfxHandle_t
      lastShotSound[4];  // sound of the last shot can be different (mauser
                         // doesn't have bolt action on last shot for example)
  ...
  sfxHandle_t
      readySound;  // an amibient sound the weapon makes when it's /not/ firing
  ...
  sfxHandle_t
      spinupSound;  //----(SA)	added // sound started when fire button goes
                    //down, and stepped on when the first fire event happens
  sfxHandle_t
      spindownSound;  //----(SA)	added // sound called if the above is
                      //running but player doesn't follow through and fire
...
typedef struct {
  ...
  //	int			identifyClientNum;			// NERVE
  //- SMF 	int			identifyClientHealth;		// NERVE - SMF
  ...
  int lastIdleTimeEnd;
  int idleAnim;
  int lastWeapSelInBank
      [MAX_WEAP_BANKS_MP];  // remember which weapon was last selected in a bank
                            // for 'weaponbank' commands //----(SA)	added
  // JPW FIXME NOTE: max_weap_banks > max_weap_banks_mp so this should be OK,
  // but if that changes, change this too
  ...
  // TAT 10/23/2002
  //		For the bot hud, we keep a bit mask for which bot_action icons
  //to show
  int botMenuIcons;
...
typedef struct {
  ...
  //----(SA)	cursor hints
  // would be nice to specify these in the menu scripts instead of permanent
  // handles...
  ...
  qhandle_t
      hMountedMG42Base;  //	trap_R_RegisterModel(
...
typedef struct {
  ...
  float smokeWindDir;    // JPW NERVE for smoke puffs & wind (arty, airstrikes,
                         // bullet impacts)
  ...
  int dumpStatsTime;  // Next stats command that comes back will be written to a
                      // logfile
...
void CG_FillRect(float x,
                 float y,
                 float width,
                 float height,
                 const float* color);
void CG_HorizontalPercentBar(float x,
                             float y,
                             float width,
                             float height,
                             float percent);
void CG_DrawPic(float x, float y, float width, float height, qhandle_t hShader);
void CG_DrawPicST(float x,
                  float y,
                  float width,
                  float height,
                  float s0,
                  ...
                  float t1,
                  qhandle_t hShader);
void CG_DrawRotatedPic(float x,
                       float y,
                       float width,
                       float height,
                       qhandle_t hShader,
...
void CG_FilledBar(float x,
                  ...
                  int flags);
// JOSEPH 10-26-99
void CG_DrawStretchPic(float x,
                       float y,
                       float width,
                       float height,
                       qhandle_t hShader);
...
void CG_DrawStringExt2(int x,
                       ...
                       int charHeight,
                       int maxChars);
void CG_DrawStringExt_Shadow(int x,
                             int y,
                             const char* string,
                             const float* setColor,
                             qboolean forceColor,
...
void CG_DrawRect(float x,
                 ...
                 float size,
                 const float* color);
void CG_DrawRect_FixedBorder(float x,
                             float y,
                             float width,
                             float height,
                             int border,
...
float CG_GetValue(int ownerDraw,
                  int type);  // 'type' is relative or absolute
                              // (fractional-'0.5' or absolute- '50' health)
...
void CG_DrawPlayerWeaponIcon(rectDef_t* rect,
...
void CG_Trace(trace_t* result,
              ...
              int skipNumber,
              int mask);
void CG_FTTrace(trace_t* result,
                const vec3_t start,
                const vec3_t mins,
                const vec3_t maxs,
                const vec3_t end,
...
void CG_ParticleSnow(qhandle_t pshader,
                     vec3_t origin,
                     vec3_t origin2,
                     int turb,
                     float range,
...
void CG_ParticleDirtBulletDebris(vec3_t org,
                                 vec3_t vel,
                                 int duration);  // DHM - Nerve
void CG_ParticleDirtBulletDebris_Core(vec3_t org,
                                      vec3_t vel,
                                      int duration,
                                      float width,
                                      float height,
                                      float alpha,
                                      qhandle_t shader);
void CG_ParticleSparks(vec3_t org,
                       vec3_t vel,
                       int duration,
                       float x,
                       float y,
...
void CG_ParticleMisc(qhandle_t pshader,
                     vec3_t origin,
                     int size,
                     int duration,
                     float alpha);
...
void CG_ParticleImpactSmokePuffExtended(
    ...
    float size);  // (SA) so I can add more parameters without screwing up the
                  // one that's there
void CG_Particle_Bleed(qhandle_t pshader,
                       vec3_t start,
                       vec3_t dir,
                       int fleshEntityNum,
                       int duration);
void CG_GetBleedOrigin(vec3_t head_origin,
                       vec3_t body_origin,
                       int fleshEntityNum);
void CG_Particle_OilParticle(qhandle_t pshader,
                             vec3_t origin,
                             vec3_t origin2,
                             int ptime,
                             int snum);
...
int CG_AddTrailJunc(int headJuncIndex,
                    void* usedby,
                    qhandle_t shader,
                    int spawnTime,
                    int sType,
                    ...
                    float sRatio,
                    float animSpeed);
int CG_AddSparkJunc(int headJuncIndex,
                    void* usedby,
                    qhandle_t shader,
                    vec3_t pos,
                    int trailLife,
                    ...
                    float startWidth,
                    float endWidth);
int CG_AddSmokeJunc(int headJuncIndex,
                    void* usedby,
                    qhandle_t shader,
                    vec3_t pos,
                    int trailLife,
...
int CG_SoundPlaySoundScript(const char* name,
                            vec3_t org,
                            int entnum,
                            qboolean buffer);
...
// Ridah, flamethrower
void CG_FireFlameChunks(centity_t* cent,
                        vec3_t origin,
                        vec3_t angles,
                        float speedScale,
                        qboolean firing);
...
// cg_effects.c
//
int CG_GetOriginForTag(centity_t* cent,
                       refEntity_t* parent,
                       char* tagName,
                       int startIndex,
                       vec3_t org,
...
//----(SA)
void CG_Spotlight(centity_t* cent,
                  float* color,
                  vec3_t start,
                  vec3_t dir,
                  int segs,
                  ...
                  int flags);
#define SL_NOTRACE \
  0x001  // don't do a trace check for shortening the beam, always draw at full
         // 'range' length
...
#define SL_LOCKTRACETORANGE \
  0x010  // only trace out as far as the specified range (rather than to max
         // spot range)
#define SL_NOFLARE \
  0x020  // don't draw a flare when the light is pointing at the camera
#define SL_NOIMPACT 0x040  // don't draw the impact mark
#define SL_LOCKUV \
  0x080  // lock the texture coordinates at the 'true' length of the requested
         // beam.
...
void trap_Cvar_VariableStringBuffer(const char* var_name,
                                    char* buffer,
                                    int bufsize);
void trap_Cvar_LatchedVariableStringBuffer(const char* var_name,
                                           char* buffer,
                                           int bufsize);
...
void trap_CM_CapsuleTrace(trace_t* results,
                          const vec3_t start,
                          const vec3_t end,
                          const vec3_t mins,
                          const vec3_t maxs,
...
void trap_R_ProjectDecal(qhandle_t hShader,
                         int numPoints,
                         vec3_t* points,
                         vec4_t projection,
                         vec4_t color,
...
void trap_S_StartSound(vec3_t origin,
                       int entityNum,
                       int entchannel,
                       sfxHandle_t sfx);
void trap_S_StartSoundVControl(vec3_t origin,
                               int entityNum,
                               int entchannel,
                               sfxHandle_t sfx,
                               int volume);
void trap_S_StartSoundEx(vec3_t origin,
                         int entityNum,
                         int entchannel,
                         sfxHandle_t sfx,
                         int flags);
void trap_S_StartSoundExVControl(vec3_t origin,
                                 int entityNum,
                                 int entchannel,
                                 sfxHandle_t sfx,
                                 int flags,
                                 int volume);
void trap_S_StopStreamingSound(
    int entnum);  // usually AI.  character is talking and needs to be shut up
                  // /now/
...
void trap_S_AddLoopingSound(const vec3_t origin,
                            const vec3_t velocity,
                            sfxHandle_t sfx,
                            int volume,
                            int soundTime);
void trap_S_AddRealLoopingSound(const vec3_t origin,
                                const vec3_t velocity,
                                sfxHandle_t sfx,
                                int range,
                                int volume,
...
void trap_S_Respatialize(int entityNum,
                         const vec3_t origin,
                         vec3_t axis[3],
                         int inwater);
sfxHandle_t trap_S_RegisterSound(
    const char* sample,
...
int trap_S_StartStreamingSound(const char* intro,
                               const char* loop,
                               int entnum,
                               int channel,
                               int attenuation);
...
qhandle_t trap_R_RegisterModel(
    const char* name);  // returns rgb axis if not found
qhandle_t trap_R_RegisterSkin(
    const char* name);  // returns all white if not found
qhandle_t trap_R_RegisterShader(
    const char* name);  // returns all white if not found
qhandle_t trap_R_RegisterShaderNoMip(
    const char* name);  // returns all white if not found

qboolean trap_R_GetSkinModel(qhandle_t skinid,
                             const char* type,
                             char* name);  //----(SA) added
qhandle_t trap_R_GetShaderFromModel(qhandle_t modelid,
                                    int surfnum,
...
// Ridah
void trap_R_AddPolysToScene(qhandle_t hShader,
                            int numVerts,
                            const polyVert_t* verts,
                            int numPolys);
// done.
void trap_R_AddLightToScene(const vec3_t org,
                            ...
                            qhandle_t hShader,
                            int flags);
void trap_R_AddCoronaToScene(const vec3_t org,
                             float r,
                             float g,
                             float b,
                             float scale,
...
void trap_R_DrawStretchPic(float x,
                           ...
                           float t2,
                           qhandle_t hShader);
void trap_R_DrawRotatedPic(float x,
                           float y,
                           float w,
                           float h,
                           float s1,
                           ...
                           qhandle_t hShader,
                           float angle);  // NERVE - SMF
void trap_R_DrawStretchPicGradient(float x,
                                   float y,
                                   float w,
                                   float h,
                                   float s1,
...
int trap_R_LerpTag(orientation_t* tag,
...
void trap_R_SetFog(int fogvar,
                   ...
                   float b,
                   float density);
void trap_R_SetGlobalFog(qboolean restore,
                         int duration,
                         float r,
                         float g,
                         float b,
...
void trap_SetUserCmdValue(int stateValue,
                          int flags,
...
qboolean trap_getCameraInfo(int camNum,
                            int time,
                            vec3_t* origin,
                            vec3_t* angles,
                            float* fov);
...
void CG_FitTextToWidth_Ext(char* instr,
                           float scale,
                           float w,
                           int size,
                           fontInfo_t* font);
...
void CG_mvWindowOverlay(int pID,
                        float b_x,
                        float b_y,
                        float b_w,
                        float b_h,
...
polyBuffer_t* CG_PB_FindFreePolyBuffer(qhandle_t shader,
                                       int numVerts,
                                       int numIndicies);
...
void CG_AddPMItem(popupMessageType_t type,
                  const char* message,
                  qhandle_t shader);
void CG_AddPMItemBig(popupMessageBigType_t type,
                     const char* message,
                     qhandle_t shader);
...
void CG_LoadPanel_DrawPin(const char* text,
                          float px,
                          float py,
                          float sx,
                          float sy,
