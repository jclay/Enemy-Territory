...
void CG_DrawConnectScreen(qboolean interactive, qboolean forcerefresh) {
  ...
  if (*buffer) {
    ...
    float x, y;
    int i;
    //		vec4_t clr1 = { 41/255.f,	51/255.f,	43/255.f,
    //204/255.f }; 		vec4_t clr2 = { 0.f,		0.f,		0.f,
    //225/255.f };
    ...
    y = 322;
    CG_Text_Paint_Centred_Ext(540, y, 0.22f, 0.22f, clr3, "SERVER INFO", 0, 0,
                              0, &bg_loadscreenfont1);

    y = 340;
    str = Info_ValueForKey(buffer, "sv_hostname");
    CG_Text_Paint_Centred_Ext(540, y, 0.2f, 0.2f, colorWhite,
    ...
    for (i = 0; i < MAX_MOTDLINES; i++) {
      ...
      CG_Text_Paint_Centred_Ext(540, y, 0.2f, 0.2f, colorWhite, str, 0, 26, 0,
                                &bg_loadscreenfont2);
...
void CG_LoadPanel_RenderCampaignTypeText(panel_button_t* button) {
  ...
  */
  CG_Text_Paint_Ext(button->rect.x, button->rect.y, button->font->scalex,
                    ...
                    va("%s:", CG_LoadPanel_GameTypeName(cgs.gametype)), 0, 0,
...
void CG_LoadPanel_RenderCampaignNameText(panel_button_t* button) {
  ...
  if (cgs.gametype == GT_WOLF_CAMPAIGN) {
    ...
    cs = va("%s %iof%i", cs, cgs.currentCampaignMap + 1,
    ...
    CG_Text_Paint_Ext(button->rect.x + (button->rect.w - w) * 0.5f,
                      button->rect.y, button->font->scalex,
                      button->font->scaley, button->font->colour, cs, 0, 0, 0,
                      button->font->font);
    ...
    w = CG_Text_Width_Ext(cgs.arenaData.longname, button->font->scalex, 0,
                          button->font->font);
    CG_Text_Paint_Ext(button->rect.x + (button->rect.w - w) * 0.5f,
                      button->rect.y, button->font->scalex,
                      button->font->scaley, button->font->colour,
                      cgs.arenaData.longname, 0, 0, 0, button->font->font);
  }
}
...
void CG_LoadPanel_DrawPin(const char* text,
  ...
  if (px + 30 + w > 440) {
    DC->fillRect(px - w - 28 + 2, py - (backheight / 2.f) + 2, 28 + w,
                 backheight, colourFadedBlack);
    DC->fillRect(px - w - 28, py - (backheight / 2.f), 28 + w, backheight,
                 colorBlack);
  } else {
    DC->fillRect(px + 2, py - (backheight / 2.f) + 2, 28 + w, backheight,
                 colourFadedBlack);
  ...
  if (px + 30 + w > 440) {
    DC->drawTextExt(px - 12 - w - 28, py + 4, sx, sy, colorWhite, text, 0, 0, 0,
                    &bg_loadscreenfont2);
  } else {
    DC->drawTextExt(px + 16, py + 4, sx, sy, colorWhite, text, 0, 0, 0,
                    &bg_loadscreenfont2);
  }
}
