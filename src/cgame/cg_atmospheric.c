...
qboolean CG_AtmosphericKludge() {
  ...
  80,W=1 2,D=1000 1000" ); CG_EffectParse( "T=RAIN,B=5 10,C=0.5,G=0.5 2,BV=50
  50,GV=200 200,W=1 2,D=1000" ); return( kludgeResult = qtrue );
  }*/
  ...
  //		CG_EffectParse( "T=SNOW,B=5 10,C=0.5,G=0.3 2,BV=50 50,GV=30
  80,W=1 2,D=5000" );
  ...
  40,W=3 5,D=500" );
  //		CG_EffectParse( "T=SNOW,B=5 10,C=0.5,G=0.3 2,BV=20 30,GV=25
  40,W=3 5,D=750" ); return( kludgeResult = qtrue );
          }*/

  /*if( !Q_stricmp( cgs.mapname, "maps/mp_goliath.bsp" ) ) {
          //CG_EffectParse( "T=SNOW,B=5 7,C=0.2,G=0.1 5,BV=15 25,GV=25 40,W=3
  5,D=400" ); CG_EffectParse( "T=SNOW,B=5 7,C=0.2,G=0.1 5,BV=15 25,GV=25 40,W=3
  5,H=512,D=2000" ); return( kludgeResult = qtrue );
  }*/
...
typedef enum { ACT_NOT, ACT_FALLING } active_t;
...
typedef struct cg_atmosphericEffect_s {
  ...
  qboolean (*ParticleGenerate)(cg_atmosphericParticle_t* particle,
                               vec3_t currvec,
                               float currweight);
...
static qboolean CG_SetParticleActive(cg_atmosphericParticle_t* particle,
                                     active_t active) {
...
static qboolean CG_RainParticleGenerate(cg_atmosphericParticle_t* particle,
                                        vec3_t currvec,
                                        float currweight) {
  ...
  // make sure it doesn't fall from too far cause it then will go over our heads
  // ('lower the ceiling')
  if (cg_atmFx.baseHeightOffset > 0) {
    if (particle->pos[2] - cg.refdef_current->vieworg[2] >
        cg_atmFx.baseHeightOffset) {
      particle->pos[2] =
          cg.refdef_current->vieworg[2] + cg_atmFx.baseHeightOffset;
  ...
  // ydnar: rain goes in bursts
  {
    ...
    maxActiveDrops = 0.50 * cg_atmFx.numDrops +
                     0.001 * cg_atmFx.numDrops * (10000 - (cg.time % 10000));
  ...
  //	particle->effectshader = &cg_atmFx.effectshaders[ (int) (random() * (
...
static qboolean CG_RainParticleCheckVisible(
  ...
  moved = (cg.time - cg_atmFx.lastRainTime) *
          0.001;  // Units moved since last frame
  VectorMA(particle->pos, moved, particle->delta, particle->pos);
  if (particle->pos[2] + particle->height <
      BG_GetSkyGroundHeightAtPoint(particle->pos)) {
    //		checkvisibletime += trap_Milliseconds() - msec;
  ...
  if ((distance[0] * distance[0] + distance[1] * distance[1]) >
      Square(MAX_ATMOSPHERIC_DISTANCE)) {
...
static qboolean CG_SnowParticleGenerate(cg_atmosphericParticle_t* particle,
                                        vec3_t currvec,
                                        float currweight) {
...
static qboolean CG_SnowParticleCheckVisible(
    cg_atmosphericParticle_t* particle) {
...
static void CG_EffectGust() {
  ...
  cg_atmFx.baseEndTime =
      cg.time + cg_atmFx.baseMinTime +
      (rand() % (cg_atmFx.baseMaxTime - cg_atmFx.baseMinTime));
  diff = cg_atmFx.changeMaxTime - cg_atmFx.changeMinTime;
  cg_atmFx.gustStartTime = cg_atmFx.baseEndTime + cg_atmFx.changeMinTime +
                           (diff ? (rand() % diff) : 0);
  diff = cg_atmFx.gustMaxTime - cg_atmFx.gustMinTime;
  cg_atmFx.gustEndTime = cg_atmFx.gustStartTime + cg_atmFx.gustMinTime +
                         (diff ? (rand() % diff) : 0);
  diff = cg_atmFx.changeMaxTime - cg_atmFx.changeMinTime;
  cg_atmFx.baseStartTime = cg_atmFx.gustEndTime + cg_atmFx.changeMinTime +
                           (diff ? (rand() % diff) : 0);
}
...
void CG_EffectParse(const char* effectstr) {
  ...
  if (atmFXType == ATM_NONE ||
      !BG_LoadTraceMap(cgs.rawmapname, cg.mapcoordsMins, cg.mapcoordsMaxs)) {
    // No effects
  ...
  cg_atmFx.numDrops = (cg_atmFx.baseDrops > cg_atmFx.gustDrops)
                          ? cg_atmFx.baseDrops
                          : cg_atmFx.gustDrops;
  ...
  for (count = 0; count < cg_atmFx.numDrops; count++)
    cg_atmFx.particles[count].nextDropTime =
        ATMOSPHERIC_DROPDELAY + (rand() % ATMOSPHERIC_DROPDELAY);
...
void CG_AddAtmosphericEffects() {
  ...
  //	getgroundtime = getskytime = rendertime = checkvisibletime =
  //generatetime = 0; 	n_getgroundtime = n_getskytime = n_rendertime =
  //n_checkvisibletime = n_generatetime = 0;

  VectorSet(cg_atmFx.viewDir, cg.refdef_current->viewaxis[0][0],
            cg.refdef_current->viewaxis[0][1], 0.f);
  ...
  //cg_atmFx.dropsActive, cg_atmFx.dropsCreated, cg_atmFx.dropsRendered,
  //cg_atmFx.dropsSkipped ); 	CG_Printf( "gg: %i gs: %i rt: %i cv: %i ge: %i\n",
  //getgroundtime, getskytime, rendertime, checkvisibletime, generatetime );
  //	CG_Printf( "\\-> %i \\-> %i \\-> %i \\-> %i \\-> %i\n", n_getgroundtime,
  //n_getskytime, n_rendertime, n_checkvisibletime, n_generatetime );
}
