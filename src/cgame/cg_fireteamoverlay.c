...
void CG_ParseFireteams() {
  ...
  //qfalse; 	qboolean isLeader =		CG_IsFireTeamLeader( cg.clientNum ) ?
  //qtrue : qfalse;
...
void CG_DrawFireTeamOverlay(rectDef_t* rect) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    /*		CG_DrawPic(x + 2, y + 2, FT_BAR_HEIGHT - 4, FT_BAR_HEIGHT - 4,
       cgs.media.movementAutonomyIcons[0]); x += FT_BAR_HEIGHT;

                    CG_DrawPic(x + 2, y + 2, FT_BAR_HEIGHT - 4, FT_BAR_HEIGHT -
       4, cgs.media.weaponAutonomyIcons[0]); x += FT_BAR_HEIGHT; x += 4;*/

    /*		if( isLeader ) {
                            CG_Text_Paint_Ext(x, y + FT_BAR_HEIGHT, .2f, .2f,
       tclr, va("%i", i+4), 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont2
       );
                    }*/
    x += 20;
...
const char* CG_FireteamGetBoxText() {
  ...
  if (cgs.propositionEndTime > cg.time) {
    ...
    return va("Accept %s's proposition to invite %s to join your fireteam?",
