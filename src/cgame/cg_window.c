...
vec4_t colorGreen2 = {0.305f, 0.475f, 0.305f,
                      0.48f};  // Slightly off from default fill

void CG_createStatsWindow(void) {
  cg_window_t* sw = CG_windowAlloc(
      WFX_TEXTSIZING | WFX_FADEIN | /*WFX_SCROLLUP|*/ WFX_TRUETYPE, 110);
  ...
  //	sw->y = (cg.snap->ps.pm_type == PM_INTERMISSION) ? -20 : -7;	// Align
  //from bottom minus offset and height
  sw->x = (cg.snap->ps.pm_type == PM_INTERMISSION) ? 10 : 4;
  sw->y = (cg.snap->ps.pm_type == PM_INTERMISSION)
              ? -20
              : -100;  // Align from bottom minus offset and height
}

void CG_createTopShotsWindow(void) {
  cg_window_t* sw = CG_windowAlloc(
      WFX_TEXTSIZING | WFX_FLASH | WFX_FADEIN | WFX_SCROLLUP | WFX_TRUETYPE,
      190);
  ...
  sw->y = (cg.snap->ps.pm_type == PM_INTERMISSION)
              ? -20
              : -60;  // Align from bottom minus offset and height
...
{
        int i;
        cg_window_t *sw = CG_windowAlloc(WFX_TEXTSIZING|WFX_FADEIN|WFX_TRUETYPE,
250); const char *help[] = {
                "^2TAB      ^Nscores",
...
250); const helpType_t help[] =
        ...
        for(i=0; i<sizeof(help)/sizeof(helpType_t); i++) {
                if(help[i].cmd != NULL) {
                        int len = strlen(CG_getBindKeyName(help[i].cmd, buf,
sizeof(buf))); if(len > maxlen) { maxlen = len;
                        }
                }
        }
        ...
        for(i=0; i<sizeof(help)/sizeof(helpType_t); i++) {
                if(help[i].cmd == NULL) {
                        ...
                        CG_printWindow(va(format, CG_getBindKeyName(help[i].cmd,
buf, sizeof(buf)), help[i].info));
                }
        }
}
*/
...
void CG_demoAviFPSDraw(void) {
  qboolean fKeyDown = cgs.fKeyPressed[K_F1] | cgs.fKeyPressed[K_F2] |
                      cgs.fKeyPressed[K_F3] | cgs.fKeyPressed[K_F4] |
                      cgs.fKeyPressed[K_F5];

  if (cg.demoPlayback && fKeyDown && cgs.aviDemoRate >= 0) {
    CG_DrawStringExt(
        42, 425,
        ((cgs.aviDemoRate > 0) ? va("^3Record AVI @ ^7%d^2fps", cgs.aviDemoRate)
                               : "^1Stop AVI Recording"),
...
void CG_demoTimescaleDraw(void) {
  if (cg.demoPlayback && cgs.timescaleUpdate > cg.time &&
      demo_drawTimeScale.integer != 0) {
    ...
    CG_DrawStringExt(42, 400, s, colorWhite, qfalse, qtrue, SMALLCHAR_WIDTH,
                     SMALLCHAR_HEIGHT, 0);
  }
}
...
void CG_windowDraw(void) {
  ...
  qboolean fAllowMV =
      (cg.snap != NULL &&
       cg.snap->ps.pm_type != PM_INTERMISSION /*&& !cg.limboMenu*/);
  vec4_t* bg;
  ...
  // Mouse cursor position for MV highlighting (offset for cursor pointer
  // position) Also allow for swingcam toggling
  ...
  for (i = 0; i < cg.winHandler.numActiveWindows; i++) {
    ...
    if (w->state == WSTATE_START) {
      ...
      if (w->effects & WFX_SCROLLUP) {
        if (tmp > 0) {
          y += (480 - y) * tmp /
               w->targetTime;  //(100 * tmp / w->targetTime) / 100;
        } else {
      ...
      if (w->effects & WFX_SCROLLUP) {
        if (tmp > 0) {
          y = w->curY +
              (480 - w->y) * t_offset /
                  w->targetTime;  //(100 * t_offset / w->targetTime) / 100;
        }
        if (tmp < 0 || y >= 480) {
          w->state = WSTATE_OFF;
          fCleanup = qtrue;
    ...
    for (j = w->lineCount - 1; j >= 0; j--) {
      if (w->effects & WFX_TRUETYPE) {
        //				CG_Text_Paint(x, y + h, w->fontScale,
        //textColor, (char*)w->lineText[j], 0.0f, 0, 0);
        CG_Text_Paint_Ext(x, y + h, w->fontScaleX, w->fontScaleY, textColor,
                          (char*)w->lineText[j], 0.0f, 0, 0,
      ...
      if (!(w->effects & WFX_TRUETYPE)) {
        CG_DrawStringExt2(x, y + h, (char*)w->lineText[j], textColor, qfalse,
                          qtrue, w->fontWidth, w->fontHeight, 0);
      }
    }
  }
  ...
  if (cg.mvTotalClients > 0 && cg.time < cgs.cursorUpdate && fAllowMV) {
    // CG_DrawPic(cgs.cursorX - CURSOR_OFFSETX, cgs.cursorY - CURSOR_OFFSETY,
    // 32, 32, cgs.media.cursor);
...
void CG_windowNormalizeOnText(cg_window_t* w) {
  ...
  for (i = 0; i < w->lineCount; i++) {
    if (w->effects & WFX_TRUETYPE) {
      tmp = CG_Text_Width_Ext((char*)w->lineText[i], w->fontScaleX, 0,
                              &cgs.media.limboFont2);
    } else {
  ...
  for (i = 0; i < w->lineCount; i++) {
    if (w->effects & WFX_TRUETYPE) {
      w->lineHeight[i] = CG_Text_Height_Ext(
          (char*)w->lineText[i], w->fontScaleY, 0, &cgs.media.limboFont2);
    } else {
...
void CG_cursorUpdate(void) {
  ...
  if (!cg.demoPlayback) {
    ...
    ny = 480.0 / 65536.0 *
         ((int_m_pitch.value < 0.0) ? (65536.0 - cg_pmove.cmd.angles[0])
                                    : cg_pmove.cmd.angles[0]);
  ...
  for (i = wh->numActiveWindows - 1; i >= 0; i--) {
    ...
    if ((w->effects & WFX_MULTIVIEW) && w != cg.mvCurrentMainview) {
      ...
      // If the current window is selected, and the button is down, then allow
      // the update to occur, as quick mouse movements can move it past the
      // window borders
      if (!fFound && (((w->mvInfo & MV_SELECTED) && fSelect) ||
                      (!fSelect && nx >= w->x && nx < w->x + w->w &&
                       ny >= w->y && ny < w->y + w->h))) {
        ...
        if (fFound) {
          break;  // Small optimization: we've found a new window, and cleared
                  // the old focus
        }
      }
    }
  ...
  // General boundary check
    ...
    for (i = TEAM_AXIS; i <= TEAM_ALLIES; i++) {
      ...
      if (cgs.cursorX >= nx && cgs.cursorY >= ny &&
            ...
            if (fSelect) {
              if (w != NULL) {
                ...
                cg.mvCurrentMainview->mvInfo =
                    (cg.mvCurrentMainview->mvInfo & ~MV_PID) |
