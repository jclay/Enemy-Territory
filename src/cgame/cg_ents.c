...
======================
*/
void CG_PositionEntityOnTag(refEntity_t* entity,
...
void CG_AddLightstyle(centity_t* cent) {
  ...
  if (cent->dl_backlerp > 1) {  // we're moving on to the next frame
    ...
    if (cent->dl_oldframe >= stringlength) {
      ...
      if (cent->dl_oldframe < 3 &&
          cent->dl_sound) {  // < 3 so if an alarm comes back into the pvs it
                             // will only start a sound if it's going to be
                             // closely synced with the light, otherwise wait
                             // till the next cycle
  ...
  lightval =
      (lastch * (1.0 - cent->dl_backlerp)) + (nextch * cent->dl_backlerp);
...
// ydnar: dlight values go from 0-1.5ish
  ...
  //%	trap_R_AddLightToScene( cent->lerpOrigin, lightval, 1.0,
  //(float)r/255.0f, (float)g/255.0f, (float)b/255.0f, 0, 0 );	// overdraw
  //forced to 0 for now

  // ydnar: if the dlight has angles, then it is a directional global dlight
  if (cent->currentState.angles[0] || cent->currentState.angles[1] ||
      cent->currentState.angles[2]) {
    vec3_t normal;

    AngleVectors(cent->currentState.angles, normal, NULL, NULL);
    trap_R_AddLightToScene(normal, 256, lightval, (float)r / 255.0f,
                           (float)r / 255.0f, (float)r / 255.0f, 0,
                           REF_DIRECTED_DLIGHT);
  }
  // normal global dlight
  else {
    trap_R_AddLightToScene(cent->lerpOrigin, 256, lightval, (float)r / 255.0f,
                           (float)g / 255.0f, (float)b / 255.0f, 0, 0);
  }
}
...
static void CG_EntityEffects(centity_t* cent) {
    ...
    if (cent->currentState.eType == ET_SPEAKER) {
      ...
      if (cent->currentState.dmgFlags) {  // range is set
        trap_S_AddRealLoopingSound(cent->lerpOrigin, vec3_origin,
                                   ...
                                   cent->currentState.dmgFlags, volume,
                                   cent->soundTime);
      } else {
        trap_S_AddRealLoopingSound(cent->lerpOrigin, vec3_origin,
                                   cgs.gameSounds[cent->currentState.loopSound],
      ...
      v = cgs.inlineModelMidpoints[cent->currentState.modelindex];
      VectorAdd(cent->lerpOrigin, v, origin);
      trap_S_AddLoopingSound(origin, vec3_origin,
                             cgs.gameSounds[cent->currentState.loopSound],
                             cent->currentState.onFireStart, cent->soundTime);
    } else {
      trap_S_AddLoopingSound(cent->lerpOrigin, vec3_origin,
                             cgs.gameSounds[cent->currentState.loopSound], 255,
                             cent->soundTime);
    }
  ...
  if (cent->currentState.constantLight) {
    ...
    if (cent->dl_stylestring[0] != 0) {  // it's probably a dlight
      ...
      trap_R_AddLightToScene(cent->lerpOrigin, i, 1.0, (float)r / 255.0f,
                             (float)g / 255.0f, (float)b / 255.0f, 0, 0);
    }
  }
  ...
  if (CG_EntOnFire(cent)) {
    ...
    trap_S_AddLoopingSound(
        cent->lerpOrigin, vec3_origin, cgs.media.flameBlowSound,
    ...
    trap_S_AddLoopingSound(cent->lerpOrigin, vec3_origin, cgs.media.flameSound,
                           (int)(0.3 * 255.0 * (pow(cent->fireRiseDir[2], 2))),
                           0);
  }
  ...
  // DHM - Nerve :: If EF_SMOKING is set, emit smoke
    ...
    if (cent->lastTrailTime < cg.time) {
      ...
      //			dir[2] = 10 + rnd * 30;
      // jpw
      CG_SmokePuff(cent->lerpOrigin, dir, 15 + (random() * 10), 0.3 + rnd,
                   0.3 + rnd, 0.3 + rnd, 0.4, 1500 + (rand() % 500), cg.time,
                   cg.time + 500, 0, cgs.media.smokePuffShader);
    }
  }
  // dhm - end
  // JPW NERVE same thing but for smoking barrels instead of nasty server-side
  // effect from single player
    ...
    if (cent->lastTrailTime < cg.time) {
      ...
      CG_SmokePuff(cent->lerpOrigin, dir, 40 + random() * 70,  // 40+(rnd*40),
                   rnd * 0.1, rnd * 0.1, rnd * 0.1, 1,
                   2800 + (rand() % 4000),  // 2500+(random()*1500),
...
static void CG_General(centity_t* cent) {
  ...
  if (cent->currentState.eType == ET_MG42_BARREL) {
    ...
    if (cent->currentState.otherEntityNum == cg.snap->ps.clientNum &&
      ...
      // AnglesToAxis(
      // cg_entities[cent->currentState.otherEntityNum].lerpAngles, ent.axis );
      AnglesToAxis(cent->lerpAngles, ent.axis);
    }
    /*		{
                            vec3_t v;
    ...
    if (s1->torsoAnim) {
      if (cg.time >= cent->lerpFrame.frameTime) {
        ...
        while (cg.time >= cent->lerpFrame.frameTime &&
               // Mad Doc xkan, 11/18/2002 - if teamNum == 1, then we are
               // supposed to stop the animation when we reach the end of this
               // loop Gordon: 27/11/02: clientNum already does this. xkan,
               // 1/8/2003 - No, it does something a little different.
               !(s1->teamNum == 1 && cent->lerpFrame.frame + s1->frame ==
                                         s1->legsAnim + s1->torsoAnim)) {
      ...
      if (cent->lerpFrame.frameTime == cent->lerpFrame.oldFrameTime) {
        ...
        cent->lerpFrame.backlerp =
            1.0 -
            (float)(cg.time - cent->lerpFrame.oldFrameTime) /
                (cent->lerpFrame.frameTime - cent->lerpFrame.oldFrameTime);
      }
      ...
      //			CG_Printf( "Gamemodel: oldframe: %i frame: %i
...
qboolean CG_PlayerSeesItem(playerState_t* ps,
                           entityState_t* item,
                           int atTime,
                           int itemType) {
  ...
  vorigin[2] += ps->viewheight;  // get the view loc up to the viewheight
                                 //	eorigin[2] += 8;						// and
  //subtract the item's offset (that is used to place it on the ground)

  VectorSubtract(vorigin, eorigin, dir);

  dist = VectorNormalize(
      dir);  // dir is now the direction from the item to the player

  if (dist > 255) {
    return qfalse;  // only run the remaining stuff on items that are close
                    // enough
  }
  ...
  //	foo = -0.94f - (dist/255.0f) * 0.057f;	// (ranging from -0.94 to
  //-0.997) (it happened to be a pretty good range)
  foo = -0.94f - (dist * (1.0f / 255.0f)) *
                     0.057f;  // (ranging from -0.94 to -0.997) (it happened to
                              // be a pretty good range)
  ...
  // (SA) okay, everything else is okay, so do a bloody trace. (so coronas on
  // treasure doesn't show through walls) <sigh>
...
static void CG_Item(centity_t* cent) {
    ...
    if (hasStand) {  // first try to put the weapon on it's 'stand'
      ...
      if (es->eFlags & EF_SPINNING) {
        if (es->groundEntityNum == -1 ||
            !es->groundEntityNum) {  // (SA) spinning with a stand will spin the
                                     // stand and the attached weap (only when
                                     // in the air)
          ...
          VectorCopy(
              cent->lastLerpAngles,
              cent->lerpAngles);  // make a tossed weapon sit on the ground in a
                                  // position that matches how it was yawed
        }
      }
      ...
      // scale the stand to match the weapon scale ( the weapon will also be
      // scaled inside CG_PositionEntityOnTag() )
      ...
      if (es->eFlags &
          EF_SPINNING) {  // spinning will override the angles set by a stand
        if (es->groundEntityNum == -1 ||
            !es->groundEntityNum) {  // (SA) spinning with a stand will spin the
                                     // stand and the attached weap (only when
                                     // in the air)
  ...
  if (es->modelindex2) {  // modelindex2 was specified for the ent, meaning it
                          // probably has an alternate model (as opposed to the
                          // one in the itemlist) try to load it first, and if
                          // it fails, default to the itemlist model
    ent.hModel = cgs.gameModels[es->modelindex2];
  } else {
    // if( item->giType == IT_WEAPON && cg_items[es->modelindex].models[2])
    // // check if there's a specific model for weapon pickup placement
  ...
  //			Can't do it when item is registered since it wouldn't
  //know about replacement model
  ...
  // items without glow textures need to keep a minimum light value so they are
  // always visible
  ...
  if (cg_drawCrosshairPickups.integer) {
    ...
    if (CG_PlayerSeesItem(&cg.predictedPlayerState, es, cg.time,
                          item->giType)) {
      highlight = qtrue;
      ...
      if (item->giType == IT_TREASURE) {
        trap_R_AddCoronaToScene(cent->highlightOrigin, 1, 0.85, 0.5, 2,
                                ...
                                qtrue);  //----(SA)	add corona to treasure
      }
    } else {
      if (item->giType == IT_TREASURE) {
        trap_R_AddCoronaToScene(
            cent->highlightOrigin, 1, 0.85, 0.5, 2, cent->currentState.number,
            qfalse);  //----(SA)	"empty corona" for proper fades
      }
    }
    ...
    if (highlight) {
      ...
      ent.hilightIntensity =
          1.0f - ((cg.time - cent->highlightTime) / 1000.0f) *
                     highlightFadeScale;  // 1 sec to dim down (diff in time
                                          // causes problems if you quickly flip
                                          // to/away from looking at the item)
    }
...
===============
*/
static void CG_DrawMineMarkerFlag(centity_t* cent,
                                  refEntity_t* ent,
                                  const weaponInfo_t* weapon) {
  entityState_t* s1;
  ...
  ent->hModel = cent->currentState.otherEntityNum2 ? weapon->modModels[1]
                                                   : weapon->modModels[0];
  ...
  if (cent->lerpFrame.frameTime == cent->lerpFrame.oldFrameTime) {
    ...
    cent->lerpFrame.backlerp =
        1.0 - (float)(cg.time - cent->lerpFrame.oldFrameTime) /
                  (cent->lerpFrame.frameTime - cent->lerpFrame.oldFrameTime);
  }
...
static void CG_Missile(centity_t* cent) {
  ...
  } else if (s1->weapon == WP_SATCHEL &&
             s1->clientNum == cg.snap->ps.clientNum) {
  ...
  if (weapon->missileSound) {
    if (cent->currentState.weapon == WP_GPG40 ||
      ...
      if (!cent->currentState.effect1Time) {
        ...
        if (flytime > 300) {
          ...
          vec3_t velocity;
          int volume =
              flytime > 375 ? 255 : (75.f / ((float)flytime - 300.f)) * 255;

          BG_EvaluateTrajectoryDelta(&cent->currentState.pos, cg.time, velocity,
                                     qfalse, -1);
          trap_S_AddLoopingSound(cent->lerpOrigin, velocity,
                                 weapon->missileSound, volume, 0);
        }
      }
    } else {
      vec3_t velocity;
      ...
      BG_EvaluateTrajectoryDelta(&cent->currentState.pos, cg.time, velocity,
                                 qfalse, -1);
      trap_S_AddLoopingSound(cent->lerpOrigin, velocity, weapon->missileSound,
                             255, 0);
    }
  }
  ...
  if (cent->currentState.weapon == WP_DYNAMITE) {
    if (cent->currentState.teamNum < 4) {
      ...
      BG_EvaluateTrajectoryDelta(&cent->currentState.pos, cg.time, velocity,
                                 qfalse, -1);
      trap_S_AddLoopingSound(cent->lerpOrigin, velocity, weapon->spindownSound,
                             255, 0);
    }
  }
  ...
  if (cent->currentState.eType == ET_FP_PARTS) {
  ...
  } else if (cent->currentState.eType == ET_FIRE_COLUMN ||
             cent->currentState.eType == ET_FIRE_COLUMN_SMOKE) {
    ...
    team_t missileTeam = cent->currentState.weapon == WP_LANDMINE
                             ? cent->currentState.teamNum % 4
  ...
  if (cent->currentState.weapon == WP_LANDMINE) {
    ...
    if (cent->currentState.teamNum < 4) {
      ...
      if ((cgs.clientinfo[cg.snap->ps.clientNum].team !=
        ...
        if (cent->currentState.density - 1 == cg.snap->ps.clientNum) {
          ...
          if (cgs.clientinfo[cg.snap->ps.clientNum].skill[SK_BATTLE_SENSE] >=
              4) {
            vec_t distSquared = DistanceSquared(
                cent->lerpOrigin, cg.predictedPlayerEntity.lerpOrigin);
        ...
        ent.hilightIntensity = 0.5f * sin((cg.time-cent->highlightTime)/1000.f)
        + 1.f;*/
      }
    }
...
extern void CG_RunLerpFrame(centity_t* cent,
                            clientInfo_t* ci,
                            lerpFrame_t* lf,
                            int newAnimation,
                            float speedScale);
...
static void CG_TrapSetAnim(centity_t* cent, lerpFrame_t* lf, int newAnim) {
  ...
  // DHM - Nerve :: teamNum specifies which set of animations to use (only 1
  // exists right now)
...
==============
CG_Trap
        // TODO: change from 'trap' to something else.  'trap' is a misnomer.
it's actually used for other stuff too
==============
*/
static void CG_Trap(centity_t* cent) {
  ...
  CG_RunLerpFrame(cent, NULL, traplf, 0,
                  1);  // use existing lerp code rather than re-writing
...
static void CG_Corona(centity_t* cent) {
  ...
  if (dist >
      cg_coronafardist.integer) {  // performance variable cg_coronafardist will
                                   // keep down super long traces
    toofar = qtrue;
  }

  dot = DotProduct(dir, cg.refdef_current->viewaxis[0]);
  if (dot >= -0.6) {  // assumes ~90 deg fov	(SA) changed value to 0.6
                      // (screen corner at 90 fov)
    behind = qtrue;   // use the dot to at least do trivial removal of those
                      // behind you.
  }
  // yeah, I could calc side planes to clip against, but would that be worth it?
  // (much better than dumb dot>= thing?)
  ...
  if (!behind && !toofar) {
    CG_Trace(&tr, cg.refdef_current->vieworg, NULL, NULL, cent->lerpOrigin, -1,
             MASK_SOLID |
                 CONTENTS_BODY);  // added blockage by players.  not sure how
                                  // this is going to be since this is their bb,
                                  // not their model (too much blockage)
  ...
  if (s1->modelindex2) {
    if (cent->currentState.powerups == STATE_UNDERCONSTRUCTION) {
      ...
      /*switch( cent->currentState.frame ) {
              case 1: trap_S_AddLoopingSound( cent->currentState.origin2,
      vec3_origin, cgs.media.buildSound[0], 255, 0 ); break; case 2:
      trap_S_AddLoopingSound( cent->currentState.origin2, vec3_origin,
      cgs.media.buildSound[1], 255, 0 ); break; case 3: trap_S_AddLoopingSound(
      cent->currentState.origin2, vec3_origin, cgs.media.buildSound[2], 255, 0
      ); break; case 4: trap_S_AddLoopingSound( cent->currentState.origin2,
      vec3_origin, cgs.media.buildSound[3], 255, 0 ); break;
      }*/
    }
...
static void CG_Waypoint( centity_t *cent ) {
        ...
        switch( cent->currentState.frame ) {
                case WAYP_ATTACK: ent.customShader =
cgs.media.waypointAttackShader; break; case WAYP_DEFEND: ent.customShader =
cgs.media.waypointDefendShader; break; case WAYP_REGROUP: ent.customShader =
...
// declaration for add bullet particles (might as well stick this one in a .h
// file I think)
extern void CG_AddBulletParticles(vec3_t origin,
                                  vec3_t dir,
                                  int speed,
                                  int duration,
                                  int count,
...
static void CG_Mover(centity_t* cent) {
  ...
  /*	if(	cent->currentState.eType == ET_ALARMBOX) {
                  if(cent->currentState.frame == 2 ) {	// i'm dead
                          if(rand()%50 == 1) {
                                  vec3_t	angNorm;
     // normalized angles VectorNormalize2(cent->lerpAngles, angNorm);
                                  //		(origin, dir, speed, duration,
     count, 'randscale') CG_AddBulletParticles( cent->lerpOrigin, angNorm, 2,
     800, 4, 16.0f ); trap_S_StartSound (NULL, cent->currentState.number,
     CHAN_AUTO, cgs.media.sparkSounds );
                          }
                  }
          }*/
}
...
void CG_Mover_PostProcess(centity_t* cent) {
  ...
  if (cg.snap->ps.eFlags & EF_MOUNTEDTANK &&
      cg_entities[cg.snap->ps.clientNum].tagParent ==
          cent->currentState.effect3Time) {
    i = cg.snap->ps.clientNum;
  } else {
    for (i = 0; i < MAX_CLIENTS; i++) {
      // Gordon: is this entity mounted on a tank, and attached to _OUR_ turret
      // entity (which could be us)
  ...
  CG_AttachBitsToTank(&cg_entities[cent->currentState.effect3Time], &mg42base,
                      &mg42upper, &mg42gun, &player, &flash, angles,
                      "tag_player",
                      cent->currentState.density & 8 ? qtrue : qfalse);

  // Gordon: if we (or someone we're spectating) is on this tank, recalc our
  // view values
  if (cg.snap->ps.eFlags & EF_MOUNTEDTANK) {
    centity_t* tank =
...
void CG_Beam_2(centity_t* cent) {
  ...
  //(int)origin[2], (int)origin2[0], (int)origin2[1], (int)origin2[2] );
  AxisClear(ent.axis);
...
void CG_SetupCabinets(void) {
  ...
  for (i = 0; i < CT_MAX; i++) {
    ...
    for (j = 0; j < MAX_CABINET_TAGS; j++) {
      cabinetInfo[i].itemmodels[j] =
          trap_R_RegisterModel(cabinetInfo[i].itemnames[j]);
    }
  }
}
...
void CG_AdjustPositionForMover(const vec3_t in,
  ...
  if (!(cent->currentState.eFlags & EF_PATH_LINK)) {
    BG_EvaluateTrajectory(&cent->currentState.pos, fromTime, oldOrigin, qfalse,
                          cent->currentState.effect2Time);
    BG_EvaluateTrajectory(&cent->currentState.apos, fromTime, oldAngles, qtrue,
                          cent->currentState.effect2Time);

    BG_EvaluateTrajectory(&cent->currentState.pos, toTime, origin, qfalse,
                          cent->currentState.effect2Time);
    BG_EvaluateTrajectory(&cent->currentState.apos, toTime, angles, qtrue,
                          cent->currentState.effect2Time);
...
static void CG_InterpolateEntityPosition(centity_t* cent) {
  ...
  if (cg.nextSnap == NULL) {
    // DHM - Nerve :: FIXME? There are some cases when in Limbo mode during a
    // map restart
...
static void CG_ProcessEntity(centity_t* cent) {
  switch (cent->currentState.eType) {
    ...
    case ET_CONSTRUCTIBLE:
      ...
      break;
      /*	case ET_WAYPOINT:
              // TAT - 8/29/2002 - draw the botgoal indicator the same way you
         draw the waypoint flag case ET_BOTGOAL_INDICATOR: CG_Waypoint( cent );
                      break;*/
...
void CG_AddCEntity(centity_t* cent) {
  ...
  // call the appropriate function which will add this entity to the view
  // accordingly
...
qboolean CG_AddLinkedEntity(centity_t* cent, qboolean ignoreframe, int atTime) {
  ...
  if (!(sParent->eFlags & EF_PATH_LINK)) {
    if (sParent->pos.trType == TR_LINEAR_PATH) {
      ...
      if (!(cent->backspline =
                BG_GetSplineData(sParent->effect2Time, &cent->back))) {
        return qfalse;
      }

      cent->backdelta = sParent->pos.trDuration
                            ? (atTime - sParent->pos.trTime) /
                                  ((float)sParent->pos.trDuration)
                            : 0;
      ...
      if (pos >= MAX_SPLINE_SEGMENTS) {
        ...
        frac = ((cent->backdelta * (MAX_SPLINE_SEGMENTS)) - pos) *
               cent->backspline->segments[pos].length;
      }

      VectorMA(cent->backspline->segments[pos].start, frac,
               cent->backspline->segments[pos].v_norm, v);
      if (sParent->apos.trBase[0]) {
        BG_LinearPathOrigin2(sParent->apos.trBase[0], &cent->backspline,
                             &cent->backdelta, v, cent->back);
      }

      VectorCopy(v, cent->lerpOrigin);

      if (s1->angles2[0]) {
        BG_LinearPathOrigin2(s1->angles2[0], &cent->backspline,
                             &cent->backdelta, v, cent->back);
      }
...
qboolean CG_AddEntityToTag(centity_t* cent) {
  ...
  if (cent->currentState.eType != ET_PLAYER) {
    if (!cent->currentState
             .density) {  // this entity should rotate with it's parent, but can
                          // turn around using it's own angles
...
void CG_AddPacketEntities(void) {
  ...
  // Gordon: changing to a single loop, child will request that their parents
  // are added first anyway
  for (num = 0; num < cg.snap->numEntities; num++) {
    CG_AddCEntity_Filter(&cg_entities[cg.snap->entities[num].number]);
  }

  // Gordon: Add tank bits as a second loop, to stop recursion problems with
  // tank bits not on base entity
  for (num = 0; num < cg.snap->numEntities; num++) {
    if (cg_entities[cg.snap->entities[num].number].currentState.eType ==
        ET_MOVER) {
...
void CG_AttachBitsToTank(centity_t* tank,
                         refEntity_t* mg42base,
                         refEntity_t* mg42upper,
                         refEntity_t* mg42gun,
                         refEntity_t* player,
