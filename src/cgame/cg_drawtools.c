...
void CG_AdjustFrom640(float* x, float* y, float* w, float* h) {
  ...
  /*	if ( (cg.showGameView) && cg.refdef_current->width ) {
                  float xscale = ( ( cg.refdef_current->width / cgs.screenXScale
     ) / 640.f ); float yscale = ( ( cg.refdef_current->height /
     cgs.screenYScale ) / 480.f );

                  (*x) = (*x) * xscale + ( cg.refdef_current->x /
     cgs.screenXScale );
                  (*y) = (*y) * yscale + ( cg.refdef_current->y /
     cgs.screenYScale );
                  (*w) *= xscale;
                  (*h) *= yscale;
          }*/
...
==============
*/
void CG_FillRectGradient(float x,
                         float y,
                         float width,
                         float height,
                         const float* color,
...
fractional value to the palyer by virtue of how full a drawn box is.

flags:
        left		- 1
        center		- 2		// direction is 'right' by default and
orientation is 'horizontal' vert		- 4 nohudalpha	- 8
// don't adjust bar's alpha value by the cg_hudalpha value bg
- 16	// background contrast box (bg set with bgColor of 'NULL' means use
default bg color (1,1,1,0.25) spacing		- 32	// some bars use
different sorts of spacing when drawing both an inner and outer box

        lerp color	- 256	// use an average of the start and end colors to
set the fill color
==============
*/
...
void CG_FilledBar(float x,
  ...
  if ((flags & BAR_BG) &&
      bgColor) {  // BAR_BG set, and color specified, use specified bg color
  ...
  // background
  if ((flags & BAR_BG)) {
    // draw background at full size and shrink the remaining box to fit inside
    // with a border.  (alternate border may be specified by a BAR_BGSPACING_xx)
  ...
  if (flags & BAR_VERT) {
    ...
    if (flags & BAR_LERP_COLOR) {
      ...
      //			CG_FillRectGradient ( x, y, w, h * frac,
    ...
    if (flags & BAR_LERP_COLOR) {
      ...
      //			CG_FillRectGradient ( x, y, w * frac, h,
      //startColor, endColor, 0 );
...
void CG_DrawStringExt_Shadow(int x,
  ...
  if (shadow) {
    ...
    while (*s && cnt < maxChars) {
      ...
      CG_DrawChar2(xx + ((charWidth < 12) ? 1 : 2),
                   y + ((charHeight < 12) ? 1 : 2), charWidth, charHeight, *s);
      cnt++;
      xx += charWidth;
      s++;
    }
...
==================
*/
/*void CG_DrawStringExt2( int x, int y, const char *string, const float
*setColor, qboolean forceColor, qboolean shadow, int charWidth, int charHeight,
int maxChars ) { vec4_t		color; const char	*s; int
xx; int			cnt;
        ...
        while ( *s && cnt < maxChars) {
                if ( Q_IsColorString( s ) ) {
                        if ( !forceColor ) {
                                memcpy( color,
g_color_table[ColorIndex(*(s+1))], sizeof( color ) ); color[3] = setColor[3];
                                trap_R_SetColor( color );
                        }
                        s += 2;
                        continue;
...
static void UI_DrawProportionalString2(int x,
                                       int y,
                                       const char* str,
                                       vec4_t color,
                                       float sizeScale,
