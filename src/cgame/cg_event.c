  ...
  if (attacker == cg.snap->ps.clientNum) {
    ...
    if (ci->team == ca->team) {
      if (mod == MOD_SWAP_PLACES) {
        s = va("%s %s", CG_TranslateString("You swapped places with"),
               targetName);
      } else {
        s = va("%s %s", CG_TranslateString("You killed ^1TEAMMATE^7"),
               targetName);
      }
    } else {
  ...
  if (ca) {
    switch (mod) {
      case MOD_KNIFE:
        ...
        message2 = "'s knife";
        // OSP - goat luvin
        //			if( attacker == cg.snap->ps.clientNum || target
        //== cg.snap->ps.clientNum ) { 				trap_S_StartSound( cg.snap->ps.origin,
        //cg.snap->ps.clientNum, CHAN_AUTO, cgs.media.goatAxis );
        //			}
        break;
    ...
    if (message) {
      ...
      if (message2) {
        ...
        CG_AddPMItem(
            PM_DEATH,
            va("%s %s %s%s", targetName, message, attackerName, message2),
            deathShader);
...
static void CG_ItemPickup(int itemNum) {
  ...
  if (bg_itemlist[itemNum].giType == IT_WEAPON) {
    if (cg_autoswitch.integer &&
      ...
      // Gordon: Leave autoswitch to secondary kar/carbine as they use alt ammo
      // and arent zoomed: Note, not that it would do this anyway as it isnt in
      // a bank....
      if (itemid != WP_FG42SCOPE && itemid != WP_GARAND_SCOPE &&
        ...
        // 1 - always switch to new weap (Q3A default)
          ...
          // 2 - switch to weap if it's not already in the player's inventory
          // (Wolf default) 4 - both 2 and 3

          // FIXME:	this works fine for predicted pickups (when you walk
          // over the weapon), but not for
          ...
          if (cg_autoswitch.integer == 3 || cg_autoswitch.integer == 4) {
            // switch away only if a primary weapon is selected (read: don't
            // switch away if current weap is a secondary mode)
...
void CG_Explodef(vec3_t origin,
                 vec3_t dir,
                 int mass,
                 int type,
                 qhandle_t sound,
                 int forceLowGrav,
                 qhandle_t shader);
void CG_RubbleFx(vec3_t origin,
                 vec3_t dir,
                 int mass,
                 int type,
                 qhandle_t sound,
...
void CG_Explode(centity_t* cent, vec3_t origin, vec3_t dir, qhandle_t shader) {
  ...
  // inherit shader
  // (SA) FIXME: do this at spawn time rather than explode time so any new
  // necessary shaders are created earlier
  if (cent->currentState.eFlags & EF_INHERITSHADER) {
    if (!shader) {
      //			inheritmodel = cent->currentState.modelindex;
      inheritmodel =
          cgs.inlineDrawModel[cent->currentState
                                  .modelindex];  // okay, this should be better.
      if (inheritmodel) {
...
void CG_RubbleFx(vec3_t origin,
  ...
  if (pieces[0] > 20) {
    pieces[0] =
        20;  // cap some of the smaller bits so they don't get out of control
  }
  if (pieces[1] > 15) {
    pieces[1] = 15;
  }
    ...
    for (howmany = 0; howmany < pieces[i]; howmany++) {
      ...
      switch (type) {
        ...
        case 5:  // "rock"
          snd = LEBS_ROCK;
          if (i == 5) {
            hmodel = cgs.media.debRock[2];  // temporarily use the next smallest
                                            // rock piece
          } else if (i == 4) {
            ...
            hmodel =
                cgs.media
                    .debBlock[1];  // temporarily use the small block pieces
          } else {
            hmodel =
                cgs.media
                    .debBlock[0];  // temporarily use the small block pieces
          }
          if (i <= 2) {
      ...
      // as it turns out, i'm not sure if setting the re->axis here will
      ...
      if (type == 6) {                      // "fabric"
        le->pos.trType = TR_GRAVITY_FLOAT;  // the fabric stuff will change to
                                            // use something that looks better
      } else {
        if (!forceLowGrav &&
            rand() & 1) {  // if low gravity is not forced and die roll goes our
                           // way use regular grav
      ...
      //cent->currentState.angles2[1] || cent->currentState.angles2[2])
      if (le->angles.trBase[0] == 1 || le->angles.trBase[1] == 1 ||
        ...
        if (dir[2]) {
          le->pos.trDelta[2] =
              random() * 200 * materialmul;  // randomize sort of a lot so they
                                             // don't all land together
        } else {
...
==============
CG_Explodef
        made this more generic for spawning hits and breaks without needing a
*cent
==============
*/
void CG_Explodef(vec3_t origin,
...
==============
CG_Effect
        Quake ed -> target_effect (0 .5 .8) (-6 -6 -6) (6 6 6) fire explode
smoke debris gore lowgrav
==============
*/
void CG_Effect(centity_t* cent, vec3_t origin, vec3_t dir) {
  ...
  if (cent->currentState.eventParm & 7) {
    ...
    for (i = 0; i < 5; i++) {
      ...
      sprVel[2] += rand() % 50;
      //			CG_ParticleExplosion( 2, sprOrg, sprVel,
  ...
  if (cent->currentState.eventParm & 2) {  // explode
    ...
    //%	CG_ImpactMark( cgs.media.burnMarkShader, origin, dir, random()*360,
  ...
  if (cent->currentState.eventParm & 8) {  // rubble
    ...
    if (s && strlen(s) > 0) {
      sh = trap_R_RegisterShader(va(
          "textures/%s", s));  // FIXME: don't do this here.  only for testing
    }
...
CG_Shard
        ...
        We should keep this separate since there will be considerable
differences in the physical properties of shard vrs debris. not to mention the
fact there is no way we can quantify what type of effects the designers will
        potentially desire. If it is still possible to merge the functionality
of cg_shard into cg_explode at a latter time I would have no problem with that
...
void CG_Shard(centity_t* cent, vec3_t origin, vec3_t dir) {
  ...
  for (i = 0; i < howmany; i++) {
    ...
    if (type) {
      le->pos.trDelta[2] =
          (random() * 200) +
          100;  // randomize sort of a lot so they don't all land together
    } else {    // glass
      le->pos.trDelta[2] =
          (random() * 100) +
  ...
  if (dist) {
    ...
    if (gdist > 1200 && gdist < 8000) {  // 1200 is max cam shakey dist (2*600)
      ...
      VectorMA(cg.refdef_current->vieworg, 800, norm,
               gorg);  // non-distance falloff makes more sense; sfx2range was
                       // gdist*0.2
...
static void CG_StartFootStepSound(bg_playerclass_t* classInfo,
                                  entityState_t* es,
                                  sfxHandle_t sfx) {
  ...
  switch (event) {
    ...
    case EV_FOOTSTEP:
      ...
      if (es->eventParm != FOOTSTEP_TOTAL) {
        if (es->eventParm) {
          CG_StartFootStepSound(
              classInfo, es, cgs.media.footsteps[es->eventParm][footstepcnt]);
        } else {
          CG_StartFootStepSound(
              classInfo, es,
              cgs.media
    ...
    case EV_WATER_UNDER:
      ...
      //----(SA)	this fog stuff for underwater is really just a test for
      //feasibility of creating the under-water effect that way.
      //----(SA)	the related issues of load/savegames, death underwater,
      //etc. are not handled at all.
      //----(SA)	the actual problem, of course, is doing underwater stuff
      //when the water is very turbulant and you can't simply
      //----(SA)	do things based on the players head being above/below
      //the water brushes top surface. (since the waves can potentially be /way/
      //above/below that)
    ...
    case EV_WATER_CLEAR:
      ...
      // trap_S_StartSound (NULL, es->number, CHAN_AUTO, CG_CustomSound(
      // es->number, "*gasp.wav" ) );
      trap_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.watrOutSound);
      if (es->eventParm) {
    ...
    case EV_ITEM_PICKUP_QUIET:
      DEBUGNAME("EV_ITEM_PICKUP");
      {
        ...
        //----(SA)	draw the HUD items for a sec since this is a special
        //item
    ...
    case EV_WEAP_OVERHEAT:
      ...
      if (es->number == cg.snap->ps.clientNum) {
        ...
        cent->overheatTime =
            cg.time;  // used to make the barrels smoke when overheated
      }

      if (BG_PlayerMounted(es->eFlags)) {
        trap_S_StartSoundVControl(NULL, es->number, CHAN_AUTO,
                                  cgs.media.hWeaponHeatSnd, 255);
      } else if (es->eFlags & EF_MOUNTEDTANK) {
        if (cg_entities[cg_entities[cg_entities[es->number].tagParent]
                            .tankparent]
                .currentState.density &
            8) {
          trap_S_StartSoundVControl(NULL, es->number, CHAN_AUTO,
                                    cgs.media.hWeaponHeatSnd_2, 255);
        } else {
          trap_S_StartSoundVControl(NULL, es->number, CHAN_AUTO,
                                    cgs.media.hWeaponHeatSnd, 255);
        }
    ...
    case EV_SPINUP:
      DEBUGNAME("EV_SPINUP");
      trap_S_StartSound(NULL, es->number, CHAN_AUTO,
                        cg_weapons[es->weapon].spinupSound);
      break;
      // jpw
    case EV_EMPTYCLIP:
    ...
    case EV_FILL_CLIP:
      ...
      if (cgs.clientinfo[cg.clientNum].skill[SK_LIGHT_WEAPONS] >= 2 &&
        ...
        trap_S_StartSound(
            ...
            cg_weapons[es->weapon]
                .reloadSound);  // JPW NERVE following sherman's SP fix, should
                                // allow killing reload sound when player dies
      }
      break;
    ...
    case EV_CHANGE_WEAPON_2:
      ...
      if (es->number == cg.snap->ps.clientNum) {
        ...
        // client will get this message if reloading while using an alternate
        // weapon client should voluntarily switch back to primary at that point
        switch (es->weapon) {
          case WP_FG42SCOPE:
            newweap = WP_FG42;
            break;
    ...
    case EV_FIRE_WEAPON_MG42:
      ...
      if (gdist > 512 && gdist < 4096) {
        ...
        if (cg_entities
          ...
          trap_S_StartSoundEx(gorg, cent->currentState.number, CHAN_WEAPON,
    ...
    case EV_GRENADE_BOUNCE:
      ...
      // DYNAMITE // Gordon: or LANDMINE FIXME: change this? (mebe a metallic
      // sound)
      ...
      /*	case EV_FLAMEBARREL_BOUNCE:
                      ...
                      if ( rand() & 1 ) {
                              trap_S_StartSound (NULL, es->number, CHAN_AUTO,
         cgs.media.fbarrelexp1 ); } else { trap_S_StartSound (NULL, es->number,
         CHAN_AUTO,  cgs.media.fbarrelexp2 );
                      }
                      break;*/
    ...
    case EV_RAILTRAIL:
      CG_RailTrail(&cgs.clientinfo[es->otherEntityNum2], es->origin2,
                   es->pos.trBase,
                   es->dmgFlags);  //----(SA)	added 'type' field
      break;
    ...
    case EV_MISSILE_MISS:
      ...
      ByteToDir(es->eventParm, dir);
      CG_MissileHitWall(
          es->weapon, 0, position, dir,
          0);  // (SA) modified to send missilehitwall surface parameters
      if (es->weapon == WP_MORTAR_SET) {
        if (!es->legsAnim) {
          CG_MortarImpact(cent, position, 3, qtrue);
        } else {
    ...
    case EV_MISSILE_MISS_LARGE:
      ...
      if (es->weapon == WP_ARTY || es->weapon == WP_SMOKE_MARKER) {
        ...
        CG_MissileHitWall(
            VERYBIGEXPLOSION, 0, position, dir,
            0);  // (SA) modified to send missilehitwall surface parameters
      }
      break;
    ...
    case EV_GLOBAL_CLIENT_SOUND:
      ...
      if (cg.snap->ps.clientNum == es->teamNum) {
        s = CG_ConfigString(CS_SOUNDS + es->eventParm);
        if (!strstr(s, ".wav")) {
          if (CG_SoundPlaySoundScript(s, NULL, -1,
    ...
    case EV_STOPSTREAMINGSOUND:
      ...
      trap_S_StartSoundEx(
          NULL, es->number, CHAN_WEAPON, 0,
          SND_CUTOFF_ALL);  // kill weapon sound (could be reloading)
      break;
    ...
    case EV_FLAMETHROWER_EFFECT:
      CG_FireFlameChunks(cent, cent->currentState.origin,
                         cent->currentState.apos.trBase, 0.6, 2);
      break;
    ...
    case EV_AIRSTRIKEMESSAGE: {
      ...
      if (wav) {
        CG_SoundPlaySoundScript(wav, NULL, -1,
                                (es->effect1Time ? qfalse : qtrue));
      }
    } break;
...
skipEvent:
  ...
  // if we've added more events than can fit into the list, make sure we only
  // add them once
  if (cent->currentState.eventSequence < cent->previousEventSequence) {
    cent->previousEventSequence -=
        (1 << 8);  // eventSequence is sent as an 8-bit through network stream
  }
  if (cent->currentState.eventSequence - cent->previousEventSequence >
      MAX_EVENTS) {
  ...
  // set the event back so we don't think it's changed next frame (unless it
  // really has)
