...
int CG_Text_Width_Ext(const char* text,
                      float scale,
                      int limit,
                      fontInfo_t* font) {
  int count, len;
...
int CG_Text_Height_Ext(const char* text,
                       float scale,
                       int limit,
                       fontInfo_t* font) {
  int len, count;
...
void CG_Text_PaintChar_Ext(float x,
                           float y,
                           float w,
                           float h,
                           float scalex,
...
void CG_Text_PaintChar(float x,
                       float y,
                       float width,
                       float height,
                       float scale,
...
void CG_Text_Paint_Centred_Ext(float x,
                               float y,
                               float scalex,
                               float scaley,
                               vec4_t color,
...
void CG_Text_Paint_Ext(float x,
                       float y,
                       float scalex,
                       float scaley,
                       vec4_t color,
    ...
    while (s && *s && count < len) {
      ...
      if (Q_IsColorString(s)) {
        ...
        if (style == ITEM_TEXTSTYLE_SHADOWED ||
          ...
          CG_Text_PaintChar_Ext(x + (glyph->pitch * scalex) + ofs,
                                y - yadj + ofs, glyph->imageWidth,
...
==============
*/
void CG_DrawKeyModel(int keynum,
                     float x,
                     float y,
                     float w,
                     float h,
...
static float CG_DrawFPS(float y) {
  ...
  if (index > FPS_FRAMES) {
    ...
    CG_Text_Paint_Ext(UPPERRIGHT_X - w, y + 11, 0.19f, 0.19f, tclr, s, 0, 0, 0,
...
static float CG_DrawTimer(float y) {
  ...
  rt = (cgs.gametype != GT_WOLF_LMS && cg_drawReinforcementTime.integer > 0)
           ? va("^F%d%s", CG_CalculateReinfTime(qfalse),
                ((cgs.timelimit <= 0.0f) ? "" : " "))
           : "";
  ...
  CG_Text_Paint_Ext(UPPERRIGHT_X - w, y + 11, 0.19f, 0.19f, color, s, 0, 0, 0,
                    &cgs.media.limboFont1);
...
static void CG_DrawTeamInfo(void) {
  ...
  if (cgs.teamLastChatPos != cgs.teamChatPos) {
    ...
    for (i = cgs.teamChatPos - 1; i >= cgs.teamLastChatPos; i--) {
      ...
      CG_Text_Paint_Ext(CHATLOC_TEXT_X,
                        CHATLOC_Y - (cgs.teamChatPos - i - 1) * lineHeight - 1,
                        0.2f, 0.2f, hcolor, cgs.teamChatMsgs[i % chatHeight], 0,
...
void CG_DrawLivesLeft(void) {
  ...
  CG_DrawPic(4, 360, 48, 24,
             cg.snap->ps.persistant[PERS_TEAM] == TEAM_ALLIES
                 ? cgs.media.hudAlliedHelmet
                 : cgs.media.hudAxisHelmet);

  CG_DrawField(44, 360, 3, cg.snap->ps.persistant[PERS_RESPAWNS_LEFT], 14, 20,
               qtrue, qtrue);
}
...
void CG_CenterPrint(const char* str, int y, int charWidth) {
  ...
  for (i = 0; i < len; i++) {
    // NOTE: subtract a few chars here so long words still get displayed
    // properly
...
==============
*/
void CG_PriorityCenterPrint(const char* str,
                            int y,
                            int charWidth,
                            int priority) {
  char* s;
...
static void CG_DrawWeapReticle(void) {
  ...
  if (fg) {
    ...
    /*		if(cgs.media.reticleShaderSimpleQ) {
       ...
       cgs.media.reticleShaderSimpleQ );	// tl trap_R_DrawStretchPic(
       x+w, 0, w, h, 1, 0, 0, 1, cgs.media.reticleShaderSimpleQ );	// tr
                            trap_R_DrawStretchPic( x,	h, w, h, 0, 1, 1, 0,
       cgs.media.reticleShaderSimpleQ );	// bl trap_R_DrawStretchPic(
       x+w, h, w, h, 1, 1, 0, 0, cgs.media.reticleShaderSimpleQ );	// br
                    }*/
...
static void CG_DrawMortarReticle(void) {
  ...
  for (val = i = 0; i < 36; i++) {
    ...
    if (i % 3 == majorOffset) {
      ...
      s = va("%i", printval);
      // CG_Text_Paint_Ext( 140 + localOffset - .5f * CG_Text_Width_Ext( s,
      // .15f, 0, &cgs.media.limboFont1 ), 244, .15f, .15f, color, s, 0, 0, 0,
      // &cgs.media.limboFont1 ); CG_FillRect( 140 + localOffset, 248, 1, 16,
      // color);
      CG_Text_Paint_Ext(
          500 - localOffset -
    ...
    if (requestFadeTime < 5000) {
      ...
      if (yaw < 0) {
        if (!hasLeftTarget) {
          ...
          CG_DrawPic(136 + 2, 236 + 38 - 10 + 1, 8, 8,
        ...
        if (!hasRightTarget) {
          ...
          trap_R_SetColor(color_firerequest);
          CG_DrawPic(350 + 154 - 10, 236 + 38 - 10 + 1, -8, 8,
                     cgs.media.ccMortarTargetArrow);
          trap_R_SetColor(NULL);
        ...
        trap_R_SetColor(color_firerequest);
        // CG_DrawPic( 320 + localOffset - 8, 264 - 8, 16, 16,
  ...
  /*s = va( "%.2f (%i / %i)",AngleNormalize360(angle - .5f * 180), majorOffset,
  min ); CG_Text_Paint( 140, 224, .25f, color, s, 0, 0, 0 ); s = va(
  "%.2f",AngleNormalize360(angle) ); CG_Text_Paint( 320 - .5f * CG_Text_Width(
  s, .25f, 0), 224, .25f, color, s, 0, 0, 0 ); s = va( "%.2f",
  AngleNormalize360(angle + .5f * 180) ); CG_Text_Paint( 500 - CG_Text_Width( s,
  .25f, 0 ), 224, .25f, color, s, 0, 0, 0 );*/
  ...
  for (val = i = 0; i < 20; i++) {
    ...
    if (i % 4 == majorOffset) {
      ...
      s = va("%i", printval);
      CG_Text_Paint_Ext(
          320 - .5f * CG_Text_Width_Ext(s, .15f, 0, &cgs.media.limboFont1),
          164 + localOffset +
...
static void CG_DrawBinocReticle(void) {
  // an alternative.  This gives nice sharp lines at the expense of a few extra
  // polys
  vec4_t color;
...
static void CG_DrawCrosshair(void) {
  ...
  switch (weapnum) {
    ...
    case WP_K43_SCOPE:
      if (!BG_PlayerMounted(cg.snap->ps.eFlags)) {
        // JPW NERVE -- don't let players run with rifles -- speed 80 == crouch,
        // 128 == walk, 256 == run
  ...
  if (cg_drawCrosshair.integer <
      0) {  //----(SA)	moved down so it doesn't keep the scoped weaps from
            //drawing reticles
    return;
  }
...
static float CG_ScanForCrosshairEntity(float* zChange, qboolean* hitClient) {
  ...
  *zChange = trace.endpos[2] - start[2];
  ...
  if (trace.entityNum >= MAX_CLIENTS) {
    ...
    // this might have some side-effects, but none right now as the script_mover
    // is the only one that sets effect1Time
...
void CG_CheckForCursorHints(void) {
  ...
  if (cg.snap->ps
          .serverCursorHint) {  // server is dictating a cursor hint, use it.
  ...
  // From here on it's client-side cursor hints.  So if the server isn't sending
  // that info (as an option) then it falls into here and you can get basic
  // cursorhint info if you want, but not the detailed info the server sends.
...
static void CG_DrawCrosshairNames(void) {
  ...
  if (cg.crosshairClientNum > MAX_CLIENTS) {
    ...
    if (cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR) {
      if (cg_entities[cg.crosshairClientNum].currentState.eType == ET_MOVER &&
        ...
        CG_DrawSmallStringColor(320 - w / 2, 170, s, color);
                 ...
                 ET_CONSTRUCTIBLE_MARKER) {
        s = Info_ValueForKey(CG_ConfigString(CS_CONSTRUCTION_NAMES),
                             va("%i", cg.crosshairClientNum));
        if (*s) {
          w = CG_DrawStrlen(s) * SMALLCHAR_WIDTH;
          CG_DrawSmallStringColor(320 - w / 2, 170, s, color);
    ...
    if ((cg_entities[cg.crosshairClientNum].currentState.powerups &
      ...
      if (cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR &&
        ...
        playerClass = BG_ClassLetterForNumber(
        ...
        playerRank =
            cgs.clientinfo[cg.crosshairClientNum].team != TEAM_AXIS
                ? rankNames_Axis[cgs.clientinfo[cg.crosshairClientNum]
                                     .disguiseRank]
                : rankNames_Allies[cgs.clientinfo[cg.crosshairClientNum]
                                       .disguiseRank];
  ...
  // Mad Doc - TDF
  // changed this from early-exiting if true, to only executing most stuff if
  // false. We want to show debug info regardless
  ...
  if (!isTank &&
    ...
    // determine player class
    playerClass = BG_ClassLetterForNumber(
        cg_entities[cg.crosshairClientNum].currentState.teamNum);
  ...
  // draw the health bar
  //	if ( isTank || (cg.crosshairClientNum == cg.snap->ps.identifyClient &&
  //drawStuff && cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR )
  //)
  {
    vec4_t bgcolor;
    ...
    CG_FilledBar(320 - 110 /*w*/ / 2, 190, 110, 10, c, NULL, bgcolor, barFrac,
                 16);
  }
...
static void CG_DrawVote(void) {
  ...
  if (cgs.complaintEndTime > cg.time && !cg.demoPlayback &&
      cg_complaintPopUp.integer > 0 && cgs.complaintClient >= 0) {
    ...
    CG_DrawStringExt(8, 200, s, color, qtrue, qtrue, TINYCHAR_WIDTH,
                     TINYCHAR_HEIGHT, 80);

    s = va(CG_TranslateString("Press '%s' for YES, or '%s' for No"), str1,
           str2);
    CG_DrawStringExt(8, 214, s, color, qtrue, qtrue, TINYCHAR_WIDTH,
                     TINYCHAR_HEIGHT, 80);
    return;
  }
  ...
  if (cgs.autoFireteamCreateEndTime > cg.time &&
      cgs.autoFireteamCreateNum == -1) {
    Q_strncpyz(str1, BindingFromName("vote yes"), 32);
    Q_strncpyz(str2, BindingFromName("vote no"), 32);
  ...
  if (cgs.voteTime) {
    ...
    if (!(cg.snap->ps.eFlags & EF_VOTED)) {
      ...
      if (cgs.clientinfo[cg.clientNum].team != TEAM_AXIS &&
          cgs.clientinfo[cg.clientNum].team != TEAM_ALLIES) {
        s = CG_TranslateString("Cannot vote as Spectator");
      } else {
        s = va(CG_TranslateString("YES(%s):%i, NO(%s):%i"), str1, cgs.voteYes,
               str2, cgs.voteNo);
      }
      CG_DrawStringExt(8, 214, s, color, qtrue, qtrue, TINYCHAR_WIDTH,
                       TINYCHAR_HEIGHT, 60);
  ...
  if (cgs.complaintEndTime > cg.time && !cg.demoPlayback &&
    ...
    if (cgs.complaintClient == -1) {
      ...
      CG_DrawStringExt(8, 200, CG_TranslateString(s), color, qtrue, qtrue,
  ...
  if (cgs.invitationEndTime > cg.time && cgs.invitationClient < 0) {
    if (cgs.invitationClient == -1) {
      ...
      CG_DrawStringExt(8, 200, CG_TranslateString(s), color, qtrue, qfalse,
                       TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 80);
      return;
    }
...
static void CG_DrawSpectatorMessage(void) {
  ...
  CG_DrawStringExt(8, 154, str, colorWhite, qtrue, qtrue, TINYCHAR_WIDTH,
                   TINYCHAR_HEIGHT, 0);

  str2 = BindingFromName("+attack");
  str = va(CG_TranslateString("Press %s to follow next player"), str2);
  CG_DrawStringExt(8, 172, str, colorWhite, qtrue, qtrue, TINYCHAR_WIDTH,
                   TINYCHAR_HEIGHT, 0);
  ...
  CG_DrawStringExt(x, y, str, colorWhite, qtrue, qtrue, TINYCHAR_WIDTH,
                   TINYCHAR_HEIGHT, 0);
  y += TINYCHAR_HEIGHT;
#endif
}
...
static void CG_DrawLimboMessage(void) {
  ...
  // JPW NERVE
  str = (ps->persistant[PERS_RESPAWNS_LEFT] == 0)
            ? CG_TranslateString("No more reinforcements this round.")
            : va(CG_TranslateString("Reinforcements deploy in %d seconds."),
...
static qboolean CG_DrawFollow(void) {
  ...
  if (cg.snap->ps.pm_flags & PMF_LIMBO) {
    if (cgs.gametype != GT_WOLF_LMS) {
        ...
        if (cg.snap->ps.persistant[PERS_RESPAWNS_PENALTY] >= 0) {
          int deployTime =
              (cgs.clientinfo[cg.snap->ps.clientNum].team == TEAM_AXIS)
                  ? cg_redlimbotime.integer
                  : cg_bluelimbotime.integer;

          deployTime *= 0.001f;

          sprintf(
              deploytime,
              CG_TranslateString("Bonus Life! Deploying in %d seconds"),
              CG_CalculateReinfTime(qfalse) +
                  cg.snap->ps.persistant[PERS_RESPAWNS_PENALTY] * deployTime);
        } else {
          sprintf(deploytime,
                  CG_TranslateString("No more deployments this round"));
        }
      } else {
      ...
      CG_DrawStringExt(INFOTEXT_STARTX, 118, deploytime, colorWhite, qtrue,
                       qtrue, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 80);
    }
    ...
    if (cg.snap->ps.clientNum != cg.clientNum) {
      ...
      CG_DrawStringExt(INFOTEXT_STARTX, 136, deploytime, colorWhite, qtrue,
                       qtrue, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 80);
    }
  } else {
    CG_DrawStringExt(INFOTEXT_STARTX, 118, CG_TranslateString("Following"),
                     colorWhite, qtrue, qtrue, BIGCHAR_WIDTH / 2,
                     BIGCHAR_HEIGHT, 0);
...
static void CG_DrawWarmup(void) {
  ...
  if (!sec) {
    if ((cgs.gamestate == GS_WARMUP && !cg.warmup) ||
      ...
      w = CG_DrawStrlen(s1);
      CG_DrawStringExt(320 - w * 12 / 2, 188, s1, colorWhite, qfalse, qtrue, 12,
                       18, 0);
      ...
      if (!cg.demoPlayback &&
        ...
        w = CG_DrawStrlen(s2);
        CG_DrawStringExt(320 - w * cw / 2, 208, s2, colorWhite, qfalse, qtrue,
                         cw, (int)(cw * 1.5), 0);
      }
      ...
      /*	if ( !sec ) {
                      if ( cgs.gamestate == GS_WAITING_FOR_PLAYERS ) {
                              ...
                              s = CG_TranslateString( "Game Stopped - Waiting
         for more players" );

                              w = CG_DrawStrlen( s );
                              CG_DrawStringExt( 320 - w * 6, 120, s, colorWhite,
         qfalse, qtrue, 12, 18, 0 );

                              if( cg_gameType.integer != GT_WOLF_LMS ) {
                              s1 = va( CG_TranslateString( "Waiting for %i
         players" ), cgs.minclients ); s2 = CG_TranslateString( "or call a vote
         to start the match" );

                              w = CG_DrawStrlen( s1 );
                              CG_DrawStringExt( 320 - w * cw/2, 160, s1,
         colorWhite, qfalse, qtrue, cw, (int)(cw * 1.5), 0 );

                              w = CG_DrawStrlen( s2 );
                              CG_DrawStringExt( 320 - w * cw/2, 180, s2,
         colorWhite, qfalse, qtrue, cw, (int)(cw * 1.5), 0 );
                              }
      */
      return;
  ...
  if (cgs.gametype == GT_WOLF_STOPWATCH) {
    ...
    w = CG_DrawStrlen(s);
    CG_DrawStringExt(320 - w * cw / 2, 140, s, colorWhite, qfalse, qtrue, cw,
                     (int)(cw * 1.5), 0);

    w = CG_DrawStrlen(s1);
    CG_DrawStringExt(320 - w * cw / 2, 160, s1, colorWhite, qfalse, qtrue, cw,
                     (int)(cw * 1.5), 0);

    w = CG_DrawStrlen(s2);
    CG_DrawStringExt(320 - w * cw / 2, 180, s2, colorWhite, qfalse, qtrue, cw,
                     (int)(cw * 1.5), 0);
  }
}
...
static void CG_DrawFlashFade(void) {
  ...
  if (cgs.fadeStartTime + cgs.fadeDuration < cg.time) {
    ...
    elapsed = (time = trap_Milliseconds()) -
              lastTime;  // we need to use trap_Milliseconds() here since the
                         // cg.time gets modified upon reloading
    lastTime = time;
  ...
  // OSP - ugh, have to inform the ui that we need to remain blacked out (or
  // not)
...
static void CG_DrawFlashZoomTransition(void) {
  ...
  if (BG_PlayerMounted(cg.snap->ps.eFlags)) {
    ...
    // keep the timer fresh so when you remove yourself from the mg42, it'll
    // fade
    cg.zoomTime = cg.time;
    return;
  }
...
void CG_Fade(int r, int g, int b, int a, int time, int duration) {
  ...
  if (time <= 0) {  // do instantly
    ...
    cg.fadeTime =
        cg.time -
        1;  // set cg.fadeTime behind cg.time so it will start out 'done'
  } else {
...
=================
*/
void CG_DrawCompassIcon(float x,
                        float y,
                        float w,
                        float h,
                        vec3_t origin,
  ...
  // we're now putting the compass icons around the square automap instead of
  // the round compass
...
static void CG_DrawNewCompass(void) {
  ...
  ) { for ( i = 0; i < snap->numEntities; i++ ) { centity_t *cent =
  &cg_entities[ snap->entities[ i ].number ];

                          if ( cent->currentState.eType !=
  ET_EXPLOSIVE_INDICATOR ) { continue;
                          }

                          if ( cent->currentState.teamNum == 1 &&
  cg.predictedPlayerState.persistant[PERS_TEAM] == TEAM_AXIS ) continue; else if
  ( cent->currentState.teamNum == 2 &&
  cg.predictedPlayerState.persistant[PERS_TEAM] == TEAM_ALLIES ) continue;
  ...
  /*	if( !(cgs.ccFilter & CC_FILTER_DESTRUCTIONS) ) {
     ...
     PC_ENGINEER ) { for ( i = 0; i < snap->numEntities; i++ ) { centity_t *cent
     = &cg_entities[ snap->entities[ i ].number ];
     ...
     cg.predictedPlayerState.persistant[PERS_TEAM] && cent->currentState.teamNum
     != 3 ) continue;

                                  CG_DrawCompassIcon( basex, basey, basew,
     baseh, cg.predictedPlayerState.origin, cent->lerpOrigin,
  ...
  /*	if( !(cgs.ccFilter & CC_FILTER_WAYPOINTS) ) {
                  // draw waypoint icons
                  for ( i = 0; i < snap->numEntities; i++ ) {
                          centity_t *cent = &cg_entities[ snap->entities[ i
     ].number ];
                          ...
                          // see if the waypoint owner is someone that you
     accept waypoints from if( !CG_IsOnSameFireteam( cg.clientNum,
     cent->currentState.clientNum )) {  // TODO: change to fireteam continue;
                          }

                          switch( cent->currentState.frame ) {
                          case WAYP_ATTACK: CG_DrawCompassIcon( basex, basey,
     basew, baseh, cg.predictedPlayerState.origin,
     cent->currentState.pos.trBase, cgs.media.waypointCompassAttackShader );
     break; case WAYP_DEFEND: CG_DrawCompassIcon( basex, basey, basew, baseh,
...
static void CG_DrawPlayerStatus(void) {
  ...
  if (value3 >= 0) {
    ...
    CG_Text_Paint_Ext(
        640 - 22 - CG_Text_Width_Ext(buffer, .25f, 0, &cgs.media.limboFont1),
        480 - 1 * (16 + 2) + 12 - 4, .25f, .25f, colorWhite, buffer, 0, 0,
        ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1);
    //		CG_DrawPic( 640 - 2 * ( 12 + 2 ) - 16 - 4, 480 - 1 * ( 16 + 2 )
    //- 4, 16, 16, cgs.media.SPPlayerInfoAmmoIcon );
  } else if (value2 >= 0) {
...
static void CG_DrawPlayerStats(void) {
  ...
  CG_Text_Paint_Ext(SKILLS_X - 28 + 2, 480 - 4, 0.2f, 0.2f, colorWhite, "HP", 0,
                    0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1);
...
static void CG_Draw2D(void) {
  ...
  // bani - #127 - no longer cheat protected, we draw crosshair/reticle in non
  // demoplayback
...
void CG_ShakeCamera() {
  ...
  if (cg.time > cg.cameraShakeTime) {
    cg.cameraShakeScale =
        0;  // JPW NERVE all pending explosions resolved, so reset shakescale
    return;
  }
...
void CG_DrawActive(stereoFrame_t stereoView) {
  ...
  cg.refdef_current->glfog.registered =
      0;  // make sure it doesn't use fog from another scene
  ...
  if (cg.showGameView) {
    ...
    CG_Letterbox((LIMBO_3D_W / 640.f) * 100, (LIMBO_3D_H / 480.f) * 100,
                 qfalse);
  }
