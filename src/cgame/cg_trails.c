...
trailJunc_t* CG_SpawnTrailJunc(trailJunc_t* headJunc) {
  ...
  j->nextJunc =
      headJunc;  // if headJunc is NULL, then we'll just be the end of the list
...
int CG_AddTrailJunc(int headJuncIndex,
  ...
  if (sType == STYPE_REPEAT) {
    if (headJunc) {
      j->sTex = headJunc->sTex +
                ((Distance(headJunc->pos, pos) / sRatio) / j->widthEnd);
    } else {
      // FIXME: need a way to specify offset timing
      j->sTex =
          (animSpeed * (1.0 - ((float)(cg.time % 1000) / 1000.0))) / (sRatio);
      //			j->sTex = 0;
    }
  }
...
#define ST_RATIO 4.0  // sprite image: width / height
  ...
  if (headJunc) {
    j->sTex = headJunc->sTex +
              ((Distance(headJunc->pos, pos) / ST_RATIO) / j->widthEnd);
  } else {
    // first junction, so this will become the "tail" very soon, make it fade
    // out
    j->sTex = 0;
    j->alphaStart = 0.0;
    j->alphaEnd = 0.0;
...
#define TRAIL_FADE_FAR_SCALE 4.0
  ...
  if (trail->flags & TJFL_SPARKHEADFLARE) {
    polyBuffer_t* pPolyBuffer =
        CG_PB_FindFreePolyBuffer(cgs.media.sparkFlareShader, 4, 6);
    if (pPolyBuffer) {
      ...
      VectorMA(pPolyBuffer->xyz[pos], -j->width * 2, vup,
               pPolyBuffer->xyz[pos]);
      VectorMA(pPolyBuffer->xyz[pos], -j->width * 2, vright,
               pPolyBuffer->xyz[pos]);
      ...
      for (i = 0; i < 4; i++) {
        ...
        pPolyBuffer->color[pPolyBuffer->numVerts + i][3] =
            (unsigned char)(j->alpha * 255.0);
      }
  ...
  if (trail->sType == STYPE_STRETCH) {
    // sInc = ((1.0 - 0.1) / (float)(numJuncs));	// hack, the end of
    // funnel shows a bit of the start (looping)
    s = 0.05;
    // s = 0.05;
  ...
  while (jNext) {
    ...
    GetPerpendicularViewVector(cg.refdef_current->vieworg, j->pos, jNext->pos,
                               up);

    // if it's a crossover, draw it twice
    if (j->flags & TJFL_CROSSOVER) {
      if (iteration > 0) {
        ProjectPointOntoVector(cg.refdef_current->vieworg, j->pos, jNext->pos,
                               viewProj);
    ...
    // do fading when moving towards the projection point onto the trail segment
    // vector
    else if (!(j->flags & TJFL_NOCULL) &&
             (j->widthEnd > 4 || jNext->widthEnd > 4)) {
      ProjectPointOntoVector(cg.refdef_current->vieworg, j->pos, jNext->pos,
                             viewProj);
      ...
      if (viewDist < (TRAIL_FADE_CLOSE_DIST * TRAIL_FADE_FAR_SCALE)) {
        if (viewDist < TRAIL_FADE_CLOSE_DIST) {
          ...
          fadeAlpha = (viewDist - TRAIL_FADE_CLOSE_DIST) /
                      (TRAIL_FADE_CLOSE_DIST * TRAIL_FADE_FAR_SCALE);
        }
  ...
  if (trail->flags & TJFL_FIXDISTORT) {
    // build the list of outVerts, by dividing up the QUAD's into 4 Tri's each,
    // so as to allow
    ...
    // send the polygons
    // FIXME: is it possible to send a GL_STRIP here? We are actually sending 2x
    // the verts we really need to
    if (!(trail->flags & TJFL_NOPOLYMERGE)) {
