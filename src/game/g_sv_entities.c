...
//game, are are used simply 		as locational indicators - seek cover spots, and ai
//markers, for example 		This is an alternate system so those don't clutter up the
//real game entities
...
g_serverEntity_t* CreateServerEntity(gentity_t* ent) {
  ...
  if (newEnt) {
    ...
    //		these don't move, so we should only have to calc it once, the
...
// TAT 11/18/2002
//		For the (possibly temporary) system of loading a separate file
//of these server entities
...
void CreateServerEntityFromData(char* classname,
                                char* targetname,
                                char* target,
                                vec3_t origin,
                                int spawnflags,
