...
=====================
*/
static qboolean G_ParseAnimationFiles(bg_character_t* character,
  ...
  // parse the text
  BG_AnimParseAnimScript(character->animModelInfo, &level.animScriptData,
                         animationScript, text);
...
==================
*/
static qboolean G_CheckForExistingAnimModelInfo(
    const char* animationGroup,
    const char* animationScript,
  ...
  for (i = 0, trav = level.animScriptData.modelInfo; i < MAX_ANIMSCRIPT_MODELS;
       i++, trav++) {
...
qboolean G_RegisterCharacter(const char* characterFile,
  ...
  if (!G_CheckForExistingAnimModelInfo(characterDef.animationGroup,
                                       characterDef.animationScript,
                                       &character->animModelInfo)) {
    if (!G_ParseAnimationFiles(character, characterDef.animationGroup,
                               characterDef.animationScript)) {
  ...
  for (team = TEAM_AXIS; team <= TEAM_ALLIES; team++) {
    for (cls = PC_SOLDIER; cls < NUM_PLAYER_CLASSES; cls++) {
      ...
      if (!G_RegisterCharacter(character->characterFile, character)) {
        G_Error(
            "ERROR: G_RegisterPlayerClasses: failed to load character file "
            "'%s' for the %s %s\n",
...
void G_UpdateCharacter(gclient_t* client) {
  ...
  if (*s) {
    ...
    if (client->pers.characterIndex != characterIndex) {
      ...
      trap_GetConfigstring(CS_CHARACTERS + characterIndex, infostring,
                           MAX_INFO_STRING);
      if (!(client->pers.character = BG_FindCharacter(infostring))) {
        // not found - create it (this should never happen as we should have
        ...
        if (!G_RegisterCharacter(infostring, client->pers.character)) {
          G_Printf(S_COLOR_YELLOW
                   "WARNING: G_UpdateCharacter: failed to load character file "
                   "'%s' for %s\n",
      ...
      // xkan: this can only be done if the model really changed - otherwise,
      // the animation may get screwed up if we are in the middle of some
      // animation and we come into this function; plus, also reset the timer so
      // we can properly start the next animation
...
set_default_character:
  ...
  character =
      BG_GetCharacter(client->sess.sessionTeam, client->sess.playerType);
