...
// building
gentity_t* G_FindByTargetnameFast(gentity_t* from,
                                  const char* match,
                                  int hash) {
  gentity_t* max = &g_entities[level.num_entities];
...
void G_UseTargets(gentity_t* ent, gentity_t* activator) {
  ...
  while ((t = G_FindByTargetnameFast(t, ent->target, hash)) != NULL) {
    if (t == ent) {
      ...
      if (t->use) {
        // G_Printf ("ent->classname %s ent->targetname %s t->targetname %s
        // t->s.number %d\n", ent->classname, ent->targetname, t->targetname,
        // t->s.number);

        t->flags |=
            (ent->flags &
             FL_KICKACTIVATE);  // (SA) If 'ent' was kicked to activate, pass
                                // this along to it's targets.
                                //		It may become handy to put a
                                //"KICKABLE" flag in ents so that it knows
                                //whether to pass this along or not 		Right now,
                                //the only situation where it would be weird
                                //would be an invisible_user that is a 'button'
                                //near 		a rotating door that it triggers.  Kick
                                //the switch and the door next to it flies open.

        t->flags |= (ent->flags &
                     FL_SOFTACTIVATE);  // (SA) likewise for soft activation

        if (activator &&
            ((Q_stricmp(t->classname, "func_door") == 0) ||
             (Q_stricmp(t->classname, "func_door_rotating") == 0))) {
          // check door usage rules before allowing any entity to trigger a door
          // open
...
void G_AddEvent(gentity_t* ent, int event, int eventParm) {
  ...
  if (ent->client) {
    // NERVE - SMF - commented in - externalEvents not being handled properly in
    // Wolf right now
...
void G_SetEntState(gentity_t* ent, entState_t state) {
  ...
  switch (state) {
    ...
    case STATE_UNDERCONSTRUCTION:
      ...
      if (ent->s.eType !=
          ET_CONSTRUCTIBLE) {  // don't make nonsolid as we want to make them
                               // partially solid for staged construction
        ent->clipmask = 0;
      }
        ...
        if (g_gametype.integer == GT_WOLF_CAMPAIGN) {
          if (!mapFound &&
            ...
            if (g_campaigns[level.campaignCount].mapCount ==
              ...
              mapFound = qtrue;
              // trap_Cvar_Set( "g_currentCampaignMap", va( "%i",
              // g_campaigns[level.campaignCount].mapCount ) );
            }
...
void G_ParseCampaigns(void) {
  ...
  if (g_gametype.integer != GT_WOLF_CAMPAIGN) {
    ...
    // map isn't found in the current campaign, see if it's the first map in
    // another campaign
    for (i = 0; i < level.campaignCount; i++) {
      if (!Q_stricmp(g_campaigns[i].mapnames[0], level.rawmapname)) {
        // someone manually specified a /map command, and it's the first map in
        // a campaign
