...
int ClientNumberFromString(gentity_t* to, char* s) {
  ...
  if (fIsNumber) {
    ...
    if (cl->pers.connected != CON_CONNECTED) {
      CPx(to - g_entities,
          va("print \"Client[lof] %i [lon]is not active\n\"", idnum));
      return -1;
    }
    return (idnum);
  }
  ...
  for (t = TEAM_AXIS; t <= TEAM_ALLIES; t++) {
    ...
    for (i = 0; i < level.num_entities; i++, list++) {
      ...
      if (!(list->aiInactive & (1 << t))) {
        G_Printf(
            "%s (%s)",
            (list->scriptName ? list->scriptName
...
void Cmd_Give_f(gentity_t* ent) {
  ...
  /*if ( Q_stricmpn( name, "damage", 6) == 0)
  {
          if(amount) {
                  ...
                  if( *name ) {
                          ...
                          if( client >= 0 ) {
                                  G_Damage( &g_entities[client], ent, ent, NULL,
  NULL, amount, DAMAGE_NO_PROTECTION, MOD_UNKNOWN );
                          }
                  } else {
  ...
  //	"give allammo <n>" allows you to give a specific amount of ammo to /all/
  //weapons while 	allowing "give ammo <n>" to only give to the selected weap.
...
qboolean SetTeam(gentity_t* ent,
  ...
  if (team != TEAM_SPECTATOR) {
    ...
    if (g_noTeamSwitching.integer &&
      ...
      trap_SendServerCommand(clientNum,
                             "cp \"You cannot switch during a match, please "
  ...
  // DHM - Nerve :: Force players to wait 30 seconds before they can join a new
  // team. OSP - changed to 5 seconds Gordon: disabling if in dev mode: cuz it
  // sucks Gordon: bleh, half of these variables don't mean what they used to,
  // so this doesn't work
  /*	if ( !g_cheats.integer ) {
                   if ( team != oldTeam && level.warmupTime == 0 &&
     !client->pers.initialSpawn && ( (level.time - client->pers.connectTime) >
     10000 ) && ( (level.time - client->pers.enterTime) < 5000 ) && !force ) {
                          CP(va("cp \"^3You must wait %i seconds before joining
     ^3a new team.\n\" 3", (int)(5 - ((level.time -
     client->pers.enterTime)/1000)))); return qfalse;
                  }
          }*/
  // dhm
  ...
  // ydnar: restore old respawn count (players cannot jump from team to team to
  // regain lives)
  ...
  if (g_gamestate.integer == GS_PLAYING &&
    ...
    if (g_gametype.integer == GT_WOLF_LMS && level.numTeamClients[0] > 0 &&
      ...
      trap_SendServerCommand(clientNum,
                             "cp \"Will spawn next round, please wait.\n\"");
      limbo(ent, qfalse);
      return (qfalse);
    } else {
      int i;
...
void StopFollowing(gentity_t* ent) {
  ...
  // divert behaviour if TEAM_SPECTATOR, moved the code from SpectatorThink to
  // put back into free fly correctly (I am not sure this can be called in
  // non-TEAM_SPECTATOR situation, better be safe)
  if (ent->client->sess.sessionTeam == TEAM_SPECTATOR) {
    // drop to free floating, somewhere above the current position (that's the
    // client you were following)
    vec3_t pos, angle;
...
void Cmd_TeamBot_f(gentity_t* foo) {
  ...
  // remove any weapon info from the userinfo, so SetWolfSpawnWeapons() doesn't
  // reset the weapon as that
...
G_EntitySound
        ...
        Note that calling G_AddEvent(..., EV_GENERAL_SOUND, ...) has the danger
of the event never getting through to the client because the entity might not be
visible (unless it has the SVF_BROADCAST flag), so if you want to make sure the
sound is heard, call this function instead.
======================*/
void G_EntitySound(
...
void G_SayTo(gentity_t* ent,
  ...
  if (match_mutespecs.integer > 0 && ent->client->sess.referee == 0 &&  // OSP
    ...
    trap_SendServerCommand(
        other - g_entities,
        va("%s \"%s%c%c%s\" %i %i",
           mode == SAY_TEAM || mode == SAY_BUDDY ? "tchat" : "chat", name,
           Q_COLOR_ESCAPE, color, message, ent - g_entities, localize));
  }
}
...
void G_VoiceTo(gentity_t* ent,
  ...
  // bots respond with voiceonly, so we check for this so they dont keep
  ...
  if (mode == SAY_TEAM || mode == SAY_BUDDY) {
    CPx(other - g_entities,
        va("%s %d %d %d %s %i %i %i", cmd, voiceonly, ent - g_entities, color,
           id, (int)ent->s.pos.trBase[0], (int)ent->s.pos.trBase[1],
           (int)ent->s.pos.trBase[2]));
  } else {
    CPx(other - g_entities,
        va("%s %d %d %d %s", cmd, voiceonly, ent - g_entities, color, id));
  }
}

void G_Voice(gentity_t* ent,
             gentity_t* target,
             int mode,
             const char* id,
             qboolean voiceonly) {
  ...
  /*if(mode == SAY_ALL && (!Q_stricmp(id, "DynamiteDefused") || !Q_stricmp(id,
  "DynamitePlanted"))) { return;
  }*/
...
static void Cmd_Voice_f(gentity_t* ent,
  ...
  if (mode != SAY_BUDDY) {
    ...
    G_Voice(ent, NULL, mode, ConcatArgs(((arg0) ? 2 + index : 3 + index)),
            voiceonly);
  }
}
...
qboolean Cmd_CallVote_f(gentity_t* ent,
  ...
  if (!fRefCommand) {
    if (level.voteInfo.voteTime) {
      ...
      if (voteFlags.integer == VOTING_DISABLED) {
        ...
        return qfalse;
      } else if (vote_limit.integer > 0 &&
                 ent->client->pers.voteCount >= vote_limit.integer) {
        CP(
            va("cpm \"You have already called the maximum number of votes "
               "(%d).\n\"",
               vote_limit.integer));
        return qfalse;
  ...
  if (trap_Argc() > 1 && (i = G_voteCmdCheck(ent, arg1, arg2, fRefCommand)) !=
                             G_NOTFOUND) {  //  --OSP
    if (i != G_OK) {
      if (i == G_NOTFOUND) {
  ...
  if (fRefCommand) {
    //		level.voteInfo.voteYes = level.voteInfo.numVotingClients + 10;
    //// JIC :)
    ...
    if (level.voteInfo.vote_fn != G_Kick_v &&
        level.voteInfo.vote_fn != G_Mute_v) {
...
void Cmd_Vote_f(gentity_t* ent) {
  ...
  if (ent->client->pers.complaintEndTime > level.time &&
      g_gamestate.integer == GS_PLAYING && g_complaintlimit.integer) {
    ...
    if (msg[0] == 'y' || msg[1] == 'Y' || msg[1] == '1') {
      ...
      if (!cl->pers.localClient) {
        ...
        if (num <= 0 || !G_FindFreeComplainIP(cl, &ip)) {
          trap_DropClient(cl - level.clients,
                          "kicked after too many complaints.",
                          cl->sess.referee ? 0 : 300);
      ...
      trap_SendServerCommand(ent->client->pers.complaintClient,
                             va("cpm \"^1Warning^7: Complaint filed against "
                                "you by %s^* You have Lost XP.\n\"",
                                ent->client->pers.netname));
      trap_SendServerCommand(ent - g_entities, "complaint -1");

      AddScore(other, WOLF_FRIENDLY_PENALTY);

      G_LoseKillSkillPoints(other, ent->sound2to1, ent->sound1to2,
                            ent->sound2to3 ? qtrue : qfalse);
    } else {
      trap_SendServerCommand(ent->client->pers.complaintClient,
  ...
  if (level.voteInfo.vote_fn == G_Kick_v) {
    ...
    if (g_entities[pid].client->sess.sessionTeam != TEAM_SPECTATOR &&
        ent->client->sess.sessionTeam !=
            g_entities[pid].client->sess.sessionTeam) {
      trap_SendServerCommand(
          ent - g_entities,
...
void Cmd_StopCamera_f(gentity_t* ent) {
  ...
  if (ent->client->cameraPortal &&
    ...
    // RF, if we are near the spawn point, save the "current" game, for
    // reloading after death
    //		sp = NULL;
    // gcc: suggests () around assignment used as truth value
    //		while ((sp = G_Find( sp, FOFS(classname),
    //"info_player_deathmatch" ))) {	// info_player_start becomes
    //info_player_deathmatch in it's spawn functon 			if (Distance(
    //ent->s.pos.trBase, sp->s.origin ) < 256 && trap_InPVS( ent->s.pos.trBase,
    //sp->s.origin )) { 				G_SaveGame( NULL ); 				break;
    //			}
    //		}
  }
}
...
void Cmd_SetCameraOrigin_f(gentity_t* ent) {
  ...
  if (ent->client->cameraPortal) {
    // G_SetOrigin( ent->client->cameraPortal, origin );	// set our
    // origin
      ...
      if (traceEnt->s.eType == ET_CORPSE) {
        if (BODY_TEAM(traceEnt) < 4 &&
          ...
          if (BODY_VALUE(traceEnt) >= 250) {
            ...
            //						BG_AnimScriptEvent(
            //&ent->client->ps, ent->client->pers.character->animModelInfo,
            //ANIM_ET_PICKUPGRENADE, qfalse, qtrue ); 						ent->client->ps.pm_flags
            //|= PMF_TIME_LOCKPLAYER; 						ent->client->ps.pm_time = 2100;
    ...
    if (traceEnt->s.eType == ET_ALARMBOX) {
      ...
      if (traceEnt->touch) {
        if (ent->client->pers.autoActivate == PICKUP_ACTIVATE) {
          ent->client->pers.autoActivate =
              PICKUP_FORCE;  //----(SA) force pickup
        }
      ...
      G_UseTargets(
          traceEnt,
          ent);  //----(SA)	added for Mike so mounting an MG42 can be a
                 //trigger event (let me know if there's any issues with this)
      found = qtrue;
  ...
  if (ent->active) {
    if (ent->client->ps.persistant[PERS_HWEAPON_USE]) {
      ...
      if (tr.startsolid) {
        ...
        ent->r.contents = CONTENTS_CORPSE;  // DHM - this will correct itself in
                                            // ClientEndFrame
      }
...
{ trap_DropClient( clent->s.number, "Dropped due to illegal ClientMonsterSlick
command\n" ); return;
        }
...
/*void Cmd_SetWayPoint_f( gentity_t *ent ) {
        ...
        if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
                trap_SendServerCommand( ent-g_entities, "print \"Not allowed to
set waypoints as spectator.\n\"" ); return;
        }
        ...
        trap_Trace( &trace, muzzlePoint, NULL, NULL, end, ent->s.number,
MASK_SHOT );
...
void Cmd_WeaponStat_f(gentity_t* ent) {
  ...
  trap_SendServerCommand(
      ent - g_entities,
      va("rws %i %i", ent->client->sess.aWeaponStats[stat].atts,
...
void Cmd_TicketTape_f(void) {
  /*	char	cmd[MAX_TOKEN_CHARS];
          ...
          trap_SendServerCommand( -1, va( "tt \"LANDMINES SPOTTED <STOP> CHECK
     COMMAND MAP FOR DETAILS <STOP>\"\n", cmd ));*/
}
...
void Cmd_SwapPlacesWithBot_f(gentity_t* ent, int botNum) {
  ...
  if (botent->health <= 0 && (botent->client->ps.pm_flags & PMF_LIMBO)) {
    trap_SendServerCommand(
        ent - g_entities,
        "print \"Bot is in limbo mode, cannot swap places.\n\"");
    return;
  }
  //
  if (client->sess.sessionTeam != botent->client->sess.sessionTeam) {
    trap_SendServerCommand(
        ent - g_entities,
        "print \"Bot is on different team, cannot swap places.\n\"");
    return;
  }
  //
...
void ClientCommand(int clientNum) {
  ...
  if (Q_stricmp(cmd, "say_team") == 0) {
    if (ent->client->sess.sessionTeam == TEAM_SPECTATOR ||
        ent->client->sess.sessionTeam == TEAM_FREE) {
      trap_SendServerCommand(ent - g_entities,
                             "print \"Can't team chat as spectator\n\"\n");
      return;
    }
  ...
  if (level.intermissiontime) {
    //		Cmd_Say_f (ent, qfalse, qtrue);			// NERVE - SMF -
    //we don't want to spam the clients with this.
    CPx(clientNum,
        va("print \"^3%s^7 not allowed during intermission.\n\"", cmd));
    return;
  }
