...
void G_BounceMissile(gentity_t* ent, trace_t* trace) {
  ...
  /*
     ...
     g_entities[trace->entityNum].r.currentOrigin[2])) {
                  g_entities[trace->entityNum].grenadeExplodeTime =
     ent->nextthink; g_entities[trace->entityNum].flags &= ~FL_AI_GRENADE_KICK;
                  Add_Ammo( &g_entities[trace->entityNum], WP_GRENADE_LAUNCHER,
     1, qfalse );	//----(SA)	modified G_FreeEntity( ent ); return;
          }
  */
  ...
  if (ent->s.eFlags & EF_BOUNCE_HALF) {
    if (ent->s.eFlags &
        EF_BOUNCE) {  // both flags marked, do a third type of bounce
    ...
    // check for stop
    //%	if ( trace->plane.normal[2] > 0.2 && VectorLengthSquared(
    //ent->s.pos.trDelta ) < SQR(40) )
    if (trace->plane.normal[2] > 0.2 &&
        VectorLengthSquared(relativeDelta) < SQR(40)) {
      //----(SA)	make the world the owner of the dynamite, so the player
...
void G_MissileImpact(gentity_t* ent, trace_t* trace, int impactDamage) {
  ...
  if (other->classname && Q_stricmp(other->classname, "func_explosive") == 0) {
    // the damage is sufficient to "break" the ent (health == 0 is
    // non-breakable)
    if (other->health && impactDamage >= other->health) {
      // check for other->takedamage needs to be inside the health check since
      // it is likely that, if successfully destroyed by the missile, in the
      // next runmissile() update takedamage would be set to '0' and the
      // func_explosive would not be removed yet, causing a bounce.
      if (other->takedamage) {
  ...
  //	SnapVectorTowards( trace->endpos, ent->s.pos.trBase );	// save net
  //bandwidth
  /*	{
                  gentity_t* tent;
...
void G_ExplodeMissile(gentity_t* ent) {
  ...
  if (etype == ET_MISSILE || etype == ET_BOMB) {
    if (ent->s.weapon == WP_LANDMINE) {
      ...
      // NERVE - SMF - made this the actual bounding box of dynamite instead of
      // range
...
================
*/
void G_MissileDie(gentity_t* self,
                  gentity_t* inflictor,
                  gentity_t* attacker,
                  int damage,
                  int mod) {
...
void G_RunMissile(gentity_t* ent) {
  ...
  if (level.tracemapLoaded &&
    ...
    if (ent->count) {
      if (ent->r.currentOrigin[0] < level.mapcoordsMins[0] ||
        ...
        // are we in worldspace again - or did we hit a ceiling from the outside
        // of the world
        if (skyHeight == 65536) {
          //			if( BG_GetSkyGroundHeightAtPoint( origin ) >=
          //origin[2] ) { 				G_FreeEntity( ent ); 				return; 			} else {
          G_RunThink(ent);
  ...
  if (tr.fraction != 1) {
    ...
    if (ent->s.weapon == WP_PANZERFAUST || ent->s.weapon == WP_MORTAR_SET) {
      ...
      impactDamage = 20;  // "grenade"/"dynamite"		// probably
                          // adjust this based on velocity
    }
    if (ent->s.weapon == WP_DYNAMITE || ent->s.weapon == WP_LANDMINE ||
        ent->s.weapon == WP_SATCHEL) {
...
================
*/
void G_PredictBounceMissile(gentity_t* ent,
                            trajectory_t* pos,
                            trace_t* trace,
                            int time) {
  ...
  for (time = level.time + FRAMETIME; time < level.time + duration;
    ...
    if (tr.fraction != 1) {
      ...
      if (allowBounce && (ent->s.eFlags & (EF_BOUNCE | EF_BOUNCE_HALF))) {
        G_PredictBounceMissile(
            ent, &pos, &tr,
            time - FRAMETIME + (int)((float)FRAMETIME * tr.fraction));
        pos.trTime = time;
        continue;
      }
...
#define FLAME_START_MAX_SIZE \
  100.0  // when the flame is spawned, it should endevour to reach this size
...
void G_RunFlamechunk(gentity_t* ent) {
  ...
  //		However, below, it was being used when we hit something and it
  //was 		uninitialized
  ...
  trap_Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, neworg,
             ent->r.ownerNum, MASK_SHOT | MASK_WATER);  // JPW NERVE
  ...
  if (level.time - ent->timestamp >
      (FLAME_LIFETIME -
       150)) {  // JPW NERVE increased to 350 from 250 to match visuals better
    G_FreeEntity(ent);
    return;
  }
...
void DynaFree(gentity_t* self) {
  // Gordon - see if the dynamite was planted near a constructable object that
...
void G_TripMineThink(gentity_t* ent) {
  ...
  if (trace.fraction == 1.f) {  // Gordon: shouldnt really happen once we do a
                                // proper range check on placing
...
qboolean sEntWillTriggerMine(gentity_t* ent, gentity_t* mine) {
  ...
  if (ent->s.eType == ET_PLAYER && ent->client) {
    ...
    // have to be within the trigger distance AND on the ground -- if we jump
    // over a mine, we don't set it off
...
void G_LandmineThink(gentity_t* self) {
  ...
  if (level.time - self->missionLevel > 200) {
    self->s.density =
        0;  // Gordon: time out the covert ops visibile thing, or we could get
            // other clients being able to see mine later, etc
  }
  ...
  for (i = 0; i < cnt; i++) {
    ...
    //%	if( !g_friendlyFire.integer && G_LandmineTeam( self ) ==
    //ent->client->sess.sessionTeam ) { %		continue; %	}

    // TAT 11/20/2002 use the unified trigger check to see if we are close
    // enough to prime the mine
...
void LandminePostThink(gentity_t* self) {
  ...
  for (i = 0; i < cnt; i++) {
    ...
    // TAT 11/20/2002 use the unifed trigger check to see if we're still
    // standing on the mine, so we don't set it off
    if (sEntWillTriggerMine(ent, self)) {
      trigger = qtrue;
      break;
    }
...
gentity_t* fire_grenade(gentity_t* self,
  ...
  // JPW NERVE -- commented out bolt->damage and bolt->splashdamage, override
  // with G_GetWeaponDamage() so it works with different netgame balance. didn't
  // uncomment bolt->damage on dynamite 'cause its so *special*
  ...
  switch (grenadeWPID) {
    ...
    case WP_DYNAMITE:
      ...
      bolt->accuracy = 0;  // JPW NERVE sets to score below if dynamite is in
      ...
      // nope - this causes the dynamite to impact on the players bb when he
      // throws it. will try setting it when it settles
      //			bolt->r.ownerNum			=
      //ENTITYNUM_WORLD;	// (SA) make the world the owner of the
      //dynamite, so the player can shoot it without modifying the bullet code
      //to ignore players id for hits
...
gentity_t* fire_mortar(gentity_t* self, vec3_t start, vec3_t dir) {
  ...
  bolt->r.svFlags =
      SVF_BROADCAST;  // broadcast sound.  not multiplayer friendly, but for
                      // mortars it should be okay
