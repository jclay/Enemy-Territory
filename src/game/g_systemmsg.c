...
const char* systemMessages[SM_NUM_SYS_MSGS] = {
    "SYS_NeedMedic",       "SYS_NeedEngineer", "SYS_NeedLT",
    "SYS_NeedCovertOps",   "SYS_MenDown",      "SYS_ObjCaptured",
    "SYS_ObjLost",         "SYS_ObjDestroyed", "SYS_ConstructComplete",
    "SYS_ConstructFailed", "SYS_Destroyed",
};
...
qboolean G_NeedEngineers(int team) {
  ...
  for (i = MAX_CLIENTS, e = &g_entities[MAX_CLIENTS]; i < level.num_entities;
       i++, e++) {
    if (!e->inuse) {
      continue;
    }

    if (e->s.eType == ET_CONSTRUCTIBLE_INDICATOR ||
...
void G_SendSystemMessage(sysMsg_t message, int team) {
  ...
  time = team == TEAM_AXIS ? &level.lastSystemMsgTime[0]
                           : &level.lastSystemMsgTime[1];
