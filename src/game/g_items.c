 ...
 * name:		g_items.c
 *
 * desc:		Items are any object that a player can touch to gain
 *some effect. Pickup will return the number of seconds until they should
 *respawn. all items should pop when dropped in lava or slime. Respawnable items
 *don't actually go away when picked up, they are just made invisible and
 *untouchable.  This allows them to ride movers and respawn apropriately.
 *
 */
...
void UseHoldableItem(gentity_t* ent, int item) {
  switch (item) {
    ...
    case HI_WATER:  // protection from drowning - 30 sec underwater breathing
                    // time
      ent->client->ps.powerups[PW_BREATHER] = 30000;
      break;

    case HI_ELECTRIC:  // protection from electric attacks - absorbs 500 points
                       // of electric damage
      ent->client->ps.powerups[PW_ELECTRIC] = 500;
      break;

    case HI_FIRE:  // protection from fire attacks - absorbs 500 points of fire
                   // damage
      ent->client->ps.powerups[PW_FIRE] = 500;
      break;

    case HI_STAMINA:  // restores fatigue bar and sets "nofatigue" for a time
                      // period (currently forced to 60 sec)
      //----(SA)	NOTE:	currently only gives free nofatigue time,
      //doesn't reset fatigue bar. 					(this is because I'd like the restore to be
      //visually gradual (on the HUD item representing 					current status of your
      //fatigue) rather than snapping back to 'full')
...
Add_Ammo
        ...
        (like the AI "infinite ammo" where they get below 900 and force back up
to 999)
...
int Add_Ammo(gentity_t* ent, int weapon, int count, qboolean fillClip) {
  ...
  if (ammoweap == WP_GRENADE_LAUNCHER) {  // make sure if he picks up a grenade
                                          // that he get's the "launcher" too
  ...
  if (ammoweap == WP_PANZERFAUST || ammoweap == WP_FLAMETHROWER) {
    ...
    if (ent->client->ps.ammoclip[ammoweap] > maxammo) {
      ent->client->ps.ammoclip[ammoweap] =
          maxammo;  // - ent->client->ps.ammoclip[BG_FindClipForWeapon(weapon)];
    }
  } else {
    ent->client->ps.ammo[ammoweap] += count;

    if (ent->client->ps.ammo[ammoweap] > maxammo) {
      ent->client->ps.ammo[ammoweap] =
          maxammo;  // - ent->client->ps.ammoclip[BG_FindClipForWeapon(weapon)];
    }
  }
...
qboolean AddMagicAmmo(gentity_t* receiver, int numOfClips) {
  return BG_AddMagicAmmo(&receiver->client->ps, receiver->client->sess.skill,
                         receiver->client->sess.sessionTeam, numOfClips);
}
...
weapon_t G_GetPrimaryWeaponForClient(gclient_t* client) {
  ...
  for (i = 0; i < MAX_WEAPS_PER_CLASS; i++) {
    if (classInfo->classWeapons[i] == WP_MP40 ||
        classInfo->classWeapons[i] == WP_THOMPSON) {
      continue;
    }
...
void G_DropWeapon(gentity_t* ent, weapon_t weapon) {
  ...
  trap_Trace(&tr, client->ps.origin, mins, maxs, org, ent->s.number,
             MASK_SOLID);
  ...
  //	ent2->item->quantity =
  //client->ps.ammoclip[BG_FindClipForWeapon(weapon)]; // Gordon: um, modifying
  //an item is not a good idea
...
qboolean G_CanPickupWeapon(weapon_t weapon, gentity_t* ent) {
  ...
  if (ent->client->sess.skill[SK_HEAVY_WEAPONS] >= 4 &&
      (weapon == WP_THOMPSON || weapon == WP_MP40)) {
    return qfalse;
  }

  return BG_WeaponIsPrimaryForClassAndTeam(
      ent->client->sess.playerType, ent->client->sess.sessionTeam, weapon);
}
...
int Pickup_Weapon(gentity_t* ent, gentity_t* other) {
  ...
  if (ent->item->giTag == WP_AMMO) {
    ...
    // if LT isn't giving ammo to self or another LT or the enemy, give him some
    // props
    if (other->client->ps.stats[STAT_PLAYER_CLASS] != PC_FIELDOPS) {
      if (ent->parent && ent->parent->client &&
        ...
        G_DebugAddSkillPoints(ent->parent, SK_SIGNALS, 1.f,
                              "ammo pack picked up");
    ...
    if (G_CanPickupWeapon(ent->item->giTag, other)) {
      ...
      if (primaryWeapon || (other->client->sess.playerType == PC_SOLDIER &&
        ...
        if (ent->item->giTag == WP_MORTAR) {
          other->client->ps.ammo[BG_FindClipForWeapon(ent->item->giTag)] =
              quantity;
          ...
          other->client->ps.ammoclip[BG_FindClipForWeapon(ent->item->giTag)] =
              quantity;
...
int Pickup_Health(gentity_t* ent, gentity_t* other) {
  ...
  // if medic isn't giving ammo to self or another medic or the enemy, give him
  // some props
...
==============
Touch_Item
        if other->client->pers.autoActivate == PICKUP_ACTIVATE	(0), he will
pick up items only when using +activate if other->client->pers.autoActivate ==
PICKUP_TOUCH		(1), he will pickup items when touched if
other->client->pers.autoActivate == PICKUP_FORCE		(2), he will
...
void Touch_Item_Auto(gentity_t* ent, gentity_t* other, trace_t* trace) {
  ...
  if (other->client->pers.autoActivate ==
      PICKUP_FORCE) {  // autoactivate probably forced by the "Cmd_Activate_f()"
                       // function
...
void Touch_Item(gentity_t* ent, gentity_t* other, trace_t* trace) {
  ...
  if (ent->noise_index) {
    ...
    // so it doesn't make the default pickup sound when the pickup event is
    // recieved
...
gentity_t* LaunchItem(gitem_t* item,
  ...
  dropped->s.otherEntityNum2 = 1;  // DHM - Nerve :: this is taking
                                   // modelindex2's place for a dropped item

  dropped->classname = item->classname;
  dropped->item = item;
  VectorSet(dropped->r.mins, -ITEM_RADIUS, -ITEM_RADIUS,
            0);  //----(SA)	so items sit on the ground
  VectorSet(dropped->r.maxs, ITEM_RADIUS, ITEM_RADIUS,
  ...
  trap_Trace(&tr, origin, dropped->r.mins, dropped->r.maxs, origin, ownerNum,
             MASK_SOLID);
  if (tr.startsolid) {
    ...
    for (i = 16; i <= 48; i += 16) {
      ...
      trap_Trace(&tr, origin, dropped->r.mins, dropped->r.maxs, origin,
                 ownerNum, MASK_SOLID);
      if (!tr.startsolid) {
        break;
      }
  ...
  if (item->giType == IT_TEAM) {  // Special case for CTF flags
    ...
    dropped->s.otherEntityNum =
        g_entities[ownerNum].client->flagParent;  // store the entitynum of our
                                                  // original flag spawner
    dropped->s.density = 1;
...
void FinishSpawningItem(gentity_t* ent) {
  ...
  ent->s.otherEntityNum2 = 0;  // DHM - Nerve :: takes modelindex2's place in
                               // signaling a dropped item
  //----(SA)	we don't use this (yet, anyway) so I'm taking it so you can
  //specify a model for treasure items and clipboards 	ent->s.modelindex2 = 0; //
  ...
  //----(SA) moved this up so it happens for suspended items too (and made it a
  //function)
  ...
  if (ent->spawnflags & 1) {  // suspended
    ...
    VectorSet(dest, ent->s.origin[0], ent->s.origin[1],
              ent->s.origin[2] - 4096);
    trap_Trace(&tr, ent->s.origin, ent->r.mins, maxs, dest, ent->s.number,
               MASK_SOLID);
  ...
  if (ent->item->giType == IT_HEALTH || ent->item->giType == IT_AMMO) {
    ...
    // having alternate models defined in bg_misc.c for a health or ammo item
    // specify it as "multi-stage" TTimo left-hand operand of comma expression
    // has no effect initial line: for(i=0;i<4,ent->item->world_model[i];i++) {}
    for (i = 0; i < 4 && ent->item->world_model[i]; i++) {
    }

    ent->s.density = i - 1;  // store number of stages in 'density' for client
                             // (most will have '1')
  }
...
void G_RunItemProp(gentity_t* ent, vec3_t origin) {
  ...
  if (owner->client && trace.startsolid && traceEnt != owner &&
      traceEnt != ent /* && !traceEnt->active*/) {
...
void G_RunItem(gentity_t* ent) {
  ...
  if (ent->s.pos.trType == TR_STATIONARY ||
      ent->s.pos.trType == TR_GRAVITY_PAUSED) {  //----(SA)
