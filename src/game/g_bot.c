...
vmCvar_t bot_debug_curAINode;   // the text of the current ainode for the bot
                                // begin debugged
  ...
  /*
          ...
          for ( n = 0; n < g_numBots ; n++ ) {
                  ...
                  for ( i=0 ; i< g_maxclients.integer ; i++ ) {
                          ...
                          if ( !(g_entities[cl->ps.clientNum].r.svFlags &
     SVF_BOT) ) { continue;
                          }
     ...
     "g_spSkill" ); if (team == TEAM_AXIS) teamstr = "red"; else if (team ==
     TEAM_ALLIES) teamstr = "blue"; else teamstr = ""; strncpy(netname, value,
     sizeof(netname)-1); netname[sizeof(netname)-1] = '\0'; Q_CleanStr(netname);
                                  trap_SendConsoleCommand( EXEC_INSERT,
...
void G_CheckMinimumPlayers(void) {
  ...
  int humanplayers[TEAM_NUM_TEAMS], botplayers[TEAM_NUM_TEAMS],
      players[TEAM_NUM_TEAMS];
  ...
  if (checkminimumplayers_time < level.time &&
      checkminimumplayers_time > level.time - 1000) {
    return;
  }
  ...
  /*	if (players[TEAM_AXIS] < minplayers) {
          ...
          if (bot_eventeams.integer) {
                  ...
                  // even up the teams
                  //
                  if (!botplayers[strongestTeam] || (minplayers &&
     players[weakestTeam] <= minplayers)) {
     ...
     { G_RemoveRandomBot( TEAM_AXIS ); return;
                  }
                  if (players[TEAM_ALLIES] > minplayers &&
     botplayers[TEAM_ALLIES]) { G_RemoveRandomBot( TEAM_ALLIES ); return;
                  }
          }*/
}
...
===============
*/
// returns the wp_ number for weapon "weaponName", or -1 if weapon not allowed
// for player/class/team combination
...
static void G_AddBot(const char* name,
                     int skill,
                     const char* team,
...
#define MAX_BOTNAMES 1024
  ...
  clientNum =
      trap_BotAllocateClient(0);  // Arnout: 0 means no prefered clientslot
  if (clientNum == -1) {
    G_Printf(S_COLOR_RED "Unable to add bot.  All player slots are in use.\n");
    G_Printf(S_COLOR_RED
             "Start server with more 'open' slots (or check setting of "
             "sv_maxclients cvar).\n");
    return;
  }
  ...
  /*	if (playerClass > 0) {
          ...
          if (playerWeapon) {
                  Info_SetValueForKey( userinfo, "pWeapon", va("%i",
     playerWeapon) );
          }*/
  // END Mad Doc - TDF
  ...
  if (!Q_stricmp((char*)name, key)) {
    ...
    char botnames[8192], *pbotnames, *listbotnames[MAX_BOTNAMES], *token,
        *oldpbotnames;
    ...
    if (len >= 0) {
      if (len > sizeof(botnames)) {
        G_Error("botfiles/botnames.txt is too big (max = %i)",
                (int)sizeof(botnames));
      }
  ...
  // if a character was specified, put the index of that character filename in
  // the CS_CHARACTERS table in the userinfo
  if (characerIndex != -1) {
...
void Svcmd_AddBot_f(void) {
  ...
  trap_Argv(
      1, name,
      sizeof(
          name));  /// read it just so we can check if it's a name (old method)
...
void G_SpawnBot(const char* text) {
  ...
  // GS  prevent bot health from counting down to 70 (i.e. don't set
  // STAT_MAX_HEALTH = 70)
  ...
  // parameters
  spawnBotCommand_t params[] = {{"/name", name, qfalse, "[name]"},
                                {"/skill", skill, qfalse, "[0-4]"},
                                {"/team", team, qfalse, "[AXIS/ALLIES]"},
                                {"/class", pClass, qfalse,
                                 "[SOLDIER/MEDIC/LIEUTENANT/ENGINEER/COVERTOPS/"
                                 "FIELDOPS]"},  // FIXME: remove LIEUTENANT from
                                                // missionpack
                                {"/weapon", pWeapon, qfalse, "[weaponValue]"},
                                {"/spawnpoint", spawnPoint, qtrue,
                                 "[targetname]"},
                                {"/respawn", respawn, qfalse, "[ON/OFF]"},
                                {"/scriptName", scriptName, qfalse,
                                 "[scriptName]"},
                                {"/character", characterFile, qfalse,
                                 "[character]"},
                                // START Mad Doc - TDF
                                {"/rank", rank, qfalse, "[rank]"},
                                {"/skills", botSkills, qfalse,
                                 "[botskills]"},  // not really to be exposed to
                                                  // script
                                // END Mad Doc - TDF
  ...
  while (cmd[0]) {
    ...
    for (j = 0; params[j].cmd; j++) {
      if (!Q_stricmp(params[j].cmd, cmd)) {
        // found a match, if this field already has an entry, then randomly
        // override it
        if (!params[j].count || (!params[j].appendParams &&
                                 ((rand() % (++params[j].count)) == 0))) {
    ...
    if (!params[j].cmd) {
      G_Printf(
          "G_SpawnBot: unknown parameter: %s\nFor usage info, use \"spawnbot "
          "/?\"\n",
          cmd);
      return;
    }
  ...
  if (!Q_stricmp(team, "red") || !Q_stricmp(team, "r") ||
      !Q_stricmp(team, "axis")) {
    teamNum = TEAM_AXIS;
  ...
  // If we have a weapon specified, and we have a class specified
    ...
    weaponSpawnNumber =
        Bot_GetWeaponForClassAndTeam(pClassInt - 1, teamNum, pWeapon);

    // Get default weapon
    if (weaponSpawnNumber == -1) {
      weaponSpawnNumber =
          BG_GetPlayerClassInfo(teamNum, pClassInt - 1)->classWeapons[0];
    }
  ...
  //	{"/botskills",	botSkills,		qfalse,		"[botskills]"},
  //// not really to be exposed to script

  // END Mad Doc - TDF

  G_AddBot(name, atoi(skill), team, spawnPoint, pClassInt, weaponSpawnNumber,
           characterInt, respawn, scriptName, rankNum, skills, prisonerOfWar);
...
void BotDropToFloor(gentity_t* ent) {
  ...
  //----(SA)	move the bounding box for the check in 1 unit on each side so
  //they can butt up against a wall and not startsolid
  ...
  trap_Trace(&tr, ent->r.currentOrigin, checkMins, checkMaxs, dest,
...
void ServerEntityDropToFloor(g_serverEntity_t* ent) {
  ...
  trap_Trace(&tr, ent->origin, checkMins, checkMaxs, dest, -1,
             MASK_PLAYERSOLID);
...
// TAT 11/18/2002 - Use this function as the setup func for ai_markers that are
// NOT generated from q3map, but are parsed from the external file
...
/*QUAKED bot_landminespot_spot (1 0.2 0) (-18 -18 -24) (18 18 48) NODROP
CROUCHING Bots will use this spot to search for landmines from, target at a
info_null
...
/*QUAKED bot_seek_cover_spot (0 0.6 0) (-18 -18 -24) (18 18 48) NODROP CROUCHING
CROUCH_TOWARDS PRONING PRONE_TOWARDS EXPOSED NEVER_PRONE Allies bots will use
this spot to seek cover while in follow formation
...
void bot_seek_cover_spot_think(g_serverEntity_t* ent) {
  ...
  if (ent->target && (ent->target[0] != 0)) {
    ...
    while ((trav = FindServerEntity(trav, SE_FOFS(name), ent->target))) {
      if (Q_stricmp(trav->classname, ent->classname)) {
        G_Error("bot_seek_cover_spot at %s is targetting a %s",
                vtos(ent->origin), trav->classname);
      }
...
// TAT 11/18/2002 - Use this function as the setup func for cover spots that are
// NOT generated from q3map, but are parsed from the external file
...
/*QUAKED bot_axis_seek_cover_spot (0.6 0.6 0.8) (-18 -18 -24) (18 18 48) NODROP
CROUCHING CROUCH_TOWARDS PRONING PRONE_TOWARDS EXPOSED NEVER_PRONE Axis bots
will use this spot to seek cover while fighting
...
Point this to the bot_seek_cover_spot that should be used for following or
bot_axis_seek_cover_spot for axis bots to use

  target : targetname of the spot to start the sequence. can point to multiple
spots.
*/
void bot_seek_cover_sequence_touch(gentity_t* self,
                                   gentity_t* other,
                                   trace_t* trace) {
  // if we have a seek cover spot, then set the guy who stepped on me to be
  // using it
...
void bot_seek_cover_sequence_init(gentity_t* ent) {
  ...
  // the first one is different, since we're getting the sequence (a gentity_t)
  // to point at the first spot (a g_serverEntity_t)
  ...
  if (ent->serverEntity == NULL) {
    G_Error(
        "bot_seek_cover_sequence has no matching spots (\"target\" = \"%s\"",
        ent->target);
  }
  //
