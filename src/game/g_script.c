...
g_script_event_define_t gScriptEvents[] = {
    ...
    {"trigger",
     G_Script_EventMatch_StringEqual},  // something has triggered us (always
                                        // followed by an identifier)
    {"pain", G_Script_EventMatch_IntInRange},       // we've been hurt
    {"death", NULL},                                // RIP
    {"activate", G_Script_EventMatch_StringEqual},  // something has triggered
                                                    // us [activator team]
    {"stopcam", NULL},
    {"playerstart", NULL},
    ...
    {"message", G_Script_EventMatch_StringEqual},  // contains a sequence of VO
                                                   // in a message
...
int G_Script_EventForString(const char* string) {
  ...
  for (i = 0; gScriptEvents[i].eventStr; i++) {
    if (gScriptEvents[i].hash == hash &&
        !Q_stricmp(string, gScriptEvents[i].eventStr)) {
      return i;
    }
  }
...
g_script_stack_action_t* G_Script_ActionForString(char* string) {
  ...
  for (i = 0; gScriptActions[i].actionString; i++) {
    if (gScriptActions[i].hash == hash &&
        !Q_stricmp(string, gScriptActions[i].actionString)) {
...
void G_Script_ScriptLoad(void) {
  ...
  // END Mad Doc - TDF
  // Arnout: make sure we terminate the script with a '\0' to prevent parser
  // from choking
  ...
  *(level.scriptEntity + len) = '\0';
  ...
  // Gordon: and make sure ppl haven't put stuff with uppercase in the string
  // table..
  G_Script_EventStringInit();

  // Gordon: discard all the comments NOW, so we dont deal with them inside
  // scripts Gordon: disabling for a sec, wanna check if i can get proper line
  // numbers from error output
  ...
  while (token[0]) {
    ...
    if (parsingCharacter) {
      ...
      if (!BG_FindCharacter(token)) {
        ...
        Q_strncpyz(character->characterFile, token,
                   sizeof(character->characterFile));

        if (!G_RegisterCharacter(token, character)) {
          G_Error(
              "ERROR: G_Script_ParseSpawnbot: failed to load character file "
              "'%s'\n",
              token);
        }
      }
    ...
    if (strrchr(token, ' ')) {  // need to wrap this param in quotes since it
...
void G_Script_ScriptParse(gentity_t* ent) {
  ...
  while (1) {
    ...
    if (!token[0]) {
      if (!wantName) {
        G_Error(
            "G_Script_ScriptParse(), Error (line %d): '}' expected, end of "
            "script found.\n",
    ...
    if (token[0] == '}') {
      ...
      if (wantName) {
        G_Error(
            "G_Script_ScriptParse(), Error (line %d): '}' found, but not "
            "expected.\n",
            COM_GetCurrentParseLine());
      }
      wantName = qtrue;
      ...
      if (wantName) {
        G_Error(
            "G_Script_ScriptParse(), Error (line %d): '{' found, NAME "
            "expected.\n",
            COM_GetCurrentParseLine());
      }
    } else if (wantName) {
      ...
      if (numEventItems >= G_MAX_SCRIPT_STACK_ITEMS) {
        G_Error(
            "G_Script_ScriptParse(), Error (line %d): G_MAX_SCRIPT_STACK_ITEMS "
            "reached (%d)\n",
      ...
      while ((token = COM_Parse(&pScript)) != NULL && (token[0] != '}')) {
        ...
        if (!action) {
          G_Error(
              "G_Script_ScriptParse(), Error (line %d): unknown action: %s.\n",
              COM_GetCurrentParseLine(), token);
        }
        ...
        if (!Q_stricmp(action->actionString, "set")) {
          ...
          while ((token = COM_Parse(&pScript)) && (token[0] != '}')) {
            ...
            if (strrchr(token, ' ')) {  // need to wrap this param in quotes
          ...
          if (!Q_stricmp(token, "spawnbot")) {
            // this is fairly indepth, so I'll move it to a separate function
            // for readability
            ...
            for (i = 0; token[0]; i++) {
              ...
              if (i == 0) {
                ...
                if (!Q_stricmp(action->actionString, "playsound")) {
                  ...
                  if (strlen(token)) {  // we know there's a [0], but don't know
                                        // if it's '0'
        ...
        if (curEvent->stack.numItems >= G_MAX_SCRIPT_STACK_ITEMS) {
          G_Error(
              "G_Script_ScriptParse(): script exceeded "
              "G_MAX_SCRIPT_STACK_ITEMS (%d), line %d\n",
...
void G_Script_ScriptChange(gentity_t* ent, int newScriptNum) {
  ...
  if (G_Script_ScriptRun(ent) &&
      (ent->scriptStatus.scriptId ==
       scriptStatusBackup.scriptId +
           1)) {  // make sure we didnt change our script via a third party
...
void G_Script_EventStringInit(void) {
  ...
  for (i = 0; gScriptActions[i].actionString; i++) {
    gScriptActions[i].hash =
        BG_StringHashValue_Lwr(gScriptActions[i].actionString);
  }
}
...
int G_Script_GetEventIndex(gentity_t* ent, char* eventStr, char* params) {
  ...
  for (i = 0; gScriptEvents[i].eventStr; i++) {
    if (gScriptEvents[i].hash == hash &&
        !Q_stricmp(eventStr, gScriptEvents[i].eventStr)) {  // match found
      eventNum = i;
      break;
    }
  }
  ...
  if (g_scriptDebug.integer) {
    G_Printf("%i : (%s) GScript event: %s %s\n", level.time,
             ent->scriptName ? ent->scriptName : "n/a", eventStr,
             params ? params : "");
  }
...
qboolean G_Script_ScriptRun(gentity_t* ent) {
  ...
  while (ent->scriptStatus.scriptStackHead < stack->numItems) {
    ...
    if (!stack->items[ent->scriptStatus.scriptStackHead].action->actionFunc(
            ent, stack->items[ent->scriptStatus.scriptStackHead].params)) {
      ent->scriptStatus.scriptFlags &= ~SCFL_FIRST_CALL;
      return qfalse;
    }
    // if the scriptId changed, a new event was triggered in our scripts which
    // did not finish
    ...
    if (g_scriptDebug.integer) {
      if (ent->scriptStatus.scriptStackHead < stack->numItems) {
        G_Printf("%i : (%s) GScript command: %s %s\n", level.time,
                 ent->scriptName,
                 stack->items[ent->scriptStatus.scriptStackHead]
                     .action->actionString,
                 (stack->items[ent->scriptStatus.scriptStackHead].params
                      ? stack->items[ent->scriptStatus.scriptStackHead].params
                      : ""));
...
void mountedmg42_fire(gentity_t* other) {
  ...
  if (self->s.density & 8) {
    Fire_Lead_Ext(other, other, MG42_SPREAD_MP, MG42_DAMAGE_MP, muzzle, forward,
                  right, up, MOD_BROWNING);
  } else {
    Fire_Lead_Ext(other, other, MG42_SPREAD_MP, MG42_DAMAGE_MP, muzzle, forward,
                  right, up, MOD_MG42);
  }
...
void script_mover_die(gentity_t* self,
                      gentity_t* inflictor,
                      gentity_t* attacker,
                      int damage,
                      int mod) {
...
void script_mover_aas_blocking(gentity_t* ent) {
  if (ent->timestamp <= level.time) {
    // are we moving?
    if (ent->s.pos.trType !=
        TR_STATIONARY /*VectorLengthSquared( ent->s.pos.trDelta )*/) {
...
RESURECTABLE COMPASS ALLIED AXIS MOUNTED_GUN Scripted brush entity. A simplified
means of moving brushes around based on events.
...
both the "modelscale" is ignored "model2" optional md3 to draw over the solid
clip brush "scriptname" name used for scripting purposes (like aiName in AI
scripting) "health" optionally make this entity damagable "description" used
with health, if the entity is damagable, it draws a healthbar with this
...
void script_model_med_use(gentity_t* ent,
                          gentity_t* other,
                          gentity_t* activator) {
  script_model_med_spawn(ent);
}

/*QUAKED script_model_med (0.5 0.25 1.0) (-16 -16 -24) (16 16 64) TriggerSpawn
Solid MEDIUM SIZED scripted entity, used for animating a model, moving it
around, etc SOLID spawnflag means this entity will clip the player and AI,
otherwise they can walk straight through it "model" the full path of the model
to use "scriptname" name used for scripting purposes (like aiName in AI
scripting)
*/
...
// Gordon: also storing some stuff that will change often but needs to be
broadcast, so we dont want to use a configstring
...
void SP_script_multiplayer(gentity_t* ent) {
  ...
  // Gordon: broadcasting this to clients now, should be cheaper in bandwidth
  // for sending landmine info
