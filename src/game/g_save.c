...
Using the "checkpoint" system, we only need to save at specific locations, but
various entities may have changed behind us, so therefore we need to save as
much as possible, but without going overboard.

For now, everything is saved from the entity, client and cast_state structures,
except the fields defined in the ignoreField structures below. Any pointer
fields need to be specified in the saveField structures below.

!! NOTE: when working on Wolf patches, make sure you only add fields to the very
end of the three main structures saved here (entity, client, cast_state). If any
fields are inserted in the middle of these structures, savegames will become
corrupted, and there is no way of checking for this, so it'll just crash.
*/
...
//.......................................................................................
// these are the fields that cannot be saved directly, so they need to be
// converted
static saveField_t gentityFields[] = {{FOFS(client), F_CLIENT},
...
static saveField_t gclientFields[] = {{CFOFS(hook), F_ENTITY},
...
static saveField_t botwaypointFields[] = {{BWPOFS(next), F_BOTWAYPOINT},
...
static saveField_t botstateFields[] = {
    ...
    {BFOFS(checkpoints), F_BOTWAYPOINT},  // for waypoints we need to store the
                                          // botai_waypoints seperately
...
//.......................................................................................
// this is where we define fields or sections of structures that we should
// totally ignore
typedef struct {
  int ofs;
  int len;
...
static ignoreField_t gentityIgnoreFields[] = {
    ...
    {FOFS(scriptEvents),
     sizeof(g_script_event_t*)},  // gets created upon parsing the script file,
                                  // this is static while playing
...
static ignoreField_t botstateIgnoreFields[] = {
    {BFOFS(objectiveMatrix),
     sizeof(botMatrix_t*)},  // these are based on the playerclass stored in the
                             // botstate and further static
...
//-----------------
// MSVC likes to needlessly(?) warn about these defines, so disable certain
...
#ifdef __linux__
  G_Error(
      "Unable to save game.\n\nPlease check that you have at least 5mb free of "
      "disk space in your home directory.");
#else
  G_Error(
      "Insufficient free disk space.\n\nPlease free at least 5mb of free space "
      "on game drive.");
#endif
}
...
G_SaveWrite
  ...
  FS_Write doesnt always accurately return how many bytes were written, so tally
them up, and check them before we rename to the real file
===============
*/
  ...
  switch (field->type) {
    ...
    case F_ITEM:
      ...
      *(int*)p = index;
      ...
      //	match this with a function address in the function list, which
      //is built using the 	"extractfuncs.bat" in the utils folder. We then save
      //the string equivalent 	of the function. This effectively gives us
      //cross-version save games.
    case F_FUNCTION:
      if (*(byte**)p == NULL) {
        len = 0;
      } else {
      ...
      if (!len) {
        *(byte**)p = NULL;
        ...
        if (len > sizeof(funcStr)) {
          G_Error("ReadField: function name is greater than buffer (%i chars)",
                  sizeof(funcStr));
        }
...
int G_Save_Encode(byte* raw, byte* out, int rawsize, int outsize) {
  ...
  while (rawcount < rawsize) {
    ...
    // calc the count
    count = 0;
    while (rawcount < rawsize && (raw[rawcount] != 0) == mode &&
           count < (((1 << (SAVE_ENCODE_COUNT_BYTES * 8 - 1)) - 1))) {
      rawcount++;
      count++;
    }
...
void WriteClient(fileHandle_t f, gclient_t* cl) {
  ...
  length =
      G_Save_Encode((byte*)&temp, clientBuf, sizeof(temp), sizeof(clientBuf));
...
void ReadClient(fileHandle_t f, gclient_t* client, int size) {
  ...
  trap_FS_Read(clientBuf, decodedSize, f);  // decode it
  ...
  for (ifield = gclientIgnoreFields; ifield->len; ifield++) {
    memcpy(((byte*)&temp) + ifield->ofs, ((byte*)client) + ifield->ofs,
           ifield->len);
  }
  ...
  // RF, disabled, not required now with screen fading, causes characters to
  // possibly spawn events before they are known in the cgame run a client frame
  // to drop exactly to the floor, initialize animations and other things
...
void WriteEntity(fileHandle_t f, gentity_t* ent) {
  ...
  // if (!G_SaveWrite (&temp, sizeof(temp), f)) G_SaveWriteError();
  length =
      G_Save_Encode((byte*)&temp, entityBuf, sizeof(temp), sizeof(entityBuf));
  if (!G_SaveWrite(&length, sizeof(length), f)) {
    G_SaveWriteError();
  }
...
void ReadEntity(fileHandle_t f, gentity_t* ent, int size) {
  ...
  for (ifield = gentityIgnoreFields; ifield->len; ifield++) {
    memcpy(((byte*)&temp) + ifield->ofs, ((byte*)ent) + ifield->ofs,
           ifield->len);
  }
  ...
  if (ent->s.eType == ET_MOVER && ent->moverState != backup.moverState) {
    if (ent->teammaster == ent || !ent->teammaster) {
      if (ent->moverState == MOVER_POS1ROTATE ||
        ...
        // portals are always opened before the mover starts to open, so we must
        // move it back to the start position, link, set portals, then move it
        // back
        backup2 = *ent;
        *ent = backup;
...
void WriteTime(fileHandle_t f) {
  ...
  trap_RealTime(&tm);
  G_SaveWrite(&tm.tm_sec, sizeof(tm.tm_sec),
              f); /* seconds after the minute - [0,59] */
  G_SaveWrite(&tm.tm_min, sizeof(tm.tm_min),
              f); /* minutes after the hour - [0,59] */
  G_SaveWrite(&tm.tm_hour, sizeof(tm.tm_hour),
              f); /* hours since midnight - [0,23] */
  G_SaveWrite(&tm.tm_mday, sizeof(tm.tm_mday),
              f); /* day of the month - [1,31] */
  G_SaveWrite(&tm.tm_mon, sizeof(tm.tm_mon),
              f); /* months since January - [0,11] */
  G_SaveWrite(&tm.tm_year, sizeof(tm.tm_year), f); /* years since 1900 */
  G_SaveWrite(&tm.tm_wday, sizeof(tm.tm_wday),
              f); /* days since Sunday - [0,6] */
  G_SaveWrite(&tm.tm_yday, sizeof(tm.tm_yday),
              f); /* days since January 1 - [0,365] */
  G_SaveWrite(&tm.tm_isdst, sizeof(tm.tm_isdst),
              f); /* daylight savings time flag */
}
...
char* G_Save_TimeStr(void) {
  ...
  return va("%2i:%s%i:%s%i %s",
            ...
            tm.tm_sec, (tm.tm_hour < 12 ? "am" : "pm"));
}

static char* monthStr[12] = {"JAN", "FEB", "MAR", "APR", "MAY", "JUN",
...
char* G_Save_DateStr(void) {
  ...
  trap_RealTime(&tm);
  //
  return va("%s %i, %i", monthStr[tm.tm_mon], tm.tm_mday, 1900 + tm.tm_year);
}
...
void WriteBotWaypoint(fileHandle_t f, bot_waypoint_t* wp) {
  ...
  // write the block
  length = G_Save_Encode((byte*)&temp, botwaypointBuf, sizeof(temp),
                         sizeof(botwaypointBuf));
  if (!G_SaveWrite(&length, sizeof(length), f)) {
    G_SaveWriteError();
  }
...
void ReadBotWaypoint(fileHandle_t f, bot_waypoint_t* wp, int size) {
  ...
  trap_FS_Read(botwaypointBuf, decodedSize, f);  // decode it
...
void WriteBotState(fileHandle_t f, bot_state_t* bs) {
  ...
  // write the block
  length = G_Save_Encode((byte*)&temp, botstateBuf, sizeof(temp),
                         sizeof(botstateBuf));
  if (!G_SaveWrite(&length, sizeof(length), f)) {
    G_SaveWriteError();
  }
...
void ReadBotState(fileHandle_t f, bot_state_t* bs, int size) {
  ...
  trap_FS_Read(botstateBuf, decodedSize, f);  // decode it
  ...
  for (ifield = botstateIgnoreFields; ifield->len; ifield++) {
    memcpy(((byte*)&temp) + ifield->ofs, ((byte*)bs) + ifield->ofs,
           ifield->len);
  }
  ...
  bs->character =
      trap_BotLoadCharacter(bs->settings.characterfile, bs->settings.skill);
  if (!bs->character) {
...
qboolean G_SaveGame(char* username) {
  ...
  // if (reloading)
  //	return qtrue;	// actually this should be qtrue, but we should make it
  //silent during reloading
  ...
  for (i = 0; i < strlen(username); i++) {
    if (!Q_isforfilename(username[i]) &&
        username[i] != '\\') {  // (allow '\\' so games can be saved in subdirs)
      G_Printf(
          "G_SaveGame: '%s'.  Invalid character (%c) in filename. Must use "
  ...
  trap_Cvar_VariableStringBuffer("svg_timestring", leveltime,
sizeof(leveltime)); if(!strlen(leveltime)) Com_sprintf(leveltime,
sizeof(leveltime), "Leveltime");

  trap_Cvar_VariableStringBuffer("svg_healthstring", healthstr,
sizeof(healthstr)); if(!strlen(healthstr)) Com_sprintf(healthstr,
sizeof(healthstr), "Health");


//	Com_sprintf( infoString, sizeof(infoString), "Mission: %s\nDate:
%s\nTime: %s\nGametime: %s\nHealth: %i", Com_sprintf( infoString,
sizeof(infoString), "%s\n%s: %s\n%s: %i", mapstr, leveltime,
//		G_Save_DateStr(),
//		G_Save_TimeStr(),
          va("%2ih%s%im%s%is",
...
second padding
  ...
  Com_sprintf(mapstr, MAX_QPATH, "profiles/%s/save/%s.sav", cl_profileStr,
              username);
...
G_LoadGame
  ...
  Always loads in "current.svg". So if loading a specific savegame, first copy
it to that.
===============
*/
void G_LoadGame(void) {
  ...
  Com_sprintf(filename, sizeof(filename), "profiles/%s/save/current.sav",
              cl_profileStr);
  ...
  if(strlen(musicString)) {
          trap_Cvar_Register( &musicCvar, "s_currentMusic", "", CVAR_ROM );
// get current music if(Q_stricmp(musicString, musicCvar.string)) {
// it's different than what's playing, so fade out and queue up
...
musicString) );		// (SA) trying this instead
  ...
  while (1) {
    ...
    //{ 			BotAIShutdownClient(i); 			trap_DropClient( i, "" );
    //		}
    ...
    for (; last < i; last++) {
      if (g_entities[last].inuse && i != ENTITYNUM_WORLD) {
        if (last < MAX_CLIENTS) {
          // free any bot data if present
          //					if( g_entities[last].r.svFlags &
          //SVF_BOT ) { 						BotAIShutdownClient(last);
          //					}
  ...
  /*
          ...
          last = level.time;
          level.time = leveltime;	// use the value we just for the save
     time G_SaveGame(NULL);
          // additionally update the last game that was loaded
          trap_Cvar_VariableStringBuffer( "savegame_filename", mapname,
     sizeof(mapname) ); if (strlen( mapname ) > 0 && !strstr( mapname,
     "autosave" )) {
                  // clear it out so we dont lose it after a map_restart
                  trap_Cvar_Set( "savegame_filename", "" );
                  if (strstr(mapname, ".svg")) mapname[strstr(mapname, ".sav") -
     mapname] = '\0'; if (strstr(mapname, "/")) { G_SaveGame( strstr(mapname,
     "/") + 1 ); } else { G_SaveGame( mapname );
                  }
          }
...
qboolean G_SavePersistant(char* nextmap) {
  ...
  trap_FS_Rename(va("profiles/%s/save/temp.psw", cl_profileStr),
                 va("profiles/%s/save/current.psw", cl_profileStr));

  // now check that it is the correct size
  Com_sprintf(filename, MAX_QPATH, "profiles/%s/save/current.psw",
              cl_profileStr);
  if (trap_FS_FOpenFile(filename, &f, FS_READ) < saveByteCount) {
    trap_FS_FCloseFile(f);
    G_SaveWriteError();
