...
void DropToFloorG(gentity_t* ent) {
  ...
  trap_Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, dest,
...
void moveit(gentity_t* ent, float yaw, float dist) {
  ...
  trap_Trace(&tr, ent->r.currentOrigin, mins, maxs, origin, ent->s.number,
             MASK_SHOT);
...
void SP_props_sparks(gentity_t* ent) {
  // (SA) don't use in multiplayer right now since it makes decyphering net
  // messages almost impossible
  ent->think = G_FreeEntity;
  return;
...
void InitProp(gentity_t* ent) {
  ...
  if (!Q_stricmp(ent->classname, "props_bench")) {
    ...
    ent->s.modelindex2 =
        G_ModelIndex("models/mapobjects/electronics/radio1.md3");
  } else if (!Q_stricmp(ent->classname, "props_locker_tall")) {
    ent->s.modelindex2 =
        G_ModelIndex("models/furniture/storage/lockertall.md3");
    ...
    ent->s.modelindex2 =
        G_ModelIndex("models/furniture/table/56x112tablew.md3");
    ...
    ent->s.modelindex2 =
        G_ModelIndex("models/mapobjects/electronics/radios.md3");
  }
...
void props_bench_die(gentity_t* ent,
                     gentity_t* inflictor,
                     gentity_t* attacker,
                     int damage,
                     int mod) {
...
void props_radio_die(gentity_t* ent,
                     gentity_t* inflictor,
                     gentity_t* attacker,
                     int damage,
                     int mod) {
...
void props_radio_dieSEVEN(gentity_t* ent,
                          gentity_t* inflictor,
                          gentity_t* attacker,
                          int damage,
                          int mod) {
...
void props_locker_tall_die(gentity_t* ent,
                           gentity_t* inflictor,
                           gentity_t* attacker,
                           int damage,
                           int mod) {
...
void Props_Chair_Die(gentity_t* ent,
                     gentity_t* inflictor,
                     gentity_t* attacker,
                     int damage,
                     int mod);
...
void Just_Got_Thrown(gentity_t* self) {
  ...
  if (self->s.groundEntityNum == -1) {
...

    {
      ...
      trap_Trace(&trace, self->r.currentOrigin, self->r.mins, self->r.maxs, end,
  ...
  self->s.dmgFlags =
      HINT_CHAIR;  // so client knows what kind of mover it is for cursorhints
...
void Props_Chair_Think(gentity_t* self) {
  ...
  BG_EvaluateTrajectory(&self->s.pos, level.time, self->s.pos.trBase, qfalse,
                        self->s.effect2Time);
  ...
  if (level.time > self->s.pos.trDuration) {
    ...
    trap_Trace(&tr, self->r.currentOrigin, mins, maxs, self->s.pos.trBase,
               self->s.number, MASK_SHOT);
...
// can be one of two types, but they have the same animations/etc, so re-use
// what you can
/*
==============
...
void Use_DamageInflictor(gentity_t* ent,
                         gentity_t* other,
                         gentity_t* activator) {
  gentity_t* daent;
...
void Use_Props_Shard_Generator(gentity_t* ent,
                               gentity_t* other,
                               gentity_t* activator) {
  int quantity;
  int type;
...
qboolean validOilSlickSpawnPoint(vec3_t point, gentity_t* ent) {
  ...
  if (traceEnt && traceEnt->classname) {
    if (!Q_stricmp(traceEnt->classname, "worldspawn")) {
      if (tr.plane.normal[0] == 0 && tr.plane.normal[1] == 0 &&
          tr.plane.normal[2] == 1) {
        return qtrue;
      }
    }
...
void Props_Barrel_Pain(gentity_t* ent,
                       gentity_t* attacker,
                       int damage,
                       vec3_t point) {
  if (ent->health <= 0) {
...
void Props_Barrel_Die(gentity_t* ent,
                      gentity_t* inflictor,
                      gentity_t* attacker,
                      int damage,
                      int mod) {
...
void props_crate32x64_die(gentity_t* ent,
                          gentity_t* inflictor,
                          gentity_t* attacker,
                          int damage,
                          int mod) {
...
!!!!!!!!!!!!!!
just a reminder to put the origin brush in the proper location for the leader
and the slave so that the table will flip over correctly.
*/
...
void props_flippy_table_die(gentity_t* ent,
                            gentity_t* inflictor,
                            gentity_t* attacker,
                            int damage,
                            int mod) {
...
void props_58x112tablew_die(gentity_t* ent,
                            gentity_t* inflictor,
                            gentity_t* attacker,
                            int damage,
                            int mod) {
...
void props_castlebed_touch(gentity_t* ent, gentity_t* other, trace_t* trace) {
  ...
  if (other->client->ps.pm_flags & PMF_JUMP_HELD &&
      other->s.groundEntityNum == ent->s.number && !other->client->ps.pm_time) {
...
void props_castlebed_die(gentity_t* ent,
                         gentity_t* inflictor,
                         gentity_t* attacker,
                         int damage,
                         int mod) {
...
void props_snowGenerator_use(gentity_t* ent,
                             gentity_t* other,
                             gentity_t* activator) {
  if (!(ent->spawnflags & 1)) {
    ent->spawnflags |= 1;
...
/*QUAKED props_FireColumn (.3 .2 .7) (-8 -8 -8) (8 8 8) CORKSCREW SMOKE GRAVITY
HALFGRAVITY this entity will require a target use an infonotnull to specifiy its
direction
...
/*void propsFireColumnUse (gentity_t *ent, gentity_t *other, gentity_t
*activator)
{
        gentity_t	*tent;
        vec3_t		start, dir;
        ...
        if (ent->speed)
        {
                ...
                VectorScale (dir, ent->speed + (crandom() * 100),
tent->s.pos.trDelta); SnapVector (tent->s.pos.trDelta); VectorCopy (start,
tent->r.currentOrigin);
        }
...
//	int i, numpieces;
        ...
        if (part)
        {
                ...
                if (ent->speed)
                {
                        ...
                        VectorScale (dir, ent->speed + (crandom() * 100),
part->s.pos.trDelta); SnapVector (part->s.pos.trDelta); VectorCopy (start,
...
"models/shards/2x4a.md3" ); else if (ent->count == 48) ent->s.modelindex =
G_ModelIndex ( "models/shards/2x4b.md3" ); else if (ent->count == 32)
                        ent->s.modelindex = G_ModelIndex (
"models/shards/2x4c.md3" ); else if (ent->count == 24) ent->s.modelindex =
G_ModelIndex ( "models/shards/2x4d.md3" ); else if (ent->count == 16)
                        ent->s.modelindex = G_ModelIndex (
"models/shards/2x4e.md3" ); else if (ent->count == 8) ent->s.modelindex =
G_ModelIndex ( "models/shards/2x4f.md3" );
        }
...
/*QUAKED props_decoration (.6 .7 .7) (-8 -8 0) (8 8 16) STARTINVIS DEBRIS
ANIMATE KEEPBLOCK TOUCHACTIVATE LOOPING STARTON "model2" will specify the model
to load
...
void props_decoration_death(gentity_t* ent,
                            gentity_t* inflictor,
                            gentity_t* attacker,
                            int damage,
                            int mod) {
...
void Use_props_decoration(gentity_t* ent,
                          gentity_t* self,
                          gentity_t* activator) {
  if (ent->spawnflags & 1) {
    trap_LinkEntity(ent);
...
/*QUAKED props_decorBRUSH (.6 .7 .7) ? STARTINVIS DEBRIS ANIMATE KEEPBLOCK -
LOOPING STARTON ANIMATE animate on death STARTON playanimation on death must
...
/*QUAKED props_decoration_scale (.6 .7 .7) (-8 -8 0) (8 8 16) STARTINVIS DEBRIS
ANIMATE KEEPBLOCK TOUCHACTIVATE LOOPING STARTON
...
void SP_skyportal(gentity_t* ent) {
  ...
  trap_SetConfigstring(
      CS_SKYBOXORG,
      va("%.2f %.2f %.2f %.1f %i %.2f %.2f %.2f %i %i", ent->s.origin[0],
         ent->s.origin[1], ent->s.origin[2], fov_x, (int)isfog, fogv[0],
         fogv[1], fogv[2], fogn, fogf));
  //----(SA)	end
}

/*QUAKED props_statue (.6 .3 .2) (-8 -8 0) (8 8 128) HURT DEBRIS ANIMATE
KEEPBLOCK "model2" will specify the model to load
...
"frames"	how many frames of animation to play
"delay"		how long of a delay before damage is inflicted ei. 0.5 sec
or 2.7 sec
...
void props_statue_blocked(gentity_t* ent) {
  ...
  if (traceEnt->takedamage && traceEnt->client) {
    ...
    VectorAdd(traceEnt->client->ps.velocity, kvel,
...
void props_statue_death(gentity_t* ent,
                        gentity_t* inflictor,
                        gentity_t* attacker,
                        int damage,
                        int mod) {
...
void props_locker_death(gentity_t* ent,
                        gentity_t* inflictor,
                        gentity_t* attacker,
                        int damage,
                        int mod);
...
void props_locker_pain(gentity_t* ent,
                       gentity_t* attacker,
                       int damage,
                       vec3_t point) {
...
void init_locker(gentity_t* ent) {
  ...
  ent->use = props_locker_use;  // trying it rattles the lock (could also allow
                                // 'waking' from trigger)
  ent->pain = props_locker_pain;

  // drop origin down 8 so the designer can put the box entity on the floor
  // rather than /in/ the floor remove if you get a new model from jason w/ the
  // origin moved up 8
  ent->s.origin[2] -= 8;
...
extern qhandle_t trap_R_GetShaderFromModel(qhandle_t modelid,
                                           int surfnum,
                                           int withlightmap);
...
void props_locker_mass(gentity_t* ent) {
  ...
  trap_SetConfigstring(
      CS_TARGETEFFECT,
      ent->dl_shader);  //----(SA)	allow shader to be set from entity
...
"wait"	 denotes how long the wait is going to be before the locked sound is
played again default is 1 sec "health" default is 1
...
"fabric") default is "wood" "mass" - defaults to 75.  This determines how much
debris is emitted.  You get one large chunk per 100 of mass (up to 8) and one
...
void props_locker_death(gentity_t* ent,
  ...
  trap_UnlinkEntity(ent);
  ent->r.maxs[2] = 11;  // (SA) make the dead bb half height so the item can
                        // look like it's sitting inside
...
void SP_props_footlocker(gentity_t* self) {
  ...
  self->s.modelindex =
      G_ModelIndex("models/mapobjects/furniture/footlocker.md3");
...
"random" how long of a random variance so the effect isnt exactly the same each
time for example 1.1 sec or 0.2 sec "size" valid ranges are 1.0 to 0.1
...
void props_flamethrower_think(gentity_t* ent) {
  ...
  if ((ent->timestamp + ent->duration) > level.time) {
    ...
    //		we actual need to create an entity that is the fire and will do
    //damage
...
void props_flamethrower_use(gentity_t* ent,
                            gentity_t* other,
                            gentity_t* activator) {
  int rval;
  int rnd;
...
void props_flamethrower_init(gentity_t* ent) {
  ...
  if (!target) {
    ...
    VectorCopy(angles, ent->s.apos.trBase);
    VectorCopy(angles,
               ent->s.angles);  // RF, added to fix weird release build issues
  }
