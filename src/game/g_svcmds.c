...
/*
 Check to see if the user is trying to sneak back in with g_enforcemaxlives
 enabled
*/
...
void G_StoreXPBackup(void) {
  ...
  for (i = 0; i < MAX_XPSTORAGEITEMS; i++) {
    if (!ipXPStorage.ipFilters[i].timeadded ||
        level.time - ipXPStorage.ipFilters[i].timeadded > (5 * 60000)) {
      trap_Cvar_Set(va("xpbackup%i", i), "");
      continue;
    }

    Com_sprintf(
        s, sizeof(s), "%u %u %f %f %f %f %f %f %f",
        ipXPStorage.ipFilters[i].filter.compare,
        ipXPStorage.ipFilters[i].filter.mask,
        ipXPStorage.ipFilters[i].skills[0], ipXPStorage.ipFilters[i].skills[1],
        ipXPStorage.ipFilters[i].skills[2], ipXPStorage.ipFilters[i].skills[3],
        ipXPStorage.ipFilters[i].skills[4], ipXPStorage.ipFilters[i].skills[5],
...
void ClearMaxLivesBans() {
  ...
  Q_strncpyz(ipMaxLivesFilters.cvarIPList, "g_maxlivesbanIPs",
             sizeof(ipMaxLivesFilters.cvarIPList));
}
...
void Svcmd_ForceTeam_f(void) {
  ...
  SetTeam(&g_entities[cl - level.clients], str, qfalse, cl->sess.playerWeapon,
          cl->sess.playerWeapon2, qtrue);
}
...
void Svcmd_StartMatch_f(void) {
  /*	if ( !g_noTeamSwitching.integer ) {
                  trap_SendServerCommand( -1, va("print \"g_noTeamSwitching not
     activated.\n\"")); return;
          }
  */
  ...
  /*
          if ( level.numPlayingClients <= 1 ) {
                  trap_SendServerCommand( -1, va("print \"Not enough playing
     clients to start match.\n\"")); return;
          }

          if ( g_gamestate.integer == GS_PLAYING ) {
                  trap_SendServerCommand( -1, va("print \"Match is already in
     progress.\n\"")); return;
          }
...
void Svcmd_SwapTeams_f(void) {
  ...
  if ((g_gamestate.integer == GS_INITIALIZE) ||
      (g_gamestate.integer == GS_WARMUP) || (g_gamestate.integer == GS_RESET)) {
    G_swapTeams();
    return;
  }
...
void Svcmd_RevivePlayer(char* name) {
  ...
  if (!g_cheats.integer) {
    trap_SendServerCommand(
        -1, va("print \"Cheats are not enabled on this server.\n\""));
    return;
  }
...
static void Svcmd_Kick_f(void) {
  ...
  if (!cl) {
    if (!Q_stricmp(name, "all")) {
      for (i = 0, cl = level.clients; i < level.numConnectedClients;
           i++, cl++) {
...
qboolean ConsoleCommand(void) {
  ...
  if (Q_stricmp(cmd, "savegame") == 0) {
    ...
    // don't allow a manual savegame command while we are waiting for the game
    // to start/exit
    ...
    if (strlen(cmd) > 0) {
      ...
      if (!Q_stricmp(cmd, "current")) {  // beginning of map
        Com_Printf(
            "sorry, '%s' is a reserved savegame name.  please use another "
            "name.\n",
            cmd);
        return qtrue;
      }
