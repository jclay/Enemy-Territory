...
static const char* gameNames[] = {"Single Player", "Cooperative",
                                  "Objective",     "Stopwatch",
...
typedef struct {
  ...
  int (*pVoteCommand)(gentity_t* ent,
...
static const vote_reference_t aVoteInfo[] = {
    ...
    {0x1ff, "gametype", G_Gametype_v, "Set Gametype to",
    ...
    {0x1ff, "campaign", G_Campaign_v, "Change campaign to",
     " <campaign>^7\n  Votes for a new map to be loaded"},
    {0x1ff, "maprestart", G_MapRestart_v, "Map Restart",
     "^7\n  Restarts the current map in progress"},
    {0x1ff, "matchreset", G_MatchReset_v, "Match Reset",
     "^7\n  Resets the entire match"},
    {0x1ff, "mutespecs", G_Mutespecs_v, "Mute Spectators",
     " <0|1>^7\n  Mutes in-game spectator chat"},
    {0x1ff, "nextmap", G_Nextmap_v, "Load Next Map",
     "^7\n  Loads the next map or campaign in the map queue"},
    {0x1ff, "pub", G_Pub_v, "Load Public Settings",
    ...
    {0x1ff, "swapteams", G_SwapTeams_v, "Swap Teams",
     " ^7\n  Switch the players on each team"},
    {0x1ff, "friendlyfire", G_FriendlyFire_v, "Friendly Fire",
     " <0|1>^7\n  Toggles ability to hurt teammates"},
    {0x1ff, "timelimit", G_Timelimit_v, "Timelimit",
    ...
    {0x1ff, "antilag", G_AntiLag_v, "Anti-Lag",
     " <0|1>^7\n  Toggles Anit-Lag on the server"},
    {0x1ff, "balancedteams", G_BalancedTeams_v, "Balanced Teams",
     " <0|1>^7\n  Toggles team balance forcing"},
    {0, 0, NULL, 0}};

// Checks for valid custom callvote requests from the client.
int G_voteCmdCheck(gentity_t* ent,
                   char* arg,
                   char* arg2,
                   qboolean fRefereeCmd) {
...
void G_voteHelp(gentity_t* ent, qboolean fShowVote) {
  int i, rows = 0,
         num_cmds = sizeof(aVoteInfo) / sizeof(aVoteInfo[0]) -
                    1;  // Remove terminator;
...
void G_voteCurrentSetting(gentity_t* ent,
                          const char* cmd,
                          const char* setting) {
...
// Vote toggling
int G_voteProcessOnOff(gentity_t* ent,
                       char* arg,
                       char* arg2,
                       qboolean fRefereeCmd,
                       int curr_setting,
  ...
  if ((atoi(arg2) && curr_setting) || (!atoi(arg2) && !curr_setting)) {
    G_refPrintf(ent, "^3%s^5 is already %s!", aVoteInfo[vote_type].pszVoteName,
                ((curr_setting) ? ENABLED : DISABLED));
    return (G_INVALID);
  }
...
int G_Comp_v(gentity_t* ent,
  ...
  if (arg) {
    if (trap_Argc() > 2) {
      ...
      return (G_INVALID);
    } else if (vote_allow_comp.integer <= 0 && ent &&
...
int G_Gametype_v(gentity_t* ent,
  ...
  if (arg) {
    ...
    if (!vote_allow_gametype.integer && ent && !ent->client->sess.referee) {
      G_voteDisableMessage(ent, arg);
      G_GametypeList(ent);
      G_voteCurrentSetting(
          ent, arg,
          va("%d (%s)", g_gametype.integer, gameNames[g_gametype.integer]));
      return (G_INVALID);
  ...
  if (arg) {
    ...
    if (!fRefereeCmd && ent) {
      if (level.clients[pid].sess.sessionTeam != TEAM_SPECTATOR &&
          level.clients[pid].sess.sessionTeam !=
...
int G_MatchReset_v(gentity_t* ent,
  ...
  if (arg) {
    if (!vote_allow_matchreset.integer && ent && !ent->client->sess.referee) {
      ...
      //				G_refPrintf(ent, "Usage: ^3%s %s%s\n",
      //((fRefereeCmd) ? "\\ref" : "\\callvote"), arg,
      //aVoteInfo[dwVoteIndex].pszVoteHelp); 				return(G_INVALID);
      //			}
    }
  ...
  if (arg) {
    if (trap_Argc() > 2) {
      G_refPrintf(ent, "Usage: ^3%s %s%s\n",
                  ...
                  aVoteInfo[dwVoteIndex].pszVoteHelp);
      return (G_INVALID);
    } else if (!vote_allow_nextmap.integer && ent &&
               !ent->client->sess.referee) {
      G_voteDisableMessage(ent, arg);
      return (G_INVALID);
    } else {
  ...
  if (arg) {
    if (trap_Argc() > 2) {
      G_refPrintf(ent, "Usage: ^3%s %s%s\n",
                  ...
                  aVoteInfo[dwVoteIndex].pszVoteHelp);
      return (G_INVALID);
    } else if (vote_allow_pub.integer <= 0 && ent &&
               !ent->client->sess.referee) {
      G_voteDisableMessage(ent, arg);
      return (G_INVALID);
    }
...
int G_Referee_v(gentity_t* ent,
  ...
  if (arg) {
    ...
    if (!ent->client->sess.referee && level.numPlayingClients < 3) {
      G_refPrintf(
          ent, "Sorry, not enough clients in the game to vote for a referee");
      return (G_INVALID);
    }
    ...
    if (level.clients[pid].sess.referee) {
      G_refPrintf(ent, "[lof]%s [lon]is already a referee!",
                  level.clients[pid].pers.netname);
      return (-1);
    }
    ...
    if (cl->pers.connected == CON_DISCONNECTED) {
      ...
      cl->sess.referee =
          RL_REFEREE;  // FIXME: Differentiate voted refs from passworded refs
  ...
  if (arg) {
    if (trap_Argc() > 2) {
      G_refPrintf(ent, "Usage: ^3%s %s%s\n\n",
                  ...
                  aVoteInfo[dwVoteIndex].pszVoteHelp);
      return (G_INVALID);
    } else if (!vote_allow_shuffleteamsxp.integer && ent &&
               !ent->client->sess.referee) {
      G_voteDisableMessage(ent, arg);
      return (G_INVALID);
    }
  ...
  if (arg) {
    if (trap_Argc() > 2) {
      G_refPrintf(ent, "Usage: ^3%s %s%s\n",
                  ...
                  aVoteInfo[dwVoteIndex].pszVoteHelp);
      return (G_INVALID);
    } else if (!vote_allow_swapteams.integer && ent &&
               !ent->client->sess.referee) {
      G_voteDisableMessage(ent, arg);
      return (G_INVALID);
    }
...
int G_AntiLag_v(gentity_t* ent,
  ...
  if (arg) {
    return (G_voteProcessOnOff(ent, arg, arg2, fRefereeCmd,
                               !!(g_antilag.integer),
                               vote_allow_antilag.integer, dwVoteIndex));
...
int G_Warmupfire_v(gentity_t* ent,
  ...
  if (arg) {
    ...
    if (i == val) {
      G_refPrintf(ent, "\n^3Warmup Damage^5 is already set to %s!",
                  warmupType[i]);
      return (G_INVALID);
    }
    ...
    trap_SendConsoleCommand(
        EXEC_APPEND, va("match_warmupDamage %s\n", level.voteInfo.vote_value));
  }
...
int G_Unreferee_v(gentity_t* ent,
  ...
  if (arg) {
    ...
    if (level.clients[pid].sess.referee == RL_NONE) {
      G_refPrintf(ent, "[lof]%s [lon]isn't a referee!",
                  level.clients[pid].pers.netname);
      return (G_INVALID);
    }

    if (level.clients[pid].sess.referee == RL_RCON) {
      G_refPrintf(ent, "[lof]%s's [lon]status cannot be removed",
                  level.clients[pid].pers.netname);
      return (G_INVALID);
    }

    if (level.clients[pid].pers.localClient) {
      G_refPrintf(ent, "[lof]%s's [lon]is the Server Host",
                  level.clients[pid].pers.netname);
      return (G_INVALID);
    }
