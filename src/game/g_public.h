...
#define SVF_VISDUMMY \
  0x00000004  // this ent is a "visibility dummy" and needs it's master to be
              // sent to clients that can see it even if they can't see the
              // master ent
...
#define SVF_BLANK \
  0x00000080  // Gordon: removed SVF_USE_CURRENT_ORIGIN as it plain doesnt do
              // anything
...
#define SVF_VISDUMMY_MULTIPLE \
  0x00000400  // so that one vis dummy can add to snapshot multiple speakers

// recent id changes
#define SVF_SINGLECLIENT \
  0x00000800  // only send to a single client (entityShared_t->singleClient)
...
#define SVF_IGNOREBMODELEXTENTS \
  0x00004000  // just use origin for in pvs check for snapshots, ignore the
              // bmodel extents
#define SVF_SELF_PORTAL 0x00008000  // use self->origin2 as portal
#define SVF_SELF_PORTAL_EXCLUSIVE \
  0x00010000  // use self->origin2 as portal and DONT add self->origin PVS ents
...
typedef enum {
  ...
  GAME_SNAPSHOT_CALLBACK,  // ( int entityNum, int clientNum ); // return qfalse
                           // if you don't want it to be added
  ...
  // zinx
  GAME_MESSAGERECEIVED,  // ( int cno, const char *buf, int buflen, int
                         // commandTime );
                         // -zinx
} gameExport_t;
