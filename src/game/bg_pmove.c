...
void PM_TraceLegs(trace_t* trace,
                  float* legsOffset,
                  vec3_t start,
                  vec3_t end,
                  trace_t* bodytrace,
...
void PM_TraceAllLegs(trace_t* trace,
                     float* legsOffset,
                     vec3_t start,
                     vec3_t end) {
...
static void PM_Friction(void) {
  ...
  // rain - #179 don't do this for PM_SPECTATOR/PM_NOCLIP, we always want them
  // to stop
...
static qboolean PM_CheckJump(void) {
  ...
  // JPW NERVE -- jumping in multiplayer uses and requires sprint juice (to
  // prevent turbo skating, sprint + jumps) don't allow jump accel
  ...
  //	if (pm->pmext->sprintTime < 2500) // JPW pulled this per id request;
  //made airborne jumpers wildly inaccurate with gunfire to compensate 		return
  //qfalse;
  // jpw
...
static qboolean PM_CheckProne(void) {
  ...
  if (pm->ps->eFlags & EF_PRONE) {
    ...
    // pm->maxs[2] = pm->ps->maxs[2] - (frac * (pm->ps->standViewHeight -
    // PRONE_VIEWHEIGHT)); pm->ps->viewheight = DEFAULT_VIEWHEIGHT - (frac *
    // (DEFAULT_VIEWHEIGHT - PRONE_VIEWHEIGHT));	// default - prone to
    // get a positive which is subtracted from default
...
static qboolean PM_CheckDodge(void) {
  ...
  //request; made airborne jumpers wildly inaccurate with gunfire to compensate
  //			return qfalse;
  // jpw
  ...
  /*	if ( pm->pmext->sprintTime < 3000 )
          ...
          if ( pm->ps->pm_flags & PMF_RESPAWNED || pm->ps->pm_flags & PMF_DUCKED
     || pm->ps->eFlags & EF_PRONE ) { return qfalse;		// don't allow
     dodge until all buttons are up, player is prone or crouching
          }
...
static void PM_CrashLand(void) {
  ...
  if (!(pml.groundTrace.surfaceFlags & SURF_NODAMAGE)) {
    ...
    /* JPW NERVE removed from MP, breaks too many levels and skill as
       no-fall-damage indicator isn't obvious
                    // Rafael gameskill
...
static void PM_GroundTraceMissed(void) {
  ...
  // If we've never yet touched the ground, it's because we're spawning, so
  // don't set to "in air"
...
static void PM_CheckDuck(void) {
  ...
  if (pm->ps->pm_type == PM_DEAD) {
    pm->maxs[2] =
        pm->ps->maxs[2];  // NOTE: must set death bounding box in game code
    ...
    if (pm->ps->pm_flags & PMF_FLAILING) {
      ...
      if (!pm->ps->pm_time) {
        pm->ps->pm_flags &= ~PMF_FLAILING;  // the eagle has landed
               ...
               !(pm->ps->pm_flags &
                 PMF_LIMBO)) {  // DHM - Nerve :: before going to limbo, play a
  ...
  if (pm->ps->groundEntityNum == ENTITYNUM_NONE) {
    if (pm->ps->pm_flags & PMF_LADDER) {  // on ladder
      if (pm->ps->velocity[2] >= 0) {
        ...
        // BG_PlayAnimName( pm->ps, "BOTH_CLIMB", ANIM_BP_BOTH, qfalse, qtrue,
        // qfalse );
      } else if (pm->ps->velocity[2] < 0) {
        animResult = BG_AnimScriptAnimation(
            pm->ps, pm->character->animModelInfo, ANIM_MT_CLIMBDOWN, qtrue);
        // BG_PlayAnimName( pm->ps, "BOTH_CLIMB_DOWN", ANIM_BP_BOTH, qfalse,
        // qtrue, qfalse );
      }
    }
...
static void PM_BeginWeaponReload(int weapon) {
  ...
  switch (weapon) {
    ...
    default:
      // DHM - Nerve :: override current animation (so reloading after firing
      // will work)
      if (pm->ps->eFlags & EF_PRONE) {
  ...
  // okay to reload while overheating without tacking the reload time onto the
  // end of the current weaponTime (the reload time is partially absorbed into
  // the overheat time)
...
void PM_BeginWeaponChange(
    int oldweapon,
    int newweapon,
    qboolean reload) {  //----(SA)	modified to play 1st person alt-mode
...
static void PM_FinishWeaponChange(void) {
  ...
  switch (newweapon) {
      ...
      // However, need to set the animspreadscale so they are initally at worst
      // accuracy
    case WP_K43_SCOPE:
    case WP_GARAND_SCOPE:
    case WP_FG42SCOPE:
  ...
  // doesn't happen too often (player switched weapons away then back very
  // quickly)
  ...
  if (pm->ps->weaponTime <= 0) {
    ...
    if (reloadRequested) {
      if (pm->ps->ammo[ammoWeap]) {
        ...
        if (BG_IsAkimboWeapon(weapon)) {
          if (pm->ps->ammoclip[BG_FindClipForWeapon(BG_AkimboSidearm(weapon))] <
              GetAmmoTableData(BG_FindClipForWeapon(BG_AkimboSidearm(weapon)))
                  ->maxclip) {
            doReload = qtrue;
          }
        }
      ...
      if (!pm->ps->ammoclip[clipWeap] && pm->ps->ammo[ammoWeap]) {
        if (BG_IsAkimboWeapon(weapon)) {
          if (!pm->ps
                   ->ammoclip[BG_FindClipForWeapon(BG_AkimboSidearm(weapon))]) {
            doReload = qtrue;
          }
...
static void PM_SwitchIfEmpty(void) {
  ...
  if (pm->ps->ammoclip[BG_FindClipForWeapon(
          pm->ps->weapon)]) {  // still got ammo in clip
    return;
  }

  if (pm->ps->ammo[BG_FindAmmoForWeapon(
          pm->ps->weapon)]) {  // still got ammo in reserve
    return;
  }

  // If this was the last one, remove the weapon and switch away before the
  // player tries to fire next

  // NOTE: giving grenade ammo to a player will re-give him the weapon (if you
  // do it through add_ammo())
  switch (pm->ps->weapon) {
    case WP_GRENADE_LAUNCHER:
    case WP_GRENADE_PINEAPPLE:
    case WP_DYNAMITE:
...
void PM_CoolWeapons(void) {
  ...
  // a weapon is currently selected, convert current heat value to 0-255 range
...
#define AIMSPREAD_DECREASE_RATE \
  200.0f  // (SA) when I made the increase/decrease floats (so slower weapon
          // recover could happen for scoped weaps) the average rate increased
          // significantly
...
void PM_AdjustAimSpreadScale(void) {
  ...
  int i;
  float increase,
      decrease;  // (SA) was losing lots of precision on slower weapons (scoped)
  ...
  switch (pm->ps->weapon) {
    ...
    case WP_MP40:
      wpnScale =
          0.6f;  // 2 handed, but not as long as mauser, so harder to keep aim
      break;
    case WP_GARAND:
      wpnScale = 0.5f;
      break;
...
static void PM_Weapon(void) {
  ...
  int ammoNeeded;
  qboolean delayedFire;  //----(SA)  true if the delay time has just expired and
  ...
  switch (pm->ps->persistant[PERS_HWEAPON_USE]) {
    case 1:
      //			PM_CoolWeapons(); // Gordon: Arnout says this is
      //how it's wanted ( bleugh ) no cooldown on weaps while using mg42, but
      //need to update heat on mg42 itself
      ...
      if (pm->cmd.buttons & BUTTON_ATTACK) {
        ...
        if (pm->ps->weapHeat[WP_DUMMY_MG42] >= MAX_MG42_HEAT) {
          ...
          pm->ps->weaponTime =
              2000;  // force "heat recovery minimum" to 2 sec right now
        }
      }
      return;
    case 2:
  ...
  if (pm->ps->eFlags & EF_MOUNTEDTANK) {
    //		PM_CoolWeapons(); // Gordon: Arnout says this is how it's wanted
    //( bleugh ) no cooldown on weaps while using mg42, but need to update heat
    //on mg42 itself
    if (pm->ps->weapHeat[WP_DUMMY_MG42]) {
      pm->ps->weapHeat[WP_DUMMY_MG42] -= (300.f * pml.frametime);
    ...
    if (pm->cmd.buttons & BUTTON_ATTACK) {
      ...
      BG_AnimScriptEvent(pm->ps, pm->character->animModelInfo,
                         ANIM_ET_FIREWEAPON, qfalse, qtrue);
      // pm->ps->viewlocked = 2;		// this enable screen jitter
      // when firing
  ...
  if (BG_IsAkimboWeapon(pm->ps->weapon)) {
    akimboFire = BG_AkimboFireSequence(
        pm->ps->weapon, pm->ps->ammoclip[BG_FindClipForWeapon(pm->ps->weapon)],
        pm->ps
            ->ammoclip[BG_FindClipForWeapon(BG_AkimboSidearm(pm->ps->weapon))]);
  } else {
    akimboFire = qfalse;
  }
  ...
  // do the recoil before setting the values, that way it will be shown next
  // frame and not this
  ...
  if (pm->ps->weapon == WP_GRENADE_LAUNCHER ||
    ...
    if (pm->ps->grenadeTimeLeft > 0) {
      ...
      if (pm->ps->weapon == WP_DYNAMITE) {
        ...
        // JPW NERVE -- in multiplayer, dynamite becomes strategic, so start
        // timer @ 30 seconds
        ...
        if (pm->ps->grenadeTimeLeft <=
            100) {  // give two frames advance notice so there's time to launch
                    // and detonate
          forcethrow = qtrue;
  ...
  if (pm->ps->weaponDelay > 0) {
    ...
    if (pm->ps->weaponDelay <= 0) {
      ...
      // double check the player is still holding the fire button down for these
      // weapons so you don't get a delayed "non-fire" (fire hit and released,
      // then shot fires)
      switch (pm->ps->weapon) {
        default:
          break;
  ...
  if (pm->ps->weaponstate == WEAPON_RAISING) {
    ...
    //WP_MOBILE_MG42 ) 			pm->pmext->proneMG42Zoomed = qtrue;
  ...
  // JPW NERVE -- in multiplayer, don't allow panzerfaust or dynamite to fire if
  // charge bar isn't full
  if (pm->ps->weapon == WP_PANZERFAUST) {
    if (pm->ps->eFlags & EF_PRONE) {
      return;
    }
  ...
  if ((!(pm->cmd.buttons & (BUTTON_ATTACK | WBUTTON_ATTACK2)) &&
       !delayedFire) ||
  ...
  if (pm->waterlevel == 3) {
    if (pm->ps->weapon != WP_KNIFE && pm->ps->weapon != WP_GRENADE_LAUNCHER &&
      ...
      PM_AddEvent(
          EV_NOFIRE_UNDERWATER);  // event for underwater 'click' for nofire
      pm->ps->weaponTime = 500;
      pm->ps->weaponDelay =
          0;  // avoid insta-fire after water exit on delayed weapon attacks
      return;
    }
  }
  ...
  switch (pm->ps->weapon) {
    default:
      if (!weaponstateFiring) {
        // delay so the weapon can get up into position before firing (and
        // showing the flash)
        ...
        if (PM_WeaponAmmoAvailable(pm->ps->weapon)) {
          if (pm->ps->weapon == WP_DYNAMITE) {
            ...
            pm->ps->grenadeTimeLeft =
                4000;  // start at four seconds and count down
          }
  ...
  //	if( pm->ps->weapon != WP_SMOKE_BOMB && pm->ps->weapon != WP_SATCHEL &&
  //pm->ps->weapon != WP_SATCHEL_DET ) {	// Arnout: not for these weapons
  ...
  if (pm->ps->weapon) {
    ...
    if (ammoNeeded > ammoAvailable) {
      ...
      // if not in auto-reload mode, and reload was not explicitely requested,
      // just play the 'out of ammo' sound
      ...
      switch (pm->ps->weapon) {
        ...
        case WP_SMOKE_BOMB:
          ...
          // some weapons not allowed to reload.  must switch back to primary
          // first
        case WP_FG42SCOPE:
        case WP_GARAND_SCOPE:
        case WP_K43_SCOPE:
  ...
  if (pm->ps->weaponDelay > 0) {
    // if it hits here, the 'fire' has just been hit and the weapon dictated a
    // delay. animations have been started, weaponstate has been set, but no
    // weapon events yet. (except possibly EV_NOAMMO) checks for delayed weapons
  ...
  // JPW NERVE -- in multiplayer, pfaust fires once then switches to pistol
  // since it's useless for a while
  if ((pm->ps->weapon == WP_PANZERFAUST) ||
  ...
  // WP_M7 and WP_GPG40 run out of ammo immediately after firing their last
  // grenade
  ...
  switch (pm->ps->weapon) {
    ...
    case WP_AKIMBO_SILENCEDCOLT:
      ...
      // added check for last shot in both guns so there's no delay for the last
      // shot
      if (!pm->ps->ammoclip[BG_FindClipForWeapon(pm->ps->weapon)]) {
        if (!akimboFire) {
          addTime = 2 * GetAmmoTableData(pm->ps->weapon)->nextShotTime;
        }
      } else if (!pm->ps->ammoclip[BG_FindClipForWeapon(
    ...
    case WP_LOCKPICK:
      ...
      break;
      // jpw
      // JPW: engineers disarm bomb "on the fly" (high sample rate) but medics &
      // LTs throw out health pack/smoke grenades slow NERVE - SMF
    case WP_PLIERS:
      addTime = 50;
      break;
    case WP_MEDKIT:
  ...
  if (GetAmmoTableData(pm->ps->weapon)->maxHeat &&
    ...
    if (pm->ps->weapHeat[pm->ps->weapon] >=
      ...
      PM_AddEvent(EV_WEAP_OVERHEAT);
      //			PM_StartWeaponAnim(WEAP_IDLE1);	// removed.
...
void PM_UpdateLean(playerState_t* ps, usercmd_t* cmd, pmove_t* tpm) {
  ...
  if (leaning) {
    ...
    VectorSet(tmins, -8, -8, -7);  // ATVI Wolfenstein Misc #472, bumped from -4
...
void PM_UpdateViewAngles(playerState_t* ps,
  ...
  if (ps->pm_type != PM_SPECTATOR && ps->stats[STAT_HEALTH] <= 0) {
    // DHM - Nerve :: Allow players to look around while 'wounded' or lock to a
    // medic if nearby
...
#define TRACE_LADDER_DIST 48.0
  ...
  */
  ...
  /*
     ...
     if we are trying to reverse onto it from above the ladder flatforward[0] =
     -pm->ps->velocity[0]; flatforward[1] = -pm->ps->velocity[1]; flatforward[2]
  ...
  */
  ...
  if (pml.ladder && !pml.walking && (trace.fraction * tracedist > 1.0)) {
    vec3_t mins;
    // if we are only just on the ladder, don't do this yet, or it may throw us
    // back off the ladder
    pml.ladder = qfalse;
  ...
  if (pml.ladder) {
    if (pml.walking) {
      // we are currently on the ground, only go up and prevent X/Y if we are
      // pushing forwards
      if (pm->cmd.forwardmove <= 0) {
        pml.ladder = qfalse;
      }
...
void PM_LadderMove(void) {
  ...
  if (pm->cmd.rightmove) {
    ...
    // VectorMA( wishvel, 0.5 * scale * (float)pm->cmd.rightmove, pml.right,
    // wishvel );
...
void PM_Sprint(void) {
  if (pm->cmd.buttons & BUTTON_SPRINT &&
    ...
    // mucked about with to eliminate client-side framerate-dependancy in wolf
    // single player
    if (pm->ps->powerups[PW_ADRENALINE]) {
      pm->pmext->sprintTime = SPRINTTIME;
    } else if (pm->ps->powerups[PW_NOFATIGUE]) {  // (SA) recharge at 2x with
                                                  // stamina powerup
      pm->pmext->sprintTime += 10;
    } else {
      ...
      pm->pmext->sprintTime +=
          rechargebase *
          pml.frametime;  // JPW NERVE adjusted for framerate independence
...
void PmoveSingle(pmove_t* pmove) {
  ...
  if (pm->cmd.wbuttons & WBUTTON_ZOOM && pm->ps->stats[STAT_HEALTH] >= 0 &&
      !(pm->ps->weaponDelay)) {
    if (pm->ps->stats[STAT_KEYS] &
        (1 << INV_BINOCS)) {  // (SA) binoculars are an inventory item
                              // (inventory==keys)
      if (!BG_IsScopedWeapon(pm->ps->weapon) &&  // don't allow binocs if using
                                                 // the sniper scope
          !BG_PlayerMounted(pm->ps->eFlags) &&   // or if mounted on a weapon
          // rain - #215 - don't allow binocs w/ mounted mob. MG42 or mortar
          // either.
  ...
  // this is the one we were using
          ...
          pm->ps->eFlags &= ~EF_PRONE;
          pm->ps->eFlags &= ~EF_PRONE_MOVING;
          pm->pmext->proneTime = -pm->cmd.serverTime;	// timestamp 'stop
  prone'
