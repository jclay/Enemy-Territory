...
/*QUAKED light (0 1 0) (-8 -8 -8) (8 8 8) nonlinear angle negative_spot
negative_point q3map_non-dynamic Non-displayed light. "light" overrides the
default 300 intensity. Nonlinear checkbox gives inverse square falloff instead
of linear Angle adds light:surface angle calculations (only valid for "Linear"
lights) (wolf) Lights pointed at a target will be spotlights. "radius" overrides
the default 64 unit radius of a spotlight at the target point. "fade"
falloff/radius adjustment value. multiply the run of the slope by "fade" (1.0f
default) (only valid for "Linear" lights) (wolf) "q3map_non-dynamic" specifies
that this light should not contribute to the world's 'light grid' and therefore
will not light dynamic models in the game.(wolf)
*/
...
negative_spot negative_point Non-displayed light that only affects dynamic game
models, but does not contribute to lightmaps "light" overrides the default 300
intensity. Nonlinear checkbox gives inverse square falloff instead of linear
Angle adds light:surface angle calculations (only valid for "Linear" lights)
(wolf) Lights pointed at a target will be spotlights. "radius" overrides the
default 64 unit radius of a spotlight at the target point. "fade" falloff/radius
adjustment value. multiply the run of the slope by "fade" (1.0f default) (only
valid for "Linear" lights) (wolf)
*/
...
void TeleportPlayer(gentity_t* player, vec3_t origin, vec3_t angles) {
  ...
  /*	if ( player->client->sess.sessionTeam != TEAM_SPECTATOR ) {
     ...
     EV_PLAYER_TELEPORT_OUT ); tent->s.clientNum = player->s.clientNum;
  ...
  /*	AngleVectors( angles, player->client->ps.velocity, NULL, NULL );
     ...
     player->client->ps.velocity ); player->client->ps.pm_time = 160;
     // hold time player->client->ps.pm_flags |= PMF_TIME_KNOCKBACK;*/
...
static int attackDurations[] = {(11 * 1000) / 15, (16 * 1000) / 15,
                                (16 * 1000) / 15};

static int attackHittimes[] = {(7 * 1000) / 15, (6 * 1000) / 15,
                               (7 * 1000) / 15};
...
void grabber_think_hit(gentity_t* ent) {
  G_RadiusDamage(ent->s.pos.trBase, NULL, ent, ent->damage, ent->duration, ent,
                 MOD_GRABBER);
  G_AddEvent(ent, EV_GENERAL_SOUND,
             ent->sound2to1);  // sound2to1 is the 'pain' sound

  ent->nextthink = level.time + (attackDurations[(ent->s.frame) - 2] -
                                 attackHittimes[(ent->s.frame) - 2]);
...
void grabber_die(gentity_t* ent,
                 gentity_t* inflictor,
                 gentity_t* attacker,
                 int damage,
                 int mod) {
...
==============
*/
void grabber_pain(gentity_t* ent,
                  gentity_t* attacker,
                  int damage,
                  vec3_t point) {
  G_AddEvent(ent, EV_GENERAL_SOUND,
...
==============
grabber_wake
        ent calling this is the bounding box for the grabber, not the grabber
ent itself. the grabber ent is 'ent->parent'
==============
*/
void grabber_wake(gentity_t* ent) {
  ...
  if (1) {  // temp fast trigger
    VectorAdd(ent->r.mins, tv(-(ent->random), -(ent->random), -(ent->random)),
              ent->r.mins);
    VectorAdd(ent->r.maxs, tv(ent->random, ent->random, ent->random),
              ent->r.maxs);
  }
  ...
  G_AddEvent(ent, EV_GENERAL_SOUND,
             ent->soundPos1);  // soundPos1 is the 'wake' sound
}
...
fields:
"adist"  - radius of 'wakeup' box.  player passing closer than distance
activates grabber (def: 64) "bdist"  - radius of 'attack' box.  player passing
into this gets a swipe.  (def: 32) "health" - how much damage grabber can take
after 'wakeup' (def: 100) "range"  - when attacking, how far from the origin the
grabber can strike (def: 64) "dmg"    - max damage to give on a successful
strike (def: 10) "wait"   - how long to wait between strikes if the player stays
within the 'attack' box (def: see below)

If you do not set a "wait" value, then it will default to the duration of the
animations.  (so since the first attack animation is 11 frames long and plays at
15 fps, the default wait after using attack 1 would be 11/15, or 0.73 seconds)
...
void SP_misc_grabber_trap(gentity_t* ent) {
  ...
  // TODO: change from 'trap' to something else.  'trap' is a misnomer.  it's
  // actually used for other stuff too
  ent->s.eType = ET_TRAP;
  ...
  VectorSet(ent->r.mins, -12, -12,
            0);  // target area for shooting it after it wakes
  ...
  // just make an even trigger box around the ent (do properly sized/oriented
  // trigger after it's working)
  if (1) {  // temp fast trigger
    VectorAdd(trig->r.mins, tv(-(trig->count), -(trig->count), -(trig->count)),
              trig->r.mins);
    VectorAdd(trig->r.maxs, tv(trig->count, trig->count, trig->count),
              trig->r.maxs);
  }
...
"model" - 'base' model that moves with the light.  Default:
"models/mapobjects/light/searchlight_pivot.md3" "target" - .camera (spline) file
for light to track.  do not specify file extension.

BACK_AND_FORTH - when end of target spline is hit, reverse direction rather than
looping (looping is default) ( /\ not active yet /\ )
*/
void SP_misc_spotlight(gentity_t* ent) {
  ...
  if (ent->model) {
    ...
    ent->s.modelindex =
        G_ModelIndex("models/mapobjects/light/searchlight_pivot.md3");
  }

  if (ent->target) {
    ent->s.density = G_FindConfigstringIndex(ent->target, CS_SPLINES,
                                             MAX_SPLINE_CONFIGSTRINGS, qtrue);
  }
}
...
START_ANIMATE md3 placed in the game at runtime (rather than in the bsp) "model"
arbitrary .md3 file to display "modelscale"	scale multiplier (defaults to
1x, and scales uniformly) "modelscale_vec"	scale multiplier (defaults to 1
1 1, scales each axis as requested) "skin"			.skin file used
to define shaders for model "trunk"			diameter of solid core
(used for trace visibility and collision (not ai pathing)) "trunkheight"
height of trunk
...
ORIENT_LOD - if flagged, the entity will yaw towards the player when the LOD
switches START_ANIMATE - if flagged, the entity will spawn animating
...
void SP_misc_gamemodel(gentity_t* ent) {
  ...
  // Gordon: static gamemodels client side only now :D so server can just wave
  // bye-bye
  ...
  if (ent->spawnflags & 2) {
    ...
    if (num_frames == 0) {
      G_Error(
          "'misc_model' with ANIMATE spawnflag set has 'frames' set to 0\n");
    }
  ...
  if (ent->model) {
    ...
    /*		if( !(ent->tagNumber = trap_LoadTag( tagname )) ) {
                            Com_Error( ERR_DROP, "Failed to load Tag File
       (%s)\n", tagname );
                    }*/
  }
  ...
  if (ent->spawnflags & 1) {
    ent->s.apos.trType =
        1;  // misc_gamemodels (since they have no movement) will use type = 0
            // for static models, type = 1 for auto-aligning models
  }
  trap_LinkEntity(ent);
}
...
active whether it is in the player's PVS or not. This entity itself is never
visible or transmitted to clients. For safety, you should have each dummy only
point at one entity (however, it's okay to have many dummies pointing at one
entity)
*/
...
If this entity is "visible" (in player's PVS) then it's target is forced to be
active whether it is in the player's PVS or not. This entity itself is never
visible or transmitted to clients. This entity was created to have multiple
speakers targeting it
*/
...
The surfaces nearest these entities will be the only surfaces lit by the
targeting light This must be within 64 world units of the surface to be lit!
*/
...
The target for a misc_portal_director.  You can set either angles or target
another entity to determine the direction of view. "roll" an angle modifier to
orient the camera around the target vector;
*/
...
void Use_Shooter(gentity_t* ent, gentity_t* other, gentity_t* activator) {
  ...
  if (ent->s.weapon == WP_MAPMORTAR) {
    AimAtTarget(ent);  // store in ent->s.origin2 the direction/force needed to
  ...
  switch (ent->s.weapon) {
    case WP_GRENADE_LAUNCHER:
      VectorScale(dir, 700,
                  dir);  //----(SA)	had to add this as fire_grenade now
                         //expects a non-normalized direction vector
    ...
    case WP_MAPMORTAR:
      AimAtTarget(ent);  // store in ent->s.origin2 the direction/force needed
...
entity "random" the number of degrees of deviance from the taget. (1.0 default)
if LAUNCH_FX is checked a smoke effect will play at the origin of this entity.
if FLASH_FX is checked a muzzle flash effect will play at the origin of this
entity.
*/
void SP_shooter_mortar(gentity_t* ent) {
  // (SA) TODO: must have a self->target.  Do a check/print if this is not the
  // case.
...
(rgb). "scale" will designate a multiplier to the default size.  (so 2.0 is
2xdefault size, 0.5 is half)
*/
...
void SP_corona(gentity_t* ent) {
  ...
  ent->s.dl_intensity = (int)ent->dl_color[0] | ((int)ent->dl_color[1] << 8) |
                        ((int)ent->dl_color[2] << 16);
...
void dlight_finish_spawning(gentity_t* ent) {
  G_FindConfigstringIndex(
      va("%i %s %i %i %i", ent->s.number, ent->dl_stylestring, ent->health,
         ent->soundLoop, ent->dl_atten),
...
should be all lowercase Stylestring characters run at 10 cps in the game.
(meaning the alphabet, at 24 characters, would take 2.4 seconds to cycle)
...
style out of sync). "atten": offset from the alpha values of the stylestring.
stylestring of "ddeeffzz" with an atten of -1 would result in "ccddeeyy" Use
color picker to set color or key "color".  values are 0.0-1.0 for each color
(rgb). FORCEACTIVE	- toggle makes sure this light stays alive in a map even
if the user has r_dynamiclight set to 0. STARTOFF	- means the dlight
doesn't spawn in until ent is triggered ONETIME		- when the dlight is
triggered, it will play through it's cycle once, then shut down until triggered
again "shader" name of shader to apply "sound" sound to loop every cycle (this
actually just plays the sound at the beginning of each cycle)
...
==============
shutoff_dlight
        the dlight knew when it was triggered to unlink after going through it's
cycle once
==============
*/
...
void SP_dlight(gentity_t* ent) {
  ...
  if (ent->dl_stylestring &&
      strlen(
          ent->dl_stylestring)) {  // if they're specified in a string, use em
  } else if (style) {
    style = max(1, style);  // clamp to predefined range
    style = min(19, style);
    ent->dl_stylestring =
        predef_lightstyles[style - 1];  // these are input as 1-20
  } else {
    ent->dl_stylestring = "mmmaaa";  // default to a strobe to call attention to
                                     // this not being set
  }
  ...
  ent->dl_color[0] =
      ent->dl_color[0] *
      255;  // range 0-255 now so the client doesn't have to on every update
  ent->dl_color[1] = ent->dl_color[1] * 255;
  ent->dl_color[2] = ent->dl_color[2] * 255;
...
==============
*/
void Fire_Lead_Ext(gentity_t* ent,
                   gentity_t* activator,
                   float spread,
                   int damage,
                   vec3_t muzzle,
  ...
  G_HistoricalTrace(activator, &tr, muzzle, NULL, NULL, end, ent->s.number,
                    MASK_SHOT);
  ...
  if (traceEnt->takedamage && traceEnt->client) {
    ...
    tent->s.otherEntityNum2 =
        activator->s.number;  // (SA) store the user id, so the client can
                              // position the tracer
...
// NOTE: this only effects the external view of the user, when using the mg42,
// the view position is set on the client-side to keep it firm behind the gun
// with interpolation
...
// Hitscan
// Use some part of entitystate to store firing barrel? or just encode it with
// event more likely Shouldn't really have smoke puff in air, but may add anyway
// for effect
//
...
void aagun_track(gentity_t* self, gentity_t* other) {
  ...
  if (other->active) {
    ...
    VectorSubtract(other->client->ps.viewangles, self->s.apos.trBase,
                   self->s.apos.trDelta);
    for (i = 0; i < 3; i++) {
...
void aagun_fire(gentity_t* other) {
  ...
  SnapVector(muzzle);
  Fire_Lead(self, other, AAGUN_SPREAD, AAGUN_DAMAGE, muzzle, forward, right,
            up);
}
...
void aagun_touch(gentity_t* self, gentity_t* other, trace_t* trace) {
  /*	if (!self->active) {
          ...
          if (other->active) {
                  ...
                  for (i = 0; i < 3; i++) {
                          dang[i] =
     SHORT2ANGLE(other->client->pers.cmd.angles[i]);
                  }

                  // now tell the client to lock the view in the direction of
     the gun other->client->ps.viewlocked = 3;
...
void mg42_fire(gentity_t* other) {
  ...
  // VectorMA (muzzle, 16, forward, muzzle); // JPW NERVE unnecessary and makes
  // it so close-range enemies get missed

  // Arnout: disabled this as it was causing troubles with murderholes. Why was
  // it there? Maybe for beach to let the mg42 point down and still shoot over
  // teh concrete? FIX: make the HIGH spawnflag actually work
  ...
  Fire_Lead(self, other, MG42_SPREAD_MP, MG42_DAMAGE_MP, muzzle, forward, right,
            up);
}
...
void mg42_think(gentity_t* self) {
    ...
    if (self->flameQuotaTime < level.time) {
      ...
      if (owner->client) {
        ...
        self->flameQuotaTime = level.time + 2000;
             ...
             (self->s.eFlags & EF_OVERHEATING) == EF_OVERHEATING) {
    self->s.eFlags &= ~EF_OVERHEATING;
  ...
  if (self->mg42weapHeat) {
    self->mg42weapHeat -=
        (300.f * FRAMETIME * 0.001);  //%	-= (300.f * 50 * 0.001);
...
void mg42_die(gentity_t* self,
  ...
  if (owner && owner->client) {
    ...
    trap_Trace(&tr, owner->r.currentOrigin, owner->r.mins, owner->r.maxs,
               owner->r.currentOrigin, owner->s.number, MASK_PLAYERSOLID);
    if (tr.startsolid) {
      VectorCopy(owner->TargetAngles, owner->client->ps.origin);
      VectorCopy(owner->TargetAngles, owner->r.currentOrigin);
      owner->r.contents =
          CONTENTS_CORPSE;  // this will correct itself in ClientEndFrame
    }
    owner->client->ps.eFlags &= ~EF_MG42_ACTIVE;  // DHM - Nerve :: unset flag
...
void mg42_spawn(gentity_t* ent) {
  ...
  if (g_knifeonly.integer != 1) {
    // Need to spawn the base even when no tripod cause the gun itself isn't
    // solid
    base = G_Spawn();
    ...
    if (!(ent->spawnflags & 2)) {  // no tripod
      ...
      // Arnout: move track and targetname over to these entities for
      // construction system
    ...
    base->target = ent->target;  //----(SA)	added so mounting mg42 can
                                 //trigger targets
    base->sound3to2 = -1;
    ...
    VectorSet(offset, ent->s.origin[0], ent->s.origin[1],
              ent->s.origin[2] + 24);
    ...
    // Arnout: move track and targetname over to these entities for construction
    // system
    gun->track = ent->track;
...
void Flak_Animate(gentity_t* ent) {
  ...
  if (ent->s.frame == GUN1_IDLE || ent->s.frame == GUN2_IDLE ||
      ent->s.frame == GUN3_IDLE || ent->s.frame == GUN4_IDLE) {
    return;
  }
...
*IMPORTANT* delay = offset distance of gunnner mount tag on gun model, from
gun's origin target = targetname of vehicle we are attached to aiTeam = 0 for
Nazi, 1 for Allies, 2 for Monster harc = visible arc, for initially sighting
enemies Default  is 220 radius = visible distance, for sighting enemies Default
is 4096
...
/*void miscGunnerEnemyScan(gentity_t *ent, vec3_t angles)
{
        ...
        for (t=g_entities; t<g_entities+level.maxclients; t++) {
                ...
                if (t->health < 0)
                        continue;
                if (VectorDistanceSquared(ent->r.currentOrigin,
t->r.currentOrigin) > SQR(ent->radius)) continue; VectorSubtract(
t->r.currentOrigin, ent->r.currentOrigin, v ); vectoangles( v, tang ); if
(!AICast_InFieldOfVision( angles, ent->harc, tang )) continue; if
(!AICast_VisibleFromPos( ent->r.currentOrigin, ent->s.number,
t->r.currentOrigin, t->s.number, qfalse )) continue;
                // found an enemy
                ent->enemy = t;
                break;
        }
...
/*void miscGunnerThink(gentity_t *ent)
{
        ...
        if (ent->enemy) {
                if (ent->enemy->health <= 0) {
                        ent->enemy = NULL;
                } else if (AICast_VisibleFromPos(ent->r.currentOrigin,
ent->s.number, ent->enemy->r.currentOrigin, ent->enemy->s.number, qfalse)) {
                        fire = qtrue;
                }
        }
        ...
        if (!fire) {
                ...
                if (ent->enemy) {
                        ...
                        // if we have just found our first enemy, release us
from the "fixed tag" angles
                        // so we can track them
                        if (!gun->s.density) {
                                gun->s.density = 1;
                                // start facing the same direct, so we don't
snap to a different angle immediately BG_EvaluateTrajectory( &truck->s.apos,
level.time, ent->s.angles ); VectorCopy( gun->s.angles, gun->s.apos.trBase );
        ...
        if (fire) {
                ...
                // rotate to them
                VectorSubtract( enemy->r.currentOrigin, gun->r.currentOrigin,
vec ); vectoangles( vec, gun->TargetAngles ); VectorCopy (gun->TargetAngles,
dang); for (i=0; i<3; i++) { dang[i] = AngleNormalize180(dang[i]);
                }
                ...
                yawspeed = 60;
                for (i=0; i<3; i++) {
                        BG_EvaluateTrajectory( &gun->s.apos, level.time,
gun->r.currentAngles ); diff = AngleDifference( dang[i], gun->r.currentAngles[i]
); if (fabs(diff) > (yawspeed * ((float)FRAMETIME/1000.0))) { clamped = qtrue;
                                if (diff > 0) {
                                        dang[i] =
AngleMod(gun->r.currentAngles[i] + (yawspeed * ((float)FRAMETIME/1000.0))); }
else { dang[i] = AngleMod(gun->r.currentAngles[i] - (yawspeed *
((float)FRAMETIME/1000.0)));
                                }
                        }
                }
...
); for (i=0; i<3; i++) { gun->s.apos.trDelta[i] = AngleNormalize180(
gun->s.apos.trDelta[i] );
                }
                VectorCopy( gun->r.currentAngles, gun->s.apos.trBase );
                VectorScale( gun->s.apos.trDelta, 1000/50, gun->s.apos.trDelta
); gun->s.apos.trTime = level.time; gun->s.apos.trType = TR_LINEAR_STOP;
                gun->s.apos.trDuration = 50;

                // if we are facing them, fire
                if (fabs( AngleNormalize180(gun->r.currentAngles[YAW] -
gun->TargetAngles[YAW]) ) < 10) { AngleVectors (gun->r.currentAngles, forward,
right, up); VectorCopy (gspot, muzzle);

                        VectorMA (muzzle, 16, forward, muzzle);
                        VectorMA (muzzle, 16, up, muzzle);

                        // snap to integer coordinates for more efficient
network bandwidth usage SnapVector( muzzle);

                        if (gun->damage)
                                Fire_Lead (gun, gun, MG42_SPREAD/gun->accuracy,
gun->damage); else Fire_Lead (gun, gun, MG42_SPREAD/gun->accuracy,
MG42_DAMAGE_AI);
                }
        }
...
Landmine entity. Make sure it is placed less than 64 units above an appropiate,
landmine placement compatible surface. It will drop down on spawn and then
settle itself.

NOTE: there is a team limit of 10 landmines. With this entity you can place more
landmines for that team in the map on start, but none of the engineers will be
able to place landmines until there are less than 10 left.
...
void landmine_setup(gentity_t* ent) {
  ...
  if (tr.startsolid || tr.fraction == 1.f ||
      !(tr.surfaceFlags &
        (SURF_GRASS | SURF_SNOW | SURF_GRAVEL | SURF_LANDMINE)) ||
      (tr.entityNum != ENTITYNUM_WORLD &&
       (!g_entities[tr.entityNum].inuse ||
        g_entities[tr.entityNum].s.eType != ET_CONSTRUCTIBLE))) {
    G_Printf(
        "^3WARNING: 'misc_landmine' entity at %.2f %.2f %.2f doesn't have a "
        "surface to settle on\n",
  ...
  if (ent->s.teamNum ==
      TEAM_AXIS) {  // store team so we can generate red or blue smoke
...
ONLY_ALLIED ISOBJECTIVE ISHEALTHAMMOCABINET ISCOMMANDPOST Command map marker
entity. When set to state default it shows, any other state and it isn't
visible.
-------- KEYS --------
(none)
-------- SPAWNFLAGS --------
