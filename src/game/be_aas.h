...
#define TFL_TEAM_AXIS_DISGUISED \
  0x10000000  // travel through axis+DISGUISED areas
#define TFL_TEAM_ALLIES_DISGUISED \
  0x2000000  // travel through allies+DISGUISED areas

#define TFL_TEAM_FLAGS                                         \
  (TFL_TEAM_AXIS | TFL_TEAM_ALLIES | TFL_TEAM_AXIS_DISGUISED | \
   TFL_TEAM_ALLIES_DISGUISED)
...
#define TFL_DEFAULT                                                            \
  (TFL_WALK | TFL_CROUCH | TFL_BARRIERJUMP | TFL_JUMP | TFL_LADDER |           \
   TFL_WALKOFFLEDGE | TFL_SWIM | TFL_WATERJUMP | TFL_TELEPORT | TFL_ELEVATOR | \
   TFL_AIR | TFL_WATER | TFL_SLIME | TFL_JUMPPAD | TFL_FUNCBOB)
...
completed, 1.0 = didn't hit anything vec3_t		endpos;		// final
position cplane_t		plane;		// surface normal at impact
        float			exp_dist;	// expanded plane distance
        int			sidenum;		// number of the brush
side hit bsp_surface_t surface;	// hit surface int
contents;	// contents on other side of surface hit int
ent;			// number of entity hit } bsp_trace_t;
//
*/
