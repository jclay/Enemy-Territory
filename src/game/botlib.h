...
// RF, these need to be here so the botlib also knows how many bot game entities
// there are
...
#define BLERR_AIMOVEINACTIVECLIENT \
  21  // BotMoveClient: cannot move inactive client
#define BLERR_AIMOVETOACTIVECLIENT \
  22  // BotMoveClient: cannot move to active client
#define BLERR_AICLIENTALREADYSHUTDOWN 23  // BotShutdownClient: client not setup
#define BLERR_AIUPDATEINACTIVECLIENT \
  24  // BotUpdateClient: called for inactive client
#define BLERR_AICMFORINACTIVECLIENT \
  25  // BotConsoleMessage: called for inactive client
#define BLERR_SETTINGSINACTIVECLIENT \
  26  // BotClientSettings: called for inactive client
#define BLERR_CANNOTLOADICHAT 27  // BotSetupClient: cannot load initial chats
#define BLERR_CANNOTLOADITEMWEIGHTS \
  28  // BotSetupClient: cannot load item weights
#define BLERR_CANNOTLOADITEMCONFIG \
  29  // BotSetupLibrary: cannot load item config
#define BLERR_CANNOTLOADWEAPONWEIGHTS \
  30  // BotSetupClient: cannot load weapon weights
#define BLERR_CANNOTLOADWEAPONCONFIG \
  31  // BotSetupLibrary: cannot load weapon config
...
typedef struct bot_entitystate_s {
  ...
  ////----(SA)	added
  //----(SA)	didn't want to comment in as I wasn't sure of any implications
...
typedef struct botlib_import_s {
  ...
  void (*EntityTrace)(bsp_trace_t* trace,
                      vec3_t start,
                      vec3_t mins,
                      vec3_t maxs,
                      vec3_t end,
  ...
  //
  void (*BSPModelMinsMaxsOrigin)(int modelnum,
                                 vec3_t angles,
                                 vec3_t mins,
                                 vec3_t maxs,
                                 vec3_t origin);
  ...
  //
  // Ridah, Cast AI stuff
  qboolean (*BotVisibleFromPos)(vec3_t srcpos,
                                int srcnum,
                                vec3_t destpos,
                                int destnum,
                                qboolean updateVisPos);
...
typedef struct aas_export_s {
  ...
  void (*AAS_PresenceTypeBoundingBox)(int presencetype,
                                      vec3_t mins,
                                      vec3_t maxs);
  float (*AAS_Time)(void);
  //--------------------------------------------
  ...
  int (*AAS_TraceAreas)(vec3_t start,
                        ...
                        vec3_t* points,
                        int maxareas);
  int (*AAS_BBoxAreas)(vec3_t absmins,
                       vec3_t absmaxs,
                       int* areas,
                       int maxareas);
  ...
  int (*AAS_AvoidDangerArea)(vec3_t srcpos,
                             ...
                             float range,
                             int travelflags);
  int (*AAS_Retreat)(int* dangerSpots,
                     int dangerSpotCount,
                     vec3_t srcpos,
                     int srcarea,
                     vec3_t dangerpos,
...
typedef struct ai_export_s {
  ...
  float (*Characteristic_BFloat)(int character,
                                 int index,
                                 float min,
                                 float max);
  ...
  void (*BotMatchVariable)(struct bot_match_s* match,
                           int variable,
                           char* buf,
                           int size);
  ...
  void (*BotMoveToGoal)(struct bot_moveresult_s* result,
                        int movestate,
                        struct bot_goal_s* goal,
                        int travelflags);
...
"basedir"					""
l_utils.c			Quake2 base directory "gamedir"
""					l_utils.c			Quake2
game directory "cddir"						""
l_utils.c			Quake2 CD directory

"autolaunchbspc"			"0"
be_aas_load.c		automatically launch (Win)BSPC "log"
"0"					l_log.c
enable/disable creating a log file "maxclients"				"4"
be_interface.c		maximum number of clients "maxentities"
"1024"				be_interface.c		maximum number of
entities

"sv_friction"				"6"
be_aas_move.c		ground friction "sv_stopspeed"
"100"				be_aas_move.c		stop speed "sv_gravity"
"800"				be_aas_move.c		gravity value
"sv_waterfriction"			"1"
be_aas_move.c		water friction "sv_watergravity"
"400"				be_aas_move.c		gravity in water
"sv_maxvelocity"			"300"
be_aas_move.c		maximum velocity "sv_maxwalkvelocity"		"300"
be_aas_move.c		maximum walk velocity "sv_maxcrouchvelocity"
"100"				be_aas_move.c		maximum crouch velocity
"sv_maxswimvelocity"		"150"				be_aas_move.c
maximum swim velocity "sv_walkaccelerate"			"10"
be_aas_move.c		walk acceleration "sv_airaccelerate"
"1"					be_aas_move.c		air acceleration
"sv_swimaccelerate"			"4"
be_aas_move.c		swim acceleration "sv_maxstep"
"18"				be_aas_move.c		maximum step height
"sv_maxbarrier"				"32"
be_aas_move.c		maximum barrier height "sv_maxsteepness"
"0.7"				be_aas_move.c		maximum floor steepness
"sv_jumpvel"				"270"
be_aas_move.c		jump z velocity "sv_maxwaterjump"
"20"				be_aas_move.c		maximum waterjump height

"max_aaslinks"				"4096"
be_aas_sample.c		maximum links in the AAS "max_bsplinks"
"4096"				be_aas_bsp.c		maximum links in the BSP

"notspawnflags"				"2048"
be_ai_goal.c		entities with these spawnflags will be removed
"itemconfig"				"items.c"
be_ai_goal.c		item configuration file "weaponconfig"
"weapons.c"			be_ai_weap.c		weapon configuration
file "synfile"					"syn.c"
be_ai_chat.c		file with synonyms "rndfile"
"rnd.c"				be_ai_chat.c		file with random strings
"matchfile"					"match.c"
be_ai_chat.c		file with match strings "max_messages"
...
items "framereachability"			""
be_aas_reach.c		number of reachabilities to calucate per frame
"forceclustering"			"0"
...
be_aas_main.c		force writing of aas file "nooptimize"
"0"					be_aas_main.c		no aas
optimization

"laserhook"					"0"
be_ai_move.c		0 = CTF hook, 1 = laser hook
