...
//syscall( BOTLIB_AAS_RT_GETHIDEPOS, srcpos, srcnum, srcarea, destpos, destnum,
//destarea, returnPos );
// }

// int trap_AAS_FindAttackSpotWithinRange(int srcnum, int rangenum, int
// enemynum, float rangedist, int travelflags, float *outpos) { 	return syscall(
//BOTLIB_AAS_FINDATTACKSPOTWITHINRANGE, srcnum, rangenum, enemynum,
...
int trap_AAS_NearestHideArea(int srcnum,
                             ...
                             float maxdist,
                             vec3_t distpos) {
  return syscall(BOTLIB_AAS_NEARESTHIDEAREA, srcnum, origin, areanum, enemynum,
                 enemyorigin, enemyareanum, travelflags, PASSFLOAT(maxdist),
                 distpos);
}
...
int trap_AAS_ListAreasInRange(vec3_t srcpos,
                              ...
                              float** outareas,
                              int maxareas) {
  return syscall(BOTLIB_AAS_LISTAREASINRANGE, srcpos, srcarea, PASSFLOAT(range),
                 travelflags, outareas, maxareas);
}
...
int trap_AAS_AvoidDangerArea(vec3_t srcpos,
                             ...
                             float range,
                             int travelflags) {
  return syscall(BOTLIB_AAS_AVOIDDANGERAREA, srcpos, srcarea, dangerpos,
...
int trap_AAS_Retreat(
    ...
    float dangerRange,
    int travelflags) {
  return syscall(BOTLIB_AAS_RETREAT, dangerSpots, dangerSpotCount, srcpos,
...
int trap_AAS_AlternativeRouteGoals(vec3_t start,
                                   ...
                                   int color) {
  return syscall(BOTLIB_AAS_ALTROUTEGOALS, start, goal, travelflags,
                 altroutegoals, maxaltroutegoals, color);
}
...
void trap_AAS_RecordTeamDeathArea(vec3_t srcpos,
                                  ...
                                  int teamCount,
                                  int travelflags) {
  syscall(BOTLIB_AAS_RECORDTEAMDEATHAREA, srcpos, srcarea, team, teamCount,
          travelflags);
}
// done.
...
float trap_Characteristic_BFloat(int character,
                                 ...
                                 float max) {
  int temp;
  temp = syscall(BOTLIB_AI_CHARACTERISTIC_BFLOAT, character, index,
