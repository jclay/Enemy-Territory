...
static const cmd_reference_t aCommandInfo[] = {
    ...
    {"bottomshots", qtrue, qfalse, G_weaponRankings_cmd,
     ...
     "stats for a weapon"},
    //	{ "callvote",		qtrue,	qfalse,	Cmd_CallVote_f, " <params>:^7
    //Calls a vote" },
    {"callvote", qtrue, qfalse,
     (void (*)(gentity_t*, unsigned int, qboolean))Cmd_CallVote_f,
     " <params>:^7 Calls a vote"},
    //	{ "captains",		qtrue,	qtrue,	NULL, ":^7 Shows team captains"
    //}, 	{ "coach",			qtrue,	qtrue,	NULL, ":^7 Accepts coach
    //invitation/restarts coach view" }, 	{ "coachdecline",	qtrue,	qtrue,
    //NULL, ":^7 Declines coach invitation or resigns coach status" }, 	{ "coachinvite",	qtrue,	qtrue,	NULL, " <player_ID>:^7 Invites a player to coach team" },
    //	{ "coachkick",		qtrue,	qtrue,	NULL, " <player_ID>:^7 Kicks
    //active coach from team" },
    {"commands", qtrue, qtrue, G_commands_cmd,
     ":^7 Gives a list of OSP-specific commands"},
    {"currenttime", qtrue, qtrue, NULL, ":^7 Displays current local time"},
    {"follow", qfalse, qtrue, Cmd_Follow_f,
     " <player_ID|allies|axis>:^7 Spectates a particular player or team"},
    //	{ "invite",			qtrue,	qtrue,	NULL, " <player_ID>:^7
    //Invites a player to join a team" },
    {"lock", qtrue, qtrue, G_lock_cmd,
     ":^7 Locks a player's team to prevent others from joining"},
    ...
    {"ref", qtrue, qtrue, G_ref_cmd,
     " <password>:^7 Become a referee (admin access)"},
    //	{ "remove",			qtrue,	qtrue,	NULL, " <player_ID>:^7
    //Removes a player from the team" }, 	{ "resign",			qtrue,
    //qtrue,	NULL, " [player_ID]:^7 Resigns captainship.  Can optionally be
    //given to another teammate" },
    ...
    {"specinvite", qtrue, qtrue, G_specinvite_cmd,
     ":^7 Invites a player to spectate a speclock'ed team"},
    {"speclock", qtrue, qtrue, G_speclock_cmd,
     ":^7 Locks a player's team from spectators"},
    //	{ "speconly",		qtrue,	qtrue,	NULL, ":^7 Toggles option to
    //stay as a spectator in 1v1" },
    {"specunlock", qtrue, qfalse, G_speclock_cmd,
     ":^7 Unlocks a player's team from spectators"},
    ...
    {"team", qtrue, qtrue, Cmd_Team_f,
     " <b|r|s|none>:^7 Joins a team (b = allies, r = axis, s = spectator)"},
    //	{ "setclass",		qtrue,	qtrue,	Cmd_SetClass_f, " <classnum>:^7
    //Selects a class" }, 	{ "setweapons",		qtrue,	qtrue,
    //Cmd_SetWeapons_f, " <weapon|weapon2>:^7 Selects your weapon loadout" },
    {"timein", qfalse, qfalse, G_pause_cmd,
     ":^7 Unpauses a match (if initiated by the issuing team)"},
    {"timeout", qfalse, qtrue, G_pause_cmd,
     ":^7 Allows a team to pause a match"},
    {"topshots", qtrue, qtrue, G_weaponRankings_cmd,
     ":^7 Shows BEST player for each weapon. Add ^3<weapon_ID>^7 to show all "
     "stats for a weapon"},
    {"unlock", qtrue, qfalse, G_lock_cmd,
     ":^7 Unlocks a player's team, allowing others to join"},
    {"unpause", qfalse, qfalse, G_pause_cmd,
     ":^7 Unpauses a match (if initiated by the issuing team)"},
    ...
    //	{ "viewcam",		qfalse,	qtrue,	NULL, ":^7 Switches to cinematic
    //camera mode" }, 	{ "vc_follow",		qfalse,	qtrue,	NULL, "
    //[player_ID]:^7 Puts viewcam in follow mode.  Can optionally to follow a
    //specific player" }, 	{ "vc_free",		qfalse,	qtrue,	NULL, ":^7
    //Toggle viewcam between manual/automatic change" }, 	{ "vc_view",
    //qfalse,	qtrue,	NULL, ":^7 Toggle ViewCam between static/dynamic views"
    //}, 	{ "viewadd",		qfalse,	qtrue,	NULL, " <player_ID>:^7 Adds a
    //player to multi-screen view" }, 	{ "viewall",		qfalse,	qtrue,
    //NULL, ":^7 Adds all active players to a multi-screen view" }, 	{
    //"viewallies",		qfalse,	qtrue,	NULL, ": ^7 Views entire
    //allies/axis team" }, 	{ "viewaxis",		qfalse,	qtrue,	NULL, ":
    //^7 Views entire allies/axis team" }, 	{ "viewcyclenext",	qfalse,	qtrue,
    //NULL, ":^7 Cycles through players in current view" }, 	{ "viewfollow",
    //qfalse,	qtrue,	NULL, ":^7 Follows current highlighted view" }, 	{ "viewnext",		qfalse,	qtrue,	NULL, ":^7 Moves through active screen in a multi-screen display" },
    //	{ "viewnone",		qfalse,	qtrue,	NULL, ":^7 Disables multiview
    //mode and goes back to spectator mode" }, 	{ "viewremove",		qfalse,
    //qtrue,	NULL, " [player_ID]:^7 Removes current selected or specific
    //player from multi-screen view" },
...
void G_pause_cmd(gentity_t* ent, unsigned int dwCommand, qboolean fPause) {
  ...
  if ((PAUSE_UNPAUSING >= level.match_pause && !fPause) ||
      (PAUSE_NONE != level.match_pause && fPause)) {
...
void G_players_cmd(gentity_t* ent, unsigned int dwCommand, qboolean fValue) {
  ...
  if (g_gamestate.integer == GS_PLAYING) {
    if (ent) {
      CP("print \"\n^3 ID^1 : ^3Player                    Nudge  Rate  MaxPkts "
         " Snaps\n\"");
      CP("print "
         "\"^1-----------------------------------------------------------^"
         "7\n\"");
    ...
    if (ent) {
      ...
      G_Printf(
          "Status   : ID : Player                    Nudge  Rate  MaxPkts  "
          "Snaps\n");
      G_Printf(
          "--------------------------------------------------------------------"
          "-\n");
...
void G_speclock_cmd(gentity_t* ent, unsigned int dwCommand, qboolean fLock) {
  ...
  if (tteam == TEAM_AXIS || tteam == TEAM_ALLIES) {
    if (teamInfo[tteam].spec_lock == fLock) {
      ...
      G_printFull(va("The %s team is now %sed from spectators", aTeams[tteam],
                     lock_status[fLock]),
                  NULL);
