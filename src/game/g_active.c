...
#define MIN_BURN_INTERVAL \
  399  // JPW NERVE set burn timeinterval so we can do more precise damage (was
       // 199 old model)
...
void P_WorldEffects(gentity_t* ent) {
  ...
  if (waterlevel == 3) {
    ...
    if (ent->client->airOutTime < level.time) {
      if (ent->client->ps
              .powerups[PW_BREATHER]) {  // take air from the breather now that
                                         // we need it
  ...
  if (ent->s.onFireEnd && ent->client) {
    if (level.time - ent->client->lastBurnTime >= MIN_BURN_INTERVAL) {
      // JPW NERVE server-side incremental damage routine / player damage/health
      // is int (not float) so I can't allocate 1.5 points per server tick, and
      // 1 is too weak and 2 is too strong. solution: allocate damage far less
      // often (MIN_BURN_INTERVAL often) and do more damage. That way minimum
      // resolution (1 point) damage changes become less critical.
      ...
      if ((ent->s.onFireEnd > level.time) && (ent->health > 0)) {
        ...
        G_Damage(ent, attacker, attacker, NULL, NULL, 5, DAMAGE_NO_KNOCKBACK,
                 MOD_FLAMETHROWER);  // JPW NERVE was 7
      }
    }
  }
  // jpw
...
void G_SetClientSound(gentity_t* ent) {
  /*	if (ent->waterlevel && (ent->watertype & CONTENTS_LAVA) )
     //----(SA)	modified since slime is no longer deadly ent->s.loopSound =
     level.snd_fry; else*/                                                                                                                                                   // Gordon: doesnt exist
  ent->s.loopSound = 0;
}
  ...
  for (i = 0; i < pm->numtouch; i++) {
    ...
    if ((ent->client) /*&& !(ent->r.svFlags & SVF_BOT)*/ &&
      ...
      /*			vec3_t dir;
                              ...
                              VectorSubtract( other->r.currentOrigin,
         ent->r.currentOrigin, dir ); VectorNormalize( dir ); if (DotProduct(
         ent->client->ps.velocity, dir ) > 0) { PushBot( ent, other );
                              }
      */
      PushBot(ent, other);
    }
...
void G_TouchTriggers(gentity_t* ent) {
  ...
  // Arnout: reset the pointer that keeps track of trigger_objective_info
  // tracking
  ...
  if (client->pers.mvCount < 1) {
    ...
    // activate button swaps places with bot
      ...
      Cmd_SwapPlacesWithBot_f(ent, ent->client->sess.spectatorClient);
                ...
                TEAM_SPECTATOR) &&  // don't let dead team players do free fly
...
qboolean ClientInactivityTimer(gclient_t* client) {
  ...
  if ((g_inactivity.integer == 0 &&
    ...
    if (!client->inactivityWarning &&
      ...
      CPx(client - level.clients,
          "cp \"^310 seconds until inactivity drop!\n\"");
      CPx(client - level.clients,
          "print \"^310 seconds until inactivity drop!\n\"");
...
void SendPendingPredictableEvents(playerState_t* ps) {
  ...
  BG_PlayerStateToEntityState extEvent = ps->externalEvent; ps->externalEvent =
  0;
...
void ClientThink_real(gentity_t* ent) {
  ...
  // non-dedicated server OSP - moved here to allow for spec inactivity checks
  // as well
  ...
  if (pm.ps->pm_type == PM_DEAD) {
    ...
    // DHM-Nerve added:: EF_DEAD is checked for in Pmove functions, but wasn't
    // being set
    //              until after Pmove
  ...
  if (client->ps.pm_type != PM_DEAD &&
    ...
    {
    case COMBATSTATE_COLD:	G_AddSkillPoints( ent, SK_BATTLE_SENSE, 0.f );
    G_DebugAddSkillPoints( ent, SK_BATTLE_SENSE, 0.f, "combatstate cold" );
    break; case COMBATSTATE_WARM:	G_AddSkillPoints( ent,
    SK_BATTLE_SENSE, 2.f ); G_DebugAddSkillPoints( ent, SK_BATTLE_SENSE, 2.f,
    "combatstate warm" ); break; case COMBATSTATE_HOT:	G_AddSkillPoints( ent,
    SK_BATTLE_SENSE, 5.f ); G_DebugAddSkillPoints( ent, SK_BATTLE_SENSE, 5.f,
    "combatstate hot" ); break; case COMBATSTATE_SUPERHOT:
    G_AddSkillPoints( ent, SK_BATTLE_SENSE, 8.f ); G_DebugAddSkillPoints( ent,
  ...
  /*for ( i = 0 ; i < level.numConnectedClients; i++ ) {
          ...
          if( !trap_InPVS( g_entities[level.sortedClients[i]].r.currentOrigin,
  ent->r.currentOrigin ) ) { continue;
          }

          VectorSubtract( g_entities[level.sortedClients[i]].r.currentOrigin,
  ent->r.currentOrigin, dist ); if( VectorLengthSquared( dist ) > SQR(512) )
                  continue;
  ...
  // Gordon: bit hacky, stop the slight lag from client -> server even on
  // locahost, switching back to the weapon you were holding
  //			and then back to what weapon you should have, became
  //VERY noticible for the kar98/carbine + gpg40, esp now i've added the
  //			animation locking
    ...
    if (level.gameManager->s.modelindex &&
      ...
      if (G_SweepForLandmines(ent->r.currentOrigin, 256.f, TEAM_AXIS)) {
        ...
        trap_SendServerCommand(
            ent - g_entities,
            "cp \"Mines have been reported in this area.\" 1");
      }
...
void SpectatorClientEndFrame(gentity_t* ent) {
  ...
  if (/*ent->client->pers.mvCount > 0 &&*/ ent->client->pers.mvScoreUpdate <
      level.time) {
  ...
  if ((ent->client->sess.spectatorState == SPECTATOR_FOLLOW) ||
    ...
    if (g_gametype.integer != GT_WOLF_LMS) {
      if ((g_maxlives.integer > 0 || g_alliedmaxlives.integer > 0 ||
    ...
    if (g_gametype.integer == GT_WOLF_LMS &&
      ...
      // Force respawn in LMS when nobody is playing and we aren't at the
      // timelimit yet
...
void ClientEndFrame(gentity_t* ent) {
  ...
  // DHM - Nerve :: If it's been a couple frames since being revived, and
  // props_frame_state
