...
cvarTable_t gameCvarTable[] = {
    ...
    {&g_bluelimbotime, "g_bluelimbotime", "30000", CVAR_SERVERINFO | CVAR_LATCH,
     0, qfalse},
    {&g_medicChargeTime, "g_medicChargeTime", "45000",
     CVAR_SERVERINFO | CVAR_LATCH, 0, qfalse, qtrue},
    {&g_engineerChargeTime, "g_engineerChargeTime", "30000",
     CVAR_SERVERINFO | CVAR_LATCH, 0, qfalse, qtrue},
    {&g_LTChargeTime, "g_LTChargeTime", "40000", CVAR_SERVERINFO | CVAR_LATCH,
     0, qfalse, qtrue},
    {&g_soldierChargeTime, "g_soldierChargeTime", "20000",
     CVAR_SERVERINFO | CVAR_LATCH, 0, qfalse, qtrue},
    // jpw

    {&g_covertopsChargeTime, "g_covertopsChargeTime", "30000",
     CVAR_SERVERINFO | CVAR_LATCH, 0, qfalse, qtrue},
...
#ifdef ALLOW_GSYNC
    {&g_synchronousClients, "g_synchronousClients", "0",
     CVAR_SYSTEMINFO | CVAR_CHEAT, 0, qfalse},
#endif  // ALLOW_GSYNC

    {&g_friendlyFire, "g_friendlyFire", "1", CVAR_SERVERINFO | CVAR_ARCHIVE, 0,
     qtrue, qtrue},

    {&g_teamForceBalance, "g_teamForceBalance", "0",
     CVAR_ARCHIVE},  // NERVE - SMF - merge from team arena
    ...
    {&g_balancedteams, "g_balancedteams", "0", CVAR_SERVERINFO | CVAR_ROM, 0,
     qtrue},
    ...
    {&g_complaintlimit, "g_complaintlimit", "6", CVAR_ARCHIVE, 0,
     qtrue},  // DHM - Nerve
    {&g_ipcomplaintlimit, "g_ipcomplaintlimit", "3", CVAR_ARCHIVE, 0, qtrue},
    {&g_filtercams, "g_filtercams", "0", CVAR_ARCHIVE, 0, qfalse},
    {&g_maxlives, "g_maxlives", "0",
     CVAR_ARCHIVE | CVAR_LATCH | CVAR_SERVERINFO, 0, qtrue},  // DHM - Nerve
    {&g_maxlivesRespawnPenalty, "g_maxlivesRespawnPenalty", "0",
     CVAR_ARCHIVE | CVAR_LATCH | CVAR_SERVERINFO, 0, qtrue},
    {&g_voiceChatsAllowed, "g_voiceChatsAllowed", "4", CVAR_ARCHIVE, 0,
     qfalse},  // DHM - Nerve

    {&g_alliedmaxlives, "g_alliedmaxlives", "0", CVAR_LATCH | CVAR_SERVERINFO,
     0, qtrue},  // Xian
    {&g_axismaxlives, "g_axismaxlives", "0", CVAR_LATCH | CVAR_SERVERINFO, 0,
     qtrue},  // Xian
    {&g_fastres, "g_fastres", "0", CVAR_ARCHIVE, 0, qtrue,
     qtrue},  // Xian - Fast Medic Resing
    {&g_knifeonly, "g_knifeonly", "0", 0, 0,
     qtrue},  // Xian - Fast Medic Resing
    {&g_enforcemaxlives, "g_enforcemaxlives", "1", CVAR_ARCHIVE, 0,
     qtrue},  // Xian - Gestapo enforce maxlives stuff by temp banning
    ...
    {&vote_allow_shuffleteamsxp, "vote_allow_shuffleteamsxp", "1", 0, 0, qfalse,
     qfalse},
    {&vote_allow_swapteams, "vote_allow_swapteams", "1", 0, 0, qfalse, qfalse},
    {&vote_allow_friendlyfire, "vote_allow_friendlyfire", "1", 0, 0, qfalse,
     qfalse},
    {&vote_allow_timelimit, "vote_allow_timelimit", "0", 0, 0, qfalse, qfalse},
    {&vote_allow_warmupdamage, "vote_allow_warmupdamage", "1", 0, 0, qfalse,
     qfalse},
    {&vote_allow_antilag, "vote_allow_antilag", "1", 0, 0, qfalse, qfalse},
    {&vote_allow_balancedteams, "vote_allow_balancedteams", "1", 0, 0, qfalse,
     qfalse},
    ...
    {&g_lms_currentMatch, "g_lms_currentMatch", "0", CVAR_ROM, 0, qfalse,
     qtrue},
    ...
    {&g_heavyWeaponRestriction, "g_heavyWeaponRestriction", "100",
...
explosive,invisible_users,items,etc.)
        ...
        checked against (in case the traceent was an invisible_user or
something)
...
void G_CheckForCursorHints(gentity_t* ent) {
  ...
  qboolean zooming, indirectHit;  // indirectHit means the checkent was not the
                                  // ent hit by the trace (checkEnt!=traceEnt)
  ...
  if ((g_gametype.integer == GT_SINGLE_PLAYER ||
      ...
      g_reloading.integer == RELOAD_NEXTMAP_WAITING) {
  ...
  // Arnout: building something - add this here because we don't have anything
  // solid to trace to - quite ugly-ish
  if (ent->client->touchingTOI && ps->stats[STAT_PLAYER_CLASS] == PC_ENGINEER) {
    gentity_t* constructible;
    if ((constructible = G_IsConstructible(ent->client->sess.sessionTeam,
  ...
  if (tr->entityNum == ENTITYNUM_WORLD) {
    ...
    if (traceEnt->client &&
      ...
      // xkan, 1/6/2003 - check for civilian, show neutral cursor (no matter
      // which team)
      hintType = HINT_PLYR_NEUTRAL;
      hintDist = CH_FRIENDLY_DIST;  // far, since this will be used to determine
                                    // whether to shoot bullet weaps or not
    }
  } else {
    checkEnt = traceEnt;
    ...
    if (traceEnt->s.eType == ET_GENERAL) {
      // ignore trigger_aidoor.  can't just not trace for triggers, since I need
      // invisible_users... damn, I would like to ignore some of these triggers
      // though.
    ...
    if (checkEnt) {
      // TDF This entire function could be the poster boy for converting to OO
      // programming!!! I'm making this into a switch in a vain attempt to make
      // this readable so I can find which brackets don't match!!!
      ...
      switch (checkEnt->s.eType) {
        ...
        case ET_EXPLOSIVE: {
          if (checkEnt->spawnflags & EXPLOSIVE_TANK) {
            ...
            hintVal = ps->serverCursorHintVal =
                0;  // no health for tank destructibles
          } else {
        ...
        case ET_MOVER:
          if (!Q_stricmp(checkEnt->classname, "script_mover")) {
            ...
            if (checkEnt->moverState ==
                MOVER_POS1ROTATE) {  // stationary/closed
              hintDist = CH_DOOR_DIST;
              hintType = HINT_DOOR_ROTATING;
              if (checkEnt->key == -1 ||
                  !G_AllowTeamsAllowed(checkEnt, ent)) {  // locked
        ...
        case ET_BOMB:
          ...
          // hint icon specified in entity (and proper contact was made, so
          // hintType was set) first try the checkent...
  ...
  for (i = 1, e = g_entities + i; i < level.num_entities; i++, e++) {
    ...
    for (j = i + 1, e2 = e + 1; j < level.num_entities; j++, e2++) {
      ...
      if (!strcmp(e->team, e2->team)) {
        ...
        e->teamchain = e2;
        e2->teammaster = e;
        //				e2->key = e->key;	// (SA) I can't
        //set the key here since the master door hasn't finished spawning yet
        //and therefore has a key of -1
      ...
      if (cv->modificationCount != cv->vmCvar->modificationCount) {
        ...
        if (cv->trackChange && !(cv->cvarFlags & CVAR_LATCH)) {
          trap_SendServerCommand(
              -1, va("print \"Server:[lof] %s [lon]changed to[lof] %s\n\"",
        ...
        // Moved this check out of the main world think loop
          ...
          if (gametype == GT_WOLF_CAMPAIGN && gametype != g_gametype.integer) {
            if (!G_MapIsValidCampaignStartMap()) {
              ...
              G_Printf(
                  "Map '%s' isn't a valid campaign start map, resetting game "
                  "type to '%i'\n",
                  level.rawmapname, gt);
          ...
          if (!level.latchGametype && g_gamestate.integer == GS_PLAYING &&
            ...
            trap_SendConsoleCommand(
                EXEC_APPEND,
                va("wait 2 ; g_gametype %i ; map_restart 10 0\n", gt));
          }
...
void bani_getmapxp(void) {
  ...
  j = 0;
  trap_Cvar_VariableStringBuffer(va("%s_axismapxp%i", GAMEVERSION, j), s,
                                 sizeof(s));
  // reassemble string...
  while (strrchr(s, '+')) {
    j++;
    *strrchr(s, '+') = (char)0;
    trap_Cvar_VariableStringBuffer(va("%s_axismapxp%i", GAMEVERSION, j), t,
                                   sizeof(t));
    strcat(s, t);
  }
  ...
  j = 0;
  trap_Cvar_VariableStringBuffer(va("%s_alliedmapxp%i", GAMEVERSION, j), s,
                                 sizeof(s));
  // reassemble string...
  while (strrchr(s, '+')) {
    j++;
    *strrchr(s, '+') = (char)0;
    trap_Cvar_VariableStringBuffer(va("%s_alliedmapxp%i", GAMEVERSION, j), t,
                                   sizeof(t));
    strcat(s, t);
  }
...
void G_InitGame(int levelTime, int randomSeed, int restart) {
  ...
  */
  ...
  if (g_gametype.integer == GT_SINGLE_PLAYER) {
    ...
    trap_Cvar_VariableStringBuffer("savegame_loading", loading,
                                   sizeof(loading));
  ...
  // Reinstate any MV views for clients -- need to do this after all init is
  // complete
  // --- maybe not the best place to do this... seems to be some race conditions
  // on map_restart
...
void MoveClientToIntermission(gentity_t* ent) {
  ...
  /*if ( ent->client->sess.sessionTeam == TEAM_AXIS ||
  ent->client->sess.sessionTeam == TEAM_ALLIES ) { timeLived = (level.time -
  ent->client->pers.lastSpawnTime) * 0.001f;
  ...
  // todo: call bot routine so they can process the transition to intermission
  // (send voice chats, etc)
...
void FindIntermissionPoint(void) {
  ...
  // Change from scripting value for winner (0==AXIS, 1==ALLIES) to spawnflag
  // value
  if (winner == 0) {
    winner = TEAM_AXIS;
  } else {
...
void LogExit(const char* string) {
  ...
  if (g_gametype.integer == GT_WOLF_STOPWATCH) {
    ...
    if (!g_currentRound.integer) {
      if (winner == defender) {
        ...
        trap_Cvar_Set(
            "g_nextTimeLimit",
            va("%f", (level.timeCurrent - level.startTime) / 60000.f));
      }
    } else {
      // reset timer
  ...
  // -NERVE - SMF
    ...
    for (teamNum = TEAM_AXIS; teamNum <= TEAM_ALLIES; teamNum++) {
      for (i = 0; i < SK_NUM_SKILLS; i++) {
        ...
        if (highestskillpointsclient >= 0) {
          ...
          for (j = highestskillpointsclient; j < level.numConnectedClients;
               j++) {
            cl = &level.clients[level.sortedClients[j]];
    ...
    int winner;
    int roundLimit =
        g_lms_roundlimit.integer < 3 ? 3 : g_lms_roundlimit.integer;
    ...
    if (g_currentRound.integer >= roundLimit ||
        g_axiswins.integer == numWinningRounds ||
      ...
      if (g_lms_currentMatch.integer + 1 >= g_lms_matchlimit.integer) {
        ...
        trap_Cvar_Set("g_lms_currentMatch",
                      va("%i", g_lms_currentMatch.integer + 1));
...
void CheckExitRules(void) {
  ...
  if (g_timelimit.value && !level.warmupTime) {
    ...
    if ((level.timeCurrent - level.startTime) >= (g_timelimit.value * 60000)) {
      ...
      if (g_gametype.integer == GT_WOLF_LMS) {
        ...
        // bani - if team was eliminated < 3 sec before round end, we _properly_
        // end it here
        if (level.teamEliminateTime) {
          LogExit(va("%s team eliminated.",
                     level.lmsWinningTeam == TEAM_ALLIES ? "Axis" : "Allied"));
        }
        ...
        if (axisSurvivors == alliedSurvivors) {
          ...
          if (level.firstbloodTeam == TEAM_AXIS) {
            ...
            trap_SendServerCommand(
                -1, "print \"Axis team wins by drawing First Blood.\n\"");
            ...
            trap_SendServerCommand(
                -1, "print \"Allied team wins by drawing First Blood.\n\"");
          } else {
          ...
          trap_SendServerCommand(
              -1, "print \"Axis team has the most survivors.\n\"");
          return;
        } else {
          trap_GetConfigstring(CS_MULTI_MAPWINNER, cs, sizeof(cs));
          Info_SetValueForKey(cs, "winner", "1");
          ...
          trap_SendServerCommand(
              -1, "print \"Allied team has the most survivors.\n\"");
          return;
        }
      } else {
  ...
  // bani - #444
  // i dont really get the point of the delay anyway, why not end it immediately
  // like maxlives games?
  if (g_gametype.integer == GT_WOLF_LMS) {
    if (!level.teamEliminateTime) {
      if (level.numFinalDead[0] >= level.numTeamClients[0] &&
          level.numTeamClients[0] > 0) {
        level.teamEliminateTime = level.time;
        level.lmsWinningTeam = TEAM_ALLIES;
        trap_GetConfigstring(CS_MULTI_MAPWINNER, cs, sizeof(cs));
        Info_SetValueForKey(cs, "winner", "1");
        trap_SetConfigstring(CS_MULTI_MAPWINNER, cs);
      } else if (level.numFinalDead[1] >= level.numTeamClients[1] &&
  ...
  if (g_gametype.integer != GT_WOLF_LMS) {
    if (g_maxlives.integer > 0 || g_axismaxlives.integer > 0 ||
        g_alliedmaxlives.integer > 0) {
      if (level.numFinalDead[0] >= level.numTeamClients[0] &&
          level.numTeamClients[0] > 0) {
        trap_GetConfigstring(CS_MULTI_MAPWINNER, cs, sizeof(cs));
...
%i\n", GS_WAITING_FOR_PLAYERS ) );
        ...
        if ( current_gs == GS_WAITING_FOR_PLAYERS && g_minGameClients.integer >
1 && level.numPlayingClients >= g_minGameClients.integer &&
...
//	static qboolean latch = qfalse;
        ...
        if(g_gametype.integer != GT_SINGLE_PLAYER && g_gametype.integer !=
GT_COOP)
  ...
  if (g_gametype.integer >= GT_WOLF) {
    ...
    if (g_gamestate.integer == GS_WARMUP) {
      if (!g_doWarmup.integer ||
        ...
        // Why scale these at all?  Minimum would mean 22s on Campaign and 44 on
        // LMS.... Once people are ready, they want to get the show rolling :)
        /*				if( g_gametype.integer ==
           GT_WOLF_CAMPAIGN ) delay *= 2; else if( g_gametype.integer ==
           GT_WOLF_LMS && !g_doWarmup.integer ) delay *= 4;
        */
...
void CheckVote(void) {
  ...
  if (level.time - level.voteInfo.voteTime >= VOTE_TIME) {
    ...
    int pcnt =
        (level.voteInfo.vote_fn == G_StartMatch_v) ? 75 : vote_percent.integer;
    int total;
    ...
    if (level.voteInfo.vote_fn == G_Kick_v) {
      ...
      if (!other->client || other->client->sess.sessionTeam == TEAM_SPECTATOR) {
        ...
        total = level.voteInfo.numVotingTeamClients
                    [other->client->sess.sessionTeam == TEAM_AXIS ? 0 : 1];
      }
    } else {
      total = level.voteInfo.numVotingClients;
    }
    ...
    if (level.voteInfo.voteYes > pcnt * total / 100) {
      ...
      if (level.voteInfo.voteYes > total + 1) {
        ...
        // who is policing people who shouldn't be joining and players don't
        // want this sort of spam in the console
        if (level.voteInfo.vote_fn != G_Kick_v) {
    ...
    if (level.reloadDelayTime) {
      if (level.reloadDelayTime < level.time) {
        /*if (g_reloading.integer == RELOAD_NEXTMAP_WAITING) {
                trap_Cvar_Set( "g_reloading", va("%d", RELOAD_NEXTMAP) );
        // set so sv_map_f will know it's okay to start a map if
        (g_cheats.integer) trap_SendConsoleCommand( EXEC_APPEND, va("spdevmap
        %s\n", level.nextMap) ); else trap_SendConsoleCommand( EXEC_APPEND,
        va("spmap %s\n", level.nextMap ) );

        }*/
        /* else if(g_reloading.integer == RELOAD_ENDGAME) {
             G_EndGame();	// kick out to the menu and start the "endgame"
     menu (credits, etc)

     }*/  // else {
        // set the loadgame flag, and restart the server
        trap_Cvar_Set("savegame_loading",
                      "2");  // 2 means it's a restart, so stop rendering until
                             // we are loaded
...
static void G_CheckLoadGame(void) {
  ...
  // tell the cgame NOT to render the scene while we are waiting for things to
  // settle
  ...
  if (strlen(loading) > 0 && atoi(loading) != 0) {
    ...
    if (!(g_reloading.integer) && atoi(loading) == 2) {
      ...
      //			reloading = qtrue;	// this gets reset at
      //the Map_Restart() since the server unloads the game dll
      trap_Cvar_Set("g_reloading", "1");
    }
    ...
    if (ready) {
      ...
      // RF, spawn a thinker that will enable rendering after the client has had
      // time to process the entities and setup the display
      ent = G_Spawn();
      ...
      trap_SendServerCommand(-1,
                             va("snd_fade 1 %d 0", 2000));  //----(SA)	added
    ...
    // not loading a game, we must be in a new level, so look for some
    // persistant data to read in, then save the game
    if (ready) {
      G_LoadPersistant();  // make sure we save the game after we have brought
                           // across the items

      saveGamePending = 0;

      // briefing menu will handle transition, just set a cvar for it to check
      // for drawing the 'continue' button
...
void CheckCvars(void) {
  ...
  if (g_gametype.integer == GT_WOLF_LMS) {
    if (g_lms_teamForceBalance.modificationCount !=
        g_lms_teamForceBalance_lastMod) {
...
======================
*/
qboolean G_PositionEntityOnTag(gentity_t* entity,
                               gentity_t* parent,
                               char* tagName) {
  int i;
  orientation_t tag;
