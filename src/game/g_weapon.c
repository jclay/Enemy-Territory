...
qboolean Bullet_Fire_Extended(gentity_t* source,
                              gentity_t* attacker,
                              vec3_t start,
                              vec3_t end,
                              float spread,
...
void Weapon_Knife(gentity_t* ent) {
  ...
  if (traceEnt->client) {
    ...
    if (DotProduct(eforward, pforward) > 0.6f) {  // from behind(-ish)
      damage =
          100;  // enough to drop a 'normal' (100 health) human with one jab
      mod = MOD_KNIFE;
...
void Weapon_Syringe(gentity_t* ent) {
  ...
  // VectorMA (muzzleTrace, -16, forward, muzzleTrace);	// DHM - Back up the
  // start point in case medic is
  ...
  if (tr.fraction < 1.0) {
    ...
    if (traceEnt->client != NULL) {
      if ((traceEnt->client->ps.pm_type == PM_DEAD) &&
        ...
        if (!traceEnt
                 ->isProp) {  // Gordon: flag for if they were teamkilled or not
          AddScore(ent,
                   WOLF_MEDIC_BONUS);  // JPW NERVE props to the medic for the
                                       // swift and dexterous bit o healitude

          G_AddSkillPoints(ent, SK_FIRST_AID, 4.f);
          G_DebugAddSkillPoints(ent, SK_FIRST_AID, 4.f, "reviving a player");
        }

        // Arnout: calculate ranks to update numFinalDead arrays. Have to do it
...
int EntsThatRadiusCanDamage(vec3_t origin, float radius, int* damagedList) {
  ...
  // bounding box was checking against radius / sqrt(2) if collision is along
  // box plane
  for (i = 0; i < 3; i++) {
  ...
  for (e = 0; e < listedEntities; e++) {
    ...
    if (warnBlockingPlayers && check->s.eType == ET_PLAYER) {
      if ((level.time - check->client->lastConstructibleBlockingWarnTime) >=
        ...
        trap_SendServerCommand(
            check->s.number,
            "cp \"Warning, leave the construction area...\" 1");
  ...
  if (constructible->s.eType == ET_CONSTRUCTIBLE &&
    ...
    if (constructible->s.angles2[0] <= 0) {
      ...
      if (level.time - constructible->lastHintCheckTime <
        ...
        return (qtrue);  // likely will come back soon - so override other plier
                         // bits anyway
      }
      // Gordon: are we scripted only?
      if (!(ent->spawnflags & CONSTRUCTIBLE_AAS_SCRIPTED)) {
        if (!(ent->spawnflags & CONSTRUCTIBLE_NO_AAS_BLOCKING)) {
          // RF, if we are blocking AAS areas when built, then clear AAS
          // blocking so we can set it again after the stage has been increased
          if (constructible->spawnflags &
              CONSTRUCTIBLE_BLOCK_PATHS_WHEN_BUILD) {
      ...
      if (constructible->count2) {
        ...
        // trap_SetBrushModel( constructible, va( "*%i",
        // constructible->conbmodels[constructible->grenadeFired-1] ) );
      }
      ...
      /*			if( constructible->parent->spawnflags & 8 ) {
         ...
         G_SoundIndex( va( "sound/world/build_stage%i.wav",
         constructible->s.frame ) ); } else { constructible->s.loopSound =
         G_SoundIndex( va( "sound/world/build_stage%i.wav",
    ...
    /*		if( ent->client->touchingTOI->spawnflags & 4 ) { //
       MESSAGE_OVERRIDE gentity_t* pm = G_PopupMessage( PM_CONSTRUCTION );
                            pm->s.effect3Time = G_StringIndex(
       ent->client->touchingTOI->spawnitem ); pm->s.effect2Time = TEAM_AXIS;
    ...
    // constructible_indicator_think got changed to free the indicator when the
    // constructible is constructed
    ...
    // if not invulnerable and dynamite-able, create a 'destructable' marker for
    // the other team
    ...
    if (!(ent->spawnflags & CONSTRUCTIBLE_AAS_SCRIPTED)) {
      if (!(ent->spawnflags & CONSTRUCTIBLE_NO_AAS_BLOCKING)) {
        // RF, a stage has been completed, either enable or disable AAS areas
        // appropriately
        if (!(constructible->spawnflags &
              CONSTRUCTIBLE_BLOCK_PATHS_WHEN_BUILD)) {
          ...
          //				if( !constructible->count2 || (
          //constructible->grenadeFired == constructible->count2 ) )
          {
...
void trap_EngineerTrace(trace_t* results,
                        const vec3_t start,
                        const vec3_t mins,
                        const vec3_t maxs,
                        const vec3_t end,
  ...
  if (G_EmplacedGunIsRepairable(traceEnt, ent)) {
    ...
    if (traceEnt->health >= 255) {
      ...
      if (traceEnt->sound3to2 != ent->client->sess.sessionTeam) {
        AddScore(ent,
                 WOLF_REPAIR_BONUS);  // JPW NERVE props to the E for the fixin'
    ...
    if (traceEnt->methodOfDeath == MOD_LANDMINE) {
      ...
      if (!(tr2.surfaceFlags & SURF_LANDMINE) ||
        ...
        return;
        // bani
        //  rain - #384 - check landmine team so that enemy mines can be
        //  disarmed even if you're using all of yours :x
        ...
        if (G_LandmineUnarmed(traceEnt)) {
          ...
          //					if ( G_LandmineTeam( traceEnt )
          //!= ent->client->sess.sessionTeam ) 						return;
        ...
        if (G_LandmineUnarmed(traceEnt)) {
          ...
          // Don't allow disarming for sec (so guy that WAS arming doesn't start
          // disarming it!
        ...
        evilbanigoto:
          ...
          if (traceEnt->health >= 250) {
            /*						traceEnt->health = 255;
                                                            traceEnt->think =
               G_FreeEntity; traceEnt->nextthink = level.time + FRAMETIME;*/

            trap_SendServerCommand(ent - g_entities,
                                   "cp \"Landmine defused...\" 1");
            ...
            /*{
                    ...
                    teamList = ent->client->sess.sessionTeam == TEAM_AXIS ?
            &mapEntityData[0] : &mapEntityData[1]; if((mEnt =
            G_FindMapEntityData(teamList, traceEnt-g_entities)) != NULL) {
                            G_FreeMapEntityData( teamList, mEnt );
                    }

                    teamList = ent->client->sess.sessionTeam == TEAM_AXIS ?
            &mapEntityData[1] : &mapEntityData[0];	// inverted if((mEnt =
            G_FindMapEntityData(teamList, traceEnt-g_entities)) != NULL) { if(
            teamNum != ent->client->sess.sessionTeam ) { G_FreeMapEntityData(
            teamList, mEnt ); } else {
                            //	mEnt->type = ME_LANDMINE;	// set it back
            to this as it might have been set to 'about to explode'.
                                    mEnt->entNum = -1;
                            }
                    }
            }*/
...
// bani - no tripmine...
      ...
      if (traceEnt->s.teamNum >= 4) {
...

        {
          ...
          for (e = 0; e < numListedEntities; e++) {
            ...
            if (hit->s.teamNum == traceEnt->s.teamNum - 4) {
              // bani
              //							G_FreeEntity(
              //traceEnt ); 							trap_SendServerCommand( ent-g_entities, "cp \"You
              //cannot arm dynamite near a friendly construction!\" 1"); 							return;
              friendlyObj = qtrue;
            }
          }
        }
        ...
        for (i = 0; i < num; i++) {
          ...
          if ((hit->s.eType == ET_OID_TRIGGER)) {
            ...
            if (((hit->spawnflags & AXIS_OBJECTIVE) &&
              ...
              //							G_FreeEntity(
              //traceEnt ); 							trap_SendServerCommand( ent-g_entities, "cp \"You
              //cannot arm dynamite near a friendly objective!\" 1"); 							return;
              friendlyObj = qtrue;
            }
        ...
        // Gordon: moved down here to prevent two prints when dynamite IS near
        // objective
        ...
        // NERVE - SMF - made this the actual bounding box of dynamite instead
        // of range, also must snap origin to line up properly
        VectorCopy(traceEnt->r.currentOrigin, origin);
        SnapVector(origin);
        VectorAdd(origin, traceEnt->r.mins, mins);
        VectorAdd(origin, traceEnt->r.maxs, maxs);
          ...
          if ((hit->s.eType == ET_OID_TRIGGER)) {
            ...
            if (hit->spawnflags & AXIS_OBJECTIVE) {
              if (ent->client->sess.sessionTeam ==
                  TEAM_ALLIES) {  // transfer score info if this is a bomb
                                  // scoring objective
            ...
            // bani
            //						i = num;
            return;  // bani - bail out here because primary obj's take
                     // precendence over constructibles
          }
        }

        // bani - reordered this check so its AFTER the primary obj check
        //  Arnout - first see if the dynamite is planted near a constructable
        ...
        if (traceEnt->health >=
            248) {  // have to do this so we don't score multiple times
          return;
        }
        dynamiteDropTeam =
            traceEnt->s.teamNum;  // set this here since we wack traceent later
                                  // but want teamnum for scoring
        ...
        if (traceEnt->health >= 248) {
          ...
          // bani - eh, why was this commented out? it makes sense, and prevents
          // a sploit.
          ...
          for (i = 0; i < num; i++) {
            ...
            if ((hit->s.eType == ET_OID_TRIGGER)) {
              ...
              // bani - prevent plant/defuse exploit near a/h cabinets or
              // non-destroyable locations (bank doors on goldrush)
              if (!hit->target_ent ||
          ...
          // bani - reordered this check so its AFTER the primary obj check
          //  Gordon - first see if the dynamite was planted near a
          //  constructable object that would have been destroyed
          {
            int entityList[MAX_GENTITIES];
            int numListedEntities;
            int e;
            ...
            for (e = 0; e < numListedEntities; e++) {
              ...
              // not completely build yet - NOTE: don't do this, in case someone
              // places dynamite before construction is complete
...
void G_AddAirstrikeToCounters(gentity_t* ent) {
  int max = min(6, 2 * (ceil(g_heavyWeaponRestriction.integer *
                             G_TeamCount(ent, -1) * 0.1f * 10 * 0.01f)));
...
qboolean weapon_checkAirStrike(gentity_t* ent) {
  ...
  // FIXME: this is a stupid workaround. Just store the parent team in the
  // enitity itself and use that - no need to look up the parent
  if (!g_friendlyFire.integer && ent->parent->client &&
  ...
  if (ent->s.teamNum == TEAM_AXIS) {
    if (level.numActiveAirstrikes[0] > 6 ||
      ...
      /*			te = G_TempEntity( ent->parent->s.pos.trBase,
         EV_GLOBAL_CLIENT_SOUND ); te->s.eventParm = G_SoundIndex(
         "axis_hq_airstrike_denied" ); te->s.teamNum =
         ent->parent->s.clientNum;*/
...
void weapon_callAirStrike(gentity_t* ent) {
  ...
  if ((tr.fraction < 1.0) &&
    ...
    G_SayTo(ent->parent, ent->parent, 2, COLOR_YELLOW,
            "Pilot: ", "Aborting, can't see target.", qtrue);

    G_GlobalClientEvent(EV_AIRSTRIKEMESSAGE, 1, ent->parent - g_entities);

    /*		te = G_TempEntity( ent->parent->s.pos.trBase,
       EV_GLOBAL_CLIENT_SOUND ); if ( ent->s.teamNum == TEAM_ALLIES ) {
                            te->s.eventParm = G_SoundIndex(
       "allies_hq_airstrike_abort" ); } else { te->s.eventParm = G_SoundIndex(
       "axis_hq_airstrike_abort" );
                    }
  ...
  for (j = 0; j < ent->count; j++) {
    RotatePointAroundVector(
        bombaxis, dir, lookaxis,
        90 + crandom() * 30);  // munge the axis line a bit so it's not totally
                               // perpendicular
    ...
    for (i = 0; i < NUMBOMBS; i++) {
      bomb = G_Spawn();
      bomb->nextthink =
          level.time + i * 100 + crandom() * 50 + 1000 +
          (j * 2000);  // 1000 for aircraft flyby, other term for tumble stagger
      ...
      bomb->s.pos.trType = TR_STATIONARY;  // was TR_GRAVITY,  might wanna go
                                           // back to this and drop from height
      // bomb->s.pos.trTime = level.time;		// move a bit on the
      // very first frame
      ...
      VectorCopy(
          bomb->s.pos.trBase,
          bomboffset);  // make sure bombs fall "on top of" nonuniform scenery
    ...
    switch (sfx) {
      case 0:
        G_AddEvent(ent, EV_GENERAL_SOUND,
                   G_SoundIndex("sound/weapons/artillery/artillery_fly_1.wav"));
        break;
      case 1:
        G_AddEvent(ent, EV_GENERAL_SOUND,
                   G_SoundIndex("sound/weapons/artillery/artillery_fly_2.wav"));
        break;
      case 2:
        G_AddEvent(ent, EV_GENERAL_SOUND,
                   G_SoundIndex("sound/weapons/artillery/artillery_fly_3.wav"));
        break;
    }
  }
}
...
void Weapon_Artillery(gentity_t* ent) {
  ...
  // TAT - 10/27/2002 - moved energy check into a func, so I can use same check
  // in bot code
  ...
  if (ent->client->sess.sessionTeam == TEAM_AXIS) {
    if (!G_AvailableAirstrikes(ent)) {
      G_SayTo(ent, ent, 2, COLOR_YELLOW,
              "Fire Mission: ", "Insufficient fire support.", qtrue);
      ent->active = qfalse;
      ...
      /*			te = G_TempEntity( ent->s.pos.trBase,
         EV_GLOBAL_CLIENT_SOUND ); te->s.eventParm = G_SoundIndex(
         "axis_hq_airstrike_denied" ); te->s.teamNum = ent-g_entities;*/
    ...
    if (!G_AvailableAirstrikes(ent)) {
      ...
      /*			te = G_TempEntity( ent->s.pos.trBase,
         EV_GLOBAL_CLIENT_SOUND ); te->s.eventParm = G_SoundIndex(
         "allies_hq_airstrike_denied" ); te->s.teamNum = ent-g_entities;*/
  ...
  if ((trace.fraction < 1.0) &&
      (!(trace.surfaceFlags &
         SURF_NOIMPACT))) {  // JPW NERVE was SURF_SKY)) ) {
    G_SayTo(ent, ent, 2, COLOR_YELLOW,
            "Fire Mission: ", "Aborting, can't see target.", qtrue);
    ...
    /*		te = G_TempEntity( ent->s.pos.trBase, EV_GLOBAL_CLIENT_SOUND );
                    if ( ent->client->sess.sessionTeam == TEAM_ALLIES ) {
                            te->s.eventParm = G_SoundIndex(
       "allies_hq_ffe_abort" ); } else { te->s.eventParm = G_SoundIndex(
       "axis_hq_ffe_abort" );
                    }
  ...
  for (i = 0; i < count; i++) {
    ...
    bomb2->s.pos.trType = TR_STATIONARY;  // was TR_GRAVITY,  might wanna go
                                          // back to this and drop from height
...
//appearing since snapping 			too far off the target surface causes the the
//distance between the transmitted impact 			point and the actual hit surface
//larger than the mark radius.  (so nothing shows) )
...
==============
EmitterCheck
        see if a new particle emitter should be created at the bullet impact
point
==============
*/
...
qboolean Bullet_Fire_Extended(gentity_t* source,
  ...
  // bani - prevent shooting ourselves in the head when prone, firing through a
  // breakable
  ...
  if (traceEnt->takedamage) {
    ...
    // allow bullets to "pass through" func_explosives if they break by taking
    // another simultanious shot
    if (traceEnt->s.eType == ET_EXPLOSIVE) {
      if (traceEnt->health <= damage) {
        // start new bullet at position this hit the bmodel and continue to the
        // end position (ignoring shot-through bmodel in next trace) spread = 0
        // as this is an extension of an already spread shot
...
gentity_t* weapon_gpg40_fire(gentity_t* ent, int grenType) {
  ...
  trace_t tr;
  vec3_t viewpos;
  //	float		upangle = 0, pitch;			//	start
  //with level throwing and adjust based on angle
  vec3_t tosspos;
  ...
  // check for valid start spot (so you don't throw through or get stuck in a
  // wall)
  ...
  // Ridah, return the grenade so we can do some prediction before deciding if
  // we really want to throw it or not
  return m;
}
...
gentity_t* weapon_grenadelauncher_fire(gentity_t* ent, int grenType) {
  ...
  upangle = max(upangle, -50);  //	now clamped to	-50 / 50	(don't
                                //allow firing straight up/down)
  ...
  if (underhand) {
    // move a little bit more away from the player (so underhand tosses don't
    // get caught on nearby lips)
  ...
  if (grenType == WP_SMOKE_MARKER) {
    m->s.teamNum =
        ent->client->sess
            .sessionTeam;  // store team so we can generate red or blue smoke
    if (ent->client->sess.skill[SK_SIGNALS] >= 3) {
      m->count = 2;
  ...
  //----(SA)	adjust for movement of character.  TODO: Probably comment in
  //later, but only for forward/back not strafing VectorAdd( m->s.pos.trDelta,
  // ent->client->ps.velocity, m->s.pos.trDelta );	// "real" physics
...
void G_BurnMeGood(gentity_t* self, gentity_t* body) {
  ...
  // JPW NERVE -- yet another flamethrower damage model, trying to find a
  // feels-good damage combo that isn't overpowered
  if (body->lastBurnedFrameNumber != level.framenum) {
    G_Damage(body, self->parent, self->parent, vec3_origin,
             self->r.currentOrigin, 5, 0,
             MOD_FLAMETHROWER);  // was 2 dmg in release ver, hit avg. 2.5 times
                                 // per frame
...
void Weapon_FlamethrowerFire(gentity_t* ent) {
  ...
  // prevent flame thrower cheat, run & fire while aiming at the ground, don't
  // get hurt 72 total box height, 18 xy -> 77 trace radius (from view point
  // towards the ground) is enough to cover the area around the feet
...
// Rafael - for activate
void CalcMuzzlePointForActivate(gentity_t* ent,
                                vec3_t forward,
                                vec3_t right,
                                vec3_t up,
                                vec3_t muzzlePoint) {
...
void CalcMuzzlePoints(gentity_t* ent, int weapon) {
  ...
  //----(SA)	modified the muzzle stuff so that weapons that need to fire down
  //a perfect trace 			straight out of the camera (SP5, Mauser right now) can have
  //that accuracy, but 			weapons that need an offset effect (bazooka/grenade/etc.)
  //can still look like 			they came out of the weap.
  CalcMuzzlePointForActivate(ent, forward, right, up, muzzleTrace);
...
void FireWeapon(gentity_t* ent) {
  ...
  if (g_userAim.integer) {
    ...
    aimSpreadScale +=
        0.15f;  // (SA) just adding a temp /maximum/ accuracy for player (this
