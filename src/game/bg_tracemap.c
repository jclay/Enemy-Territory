...
void CG_GenerateTracemap(void) {
  ...
  if (!cg.mapcoordsValid) {
    CG_Printf(
        "Need valid mapcoords in the worldspawn to be able to generate a "
        "tracemap.\n");
    return;
  }

  if ((cg.mapcoordsMaxs[0] - cg.mapcoordsMins[0]) !=
      (cg.mapcoordsMins[1] - cg.mapcoordsMaxs[1])) {
  ...
  for (i = 0; i < TRACEMAP_SIZE; i++) {
    ...
    for (j = 0; j < TRACEMAP_SIZE; j++) {
      ...
      // Find the ceiling
      CG_Trace(&tr, start, NULL, NULL, end, ENTITYNUM_NONE,
      ...
      // Find ground
      while (1) {
        // Perform traces up to the sky, repeating at a higher start height if
        // we start inside a solid.
        ...
        if (tr.startsolid) {  // Stuck in something, skip over it.
          start[2] -= 64;
        } else if (tr.fraction == 1) {  // Didn't hit anything, we're (probably)
                                        // outside the world
          tracemap.ground[j][i] = MIN_WORLD_HEIGHT;
          break;
        } else {
      ...
      if (!((lastDraw <= ms) && (lastDraw > ms - 500))) {
        ...
        CG_Printf(
            "%i of %i gridpoints calculated (%.2f%%), %i total traces\n",
  ...
  CG_Printf(
      ...
      ((i * TRACEMAP_SIZE) / (float)(TRACEMAP_SIZE * TRACEMAP_SIZE)) * 100.f,
      tracecount);
  trap_UpdateScreen();
    ...
    for (j = 0; j < TRACEMAP_SIZE; j++) {
      ...
      if (start[2] == MIN_WORLD_HEIGHT) {
        ...
        while (1) {
          ...
          tracecount++;
          if (tr.startsolid) {  // Stuck in something, skip over it.
            // can happen, tr.endpos still is valid even if we're starting in a
            // solid but trace out of it hitting the next surface
            if (tr.surfaceFlags & SURF_SKY) {
              // are we in a solid?
              if (!(CG_PointContents(tr.endpos, ENTITYNUM_NONE) &
                    (MASK_SOLID | MASK_WATER))) {
          ...
          } else if (tr.fraction == 1) {  // Didn't hit anything, we're
                                          // (probably) outside the world
            tracemap.sky[j][i] = MAX_WORLD_HEIGHT;
            break;
          } else if (tr.surfaceFlags &
                     SURF_SKY) {  // Hit sky, this is where we start.
            tracemap.sky[j][i] = tr.endpos[2];
            if (tracemap.sky[j][i] > max) {
              max = tracemap.sky[j][i];
            }
  ...
  for (i = 0; i < TRACEMAP_SIZE; i++) {
    ...
    for (j = 0; j < TRACEMAP_SIZE; j++) {
      ...
      if (tracemap.sky[j][i] == MAX_WORLD_HEIGHT &&
          tracemap.ground[j][i] != MIN_WORLD_HEIGHT) {
        // Find the ceiling
        CG_Trace(&tr, start, NULL, NULL, end, ENTITYNUM_NONE,
                 MASK_SOLID | MASK_WATER);
        tracecount++;
        if (tr.fraction ==
            1) {  // Didn't hit anything, we're (probably) outside the world
  ...
  // write tga
  trap_FS_FOpenFile(va("maps/%s_tracemap.tga", Q_strlwr(cgs.rawmapname)), &f,
                    FS_WRITE);
    ...
    for (i = 0; i < TRACEMAP_SIZE; i++) {
      ...
      for (j = 0; j < TRACEMAP_SIZE; j++) {
        if (i == 0 && j < 6) {
          ...
          switch (j) {
            case 0:
              ground_min = datablock[j][0] | (datablock[j][1] << 8) |
                           (datablock[j][2] << 16) | (datablock[j][3] << 24);
              break;
            case 1:
              ground_max = datablock[j][0] | (datablock[j][1] << 8) |
                           (datablock[j][2] << 16) | (datablock[j][3] << 24);
              break;
            case 2:
              skyground_min = datablock[j][0] | (datablock[j][1] << 8) |
                              (datablock[j][2] << 16) | (datablock[j][3] << 24);
              break;
            case 3:
              skyground_max = datablock[j][0] | (datablock[j][1] << 8) |
                              (datablock[j][2] << 16) | (datablock[j][3] << 24);
              break;
            case 4:
              sky_min = datablock[j][0] | (datablock[j][1] << 8) |
                        (datablock[j][2] << 16) | (datablock[j][3] << 24);
              break;
            case 5:
              sky_max = datablock[j][0] | (datablock[j][1] << 8) |
                        (datablock[j][2] << 16) | (datablock[j][3] << 24);
              break;
          }
      ...
      /*for( j = 0; j < TRACEMAP_SIZE; j++ ) {
              if( i == 0 && j < 6 ) {
                      // abuse first six pixels for our extended data
                      switch( j ) {
                              case 0:	trap_FS_Read( &ground_min,
      sizeof(ground_min), f ); break; case 1: trap_FS_Read( &ground_max,
      sizeof(ground_max), f ); break; case 2:	trap_FS_Read( &skyground_min,
      sizeof(skyground_min), f ); break; case 3: trap_FS_Read( &skyground_max,
      sizeof(skyground_max), f ); break; case 4: trap_FS_Read( &sky_min,
      sizeof(sky_min), f ); break; case 5: trap_FS_Read( &sky_max,
      sizeof(sky_max), f ); break;
                      }
                      tracemap.sky[TRACEMAP_SIZE - 1 - i][j] = MAX_WORLD_HEIGHT;
                      tracemap.skyground[TRACEMAP_SIZE - 1 - i][j] =
      MAX_WORLD_HEIGHT; tracemap.ground[TRACEMAP_SIZE - 1 - i][j] =
      MIN_WORLD_HEIGHT; continue;
              }

              trap_FS_Read( &datablock, sizeof(datablock), f );	// b g r a
              tracemap.sky[TRACEMAP_SIZE - 1 - i][j] = (float)datablock[0];
      // FIXME: swap if( tracemap.sky[TRACEMAP_SIZE - 1 - i][j] == 0 )
                      tracemap.sky[TRACEMAP_SIZE - 1 - i][j] = MAX_WORLD_HEIGHT;

              //trap_FS_Read( &data, 1, f ); // g
              tracemap.skyground[TRACEMAP_SIZE - 1 - i][j] =
      (float)datablock[1];	// FIXME: swap if(
      tracemap.skyground[TRACEMAP_SIZE - 1 - i][j] == 0 )
                      tracemap.skyground[TRACEMAP_SIZE - 1 - i][j] =
      MAX_WORLD_HEIGHT;

              //trap_FS_Read( &data, sizeof(data), f );	// r
              tracemap.ground[TRACEMAP_SIZE - 1 - i][j] = (float)datablock[2];
      // FIXME: swap if( tracemap.ground[TRACEMAP_SIZE - 1 - i][j] == 0 )
                      tracemap.ground[TRACEMAP_SIZE - 1 - i][j] =
      MIN_WORLD_HEIGHT;
