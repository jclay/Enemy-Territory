...
// bg_animation.c
//
//	Incorporates several elements related to the new flexible animation
//system.
//
...
animStringItem_t animStateStr[] = {
    {"RELAXED", -1}, {"QUERY", -1}, {"ALERT", -1}, {"COMBAT", -1},
...
static animStringItem_t animMoveTypesStr[] = {
    ...
    {"WALKCRBK", -1},     {"RUN", -1},
    {"RUNBK", -1},        {"SWIM", -1},
    {"SWIMBK", -1},       {"STRAFERIGHT", -1},
    {"STRAFELEFT", -1},   {"TURNRIGHT", -1},
    {"TURNLEFT", -1},     {"CLIMBUP", -1},
    {"CLIMBDOWN", -1},    {"FALLEN", -1},  // DHM - Nerve :: dead, before limbo
    {"PRONE", -1},        {"PRONEBK", -1},
    {"IDLEPRONE", -1},    {"FLAILING", -1},
...
animStringItem_t animBodyPartsStr[] = {
    {"** UNUSED **", -1}, {"LEGS", -1}, {"TORSO", -1}, {"BOTH", -1},
...
// !!! NOTE: this must be kept in sync with the tag names in
...
static animStringItem_t animEnemyTeamsStr[] = {
    {"NAZI", -1},   {"ALLIES", -1}, {"MONSTER", -1}, {"SPARE1", -1},
    {"SPARE2", -1}, {"SPARE3", -1}, {"SPARE4", -1},  {"NEUTRAL", -1}};
...
static animStringItem_t animFlailTypeStr[] = {
    {"** UNUSED **", -1}, {"INAIR", -1}, {"VCRASH", -1}, {"HCRASH", -1},
...
static animStringItem_t animAIStateStr[] = {{"RELAXED", -1},
                                            {"QUERY", -1},
                                            {"ALERT", -1},
                                            {"COMBAT", -1}};
...
/*
================
return a hash value for the given string (make sure the strings and lowered
first)
================
*/
...
void QDECL BG_AnimParseError(const char* msg, ...) {
  ...
  if (globalFilename) {
    Com_Error(ERR_DROP, "%s: (%s, line %i)", text, globalFilename,
...
/*animModelInfo_t *BG_ModelInfoForClient( int client ) {
        if (!globalScriptData)
                BG_AnimParseError( "BG_ModelInfoForClient: NULL
globalScriptData" );
        //
        if (!globalScriptData->clientModels[client])
                BG_AnimParseError( "BG_ModelInfoForClient: client %i has no
modelinfo", client );
        //
        return
&globalScriptData->modelInfo[globalScriptData->clientModels[client] - 1];
}*/
...
/*animModelInfo_t *BG_ModelInfoForModelname( char *modelname ) {
        ...
        //
        if (!globalScriptData)
                BG_AnimParseError( "BG_ModelInfoForModelname: NULL
globalScriptData" );
        //
        for (i=0, modelInfo=globalScriptData->modelInfo;
i<MAX_ANIMSCRIPT_MODELS; i++, modelInfo++) { if (!modelInfo->modelname[0])
                        continue;
...
=================
*/
static int BG_AnimationIndexForString(char* string,
  ...
  // no match found
  BG_AnimParseError(
      "BG_AnimationIndexForString: unknown index '%s' for animation group '%s'",
...
animation_t* BG_AnimationForString(char* string,
  ...
  Com_Error(
      ERR_DROP,
      "BG_AnimationForString: unknown animation '%s' for animation group '%s'",
      string, animModelInfo->animationGroup);
...
=================
*/
int BG_IndexForString(char* token,
...
===============
*/
char* BG_CopyStringIntoBuffer(char* string,
                              char* buffer,
                              int bufSize,
                              int* offset) {
  char* pch;
...
void BG_ParseConditionBits(char** text_pp,
  ...
  while (!endFlag) {
    ...
    if (!endFlag && Q_stricmp(token, "AND") &&
        Q_stricmp(token, "MINUS")) {  // must be a index
    ...
    if (!Q_stricmp(token, "AND") || !Q_stricmp(token, "MINUS") || endFlag) {
      ...
      if (!strlen(currentString)) {
        if (endFlag) {
          BG_AnimParseError(
              "BG_AnimParseAnimScript: unexpected end of condition");
        } else {
      ...
      if (!Q_stricmp(currentString, "all")) {
        ...
        indexFound =
            BG_IndexForString(currentString, defineStr[condIndex], qtrue);
        if (indexFound >= 0) {
  ...
  while (1) {
    ...
    switch (animConditionsTable[conditionIndex].type) {
      case ANIM_CONDTYPE_BITFLAGS:
        BG_ParseConditionBits(text_pp,
                              animConditionsTable[conditionIndex].values,
      ...
      case ANIM_CONDTYPE_VALUE:
        if (animConditionsTable[conditionIndex].values) {
          ...
          if (!token || !token[0]) {
            BG_AnimParseError(
                "BG_AnimParseAnimScript: expected condition value, found end "
                "of line");  // RF modification
          }
          // check for a comma (condition divider)
          if (token[strlen(token) - 1] == ',') {
            token[strlen(token) - 1] = '\0';
          }
          conditionValue[0] = BG_IndexForString(
              token, animConditionsTable[conditionIndex].values, qfalse);
        } else {
          conditionValue[0] =
              1;  // not used, just check for a positive condition
        }
        break;
    ...
    scriptItem->conditions[scriptItem->numConditions].value[0] =
        conditionValue[0];
    scriptItem->conditions[scriptItem->numConditions].value[1] =
        conditionValue[1];
...
static void BG_ParseCommands(char** input,
  ...
  while (1) {
    ...
    command->bodyPart[partIndex] =
        BG_IndexForString(token, animBodyPartsStr, qtrue);
    if (command->bodyPart[partIndex] > 0) {
      ...
      command->animIndex[partIndex] =
          BG_AnimationIndexForString(token, animModelInfo);
      command->animDuration[partIndex] =
          animModelInfo->animations[command->animIndex[partIndex]]->duration;
      ...
      if (parseMovetype != ANIM_MT_UNUSED &&
          command->bodyPart[partIndex] != ANIM_BP_TORSO) {
        animModelInfo->animations[command->animIndex[partIndex]]->movetype |=
            (1 << parseMovetype);
      }
      // if this is a fireweapon event, then this is a firing animation
      if (parseEvent == ANIM_ET_FIREWEAPON ||
          parseEvent == ANIM_ET_FIREWEAPONPRONE) {
        animModelInfo->animations[command->animIndex[partIndex]]->flags |=
            ANIMFL_FIRINGANIM;
        animModelInfo->animations[command->animIndex[partIndex]]->initialLerp =
            40;
      }
    ...
    while (1) {
      ...
      if (!Q_stricmp(token, "sound")) {
        ...
        if (strstr(token, ".wav")) {
          BG_AnimParseError(
              "BG_ParseCommands: wav files not supported, only sound scripts");  // RF mod
        }
        ...
        command->soundIndex = globalScriptData->soundIndex != NULL
                                  ? globalScriptData->soundIndex(token)
                                  : 0;
...
static animStringItem_t animParseModesStr[] = {
    {"defines", -1},      {"animations", -1}, {"canned_animations", -1},
    {"statechanges", -1}, {"events", -1},
...
#define MAX_INDENT_LEVELS 3
  ...
  while (1) {
    ...
    if (newParseMode >= 0) {
      if (indentLevel) {
        BG_AnimParseError("BG_AnimParseAnimScript: unexpected '%s'",
                          token);  // RF mod
      }
    ...
    switch (parseMode) {
      case PARSEMODE_DEFINES:
        ...
        if (!Q_stricmp(token, "set")) {
          ...
          if (!token || !*token) {
            BG_AnimParseError(
                "BG_AnimParseAnimScript: expected condition type string");  // RF mod
          }
          ...
          if (!token || !*token) {
            BG_AnimParseError(
                "BG_AnimParseAnimScript: expected condition define string");  // RF mod
          }

          // copy the define to the strings list
          defineStr[defineType][numDefines[defineType]].string =
              BG_CopyStringIntoBuffer(token, defineStrings,
          ...
          if (!token) {
            BG_AnimParseError(
                "BG_AnimParseAnimScript: expected '=', found end of line");  // RF mod
          }
          if (Q_stricmp(token, "=")) {
            BG_AnimParseError(
          ...
          // parse the bits
          BG_ParseConditionBits(&text_p, animConditionsTable[defineType].values,
                                defineType,
                                defineBits[defineType][numDefines[defineType]]);
          numDefines[defineType]++;

          // copy the weapon defines over to the enemy_weapon defines
          memcpy(&defineStr[ANIM_COND_ENEMY_WEAPON][0],
                 &defineStr[ANIM_COND_WEAPON][0],
                 sizeof(animStringItem_t) * MAX_ANIM_DEFINES);
          memcpy(
              &defineBits[ANIM_COND_ENEMY_WEAPON][0],
              &defineBits[ANIM_COND_WEAPON][0],
      ...
      case PARSEMODE_CANNED_ANIMATIONS:
        ...
        if (!Q_stricmp(token, "{")) {
          // about to increment indent level, check that we have enough
          // information to do this
          if (indentLevel >= MAX_INDENT_LEVELS) {  // too many indentations
            BG_AnimParseError("BG_AnimParseAnimScript: unexpected '%s'",
                              token);  // RF mod
          }
          if (indexes[indentLevel] <
              0) {  // we havent found out what this new group is yet
            BG_AnimParseError("BG_AnimParseAnimScript: unexpected '%s'",
                              token);  // RF mod
          }
          //
          ...
          if (Q_stricmp(token, "state")) {
            BG_AnimParseError(
                "BG_AnimParseAnimScript: expected 'state'");  // RF mod
          }
          ...
          if (!token) {
            BG_AnimParseError(
                "BG_AnimParseAnimScript: expected state type");  // RF mod
          }
          ...
          indexes[indentLevel] =
              BG_IndexForString(token, animMoveTypesStr, qfalse);
          ...
          if (Q_strncmp(text_p, token, strlen(token))) {
            // this should never happen, just here to check that this operation
            // is correct before code goes live
          ...
          currentScript->items[currentScript->numItems] =
              &animModelInfo->scriptItems[animModelInfo->numScriptItems++];
          ...
          *currentScriptItem = tempScriptItem;
          ...
          // we are reading the commands, so parse this line as if it were a
          // command move the text_p backwards so we can read in the last token
          // again
          text_p -= strlen(token);
          // sanity check that
          if (Q_strncmp(text_p, token, strlen(token))) {
      ...
      case PARSEMODE_EVENTS:
        ...
        if (!Q_stricmp(token, "{")) {
          ...
          /*				if ( parseMode == PARSEMODE_STATECHANGES
                                                  ...
                                                  if ( Q_stricmp( token,
          "statechange" ) ) { BG_AnimParseError( "BG_AnimParseAnimScript:
          expected 'statechange', got '%s'", token );	// RF mod
                                                  }

                                                  // read in the old state type
                                                  token = COM_ParseExt( &text_p,
          qfalse ); if ( !token ) { BG_AnimParseError( "BG_AnimParseAnimScript:
          expected <state type>" );	// RF mod
                                                  }
                                                  oldState = BG_IndexForString(
          token, animStateStr, qfalse );
          ...
          expected <state type>" );	// RF mod
                                                  ...
                                                  indexes[indentLevel] =
          BG_IndexForString( token, animStateStr, qfalse );

                                                  currentScript =
          &animModelInfo->scriptStateChange[oldState][indexes[indentLevel]];

          //----(SA)		// RF mod
                                                  // check for the open bracket
                                                  token = COM_ParseExt( &text_p,
          qtrue ); if ( !token || Q_stricmp( token, "{" ) ) { BG_AnimParseError(
          ...
          indexes[indentLevel] =
              BG_IndexForString(token, animEventTypesStr, qfalse);
...
//------------------------------------------------------------------------
//
// run-time gameplay functions, these are called during gameplay, so they must
// be cpu efficient.
//
...
qboolean BG_EvaluateConditions(int client, animScriptItem_t* scriptItem) {
  ...
  for (i = 0, cond = scriptItem->conditions; i < scriptItem->numConditions;
       i++, cond++) {
    switch (animConditionsTable[cond->index].type) {
      case ANIM_CONDTYPE_BITFLAGS:
        if (!(globalScriptData->clientConditions[client][cond->index][0] &
              cond->value[0]) &&
            !(globalScriptData->clientConditions[client][cond->index][1] &
              cond->value[1])) {
          return qfalse;
        }
        break;
...
animScriptItem_t* BG_FirstValidItem(int client, animScript_t* script) {
  ...
  for (i = 0, ppScriptItem = script->items; i < script->numItems;
       i++, ppScriptItem++) {
...
===============
*/
int BG_PlayAnim(playerState_t* ps,
                animModelInfo_t* animModelInfo,
                int animNum,
  ...
  if (forceDuration) {
    ...
    duration = animModelInfo->animations[animNum]->duration +
               50;  // account for lerping between anims
  }
    ...
    case ANIM_BP_LEGS:
      ...
      if ((ps->legsTimer < 50) || force) {
        if (!isContinue || !((ps->legsAnim & ~ANIM_TOGGLEBIT) == animNum)) {
          wasSet = qtrue;
          ps->legsAnim =
              ((ps->legsAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) | animNum;
          if (setTimer) {
    ...
    case ANIM_BP_TORSO:
      ...
      if ((ps->torsoTimer < 50) || force) {
        if (!isContinue || !((ps->torsoAnim & ~ANIM_TOGGLEBIT) == animNum)) {
          ps->torsoAnim =
              ((ps->torsoAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) | animNum;
          if (setTimer) {
...
===============
*/
int BG_PlayAnimName(playerState_t* ps,
                    animModelInfo_t* animModelInfo,
                    char* animName,
                    animBodyPart_t bodyPart,
                    qboolean setTimer,
                    qboolean isContinue,
                    qboolean force) {
  // return BG_PlayAnim( ps, BG_AnimationIndexForString( animName,
  // BG_GetCharacterForPlayerstate( ps )->animModelInfo ), bodyPart, 0,
  // setTimer, isContinue, force );
  return BG_PlayAnim(ps, animModelInfo,
                     BG_AnimationIndexForString(animName, animModelInfo),
                     bodyPart, 0, setTimer, isContinue, force);
}
...
===============
*/
int BG_ExecuteCommand(playerState_t* ps,
                      animModelInfo_t* animModelInfo,
  ...
  if (scriptCommand->bodyPart[0]) {
    ...
    if (scriptCommand->bodyPart[0] == ANIM_BP_BOTH ||
      ...
      playedLegsAnim =
          (BG_PlayAnim(ps, animModelInfo, scriptCommand->animIndex[0],
                       scriptCommand->bodyPart[0], duration, setTimer,
                       isContinue, force) > -1);
    } else {
      BG_PlayAnim(ps, animModelInfo, scriptCommand->animIndex[0],
                  scriptCommand->bodyPart[0], duration, setTimer, isContinue,
                  force);
    }
  }
  if (scriptCommand->bodyPart[1]) {
    ...
    if (scriptCommand->bodyPart[1] == ANIM_BP_BOTH ||
        scriptCommand->bodyPart[1] == ANIM_BP_LEGS) {
      playedLegsAnim =
          (BG_PlayAnim(ps, animModelInfo, scriptCommand->animIndex[1],
                       scriptCommand->bodyPart[1], duration, setTimer,
                       isContinue, force) > -1);
    } else {
      BG_PlayAnim(ps, animModelInfo, scriptCommand->animIndex[1],
                  scriptCommand->bodyPart[1], duration, setTimer, isContinue,
                  force);
    }
  }

  if (scriptCommand->soundIndex) {
    globalScriptData->playSound(scriptCommand->soundIndex, ps->origin,
                                ps->clientNum);
  }
...
================
*/
int BG_AnimScriptAnimation(playerState_t* ps,
                           animModelInfo_t* animModelInfo,
  ...
  // pick the correct animation for this character (animations must be constant
  // for each character, otherwise they'll constantly change)
  scriptCommand =
      &scriptItem->commands[ps->clientNum % scriptItem->numCommands];

#ifdef DBGANIMS
  if (scriptCommand->bodyPart[0]) {
    Com_Printf("anim0 (%s): %s",
               animBodyPartsStr[scriptCommand->bodyPart[0]].string,
               animModelInfo->animations[scriptCommand->animIndex[0]]->name);
  }
  if (scriptCommand->bodyPart[1]) {
    Com_Printf("anim1 (%s): %s",
               animBodyPartsStr[scriptCommand->bodyPart[1]].string,
  ...
  // run it
  return (BG_ExecuteCommand(ps, animModelInfo, scriptCommand, qfalse,
                            isContinue, qfalse) != -1);
}
...
================
*/
int BG_AnimScriptCannedAnimation(playerState_t* ps,
                                 animModelInfo_t* animModelInfo) {
  animScript_t* script;
  animScriptItem_t* scriptItem;
  animScriptCommand_t* scriptCommand;
  ...
  movetype =
      globalScriptData->clientConditions[ps->clientNum][ANIM_COND_MOVETYPE][0];
  ...
  // run it
  return BG_ExecuteCommand(ps, animModelInfo, scriptCommand, qtrue, qfalse,
                           qfalse);
}
...
================
*/
int BG_AnimScriptEvent(playerState_t* ps,
                       animModelInfo_t* animModelInfo,
...
/*qboolean BG_ValidAnimScript( int clientNum )
{
        if (!globalScriptData->clientModels[clientNum])
                return qfalse;
        //
        if (!globalScriptData->modelInfo[
globalScriptData->clientModels[clientNum] ].numScriptItems) return qfalse;
        //
        return qtrue;
}*/
...
==============
*/
void BG_UpdateConditionValue(int client,
                             int condition,
                             int value,
  ...
  if (checkConversion == qtrue) {
    if (animConditionsTable[condition].type == ANIM_CONDTYPE_BITFLAGS) {
      ...
      // DHM - Nerve :: We want to set the ScriptData to the explicit value
      // passed in.
      //				COM_BitSet will OR values on top of each
      //other, so clear it first.
...
int BG_GetConditionValue(int client, int condition, qboolean checkConversion) {
  ...
  if (animConditionsTable[condition].type == ANIM_CONDTYPE_BITFLAGS) {
    if (checkConversion) {
      ...
      // if (!value)
      //	return 0;
      for (i = 0; i < 8 * sizeof(globalScriptData->clientConditions[0][0]);
           i++) {
        if (COM_BitCheck(globalScriptData->clientConditions[client][condition],
                         i)) {
          return i;
        }
      }
...
qboolean BG_GetConditionBitFlag(int client, int condition, int bitNumber) {
  if (animConditionsTable[condition].type == ANIM_CONDTYPE_BITFLAGS) {
    return (COM_BitCheck(globalScriptData->clientConditions[client][condition],
                         bitNumber));
  } else {
    Com_Error(ERR_DROP,
              "BG_GetConditionBitFlag: animation condition %i is not a bitflag "
              "condition",
...
void BG_ClearConditionBitFlag(int client, int condition, int bitNumber) {
  COM_BitClear(globalScriptData->clientConditions[client][condition],
               bitNumber);
}
...
BG_GetAnimScriptAnimation
  ...
  returns the locomotion animation index, -1 if no animation was found, 0
otherwise
================
*/
int BG_GetAnimScriptAnimation(int client,
                              animModelInfo_t* animModelInfo,
...
===============
*/
animation_t* BG_GetAnimationForIndex(animModelInfo_t* animModelInfo,
                                     int index) {
...
void BG_AnimUpdatePlayerStateConditions(pmove_t* pmove) {
  ...
  if (ps->eFlags & EF_ZOOMING) {
    BG_UpdateConditionValue(ps->clientNum, ANIM_COND_WEAPON, WP_BINOCULARS,
                            qtrue);
    BG_SetConditionBitFlag(ps->clientNum, ANIM_COND_GEN_BITFLAG,
                           ANIM_BITFLAG_ZOOMING);
  } else {
    BG_UpdateConditionValue(ps->clientNum, ANIM_COND_WEAPON, ps->weapon, qtrue);
    BG_ClearConditionBitFlag(ps->clientNum, ANIM_COND_GEN_BITFLAG,
                             ANIM_BITFLAG_ZOOMING);
  }

  // MOUNTED
  if (ps->eFlags & EF_MG42_ACTIVE || ps->eFlags & EF_MOUNTEDTANK) {
    BG_UpdateConditionValue(ps->clientNum, ANIM_COND_MOUNTED, MOUNTED_MG42,
                            qtrue);
  } else if (ps->eFlags & EF_AAGUN_ACTIVE) {
    BG_UpdateConditionValue(ps->clientNum, ANIM_COND_MOUNTED, MOUNTED_AAGUN,
                            qtrue);
  } else {
    BG_UpdateConditionValue(ps->clientNum, ANIM_COND_MOUNTED, MOUNTED_UNUSED,
                            qtrue);
  }

  // UNDERHAND
  BG_UpdateConditionValue(ps->clientNum, ANIM_COND_UNDERHAND,
                          ps->viewangles[0] > 0, qtrue);
  ...
  if (ps->pm_flags & PMF_FLAILING) {
    if (ps->groundEntityNum == ENTITYNUM_NONE) {
      BG_UpdateConditionValue(ps->clientNum, ANIM_COND_FLAILING_TYPE,
                              FLAILING_INAIR, qtrue);
      ps->pm_time = 750;
    } else {
      if (globalScriptData
              ->clientConditions[ps->clientNum][ANIM_COND_FLAILING_TYPE][0] !=
          FLAILING_VCRASH) {
        BG_UpdateConditionValue(ps->clientNum, ANIM_COND_FLAILING_TYPE,
                                FLAILING_VCRASH, qtrue);
        ps->pm_time = 750;
      }
    }
  }
