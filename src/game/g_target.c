...
*/
void Use_target_remove_powerups(gentity_t* ent,
                                gentity_t* other,
                                gentity_t* activator) {
  if (!activator->client) {
    return;
...
void Use_Target_Score(gentity_t* ent, gentity_t* other, gentity_t* activator) {
  ...
  // G_AddExperience( activator, 0 ); // Gordon: FIXME: add this to part of the
  // entity?
}
...
*/
void Use_Target_Speaker(gentity_t* ent,
                        gentity_t* other,
                        gentity_t* activator) {
...
void target_teleporter_use(gentity_t* self,
                           gentity_t* other,
                           gentity_t* activator) {
  gentity_t* dest;
  ...
  if (activator) {  // activator can be NULL if called from script
    if (self->key) {
      ...
      /*			item = BG_FindItemForKey(self->key, 0);
         ...
         // relay is NOKEY_ONLY and player has key if (self->soundPos1) G_Sound(
         self, self->soundPos1);	//----(SA)	added return;
                                              }
                                      }
                                      else
         // user does not have key
                                      {
                                              if (!(self->spawnflags & 8) )
                                              {
                                                      if (self->soundPos1)
                                                              G_Sound( self,
         self->soundPos1);	//----(SA)	added return;
                                              }
                                      }
                              }*/
  ...
  while ((targ = G_FindByTargetname(targ, target))) {
    ...
    if (targ->s.eType == ET_MOVER &&
      ...
      G_Damage(targ, killer, killer, NULL, NULL, 100000, DAMAGE_NO_PROTECTION,
               MOD_TELEFRAG);
      //			targ->die(targ, killer, killer, targ->health,
      //0);
      continue;
    }
...
void SP_target_location(gentity_t* self) {
  G_Printf(S_COLOR_YELLOW
           "WARNING: target_location entities are now obsolete. Please remove "
           "ASAP\n");
...
==============
*/
void Use_Target_Counter(gentity_t* ent,
                        gentity_t* other,
                        gentity_t* activator) {
...
Gravity 1 second	= 1000 1 FRAME		= 100 delay		= 100 =
one millisecond default this is the maximum smoke that will show up time
= 5000 default before the smoke disipates duration	= 2000 before the smoke
starts to alpha start_size	= 24 default end_size	= 96 default wait
= default is 50 the rate at which it will travel up shader		= custom
...
void SP_target_smoke(gentity_t* ent) {
  ...
  // Arnout - modified this a lot to be sent to the client as one entity and
  // then is shown at the client
  if (!ent->delay) {
    ent->delay = 100;
  }
...
*/
void target_script_trigger_use(gentity_t* ent,
                               gentity_t* other,
                               gentity_t* activator) {
  ...
  // Are we using ainame to find another ent instead of using scriptname for
  // this one?
  if (ent->aiName) {
