...
#include "g_local.h"
qboolean G_SpawnStringExt(const char* key,
  ...
  if (!level.spawning) {
    *out = (char*)defaultString;
    ...
    // see InitMover
    G_Error("G_SpawnString() called while not spawning, file %s, line %i", file,
            line);
  }
  ...
  for (item = bg_itemlist + 1; item->classname; item++) {
    if (!strcmp(item->classname, ent->classname)) {
      ...
      if (g_gametype.integer !=
          GT_WOLF_LMS) {  // Gordon: lets not have items in last man standing
                          // for the moment
