...
#define SND_REQUESTCUT \
  0x002  // Allow sound to be cut off by following sounds on this channel only
         // for sounds who request cutoff
#define SND_CUTOFF \
...
#define SND_NOCUT \
  0x010  // Don't cut off.  Always let finish (overridden by SND_CUTOFF_ALL)
#define SND_NO_ATTENUATION \
  0x020  // don't attenuate (even though the sound is in voice channel, for
         // example)
...
typedef enum {
  ...
  ERR_VID_FATAL,  // exit the entire game with a popup window and doesn't delete
                  // profile.pid
...
void GetPerpendicularViewVector(const vec3_t point,
                                ...
                                const vec3_t p2,
                                vec3_t up);
void ProjectPointOntoVector(vec3_t point,
                            vec3_t vStart,
                            vec3_t vEnd,
                            vec3_t vProj);
void ProjectPointOntoVectorBounded(vec3_t point,
                                   vec3_t vStart,
                                   vec3_t vEnd,
                                   vec3_t vProj);
...
#define CVAR_UNSAFE \
  4096  // ydnar: unsafe system cvars (renderer, sound settings, anything that
        // might cause a crash)
#define CVAR_SERVERINFO_NOUPDATE \
  8192  // gordon: WONT automatically send this to clients, but server browsers
        // will see it
...
#define SNAPFLAG_NOT_ACTIVE \
  2  // snapshot used during connection and for zombies
...
#define MAX_MODELS \
  256  // these are sent over the net as 8 bits (Gordon: upped to 9 bits, erm
       // actually it was already at 9 bits, wtf? NEVAR TRUST GAMECODE COMMENTS,
       // comments are evil :E, lets hope it doesnt horribly break anything....)
...
// (Gordon: unless it doesnt need transmitted over the network, in which case it
// should prolly go in the new pmext struct anyway)
...
typedef struct playerState_s {
  ...
  vec3_t velocity;
  int weaponTime;
  int weaponDelay;  // for weapons that don't fire immediately when 'fire' is
                    // hit (grenades, venom, ...)
  int grenadeTimeLeft;  // for delayed grenade throwing.  this is set to a
                        // #define for grenade lifetime when the attack button
                        // goes down, then when attack is released this is the
                        // amount of time left before the grenade goes off (or
                        // if it gets to 0 while in players hand, it explodes)
  ...
  trace_t
      serverCursorHintTrace;  // not communicated over net, but used to store
                              // the current server-side cursorhint trace

  // ----------------------------------------------------------------------
  // So to use persistent variables here, which don't need to come from the
  // server, we could use a marker variable, and use that to store everything
  // after it before we read in the new values for the predictedPlayerState,
  // then restore them after copying the structure recieved from the server.
  ...
  // JPW NERVE -- value for all multiplayer classes with regenerating "class
  // weapons" -- ie LT artillery, medic medpack, engineer build points, etc
  ...
  int weapAnim;  // mask off ANIM_TOGGLEBIT
                 // //----(SA)	added		// Arnout : DOES get send over
                 // the network
  ...
  // seems like heat and aimspread could be tied together somehow, however, they
  // (appear to) change at different rates and I can't currently see how to
  // optimize this to one server->client transmission "weapstatus" value.
  int weapHeat[MAX_WEAPONS];  // some weapons can overheat.  this tracks
                              // (server-side) how hot each weapon currently is.
  int curWeapHeat;  // value for the currently selected weapon (for transmission
                    // to client)		// Arnout : DOES get send over
                    // the network
...
typedef enum {
  ...
  ET_GAMEMODEL,   // misc_gamemodel.  similar to misc_model, but it's a dynamic
                  // model so we have LOD
  ...
  // Start - TAT - 8/29/2002
  // An indicator object created by the bot code to show where the bots are
  // moving to
...
typedef struct entityState_s {
  ...
  int powerups;   // bit flags	// Arnout: used to store entState_t for
                  // non-player entities (so we know to draw them translucent
                  // clientsided)
  ...
  int dmgFlags;  // to pass along additional information for damage effects for
                 // players/ Also used for cursorhints for non-player entities
  ...
  aistateEnum_t aiState;  // xkan, 1/10/2003
  int animMovetype;  // clients can't derive movetype of other clients for anim
                     // scripting system
} entityState_t;
