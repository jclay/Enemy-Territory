...
void TossClientItems(gentity_t* self) {
  ...
  drop->item->quantity =
  self->client->ps.ammoclip[BG_FindClipForWeapon(weapon)];*/
...
char* modNames[] = {
    ...
    "MOD_KNIFE", "MOD_LUGER", "MOD_COLT", "MOD_MP40", "MOD_THOMPSON",
    "MOD_STEN", "MOD_GARAND", "MOD_SNOOPERSCOPE",
    "MOD_SILENCER",  //----(SA)
    "MOD_FG42", "MOD_FG42SCOPE", "MOD_PANZERFAUST", "MOD_GRENADE_LAUNCHER",
    "MOD_FLAMETHROWER", "MOD_GRENADE_PINEAPPLE", "MOD_CROSS",
    // end

    "MOD_MAPMORTAR", "MOD_MAPMORTAR_SPLASH",

    "MOD_KICKED", "MOD_GRABBER",
    ...
    "MOD_FALLING", "MOD_SUICIDE", "MOD_TARGET_LASER", "MOD_TRIGGER_HURT",
    "MOD_EXPLOSIVE",

    "MOD_CARBINE", "MOD_KAR98", "MOD_GPG40", "MOD_M7", "MOD_LANDMINE",
    "MOD_SATCHEL", "MOD_TRIPMINE", "MOD_SMOKEBOMB", "MOD_MOBILE_MG42",
    "MOD_SILENCED_COLT", "MOD_GARAND_SCOPE",

    "MOD_CRUSH_CONSTRUCTION", "MOD_CRUSH_CONSTRUCTIONDEATH",
    "MOD_CRUSH_CONSTRUCTIONDEATH_NOATTACKER",

    "MOD_K43", "MOD_K43_SCOPE",

    "MOD_MORTAR",

    "MOD_AKIMBO_COLT", "MOD_AKIMBO_LUGER", "MOD_AKIMBO_SILENCEDCOLT",
...
void player_die(gentity_t* self,
  ...
  // RF, record this death in AAS system so that bots avoid areas which have
  // high death rates
  ...
  if (self->client && meansOfDeath == MOD_LANDMINE) {
    ...
    teamList = self->client->sess.sessionTeam == TEAM_AXIS ? &mapEntityData[0] :
    &mapEntityData[1]; if((mEnt = G_FindMapEntityData(teamList,
    inflictor-g_entities)) != NULL) { G_FreeMapEntityData( teamList, mEnt );
    }

    if( teamNum != self->client->sess.sessionTeam ) {
            teamList = self->client->sess.sessionTeam == TEAM_AXIS ?
    &mapEntityData[1] : &mapEntityData[0]; if((mEnt =
    G_FindMapEntityData(teamList, inflictor-g_entities)) != NULL) {
...

  {
    mapEntityData_t* mEnt;
    mapEntityData_Team_t* teamList =
        self->client->sess.sessionTeam == TEAM_AXIS
            ? &mapEntityData[1]
    ...
    while (mEnt) {
      if (mEnt->type == ME_PLAYER_DISGUISED) {
        ...
        mEnt =
            G_FindMapEntityDataSingleClient(teamList, mEnt, self->s.number, -1);
  ...
  if (attacker) {
    ...
    killerName =
        (attacker->client) ? attacker->client->pers.netname : "<non-client>";
  }
    ...
    if (attacker == self || OnSameTeam(self, attacker)) {
      ...
      if (attacker != self && level.warmupTime <= 0 &&
        ...
        if (attacker->client->pers.localClient) {
          ...
          if (meansOfDeath != MOD_CRUSH_CONSTRUCTION &&
            ...
            if (g_complaintlimit.integer) {
              if (!(meansOfDeath == MOD_LANDMINE &&
                    g_disableComplaints.integer & TKFL_MINES) &&
                  !((meansOfDeath == MOD_ARTY ||
                     meansOfDeath == MOD_AIRSTRIKE) &&
                    g_disableComplaints.integer & TKFL_AIRSTRIKE) &&
                  !(meansOfDeath == MOD_MORTAR &&
                    g_disableComplaints.integer & TKFL_MORTAR)) {
                trap_SendServerCommand(self - g_entities,
                                       va("complaint %i", attacker->s.number));
      ...
      // JPW NERVE -- mostly added as conveneience so we can tweak from the
      // #defines all in one place
  ...
  if (item) {
    ...
    gentity_t* flag =
        LaunchItem(item, self->r.currentOrigin, launchvel, self->s.number);

    flag->s.modelindex2 =
        self->s.otherEntityNum2;  // JPW NERVE FIXME set player->otherentitynum2
  ...
  self->client->limboDropWeapon =
      self->s.weapon;  // store this so it can be dropped in limbo
  ...
  self->r.maxs[2] =
      self->client->ps.crouchMaxZ;  //%	0;			// ydnar: so
                                    //bodies don't clip into world
  self->client->ps.maxs[2] =
      self->client->ps.crouchMaxZ;  //%	0;	// ydnar: so bodies don't clip
                                    //into world
  ...
  if (nogib) {
    ...
    /*		if( self->client->ps.groundEntityNum == ENTITYNUM_NONE ) {
                            ...
                            BG_AnimScriptAnimation( &self->client->ps,
       ANIM_MT_FLAILING, qtrue );
                            ...
                            // Face explosion directory
                            {
                                    ...
                                    vectoangles( self->client->ps.velocity,
       angles ); self->client->ps.viewangles[YAW] = angles[YAW];
                                    SetClientViewAngle( self,
       self->client->ps.viewangles );
                            }
                    } else*/

    // DHM - Play death animation, and set pm_time to delay 'fallen' animation
    // if( G_IsSinglePlayerGame() && self->client->sess.sessionTeam ==
    // TEAM_ALLIES ) {
    ...
    //self->client->pers.character->animModelInfo, ANIM_ET_FALLDOWN, qfalse,
    //qtrue ); 	G_StartPlayerAppropriateSound(self, "death"); } else {
    self->client->ps.pm_time = BG_AnimScriptEvent(
        &self->client->ps, self->client->pers.character->animModelInfo,
...
gentity_t* G_BuildHead(gentity_t* ent) {
  ...
  if (trap_GetTag(ent->s.number, 0, "tag_head", & or)) {
    ...
    } else if (ent->client->ps.pm_flags &
               PMF_DUCKED) {  // closer fake offset for 'head' box when
                              // crouching
  ...
  VectorSet(head->r.mins, -6, -6,
            -2);  // JPW NERVE changed this z from -12 to -6 for crouching, also
                  // removed standing offset
...
void G_Damage(gentity_t* targ,
  ...
  if (level.intermissionQueued ||
      (g_gamestate.integer != GS_PLAYING && match_warmupDamage.integer == 0)) {
    return;
  }
  ...
  if (targ->client && attacker && attacker->client && attacker != targ) {
    ...
    if( targ->client->combatState < COMBATSTATE_HOT ) {
            ...
            VectorSubtract( targ->r.currentOrigin, attacker->r.currentOrigin,
    shotvec ); dist = VectorLengthSquared( shotvec );

            if( dist < Square(1500.f) && targ->client->combatState ==
    COMBATSTATE_WARM ) targ->client->combatState = COMBATSTATE_HOT;
    }
    ...
    attacker->r.currentOrigin, shotvec ); dist = VectorLengthSquared( shotvec );
            }
  ...
  // TAT 11/22/2002
  //		In the old code, this check wasn't done for props, so I put that
  //check back in to make props_statue properly work
  ...
  if (targ->s.eType == ET_MOVER && (targ->spawnflags & 4) && !targ->isProp) {
    ...
    // 512 Satchel only
    if ((targ->spawnflags & 32) || (targ->spawnflags & 64) || (targ->spawnflags
    & 256) || (targ->spawnflags & 512))
    {
            switch (mod) {
            case MOD_GRENADE:
            case MOD_GRENADE_LAUNCHER:
    ...
    // check for team
    //		if( G_GetWeaponClassForMOD( mod ) != -1 && targ->s.teamNum ==
    //inflictor->s.teamNum ) { 			return;
    //		}
    if (G_GetTeamFromEntity(inflictor) == G_GetTeamFromEntity(targ)) {
      return;
    }
    ...
    if (G_GetWeaponClassForMOD(mod) < targ->constructibleStats.weaponclass) {
      return;
             ...
             targ->methodOfDeath == MOD_LANDMINE) {
    if (targ->s.modelindex2) {
      if (G_WeaponIsExplosive(mod)) {
        mapEntityData_t* mEnt;

        if ((mEnt = G_FindMapEntityData(&mapEntityData[0],
                                        targ - g_entities)) != NULL) {
          G_FreeMapEntityData(&mapEntityData[0], mEnt);
        }
  ...
  if (client && (client->ps.weapon == WP_MORTAR_SET ||
  ...
  if (knockback && targ->client) {
    ...
    /*if( mod == MOD_GRENADE ||
            ...
            mod == MOD_M7 ||
            mod == MOD_LANDMINE ) {
            targ->client->ps.velocity[2] *= 2.f;
    // gimme air baby! targ->client->ps.groundEntityNum = ENTITYNUM_NONE;
    // flying high! } else if( mod == MOD_ROCKET ) {
            targ->client->ps.velocity[2] *= .75f;
    // but not to the moon please! targ->client->ps.groundEntityNum =
    ENTITYNUM_NONE;	// flying high!
    }*/

    if (targ == attacker &&
        !(mod != MOD_ROCKET && mod != MOD_GRENADE &&
  ...
  if (!(dflags & DAMAGE_NO_PROTECTION)) {
    ...
    if (targ != attacker && OnSameTeam(targ, attacker)) {
      if ((g_gamestate.integer != GS_PLAYING &&
           match_warmupDamage.integer == 1)) {
        return;
      } else if (!g_friendlyFire.integer) {
        // record "fake" pain - although the bot is not really hurt, his feeling
        // has been hurt :-) well at least he wants to shout "watch your fire".
  ...
  if (targ->client &&
    ...
    if (mod == MOD_GRENADE || mod == MOD_GRENADE_LAUNCHER ||
        mod == MOD_ROCKET || mod == MOD_GRENADE_PINEAPPLE ||
        mod == MOD_MAPMORTAR || mod == MOD_MAPMORTAR_SPLASH ||
        mod == MOD_EXPLOSIVE || mod == MOD_LANDMINE || mod == MOD_GPG40 ||
        mod == MOD_M7 || mod == MOD_SATCHEL || mod == MOD_ARTY ||
        mod == MOD_AIRSTRIKE || mod == MOD_DYNAMITE || mod == MOD_MORTAR ||
        mod == MOD_PANZERFAUST || mod == MOD_MAPMORTAR) {
      take -= take * .5f;
    }
  }
  ...
  if (headShot) {
    if (take * 2 <
        50) {  // head shots, all weapons, do minimum 50 points damage
      take = 50;
    } else {
    ...
    if (!(targ->client->ps.eFlags &
          EF_HEADSHOT)) {  // only toss hat on first headshot
  ...
  if (take) {
    ...
    // Arnout: attacker == inflictor can happen in other cases as well! (movers
    // trying to gib things)
    ...
    //		if ((take > 190)) // 190 is greater than 2x mauser headshot, so
    //headshots don't gib
    // Arnout: only player entities! messes up ents like func_constructibles and
    // func_explosives otherwise
    if (((targ->s.number < MAX_CLIENTS) && (take > 190)) &&
        !(targ->r.svFlags & SVF_POW)) {
      targ->health = GIB_HEALTH - 1;
    }
    ...
    if (targ->s.eType == ET_MOVER &&
      ...
      targ->s.dl_intensity =
          255.f * (targ->health / (float)targ->count);  // send it to the client
    }
    ...
    if (targ->health <= 0) {
      if (client && !wasAlive) {
        ...
        if (targ->die) {
          ...
          // function pointer, so that next time die is applied to the dead
          // body.
        ...
        if (targ->s.eType == ET_MOVER &&
            !Q_stricmp(targ->classname, "script_mover") &&
            (targ->spawnflags & 8)) {
          return;  // reseructable script mover doesn't unlink itself but we
                   // don't want a second death script to be called
        }
          ...
          if (targ->r.svFlags & SVF_BOT) {
            ...
            if (wasAlive) {
              Bot_ScriptEvent(targ->s.number, "death", "");
                      ...
                      targ->s.eType != ET_EXPLOSIVE) ||
                     (targ->s.eType == ET_CONSTRUCTIBLE &&
                      !targ->desstages)) {  // call manually if using desstages
    ...
    // RF, entity scripting
    G_Script_ScriptEvent(targ, "pain",
                         va("%d %d", targ->health, targ->health + take));
    if (targ->s.number < MAX_CLIENTS && (targ->r.svFlags & SVF_BOT)) {
      Bot_ScriptEvent(targ->s.number, "pain",
                      va("%d %d", targ->health, targ->health + take));
    }
    ...
    // Ridah, this needs to be done last, incase the health is altered in one of
    // the event calls
    if (targ->client) {
...
qboolean CanDamage(gentity_t* targ, vec3_t origin) {
  ...
  if (targ->r.currentOrigin[0] || targ->r.currentOrigin[1] ||
      targ->r.currentOrigin[2]) {
...
void G_AdjustedDamageVec(gentity_t* ent, vec3_t origin, vec3_t v) {
  ...
  if (!ent->r.bmodel) {
    VectorSubtract(ent->r.currentOrigin, origin,
                   v);  // JPW NERVE simpler centroid check that doesn't have
                        // box alignment weirdness
  } else {
    for (i = 0; i < 3; i++) {
...
============
etpro_RadiusDamage
mutation of G_RadiusDamage which lets us selectively damage only clients or only
non clients
============
*/
