...
int G_FindFreeFireteamIdent(team_t team) {
  ...
  for (i = 0; i < MAX_FIRETEAMS; i++) {
    ...
    if (g_entities[(int)level.fireTeams[i].joinOrder[0]]
...
void G_RegisterFireteam(/*const char* name,*/ int entityNum) {
  ...
  if (!leader->client) {
    G_Error(
        "G_RegisterFireteam: attempting to register a Fireteam to an entity "
        "with no client\n");
  }
  ...
  if (leader->client->sess.sessionTeam != TEAM_AXIS &&
      leader->client->sess.sessionTeam != TEAM_ALLIES) {
    G_ClientPrintAndReturn(entityNum,
                           "Only players on a team can create a fireteam");
  }
  ...
  if (count >= MAX_FIRETEAMS / 2) {
    G_ClientPrintAndReturn(
        entityNum,
        "Your team already has the maximum number of fireteams allowed");
  }
...
void G_AddClientToFireteam(int entityNum, int leaderNum) {
  ...
  if (g_entities[leaderNum].client->sess.sessionTeam !=
      g_entities[entityNum].client->sess.sessionTeam) {
    G_ClientPrintAndReturn(entityNum,
                           "You are not on the same team as that fireteam");
  }
...
void G_InviteToFireTeam(int entityNum, int otherEntityNum) {
  ...
  if (g_entities[entityNum].client->sess.sessionTeam !=
      g_entities[otherEntityNum].client->sess.sessionTeam) {
    G_ClientPrintAndReturn(entityNum,
                           "You are not on the same team as the other player");
  }
...
void Cmd_FireTeam_MP_f(gentity_t* ent) {
  ...
  if (trap_Argc() < 2) {
    G_ClientPrintAndReturn(ent - g_entities,
                           "usage: fireteam <create|leave|apply|invite>");
  }
  ...
  if (!Q_stricmp(command, "create")) {
    ...
    if (trap_Argc() < 3) {
      G_ClientPrintAndReturn(
          ent - g_entities,
          "usage: fireteam apply <fireteamname|fireteamnumber>");
    }
    ...
    if (trap_Argc() < 3) {
      G_ClientPrintAndReturn(
          ent - g_entities, "usage: fireteam invite <clientname|clientnumber>");
    }
    ...
    if (trap_Argc() < 3) {
      G_ClientPrintAndReturn(
          ent - g_entities,
          "usage: fireteam propose <clientname|clientnumber>");
    }
