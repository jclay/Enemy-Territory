...
#define FL_TOGGLE \
  0x00020000  //----(SA)	ent is toggling (doors use this for ex.)
...
struct gentity_s {
  ...
  vec3_t instantVelocity;  // ydnar: per entity instantaneous velocity, set per
                           // frame
  ...
  int realClipmask;  // Arnout: use these to backup the contents value when we
                     // go to state under construction
  int realContents;
  ...
  float speed;
  float
      closespeed;  // for movers that close at a different speed than they open
  vec3_t movedir;
  ...
  int grenadeExplodeTime;  // we've caught a grenade, which was due to explode
                           // at this time
  ...
  int numScriptEvents;
  g_script_event_t* scriptEvents;  // contains a list of actions to perform for
                                   // each event type
  ...
  // TAT 10/13/2002 - for seek cover sequence - we need a pointer to a server
  // entity
...
typedef struct {
  ...
  int spawnObjectiveIndex;  // JPW NERVE index of objective to spawn nearest to
                            // (returned from UI)
  ...
  float startskillpoints[SK_NUM_SKILLS];  // Gordon: initial skillpoints at map
                                          // beginning
  float startxptotal;
  ...
  weapon_stat_t aWeaponStats[WS_MAX + 1];  // Weapon stats.  +1 to avoid invalid
                                           // weapon check
  // OSP
...
typedef struct {
  ...
  int autoActivate;  // based on cg_autoactivate userinfo		(uses
                     // the PICKUP_ values above)
  ...
  int complaints;  // DHM - Nerve :: number of complaints lodged against this
                   // client
  int complaintClient;   // DHM - Nerve :: able to lodge complaint against this
                         // client
  ...
  int applicationClient;   // Gordon: this client has requested to join your
                           // fireteam
  int applicationEndTime;  // Gordon: you have X seconds to reply or this
                           // message will self destruct!

  int invitationClient;   // Gordon: you have been invited to join this client's
                          // fireteam
  int invitationEndTime;  // Gordon: quickly now, chop chop!.....

  int propositionClient;  // Gordon: propositionClient2 has requested you invite
                          // this client to join the fireteam
  ...
  int mvReferenceList;  // Reference list used to generate views after a
                        // map_restart
  int mvScoreUpdate;    // Period to send score info to MV clients
  int panzerDropTime;   // Time which a player dropping panzer still "has it" if
                        // limiting panzer counts
...
struct gclient_s {
  ...
  //	int			accurateCount;		// for "impressive"
  //reward sound
  ...
  int PCSpecialPickedUpCount;  // JPW NERVE used to count # of times somebody's
                               // picked up this LTs ammo (or medic health) (for
                               // scoring)
  int saved_persistant[MAX_PERSISTANT];  // DHM - Nerve :: Save ps->persistant
                                         // here during Limbo

  gentity_t* touchingTOI;  // Arnout: the trigger_objective_info a player is
                           // touching this frame
  ...
  gentity_t* tempHead;  // Gordon: storing a temporary head for bullet head shot
                        // detection
  gentity_t* tempLeg;   // Arnout: storing a temporary leg for bullet head shot
                        // detection

  int botSlotNumber;  // the slot the bot falls into (set up in the initial UI
                      // screen)
  // END		xkan, 8/27/2002
...
typedef struct {
  ...
  int reloadDelayTime;  // don't start loading the savegame until this has
                        // expired
  ...
  int numFinalDead[2];  // DHM - Nerve :: unable to respawn and in limbo (per
                        // team)
  ...
  qboolean doorAllowTeams;  // used by bots to decide whether or not to use team
                            // travel flags
  ...
  int timeDelta;  // Offset from internal clock - used to calculate real match
                  // time
  // OSP
...
qboolean G_SpawnStringExt(
    const char* key,
    const char* defaultString,
    char** out,
    const char* file,
    int line);  // spawn string returns a temporary reference, you must
                // CopyString() if you want to keep it
qboolean G_SpawnFloatExt(const char* key,
                         const char* defaultString,
                         float* out,
                         const char* file,
                         int line);
qboolean G_SpawnIntExt(const char* key,
                       const char* defaultString,
                       int* out,
                       const char* file,
                       int line);
qboolean G_SpawnVectorExt(const char* key,
                          const char* defaultString,
                          float* out,
                          const char* file,
                          int line);
qboolean G_SpawnVector2DExt(const char* key,
                            const char* defaultString,
                            float* out,
                            const char* file,
                            int line);
...
void G_TeamDataForString(const char* teamstr,
                         int clientNum,
                         team_t* team,
                         spectatorState_t* sState,
                         int* specClient);
qboolean SetTeam(gentity_t* ent,
                 char* s,
                 qboolean force,
                 weapon_t w1,
                 weapon_t w2,
                 qboolean setweapons);
void G_SetClientWeapons(gentity_t* ent,
                        weapon_t w1,
                        weapon_t w2,
...
//
// Ridah
int G_FindConfigstringIndex(const char* name,
                            int start,
                            int max,
                            qboolean create);
// done.
...
qboolean G_RadiusDamage(vec3_t origin,
                        ...
                        gentity_t* ignore,
                        int mod);
qboolean etpro_RadiusDamage(vec3_t origin,
                            gentity_t* inflictor,
                            gentity_t* attacker,
                            float damage,
                            float radius,
...
int G_PredictMissile(gentity_t* ent,
                     int duration,
                     vec3_t endPos,
...
qboolean G_CheckForExistingModelInfo(bg_playerclass_t* classInfo,
...
// g_cmds.c
//
void G_SayTo(gentity_t* ent,
             gentity_t* other,
             int mode,
             int color,
             const char* name,
             const char* message,
             qboolean localize);  // JPW NERVE removed static declaration so it
                                  // would link
qboolean Cmd_CallVote_f(gentity_t* ent,
...
// are constructing
qboolean ReadyToConstruct(gentity_t* ent,
...
int Bot_GetWeaponForClassAndTeam(int classNum,
                                 int teamNum,
...
// ai_main.c
// TTimo - wraps to BotGetTargetExplosives, without dependency to bot_target_s
// (which breaks gcc build)
...
extern vmCvar_t
    g_maxlives;  // DHM - Nerve :: number of respawns allowed (0==infinite)
...
extern vmCvar_t
    g_enforcemaxlives;  // Xian - Temp ban with maxlives between rounds
...
void trap_TraceCapsule(trace_t* results,
                       ...
                       int passEntityNum,
                       int contentmask);
void trap_TraceCapsuleNoEnts(trace_t* results,
                             const vec3_t start,
                             const vec3_t mins,
                             const vec3_t maxs,
                             const vec3_t end,
                             int passEntityNum,
                             int contentmask);
void trap_TraceNoEnts(trace_t* results,
                      const vec3_t start,
                      const vec3_t mins,
                      const vec3_t maxs,
                      const vec3_t end,
...
qboolean trap_GetTag(int clientNum,
                     int tagFileNumber,
                     char* tagName,
                     orientation_t* or);
...
int trap_AAS_NearestHideArea(int srcnum,
                             vec3_t origin,
                             int areanum,
                             int enemynum,
                             vec3_t enemyorigin,
                             ...
                             float maxdist,
                             vec3_t distpos);
int trap_AAS_ListAreasInRange(vec3_t srcpos,
                              int srcarea,
                              float range,
                              int travelflags,
                              float** outareas,
                              int maxareas);
int trap_AAS_AvoidDangerArea(vec3_t srcpos,
                             int srcarea,
                             vec3_t dangerpos,
                             int dangerarea,
                             float range,
...
void trap_AAS_RecordTeamDeathArea(vec3_t srcpos,
                                  int srcarea,
                                  int team,
                                  int teamCount,
                                  int travelflags);
...
void G_HistoricalTrace(gentity_t* ent,
                       trace_t* results,
                       const vec3_t start,
                       const vec3_t mins,
                       const vec3_t maxs,
...
qboolean G_VisibleFromBinoculars(gentity_t* viewer,
                                 gentity_t* ent,
                                 vec3_t origin);
...
void G_AddKillSkillPoints(gentity_t* attacker,
                          meansOfDeath_t mod,
                          hitRegion_t hr,
                          qboolean splash);
void G_AddKillSkillPointsForDestruction(
    ...
    meansOfDeath_t mod,
    g_constructible_stats_t* constructibleStats);
void G_LoseKillSkillPoints(gentity_t* tker,
                           meansOfDeath_t mod,
                           hitRegion_t hr,
                           qboolean splash);
...
void G_DebugAddSkillPoints(gentity_t* ent,
                           skillType_t skill,
                           float points,
                           const char* reason);
...
void G_RemoveClientFromFireteams(int entityNum,
                                 qboolean update,
                                 qboolean print);
...
struct g_serverEntity_s {
  ...
  int areaNum;    // This thing doesn't move, so we should only need to calc the
                  // areanum once
  int botIgnoreTime;  // So that multiple bots don't use the same seek cover
                      // spot

  void (*setup)(g_serverEntity_t* self);  // Setup function called once after
                                          // all server objects are created
};
...
g_serverEntity_t* FindServerEntity(g_serverEntity_t* from,
                                   int fieldofs,
                                   char* match);
...
qboolean G_commandHelp(gentity_t* ent,
                       char* pszCommand,
                       unsigned int dwCommand);
...
void G_weaponRankings_cmd(gentity_t* ent,
                          unsigned int dwCommand,
                          qboolean state);
void G_weaponStats_cmd(gentity_t* ent, unsigned int dwCommand, qboolean fDump);
void G_weaponStatsLeaders_cmd(gentity_t* ent,
                              qboolean doTop,
                              qboolean doWindow);
void G_VoiceTo(gentity_t* ent,
               gentity_t* other,
               int mode,
               const char* id,
               qboolean voiceonly);
...
int G_Comp_v(gentity_t* ent,
             unsigned int dwVoteIndex,
             char* arg,
             char* arg2,
             qboolean fRefereeCmd);
int G_Gametype_v(gentity_t* ent,
                 unsigned int dwVoteIndex,
                 char* arg,
                 char* arg2,
                 qboolean fRefereeCmd);
int G_Kick_v(gentity_t* ent,
             unsigned int dwVoteIndex,
             char* arg,
             char* arg2,
             qboolean fRefereeCmd);
int G_Mute_v(gentity_t* ent,
             unsigned int dwVoteIndex,
             char* arg,
             char* arg2,
             qboolean fRefereeCmd);
int G_UnMute_v(gentity_t* ent,
               unsigned int dwVoteIndex,
               char* arg,
               char* arg2,
               qboolean fRefereeCmd);
int G_Map_v(gentity_t* ent,
            unsigned int dwVoteIndex,
            char* arg,
            char* arg2,
            qboolean fRefereeCmd);
int G_Campaign_v(gentity_t* ent,
                 unsigned int dwVoteIndex,
                 char* arg,
                 char* arg2,
                 qboolean fRefereeCmd);
int G_MapRestart_v(gentity_t* ent,
                   unsigned int dwVoteIndex,
                   char* arg,
                   char* arg2,
                   qboolean fRefereeCmd);
int G_MatchReset_v(gentity_t* ent,
                   unsigned int dwVoteIndex,
                   char* arg,
                   char* arg2,
                   qboolean fRefereeCmd);
int G_Mutespecs_v(gentity_t* ent,
                  unsigned int dwVoteIndex,
                  char* arg,
                  char* arg2,
                  qboolean fRefereeCmd);
int G_Nextmap_v(gentity_t* ent,
                unsigned int dwVoteIndex,
                char* arg,
                char* arg2,
                qboolean fRefereeCmd);
int G_Pub_v(gentity_t* ent,
            unsigned int dwVoteIndex,
            char* arg,
            char* arg2,
            qboolean fRefereeCmd);
int G_Referee_v(gentity_t* ent,
                unsigned int dwVoteIndex,
                char* arg,
                char* arg2,
                qboolean fRefereeCmd);
int G_ShuffleTeams_v(gentity_t* ent,
                     unsigned int dwVoteIndex,
                     char* arg,
                     char* arg2,
                     qboolean fRefereeCmd);
int G_StartMatch_v(gentity_t* ent,
                   unsigned int dwVoteIndex,
                   char* arg,
                   char* arg2,
                   qboolean fRefereeCmd);
int G_SwapTeams_v(gentity_t* ent,
                  unsigned int dwVoteIndex,
                  char* arg,
                  char* arg2,
                  qboolean fRefereeCmd);
int G_FriendlyFire_v(gentity_t* ent,
                     unsigned int dwVoteIndex,
                     char* arg,
                     char* arg2,
                     qboolean fRefereeCmd);
int G_Timelimit_v(gentity_t* ent,
                  unsigned int dwVoteIndex,
                  char* arg,
                  char* arg2,
                  qboolean fRefereeCmd);
int G_Warmupfire_v(gentity_t* ent,
                   unsigned int dwVoteIndex,
                   char* arg,
                   char* arg2,
                   qboolean fRefereeCmd);
int G_Unreferee_v(gentity_t* ent,
                  unsigned int dwVoteIndex,
                  char* arg,
                  char* arg2,
                  qboolean fRefereeCmd);
int G_AntiLag_v(gentity_t* ent,
                unsigned int dwVoteIndex,
                char* arg,
                char* arg2,
                qboolean fRefereeCmd);
int G_BalancedTeams_v(gentity_t* ent,
                      unsigned int dwVoteIndex,
                      char* arg,
                      char* arg2,
                      qboolean fRefereeCmd);
...
void G_KillEnts(const char* target,
                gentity_t* ignore,
                gentity_t* killer,
                meansOfDeath_t mod);
