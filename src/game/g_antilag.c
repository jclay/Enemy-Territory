...
static void G_AdjustSingleClientPosition(gentity_t* ent, int time) {
  ...
  if (i != ent->client->topMarker) {
    float frac = (float)(time - ent->client->clientMarkers[i].time) /
                 (float)(ent->client->clientMarkers[j].time -
                         ent->client->clientMarkers[i].time);

    LerpPosition(ent->client->clientMarkers[i].origin,
                 ent->client->clientMarkers[j].origin, frac,
                 ent->r.currentOrigin);
    LerpPosition(ent->client->clientMarkers[i].mins,
                 ent->client->clientMarkers[j].mins, frac, ent->r.mins);
    LerpPosition(ent->client->clientMarkers[i].maxs,
                 ent->client->clientMarkers[j].maxs, frac, ent->r.maxs);
  } else {
...
void G_AdjustClientPositions(gentity_t* ent, int time, qboolean forward) {
  ...
  for (i = 0; i < level.numConnectedClients; i++, list++) {
    ...
    if (list->client && list->inuse &&
        (list->client->sess.sessionTeam == TEAM_AXIS ||
         list->client->sess.sessionTeam == TEAM_ALLIES) &&
        (list != ent) && list->r.linked && (list->health > 0) &&
...
void G_ResetMarkers(gentity_t* ent) {
  ...
  for (i = MAX_CLIENT_MARKERS - 1, time = level.time; i >= 0;
       i--, time -= period) {
