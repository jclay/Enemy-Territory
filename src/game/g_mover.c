...
gentity_t* G_TestEntityPosition(gentity_t* ent) {
  ...
  if (ent->clipmask) {
    //		if ( ent->r.contents == CONTENTS_CORPSE && ent->health <= 0 ) {
    //// Arnout: players waiting to be revived are important 		if (
  ...
  if (ent->client) {
    ...
    if (!tr.startsolid && ent->client->ps.eFlags & EF_PRONE) {
      ...
      // VectorSet( point, org[0], org[1], org[2] - 9.6f - 24.f );	// diff
      // between playerlegsMins and playerlegsMaxs z + 24 units to play with
      VectorSet(point, org[0], org[1],
                org[2] - (24.f - 2.4f) -
                    24.f);  // diff between playerlegsMins and playerlegsMaxs z
                            // + 24 units to play with

      trap_TraceCapsule(&tr, org, playerlegsProneMins, playerlegsProneMaxs,
                        point, ent->s.number, mask);
    ...
    pos[2] += 4;  // move up a bit - corpses normally got their origin slightly
                  // in the ground
    ...
    trap_Trace(&tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs,
               ent->s.pos.trBase, ent->r.ownerNum, mask);
  } else {
    trap_Trace(&tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs,
...
void G_TestEntityDropToFloor(gentity_t* ent, float maxdrop) {
  ...
  if (ent->client) {
    trap_TraceCapsule(&tr, ent->client->ps.origin, ent->r.mins, ent->r.maxs,
                      endpos, ent->s.number, mask);
  } else {
    trap_Trace(&tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, endpos,
               ent->s.number, mask);
  }
...
void G_TestEntityMoveTowardsPos(gentity_t* ent, vec3_t pos) {
  ...
  if (ent->client) {
    trap_TraceCapsule(&tr, ent->client->ps.origin, ent->r.mins, ent->r.maxs,
                      pos, ent->s.number, mask);
  } else {
    trap_Trace(&tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, pos,
               ent->s.number, mask);
  }
...
#define JITTER_MAX (check->r.maxs[0] / 2.0)
  ...
  if (check->client) {
    ...
    if ((check->client->ps.eFlags & EF_PRONE) ||
        check->s.weapon == WP_MORTAR_SET) {
  ...
  if (block->client) {
    pushedStackDepth++;
    if (pushedStackDepth < PUSH_STACK_DEPTH &&
        G_TryPushingEntity(block, pusher, move, amove)) {
      // pushed ok
      if (check->client) {
        VectorCopy(check->client->ps.origin, check->r.currentOrigin);
      } else {
  ...
  *obstacle = NULL;
  ...
  for (e = 0; e < listedEntities; e++) {
    ...
    if (check->s.eType == ET_PLAYER && check->client &&
        ((check->client->ps.eFlags & EF_TAGCONNECT) ||
         (check->client->ps.pm_type == PM_NOCLIP))) {
      continue;
    }

    // if ( check->s.eType == ET_MISSILE && VectorLengthSquared(
    // check->s.pos.trDelta ) ) { 	continue;	// it's moving
    // }
...
void G_MoverTeam(gentity_t* ent) {
  ...
  for (part = ent; part; part = part->teamchain) {
    ...
    BG_EvaluateTrajectory(&part->s.pos, level.time, origin, qfalse,
                          ent->s.effect2Time);
    BG_EvaluateTrajectory(&part->s.apos, level.time, angles, qtrue,
                          ent->s.effect2Time);
  ...
  if (part) {
    ...
    for (part = ent; part; part = part->teamchain) {
      ...
      BG_EvaluateTrajectory(&part->s.pos, level.time, part->r.currentOrigin,
                            qfalse, ent->s.effect2Time);
      BG_EvaluateTrajectory(&part->s.apos, level.time, part->r.currentAngles,
                            qtrue, ent->s.effect2Time);
...
void SetMoverState(gentity_t* ent, moverState_t moverState, int time) {
  ...
  switch (moverState) {
    ...
    case MOVER_2TO1:  // closing
      ...
      if (ent->closespeed) {  //----(SA)	handle doors with different
                              //close speeds
    ...
    case MOVER_1TO2ROTATE:              // opening
      ...
      if (kicked) {
        ...
        //ent->gDurationBack;	// (SA) durationback?
    ...
    case MOVER_2TO1ROTATE:  // closing
      VectorScale(ent->rotate, ent->angle,
                  ent->s.apos.trBase);  // set base to end position
  ...
  BG_EvaluateTrajectory(&ent->s.pos, level.time, ent->r.currentOrigin, qfalse,
                        ent->s.effect2Time);
  //	if (!(ent->r.svFlags & SVF_NOCLIENT) || (ent->r.contents))	// RF,
  //added this for bats, but this is safe for all movers, since if they aren't
  //solid, and aren't visible to the client, they don't need to be linked
  //		trap_LinkEntity( ent );

  if (!(ent->r.svFlags & SVF_NOCLIENT) ||
      (ent->r
           .contents)) {  // RF, added this for bats, but this is safe for all
                          // movers, since if they aren't solid, and aren't
                          // visible to the client, they don't need to be linked
    trap_LinkEntity(ent);
    ...
    } else if (ent->key || ent->allowteams) {  // doors should only block when
                                               // locked and at POS1
      switch (moverState) {
        ...
        case MOVER_POS1:
          ...
          if (ent->allowteams) {
            ...
            if (!blockflags) {
              if (g_cheats.integer) {
                G_Printf(
                    "WARNING: %s at %s has allowteams flag, but no spawnflags "
                    "to define which team is allowed\n",
...
*/
void MatchTeamReverseAngleOnSlaves(gentity_t* teamLeader,
                                   int moverState,
                                   int time) {
  gentity_t* slave;
...
void Reached_BinaryMover(gentity_t* ent) {
  ...
  //	ent->flags &= ~(FL_KICKACTIVATE|FL_SOFTACTIVATE);	// (SA) it was
  //not opened normally.  Clear this so it thinks it's closed normally
  ent->flags &= ~FL_KICKACTIVATE;  // (SA) it was not opened normally.  Clear
                                   // this so it thinks it's closed normally
}
...
================
*/
qboolean IsBinaryMoverBlocked(gentity_t* ent,
                              gentity_t* other,
                              gentity_t* activator) {
  vec3_t dir, angles;
  vec3_t pos;
...
void Use_BinaryMover(gentity_t* ent, gentity_t* other, gentity_t* activator) {
  ...
  if (level.time <=
      4000) {  // hack.  don't play door sounds if in the first /four/ seconds
               // of game (FIXME: TODO: THIS IS STILL A HACK)
    nosound = qtrue;
  }
  ...
  if (isblocked) {
    ...
    if (!nosound) {
      if (ent->flags & FL_SOFTACTIVATE) {
        ...
        //				if(activator)
        //					AICast_AudibleEvent(
        //activator->s.number, ent->s.origin, HEAR_RANGE_DOOR_KICKOPEN );
        //// "someone kicked open a door near me!"
        G_AddEvent(ent, EV_GENERAL_SOUND, ent->sound1to2);
      }
    }
  ...
  // Rafael
  if (ent->nextTrain && ent->nextTrain->wait == -1 &&
      ent->nextTrain->count == 1) {
  ...
  if (ent->moverState == MOVER_POS1ROTATE) {
    ...
    if (!nosound) {
      if (ent->flags & FL_SOFTACTIVATE) {
        ...
        //				if(activator)
        //					AICast_AudibleEvent(
        //activator->s.number, ent->s.origin, HEAR_RANGE_DOOR_OPEN );	//
...
void InitMoverRotate(gentity_t* ent) {
  ...
  ent->gDuration = ent->gDurationBack =
      ent->s.apos.trDuration;  // (SA) store 'real' durations so doors can be
                               // opened/closed at different speeds
}
...
void Blocked_Door(gentity_t* ent, gentity_t* other) {
  ...
  for (slave = ent; slave; slave = slave->teamchain) {
    ...
    time = level.time -
           (slave->s.pos.trDuration - (level.time - slave->s.pos.trTime));
...
================
*/
static void Touch_DoorTriggerSpectator(gentity_t* ent,
                                       gentity_t* other,
                                       trace_t* trace) {
  int i, axis;
...
void Blocked_DoorRotate(gentity_t* ent, gentity_t* other) {
  ...
  for (slave = ent; slave; slave = slave->teamchain) {
    ...
    time = level.time -
           (slave->s.apos.trDuration - (level.time - slave->s.apos.trTime));
...
findNonAIBrushTargeter
        determine if there is an entity pointing at ent that is not a
"trigger_aidoor" (used now for checking which key to set for a door)
==============
*/
...
void finishSpawningKeyedMover(gentity_t* ent) {
  ...
  // all ents should be spawned, so it's okay to check for special door triggers
  // now
  ...
  if (ent->key == -2) {  // the key was not set in the spawn
    if (ent->targetname && findNonAIBrushTargeter(ent)) {
      ent->key = -1;  // something is targeting this (other than a
                      // trigger_aidoor) so leave locked
    } else {
      ent->key = 0;
    }
  }
  ...
  if (!(ent->flags & FL_TEAMSLAVE)) {
    ...
    // (SA) is this safe?  is ent->spawnflags & 8 consistant among all keyed
    // ents?
...
void DoorSetSounds(gentity_t* ent, int doortype, qboolean isRotating) {
  ent->sound1to2 = G_SoundIndex(
      va("sound/movers/doors/door%i_open.wav", doortype));  // opening
  ent->soundPos2 =
      G_SoundIndex(va("sound/movers/doors/door%i_endo.wav", doortype));  // open
  ent->sound2to1 = G_SoundIndex(
      va("sound/movers/doors/door%i_close.wav", doortype));  // closing
  ent->soundPos1 = G_SoundIndex(
      va("sound/movers/doors/door%i_endc.wav", doortype));  // closed
  ent->sound2to3 = G_SoundIndex(
      va("sound/movers/doors/door%i_loopo.wav", doortype));  // loopopen
  ent->sound3to2 = G_SoundIndex(
      va("sound/movers/doors/door%i_loopc.wav", doortype));  // loopclosed
  ent->soundPos3 = G_SoundIndex(
      va("sound/movers/doors/door%i_locked.wav", doortype));  // locked

  ent->soundSoftopen = G_SoundIndex(
      va("sound/movers/doors/door%i_openq.wav", doortype));  // opening quietly
  ent->soundSoftendo = G_SoundIndex(
      va("sound/movers/doors/door%i_endoq.wav", doortype));  // open quietly
  ent->soundSoftclose = G_SoundIndex(
      va("sound/movers/doors/door%i_closeq.wav", doortype));  // closing quietly
  ent->soundSoftendc = G_SoundIndex(
      va("sound/movers/doors/door%i_endcq.wav", doortype));  // closed quietly
}
...
==============
G_TryDoor
        seemed better to have this isolated.  this way i can get
func_invisible_user's using the regular rules of doors.
==============
*/
    ...
    if (ent->active == qfalse) {
      if (ent->key < 0 || !G_AllowTeamsAllowed(ent, activator) ||
          (G_IsSinglePlayerGame() &&
           ent->key == KEY_LOCKED_PICKABLE)) {  // door force locked
        //				if(!walking && activator)	// only
        //send audible event if not trying to open slowly 					AICast_AudibleEvent(
        //activator->s.clientNum, ent->s.origin, HEAR_RANGE_DOOR_LOCKED );
      ...
      */
      ...
      if (ent->teammaster && ent->team && ent != ent->teammaster) {
        ...
        if (walking) {
          ...
          //					if(activator)
          //						AICast_AudibleEvent(
          //activator->s.clientNum, ent->s.origin, HEAR_RANGE_DOOR_OPEN );
          //// "someone opened door near me!"
        }
...
"key"       -1 for locked, key number for which key opens, 0 for open.  default
'0' unless door is targeted.  (trigger_aidoor entities targeting this door do
/not/ affect the key status) "model2"    .md3 model to also draw "angle"
determines the opening direction "targetname" if set, no touch field will be
spawned and a remote button or trigger field activates the door. "speed"
movement speed (100 default) "closespeed" optional different movement speed for
door closing "wait"      wait before returning (3 default, -1 = never return)
...
syncronicity "lockpickTime" time to unpick the lock on a locked door - use with
key set to 99 "type"		use sounds based on construction of door: 0 -
nosound (default) 1 - metal 2 - stone 3 - lab 4 - wood 5 - iron/jail 6 -
portcullis 7 - wood (quiet)
...
void SP_func_door(gentity_t* ent) {
  ...
  if (doortype) {  // /*why on earthy did this check for <=8?*/ && doortype <=
                   // 8)	// no doortype = silent
  ...
  if (BotSinglePlayer() && (ent->key == 99)) {
    ...
    // TAT 1/29/2003 - also load how long it takes to pick the lock - default to
    // 30 seconds
...
determines the opening direction "targetname" if set, no touch field will be
spawned and a remote button or trigger field activates the door. "speed"
movement speed (100 default) "wait"      wait before returning (2 default, -1 =
never return) "lip"       lip remaining at end of move (8 default) "dmg" damage
to inflict when blocked (2 default) "color"     constantLight color "light"
constantLight radius "health"    if set, the door must be shot open
...
void SP_func_secret(gentity_t* ent) {
  ...
  ent->sound1to2 = ent->sound2to1 = ent->sound2to3 =
      G_SoundIndex("sound/movers/doors/dr1_strt.wav");
  ent->soundPos1 = ent->soundPos3 =
...
"count2" used only in conjunction with the truck_cam to control playing of gear
changes
*/
...
void SP_info_train_spline_main(gentity_t* self) {
  ...
  for (i = 1;; i++) {
    ...
    if (!G_SpawnString(i == 1 ? va("control") : va("control%i", i), "",
                       &control)) {
      break;
    }
...
Camera for limbo menu, target at an appropriate entity (spawn flag, script
mover, position marker...)
*/
...
void Static_Pain(gentity_t* ent,
                 gentity_t* attacker,
                 int damage,
                 vec3_t point) {
  vec3_t temp;
...
color "light"		constantLight radius "start_invis" will start the entity
as non-existant If targeted, it will toggle existance when triggered
...
STAYOPEN TAKE_KEY You need to have an origin brush as part of this entity.  The
center of that brush will be the point around which it is rotated. It will
rotate around the Z axis by default.  You can check either the X_AXIS or Y_AXIS
box to change that (only one axis allowed. If both X and Y are checked, the
default of Z will be used). FORCE		door opens even if blocked
...
ACCEPT_CVOPS Accept a disguised covert ops as a valid team member for team only
doors

"key"       -1 for locked, key number for which key opens, 0 for open.  default
'0' unless door is targeted.  (trigger_aidoor entities targeting this door do
/not/ affect the key status) "model2"    .md3 model to also draw "degrees"
determines how many degrees it will turn (90 default) "speed"	    movement
speed (100 default) "closespeed" optional different movement speed for door
closing "time"      how many milliseconds it will take to open 1 sec = 1000
"dmg"       damage to inflict when blocked (2 default)
"color"     constantLight color
"light"     constantLight radius
"lockpickTime" time to unpick the lock on a locked door - use with key set to 99
...
void SP_func_door_rotating(gentity_t* ent) {
  ...
  if (VectorLengthSquared(ent->rotate) >
      SQR(1)) {  // check that rotation is only set for one axis
    G_Error(
        "Too many axis marked in func_door_rotating entity.  Only choose one "
        "axis of rotation. (defaulting to standard door rotation)");
    VectorClear(ent->rotate);
    ent->rotate[1] = 1;
  }
...
EFFECTS
  ...
  I'm keeping all this stuff in here just to avoid collisions with Raf right now
in g_misc or g_props Will move.
===============================================================================
*/
...
void target_effect(gentity_t* self, gentity_t* other, gentity_t* activator) {
  ...
  trap_SetConfigstring(
      CS_TARGETEFFECT,
      self->dl_shader);  //----(SA)	allow shader to be set from entity

  // (SA) this should match the values from func_explosive
  tent->s.frame =
      self->key;  // pass the type to the client ("glass", "wood", "metal",
                  // "gibs", "brick", "stone", "fabric", 0, 1, 2, 3, 4, 5, 6)

  tent->s.eventParm = self->spawnflags;
  tent->s.density = self->health;

  if (self->damage) {
    G_RadiusDamage(self->s.pos.trBase, NULL, self, self->damage, self->damage,
                   self, MOD_EXPLOSIVE);
  }
...
/*QUAKED target_effect (0 .5 .8) (-6 -6 -6) (6 6 6) fire explode smoke rubble
gore lowgrav debris "mass" defaults to 15.  This determines how much debris is
emitted when it explodes.  (number of pieces) "dmg" defaults to 0.  damage
radius blast when triggered "type" - if 'rubble' is specified, this is the model
type ("glass", "wood", "metal", "gibs", "brick", "rock", "fabric") default is
"wood"
*/
void SP_target_effect(gentity_t* ent) {
  ...
  if (ent->dl_shader) {
    G_Printf(
        "^1Warning: This feature needs to be cleaned up from original wolf "
        "since you are using it, go poke Gordon about it\n");
  }
}
...
==============
BecomeExplosion
        nuke the original entity and create all the debris entities that need to
...
==============
*/
void func_explosive_explode(gentity_t* self,
                            gentity_t* inflictor,
                            gentity_t* attacker,
                            int damage,
                            int mod) {
  ...
  self->takedamage =
      qfalse;  // don't allow anything try to hurt me now that i'm exploding
  ...
  if (self->damage) {
    //		G_RadiusDamage(self->s.origin, self, self->damage,
    //self->damage+40, self, MOD_EXPLOSIVE);
    G_RadiusDamage(self->s.pos.trBase, NULL, self, self->damage,
                   self->damage + 40, self, MOD_EXPLOSIVE);
  }
  ...
  // if a valid target entity was not found, check for a specified 'angle' for
  // the explosion direction
  if (!tent) {
    if (self->s.angles[1]) {
      // up
  ...
  if (G_GetWeaponClassForMOD(mod) >= self->constructibleStats.weaponclass) {
    G_AddKillSkillPointsForDestruction(attacker, mod,
                                       &self->constructibleStats);
  }
}
...
==============
*/
void func_explosive_use(gentity_t* self,
                        gentity_t* other,
                        gentity_t* activator) {
  G_Script_ScriptEvent(self, "death",
                       "");  // JPW NERVE used to trigger script stuff for MP
  func_explosive_explode(self, self, other, self->damage, 0);
}
...
==============
*/
void func_explosive_spawn(gentity_t* self,
                          gentity_t* other,
                          gentity_t* activator) {
...
"type" - type of debris ("glass", "wood", "metal", "gibs", "brick", "rock",
"fabric") default is "wood"
*/

void target_explosion_use(gentity_t* self,
                          gentity_t* other,
                          gentity_t* attacker) {
  vec3_t dir = {0, 0, 1};
  gentity_t* tent = NULL;
    ...
    if (Q_stricmp(s, "nosound")) {
      ...
      ent->s.dl_intensity = -1;
           ...
           switch(ent->key) {
                   case 0:	// "wood"
                           ent->s.dl_intensity =
   G_SoundIndex("sound/world/boardbreak.wav"); break; case 1:	// "glass"
                           ent->s.dl_intensity =
   G_SoundIndex("sound/world/glassbreak_01.wav"); break; case 2:	//
   "metal" ent->s.dl_intensity = G_SoundIndex("sound/world/metalbreak.wav");
                           break;
                   case 3:	// "gibs"
                           ent->s.dl_intensity =
   G_SoundIndex("sound/player/gibsplit1.wav"); break; case 4:	// "brick"
                           ent->s.dl_intensity =
   G_SoundIndex("sound/world/debris1.wav"); break; case 5:	// "stone"
                           ent->s.dl_intensity =
   G_SoundIndex("sound/world/stonefall.wav"); break;
...
NOBLOCKAAS TANK Any brush that you want to explode or break apart.  If you want
an explosion, set dmg and it will do a radius explosion of that amount at the
center of the bursh. TOUCHABLE means automatic use on player contact. USESHADER
will apply the shader used on the brush model to the debris. LOWGRAV specifies
that the debris will /always/ fall slowly "item" - when it explodes, pop this
item out with the debirs (use QUAKED name. ex: "item_health_small") "dmg" - how
much radius damage should be done, defaults to 0 "health" - defaults to 100.  If
health is set to '0' the brush will not be shootable. "targetname" - if set, no
touch field will be spawned and a remote button or trigger field triggers the
explosion. "type" - type of debris ("glass", "wood", "metal", "gibs", "brick",
"rock", "fabric") default is "wood" "mass" - defaults to 75.  This determines
how much debris is emitted when it explodes.  You get one large chunk per 100 of
mass (up to 8) and one small chunk per 25 of mass (up to 16).  So 800 gives the
most. "noise" - sound to play when triggered.  The explosive will default to a
sound that matches it's 'type'.  Use the sound name "nosound" (case
in-sensitive) if you want it silent. the default sounds are: "wood"	-
"sound/world/boardbreak.wav" "glass"	- "sound/world/glassbreak_01.wav"
...
void SP_func_explosive(gentity_t* ent) {
  ...
  if ((ent->spawnflags & EXPLOSIVE_USESHADER) && ent->model &&
      strlen(ent->model)) {  // use shader
  ...
  if (i > 0 && i <= NUM_CONSTRUCTIBLE_CLASSES) {
    ...
    G_SpawnFloat("constructible_destructxpbonus",
                 va("%f", ent->constructibleStats.destructxpbonus),
                 &ent->constructibleStats.destructxpbonus);
    G_SpawnInt("constructible_health", va("%i", ent->constructibleStats.health),
               &ent->constructibleStats.health);
    G_SpawnInt("constructible_weaponclass",
               va("%i", ent->constructibleStats.weaponclass),
               &ent->constructibleStats.weaponclass);
  } else {
    G_SpawnFloat("constructible_destructxpbonus", "0",
                 &ent->constructibleStats.destructxpbonus);
    G_SpawnInt("constructible_health", "-1", &ent->constructibleStats.health);
    G_SpawnInt("constructible_weaponclass", "0",
               &ent->constructibleStats.weaponclass);
  }
  ent->constructibleStats.weaponclass--;
...
// TODO: stick this somewhere ;) "sonic from australia is the bestest ninja in
// the world"

/*QUAKED func_invisible_user (.3 .5 .8) ? STARTOFF HAS_USER NO_OFF_NOISE
NOT_KICKABLE when activated will use its target "delay" - time (in seconds)
before it can be used again "offnoise" - specifies an alternate sound
"cursorhint" - overrides the auto-location of targeted entity (list below)
Normally when a player 'activates' this entity, if the entity has been turned
'off' (by a scripted command) you will hear a sound to indicate that you cannot
activate the user. The sound defaults to "sound/movers/invis_user_off.wav"
...
"cursorhint" cursor types: (probably more, ask sherman if you think the list is
out of date) they /don't/ need to be all uppercase HINT_NONE HINT_PLAYER
        HINT_ACTIVATE
        HINT_DOOR
        HINT_DOOR_ROTATING
        HINT_DOOR_LOCKED
...
void use_invisible_user(gentity_t* ent,
                        gentity_t* other,
                        gentity_t* activator) {
  ...
  if (other->client && ent->spawnflags & 1) {
    //----(SA)	play 'off' sound
    //----(SA)	I think this is where this goes.  Raf, let me know if it's
    //wrong.  I need someone to tell me what a test map is for this (I'll ask
    //Dan tomorrow)
  ...
  G_UseTargets(ent, other);  //----(SA)	how about this so the triggered targets
                             //have an 'activator' as well as an 'other'?
                             //----(SA)	Please let me know if you forsee any
                             //problems with this.
}
...
void G_Activate(gentity_t* ent, gentity_t* activator) {
  if ((ent->s.apos.trType == TR_STATIONARY &&
       ent->s.pos.trType == TR_STATIONARY) &&
      ent->active == qfalse) {
    // trigger the ent if possible, if not, then we'll just wait at the marker
    // until it opens, which could be never(!?)
    if (ent->key < 0) {  // ent force locked
      return;
    }

    /*		// TAT 2/3/2003 - want keys for SP
                    if (G_IsSinglePlayerGame() && ent->key > 0)	// ent requires
       key
                    {
...
void func_constructible_spawn(gentity_t* self,
                              gentity_t* other,
                              gentity_t* activator);
...
==============
*/
void func_constructible_use(gentity_t* self,
                            gentity_t* other,
                            gentity_t* activator) {
  self->s.angles2[0] = 0;
  self->grenadeFired = 0;
  ...
  if (!(self->spawnflags & CONSTRUCTIBLE_BLOCK_PATHS_WHEN_BUILD)) {
    ...
    if (!self->count2) {
      ...
      trap_SetBrushModel(
          self, va("*%i",
                   self->conbmodels[self->count2 - 1]));  // set the final stage
    }
    trap_LinkEntity(self);
...
==============
*/
void func_constructible_explode(gentity_t* self,
                                gentity_t* inflictor,
                                gentity_t* attacker,
                                int damage,
                                int mod) {
    ...
    if (self->grenadeFired > 1) {
      ...
      if (self->grenadeFired == self->count2) {
        ...
        switch (self->grenadeFired) {
            // case 1: G_Script_ScriptEvent( self, "destroyed", "stage1" );
            // break;
          case 2:
      ...
      // backup...
      {
        ...
        trap_SetBrushModel(self,
                           va("*%i", self->desbmodels[self->grenadeFired - 1]));
      ...
      listedEntities = trap_EntitiesInBox(self->r.absmin, self->r.absmax,
      ...
      for (e = 0; e < listedEntities; e++) {
        ...
        if (block != self) {
          // the entity is blocked by another entity - that block this should
        ...
        if (check->client || check->s.eType == ET_CORPSE) {
          ...
          G_Damage(check, self, attacker, NULL, NULL, 9999, 0,
                   MOD_CRUSH_CONSTRUCTIONDEATH);
        } else if (check->s.eType == ET_ITEM &&
                   check->item->giType == IT_TEAM) {
          // see if it's a critical entity, one that we can't just simply kill
          // (basically flags)
      ...
      if (!(self->spawnflags & CONSTRUCTIBLE_NO_AAS_BLOCKING)) {
        ...
        if (!(self->spawnflags & CONSTRUCTIBLE_BLOCK_PATHS_WHEN_BUILD)) {
          ...
          if (!self->count2) {
            ...
            trap_SetBrushModel(
                self, va("*%i",
                         self->conbmodels[self->grenadeFired]));  // set the
                                                                  // final stage
          }
          trap_UnlinkEntity(self);
        } else {
      ...
      G_UseEntity(
          self, inflictor,
          attacker);  // this will unlink (call func_constructible_use), if
                      // another function is used something is VERY wrong
    }
  } else {
    if (!(self->spawnflags & CONSTRUCTIBLE_NO_AAS_BLOCKING)) {
      if (!(self->spawnflags & CONSTRUCTIBLE_BLOCK_PATHS_WHEN_BUILD)) {
        ...
        if (!self->count2) {
          ...
          trap_SetBrushModel(
              self,
              va("*%i",
                 self->conbmodels[self->grenadeFired]));  // set the final stage
        }
        trap_UnlinkEntity(self);
      } else {
        // Gordon: are we scripted only?
  ...
  if (level.time - ent->lastHintCheckTime >= CONSTRUCT_PREDECAY_TIME) {
    ...
    if (ent->s.angles2[0] < 5) {
      ...
      if (ent->count2) {
        ...
        ent->grenadeFired--;
        ent->s.modelindex2 = 0;
        // trap_SetBrushModel( ent, va( "*%i",
        // ent->conbmodels[ent->grenadeFired-1] ) );
      } else {
        // call script
        G_Script_ScriptEvent(ent, "decayed", "final");
      }
      ...
      ent->lastHintCheckTime =
          level.time;  // don't allow building again for a lil while
      return;
    }
  }
...
void func_constructiblespawn(gentity_t* ent) {
  ...
  if (ent->constages) {
    ...
    for (target_ptr = ptr = ent->constages; *ptr; ptr++) {
      if (*ptr == ';') {
        ...
        if (ent->count2 == MAX_CONSTRUCT_STAGES) {
          G_Error(
              "'func_constructible' has more than %i targets in the constages "
              "key\n",
    ...
    ent->conbmodels[ent->count2++] =
        atoi(ent->model +
             1);  // the brushmodel of the func_constructible is the final stage
    ...
    if (ent->count2 && ent->desstages) {
      ...
      for (target_ptr = ptr = ent->desstages; *ptr; ptr++) {
        if (*ptr == ';') {
          ...
          if (numDesStages == MAX_CONSTRUCT_STAGES - 1) {
            G_Error(
                "'func_constructible' has more than %i targets in the "
                "desstages key\n",
      ...
      if (numDesStages != ent->count2 - 1) {
        G_Error(
            "'func_constructible' has %i entries in the desstages and %i "
            "targets in the constages key\n",
  ...
  if (!(ent->spawnflags & CONSTRUCTIBLE_START_BUILT)) {  // if not start build
    ...
    // RF, first we must temporarily "build" it so we can disable the areas that
    // it touches
    if (!ent->count2) {
      trap_SetBrushModel(ent, ent->model);
    } else {
      trap_SetBrushModel(
          ent,
          va("*%i", ent->conbmodels[ent->count2 - 1]));  // set the final stage
    }
    ...
    if (!ent->count2) {
      ...
      trap_SetBrushModel(ent, ent->model);
      ent->s.modelindex = 0;
      //			ent->s.solid = CONTENTS_SOLID;	// FIXME: allow
      //other contents?
      trap_LinkEntity(ent);
...
AXIS_CONSTRUCTIBLE ALLIED_CONSTRUCTIBLE BLOCK_PATHS_WHEN_BUILT NO_AAS_BLOCKING
AAS_SCRIPTED A constructible object that functions as target for engineers.
...
"constages"	list of target func_brushmodel entities (up to 3) that
                        make up the construction stages (example:
"stage1;stage2;") "desstages"	list of target func_brushmodel entities (up to
3) that make up the destruction stages (example: "desstage1;desstage2;")
...
void SP_func_constructible(gentity_t* ent) {
  ...
  G_SpawnInt( "health", "100", &health ); // default low enough to get killed by
  dynamite ent->sound1to2 = ent->health = health;*/
  ...
  if (i > 0 && i <= NUM_CONSTRUCTIBLE_CLASSES) {
    ...
    G_SpawnFloat("constructible_chargebarreq",
                 va("%f", ent->constructibleStats.chargebarreq),
                 &ent->constructibleStats.chargebarreq);
    G_SpawnFloat("constructible_constructxpbonus",
                 va("%f", ent->constructibleStats.constructxpbonus),
    ...
    G_SpawnInt("constructible_weaponclass",
               va("%i", ent->constructibleStats.weaponclass),
               &ent->constructibleStats.weaponclass);
    G_SpawnInt("constructible_duration",
               va("%i", ent->constructibleStats.duration),
               &ent->constructibleStats.duration);
  } else {
    G_SpawnFloat("constructible_chargebarreq", "1",
                 &ent->constructibleStats.chargebarreq);
    G_SpawnFloat("constructible_constructxpbonus", "0",
                 &ent->constructibleStats.constructxpbonus);
    G_SpawnFloat("constructible_destructxpbonus", "0",
                 &ent->constructibleStats.destructxpbonus);
    ...
    G_SpawnInt("constructible_weaponclass", "0",
               &ent->constructibleStats.weaponclass);
    G_SpawnInt("constructible_duration", "5000",
               &ent->constructibleStats.duration);
  }
  ent->constructibleStats.weaponclass--;
  ent->health = ent->constructibleStats.health;
...
void G_LinkDamageParents(void) {
  ...
  for (i = 0; i < level.num_entities; i++) {
    ...
    if (!(g_entities[i].dmgparent =
              G_FindByTargetname(NULL, g_entities[i].damageparent))) {
