...
void G_smvAdd_cmd(gentity_t* ent) {
  ...
  if (pID < 0 || pID > level.maxclients ||
      g_entities[pID].client->pers.connected != CON_CONNECTED) {
  ...
  if (g_entities[pID].client->sess.sessionTeam == TEAM_SPECTATOR) {
    CP(
        va("print \"[lof]** [lon]Client[lof] %s^7 [lon]is not in the "
           "game[lof]!\n\"",
  ...
  if (!G_allowFollow(ent, G_teamID(&g_entities[pID]))) {
    CP(va(
        "print \"[lof]** [lon]The %s team is locked from spectators[lof]!\n\"",
...
void G_smvAddTeam_cmd(gentity_t* ent, int nTeam) {
  ...
  for (i = 0; i < level.numPlayingClients; i++) {
    ...
    if (g_entities[pID].client->sess.sessionTeam == nTeam &&
        ent != &g_entities[pID]) {
      G_smvAddView(ent, pID);
    }
  }
...
void G_smvDel_cmd(gentity_t* ent) {
  ...
  if (!G_smvLocateEntityInMVList(ent, pID, qtrue)) {
    CP(
        va("print \"[lof]** [lon]Client[lof] %s^7 [lon]is not currently "
           "viewed[lof]!\n\"",
           level.clients[pID].pers.netname));
  }
}
...
void G_smvAddView(gentity_t* ent, int pID) {
  ...
  if (mv == NULL) {
    CP(
        va("print \"[lof]** [lon]Sorry, no more MV slots available (all[lof] "
           "%d [lon]in use)[lof]\n\"",
           MULTIVIEW_MAXVIEWS));
    return;
  }
  ...
  if (ent->client->sess.sessionTeam ==
          TEAM_SPECTATOR && /*ent->client->sess.sessionTeam != TEAM_SPECTATOR
                               ||*/
  ...
  v->r.svFlags =
      SVF_PORTAL |
      SVF_SINGLECLIENT;  // Only merge snapshots for the target client
...
qboolean G_smvLocateEntityInMVList(gentity_t* ent, int pID, qboolean fRemove) {
  ...
  if (ent->client->pers.mvCount > 0) {
    for (i = 0; i < MULTIVIEW_MAXVIEWS; i++) {
      if (ent->client->pers.mv[i].fActive &&
          ent->client->pers.mv[i].entID == pID) {
        if (fRemove) {
...
void G_smvRemoveInvalidClients(gentity_t* ent, int nTeam) {
  ...
  for (i = 0; i < level.numConnectedClients; i++) {
    ...
    if (level.clients[id].sess.sessionTeam != nTeam &&
...
qboolean G_smvRunCamera(gentity_t* ent) {
  ...
  if (ent->target_ent->client->pers.connected != CON_CONNECTED ||
    ...
    G_smvLocateEntityInMVList(ent->tagParent, ent->target_ent - g_entities,
                              qtrue);
    return (qtrue);
  }

  // Seems that depending on player's state, we pull from either
  // r.currentOrigin, or s.origin
  ...
  chargeTime =
      (level.time - tps->classWeaponTime >= (int)chargeTime)
          ? 0
          : (1 + floor(15.0f * (float)(level.time - tps->classWeaponTime) /
                       chargeTime));
  sprintTime =
      (ent->target_ent->client->pmext.sprintTime >= 20000)
          ? 0.0f
          : (1 + floor(7.0f * (float)ent->target_ent->client->pmext.sprintTime /
                       20000.0f));
  ...
  hintTime =
      (tps->serverCursorHint != HINT_BUILD &&
       (tps->serverCursorHintVal >= 255 || tps->serverCursorHintVal == 0))
          ? 0
          : (1 + floor(15.0f * (float)tps->serverCursorHintVal / 255.0f));
  ...
  if (tps->pm_flags & PMF_LIMBO) {
    ...
    ps->ammo[id] =
        (((ent->target_ent->health > 0) ? ent->target_ent->health : 0) &
         0xFF);                               // Meds up to 140 :(
    ps->ammo[id] |= (hintTime & 0x0F) << 8;   // 4 bits for work on current item
                                              // (dynamite, weapon repair, etc.)
    ps->ammo[id] |= (weapHeat & 0x0F) << 12;  // 4 bits for weapon heat info

    ps->ammo[id - 1] = tps->ammo[BG_FindAmmoForWeapon(tps->weapon)] &
                       0x3FF;  // 11 bits needed to cover 1500 Venom ammo
    ps->ammo[id - 1] |= (BG_simpleWeaponState(tps->weaponstate) & 0x03)
                        << 11;  // 2 bits for current weapon state
    ps->ammo[id - 1] |= ((tps->persistant[PERS_HWEAPON_USE]) ? 1 : 0)
                        << 13;  // 1 bit for mg42 use
    ps->ammo[id - 1] |=
        (BG_simpleHintsCollapse(tps->serverCursorHint, hintTime) & 0x03)
        << 14;  // 2 bits for cursor hints

    //	G_Printf("tps->hint: %d, dr: %d, collapse: %d\n", tps->serverCursorHint,
    //HINT_DOOR_ROTATING, G_simpleHintsCollapse(tps->serverCursorHint,
    //hintTime));

    ps->ammoclip[id - 1] = tps->ammoclip[BG_FindClipForWeapon(tps->weapon)] &
                           0x1FF;  // 9 bits to cover 500 Venom ammo clip
    ps->ammoclip[id - 1] |= (chargeTime & 0x0F)
                            << 9;  // 4 bits for weapon charge time
