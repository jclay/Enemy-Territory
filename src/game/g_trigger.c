...
SOLDIERONLY LTONLY MEDICONLY ENGINEERONLY COVERTOPSONLY "wait" : Seconds between
triggerings, 0.5 default, -1 = one time only. "random"	wait variance, default
is 0 Variable sized repeatable trigger.  Must be targeted at one or more
entities. so, the basic time between firing is a random time between (wait -
...
"life"	time this brush will exist if value is zero will live for ever ei 0.5
sec 2.sec default is zero
...
"healrate"		rate of healing per second, default 5 (whole numbers
only) "healtotal"		the maximum of healing this trigger can do. if
<= 0, it's unlimited. default 0 (whole numbers only) "target"		cabinet
that this entity is linked to
*/
...
qboolean G_IsAllowedHeal(gentity_t* ent) {
  ...
  /*	for( i = 0; i < 2; i++ ) {
                  if( !ent->client->lastHealTimes[i] || (level.time -
     ent->client->lastHealTimes[i] > 60000) ) { ent->client->lastHealTimes[i] =
...
void heal_touch(gentity_t* self, gentity_t* other, trace_t* trace) {
  ...
  for (i = 0; i < level.numConnectedClients; i++) {
    ...
    if (level.clients[j].ps.stats[STAT_MAX_HEALTH] > g_entities[j].health &&
        trap_EntityContactCapsule(g_entities[j].r.absmin,
                                  g_entities[j].r.absmax, self) &&
  ...
  for (i = 0; i < clientcount; i++) {
    healvalue = min(touchClients[i]->client->ps.stats[STAT_MAX_HEALTH] -
                        touchClients[i]->health,
                    self->damage);
    if (self->health != -9999) {
    ...
    G_AddPredictableEvent(
        other, EV_ITEM_PICKUP,
        BG_FindItemForClassName("item_health_cabinet") - bg_itemlist);
...
"ammorate"		rate of ammo clips per second. default 1. (whole number
only) "ammototal"		the maximum clips of ammo this trigger can add.
if <= 0, it's unlimited. default 0 (whole numbers only) "target"
...
qboolean G_IsAllowedAmmo(gentity_t* ent) {
  ...
  /*	for( i = 0; i < 2; i++ ) {
                  if( !ent->client->lastAmmoTimes[i] || (level.time -
     ent->client->lastAmmoTimes[i] > 60000) ) { ent->client->lastAmmoTimes[i] =
     level.time;
...
void ammo_touch(gentity_t* self, gentity_t* other, trace_t* trace) {
  ...
  for (i = 0; i < level.numConnectedClients; i++) {
    ...
    if (trap_EntityContactCapsule(g_entities[j].r.absmin,
                                  g_entities[j].r.absmax, self) &&
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ammoAdded) {
      ...
      G_AddPredictableEvent(touchClients[i], EV_ITEM_PICKUP,
                            BG_FindItem("Ammo Pack") - bg_itemlist);
      if (self->health != -9999) {
...
void SP_func_timer(gentity_t* self) {
  ...
  if (self->random >= self->wait) {
    self->random =
        self->wait -
        (FRAMETIME / 1000.f);  // Gordon div 1000 for milisecs...*cough*
...
void trigger_aidoor_stayopen(gentity_t* ent, gentity_t* other, trace_t* trace) {
  ...
  // only use this in single player. It was taken out of multiplayer, and I'm
  // guessing there was a good reason.
  if (g_gametype.integer != GT_SINGLE_PLAYER && g_gametype.integer != GT_COOP) {
    return;
  }

  // FIXME: port this code over to moving doors (use MOVER_POSx instead of
  // MOVER_POSxROTATE)
  if (other->client && other->health > 0) {
    if (!ent->target || !(strlen(ent->target))) {
      ...
      //			G_Printf( "trigger_aidoor at loc %s does not
      //have a target door\n", vtos (ent->s.origin) );
      return;
    }
    ...
    if ((door->moverState == MOVER_POS2ROTATE) ||
        (door->moverState ==
         MOVER_POS2)) {  // door is in open state waiting to close keep it open
      door->nextthink = level.time + door->wait + 3000;
    }

    // what about other move states?

    // for now, don't worry about getting the bots out of the way. this is just
    // for single player, and the bots should have orders to follow anyway
  }
}
...
void SP_trigger_aidoor(gentity_t* ent) {
  if (!ent->targetname) {
    G_Printf(
        "trigger_aidoor at loc %s does not have a targetname for ai_marker "
        "assignments\n",
        vtos(ent->s.origin));
  }
...
void Touch_flagonly(gentity_t* ent, gentity_t* other, trace_t* trace) {
  ...
  if (ent->spawnflags & RED_FLAG && other->client->ps.powerups[PW_REDFLAG]) {
    ...
    G_Script_ScriptEvent(&g_entities[other->client->flagParent], "trigger",
                         "captured");
    ...
    ent->nextthink = level.time + FRAMETIME;
    ent->think = G_FreeEntity;
  } else if (ent->spawnflags & BLUE_FLAG &&
             other->client->ps.powerups[PW_BLUEFLAG]) {
    if (ent->spawnflags & 4) {
...
void SP_trigger_flagonly(gentity_t* ent) {
  ...
  // JPW NERVE -- if this trigger has a "score" field set, then completing an
  // objective
  //  inside of this field will add "score" to the right player team.  storing
  //  this in ent->accuracy since that's unused.
...
void explosive_indicator_think(gentity_t* ent) {
  ...
  if (!parent->inuse ||
      (parent->s.eType == ET_CONSTRUCTIBLE && !parent->r.linked)) {
    // update our map
    {
  ...
  if (ent->s.eType == ET_TANK_INDICATOR ||
      ent->s.eType == ET_TANK_INDICATOR_DEAD) {
...
void constructible_indicator_think(gentity_t* ent) {
  ...
  if (!parent->inuse || !parent->r.linked ||
      (constructible && constructible->s.angles2[1] != 0)) {
    // update our map
    {
      mapEntityData_t* mEnt;
      ...
      if (parent->spawnflags & 8) {
        ...
        teamList =
            ent->s.teamNum == TEAM_AXIS ? &mapEntityData[0] : &mapEntityData[1];
...
// Arnout: links the trigger to it's objective, determining if it's a
// func_explosive of func_constructible and spawning the right indicator
void Think_SetupObjectiveInfo(gentity_t* ent) {
  ...
  if (ent->target_ent->s.eType == ET_EXPLOSIVE) {
    ...
    constructibles[1] = G_FindByTargetname(
        constructibles[0], ent->target);  // see if we are targetting a 2nd one
                                          // for two team constructibles

    team[0] = constructibles[0]->spawnflags & AXIS_CONSTRUCTIBLE ? TEAM_AXIS
                                                                 : TEAM_ALLIES;

    constructibles[0]->s.otherEntityNum2 = ent->s.teamNum;

    if (constructibles[1]) {
      team[1] = constructibles[1]->spawnflags & AXIS_CONSTRUCTIBLE
                    ? TEAM_AXIS
                    : TEAM_ALLIES;

      if (constructibles[1]->s.eType != ET_CONSTRUCTIBLE) {
        G_Error(
            "'trigger_objective_info' targets multiple entities with "
            "targetname '%s', the second one isn't a 'func_constructible'\n",
            ent->target);
      }

      if (team[0] == team[1]) {
        G_Error(
            "'trigger_objective_info' targets two 'func_constructible' "
            "entities with targetname '%s' that are constructible by the same "
            "team\n",
            ent->target);
      }
    ...
    if (constructibles[0]->s.angles2[1] != 0) {
      ...
      if (constructibles[1]) {
        // see if one of the two is still partially built (happens when a
        // multistage destructible construction blows up for the first time)
        if (constructibles[0]->count2 && constructibles[0]->grenadeFired > 1) {
          e->s.teamNum = team[0];
        } else if (constructibles[1]->count2 &&
                   constructibles[1]->grenadeFired > 1) {
          e->s.teamNum = team[1];
        } else {
...
MESSAGE_OVERRIDE TANK IS_OBJECTIVE IS_HEALTHAMMOCABINET IS_COMMANDPOST Players
in this field will see a message saying that they are near an objective.
...
void SP_trigger_objective_info(gentity_t* ent) {
  ...
  if (ent->spawnflags & MESSAGE_OVERRIDE) {
    if (!ent->spawnitem) {
      G_Error(
          "'trigger_objective_info' has override flag set but no override "
          "text\n");
    }
  }
  ...
  G_SetConfigStringValue(CS_OID_DATA + level.numOidTriggers, "cix",
                         va("%i", cix));
  G_SetConfigStringValue(CS_OID_DATA + level.numOidTriggers, "cia",
                         va("%i", cia));
  G_SetConfigStringValue(CS_OID_DATA + level.numOidTriggers, "s",
                         va("%i", ent->spawnflags));
  G_SetConfigStringValue(CS_OID_DATA + level.numOidTriggers, "n",
  ...
  // JPW NERVE -- if this trigger has a "score" field set, then blowing up an
  // objective
  //  inside of this field will add "score" to the right player team.  storing
  //  this in ent->accuracy since that's unused.
  ...
  if (ent->s.origin[0] || ent->s.origin[1] || ent->s.origin[2]) {
    G_SetConfigStringValue(CS_OID_DATA + level.numOidTriggers, "x",
                           va("%i", (int)ent->s.origin[0]));
    G_SetConfigStringValue(CS_OID_DATA + level.numOidTriggers, "y",
                           va("%i", (int)ent->s.origin[1]));
    G_SetConfigStringValue(CS_OID_DATA + level.numOidTriggers, "z",
                           va("%i", (int)ent->s.origin[2]));
  } else {
    vec3_t mid;
  ...
  if (!ent->target) {
    ...
    // Arnout: finalize spawing on fourth frame to allow for proper linking with
    // targets
...
// JPW NERVE -- field which is acted upon (cgame side) by screenshakes to drop
// dust particles
void trigger_concussive_touch(gentity_t* ent,
                              gentity_t* other,
                              trace_t* trace) {
  return;  // FIXME this should be NULLed out in SP_trigger_concussive_dust
