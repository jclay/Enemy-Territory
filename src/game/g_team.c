...
void QDECL PrintMsg(gentity_t* ent, const char* fmt, ...) {
  ...
  // NOTE: if buffer overflow, it's more likely to corrupt stack and crash than
  // do a proper G_Error?
  va_start(argptr, fmt);
...
void Team_FragBonuses(gentity_t* targ,
  ...
  if (flag) {  // JPW NERVE -- added some more stuff after this fn
    ...
    if ((VectorLengthSquared(v1) < SQR(CTF_TARGET_PROTECT_RADIUS) ||
         ...
         CanDamage(flag, targ->client->ps.origin) ||
         CanDamage(flag, attacker->client->ps.origin)) &&
        attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {
      // we defended the base flag
      // JPW NERVE FIXME -- don't report flag defense messages, change to gooder
      // message
    ...
    /*		if (g_gametype.integer < GT_WOLF) { // JPW NERVE no attacker
       protect in wolf MP if (carrier && carrier != attacker) {
                                    VectorSubtract(targ->s.origin,
       carrier->s.origin, v1); VectorSubtract(attacker->s.origin,
       carrier->s.origin, v1);

                                    if (VectorLengthSquared(v1) <
       SQR(CTF_ATTACKER_PROTECT_RADIUS) || VectorLengthSquared(v2) <
       SQR(CTF_ATTACKER_PROTECT_RADIUS) || CanDamage(carrier, targ->s.origin) ||
       CanDamage(carrier, attacker->s.origin)) { AddScore(attacker,
  ...
  flag = NULL;
  while ((flag = G_Find(flag, FOFS(classname), "team_WOLF_checkpoint")) !=
         NULL) {
    VectorSubtract(targ->client->ps.origin, flag->s.origin, v1);
    if ((flag->s.frame != WCP_ANIM_NOFLAG) &&
        (flag->count == attacker->client->sess.sessionTeam)) {
...
void Team_DroppedFlagThink(gentity_t* ent) {
  if (ent->item->giTag == PW_REDFLAG) {
    ...
    //		trap_SendServerCommand(-1, "cp \"Allies have returned the
    //objective!\" 2");
  }
...
int Team_TouchOurFlag(gentity_t* ent, gentity_t* other, int team) {
  ...
  if (ent->flags & FL_DROPPED_ITEM) {
    ...
    if (cl->sess.sessionTeam == TEAM_AXIS) {
      ...
      //			trap_SendServerCommand(-1, va("cp \"Axis have
      //returned %s!\n\" 2", ent->message));
      ...
      //			trap_SendServerCommand(-1, va("cp \"Allies have
      //returned %s!\n\" 2", ent->message));
...
int Team_TouchEnemyFlag(gentity_t* ent, gentity_t* other, int team) {
  ...
  if (cl->sess.sessionTeam == TEAM_AXIS) {
    ...
    //		te->s.eventParm = G_SoundIndex(
    //"sound/chat/axis/g-objective_taken.wav" );
    ...
    //		te->s.eventParm = G_SoundIndex(
    //"sound/chat/allies/a-objective_taken.wav" );
...
int Pickup_Team(gentity_t* ent, gentity_t* other) {
  ...
  if ((g_gametype.integer != GT_SINGLE_PLAYER) &&
    ...
    // JPW NERVE -- set flag model in carrying entity if multiplayer and
    // flagmodel is set
...
int FindFarthestObjectiveIndex(vec3_t source) {
  ...
  /*
          ...
          G_Printf("got furthest dist (%f) at point %d (%s) of
...
NERVE - SMF - pick MP objective closest to the passed in vector, return table
index
=======================
*/
...
gentity_t* SelectRandomTeamSpawnPoint(int teamstate,
  ...
  while ((spot = G_Find(spot, FOFS(classname), classname)) != NULL) {
    ...
    // Arnout - modified to allow initial spawnpoints to be disabled at
    // gamestart
...
void TeamplayInfoMessage(team_t team) {
  ...
  for (i = 0, cnt = 0; i < level.numConnectedClients; i++) {
    ...
    if (player->inuse && player->client->sess.sessionTeam == team) {
      ...
      Com_sprintf(
          entry, sizeof(entry), " %i %i %i %i %i", level.sortedClients[i],
...
void CheckTeamStatus(void) {
  ...
  if (level.time - level.lastTeamLocationTime > TEAM_LOCATION_UPDATE_TIME) {
    ...
    for (i = 0; i < level.numConnectedClients; i++) {
      ...
      if (ent->inuse && (ent->client->sess.sessionTeam == TEAM_AXIS ||
...
void Use_Team_InitialSpawnpoint(gentity_t* ent,
                                gentity_t* other,
                                gentity_t* activator) {
  if (ent->spawnflags & 4) {
    ent->spawnflags &= ~4;
...
void Use_Team_Spawnpoint(gentity_t* ent,
                         gentity_t* other,
                         gentity_t* activator) {
  if (ent->spawnflags & 2) {
    ent->spawnflags &= ~2;
...
/*QUAKED team_CTF_redplayer (0.5 0 0) (-16 -16 -16) (16 16 32) INVULNERABLE -
STARTDISABLED Axis players spawn here at game start.
*/
void SP_team_CTF_redplayer(gentity_t* ent) {
  // Gordon: these are obsolete
  G_Printf(
      "^1team_ctf_*player entities are now obsolete, please remove them!\n");
  G_FreeEntity(ent);
  return;
...
/*QUAKED team_CTF_blueplayer (0 0 0.5) (-16 -16 -16) (16 16 32) INVULNERABLE -
STARTDISABLED Allied players spawn here at game start.
*/
...
/*QUAKED team_CTF_redspawn (1 0 0) (-16 -16 -24) (16 16 32) ? INVULNERABLE
STARTACTIVE potential spawning position for axis team in wolfdm games.
...
// JPW NERVE edited quaked def
/*QUAKED team_CTF_bluespawn (0 0 1) (-16 -16 -24) (16 16 32) ? INVULNERABLE
STARTACTIVE potential spawning position for allied team in wolfdm games.
...
DEFAULT_ALLIES marker for objective
...
screen.
        ...
        "description"	short text key for objective name that will appear in
objective selection in limbo UI.
...
// swaps the team
void team_wolf_objective_use(gentity_t* self,
                             gentity_t* other,
                             gentity_t* activator) {
  char cs[MAX_STRING_CHARS];
  ...
  if (ent->count2 == level.time) {
    if (holderteam == TEAM_AXIS) {
      ...
      trap_SendServerCommand(
          ...
          va("cp \"Flag will be held in %i seconds!\n\"", time));
    } else {
...
void checkpoint_spawntouch(gentity_t* self,
                           gentity_t* other,
...
// JPW NERVE -- if spawn flag is set, use this touch fn instead to turn on/off
// targeted spawnpoints
void checkpoint_spawntouch(gentity_t* self, gentity_t* other, trace_t* trace) {
  ...
  // Arnout - updated this to allow toggling of initial spawnpoints as well,
  // plus now it only toggles spawnflags 2 for spawnpoint entities
  if (self->target) {
    while (1) {
      ...
      if (other->client->sess.sessionTeam == TEAM_AXIS) {
        if (!strcmp(ent->classname, "team_CTF_redspawn")) {
          ...
          // Gordon: these are obsolete
          /*				} else if
             (!strcmp(ent->classname,"team_CTF_redplayer")) { ent->spawnflags &=
             ~4; } else if (!strcmp(ent->classname,"team_CTF_blueplayer")) {
                                                  ent->spawnflags |= 4;*/
        }
      } else {
        if (!strcmp(ent->classname, "team_CTF_bluespawn")) {
          ...
          // Gordon: these are obsolete
          /*				} else if
             (!strcmp(ent->classname,"team_CTF_blueplayer")) { ent->spawnflags
             &= ~4; } else if (!strcmp(ent->classname,"team_CTF_redplayer")) {
                                                  ent->spawnflags |= 4;*/
        }
      }
    }
...
// jpw
/*QUAKED team_WOLF_checkpoint (.9 .3 .9) (-16 -16 0) (16 16 128) SPAWNPOINT
CP_HOLD AXIS_ONLY ALLIED_ONLY This is the flagpole players touch in Capture and
Hold game scenarios.
...
// JPW NERVE if spawnpoint flag is set, think will turn on spawnpoints
...
void SP_team_WOLF_checkpoint(gentity_t* ent) {
  ...
  // Model is user assignable, but it will always try and use the animations for
  // flagpole.md3
  if (ent->model) {
    ent->s.modelindex = G_ModelIndex(ent->model);
  } else {
    ent->s.modelindex =
        G_ModelIndex("models/multiplayer/flagpole/flagpole.md3");
  }
...
void G_teamReset(int team_num, qboolean fClearSpecLock) {
  teamInfo[team_num].team_lock =
      (match_latejoin.integer == 0 && g_gamestate.integer == GS_PLAYING);
...
qboolean G_checkReady(void) {
  ...
  if (level.numNonSpectatorClients >= match_minplayers.integer &&
      level.voteInfo.numVotingClients > 0) {
...
void G_verifyMatchState(int nTeam) {
  ...
  if ((level.lastRestartTime + 1000) < level.time &&
      (nTeam == TEAM_ALLIES || nTeam == TEAM_AXIS) &&
      (gs == GS_PLAYING || gs == GS_WARMUP_COUNTDOWN ||
       gs == GS_INTERMISSION)) {
  ...
  if ((team_num == TEAM_AXIS || team_num == TEAM_ALLIES)) {
    ...
    if (g_gametype.integer != GT_WOLF_LMS) {
      ...
      if (team_maxplayers.integer > 0 && team_maxplayers.integer <= cnt) {
        G_printFull(va("The %s team is full!", aTeams[team_num]), ent);
        return (qfalse);
      } else if (g_gamestate.integer == GS_PLAYING && g_lms_lockTeams.integer &&
                 (!(ent->client->pers.invite & team_num))) {
        G_printFull(va("The %s team is LOCKED!", aTeams[team_num]), ent);
        return (qfalse);
      }
...
void G_removeSpecInvite(int team) {
  ...
  for (i = 0; i < level.numConnectedClients; i++) {
    ...
    if (!cl->inuse || cl->client->sess.referee ||
        cl->client->sess.coach_team == team) {
      continue;
    }
...
qboolean G_allowFollow(gentity_t* ent, int nTeam) {
  ...
  return ((!teamInfo[nTeam].spec_lock ||
...
qboolean G_desiredFollow(gentity_t* ent, int nTeam) {
  if (G_allowFollow(ent, nTeam) && (ent->client->sess.spec_team == 0 ||
                                    ent->client->sess.spec_team == nTeam)) {
    return (qtrue);
  }
