...
qboolean G_ScriptAction_FollowPath(gentity_t* ent, char* params) {
  ...
  if (!params ||
      ent->scriptStatus.scriptStackChangeTime <
          level.time) {  // we are waiting for it to reach destination
    if (ent->s.pos.trTime + ent->s.pos.trDuration <=
        level.time) {  // we made it
      ent->scriptStatus.scriptFlags &= ~SCFL_GOING_TO_MARKER;

      // set the angles at the destination
      BG_EvaluateTrajectory(&ent->s.apos,
                            ent->s.apos.trTime + ent->s.apos.trDuration,
                            ent->s.angles, qtrue, ent->s.effect2Time);
      ...
      // stop moving
      BG_EvaluateTrajectory(&ent->s.pos, level.time, ent->s.origin, qfalse,
                            ent->s.effect2Time);
    ...
    ent->s.effect2Time =
        backward ? -(pSpline - splinePaths + 1) : pSpline - splinePaths + 1;
    ...
    if (!wait) {
      ...
      if (ent->s.pos.trDuration % 50) {
        ...
        frac = (float)(((ent->s.pos.trDuration / 50) * 50 + 50) -
                       ent->s.pos.trDuration) /
               (float)(ent->s.pos.trDuration);
        if (frac < 1) {
          ent->s.apos.trDuration = ent->s.pos.trDuration =
              (ent->s.pos.trDuration / 50) * 50 + 50;
        }
      }

      // set the goto flag, so we can keep processing the move until we reach
      // the destination
      ent->scriptStatus.scriptFlags |= SCFL_GOING_TO_MARKER;
  ...
  BG_EvaluateTrajectory(&ent->s.pos, level.time, ent->r.currentOrigin, qfalse,
                        ent->s.effect2Time);
  BG_EvaluateTrajectory(&ent->s.apos, level.time, ent->r.currentAngles, qtrue,
                        ent->s.effect2Time);
  script_linkentity(ent);
...
qboolean G_ScriptAction_SetSpeed(gentity_t* ent, char* params) {
  ...
  BG_EvaluateTrajectory(&ent->s.pos, level.time, ent->r.currentOrigin, qtrue,
                        ent->s.effect2Time);
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (!token || !*token) {
      G_Error(
          "G_Scripting: syntax: setspeed <x> <y> <z> [gravity|lowgravity]\n");
    }
    speed[i] = atoi(token);
  }
...
qboolean G_ScriptAction_SetRotation(gentity_t* ent, char* params) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (!token || !token[0]) {
      G_Error(
          "G_Scripting: syntax: setrotation <pitchspeed> <yawspeed> "
          "<rollspeed>\n");
    }
    angles[i] = atoi(token);
  }
...
qboolean G_ScriptAction_SetChargeTimeFactor(gentity_t* ent, char* params) {
  ...
  } else if (!Q_stricmp(playerclass,
                        "lieutenant")) {  // FIXME: remove from missionpack
    level.lieutenantChargeTimeModifier[team] = factor;
    level.lieutenantChargeTime[team] = g_LTChargeTime.integer * factor;
...
G_ScriptAction_SetGlobalFog
  ...
  syntax: setglobalfog <bool:restore> <int:duration> [float:r] [float:g]
...
qboolean G_ScriptAction_SetGlobalFog(gentity_t* ent, char* params) {
  ...
  if (!token[0]) {
    G_Error(
        "G_Scripting: setglobalfog without restore flag must have a depth for "
        "opaque value\n");
  }

  depthForOpaque = atof(token);

  trap_SetConfigstring(CS_GLOBALFOGVARS,
                       va("0 %i %f %f %f %f", duration, color[0], color[1],
                          color[2], depthForOpaque));
    ...
    if (!wait) {
      ...
      if (ent->s.pos.trDuration % 50) {
        ...
        frac = (float)(((ent->s.pos.trDuration / 50) * 50 + 50) -
               ...
               (float)(ent->s.pos.trDuration);
        if (frac < 1) {
          VectorScale(ent->s.pos.trDelta, 1.0 / (1.0 + frac),
                      ent->s.pos.trDelta);
...
qboolean G_ScriptAction_Trigger(gentity_t* ent, char* params) {
  ...
  if (!Q_stricmp(name, "self")) {
    ...
    if (ent->activator && ent->activator->client &&
        (ent->activator->r.svFlags & SVF_BOT) && ent->inuse &&
...
script.
  ...
  Use the optional LOOPING paramater to attach the sound to the entities looping
channel.
================
*/
...
G_ScriptAction_FadeAllSounds
  ...
  Fades all sounds up or down, going from the current level to max or zero in
'time' milliseconds
...
qboolean G_ScriptAction_FadeAllSounds(gentity_t* ent, char* params) {
  ...
  trap_SendServerCommand(
      -1, va("snd_fade %f %d %i", (up) ? 1.0 : 0.0, time, (up) ? 0 : 1));
...
qboolean G_ScriptAction_MusicFade(gentity_t* ent, char* params) {
  ...
  if (!token[0]) {
    G_Error(
        "G_Scripting: syntax: mu_fade <target volume 0.0-1.0> <fadeout time>");
  }
...
G_ScriptAction_PlayAnim
  ...
  syntax: playanim <startframe> <endframe> [looping <FOREVER/duration>] [rate
<FPS>]
  ...
  if (token[0]) {
    if (!Q_stricmp(token, "looping")) {
      ...
      if (!Q_stricmp(token, "untilreachmarker")) {
        ...
        endtime =
            level.time +
            100;  // we don't care when it ends, since we are going forever!
        forever = qtrue;
      } else {
    ...
    if (token[0] && !Q_stricmp(token, "rate")) {
      ...
      if (!token[0]) {
        G_Error(
            "G_Scripting: playanim has RATE parameter without an actual rate "
            "specified");
      }
      rate = atoi(token);
    }
  ...
  while (1) {
    ...
    if (alertent->client) {
      ...
      if (!alertent->AIScript_AlertEntity) {
        G_Error(
            "G_Scripting: alertentity \"%s\" (classname = %s) doesn't have an "
            "\"AIScript_AlertEntity\" function\n",
      ...
      if (!alertent->use) {
        G_Error(
            "G_Scripting: alertentity \"%s\" (classname = %s) doesn't have a "
            "\"use\" function\n",
            params, alertent->classname);
      }
...
qboolean G_ScriptAction_ToggleSpeaker(gentity_t* ent, char* params) {
  ...
  for (i = 0; i < BG_NumScriptSpeakers(); i++) {
    ...
    if (hash != speaker->targetnamehash &&
        Q_stricmp(params, speaker->targetname)) {
      continue;
    }
...
qboolean G_ScriptAction_Accum(gentity_t* ent, char* params) {
  ...
  if (!Q_stricmp(lastToken, "inc")) {
    ...
    if (ent->scriptAccumBuffer[bufferIndex] == atoi(token)) {
      ...
      //
      //			G_Error( "G_Scripting: trigger has unknown name:
      //%s\n", name );
      G_Printf("G_Scripting: trigger has unknown name: %s\n", name);
      return qtrue;
    }
...
G_ScriptAction_FaceAngles
  ...
  GOTOTIME is given instead of a timed duration, the duration calculated from
...
qboolean G_ScriptAction_FaceAngles(gentity_t* ent, char* params) {
  ...
  if (!params || !params[0]) {
    G_Error(
        "G_Scripting: syntax: faceangles <pitch> <yaw> <roll> "
        "<duration/GOTOTIME>\n");
  }
  ...
  if (ent->scriptStatus.scriptStackChangeTime == level.time) {
    ...
    if (!token || !token[0]) {
      G_Error(
          "G_Scripting: faceangles requires a <pitch> <yaw> <roll> "
          "<duration/GOTOTIME>\n");
    }
...
G_ScriptAction_ResetScript
        ...
        causes any currently running scripts to abort, in favour of the current
script
===================
*/
...
qboolean G_ScriptAction_TagConnect(gentity_t* ent, char* params) {
  ...
  if (!token[0]) {
    G_Error(
        "G_ScriptAction_TagConnect: syntax: attachtotag <targetname> "
        "<tagname>\n");
  }
  ...
  if (!parent) {
    ...
    if (!parent) {
      G_Error(
          "G_ScriptAction_TagConnect: unable to find entity with targetname "
          "\"%s\"",
          token);
    }
  }
...
qboolean G_ScriptAction_Halt(gentity_t* ent, char* params) {
  if (level.time == ent->scriptStatus.scriptStackChangeTime) {
    ...
    // stop the angles
    BG_EvaluateTrajectory(&ent->s.apos, level.time, ent->s.angles, qtrue,
                          ent->s.effect2Time);
    VectorCopy(ent->s.angles, ent->s.apos.trBase);
    VectorCopy(ent->s.angles, ent->r.currentAngles);
    ent->s.apos.trTime = level.time;
...
qboolean G_ScriptAction_AxisRespawntime(gentity_t* ent, char* params) {
  ...
  if (g_userAxisRespawnTime.integer) {
    trap_Cvar_Set("g_redlimbotime",
                  va("%i", g_userAxisRespawnTime.integer * 1000));
  } else {
...
qboolean G_ScriptAction_AlliedRespawntime(gentity_t* ent, char* params) {
  ...
  if (g_userAlliedRespawnTime.integer) {
    trap_Cvar_Set("g_bluelimbotime",
                  va("%i", g_userAlliedRespawnTime.integer * 1000));
  } else {
...
qboolean G_ScriptAction_NumberofObjectives(gentity_t* ent, char* params) {
  ...
  if (num < 1 || num > MAX_OBJECTIVES) {
    G_Error(
        "G_ScriptAction_NumberofObjectives: Invalid number of objectives\n");
  }
...
qboolean G_ScriptAction_SetMainObjective(gentity_t* ent, char* params) {
  /*	char *pString, *token;
          ...
          token = COM_Parse(&pString);
          if (!token[0]) {
                  G_Error( "G_ScriptAction_ObjectiveImage: number parameter
     required\n" );
          }
     ...
     CS_MAIN_ALLIES_OBJECTIVE; trap_GetConfigstring( cs_obj, cs, sizeof(cs) );
...
qboolean G_ScriptAction_ObjectiveStatus(gentity_t* ent, char* params) {
  ...
  if (atoi(token) != 0 && atoi(token) != 1 && atoi(token) != 2) {
    G_Error(
        "G_ScriptAction_ObjectiveImage: status parameter must be 0 (default), "
        "1 (complete) or 2 (failed)\n");
  }
...
qboolean G_ScriptAction_VoiceAnnounce(gentity_t* ent, char* params) {
  ...
  if (!token[0]) {
    G_Error(
        "G_ScriptAction_VoiceAnnounce: system message parameter required\n");
  }
...
qboolean G_ScriptAction_RemoveTeamVoiceAnnounce(gentity_t* ent, char* params) {
  ...
  if (!*token) {
    G_Error(
        "G_ScriptAction_RemoveTeamVoiceAnnounce: team parameter required\n");
  }
  ...
  if (!*token) {
    G_Error(
        "G_ScriptAction_RemoveTeamVoiceAnnounce: sound parameter required\n");
  }
...
qboolean G_ScriptAction_Announce_Icon(gentity_t* ent, char* params) {
  ...
  if (iconnumber < 0 || iconnumber >= PM_NUM_TYPES) {
    G_Error(
        "G_ScriptAction_Announce_Icon: icon index parameter out of range %i\n",
        iconnumber);
  }
...
qboolean G_ScriptAction_PrintGlobalAccum(gentity_t* ent, char* params) {
  ...
  if (!params || !params[0]) {
    G_Error(
        "G_ScriptAction_PrintGlobalAccum: syntax: PrintGlobalAccum "
        "<globalAccumNumber>\n");
  }
...
qboolean G_ScriptAction_SetAASState(gentity_t* ent, char* params) {
  ...
  if (!token || !*token) {
    G_Error(
        "G_Scripting: setaasstate must have a name and a state "
        "[enabled|disabled|avoid]\n");
  }
...
qboolean G_ScriptAction_Construct(gentity_t* ent, char* params) {
  ...
  if (!constructible || !constructible->inuse ||
    ...
    G_Error(
        "G_Scripting: \"construct\" could not find entity with targetname: "
        "%s\n",
        token);
  }
...
===================
*/
qboolean G_ScriptAction_ConstructibleChargeBarReq(gentity_t* ent,
                                                  char* params) {
  char *pString, *token;
  float value;
  ...
  if (!(token = COM_ParseExt(&pString, qfalse))) {
    G_Error(
        "G_Scripting: \"constructible_chargebarreq\" must have a fraction "
        "value\n");
  }
...
===================
*/
qboolean G_ScriptAction_ConstructibleConstructXPBonus(gentity_t* ent,
                                                      char* params) {
  char *pString, *token;
  int value;
  ...
  if (!(token = COM_ParseExt(&pString, qfalse))) {
    G_Error(
        "G_Scripting: \"constructible_constructxpbonus\" must have a xppoints "
        "value\n");
  }
  ...
  if (value < 0) {
    G_Error(
        "G_Scripting: \"constructible_constructxpbonus\" has a bad value %i\n",
        value);
  }
...
===================
*/
qboolean G_ScriptAction_ConstructibleDestructXPBonus(gentity_t* ent,
                                                     char* params) {
  char *pString, *token;
  int value;
  ...
  if (!(token = COM_ParseExt(&pString, qfalse))) {
    G_Error(
        "G_Scripting: \"constructible_destructxpbonus\" must have a xppoints "
        "value\n");
  }
  ...
  if (value < 0) {
    G_Error(
        "G_Scripting: \"constructible_destructxpbonus\" has a bad value %i\n",
        value);
  }
...
qboolean G_ScriptAction_ConstructibleWeaponclass(gentity_t* ent, char* params) {
  ...
  if (!(token = COM_ParseExt(&pString, qfalse))) {
    G_Error(
        "G_Scripting: \"constructible_weaponclass\" must have a weapon class "
        "value\n");
  }
...
qboolean G_ScriptAction_ConstructibleDuration(gentity_t* ent, char* params) {
  ...
  if (!(token = COM_ParseExt(&pString, qfalse))) {
    G_Error(
        "G_Scripting: \"constructible_duration\" must have a duration value\n");
  }
    ...
    if (cvarValue != atoi(token)) {
      ...
      ent->scriptStatus.scriptStackHead =
          ent->scriptEvents[ent->scriptStatus.scriptEventIndex].stack.numItems;
             ...
             !Q_stricmp(lastToken, "abort_if_equals")) {
    if (!token[0]) {
      G_Error("G_Scripting: cvar %s requires a parameter\n", lastToken);
    }
    if (cvarValue == atoi(token)) {
...
available fields can be found in field_t of g_spawn.c, it is quite simple to add
new ones
===================
*/
  ...
  while (1) {
    ...
    if (g_scriptDebug.integer) {
      G_Printf("%d : (%s) %s: set [%s] [%s] [%s]\n", level.time,
               ent->scriptName, GAMEVERSION, ent->scriptName, key, value);
    }
