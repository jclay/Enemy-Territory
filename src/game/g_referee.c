...
//	--> ref arg allows for the server console to utilize all referee
//commands (ent == NULL)
//
...
void G_refHelp_cmd(gentity_t* ent) {
  ...
  if (ent) {
    ...
    CP("print \"\n^5allready         putallies^7 <pid>  ^5speclock          "
       "warmup\n\"");
    CP("print \"^5lock             putaxis^7 <pid>    ^5specunlock        warn "
       "^7<pid>\n\"");
    CP("print \"^5help             remove           unlock            mute "
       "^7<pid>\n\"");
    CP("print \"^5pause            restart          unpause           unmute "
       "^7<pid>\n\"");
...
void G_ref_cmd(gentity_t* ent, unsigned int dwCommand, qboolean fValue) {
  ...
  if (ent) {
    if (!Q_stricmp(refereePassword.string, "none") ||
        !refereePassword.string[0]) {
...
void G_refLockTeams_cmd(gentity_t* ent, qboolean fLock) {
  ...
  teamInfo[TEAM_ALLIES].team_lock =
      (TeamCount(-1, TEAM_ALLIES)) ? fLock : qfalse;
...
void G_refPause_cmd(gentity_t* ent, qboolean fPause) {
  ...
  if (fPause) {
    ...
    AP(
        va("print \"\n^3%s ^5UNPAUSES^3 the match ... resuming in 10 "
           "seconds!\n\n\"",
           referee));
...
void G_refPlayerPut_cmd(gentity_t* ent, int team_id) {
  ...
  if (team_maxplayers.integer &&
      TeamCount(-1, team_id) >= team_maxplayers.integer) {
...
void G_refSpeclockTeams_cmd(gentity_t* ent, qboolean fLock) {
  ...
  status =
      va("Referee has ^3SPECTATOR %sLOCKED^7 teams", ((fLock) ? "" : "UN"));
...
void G_refWarning_cmd(gentity_t* ent) {
  ...
  if (kicknum != MAX_CLIENTS) {
    if (level.clients[kicknum].sess.referee == RL_NONE ||
        ((!ent || ent->client->sess.referee == RL_RCON) &&
         level.clients[kicknum].sess.referee <= RL_REFEREE)) {
      trap_SendServerCommand(
          -1, va("cpm \"%s^7 was issued a ^1Warning^7 (%s)\n\"\n",
...
void G_refMute_cmd(gentity_t* ent, qboolean mute) {
  ...
  if (player->client->sess.muted == mute) {
    G_refPrintf(ent, "\"%s^*\" %s\n", player->client->pers.netname,
                mute ? "is already muted!" : "is not muted!");
    return;
  }
...
int G_refClientnumForName(gentity_t* ent, const char* name) {
  ...
  for (i = 0; i < level.numConnectedClients; i++) {
    Q_strncpyz(cleanName, level.clients[level.sortedClients[i]].pers.netname,
               sizeof(cleanName));
