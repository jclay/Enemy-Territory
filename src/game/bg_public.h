...
#define ITEM_RADIUS \
  10  // Rafael changed the radius so that the items would fit in the 3 new
      // containers
...
typedef enum {
  ...
  STATE_INVISIBLE,  // ent is unlinked, can't be used, doesn't think and is not
                    // solid
...
qboolean BG_LoadCampaignSave(const char* filename,
                             cpsFile_t* file,
                             const char* profile);
qboolean BG_StoreCampaignSave(const char* filename,
                              cpsFile_t* file,
                              const char* profile);
...
typedef struct {
  ...
  vec2_t mapTC[2];
  cpsCampaign_t*
      cpsCampaign;  // if this campaign was found in the campaignsave, more
...
#define CS_INTERMISSION \
  12  // when 1, intermission will start in a second or two
...
typedef enum {
  ...
  GT_WOLF,
  GT_WOLF_STOPWATCH,
  GT_WOLF_CAMPAIGN,  // Exactly the same as GT_WOLF, but uses campaign roulation
                     // (multiple maps form one virtual map)
  GT_WOLF_LMS,
  GT_MAX_GAME_TYPE
} gametype_t;
// #define GAMETYPES
...
typedef enum {
  ...
  WEAPON_RELAXING,  // weapon is ready, but since not firing, it's on it's way
                    // to a "relaxed" stance
  WEAPON_FIRING,
  WEAPON_FIRINGALT,
...
typedef struct {
  ...
  int weapAnimTimer;    // don't change low priority animations until this runs
                        // out		//----(SA)	added
  int silencedSideArm;  // Gordon: Keep track of whether the luger/colt is
                        // silenced "in holster", prolly want to do this for the
                        // kar98 etc too
  int sprintTime;
  ...
  int dodgeTime;
  int proneTime;  // time a go-prone or stop-prone move starts, to sync the
                  // animation to
  ...
  int weapRecoilTime;  // Arnout: time at which a weapon that has a recoil
                       // kickback has been fired last
...
typedef struct {
  ...
  qboolean
      noWeapClips;  // if the game is setup for no weapon clips by the server
  qboolean
  ...
  qboolean leadership;  // within 512 units of a player with level 5 Signals
                        // skill (that player has to be in PVS as well to make
                        // sue we can predict it)
#endif                  // GAMEDLL
...
typedef enum {
  ...
  STAT_XP,  // Gordon: "realtime" version of xp that doesnt need to go thru the
            // scoreboard
} statIndex_t;
...
typedef enum {
  ...
  PERS_RESPAWNS_PENALTY,  // how many respawns you have to sit through before
                          // respawning again
  ...
  // Rafael - mg42		// (SA) I don't understand these here.  can
  // someone explain?
  PERS_HWEAPON_USE,
  // Rafael wolfkick
  PERS_WOLFKICK
...
#define EF_FORCE_END_FRAME \
  EF_NONSOLID_BMODEL  // force client to end of current animation (after loading
                      // a savegame)
...
#define EF_INHERITSHADER \
  EF_FIRING  // some ents will never use EF_FIRING, hijack it for "USESHADER"

#define EF_SPINNING \
  0x00000100  // (SA) added for level editor control of spinning pickup items
#define EF_BREATH \
  EF_SPINNING  // Characters will not have EF_SPINNING set, hijack for drawing
               // character breath
...
#define EF_HEADSHOT \
  0x00001000  // last hit to player was head shot (Gordon: NOTE: not last hit,
              // but has BEEN shot in the head since respawn)
#define EF_SMOKING \
  0x00002000  // DHM - Nerve :: ET_GENERAL ents will emit smoke if set // JPW
              // switched to this after my code change
...
#define EF_MOVER_STOP \
  0x10000000  // will push otherwise	// (SA) moved down to make space for one
              // more client flag
#define EF_MOVER_BLOCKED \
  0x20000000  // mover was blocked dont lerp on the client // xkan, moved down
              // to make space for client flag
...
typedef enum {
  ...
  PW_BLACKOUT =
      14,  // OSP - spec blackouts. FIXME: we don't need 32bits here...relocate
...
typedef enum {
  //----(SA)	These will probably all change to INV_n to get the word 'key'
  //out of the game.
  //			id and DM don't want references to 'keys' in the game.
  //			I'll change to 'INV' as the item becomes 'permanent' and
  //not a test item.
...
typedef enum {
  ...
  WP_MEDIC_SYRINGE,      // 11	// JPW NERVE -- broken out from CLASS_SPECIAL
                         // per Id request
  ...
  // Gordon: ONLY secondaries below this mark, as they are checked >= WP_GPG40
  // && < WP_NUM_WEAPONS
...
typedef struct ammotable_s {
  ...
  int nextShotTime;         //
                            //----(SA)	added
  int maxHeat;   // max active firing time before weapon 'overheats' (at which
                 // point the weapon will fail)
...
typedef enum {
  ...
  EV_ITEM_PICKUP_QUIET,   // (SA) same, but don't play the default pickup sound
                          // as it was specified in the ent
  ...
  EV_GLOBAL_CLIENT_SOUND,  // DHM - Nerve :: no attenuation, only plays for
                           // specified client
  ...
  EV_SHARD,
  EV_JUNK,
  EV_EMITTER,  //----(SA)	added // generic particle emitter that uses
...
typedef enum {
  ...
  TORSO_MOVE,  // torso anim to play while moving and not firing (swinging arms
               // type thing)
  TORSO_MOVE_ALT,
...
typedef enum {
  ...
  WEAP_ATTACK1,
  WEAP_ATTACK2,
  WEAP_ATTACK_LASTSHOT,  // used when firing the last round before having an
                         // empty clip.
  WEAP_DROP,
  WEAP_RAISE,
  WEAP_RELOAD1,
...
typedef enum {
  ...
  IT_HOLDABLE,  // single use, holdable item
                // EFX: rotate + bob
  IT_KEY,
  IT_TREASURE,  // gold bars, etc.  things that can be picked up and counted for
                // a tally at end-level
  IT_TEAM,
} itemType_t;
...
typedef struct gitem_s {
  ...
  int giAmmoIndex;  // type of weapon ammo this uses.  (ex. WP_MP40 and WP_LUGER
                    // share 9mm ammo, so they both have WP_LUGER for
                    // giAmmoIndex)
  int giClipIndex;  // which clip this weapon uses.  this allows the sniper
                    // rifle to use the same clip as the garand, etc.
...
qboolean BG_CanItemBeGrabbed(const entityState_t* ent,
...
#define MASK_OPAQUE \
  (CONTENTS_SOLID | \
   CONTENTS_LAVA)  //----(SA)	modified since slime is no longer deadly
...
qboolean BG_AddMagicAmmo(playerState_t* ps,
                         int* skill,
                         int teamNum,
                         int numOfClips);
...
typedef enum {
  ...
  ANIM_COND_GEN_BITFLAG,  // xkan 1/15/2003 - general bit flags (to save some
                          // space)
  ANIM_COND_AISTATE,  // xkan 1/17/2003 - our current ai state (sometimes more
                      // convenient than creating a separate section)
...
typedef struct {
  int numItems;
  animScriptItem_t*
      items[MAX_ANIMSCRIPT_ITEMS];  // pointers into a global list of items
} animScript_t;
...
typedef struct {
  ...
  // parsed from the start of the cfg file (this is basically obsolete now -
  ...
  animScript_t
      scriptEvents[NUM_ANIM_EVENTTYPES];  // events that trigger special anims
...
typedef enum {
  ...
  ACC_MOUTH3,      //
  ACC_RANK,        //
  ACC_MAX  // this is bound by network limits, must change network stream to
           // increase this
} accType_t;
...
qboolean BG_WeaponIsPrimaryForClassAndTeam(int classnum,
                                           team_t team,
                                           weapon_t weapon);
...
splinePath_t* BG_AddSplinePath(const char* name,
                               const char* target,
                               vec3_t origin);
...
void BG_LinearPathOrigin2(float radius,
...
typedef struct {
  int ident;
  char joinOrder[MAX_CLIENTS];  // order in which clients joined the fire team
                                // (server), client uses to store if a client is
                                // on this fireteam
...
void BG_AdjustAAGunMuzzleForBarrel(vec_t* origin,
                                   vec_t* forward,
                                   vec_t* right,
                                   vec_t* up,
                                   int barrel);
...
int BG_cleanName(const char* pszIn,
                 char* pszOut,
...
#endif  // CGAMEDLL
qboolean BG_LoadTraceMap(char* rawmapname,
                         vec2_t world_mins,
                         vec2_t world_maxs);
