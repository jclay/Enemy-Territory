...
void G_loadMatchGame(void) {
  ...
  if (server_autoconfig.integer > 0 &&
      (!(z_serverflags.integer & ZSF_COMP) || level.newSession)) {
  ...
  //	trap_SetConfigstring(CS_OSPVERSION, va("%s", G_BASEVERSION));	// Add
  ...
  strcpy(strReinfSeeds,
         va("%d %d",
            (dwBlueOffset << REINF_BLUEDELT) + (rand() % (1 << REINF_BLUEDELT)),
  ...
  level.dwBlueReinfOffset =
      1000 * aRandomValues[dwBlueOffset] / aReinfSeeds[dwBlueOffset];
  level.dwRedReinfOffset =
      1000 * aRandomValues[dwRedOffset] / aReinfSeeds[dwRedOffset];
  ...
  switch (dpent->spawnflags) {
    case DP_PAUSEINFO: {
      if (level.match_pause > PAUSE_UNPAUSING) {
        int cSeconds = match_timeoutlength.integer * 1000 -
                       (level.time - dpent->timestamp);
...
static char* pszDPInfo[] = {
    "DPRINTF_PAUSEINFO", "DPRINTF_UNPAUSING", "DPRINTF_CONNECTINFO",
    "DPRINTF_MVSPAWN",   "DPRINTF_UNK1",      "DPRINTF_UNK2",
    "DPRINTF_UNK3",      "DPRINTF_UNK4",      "DPRINTF_UNK5"};
...
void G_addStats(gentity_t* targ, gentity_t* attacker, int dmg_ref, int mod) {
  ...
  if (targ->health <= 0 && targ->client->ps.pm_type == PM_DEAD) {
    if (mod < MOD_CROSS && attacker && attacker->client) {
      int x =
          attacker->client->sess.aWeaponStats[G_weapStatIndex_MOD(mod)].atts--;
      if (x < 1) {
...
char* G_createStats(gentity_t* refEnt) {
  ...
  for (i = WS_KNIFE; i < WS_MAX; i++) {
    if (refEnt->client->sess.aWeaponStats[i].atts ||
      ...
      Q_strcat(strWeapInfo, sizeof(strWeapInfo),
               va(" %d %d %d %d %d", refEnt->client->sess.aWeaponStats[i].hits,
  ...
  // Additional info
  Q_strcat(strWeapInfo, sizeof(strWeapInfo),
           va(" %d %d %d", refEnt->client->sess.damage_given,
  ...
  for (i = SK_BATTLE_SENSE; i < SK_NUM_SKILLS; i++) {
    if (refEnt->client->sess.skillpoints[i] > 0) {
      ...
      Q_strcat(strSkillInfo, sizeof(strSkillInfo),
               va(" %d", (int)refEnt->client->sess.skillpoints[i]));
    }
  }
...
void G_printMatchInfo(gentity_t* ent) {
  ...
  for (i = TEAM_AXIS; i <= TEAM_ALLIES; i++) {
    ...
    CP("sc \"\n^7TEAM   Player          Kll Dth Sui TK Eff  ^3GP^7    ^2DG    "
       "^1DR   ^6TD  ^3Score\n"
       "^7---------------------------------------------------------------------"
       "\n\"");
    ...
    for (j = 0; j < level.numPlayingClients; j++) {
      ...
      eff = (cl->sess.deaths + cl->sess.kills == 0)
                ? 0
                : 100 * cl->sess.kills / (cl->sess.deaths + cl->sess.kills);
      if (eff < 0) {
        eff = 0;
      }
      ...
      cnt++;
      CP(va(
          "sc \"%-10s %s%-15s^3%4d%4d%4d%3d%s%4d^3%4d^2%6d^1%6d^6%5d^3%7d\n\"",
          aTeams[i], ref, n2, cl->sess.kills, cl->sess.deaths,
          cl->sess.suicides, cl->sess.team_kills, ref, eff,
          cl->sess.game_points - (cl->sess.kills * WOLF_FRAG_BONUS),
          cl->sess.damage_given, cl->sess.damage_received, cl->sess.team_damage,
          cl->ps.persistant[PERS_SCORE]));
    }

    eff = (tot_kills + tot_deaths == 0)
              ? 0
              : 100 * tot_kills / (tot_kills + tot_deaths);
    if (eff < 0) {
      eff = 0;
    }
    ...
    CP(
        va("sc "
           ...
           aTeams[i], "Totals", tot_kills, tot_deaths, tot_sui, tot_tk, eff,
           tot_gp - (tot_kills * WOLF_FRAG_BONUS), tot_dg, tot_dr, tot_td,
           tot_gp));
  }
        ...
        if ((cl->pers.autoaction & AA_STATSALL) || cl->pers.mvCount > 0) {
          ...
          if (cl->pers.autoaction & AA_STATSTEAM) {
            G_statsall_cmd(ent, cl->sess.sessionTeam,
                           qfalse);  // Currently broken.. need to support the
                                     // overloading of dwCommandID
          } else {
            CP(va("ws %s\n", G_createStats(ent)));
          }
      ...
      if (g_gametype.integer == GT_WOLF_STOPWATCH) {
        if (g_currentRound.integer == 1) {  // We've already missed the switch
          ...
          if (val < g_timelimit.value) {
            CP(va(
                "print \">>> ^3Objective reached at %d:%02d (original: "
                "%d:%02d)\n\n\n\"",
            ...
            CP(va(
                "print \">>> ^3Objective NOT reached in time (%d:%02d)\n\n\n\"",
                g_timelimit.integer,
                (int)(60.0 *
                      (float)(g_timelimit.value - g_timelimit.integer))));
          }
...
void G_statsPrint(gentity_t* ent, int nType) {
  ...
  char* cmd = (nType == 0)
                  ? "ws"
                  : ((nType == 1) ? "wws" : "gstats");  // Yes, not the cleanest
  ...
  if (trap_Argc() < 2) {
    if (ent->client->sess.sessionTeam != TEAM_SPECTATOR) {
      ...
      CP(va("%s %s\n", cmd,
            G_createStats(g_entities + ent->client->sess.spectatorClient)));
    } else {
      CP("cpm \"Type ^3\\stats <player_id>^7 to see stats on an active "
         "player.\n\"");
      return;
    }
  } else {
...
ent->client->sess.latchPlayerWeapon != 8) { ent->client->pers.panzerSelectTime =
0; return(qtrue);
        }
        ...
        if(team_maxPanzers.integer == 0) {
                if(ent->client->pers.cmd_debounce < level.time) {
                        ...
                        G_printFull("[lof]^3*** [lon]Panzers are disabled on
this server[lof].", ent);
                }
                return(qfalse);
        }
...
continue; if(cl->sess.latchPlayerType != PC_SOLDIER) continue;

                // ACTIVE panzers take precedence.  Limbo players will fight
amongst themselves if(COM_BitCheck(cl->ps.weapons, WP_PANZERFAUST) ||
cl->pers.panzerDropTime > level.time) { cPanzers++; continue;
                }

                // Deal with waiting-to-spawn clients where there is contention
on who gets a panzer if((cl->ps.pm_flags & PMF_LIMBO) &&
                  ((cl->pers.panzerSelectTime != 0 &&
ent->client->pers.panzerSelectTime == 0) || (cl->pers.panzerSelectTime > 0 &&
cl->pers.panzerSelectTime < ent->client->pers.panzerSelectTime)))
                {
                        cPanzers++;
                        continue;
                }
        ...
        if(ent->client->pers.cmd_debounce < level.time) {
                ent->client->pers.cmd_debounce = level.time + 3000;
                G_printFull(va("[lof]^3*** [lon]Already[lof %d [lon]panzers in
the game[lof].", team_maxPanzers.integer), ent);
        }
