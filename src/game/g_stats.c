...
void G_LogDeath(gentity_t* ent, weapon_t weap) {
  ...
  trap_PbStat(ent - g_entities, "death",
              va("%d %d %d", ent->client->sess.sessionTeam,
                 ent->client->sess.playerType, weap));
}
...
void G_LogKill(gentity_t* ent, weapon_t weap) {
  ...
  if (ent->client->sess.playerType == PC_SOLDIER) {
    ...
    ent->client->soliderKillTimes[ent->client->soldierKillMarker++] =
        level.timeCurrent;
    ...
    for (i = 0, j = ent->client->soldierKillMarker; i < NUM_SOLDIERKILL_TIMES;
         i++) {
      if (!ent->client->soliderKillTimes[j] ||
          (ent->client->soliderKillTimes[j] <
           level.timeCurrent - SOLDIERKILL_MAXTIME)) {
        pass = qfalse;
        break;
      }
...
void G_PrintAccuracyLog(gentity_t* ent) {
  ...
  for (i = 0; i < WP_NUM_WEAPONS; i++) {
    ...
    Q_strcat(buffer, 2048,
             va(" %i %i %i", ent->client->pers.playerStats.weaponStats[i].kills,
  ...
  for (i = 0; i < HR_NUM_HITREGIONS; i++) {
    Q_strcat(buffer, 2048,
             va(" %i", ent->client->pers.playerStats.hitRegions[i]));
  }
  ...
  for (i = 0; i < 6 /*level.numOidTriggers*/; i++) {
    Q_strcat(buffer, 2048,
             va(" %i", ent->client->pers.playerStats.objectiveStats[i]));
    Q_strcat(buffer, 2048,
             va(" %i", ent->client->sess.sessionTeam == TEAM_AXIS
                           ? level.objectiveStatsAxis[i]
                           : level.objectiveStatsAllies[i]));
  }
...
// TAT 11/6/2002
//		Local func to actual do skill upgrade, used by both MP skill
//system, and SP scripted skill system
  ...
  if (skill == SK_BATTLE_SENSE && ent->client->sess.skill[skill] == 1) {
    if (AddWeaponToPlayer(ent->client, WP_BINOCULARS, 1, 0, qfalse)) {
      ent->client->ps.stats[STAT_KEYS] |= (1 << INV_BINOCS);
             ...
             ent->client->sess.skill[skill] == 4) {
    AddWeaponToPlayer(
        ent->client, WP_MEDIC_ADRENALINE,
...
void G_LoseSkillPoints(gentity_t* ent, skillType_t skill, float points) {
  ...
  G_Printf(
      "%s just lost %f skill points for skill %s\n", ent->client->pers.netname,
  ...
  trap_PbStat(ent - g_entities, "loseskill",
              va("%d %d %d %f", ent->client->sess.sessionTeam,
                 ent->client->sess.playerType, skill,
                 oldskillpoints - ent->client->sess.skillpoints[skill]));
...
void G_AddSkillPoints(gentity_t* ent, skillType_t skill, float points) {
  ...
  trap_PbStat(ent - g_entities, "addskill",
              va("%d %d %d %f", ent->client->sess.sessionTeam,
                 ent->client->sess.playerType, skill, points));
...
void G_LoseKillSkillPoints(gentity_t* tker,
  ...
  switch (mod) {
    ...
    case MOD_SMOKEGRENADE:
      ...
      //			G_DebugAddSkillPoints( attacker,
      //SK_LIGHT_WEAPONS, 2.f, "kill" );
      break;
    ...
    case MOD_SATCHEL:
      ...
      //			G_DebugAddSkillPoints( attacker,
      //SK_LIGHT_WEAPONS, 2.f, "legshot kill" );
      break;
    ...
    case MOD_MORTAR:
      ...
      //			G_DebugAddSkillPoints( attacker,
      //SK_HEAVY_WEAPONS, 3.f, "emplaced mg42 kill" );
      break;
    ...
    case MOD_M7:
      ...
      //			G_DebugAddSkillPoints( attacker,
      //SK_EXPLOSIVES_AND_CONSTRUCTION, 4.f, "dynamite or landmine kill" );
      break;
...
void G_AddKillSkillPoints(gentity_t* attacker,
  ...
  switch (mod) {
    ...
    case MOD_AKIMBO_SILENCEDLUGER:
      switch (hr) {
        case HR_HEAD:
          ...
          G_DebugAddSkillPoints(attacker, SK_LIGHT_WEAPONS, 5.f,
                                "headshot kill");
          break;
        case HR_ARMS:
          G_AddSkillPoints(attacker, SK_LIGHT_WEAPONS, 3.f);
          G_DebugAddSkillPoints(attacker, SK_LIGHT_WEAPONS, 3.f,
                                "armshot kill");
          break;
        case HR_BODY:
          G_AddSkillPoints(attacker, SK_LIGHT_WEAPONS, 3.f);
    ...
    case MOD_MOBILE_MG42:
      ...
      G_DebugAddSkillPoints(attacker, SK_HEAVY_WEAPONS, 3.f,
                            "mobile mg42 kill");
      break;
    ...
    case MOD_FG42SCOPE:
      switch (hr) {
        case HR_HEAD:
          G_AddSkillPoints(attacker,
                           SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, 5.f);
          G_DebugAddSkillPoints(attacker,
        ...
        case HR_ARMS:
          G_AddSkillPoints(attacker,
                           SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, 3.f);
          G_DebugAddSkillPoints(attacker,
                                SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS,
                                2.f, "armshot kill");
          break;
    ...
    case MOD_SATCHEL:
      G_AddSkillPoints(attacker, SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS,
                       5.f);
      G_DebugAddSkillPoints(attacker,
                            SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, 5.f,
                            "satchel charge kill");
    ...
    case MOD_PANZERFAUST:
      if (splash) {
        G_AddSkillPoints(attacker, SK_HEAVY_WEAPONS, 3.f);
        G_DebugAddSkillPoints(attacker, SK_HEAVY_WEAPONS, 3.f,
                              "panzerfaust splash damage kill");
      } else {
        G_AddSkillPoints(attacker, SK_HEAVY_WEAPONS, 3.f);
        G_DebugAddSkillPoints(attacker, SK_HEAVY_WEAPONS, 3.f,
                              "panzerfaust direct hit kill");
      }
      break;
    case MOD_FLAMETHROWER:
      G_AddSkillPoints(attacker, SK_HEAVY_WEAPONS, 3.f);
    ...
    case MOD_MORTAR:
      if (splash) {
        G_AddSkillPoints(attacker, SK_HEAVY_WEAPONS, 3.f);
        G_DebugAddSkillPoints(attacker, SK_HEAVY_WEAPONS, 3.f,
                              "mortar splash damage kill");
      } else {
        G_AddSkillPoints(attacker, SK_HEAVY_WEAPONS, 3.f);
        G_DebugAddSkillPoints(attacker, SK_HEAVY_WEAPONS, 3.f,
                              "mortar direct hit kill");
      }
      break;
    case MOD_GRENADE_LAUNCHER:
    case MOD_GRENADE_PINEAPPLE:
    ...
    case MOD_LANDMINE:
      ...
      G_DebugAddSkillPoints(attacker, SK_EXPLOSIVES_AND_CONSTRUCTION, 4.f,
                            "dynamite or landmine kill");
      break;
    case MOD_ARTY:
    ...
    case MOD_M7:
      ...
      G_DebugAddSkillPoints(attacker, SK_EXPLOSIVES_AND_CONSTRUCTION, 3.f,
                            "rifle grenade kill");
      break;
...
void G_AddKillSkillPointsForDestruction(
  ...
  switch (mod) {
    case MOD_GRENADE_LAUNCHER:
    case MOD_GRENADE_PINEAPPLE:
      G_AddSkillPoints(attacker, SK_LIGHT_WEAPONS,
                       constructibleStats->destructxpbonus);
      G_DebugAddSkillPoints(attacker, SK_LIGHT_WEAPONS,
                            constructibleStats->destructxpbonus,
    ...
    case MOD_DYNAMITE:
    case MOD_LANDMINE:
      G_AddSkillPoints(attacker, SK_EXPLOSIVES_AND_CONSTRUCTION,
                       constructibleStats->destructxpbonus);
      G_DebugAddSkillPoints(attacker, SK_EXPLOSIVES_AND_CONSTRUCTION,
                            constructibleStats->destructxpbonus,
                            "destroying a constructible/explosive");
      break;
    case MOD_PANZERFAUST:
    case MOD_MORTAR:
      G_AddSkillPoints(attacker, SK_HEAVY_WEAPONS,
                       constructibleStats->destructxpbonus);
      G_DebugAddSkillPoints(attacker, SK_HEAVY_WEAPONS,
                            constructibleStats->destructxpbonus,
                            "destroying a constructible/explosive");
      break;
    case MOD_ARTY:
    case MOD_AIRSTRIKE:
      G_AddSkillPoints(attacker, SK_SIGNALS,
                       constructibleStats->destructxpbonus);
      G_DebugAddSkillPoints(attacker, SK_SIGNALS,
                            constructibleStats->destructxpbonus,
                            "destroying a constructible/explosive");
      break;
    case MOD_SATCHEL:
...
void G_DebugAddSkillLevel(gentity_t* ent, skillType_t skill) {
  ...
  trap_SendServerCommand(ent - g_entities,
                         va("sdbg \"^%c(SK: %2i XP: %6.2f) %s: You raised your "
                            "skill level to %i.\"\n",
                            COLOR_RED + skill, ent->client->sess.skill[skill],
                            ent->client->sess.skillpoints[skill],
                            skillNames[skill], ent->client->sess.skill[skill]));

  trap_RealTime(&ct);

  if (g_debugSkills.integer >= 2 && skillDebugLog != -1) {
    char* s =
        va("%02d:%02d:%02d : ^%c(SK: %2i XP: %6.2f) %s: %s raised in skill "
           "level to %i.\n",
           ct.tm_hour, ct.tm_min, ct.tm_sec, COLOR_RED + skill,
           ent->client->sess.skill[skill], ent->client->sess.skillpoints[skill],
           skillNames[skill], ent->client->pers.netname,
...
void G_DebugAddSkillPoints(gentity_t* ent,
  ...
  trap_SendServerCommand(
      ent - g_entities,
      va("sdbg \"^%c(SK: %2i XP: %6.2f) %s: You gained %.2fXP, reason: %s.\"\n",
         COLOR_RED + skill, ent->client->sess.skill[skill],
         ent->client->sess.skillpoints[skill], skillNames[skill], points,
         reason));
  ...
  if (g_debugSkills.integer >= 2 && skillDebugLog != -1) {
    char* s =
        va("%02d:%02d:%02d : ^%c(SK: %2i XP: %6.2f) %s: %s gained %.2fXP, "
           "reason: %s.\n",
           ct.tm_hour, ct.tm_min, ct.tm_sec, COLOR_RED + skill,
           ent->client->sess.skill[skill], ent->client->sess.skillpoints[skill],
...
#define CHECKSTAT1(XX)                                      \
  ...
  Q_strcat(buffer, 1024,                                    \
           va(";%s; %i ", best ? best->pers.netname : "",   \
              best ? best->sess.sessionTeam : -1))
...
#define CHECKSTATMIN(XX, YY)                                                \
  ...
  Q_strcat(buffer, 1024,                                                    \
           va(";%s; %i ", best && best->XX >= YY ? best->pers.netname : "", \
...
#define CHECKSTATSKILL(XX)                                                    \
  ...
  Q_strcat(                                                                   \
      buffer, 1024,                                                           \
      va(";%s; %i ",                                                          \
         best && best->sess.skillpoints[XX] >= 20 ? best->pers.netname : "",  \
         best && best->sess.skillpoints[XX] >= 20 ? best->sess.sessionTeam    \
                                                  : -1))
...
#define CHECKSTATTIME(XX, YY)                                                 \
  ...
  for (i = 0; i < level.numConnectedClients; i++) {                           \
    ...
    if (!best || (cl->XX / (float)(level.time - cl->YY)) >                    \
                     (best->XX / (float)(level.time - best->YY))) {           \
      best = cl;                                                              \
    }                                                                         \
  }                                                                           \
  if (best) {                                                                 \
    if ((best->sess.startxptotal - best->ps.persistant[PERS_SCORE]) >= 100 || \
        best->medals || best->hasaward) {                                     \
      best = NULL;                                                            \
    }                                                                         \
  }                                                                           \
  Q_strcat(buffer, 1024,                                                      \
