...
void Remove(char *buf, int length, char *from, char *to, char *skip)
{
        ...
        for (i = 0; i < length; i++)
        {
                ...
                else
                {
                        ...
                        if (!strncmp(&buf[i], from, strlen(from))) remove =
true; } //end else } //end for } //end of the function Remove
...
void AddFunctionName(char* funcname, char* filename, tokenList_t* head) {
  ...
  f = (replacefunc_t*)GetMemory(sizeof(replacefunc_t) + strlen(funcname) + 1 +
                                6 + strlen(filename) + 1);
  ...
  f->filename = (char*)f + sizeof(replacefunc_t) + strlen(funcname) + 1 +
                strlen(f->newname) + 1;
...
void Usage() {
  Error(
      "USAGE: screwup <file filter> [-o <funcs> <func_desc>] [-d <define>]\n"
...
void Usage() {
  Error(
      "USAGE: screwup  [-o <funcs> <func_desc>] (-d <define1> (<define2> ..)) "
      "-f <file1> [<file2> ..]");
}
...
int main(int argc, char* argv[]) {
  ...
  if (!Q_stricmp(argv[argbase], "-d")) {
    ...
    while (Q_stricmp(argv[argbase + i], "-f")) {
      ...
      strncpy(defines[numdefines], argv[argbase + i],
              strlen(argv[argbase + i]));
      numdefines++;
      i++;
    }
    argbase += i + 1;
