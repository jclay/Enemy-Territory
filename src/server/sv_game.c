...
==================
*/
qboolean SV_EntityContact(const vec3_t mins,
                          const vec3_t maxs,
...
====================
*/
void SV_GameBinaryMessageReceived(int cno,
                                  const char* buf,
                                  int buflen,
                                  int commandTime) {
...
int SV_GameSystemCalls(int* args) {
  switch (args[0]) {
    ...
    case BOTLIB_AAS_RT_SHOWROUTE:
      ...
      //	return botlib_export->aas.AAS_RT_GetHidePos( VMA(1), args[2],
      //args[3], VMA(4), args[5], args[6], VMA(7) );

      // case BOTLIB_AAS_FINDATTACKSPOTWITHINRANGE:
      //	return botlib_export->aas.AAS_FindAttackSpotWithinRange(
      //args[1], args[2], args[3], VMF(4), args[5], VMA(6) );

    case BOTLIB_AAS_NEARESTHIDEAREA:
      return botlib_export->aas.AAS_NearestHideArea(args[1], VMA(2), args[3],
                                                    args[4], VMA(5), args[6],
                                                    args[7], VMF(8), VMA(9));

    case BOTLIB_AAS_LISTAREASINRANGE:
      return botlib_export->aas.AAS_ListAreasInRange(VMA(1), args[2], VMF(3),
                                                     args[4], VMA(5), args[6]);

    case BOTLIB_AAS_AVOIDDANGERAREA:
      return botlib_export->aas.AAS_AvoidDangerArea(VMA(1), args[2], VMA(3),
                                                    args[4], VMF(5), args[6]);

    case BOTLIB_AAS_RETREAT:
      return botlib_export->aas.AAS_Retreat(VMA(1), args[2], VMA(3), args[4],
                                            VMA(5), args[6], VMF(7), VMF(8),
                                            args[9]);

    case BOTLIB_AAS_ALTROUTEGOALS:
      return botlib_export->aas.AAS_AlternativeRouteGoals(
          VMA(1), VMA(2), args[3], VMA(4), args[5], args[6]);
    ...
    case BOTLIB_AAS_RECORDTEAMDEATHAREA:
      botlib_export->aas.AAS_RecordTeamDeathArea(VMA(1), args[2], args[3],
                                                 args[4], args[5]);
      return 0;
      // done.
    ...
    case BOTLIB_AI_INITIAL_CHAT:
      botlib_export->ai.BotInitialChat(args[1], VMA(2), args[3], VMA(4), VMA(5),
                                       VMA(6), VMA(7), VMA(8), VMA(9), VMA(10),
                                       VMA(11));
      return 0;
...
qboolean SV_GetTag(int clientNum,
                   int tagFileNumber,
                   char* tagname,
                   orientation_t* or) {
  int i;
  ...
  // Gordon: bleh, some code in clientthink_real really relies on this working
  // on player models...
