...
// this gets you a better throughput, but you loose the ability to control the
// download usage
...
char* SV_ExpandNewlines(char* in) {
  ...
  while (*in && l < sizeof(string) - 3) {
    if (*in == '\n') {
      ...
      // NERVE - SMF - HACK - strip out localization tokens before string
      // command is displayed in syscon window
...
// #define	HEARTBEAT_DEAD	"WolfFlatline-1"			// NERVE
// - SMF
...
void SVC_Info(netadr_t from) {
  ...
  // Rafael gameskill
  //	Info_SetValueForKey (infostring, "gameskill", va ("%i",
  //sv_gameskill->integer));
  // done

  Info_SetValueForKey(infostring, "friendlyFire",
                      va("%i", sv_friendlyFire->integer));  // NERVE - SMF
  Info_SetValueForKey(infostring, "maxlives",
                      va("%i", sv_maxlives->integer ? 1 : 0));  // NERVE - SMF
  Info_SetValueForKey(infostring, "needpass",
                      va("%i", sv_needpass->integer ? 1 : 0));
  Info_SetValueForKey(
      infostring, "gamename",
...
void SVC_RemoteCommand(netadr_t from, msg_t* msg) {
  ...
  // buffer overflow the buffer overflow will be fixed in > 1.31 clients but we
  // want a server side fix we must NEVER send an OOB message that will be
  // > 1.31 MAXPRINTMSG (4096)
...
int SV_LoadTag(const char* mod_name) {
  ...
  if (version != TAG_VERSION) {
    Com_Printf(S_COLOR_YELLOW
               "WARNING: SV_LoadTag: %s has wrong version (%i should be %i)\n",
