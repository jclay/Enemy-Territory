  ...
  for (index = 0; index < MAX_CONFIGSTRINGS; index++) {
    ...
    if (sv.state == SS_GAME || sv.restarting) {
      ...
      for (i = 0, client = svs.clients; i < sv_maxclients->integer;
        ...
        // Arnout: added coop
        if ((SV_GameIsSinglePlayer() || SV_GameIsCoop()) && client->gentity &&
...
void SV_BoundMaxClients(int minimum) {
  ...
  // allow many bots in single player. note that this pretty much means all
  // previous settings will be ignored (including the one set through "seta
  // sv_maxclients <num>" in user profile's wolfconfig_mp.cfg). also that if the
  // user subsequently start the server in multiplayer mode, the number of
  // clients will still be the number set here, which may be wrong - we can
  // certainly just set it to a sensible number when it is not in single player
  // mode in the else part of the if statement when necessary
...
void SV_SetExpectedHunkUsage(char* mapname) {
  ...
  if (len >=
      0) {  // the file exists, so read it in, strip out the current entry for
            // this map, and save it out, so we can append the new value
  ...
  // just set it to a negative number,so the cgame knows not to draw the percent
  // bar
...
================
SV_TouchCGameDLL
  touch the cgame DLL so that a pure client (with DLL sv_pure support) can load
do the correct checks
================
*/
...
void SV_SpawnServer(char* server, qboolean killBots) {
  ...
  // DO_LIGHT_DEDICATED
  // only comment out when you need a new pure checksum string and it's
  // associated random feed
  // Com_DPrintf("SV_SpawnServer checksum feed: %p\n", sv.checksumFeed);

#else  // DO_LIGHT_DEDICATED implementation below
       // we are not able to randomize the checksum feed since the feed is used
       // as key for pure_checksum computations files.c 1776 :
       // pack->pure_checksum = Com_BlockChecksumKey( fs_headerLongs, 4 *
       // fs_numHeaderLongs, LittleLong(fs_checksumFeed) ); we request a fake
       // randomized feed, files.c knows the answer
  srand(Sys_Milliseconds());
  ...
  // we want the server to reference the mp_bin pk3 that the client is expected
  // to load from
  SV_TouchCGameDLL();
...
void SV_Init(void) {
  ...
  // Rafael gameskill
  //	sv_gameskill = Cvar_Get ("g_gameskill", "3", CVAR_SERVERINFO |
  //CVAR_LATCH );
  // done
  ...
  sv_hostname =
      Cvar_Get("sv_hostname", "ETHost", CVAR_SERVERINFO | CVAR_ARCHIVE);
  //
#ifdef __MACOS__
  sv_maxclients =
      Cvar_Get("sv_maxclients", "16", CVAR_SERVERINFO | CVAR_LATCH);  // DAJ HOG
#else
  ...
  sv_friendlyFire = Cvar_Get("g_friendlyFire", "1",
                             CVAR_SERVERINFO | CVAR_ARCHIVE);  // NERVE - SMF
  sv_maxlives =
      Cvar_Get("g_maxlives", "0",
               CVAR_ARCHIVE | CVAR_LATCH | CVAR_SERVERINFO);  // NERVE - SMF
  ...
  sv_tempbanmessage = Cvar_Get("sv_tempbanmessage",
                               "You have been kicked and are temporarily "
                               "banned from joining this server.",
                               0);
  ...
  sv_onlyVisibleClients =
      Cvar_Get("sv_onlyVisibleClients", "0", 0);  // DHM - Nerve
  ...
  g_gameType = Cvar_Get("g_gametype", va("%i", com_gameInfo.defaultGameType),
