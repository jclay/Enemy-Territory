...
==============
*/
static void SV_Netchan_Encode(client_t* client,
                              msg_t* msg,
                              char* commandString) {
  ...
  for (i = SV_ENCODE_START; i < msg->cursize; i++) {
    // modify the key with the last received and with this message acknowledged
    // client command
    if (!string[index]) {
      index = 0;
    }
...
void SV_Netchan_TransmitNextFragment(client_t* client) {
  ...
  while (!client->netchan.unsentFragments && client->netchan_start_queue) {
    ...
    //%		Com_Error(ERR_DROP, "netchan queue is not properly initialized
    //in SV_Netchan_TransmitNextFragment\n"); %	}
