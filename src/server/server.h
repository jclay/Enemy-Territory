...
typedef struct client_s {
  ...
  // www downloading
  qboolean bDlOK;  // passed from cl_wwwDownload CVAR_USERINFO, wether this
                   // client supports www dl
  ...
  qboolean bFallback;  // last www download attempt failed, fallback to regular
                       // download
  ...
  // packet bursts in case large fragmented messages are stacking up buffer them
  // into this queue, and hand them out to netchan as needed
...
// sv_main.c
//
void SV_FinalCommand(
    char* cmd,
    qboolean disconnect);  // ydnar: added disconnect flag so map changes can
...
// sv.gentitySize*(num))) #define SV_GameClientNum( num ) ((playerState_t
// *)((byte *)sv.gameClients + sv.gameClientSize*(num)))
...
int SV_AreaEntities(const vec3_t mins,
                    const vec3_t maxs,
                    int* entityList,
                    int maxcount);
...
void SV_ClipToEntity(trace_t* trace,
                     const vec3_t start,
                     const vec3_t mins,
                     const vec3_t maxs,
                     const vec3_t end,
