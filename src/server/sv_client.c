...
void SV_GetChallenge(netadr_t from) {
  ...
  if (Sys_IsLANAddress(from)) {
    ...
    if (sv_onlyVisibleClients->integer) {
      NET_OutOfBandPrint(NS_SERVER, from, "challengeResponse %i %i",
                         challenge->challenge, sv_onlyVisibleClients->integer);
    } else {
  ...
  if (svs.time - challenge->firstTime > AUTHORIZE_TIMEOUT) {
    ...
    challenge->pingTime = svs.time;
    if (sv_onlyVisibleClients->integer) {
      NET_OutOfBandPrint(NS_SERVER, challenge->adr, "challengeResponse %i %i",
                         challenge->challenge, sv_onlyVisibleClients->integer);
    } else {
      NET_OutOfBandPrint(NS_SERVER, challenge->adr, "challengeResponse %i",
                         challenge->challenge);
    }
  ...
  if (svs.authorizeAddress.type != NA_BAD) {
    ...
    NET_OutOfBandPrint(NS_SERVER, svs.authorizeAddress,
                       ...
                       svs.challenges[i].challenge, from.ip[0], from.ip[1],
                       from.ip[2], from.ip[3], game, fs->integer);
  }
...
void SV_AuthorizeIpPacket(netadr_t from) {
  ...
  if (!Q_stricmp(s, "accept")) {
    if (sv_onlyVisibleClients->integer) {
      NET_OutOfBandPrint(NS_SERVER, svs.challenges[i].adr,
                         "challengeResponse %i %i", svs.challenges[i].challenge,
...
gotnewcl:
  ...
  *newcl = temp;
  ...
  denied = (char*)VM_Call(gvm, GAME_CLIENT_CONNECT, clientNum, qtrue,
...
void SV_DropClient(client_t* drop, const char* reason) {
  ...
  if ((!SV_GameIsSinglePlayer()) || (!isBot)) {
    ...
    SV_SendServerCommand(NULL, "cpm \"%s" S_COLOR_WHITE " %s\n\"", drop->name,
                         reason);
    //		SV_SendServerCommand( NULL, "print \"[lof]%s" S_COLOR_WHITE "
    //[lon]%s\n\"", drop->name, reason );
  }
...
void SV_WWWDownload_f(client_t* cl) {
  ...
  if (!Q_stricmp(subcmd, "done")) {
    ...
    *cl->downloadName = 0;
    ...
    Com_Printf(
        "WARNING: client '%s' reports that the redirect download for '%s' had "
        "wrong checksum.\n",
...
ftp/http didn't work (or is disabled on client's end) return true when a
redirect URL message was filled up when the cvar is set to something, the
download server will effectively never use a legacy download strategy
==================
*/
...
void SV_WriteDownloadToClient(client_t* cl, msg_t* msg) {
  ...
  if (!cl->download) {
    ...
    if (!sv_allowDownload->integer || idPack) {
      ...
      if (idPack) {
        ...
        Com_sprintf(errorMessage, sizeof(errorMessage),
                    "Cannot autodownload official pk3 file \"%s\"",
                    cl->downloadName);
      } else {
    ...
    // FIXME: we need to abstract this to an independant module for maximum
        ...
        if (!cl->bFallback) {
          ...
          if (downloadSize) {
            ...
            Q_strncpyz(cl->downloadURL,
                       va("%s/%s", sv_wwwBaseURL->string, cl->downloadName),
            ...
            // anyway
            Com_Printf(
                "ERROR: Client '%s': couldn't extract file size for %s\n",
                cl->name, cl->downloadName);
          }
        } else {
          cl->bFallback = qfalse;
  ...
  // we do this everytime because the client might change it's rate during the
  // download
...
static void SV_VerifyPaks_f(client_t* cl) {
  ...
  if (sv_pure->integer != 0) {
    ...
    bGood = (FS_FileIsInPAK(
                 FS_ShiftStr(SYS_DLLNAME_CGAME, -SYS_DLLNAME_CGAME_SHIFT),
                 &nChkSum1) == 1);
    if (bGood) {
      bGood =
          (FS_FileIsInPAK(FS_ShiftStr(SYS_DLLNAME_UI, -SYS_DLLNAME_UI_SHIFT),
                          &nChkSum2) == 1);
    }
    ...
    if (!pArg) {
      ...
      // show_bug.cgi?id=475
      // we may get incoming cp sequences from a previous checksumFeed, which we
      // need to ignore since serverId is a frame count, it always goes up
...
void SV_UserinfoChanged(client_t* cl) {
  ...
  // may be lost when client updates it's userinfo the banning code relies on
  // this being consistently present zinx - modified to always keep this
  // consistent, instead of only when "ip" is 0-length, so users can't supply
  // their own IP
...
static ucmd_t ucmds[] = {{"userinfo", SV_UpdateUserinfo_f, qfalse},
...
===============
*/
static qboolean SV_ClientCommand(client_t* cl,
                                 msg_t* msg,
                                 qboolean premaprestart) {
  int seq;
  const char* s;
...
static void SV_UserMove(client_t* cl, msg_t* msg, qboolean delta) {
  ...
  for (i = 0; i < cmdCount; i++) {
    ...
    //	continue;
    //}
    if (!SV_GameIsSinglePlayer()) {  // We need to allow this in single player,
                                     // where loadgame's can cause the player to
                                     // freeze after reloading if we do this
                                     // check
      // don't execute if this is an old cmd which is already executed
      // these old cmds are included when cl_packetdup > 0
      if (cmds[i].serverTime <= cl->lastUsercmd.serverTime) {  // Q3_MISSIONPACK
        //			if ( cmds[i].serverTime >
        //cmds[cmdCount-1].serverTime ) {
...
static void SV_ParseBinaryMessage(client_t* cl, msg_t* msg) {
  ...
  SV_GameBinaryMessageReceived(cl - svs.clients, &msg->data[msg->readcount],
                               size, cl->lastUsercmd.serverTime);
}
