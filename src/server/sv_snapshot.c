...
===============
*/
static void SV_AddEntToSnapshot(sharedEntity_t* clientEnt,
                                svEntity_t* svEnt,
                                sharedEntity_t* gEnt,
  ...
  if (gEnt->r.snapshotCallback) {
    if (!(qboolean)VM_Call(gvm, GAME_SNAPSHOT_CALLBACK, gEnt->s.number,
                           clientEnt->s.number)) {
      return;
    }
  }
...
static void SV_AddEntitiesVisibleFromPoint(
    ...
    //									snapshotEntityNumbers_t
    //*eNums, qboolean portal, clientSnapshot_t *oldframe, qboolean localClient
    //) { 									snapshotEntityNumbers_t *eNums, qboolean portal ) {
    snapshotEntityNumbers_t*
        eNums /*, qboolean portal, qboolean localClient*/) {
  int e, i;
  ...
  for (e = 0; e < sv.num_entities; e++) {
    ...
    if (ent->r.svFlags & SVF_IGNOREBMODELEXTENTS) {
      if (bitvector[svEnt->originCluster >> 3] &
          (1 << (svEnt->originCluster & 7))) {
        SV_AddEntToSnapshot(playerEnt, svEnt, ent, eNums);
      }
      continue;
    }
    ...
    if (ent->r.svFlags & SVF_PORTAL) {
      //			SV_AddEntitiesVisibleFromPoint( ent->s.origin2,
      //frame, eNums, qtrue, oldframe, localClient );
      SV_AddEntitiesVisibleFromPoint(ent->s.origin2, frame,
                                     eNums /*, qtrue, localClient*/);
    }
...
====================
*/
#define HEADER_RATE_BYTES \
  48  // include our header, IP header, and some overhead
...
void SV_SendClientSnapshot(client_t* client) {
  ...
  if (client->state < CS_ACTIVE) {
    ...
    // reliable commands (eg so they can pick up the disconnect reason)
  ...
  if (sv_showAverageBPS->integer && numclients > 0) {
    ...
    if (sv.bpsWindowSteps >= MAX_BPS_WINDOW) {
      ...
      Com_DPrintf(
          ...
          "acr(%2.2f)\n",
          ave / (float)numclients, ave, sv.bpsMaxBytes, uave, sv.ubpsMaxBytes,
          comp_ratio, sv.ucompAve / sv.ucompNum);
    }
  }
  // -NERVE - SMF
}
