...
void SV_LinkEntity(sharedEntity_t* gEnt) {
  ...
  if (!gEnt->r.bmodel && VectorCompare(gEnt->r.currentOrigin, vec3_origin)) {
    Com_DPrintf(
        "WARNING: BBOX entity is being linked at world origin, this is "
        "probably a bug\n");
  }
      ...
      if (ent->areanum != -1 && ent->areanum != area) {
        if (ent->areanum2 != -1 && ent->areanum2 != area &&
          ...
          Com_DPrintf("Object %i touching 3 areas at %f %f %f\n",
                      gEnt->s.number, gEnt->r.absmin[0], gEnt->r.absmin[1],
                      gEnt->r.absmin[2]);
        }
        ent->areanum2 = area;
      } else {
...
void SV_ClipToEntity(trace_t* trace,
  ...
  CM_TransformedBoxTrace(trace, (float*)start, (float*)end, (float*)mins,
                         (float*)maxs, clipHandle, contentmask, origin, angles,
                         capsule);
#else
  CM_TransformedBoxTrace(trace, start, end, mins, maxs, clipHandle, contentmask,
...
#else
    CM_TransformedBoxTrace(&trace, clip->start, clip->end, clip->mins,
                           clip->maxs, clipHandle, clip->contentmask, origin,
                           angles, clip->capsule);
#endif
    if (trace.allsolid) {
...
int SV_PointContents(const vec3_t p, int passEntityNum) {
  ...
  for (i = 0; i < num; i++) {
    ...
    c2 = CM_TransformedPointContents(p, clipHandle, hit->r.currentOrigin,
                                     hit->r.currentAngles);
    // Gordon: s.origin/angles is base origin/angles, need to use the current
    // origin/angles for moving entity based water, or water locks in movement
    // start position.
