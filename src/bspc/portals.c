...
void RemovePortalFromNode(portal_t* portal, node_t* l) {
  ...
  for (p = l->portals; p; p = p->next[s]) {
    ...
    portals[n] = p;
    s = (p->nodes[1] == l);
    //		if (++n >= 4096) Error("RemovePortalFromNode: more than 4096
    //portals\n");
  }  // end for
  // #endif
...
/*  //NOTE: don't use this winding ok check
   ...
   WindingErrorString()); FreeWinding(frontwinding); frontwinding = NULL; }
   //end if if (backwinding && WindingError(backwinding))
                {
                        Log_Print("SplitNodePortals: back %s\n",
   WindingErrorString()); FreeWinding(backwinding); backwinding = NULL; } //end
   if*/
#endif  // DEBUG
...
void EmitAreaPortals(node_t *headnode)
{
        ...
        for (i=1 ; i<=c_areas ; i++)
        {
                ...
                dareas[i].numareaportals = numareaportals -
dareas[i].firstareaportal; } //end for
