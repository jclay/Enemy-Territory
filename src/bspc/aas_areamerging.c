...
//
// Parameter:				seperatingface		: face that
// seperates two areas Returns:					- Changes
// Globals:		-
//===========================================================================
int AAS_TryMergeFaceAreas(tmp_face_t* seperatingface) {
  ...
  //tmparea1->areanum, tmparea2->areanum, newarea->areanum, numfaces); 	return
  //false;
  //
...
//		if (!(tmparea->settings->areaflags & AREA_GROUNDED)) continue;
                ...
                for (face = tmparea->tmpfaces; face; face = face->next[side])
                {
                        ...
                        {
                                //
                                if (face->frontarea == tmparea) othertmparea =
face->backarea; else othertmparea = face->frontarea;
//				if (!(othertmparea->settings->areaflags &
AREA_GROUNDED)) continue;
//				Log_Print("  checking area %d with %d\n",
face->frontarea, face->backarea); if (AAS_TryMergeFaceAreas(face))
                                {
        ...
        if (face->frontarea && face->backarea) {
          ...
          if ((AAS_GroundArea(tmparea) != AAS_GroundArea(othertmparea)) ||
              ...
              ((tmparea->contents & AREACONTENTS_MOVER) !=
               (othertmparea->contents & AREACONTENTS_MOVER)) ||
              ((tmparea->contents & AREACONTENTS_WATER) !=
               (othertmparea->contents & AREACONTENTS_WATER))) {
            continue;
          }
