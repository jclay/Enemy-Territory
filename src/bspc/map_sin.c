...
) side->contents |= CONTENTS_DETAIL; if (fulldetail) side->contents &=
~CONTENTS_DETAIL; if (!(side->contents & ((LAST_VISIBLE_CONTENTS-1) |
CONTENTS_PLAYERCLIP|CONTENTS_MONSTERCLIP|CONTENTS_MIST)  ) ) side->contents |=
CONTENTS_SOLID;
                ...
                //
                planenum = PlaneFromPoints (planepts[0], planepts[1],
planepts[2]); if (planenum == -1)
                {
...
CONTENTS_WATER)) )
        ...
        if (b->contents & CONTENTS_ORIGIN)
        {
                ...
                if (num_entities == 1)
                {
                        Error ("Entity %i, Brush %i: origin brushes not allowed
in world" , b->entitynum, b->brushnum); return;
                }
...
(int)origin[2]); SetKeyValue (&entities[b->entitynum], "origin", string);
...
newbrushes*sizeof(mapbrush_t));
        ...
        memcpy (mapbrushes + worldbrushes, temp, sizeof(mapbrush_t) *
newbrushes);
...
#ifdef SIN
        if (!(strlen(ValueForKey(mapent, "origin")))  && ((num_entities-1) !=
0))
                {
                mapbrush_t     *brush;
                vec3_t		    origin;
                ...
                for (j=start ; j<end ; j++)
                        ...
                        brush = &mapbrushes[j];
                        if (!brush->numsides)
                                continue;	// not a real brush (origin
brush) - shouldn't happen AddPointToBounds (brush->mins, mins, maxs);
                ...
                sprintf (string, "%i %i %i", (int)origin[0], (int)origin[1],
(int)origin[2]); SetKeyValue ( mapent, "origin", string);
...
#ifdef SIN
        if (
                 (!strcmp ("func_areaportal", ValueForKey (mapent,
"classname"))) ||
                 (!strcmp ("func_group", ValueForKey (mapent, "classname"))) ||
                (!strcmp ("detail", ValueForKey (mapent, "classname")) &&
!entitydetails)
          )
          {
                ...
                for (i=0 ; i<mapent->numbrushes ; i++)
                {
                        ...
                        for (j=0 ; j<b->numsides ; j++)
                        {
                                ...
                                newdist = mapplanes[s->planenum].dist -
                                        DotProduct
(mapplanes[s->planenum].normal, mapent->origin); s->planenum = FindFloatPlane
(mapplanes[s->planenum].normal, newdist, 0, NULL); #ifdef SIN s->texinfo =
TexinfoForBrushTexture (&mapplanes[s->planenum],
                                        &side_brushtextures[s-brushsides],
mapent->origin, &side_newrefs[s-brushsides]);
                        //
                        // save off lightinfo
                        //
                        s->lightinfo = LightinfoForBrushTexture (
&side_brushtextures[s-brushsides] ); #else s->texinfo = TexinfoForBrushTexture
(&mapplanes[s->planenum], &side_brushtextures[s-brushsides], mapent->origin);
#endif
                        }
                        MakeBrushWindings (b);
                }
        ...
        if (!strcmp ("detail", ValueForKey (mapent, "classname")) &&
!entitydetails)
        {
                for (i=0 ; i<mapent->numbrushes ; i++)
                {
                 ...
                 if (!fulldetail)
                  ...
                  // set the contents for the entire brush
                         b->contents |= CONTENTS_DETAIL;
                           ...
                           for (j=0, s=b->original_sides ; j<b->numsides ;
j++,s++)
                           {
                           s->contents |= CONTENTS_DETAIL;
                        }
...
j++,s++)
        ...
        if (!strcmp ("func_areaportal", ValueForKey (mapent, "classname")))
        {
                ...
                if (mapent->numbrushes != 1)
                        Error ("Entity %i: func_areaportal can only be a single
brush", num_entities-1);
...
void Sin_BSPBrushToMapBrush(sin_dbrush_t* bspbrush, entity_t* mapent) {
  ...
  for (n = 0; n < bspbrush->numsides; n++) {
    ...
    if (!(side->contents & ((LAST_VISIBLE_CONTENTS - 1) | CONTENTS_PLAYERCLIP |
                            CONTENTS_MONSTERCLIP | CONTENTS_MIST))) {
    ...
    // keep this side
    //
    // ME: reset pointer to side, why? hell I dunno (pointer is set above
    // already)
    side = b->original_sides + b->numsides;
