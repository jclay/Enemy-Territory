...
void DefaultCfg(void) {
  ...
  //	cfg.phys_maxbarrier			= -999;//32;	// RIDAH: this
  //is calculated at run-time now, from the gravity and jump velocity settings
}  // end of the function DefaultCfg
#endif
...
int LoadCfgFile(char* filename) {
  ...
  while (PC_ReadToken(source, &token)) {
    if (!stricmp(token.string, "bbox")) {
      ...
      if (!ReadStructure(source, &bbox_struct,
                         (char*)&cfg.bboxes[cfg.numbboxes])) {
        FreeSource(source);
        return false;
      }  // end if
