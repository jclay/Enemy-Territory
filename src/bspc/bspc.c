...
// Ryan, I changed this from c to d so our designers would know which .exe they
// had...
...
qboolean
    lessbrushes;  // create less brushes instead of correct texture placement
qboolean cancelconversion;   // true if the conversion is being cancelled
qboolean noliquids;          // no liquids when writing map file
qboolean forcesidesvisible;  // force all brush sides to be visible when loaded
                             // from bsp
...
qboolean capsule_collision =
    false;  // capsule collision// Ridah, allow to specify an extension for
            // multiple AAS files per map
int mingroundarea =
    0;  // areas with ground surface area less than this will be removed
...
void ProcessModels (void)
{
        ...
        for (entity_num = 0; entity_num < num_entities; entity_num++)
        {
                ...
                Log_Print("############### model %i ###############\n",
nummodels); BeginModel(); if (entity_num == 0) ProcessWorldModel(); else
ProcessSubModel(); EndModel();

                if (!verboseentities)
                        verbose = false;	// don't bother printing
submodels } //end for EndBSPFile(); } //end of the function ProcessModels
//===========================================================================
//
// Parameter:				-
// Returns:					-
...
int main(int argc, char** argv) {
  ...
  if (argc > 1 && i == argc) {
    ...
    Log_Print(
        ...
        //"   aasall   <quake3folder>              = create AAS files for all
        //BSPs\n"
        "   reach    <filter.bsp>                = compute reachability & "
        "clusters\n"
        "   cluster  <filter.aas>                = compute clusters\n"
        "   aasopt   <filter.aas>                = optimize aas file\n"
        //"   tetra    <filter.aas>                = tetrahedral
        //decomposition\n"
        ...
        "   forcesidesvisible                    = force all sides to be "
        "visible\n"
        "   grapplereach                         = calculate grapple "
        "reachabilities\n"
        ...
        /*			"   glview     = output a GL view\n"
                                ...
                                "   onlyents   = only compile entities with
           bsp\n" "   micro <volume>\n" "              = sets the micro volume
           to the given float\n" "   leaktest   = perform a leak test\n" "
           verboseentities\n" "              = enable entity verbose mode\n" "
           chop <subdivide_size>\n" "              = sets the subdivide size to
           the given float\n"*/
        "\n");
  }             // end else
