...
//			because the bsp tree isn't refreshes like with ladder
//subdivision
...
winding_t* AAS_SplitWinding(tmp_area_t* tmparea, int planenum) {
  ...
  for (face = tmparea->tmpfaces; face && splitwinding;
    ...
    ChopWindingInPlace(&splitwinding, plane->normal, plane->dist,
                       0);  // PLANESIDE_EPSILON);
  }                         // end for
  return splitwinding;
...
void AAS_SplitArea(tmp_area_t* tmparea,
  ...
  normal[1], normal[2], dist); AAS_TestSplitPlane(areanum, normal, dist,
  &facesplits, &groundsplits, &epsilonface); Log_Print("face splits = %d\nground
  splits = %d\n", facesplits, groundsplits); if (epsilonface) Log_Print("aaahh
  epsilon face\n"); #endif //AW_DEBUG*/
  // the original area
  ...
  /*	//create a split winding, first base winding for plane
          splitwinding = BaseWindingForPlane(splitplane->normal,
     splitplane->dist);
          //chop with all the faces of the area
          for (face = tmparea->tmpfaces; face && splitwinding; face =
     face->next[side])
          {
                  //side of the face the original area was on
                  ...
                  plane = &mapplanes[face->planenum ^ side];
                  ChopWindingInPlace(&splitwinding, plane->normal, plane->dist,
     0); // PLANESIDE_EPSILON); } //end for*/
  splitwinding = AAS_SplitWinding(tmparea, planenum);
  if (!splitwinding) {
    /*
    #ifdef DEBUG
                    AAS_TestSplitPlane(areanum, normal, dist, &facesplits,
    &groundsplits, &epsilonface); Log_Print("\nface splits = %d\nground splits =
    %d\n", facesplits, groundsplits); if (epsilonface) Log_Print("aaahh epsilon
    face\n"); #endif //DEBUG*/
  ...
  for (face = tmparea->tmpfaces; face; face = nextface) {
    ...
    if (faceotherarea) {
      ...
      if (backface) {
       ...
       // add the front and back part left after splitting the original face to
       // the new areas
    if (frontface) {
...
int AAS_FindBestAreaSplitPlane(tmp_area_t* tmparea,
  ...
  for (face1 = tmparea->tmpfaces; face1; face1 = face1->next[side1]) {
    ...
    for (face2 = face1->next[side1]; face2; face2 = face2->next[side2]) {
      ...
      if (!FindPlaneSeperatingWindings(face1->winding, face2->winding,
                                       invgravity, tmpnormal, &tmpdist,
                                       points)) {
        continue;
      }
      ...
      Log_Print("normal = \'%f %f %f\', dist = %f\n", tmpnormal[0],
                tmpnormal[1], tmpnormal[2], tmpdist);
#endif  // AW_DEBUG

      // get metrics for this vertical plane
      if (!AAS_TestSplitPlane(tmparea, tmpnormal, tmpdist, &facesplits,
                              &groundsplits, &epsilonfaces, points)) {
        continue;
      }  // end if
...
tmp_node_t* AAS_RefreshLadderSubdividedTree_r(tmp_node_t* tmpnode,
  ...
  if (tmpnode->tmparea) {
    ...
    return tmpnode;
     ...
     // do the children recursively
  tmpnode->children[0] = AAS_RefreshLadderSubdividedTree_r(
      tmpnode->children[0], tmparea, tmpnode1, tmpnode2, planenum);
  tmpnode->children[1] = AAS_RefreshLadderSubdividedTree_r(
      tmpnode->children[1], tmparea, tmpnode1, tmpnode2, planenum);
  return tmpnode;
...
tmp_node_t* AAS_LadderSubdivideArea_r(tmp_node_t* tmpnode) {
  ...
  if (tmparea->contents &
      (AREACONTENTS_WATER | AREACONTENTS_LAVA | AREACONTENTS_SLIME)) {
    return tmpnode;
  }
