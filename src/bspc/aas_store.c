...
#define VERTEX_HASH_SIZE \
  ((MAX_MAP_BOUNDS >> (VERTEX_HASH_SHIFT - 1)) + 1)  // was 64
//
...
int aas_hashverts[VERTEX_HASH_SIZE *
                  VERTEX_HASH_SIZE];  // a vertex number, or 0 for no verts
// plane hash
int* aas_planechain;
...
void AAS_AllocMaxAAS(void) {
  ...
  (*aasworld).vertexes = (aas_vertex_t*)GetClearedMemory(max_aas.max_vertexes *
                                                         sizeof(aas_vertex_t));
  allocatedaasmem += max_aas.max_vertexes * sizeof(aas_vertex_t);
  // planes
  (*aasworld).numplanes = 0;
  (*aasworld).planes =
      (aas_plane_t*)GetClearedMemory(max_aas.max_planes * sizeof(aas_plane_t));
  ...
  (*aasworld).edgeindex = (aas_edgeindex_t*)GetClearedMemory(
      max_aas.max_edgeindexsize * sizeof(aas_edgeindex_t));
  ...
  (*aasworld).faceindex = (aas_faceindex_t*)GetClearedMemory(
      max_aas.max_faceindexsize * sizeof(aas_faceindex_t));
  ...
  (*aasworld).areasettings = (aas_areasettings_t*)GetClearedMemory(
      max_aas.max_areasettings * sizeof(aas_areasettings_t));
...
qboolean AAS_GetFace(winding_t *w, plane_t *p, int side, int *facenum)
{
        ...
        for (i = 0; i < w->numpoints; i++)
        {
                ...
                foundedges &= AAS_GetEdge(w->p[i], w->p[(i+1 >= w->numpoints ? 0
: i+1)], &edges[i]); } //end for
                ...
                for (i = 0; i < (*aasworld).numfaces; i++)
                {
                        ...
                        if (planenum == face->planenum)
                        {
                                if (numedges == face->numedges)
                                {
                                        ...
                                        {
                                                edgenum =
abs((*aasworld).edgeindex[face->firstedge + j]); if (abs(edges[i]) != edgenum)
break; } //end for if (j == numedges)
                                        {
        ...
        for (i = 0; i < numedges; i++)
        {
                if ((*aasworld).edgeindexsize >= max_aas.max_edgeindexsize)
                {
                        Error("AAS_MAX_EDGEINDEXSIZE = %d",
max_aas.max_edgeindexsize); } //end if
...
void AAS_StoreAreaSettings(tmp_areasettings_t* tmpareasettings) {
  ...
  areasettings->contents |=
      (tmpareasettings->modelnum & AREACONTENTS_MAXMODELNUM)
...
int AAS_StoreArea(tmp_area_t* tmparea) {
  ...
  //aasarea->areanum); store the aas area number at the tmp area
  ...
  //				aasarea->center[0], aasarea->center[1],
  //aasarea->center[2]); store the area settings
  ...
  //
  if ((*aasworld).areasettings[aasarea->areanum].contents &
