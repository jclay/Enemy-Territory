      ...
      for (axis = 0; axis < 3; axis++) {
        for (dir = -1; dir <= 1; dir += 2) {
          ...
          for (k = 0; k < b->numsides; k++) {
            ...
            if (PlaneEqual(&mapplanes[b->original_sides[k].planenum], normal,
                           dist)) {
              break;
            }
    ...
    if (!(s->flags & SFL_BEVEL)) {
      ...
      if (s->texinfo == TEXINFO_NODE) {
        if (brush->contents & CONTENTS_PLAYERCLIP) {
          ...
          else if (loadedmaptype ==
                   MAPTYPE_QUAKE2) {  // FIXME: don't always use e1u1
      ...
      /*			else if (loadedmaptype == MAPTYPE_SIN &&
         s->texinfo == 0)
                              {
                                      if (brush->contents & CONTENTS_DUMMYFENCE)
                                      {
                                              if (fprintf(fp,
         "generic/misc/fence 0 0 0 1 1") < 0) return false; } //end if else if
         (brush->contents & CONTENTS_MIST)
                                      {
                                              if (fprintf(fp,
         "generic/misc/volumetric_base 0 0 0 1 1") < 0) return false; } //end if
                                      else //unknown so far
                                      {
                                              if (fprintf(fp, "generic/misc/red
         0 0 0 1 1") < 0) return false; } //end else } //end if*/
      else if (loadedmaptype == MAPTYPE_QUAKE3) {
        ...
        if (fprintf(fp, "%s %d %d %d", ti->texture, shift[0], shift[1],
                    rotate) < 0) {
          return false;
        }
        ...
        if (loadedmaptype == MAPTYPE_QUAKE2) {
          if (fprintf(fp, " %ld %ld %ld", s->contents, ti->flags, ti->value) <
              0) {
            return false;
          }
        }  // end if
...
qboolean WriteOriginBrush(FILE* fp, vec3_t origin) {
  ...
  for (i = 0; i < 3; i++) {
    for (s = -1; s <= 1; s += 2) {
      ...
      // three non-colinear points to define the plane
      if (fprintf(fp, "  ( %5i %5i %5i ) ", (int)w->p[0][0], (int)w->p[0][1],
                  (int)w->p[0][2]) < 0) {
        return false;
      }
      if (fprintf(fp, "( %5i %5i %5i ) ", (int)w->p[1][0], (int)w->p[1][1],
                  (int)w->p[1][2]) < 0) {
        return false;
      }
      if (fprintf(fp, "( %5i %5i %5i ) ", (int)w->p[2][0], (int)w->p[2][1],
...
int LoadMapFromBSP(struct quakefile_s* qf) {
  ...
  if (ReadQuakeFile(qf, &idheader, 0, sizeof(idheader_t)) !=
      sizeof(idheader_t)) {
    return false;
  }  // end if
  ...
  if (idheader.ident == Q3_BSP_IDENT && idheader.version == Q3_BSP_VERSION) {
    ...
    Error("unknown BSP format %c%c%c%c, version %d\n", (idheader.ident & 0xFF),
          ((idheader.ident >> 8) & 0xFF), ((idheader.ident >> 16) & 0xFF),
