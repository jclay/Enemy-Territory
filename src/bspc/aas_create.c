...
extern int mingroundarea;  // areas with ground surface area less than this will
                           // be removed
...
void AAS_CheckArea(tmp_area_t* tmparea) {
  ...
  for (face = tmparea->tmpfaces; face; face = face->next[side]) {
    ...
    if (fabs(dist - plane->dist) > 0.4 ||
      ...
      Log_Write(
          "AAS_CheckArea: area %d face %d winding plane unequal to face "
          "plane\r\n",
...
void AAS_CheckFaceWindingPlane(tmp_face_t* face) {
  ...
  if (fabs(dist - plane->dist) > 0.4 ||
    ...
    if (fabs(dist - plane->dist) > 0.4 ||
        ...
        fabs(normal[2] - plane->normal[2]) > 0.0001) {
      Log_Write(
          "AAS_CheckFaceWindingPlane: face %d winding plane unequal to face "
          "plane\r\n",
          face->num);
      //
  ...
  for (tmparea = tmpaasworld.areas; tmparea; tmparea = tmparea->l_next) {
    ...
    for (face = tmparea->tmpfaces; face; face = face->next[side]) {
      ...
      if (face->faceflags & FACE_GROUND) {
        tmparea->settings->groundsteepness +=
            (1.0 - mapplanes[face->planenum ^ side].normal[2]);
        count++;
      }
    }  // end for
    ...
    if (tmparea->contents &
        (AREACONTENTS_WATER | AREACONTENTS_SLIME | AREACONTENTS_LAVA)) {
...
tmp_node_t* AAS_CreateArea(node_t* node) {
  ...
  for (p = node->portals; p; p = p->next[pside]) {
    ...
    /*
                    ...
                    // RF, if this face has a solid at the other side, and it is
       not a GROUND, then we cannot free if its too small if (allowFreeIfSmall
       && (p->tmpface->faceflags & FACE_SOLID) && !(p->tmpface->faceflags &
       FACE_GROUND)) {
                            // make sure it's not a ceiling
                            if (!(DotProduct(cfg.phys_gravitydirection,
       mapplanes[p->tmpface->planenum].normal) > cfg.phys_maxsteepness)) {
                                    allowFreeIfSmall = 0;
                            }
                    }
    */
  ...
  /*
          // RF, if we are using only ground areas, then ignore areas that arent
     grounded or attached to ladders, or underwater if (groundonly &&
     !(areafaceflags & (FACE_GROUND|FACE_LADDER)) && !(tmparea->contents &
     (AREACONTENTS_WATER|AREACONTENTS_SLIME|AREACONTENTS_LAVA))) {
                  numGroundOnlyFreed++;
  ...
  */
  /*
          // RF, if this is a really small area, and it is surrounded by
     non-solid portals, then nuke it if (allowFreeIfSmall && !(areafaceflags &
     (FACE_LADDER)) && VectorDistance( mins, maxs ) < 32) { numTinyFreed++;
                  tmparea->invalid = true;
          }
  */
...
void AAS_CheckAreaSharedFaces(tmp_area_t* tmparea1, tmp_area_t* tmparea2) {
  ...
  if (numsharedfaces > 1) {
    ...
    for (face1 = tmparea1->tmpfaces; face1; face1 = face1->next[side]) {
      ...
      if (face1->backarea == tmparea2 || face1->frontarea == tmparea2) {
        Log_Write(
            "face %d, planenum = %d, face->frontarea = %d face->backarea = "
            "%d\r\n",
...
void AAS_FlipAreaSharedFaces(tmp_area_t *tmparea1, tmp_area_t *tmparea2)
{
        ...
        sharedface = NULL;
        numsharedfaces = 0;
        area1facing = 0;		//number of shared faces facing towards
area 1 area2facing = 0;		//number of shared faces facing towards area 2
        for (face1 = tmparea1->tmpfaces; face1; face1 = face1->next[side])
        {
                side = face1->frontarea != tmparea1;
                if (face1->backarea == tmparea2 || face1->frontarea == tmparea2)
        ...
        if (numsharedfaces == area1facing || numsharedfaces == area2facing)
return;
        //
        do
        {
                for (face1 = tmparea1->tmpfaces; face1; face1 =
face1->next[side])
                {
                        side = face1->frontarea != tmparea1;
                        if (face1->backarea == tmparea2 || face1->frontarea ==
tmparea2)
                        {
...
void AAS_FlipSharedFaces(void)
{
        ...
        for (tmparea1 = tmpaasworld.areas; tmparea1; tmparea1 =
tmparea1->l_next)
        {
                if (tmparea1->invalid) continue;
                for (tmparea2 = tmpaasworld.areas; tmparea2; tmparea2 =
tmparea2->l_next)
                {
...
void AAS_RemoveTinyAreas(void) {
  // RF, disabled, can cause problems, should do checks on minsidelength
  // instead, and also this should be a post-clustering
