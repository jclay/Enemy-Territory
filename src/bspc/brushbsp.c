...
// if a brush just barely pokes onto the other side, let it slide by without
// chopping
...
qboolean WriteBSPBrush(FILE* fp, bspbrush_t* brush) {
  ...
  for (sn = 0; sn < brush->numsides; sn++) {
    ...
    if (fprintf(fp, "  ( %5i %5i %5i ) ", (int)w->p[p1][0], (int)w->p[p1][1],
                (int)w->p[p1][2]) < 0) {
      return false;
    }
    if (fprintf(fp, "( %5i %5i %5i ) ", (int)w->p[!p1][0], (int)w->p[!p1][1],
    ...
    if (fprintf(fp, "( %5i %5i %5i ) ", (int)w->p[2][0], (int)w->p[2][1],
                (int)w->p[2][2]) < 0) {
      return false;
    }
    // free the winding
...
static int MrE_BoxOnPlaneSide(vec3_t mins,
                              vec3_t maxs,
                              plane_t* plane,
                              float epsilon) {
  vec3_t center;
...
// Changes Globals:		-
//===========================================================================
int TestBrushToPlanenum(bspbrush_t* brush,
                        int planenum,
                        int* numsplits,
                        qboolean* hintsplit,
                        int* epsilonbrush) {
...
side_t* SelectSplitSide(bspbrush_t* brushes, node_t* node) {
  ...
  for (pass = 0; pass < numpasses; pass++) {
    for (brush = brushes; brush; brush = brush->next) {
      ...
      //			if ( (pass & 1) && !(brush->original->contents &
      //CONTENTS_DETAIL) ) 				continue; 			if ( !(pass & 1) &&
      //(brush->original->contents & CONTENTS_DETAIL) ) 				continue;
      for (i = 0; i < brush->numsides; i++) {
        side = brush->sides + i;
//				if (side->flags & SFL_BEVEL)
//					continue;	// never use a bevel as
//a spliter
#ifndef MRE_ET
        if (!side->winding) {
        ...
        //					continue;	// skip surfaces
        //are never chosen 				if (!(side->flags & SFL_VISIBLE) && (pass < 2))
        //					continue;	// only check
        //visible faces on first pass
        ...
        if (mapplanes[pnum].type < 3) {
          value = 20 * facing - 10 * splits - abs(front - back) -
                  epsilonbrush * 1000;
        } else {
          value = 15 * facing - 10 * splits - abs(front - back) -
                  epsilonbrush * 1000;
        }
...
int SplitBrush(bspbrush_t* brush,
               int planenum,
               bspbrush_t** front,
               bspbrush_t** back) {
  int res, i, j;
  ...
  for (i = 0; i < brush->numsides && mid; i++) {
    ...
    ChopWindingInPlace(&mid, splitPlane->normal, splitPlane->dist,
                       BRUSH_EPSILON);
  }
  ...
  *back = AllocBrush(brush->numsides + 1);
  ...
  for (i = 0; i < brush->numsides; i++) {
    ...
    // if completely at the back
      ...
      *cs = *side;
      ...
      // split the side
      ClipWindingEpsilon(side->winding, plane->normal, plane->dist,
                         BRUSH_EPSILON, &frontSide, &backSide);
      if (frontSide) {
        cs = &(*front)->sides[(*front)->numsides];
        (*front)->numsides++;
        *cs = *side;
        ...
        *cs = *side;
        cs->winding = CopyWinding(side->winding);
        ChopWindingInPlace(&cs->winding, plane->normal,
                           plane->dist - (BRUSH_EPSILON + 0.02f), 0.01f);
        assert(cs->winding);
        cs->flags &= ~SFL_TESTED;
      }
      if (backSide) {
        ...
        *cs = *side;
        ...
        ChopWindingInPlace(&cs->winding, normal,
                           -(plane->dist + (BRUSH_EPSILON + 0.02f)), 0.01f);
        assert(cs->winding);
        cs->flags &= ~SFL_TESTED;
      }
  ...
  /*
  {
          ...
          for ( i = 0; i < 2 ; i++ ) {
                  b = i ? *front : *back;
                  for ( j = 0; j < 3; j++ ) {
                          if ( b->mins[j] < -MAX_MAP_BOUNDS || b->maxs[j] >
  MAX_MAP_BOUNDS ) { break;
                          }
                  }
...
void BuildGrid_r(node_t* node) {
  ...
  for (axis = 0; axis < 3; axis++) {
    if (halfSize[axis] > bsp_grid_size) {
      dist =
          bsp_grid_size *
          (floor((node->volume->mins[axis] + halfSize[axis]) / bsp_grid_size) +
           1);
    } else {
      dist =
          bsp_grid_size * (floor(node->volume->mins[axis] / bsp_grid_size) + 1);
    }

    if (dist > node->volume->mins[axis] + 1.0f &&
        dist < node->volume->maxs[axis] - 1.0f) {
      break;
    }
  }
  ...
  node->side = NULL;
  node->planenum = FindFloatPlane(normal, (int)dist, 0, NULL) &
                   ~1;  // always use front facing
  ...
  for (brush = node->brushlist; brush; brush = brush->next) {
    splits = 0;
    brush->side =
        TestBrushToPlanenum(brush, node->planenum, &splits, &splits, &splits);
  }

  // split the brush list in two for both children
  SplitBrushList(node->brushlist, node, &node->children[0]->brushlist,
                 &node->children[1]->brushlist);
...
tree_t* BrushBSP(bspbrush_t* brushlist, vec3_t mins, vec3_t maxs) {
  ...
  if (numthreads == 1) {
    ...
    //10); 		Log_Print("%6i KB of winding memory\n", WindingMemory() >> 10);
