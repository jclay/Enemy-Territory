...
// Changes Globals:		-
//===========================================================================
vec_t BoxOriginDistanceFromPlane(vec3_t normal,
                                 vec3_t mins,
                                 vec3_t maxs,
                                 int side) {
  vec3_t v1, v2;
...
void AAS_SetTexinfo(mapbrush_t* brush) {
  ...
  if (brush->contents &
      (CONTENTS_LADDER | CONTENTS_AREAPORTAL | CONTENTS_CLUSTERPORTAL |
       CONTENTS_TELEPORTER | CONTENTS_JUMPPAD | CONTENTS_DONOTENTER |
       CONTENTS_DONOTENTER_LARGE | CONTENTS_WATER | CONTENTS_LAVA |
       CONTENTS_SLIME | CONTENTS_WINDOW | CONTENTS_PLAYERCLIP |
       CONTENTS_MONSTERCLIP | CONTENTS_MOVER)) {
    ...
    for (n = 0; n < brush->numsides; n++) {
      side = brush->original_sides + n;
      // don't use side as splitter (set texinfo to TEXINFO_NODE) if not
      // textured
  ...
  for (i = 0; i < brush->numsides; i++) {
    for (j = 0; j < brush->numsides; j++) {
      ...
      if (WindingsNonConvex(brush->original_sides[i].winding,
                            brush->original_sides[j].winding, plane1->normal,
  ...
  for (i = 0; i < ob->numsides; i++) {
    for (j = 0; j < ob->numsides; j++) {
      ...
      if (WindingsNonConvex(ob->original_sides[i].winding,
                            ob->original_sides[j].winding, plane1->normal,
                            plane2->normal, plane1->dist, plane2->dist)) {
        Log_Print("non convex brush");
      }  // end if
    }    // end for
...
int AAS_AlwaysTriggered(char* targetname) {
  ...
  for (i = 0; i < num_entities; i++) {
    ...
    if (!strcmp(targetname, ValueForKey(&entities[i], "target"))) {
      ...
      if (!(atoi(ValueForKey(&entities[i], "spawnflags")) &
...
int AAS_ValidEntity(entity_t* mapent) {
  ...
  if (mapent == &entities[0]) {
    ...
    // if (!(atoi(ValueForKey(mapent, "spawnflags")) &
    // SPAWNFLAG_NOT_DEATHMATCH))
    {
    ...
    if (strcmp("reardump", ValueForKey(mapent, "targetname"))) {
      Log_Print(
          "\n\n\nTEMP!!!\nTEMP!!!\nIgnoring all func_constructibles except "
          "\"reardump\"\nTEMP!!!\nTEMP!!!\n\n\n");
      return false;
    }
...
void AAS_PositionBrush(entity_t* mapent, mapbrush_t* brush) {
  ...
  if (!strcmp(ValueForKey(mapent, "classname"), "func_door_rotating")) {
    ...
    if (mapent->origin[0] || mapent->origin[1] || mapent->origin[2]) {
      for (i = 0; i < brush->numsides; i++) {
        ...
        s->planenum =
            FindFloatPlane(mapplanes[s->planenum].normal, newdist, 0, NULL);
      }
    }
  }
  ...
  if (!strcmp("trigger_push", ValueForKey(mapent, "classname"))) {
    ...
    // Log_Print("found trigger_teleport teleporter brush\n");
           ...
           !strcmp("func_door_rotating", ValueForKey(mapent, "classname"))) {
    // set mover contents
...
void AAS_CreateMapBrushes(mapbrush_t* brush, entity_t* mapent, int addbevels) {
  ...
  */
  ...
  if (!(brush->contents &
        (CONTENTS_SOLID | CONTENTS_LADDER | CONTENTS_CLUSTERPORTAL |
         CONTENTS_DONOTENTER | CONTENTS_DONOTENTER_LARGE | CONTENTS_TELEPORTER |
         CONTENTS_JUMPPAD | CONTENTS_WATER | CONTENTS_LAVA | CONTENTS_SLIME |
         CONTENTS_MOVER))) {
    nummapbrushsides -= brush->numsides;
    brush->numsides = 0;
    return;
     ...
     // if brush bevels should be added (for real map brushes, not bsp map
     // brushes)
  if (addbevels) {
    // NOTE: we first have to get the mins and maxs of the brush before
    //			creating the brush bevels... the mins and maxs are used
    //to 			create them. so we call MakeBrushWindings to get the mins 			and maxs and
    //then after creating the bevels we free the 			windings because they are
    //created for all sides (including 			bevels) a little later
    AAS_MakeBrushWindings(brush);
    AddBrushBevels(brush);
    FreeBrushWindings(brush);
  }  // end if
  ...
  if (brush->contents &
      (CONTENTS_WATER | CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_TELEPORTER |
       CONTENTS_JUMPPAD | CONTENTS_DONOTENTER | CONTENTS_DONOTENTER_LARGE |
       CONTENTS_MOVER)) {
