...
// Changes Globals:		-
//===========================================================================
void Q3_PlaneFromPoints(vec3_t p0,
                        vec3_t p1,
                        vec3_t p2,
                        vec3_t normal,
                        float* dist) {
...
void Q3_SurfacePlane(q3_dsurface_t* surface, vec3_t normal, float* dist) {
  ...
  /*
          ...
          {
                  p0 = q3_drawVerts[surface->firstVert + ((i) %
     surface->numVerts)].xyz; p1 = q3_drawVerts[surface->firstVert + ((i+1) %
     surface->numVerts)].xyz; p2 = q3_drawVerts[surface->firstVert + ((i+2) %
     surface->numVerts)].xyz; VectorSubtract(p0, p1, t1); VectorSubtract(p2, p1,
     t2); VectorNormalize(t1); VectorNormalize(t2); dot = DotProduct(t1, t2); if
     (dot > -0.9 && dot < 0.9 && VectorLength(t1) > 0.1 && VectorLength(t2) >
     0.1) break; } //end for CrossProduct(t1, t2, normal);
          VectorNormalize(normal);
  */
...
void Q3_CreatePlanarSurfacePlanes(void) {
  ...
  q3_surfaceplanes =
      (q3_dplane_t*)GetClearedMemory(q3_numDrawSurfaces * sizeof(q3_dplane_t));
  ...
  for (i = 0; i < q3_numDrawSurfaces; i++) {
    ...
    // q3_surfaceplanes[i].normal[0], 											q3_surfaceplanes[i].normal[1],
    //											q3_surfaceplanes[i].normal[2],
    //q3_surfaceplanes[i].dist);
  }  // end for
...
void Q3_SurfacePlane(q3_dsurface_t *surface, vec3_t normal, float *dist)
{
        ...
        Q3_PlaneFromPoints(q3_drawVerts[surface->firstVert].xyz,
                                                ...
                                                q3_drawVerts[surface->firstVert+2].xyz,
normal, dist); } //end of the function Q3_SurfacePlane*/
//===========================================================================
...
void Q3_FindVisibleBrushSides(void) {
  ...
  if (forcesidesvisible) {
    ...
    for (i = 0; i < q3_numbrushes; i++) {
      brush = &q3_dbrushes[i];
      // if one of the brush sides uses the terrain shader mark all sides as
      // visible
      ...
      for (j = 0; j < brush->numSides; j++) {
        ...
        if (!w) {
          ...
          if (WindingIsTiny(w)) {
            ...
            we = WindingError(w);
            if (we == WE_NOTENOUGHPOINTS || we == WE_SMALLAREA ||
        ...
        for (k = 0; k < q3_numDrawSurfaces; k++) {
          ...
          // the surface plane and the brush side plane should be pretty much
          // the same
          if (fabs(fabs(plane->dist) -
...
void Q3_SwapBSPFile(void) {
  ...
  for (i = 0; i < q3_numShaders; i++) {
    ...
    q3_dshaders[i].contentFlags &=
        (CONTENTS_SOLID | CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER |
         CONTENTS_AREAPORTAL | CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP |
         CONTENTS_DETAIL | CONTENTS_CLUSTERPORTAL);
  }
...
void Q3_LoadBSPFile(struct quakefile_s* qf) {
  ...
  q3_numShaders = Q3_CopyLump(header, Q3_LUMP_SHADERS, (void*)&q3_dshaders,
                              sizeof(q3_dshader_t));
  q3_nummodels = Q3_CopyLump(header, Q3_LUMP_MODELS, (void*)&q3_dmodels,
                             sizeof(q3_dmodel_t));
  q3_numplanes = Q3_CopyLump(header, Q3_LUMP_PLANES, (void*)&q3_dplanes,
                             sizeof(q3_dplane_t));
  q3_numleafs =
      Q3_CopyLump(header, Q3_LUMP_LEAFS, (void*)&q3_dleafs, sizeof(q3_dleaf_t));
  q3_numnodes =
      Q3_CopyLump(header, Q3_LUMP_NODES, (void*)&q3_dnodes, sizeof(q3_dnode_t));
  q3_numleafsurfaces =
  ...
  q3_numbrushes = Q3_CopyLump(header, Q3_LUMP_BRUSHES, (void*)&q3_dbrushes,
                              sizeof(q3_dbrush_t));
  q3_numbrushsides =
      Q3_CopyLump(header, Q3_LUMP_BRUSHSIDES, (void*)&q3_dbrushsides,
                  sizeof(q3_dbrushside_t));
  q3_numDrawVerts = Q3_CopyLump(header, Q3_LUMP_DRAWVERTS, (void*)&q3_drawVerts,
                                sizeof(q3_drawVert_t));
  q3_numDrawSurfaces = Q3_CopyLump(
      header, Q3_LUMP_SURFACES, (void*)&q3_drawSurfaces, sizeof(q3_dsurface_t));
  q3_numFogs =
