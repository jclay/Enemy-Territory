...
void DPlanes2MapPlanes(void) {
  ...
  for (i = 0; i < numplanes; i++) {
    dplanes2mapplanes[i] =
        FindFloatPlane(dplanes[i].normal, dplanes[i].dist, 0, NULL);
  }  // end for
...
void Q2_ParseBrush(script_t* script, entity_t* mapent) {
  ...
  do {
    ...
    side->texinfo =
        TexinfoForBrushTexture(&mapplanes[planenum], &td, vec3_origin);
  ...
  if (create_aas) {
    ...
    // NOTE: if we return here then duplicate plane errors occur for the non
    // world entities
    return;
  }     // end if
#endif  // ME
...
void Q2_BSPBrushToMapBrush(dbrush_t* bspbrush, entity_t* mapent) {
  ...
  for (n = 0; n < bspbrush->numsides; n++) {
    ...
    for (k = 0; k < b->numsides; k++) {
      s2 = b->original_sides + k;
      //			if (DotProduct (mapplanes[s2->planenum].normal,
      //mapplanes[planenum].normal) > 0.999
      //							&&
