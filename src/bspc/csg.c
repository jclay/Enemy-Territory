...
void CheckBSPBrush(bspbrush_t* brush) {
  ...
  for (i = 0; i < brush->numsides; i++) {
    for (j = 0; j < brush->numsides; j++) {
      ...
      //
      if (WindingsNonConvex(brush->sides[i].winding, brush->sides[j].winding,
                            plane1->normal, plane2->normal, plane1->dist,
                            plane2->dist)) {
...
bspbrush_t* TryMergeBrushes(bspbrush_t* brush1, bspbrush_t* brush2) {
  ...
  for (i = 0; i < brush1->numsides; i++) {
    ...
    for (j = 0; j < brush2->numsides; j++) {
      ...
      //
      if (WindingsNonConvex(side1->winding, side2->winding, plane1->normal,
...
// Changes Globals:		-
//===========================================================================
void SplitBrush2(bspbrush_t* brush,
                 int planenum,
                 bspbrush_t** front,
                 bspbrush_t** back) {
...
bspbrush_t* ClipBrushToBox(bspbrush_t* brush,
  ...
  for (i = 0; i < brush->numsides; i++) {
    ...
    if (p == maxplanenums[0] || p == maxplanenums[1] || p == minplanenums[0] ||
        p == minplanenums[1]) {
...
mapplanes[s->planenum].dist);
                        ...
                        fprintf (f,"( %i %i %i ) ", (int)w->p[0][0],
(int)w->p[0][1], (int)w->p[0][2]); fprintf (f,"( %i %i %i ) ", (int)w->p[1][0],
(int)w->p[1][1], (int)w->p[1][2]); fprintf (f,"( %i %i %i ) ", (int)w->p[2][0],
(int)w->p[2][1], (int)w->p[2][2]);

                        fprintf (f, "%s 0 0 0 1 1\n",
texinfo[s->texinfo].texture); FreeWinding (w);
                }
                fprintf (f, "}\n");
        }
        fprintf (f, "}\n");
...
__inline qboolean BrushGE(bspbrush_t* b1, bspbrush_t* b2) {
  ...
  if (b1->original->contents == CONTENTS_MOVER &&
      b2->original->contents == CONTENTS_MOVER) {
    return true;
  }
#ifdef ME
  if (create_aas) {
