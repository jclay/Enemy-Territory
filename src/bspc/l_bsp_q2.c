...
void Q2_AllocMaxBSP(void) {
  ...
  // leaf faces
  numleaffaces = 0;
  dleaffaces = (unsigned short*)GetClearedMemory(MAX_MAP_LEAFFACES *
                                                 sizeof(unsigned short));
  allocatedbspmem += MAX_MAP_LEAFFACES * sizeof(unsigned short);
  // leaf brushes
  numleafbrushes = 0;
  dleafbrushes = (unsigned short*)GetClearedMemory(MAX_MAP_LEAFBRUSHES *
                                                   sizeof(unsigned short));
  ...
  // brushsides
  numbrushsides = 0;
  dbrushsides = (dbrushside_t*)GetClearedMemory(MAX_MAP_BRUSHSIDES *
                                                sizeof(dbrushside_t));
  ...
  // area portals
  numareaportals = 0;
  dareaportals = (dareaportal_t*)GetClearedMemory(MAX_MAP_AREAPORTALS *
...
int Q2_HintSkipBrush(dbrush_t* brush) {
  ...
  for (j = 0; j < brush->numsides; j++) {
    ...
    if (brushside->texinfo > 0) {
      //			if (texinfo[brushside->texinfo].flags &
      //(SURF_SKIP|SURF_HINT))
      //			{
      //				return true;
      //			} //end if
...
void Q2_LoadBSPFile(char* filename, int offset, int length) {
  ...
  numleaffaces = Q2_CopyLump(LUMP_LEAFFACES, dleaffaces, sizeof(dleaffaces[0]),
                             MAX_MAP_LEAFFACES);
  numleafbrushes = Q2_CopyLump(LUMP_LEAFBRUSHES, dleafbrushes,
                               sizeof(dleafbrushes[0]), MAX_MAP_LEAFBRUSHES);
  numsurfedges = Q2_CopyLump(LUMP_SURFEDGES, dsurfedges, sizeof(dsurfedges[0]),
                             MAX_MAP_SURFEDGES);
  ...
  numbrushsides = Q2_CopyLump(LUMP_BRUSHSIDES, dbrushsides,
                              sizeof(dbrushside_t), MAX_MAP_BRUSHSIDES);
  numareas = Q2_CopyLump(LUMP_AREAS, dareas, sizeof(darea_t), MAX_MAP_AREAS);
  numareaportals = Q2_CopyLump(LUMP_AREAPORTALS, dareaportals,
                               sizeof(dareaportal_t), MAX_MAP_AREAPORTALS);
