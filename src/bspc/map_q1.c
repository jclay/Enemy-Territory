...
// Tab Size:		3
// NOTES:			the recursive splitting of the huge brush
// sometimes still
//						creates bad brushes
//===========================================================================
...
bspbrush_t* Q1_CreateBrushes_r(bspbrush_t* brush, int nodenum) {
  ...
  planenum = FindFloatPlane(q1_dplanes[planenum].normal,
                            q1_dplanes[planenum].dist, 0, NULL);
...
bspbrush_t* Q1_CreateBrushesFromBSP(int modelnum) {
  ...
  if (!modelnum) {
    Log_Print("brush size: %5.0f,%5.0f,%5.0f to %5.0f,%5.0f,%5.0f\n",
              map_mins[0], map_mins[1], map_mins[2], map_maxs[0], map_maxs[1],
              map_maxs[2]);
  }  // end if
      ...
      if (!lessbrushes) {
        for (i = 0; i < q1_numfaces; i++) {
          ...
          if (q1_dfaces[i].planenum == q1_dnodes[sidenodenum].planenum) {
            ...
            // if this face overlaps the brush side winding more than previous
            // faces
            if (area > largestarea) {
      ...
      if (bestfacenum >= 0) {
        ...
        for (n = 0; n < 4; n++) {
          ...
          map_texinfo[texinfonum].vecs[1][n] =
           ...
           // make sure the two vectors aren't of zero length otherwise use the
           // default vector to prevent a divide by zero in the map writing
        ...
        map_texinfo[texinfonum].value =
            0;  // Q1 and HL texinfos don't have a value
        // the mip texture
        miptexlump = (q1_dmiptexlump_t*)q1_dtexdata;
        miptex =
            (q1_miptex_t*)((byte*)miptexlump +
                           miptexlump->dataofs[q1_texinfo[texinfonum].miptex]);
...
void Q1_FixContentsTextures(bspbrush_t* brushlist) {
  ...
  for (brush = brushlist; brush; brush = brush->next) {
    ...
    if (brush->side != CONTENTS_WATER && brush->side != CONTENTS_SLIME &&
        brush->side != CONTENTS_LAVA) {
      continue;
    }
    //
  ...
  for (brush = brushlist; brush; brush = brush->next)
  {
          ...
          for (i = 0; i < brush->numsides; i++)
          {
                  if (Q1_TextureContents(map_texinfo[texinfonum].texture) !=
  brush->side)
                  {
                          Error("brush contents %d with wrong contents textures
  %s\n", brush->side, Q1_TextureContents(map_texinfo[texinfonum].texture)); }
...
// Changes Globals:		-
//===========================================================================
void Q1_ResetMapLoading(void) {}  // end of the function Q1_ResetMapLoading
//===========================================================================
//
// Parameter:				-
// Returns:					-
