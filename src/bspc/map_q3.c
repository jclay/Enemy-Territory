...
int Q3_BrushContents(mapbrush_t* b) {
  ...
  if (mixed) {
    ...
    if (contents & CONTENTS_DONOTENTER) {
      return CONTENTS_DONOTENTER;  // Log_Print("mixed contents with
                                   // donotenter\n");
    }
    /*
...
void Q3_BSPBrushToMapBrush(q3_dbrush_t* bspbrush, entity_t* mapent) {
  ...
  if (!strcmp("func_invisible_user", classname) ||
      !strcmp("script_mover", classname) ||
      !strcmp("trigger_objective_info", classname)) {
  ...
  for (n = 0; n < bspbrush->numSides; n++) {
    ...
    for (k = n + 1; k < bspbrush->numSides; k++) {
      if (q3_dbrushsides[bspbrush->firstSide + k].planeNum ==
          bspbrushside->planeNum) {
        break;
      }
    }
    ...
    if (bspbrushside->shaderNum < 0) {
      ...
      /*			if
         (strstr(q3_dshaders[bspbrushside->shaderNum].shader, "common/hint")) {
    ...
    /*		if (!(strstr(q3_dshaders[bspbrushside->shaderNum].shader,
       "common/slip"))) { side->flags |= SFL_VISIBLE; } else if (side->surf &
       SURF_NODRAW) { side->flags |= SFL_TEXTURED | SFL_VISIBLE;
                    }*/
    ...
    if (side->winding) {
      side->planenum =
          FindFloatPlane(bspplane->normal, bspplane->dist,
                         side->winding->numpoints, side->winding->p);
    } else {
      side->planenum =
          FindFloatPlane(bspplane->normal, bspplane->dist, 0, NULL);
    }
    //
    ...
    /*		for (k = 0; k < b->numsides; k++) {
                            ...
                            if (s2->planenum == side->planenum) {
                                    Log_Print("Entity %i, Brush %i: duplicate
       plane\n", b->entitynum, b->brushnum); break;
                            }
                            if ( s2->planenum == (side->planenum^1) ) {
                                    Log_Print("Entity %i, Brush %i: mirrored
       plane\n", b->entitynum, b->brushnum); break;
                            }
                    }
  ...
  // Ridah, Wolf has ladders (if we call Q3_BrushContents(), we'll get the solid
  // area bug
  b->contents &= ~(/*CONTENTS_LADDER|*/ CONTENTS_FOG | CONTENTS_STRUCTURAL);
  // b->contents = Q3_BrushContents(b);
  //  RF, only allow known contents
  b->contents &=
      (CONTENTS_SOLID | CONTENTS_LADDER | CONTENTS_LAVA | CONTENTS_SLIME |
       CONTENTS_WATER | CONTENTS_AREAPORTAL | CONTENTS_PLAYERCLIP |
       CONTENTS_MONSTERCLIP | CONTENTS_DETAIL | CONTENTS_CLUSTERPORTAL);
  //
  // Ridah, CONTENTS_MOSTERCLIP should prevent AAS from being created, but not
  // clip players/AI in the game
  if (b->contents & CONTENTS_MONSTERCLIP) {
    b->contents |= CONTENTS_PLAYERCLIP;
  }

  // RF, optimization, if the brush is one of the following kind, make it
  // standard solid
  // if (b->contents & (CONTENTS_MONSTERCLIP|CONTENTS_PLAYERCLIP)) b->contents =
  // CONTENTS_SOLID;
  ...
  if (create_aas) {
    // create the AAS brushes from this brush, don't add brush bevels because
    // the .bsp brush already has them
  ...
  */
  ...
  for (i = 0; i < q3_dmodels[mapent->modelnum].numBrushes; i++) {
    ...
    Q3_BSPBrushToMapBrush(
        &q3_dbrushes[q3_dmodels[mapent->modelnum].firstBrush + i], mapent);
  }  // end for
...
void AAS_CreateCurveBrushes(void) {
  ...
  for (i = 0; i < q3_numDrawSurfaces; i++) {
    ...
    // Gordon: only on patches, other surface types use this for other stuff
    // (foliage for example)
    ...
    if (!(q3_dshaders[surface->shaderNum].contentFlags &
    ...
    for (j = 0; j < pc->numFacets; j++) {
      ...
      side->surf = 0;
      //
      winding = BaseWindingForPlane(mapplanes[side->planenum].normal,
      ...
      for (n = 0; n < facet->numBorders; n++) {
        ...
        side->planenum = FindFloatPlane(
            pc->planes[facet->borderPlanes[n]].plane,
        ...
        if (winding) {
          ChopWindingInPlace(&winding, mapplanes[side->planenum ^ 1].normal,
                             mapplanes[side->planenum ^ 1].dist,
                             0.1);  // CLIP_EPSILON);
        }
      }  // end for
      ...
      if (create_aas) {
        // add bevels here because we told CM_GeneratePatchCollide not to add
        // them
