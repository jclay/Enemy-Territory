...
#define MAX_MAP_BOUNDS \
  (128 * 1024)  // (SA) (9/17/01) new map dimensions (from Q3TA)
...
typedef struct side_s {
  ...
  struct side_s*
      original;          // bspbrush_t sides will reference the mapbrush_t sides
...
typedef struct face_s {
  ...
  int numpoints;
  qboolean
      badstartvert;  // tjunctions cannot be fixed without a midpoint vertex
