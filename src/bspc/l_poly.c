...
int WindingError(winding_t* w) {
  ...
  for (i = 0; i < w->numpoints; i++) {
    ...
    for (j = 0; j < 3; j++) {
      if (p1[j] > BOGUS_RANGE || p1[j] < -BOGUS_RANGE) {
        sprintf(windingerror, "winding point %d BUGUS_RANGE \'%f %f %f\'", j,
                p1[0], p1[1], p1[2]);
...
//
// Parameter:				-
// Returns:					the new winding with the added
...
//
// Parameter:				-
// Returns:					true if the point is on the
// winding Changes Globals:		-
//===========================================================================
...
int FindPlaneSeperatingWindings(winding_t* w1,
  ...
  for (i = 0; i < w1->numpoints; i++) {
    ...
    if (VectorLength(v1) < 0.1) {
      // Log_Write("FindPlaneSeperatingWindings: winding1 with degenerate
      // edge\r\n");
      continue;
    }
    ...
    for (j = 0; j < w2->numpoints; j++) {
      ...
      if (VectorLength(v2) < 0.1) {
        // Log_Write("FindPlaneSeperatingWindings: winding2 with degenerate
        // edge\r\n");
        continue;
      }
...
winding_t *AAS_MergeWindings(winding_t *w1, winding_t *w2, vec3_t windingnormal)
{
        ...
        if (!FindPlaneSeperatingWindings(w1, w2, windingnormal, normal, &dist))
return NULL;
        ...
        for (n = 0; n < 2; n++)
        {
                ...
                //get the points of the winding which are on the seperating
plane for (i = 0; i < winding->numpoints; i++)
                {
                        dot = DotProduct(winding->p[i], normal) - dist;
                        if (dot > -ON_EPSILON && dot < ON_EPSILON)
                        {
                                //don't allow more than 64 points on the
seperating plane if (numpoints[n] >= 64) Error("AAS_MergeWindings: more than 64
points on seperating plane\n"); points[n][numpoints[n]++] = i; } //end if }
//end for
                //there must be at least two points of each winding on the
seperating plane if (numpoints[n] < 2) return NULL; } //end for

        //if the first point of winding1 (which is on the seperating plane) is
unequal
        //to the last point of winding2 (which is on the seperating plane)
        if (!EqualVertexes(w1->p[points[0][0]],
w2->p[points[1][numpoints[1]-1]]))
        {
                return NULL;
        } //end if
        //if the last point of winding1 (which is on the seperating plane) is
unequal
        //to the first point of winding2 (which is on the seperating plane)
        if (!EqualVertexes(w1->p[points[0][numpoints[0]-1]],
w2->p[points[1][0]]))
        {
                return NULL;
        } //end if
        ...
        dot = DotProduct(delta, normal);
        if (dot > CONTINUOUS_EPSILON) return NULL; //merging would create a
non-convex polygon keep[0] = (qboolean)(dot < -CONTINUOUS_EPSILON);
        ...
        dot = DotProduct(delta, normal);
        if (dot > CONTINUOUS_EPSILON) return NULL; //merging would create a
non-convex polygon keep[1] = (qboolean)(dot < -CONTINUOUS_EPSILON);

        //number of points on the new winding
        newnumpoints = w1->numpoints - numpoints[0] + w2->numpoints -
numpoints[1] + 2;
        ...
        for (n = 0; n < 2; n++)
        {
                ...
                //copy the points of the winding starting with the last point on
the
                //seperating plane and ending before the first point on the
seperating plane for (i = points[n][numpoints[n]-1]; i != points[n][0]; i =
(i+1)%winding->numpoints)
                {
                        if (k >= newnumpoints)
                        {
                                Log_Print("numpoints[0] = %d\n", numpoints[0]);
                                Log_Print("numpoints[1] = %d\n", numpoints[1]);
                                Error("AAS_MergeWindings: k = %d >= newnumpoints
= %d\n", k, newnumpoints); } //end if VectorCopy(winding->p[i], neww->p[k]);
                        k++;
                } //end for
        } //end for
