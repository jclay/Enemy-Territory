  ...
  if (tw->sphere.use) {
    for (i = 0; i < 3; i++) {
      if (tw->start[i] < tw->end[i]) {
        tw->bounds[0][i] =
            tw->start[i] - Q_fabs(tw->sphere.offset[i]) - tw->sphere.radius;
        tw->bounds[1][i] = tw->start[i] + tw->trace.fraction * tw->dir[i] +
                           Q_fabs(tw->sphere.offset[i]) + tw->sphere.radius;
      } else {
    ...
    for (i = 0; i < 3; i++) {
      if (tw->start[i] < tw->end[i]) {
        ...
        tw->bounds[1][i] =
            tw->start[i] + tw->trace.fraction * tw->dir[i] + tw->size[1][i];
      } else {
        tw->bounds[0][i] =
            tw->start[i] + tw->trace.fraction * tw->dir[i] + tw->size[0][i];
...
================
*/
static float CM_BoxDistanceFromPlane(vec3_t center,
                                     vec3_t extents,
                                     cplane_t* plane) {
  float d1, d2;
...
static int CM_TraceThroughBounds(traceWork_t* tw, vec3_t mins, vec3_t maxs) {
  ...
  if (Q_fabs(CM_BoxDistanceFromPlane(center, extents, &tw->tracePlane1)) >
      tw->traceDist1) {
    return qfalse;
  }
  if (Q_fabs(CM_BoxDistanceFromPlane(center, extents, &tw->tracePlane2)) >
      tw->traceDist2) {
    return qfalse;
  }
...
#endif
      {
        if (!CM_TraceThroughBounds(tw, patch->pc->bounds[0],
                                   patch->pc->bounds[1])) {
          continue;
        }
      }
...
================
*/
static void CM_TraceThroughSphere(traceWork_t* tw,
                                  vec3_t origin,
                                  float radius,
                                  vec3_t start,
                                  vec3_t end) {
...
================
*/
static void CM_TraceThroughVerticalCylinder(traceWork_t* tw,
                                            vec3_t origin,
                                            float radius,
                                            float halfheight,
                                            vec3_t start,
...
================
*/
static void CM_TraceBoundingBoxThroughCapsule(traceWork_t* tw,
                                              clipHandle_t model) {
  vec3_t mins, maxs, offset, size[2];
  clipHandle_t h;
  cmodel_t* cmod;
...
==================
*/
static void CM_TraceThroughTree(traceWork_t* tw,
                                int num,
                                float p1f,
                                float p2f,
                                vec3_t p1,
...
static void CM_Trace(trace_t* results,
  ...
  if (positionTest) {
    ...
    if (tw.isPoint) {
      ...
      for (i = 0; i < 8; i++) {
        dist = Q_fabs(DotProduct(tw.tracePlane1.normal, tw.offsets[i]) -
                      tw.tracePlane1.dist);
        if (dist > tw.traceDist1) {
          tw.traceDist1 = dist;
        }
        dist = Q_fabs(DotProduct(tw.tracePlane2.normal, tw.offsets[i]) -
                      tw.tracePlane2.dist);
    ...
    for (i = 0; i < 3; i++) {
      if (tw.start[i] < tw.end[i]) {
        tw.bounds[0][i] =
            tw.start[i] - Q_fabs(tw.sphere.offset[i]) - tw.sphere.radius;
        tw.bounds[1][i] =
            tw.end[i] + Q_fabs(tw.sphere.offset[i]) + tw.sphere.radius;
      } else {
        tw.bounds[0][i] =
            tw.end[i] - Q_fabs(tw.sphere.offset[i]) - tw.sphere.radius;
        tw.bounds[1][i] =
