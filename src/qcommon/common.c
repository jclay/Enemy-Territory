...
#define MIN_COMHUNKMEGS \
  42  // JPW NERVE changed this to 42 for MP, was 56 for team arena and 75 for
      // wolfSP
#define DEF_COMHUNKMEGS \
  "56"  // RF, increased this, some maps are exceeding 56mb // JPW NERVE changed
        // this for multiplayer back to 42, 56 for depot/mp_cpdepot, 42 for
        // everything else
#define DEF_COMZONEMEGS \
  "24"  // RF, increased this from 16, to account for botlib/AAS
...
cvar_t* com_crashed = NULL;  // ydnar: set in case of a crash, prevents
                             // CVAR_UNSAFE variables from being set from a cfg
// bani - explicit NULL to make win32 teh happy

cvar_t* com_ignorecrash = NULL;  // bani - let experienced users ignore crashes,
...
memblock_t* debugblock;  // RF, jusy so we can track a block to find out when
...
megs", (float)s_smallZoneTotal / (1024*1024) );
        ...
        cv = Cvar_Get( "com_zoneMegs", DEF_COMZONEMEGS, CVAR_LATCH |
CVAR_ARCHIVE );
...
void Hunk_SmallLog(void) {
  ...
  for (block = hunkblocks; block; block = block->next) {
    ...
    Com_sprintf(buf, sizeof(buf),
                ...
                locsize, locsize / Square(1024.f),
                (size + block->size) / Square(1024.f), block->file, block->line,
                block->label);
    FS_Write(buf, strlen(buf), logfile);
#endif
    size += block->size;
...
// TTimo: centralizing the cl_cdkey stuff after I discovered a buffer overflow
// problem with the dedicated server version
//   not sure it's necessary to have different defaults for regular and
//   dedicated, but I don't want to take the risk
#ifndef DEDICATED
char cl_cdkey[34] = "                                ";
#else
...
#else
  FS_Printf(f,
            "// id Software and Activision will NOT ask you to send this file "
            "to them.\r\n");
#endif
  FS_FCloseFile(f);
}
...
void Com_SetRecommended() {
  ...
  if (cpuSpeed > 1500) {
    ...
    Cbuf_AddText("exec preset_normal.cfg\n");
    Cvar_Set("com_recommended", "1");
  } else if (cpuSpeed < 200) {  // do the < 200 check just in case we barf,
                                // better than falling back to ugly fast
    if (goodVideo) {
      Com_Printf(
          "Found high quality video but didn't manage to detect a CPU "
          "properly\n");
    } else {
      Com_Printf(
          "Found low quality video but didn't manage to detect a CPU "
          "properly\n");
    }
    Cbuf_AddText("exec preset_normal.cfg\n");
    Cvar_Set("com_recommended", "1");
...
// Arnout: gameinfo, to let the engine know which gametypes are SP and if we
// should use profiles. This can't be dependant on gamecode as we sometimes need
// to know about it when no game-modules are loaded
...
// bani - track profile changes, delete old profile.pid if we change
// fs_game(dir) hackish, we fiddle with fs_gamedir to make FS_* calls work
// "right"
void Com_TrackProfile(char* profile_path) {
  char temp_fs_gamedir[MAX_OSPATH];

  //	Com_Printf( "Com_TrackProfile: Tracking profile [%s] [%s]\n",
  // fs_gamedir, profile_path ); have we changed fs_game(dir)?
...
void Com_Init(char* commandLine) {
  char* s;
  int pid;
  // TTimo gcc warning: variable `safeMode' might be clobbered by `longjmp' or
  // `vfork'
  ...
  //	com_blood = Cvar_Get ("com_blood", "1", CVAR_ARCHIVE); // Gordon: no
  // longer used?
  ...
  Netchan_Init(Com_Milliseconds() &
               0xffff);  // pick a port value that should be nice and random
  VM_Init();
  SV_Init();
...
typedef enum {
  ...
  PRE_READ_WRITE  // prefetch assuming that buffer is used for both reading and
                  // writing
} e_prefetch;
...
void _copyDWord(unsigned int* dest,
...
qboolean Com_Memcmp(const void* src0,
                    const void* src1,
                    const unsigned int count) {
  unsigned int i;
  // MMX version anyone?
...
// field we are working on, passed to Field_CompleteCommand (&g_consoleCommand
// for instance)
...
void Field_CompleteCommand(field_t* field) {
  ...
  Com_sprintf(completionField->buffer, sizeof(completionField->buffer), "\\%s",
              shortestMatch);
  completionField->cursor = strlen(completionField->buffer);
  ConcatRemaining(temp.buffer, completionString);
