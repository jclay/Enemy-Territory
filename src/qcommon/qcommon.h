...
void MSG_WriteDeltaUsercmd(msg_t* msg,
                           struct usercmd_s* from,
                           struct usercmd_s* to);
void MSG_ReadDeltaUsercmd(msg_t* msg,
                          struct usercmd_s* from,
                          struct usercmd_s* to);

void MSG_WriteDeltaUsercmdKey(msg_t* msg,
                              int key,
                              usercmd_t* from,
                              usercmd_t* to);
void MSG_ReadDeltaUsercmdKey(msg_t* msg,
                             int key,
                             usercmd_t* from,
                             usercmd_t* to);
...
// when loading a savegame, which is hard to fix on that side, since we can't
// really spread out a loadgame among several frames
// #define	MAX_RELIABLE_COMMANDS	64			// max string
// commands buffered for restransmit #define	MAX_RELIABLE_COMMANDS	128
...
void QDECL NET_OutOfBandPrint(netsrc_t net_socket,
                              netadr_t adr,
                              const char* format,
                              ...);
...
// #define	MAX_MSGLEN				16384		// max
// length of a message, which may
...
#define MOTD_SERVER_NAME \
  "etmaster.idsoftware.com"  //"etmotd.idsoftware.com"			//
                             //?.?.?.?
...
void Cvar_LatchedVariableStringBuffer(const char* var_name,
                                      char* buffer,
                                      int bufsize);
// Gordon: returns the latched value if there is one, else the normal one, empty
// string if not defined as usual
...
char** FS_ListFiles(const char* directory,
...
qboolean FS_OS_FileExists(
    const char* file);  // TTimo - test file existence given OS path
...
int FS_FOpenFileRead(const char* qpath,
                     fileHandle_t* file,
                     qboolean uniqueFILE);
/*
...
int FS_Delete(char* filename);  // only works inside the 'save' directory (for
                                // deleting savegames/images)
...
qboolean FS_CL_ExtractFromPakFile(const char* path,
                                  const char* gamedir,
                                  const char* filename,
...
int QDECL Com_VPrintf(const char* fmt, va_list argptr) _attribute(
    (format(printf,
            1,
            0)));  // conforms to vprintf prototype for print callback passing
void QDECL Com_Printf(const char* fmt, ...)
    _attribute((format(printf, 1, 2)));  // this one calls to Com_VPrintf now
...
void* Z_TagMallocDebug(int size,
                       int tag,
                       char* label,
                       char* file,
                       int line);  // NOT 0 filled memory
void* Z_MallocDebug(int size,
                    char* label,
                    char* file,
                    int line);  // returns 0 filled memory
void* S_MallocDebug(int size,
                    char* label,
                    char* file,
                    int line);  // returns 0 filled memory
#else
...
// fqpath param added 2/15/02 by T.Ray - Sys_LoadDll is only called in vm.c at
// this time
