...
===============
*/
static void CM_Subdivide(vec3_t a,
                         vec3_t b,
                         vec3_t c,
                         vec3_t out1,
                         vec3_t out2,
...
==================
*/
static int CM_GridPlane(int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2],
                        int i,
                        int j,
                        int tri) {
  int p;
...
===================
*/
static void CM_SetBorderInward(facet_t* facet,
                               cGrid_t* grid,
                               int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2],
                               int i,
                               int j,
    ...
    for (axis = 0; axis < 3; axis++) {
      for (dir = -1; dir <= 1; dir += 2) {
        ...
        if (i == facet->numBorders) {
          ...
          if (!w2) {
            // TTimo: any map load spams with this error .. don't print it
            // anymore
...
==================
*/
static void CM_PatchCollideFromGrid(cGrid_t* grid,
                                    patchCollide_t* pf,
                                    qboolean addBevels) {
  int i, j;
  float *p1, *p2, *p3;
...
entire patch if (d1 > 0 && ( d2 >= SURFACE_CLIP_EPSILON || d2 >= d1 )  ) {
                        continue;
                }
...
tw->start[j]) * fraction;
                ...
                for (j = 0; j < facet->numBorders; j++)
                {
                        ...
                        //offset = 0;
                        planedist -= offset;
                        //the hit point should be in front of the (inward
facing) border plane if (DotProduct(point, normal) - planedist < -ON_EPSILON)
break; } //end for if (j < facet->numBorders) continue;
                //
