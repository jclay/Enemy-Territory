...
void callAsmCall(void) {
  ...
  *(int*)((byte*)currentVM->dataBase + callProgramStack + 4) = callSyscallNum;
  // VM_LogSyscalls(  (int *)((byte *)currentVM->dataBase + programStack + 4) );
  *(callOpStack + 1) = currentVM->systemCall(
...
static int Constant4(void) {
  ...
  v = code[pc] | (code[pc + 1] << 8) | (code[pc + 2] << 16) |
      (code[pc + 3] << 24);
  pc += 4;
  return v;
}
  ...
  while (instruction < header->instructionCount) {
    ...
    switch (op) {
      ...
      case OP_JUMP:
        ...
        // FIXME: range check
        EmitString(
            "FF 24 85");  // jmp dword ptr [instructionPointers + eax * 4]
        Emit4((int)vm->instructionPointers);
        break;
      default:
        Com_Error(ERR_DROP, "VM_CompileX86: bad opcode %i at offset %i", op,
                  pc);
    }
  }
