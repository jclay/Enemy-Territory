...
char fs_gamedir[MAX_OSPATH];  // this will be a single file name with no
                              // separators
...
// pure ex: when fs_numServerPaks != 0, FS_FOpenFileRead won't load anything
// outside of pk3 except .cfg .menu .game .dat
...
qboolean FS_PakIsPure(pack_t* pack) {
  ...
  if (fs_numServerPaks) {
    for (i = 0; i < fs_numServerPaks; i++) {
      ...
      // validated too
      //   I don't see this as allowing for any exploit, it would only happen if
      //   the client does manips of it's file names 'not a bug'
...
int FS_FOpenFileRead(const char* filename,
                     fileHandle_t* file,
                     qboolean uniqueFILE) {
  searchpath_t* search;
  char* netpath;
  ...
  // TTimo - NOTE
  // when checking for file existence, it's probably safer to use FS_FileExists,
  // as I'm not sure this chunk of code is really up to date with everything
  if (file == NULL) {
    ...
    if (search->pack && search->pack->hashTable[hash]) {
      ...
      do {
        ...
        if (!FS_FilenameCompare(pakFile->name, filename)) {
          ...
          // for OS client/server interoperability, we expect binaries for .so
          // and .dll to be in the same pk3 so that when we reference the DLL
          // files on any platform, this covers everyone else
          ...
          if (!(pak->referenced & FS_QAGAME_REF) &&
              FS_ShiftedStrStr(filename, SYS_DLLNAME_QAGAME,
                               -SYS_DLLNAME_QAGAME_SHIFT)) {
            pak->referenced |= FS_QAGAME_REF;
          }
          // cgame dll
          if (!(pak->referenced & FS_CGAME_REF) &&
              FS_ShiftedStrStr(filename, SYS_DLLNAME_CGAME,
                               -SYS_DLLNAME_CGAME_SHIFT)) {
            pak->referenced |= FS_CGAME_REF;
          }
          // ui dll
          if (!(pak->referenced & FS_UI_REF) &&
              FS_ShiftedStrStr(filename, SYS_DLLNAME_UI,
                               -SYS_DLLNAME_UI_SHIFT)) {
            pak->referenced |= FS_UI_REF;
          }

          // #if !defined(PRE_RELEASE_DEMO) && !defined(DO_LIGHT_DEDICATED)
          //					// DHM -- Nerve :: Don't allow
          //maps to be loaded from pak0 (singleplayer) 					if ( Q_stricmp(filename +
          //l - 4, ".bsp") == 0 && 						Q_stricmp( pak->pakBasename, "pak0" ) == 0 )
          //{
          //
          //						*file = 0;
          //						return -1;
          ...
          /*if ( fs_copyfiles->integer && fs_buildpath->string[0] && Q_stricmpn(
          fs_buildpath->string, pak->pakFilename, strlen(fs_buildpath->string) )
          ) { char			copypath[MAX_OSPATH]; fileHandle_t
          f; byte			*srcData; int
          len = zfi->cur_file_info.uncompressed_size;

                  Q_strncpyz( copypath, FS_BuildOSPath( fs_buildpath->string,
          fs_buildgame->string, filename ), sizeof(copypath) ); netpath =
          FS_BuildOSPath( fs_basepath->string, fs_gamedir, filename );

                  f = FS_FOpenFileWrite( filename );
                  if ( !f ) {
                          Com_Printf( "FS_FOpenFileRead Failed to open %s for
          copying\n", filename ); } else { srcData = Hunk_AllocateTempMemory(
          len) ; FS_Read( srcData, len, *file ); FS_Write( srcData, len, f );
                          FS_FCloseFile( f );
                          Hunk_FreeTempMemory( srcData );

                          if (rename( netpath, copypath )) {
                                  // Failed, try copying it and deleting the
          original FS_CopyFile ( netpath, copypath ); FS_Remove ( netpath );
                          }
                  }
          }*/
      ...
      // if we are running restricted, or if the filesystem is configured for
      // pure (fs_numServerPaks) the only files we will allow to come from the
      // directory are .cfg files
      l = strlen(filename);
      ...
      } else if (fs_copyfiles->integer && fs_buildpath->string[0] &&
                 Q_stricmp(dir->path, fs_buildpath->string)) {
        char* copypath;

        copypath = FS_BuildOSPath(fs_buildpath->string, fs_buildgame->string,
                                  filename);
        FS_CopyFile(netpath, copypath);
      }
...
int FS_FOpenFileRead_Filtered(const char* qpath,
                              fileHandle_t* file,
                              qboolean uniqueFILE,
                              int filter_flag) {
  int ret;
...
NOTE TTimo:
  ...
  the return value doesn't tell wether file was extracted or not, it just says
wether it's ok to continue (i.e. either the right file was extracted
successfully, or it was already present)

  cvar_lastVersion is the optional name of a CVAR_ARCHIVE used to store the wolf
version for the last extracted .so show_bug.cgi?id=463

==================
*/
qboolean FS_CL_ExtractFromPakFile(const char* fullpath,
                                  const char* gamedir,
                                  const char* filename,
                                  const char* cvar_lastVersion) {
  int srcLength;
  int destLength;
...
int FS_DeleteDir(char* dirname, qboolean nonEmpty, qboolean recursive) {
  ...
  if (nonEmpty) {
    ...
    for (i = 0; i < nFiles; i++) {
      ospath = FS_BuildOSPath(fs_homepath->string, fs_gamedir,
                              va("%s/%s", dirname, pFiles[i]));
...
int FS_Seek(fileHandle_t f, long offset, int origin) {
  ...
  if (fsh[f].zipFile == qtrue) {
    if (offset == 0 && origin == FS_SEEK_SET) {
      ...
      unzSetCurrentFileInfoPosition(fsh[f].handleFiles.file.z,
                                    fsh[f].zipFilePos);
...
==================
*/
static int FS_AddFileToList(char* name,
...
===============
*/
char** FS_ListFilteredFiles(const char* path,
...
================
*/
int FS_GetFileList(const char* path,
                   const char* extension,
                   char* listbuf,
                   int bufsize) {
...
static char** Sys_ConcatenateFileLists(char** list0,
                                       char** list1,
                                       char** list2) {
  int totalLength = 0;
  ...
  *listbuf = 0;
  ...
  for (i = 0; i < nPotential; i++) {
    ...
    if (Q_stricmp(name, BASEGAME) && Q_stricmpn(name, ".", 1)) {
      ...
      // NOTE TTimo: (actually I'm not sure why .. what if it's a mod under
      // developement with no .pk3?) we didn't keep the information when we
      // merged the directory names, as to what OS Path it was found under
      ...
      Sys_FreeFileList(pPaks);  // we only use Sys_ListFiles to check wether
                                // .pk3 files are present
...
static void FS_AddGameDirectory(const char* path, const char* dir) {
  ...
  for (i = 0; i < numfiles; i++) {
    sorted[i] = pakfiles[i];
    // JPW NERVE KLUDGE: sorry, temp mod mp_* to _p_* so "mp_pak*" gets
    // alphabetically sorted before "pak*"
...
qboolean FS_idPak(char* pak, char* base) {
  ...
  for (i = 0; i < NUM_ID_PAKS; i++) {
    ...
    /*
    // JPW NERVE -- this fn prevents external sources from
    downloading/overwriting official files, so exclude both SP and MP files from
    this list as well if ( !FS_FilenameCompare(pak, va("%s/mp_pak%d",base,i)) )
    { break;
                    }
  ...
  for (i = 0; i < fs_numServerPaks; i++) {
    for (sp = fs_searchpaths; sp; sp = sp->next) {
      if (sp->pack && sp->pack->checksum == fs_serverPaks[i]) {
        ...
        Com_sprintf(packPath, sizeof(packPath), "%s/%s", sp->pack->pakGamename,
                    sp->pack->pakBasename);
    ...
    if (!havepak && fs_serverReferencedPakNames[i] &&
      ...
      if (dlstring) {
        ...
        if (FS_SV_FileExists(va("%s.pk3", fs_serverReferencedPakNames[i]))) {
          char st[MAX_ZPATH];
          // We already have one called this, we need to download it to another
          // name Make something up with the checksum in it
...
#ifndef DEDICATED
          // let the client subsystem track bad download redirects (dl file with
          // wrong checksums) this is a bit ugly but the only other solution
          // would have been callback passing..
          if (CL_WWWBadChecksum(va("%s.pk3", fs_serverReferencedPakNames[i]))) {
            // remove a potentially malicious download file
            // (this is also intended to avoid expansion of the pk3 into a file
            // with different checksum .. messes up wwwdl chkfail)
            char* rmv = FS_BuildOSPath(
                fs_homepath->string,
...
static void FS_ReorderPurePaks() {
  ...
  for (i = 0; i < fs_numServerPaks; i++) {
    ...
    for (s = *p_insert_index; s;
         s = s->next) {  // the part of the list before p_insert_index has been
                         // sorted already
...
#define SYS_PAKNAME_PAK0 "whr7"
...
#endif
      if (FS_ShiftedStrStr(path->pack->pakBasename, SYS_PAKNAME_MPBIN,
                           -SYS_PAKNAME_MPBIN_SHIFT)) {
      ...
      } else if (FS_ShiftedStrStr(path->pack->pakBasename, SYS_PAKNAME_PAK0,
                                  -SYS_PAKNAME_PAK0_SHIFT)) {
        // a tiny attempt to keep the checksum from being scannable from the exe
...
that only matters to the pk3 we have replaced by their lighter version,
currently:
...
all the files above have their 'server required' content collapsed into a single
pak0.pk3
...
// those don't need to be encrypted or anything, that's what you see in the +set
// developer 1
static const char* pak_checksums =
    "-137448799 131270674 125907563 -1023558518 764840216 1886207346";
static const char* pak_names = "mp_pak4 mp_pak3 mp_pak2 mp_pak1 mp_pak0 pak0";

/*
this is the pure checksum string for a constant value of fs_checksumFeed we have
choosen (see SV_SpawnServer) to obtain the new string for a different
fs_checksumFeed value, run a regular server and enable the relevant verbosity
code in SV_SpawnServer and FS_LoadedPakPureChecksums (the full server version of
course)

NOTE: if you have an mp_bin in the middle, you need to take out it's checksum
  (we keep mp_bin out of the faked stuff because we don't want to have to update
those feeds too often heh)

once you have the clear versions, you can shift them by commenting out the code
chunk in FS_RandChecksumFeed you need to use the right line in
FS_LoadedPakPureChecksums wether you are running on clear strings, or shifted
ones
*/

/*
// clear checksums, rebuild those from a regular server and you will shift them
next static const int feeds[5] = { 0x14d48835, 0xc44ed670, 0xd1c8da0d,
0x98df0626, 0xb4e51e7a
};
...
static const int feeds[5] = {0x14d48835, 0xc44ed670, 0xd1c8da0d, 0x98df0626,
                             0xb4e51e7a};
...
FS_LoadPakPureChecksums is used locally to compare against what the client sends
back

the pure_checksums are computed by Com_MemoryBlockChecksum with a random key
(fs_checksumFeed) since we can't do this on restricted server, we always use the
same fs_checksumFeed value
...
const char* FS_LoadedPakPureChecksums(void) {
  ...
  for (search = fs_searchpaths; search; search = search->next) {
    ...
    if (strcmp(search->pack->pakBasename, "pak0")) {
      ...
      Q_strcat(info, sizeof(info),
               va("%s ",
                  FS_ShiftStr(pak_purechecksums[lookup_randomized[feed_index]],
                              -13 - lookup_randomized[feed_index])));
      // use this if you are running on clear checksum strings instead of
      // shifted ones
      // Q_strcat( info, sizeof( info ), va("%s ",
      // pak_purechecksums[lookup_randomized[feed_index]] ) );
    }
  }
...
mp_pakmaps0 would be a worthy candidate for download though, but we don't have
it anyway the only thing if we omit sending of some referenced stuff, you don't
get the console message that says "you're missing this"
=====================
*/
const char* FS_ReferencedPakChecksums(void) {
  static char info[BIG_INFO_STRING];
...
void FS_InitFilesystem(void) {
  ...
  if (FS_ReadFile("default.cfg", NULL) <= 0) {
    Com_Error(ERR_FATAL,
              "Couldn't load default.cfg - I am missing essential files - "
...
//	CL_PurgeCache();
  ...
  if (FS_ReadFile("default.cfg", NULL) <= 0) {
    ...
    // TTimo - added some verbosity, 'couldn't load default.cfg' confuses the
    // hell out of users
    Com_Error(ERR_FATAL,
              "Couldn't load default.cfg - I am missing essential files - "
              "verify your installation?");
...
#ifndef _DEBUG
          Com_Printf(
              "^3WARNING: profile.pid found for profile '%s' - system settings "
