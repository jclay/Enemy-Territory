...
vm_t* VM_Restart(vm_t* vm) {
  ...
  // validate
  if (header->vmMagic != VM_MAGIC || header->bssLength < 0 ||
  ...
  Com_Memcpy(vm->dataBase, (byte*)header + header->dataOffset,
...
int QDECL VM_Call(vm_t* vm, int callnum, ...) {
  ...
  if (vm->entryPoint) {
    ...
    r = vm->entryPoint((&callnum)[0], (&callnum)[1], (&callnum)[2],
                       (&callnum)[3], (&callnum)[4], (&callnum)[5],
                       (&callnum)[6], (&callnum)[7], (&callnum)[8],
                       (&callnum)[9], (&callnum)[10], (&callnum)[11],
                       (&callnum)[12]);
#endif
  } else if (vm->compiled) {
...
void VM_LogSyscalls(int* args) {
  ...
  fprintf(f, "%i: %i (%i) = %i %i %i %i\n", callnum,
          args - (int*)currentVM->dataBase, args[0], args[1], args[2], args[3],
          args[4]);
}
