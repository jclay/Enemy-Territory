...
#define SHADER_MAX_VERTEXES \
  1025  // Arnout: 1024+1 (1 buffer for RB_EndSurface overflow check) // JPW
...
version history:
        2 - initial version
        3 - removed all frame data, this format is pure mesh and bone references
now
...
/*typedef struct {
        vec3_t		bounds[2];			// bounds of all
surfaces of all LOD's for this frame vec3_t		localOrigin;
// midpoint of bounds, used for sphere cull float		radius;
// dist from localOrigin to corner vec3_t		parentOffset;
// one bone is an ascendant of all other bones, it starts the hierachy at this
position } mdmFrame_t;*/
...
typedef struct {
  ...
  int numFrames;
  int numBones;
  int ofsFrames;    // (mdxFrame_t + mdxBoneFrameCompressed_t[numBones]) *
                    // numframes
