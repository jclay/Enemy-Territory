...
** Print progress
*/
BOOL HTAlertCallback_progress(HTRequest* request,
                              HTAlertOpcode op,
                              int msgnum,
                              const char* dfault,
                              void* input,
  ...
  if (op == HT_PROG_READ) {
    if (!dl_is_ftp) {
      ...
      Cvar_SetValue("cl_downloadCount",
...
*/
BOOL HTAlertCallback_confirm(HTRequest* request,
                             HTAlertOpcode op,
                             int msgnum,
                             const char* dfault,
                             void* input,
...
*/
BOOL HTAlertCallback_prompt(HTRequest* request,
                            HTAlertOpcode op,
                            int msgnum,
                            const char* dfault,
                            void* input,
                            HTAlertPar* reply) {
  Com_Printf(
      "Aborting, libwww prompt message id: %d (prompted for a "
      "login/password?)\n",
      msgnum);
  HTEventList_stopLoop();
  return NO;
...
void DL_InitDownload() {
  ...
  // This file already exists - replace existing file? (y/n) Can't open output
  // file and cause a www download to fail
  HTAlertInit();
...
int DL_BeginDownload(const char* localName, const char* remoteName, int debug) {
  ...
  if (dl_running) {
    Com_Printf(
        "ERROR: DL_BeginDownload called with a download request already "
        "active\n");
    return 0;
  }
  ...
  In case of ftp download, we leave no timeout during connect phase cause of
  libwww bugs show_bug.cgi?id=605
  */
...
dlStatus_t DL_DownloadLoop() {
  ...
  */
  ...
  /* NOTE: there is HTEventList_status, but it says != HT_OK as soon as
   * HTEventList_pump returns NO */
