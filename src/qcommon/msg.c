...
void MSG_WriteDeltaUsercmdKey(msg_t* msg,
  ...
  if (from->angles[0] == to->angles[0] && from->angles[1] == to->angles[1] &&
      ...
      from->buttons == to->buttons && from->wbuttons == to->wbuttons &&
...
void MSG_ReadDeltaUsercmdKey(msg_t* msg,
  ...
  if (MSG_ReadBits(msg, 1)) {
    ...
    to->identClient =
        MSG_ReadDeltaKey(msg, key, from->identClient, 8);  // NERVE - SMF
  } else {
...
netField_t entityStateFields[] = {
    ...
    {NETF(dl_intensity),
     32},  //----(SA)	longer now to carry the corona colors
    {NETF(modelindex), 9},
    {NETF(modelindex2), 9},
    {NETF(frame), 16},
    {NETF(clientNum), 8},
...
void MSG_ReadDeltaEntity(msg_t* msg,
  ...
  if (lc > numFields) {  // DAJ FIXME have gotten lc = 76 which is bigger than
                         // numFields!
    lc = numFields;
  }
    ...
    if (statsbits) {
      ...
      for (i = 0; i < 16; i++)
        if (statsbits & (1 << i)) {
          ...
          //to->stats[i]);
          MSG_WriteShort(msg,
                         to->stats[i]);  //----(SA)	back to short since
                                         //weapon bits are handled elsewhere now
        }
    } else {
...
//		If it did, send individual segments specifying offset values for each item.
  ...
  // j == 0 : weaps 0-15
  // j == 1 : weaps 16-31
  // j == 2 : weaps 32-47	//----(SA)	now up to 64 (but still pretty
  // net-friendly) j == 3 : weaps 48-63
  ...
  //----(SA)	also encapsulated ammo changes into one check.  clip values will
  //change frequently,
  // but ammo will not.  (only when you get ammo/reload rather than each shot)
  if (ammobits[0] || ammobits[1] || ammobits[2] ||
      ammobits[3]) {           // if any were set...
    MSG_WriteBits(msg, 1, 1);  // changed
    for (j = 0; j < 4; j++) {
      if (ammobits[j]) {
        MSG_WriteBits(msg, 1, 1);  // changed
...
void MSG_ReadDeltaPlayerstate(msg_t* msg,
  ...
  *to = *from;
  ...
  // read the arrays
  if (MSG_ReadBits(msg, 1)) {  // one general bit tells if any of this
                               // infrequently changing stuff has changed
    // parse stats
    if (MSG_ReadBits(msg, 1)) {
      LOG("PS_STATS");
      ...
      for (i = 0; i < 16; i++) {
        if (bits & (1 << i)) {
          ...
          //MSG_ReadLong(msg);
          to->stats[i] =
              MSG_ReadShort(msg);  //----(SA)	back to short since weapon bits
                                   //are handled elsewhere now
        }
      }
    }
