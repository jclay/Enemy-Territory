...
char* Cvar_ClearForeignCharacters(const char* value) {
  ...
  for (i = 0; value[i] != '\0'; i++) {
    ...
    if (((byte*)value)[i] != 0xFF &&
        (((byte*)value)[i] <= 127 || ((byte*)value)[i] >= 161)) {
      clean[j] = value[i];
      j++;
    }
  }
...
cvar_t* Cvar_Get(const char* var_name, const char* var_value, int flags) {
  ...
  if (var) {
    ...
    // TTimo
    // if CVAR_USERINFO was toggled on for an existing cvar, check wether the
    // value needs to be cleaned from foreigh characters (for instance, seta
    // name "name-with-foreign-chars" in the config file, and toggle to
    // CVAR_USERINFO happens later in CL_Init)
    if (flags & CVAR_USERINFO) {
      char* cleaned = Cvar_ClearForeignCharacters(
          var->string);  // NOTE: it is probably harmless to call Cvar_Set2 in
                         // all cases, but I don't want to risk it
      if (strcmp(var->string, cleaned)) {
        Cvar_Set2(var->name, var->string,
                  qfalse);  // call Cvar_Set2 with the value to be cleaned up
                            // for verbosity
      }
    }
...
*/
#define FOREIGN_MSG \
  "Foreign characters are not allowed in userinfo variables.\n"
#ifndef DEDICATED
...
cvar_t* Cvar_Set2(const char* var_name, const char* value, qboolean force) {
  ...
  if (!force) {
    ...
    if ((var->flags & CVAR_UNSAFE) && com_crashed != NULL &&
        com_crashed->integer) {
...
void Cvar_Update(vmCvar_t* vmCvar) {
  ...
  if ((unsigned)vmCvar->handle >= cvar_numIndexes) {
    Com_Error(ERR_DROP, "Cvar_Update: handle %d out of range",
              (unsigned)vmCvar->handle);
  }
  ...
  if (strlen(cv->string) + 1 > MAX_CVAR_VALUE_STRING) {
    Com_Error(ERR_DROP,
              "Cvar_Update: src %s length %d exceeds MAX_CVAR_VALUE_STRING(%d)",
              cv->string, strlen(cv->string), sizeof(vmCvar->string));
  }
  // bk001212 - Q_strncpyz guarantees zero padding and
  // dest[MAX_CVAR_VALUE_STRING-1]==0 bk001129 - paranoia. Never trust the
  // destination string. bk001129 - beware, sizeof(char*) is always 4 (for
  // cv->string).
