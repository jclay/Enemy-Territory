...
inline idStr::idStr(const char* text) : m_data(NULL) {
  int len;
...
inline idStr::idStr(const idStr& text) : m_data(NULL) {
  m_data = text.m_data;
  m_data->AddRef();
}

inline idStr::idStr(const idStr text, int start, int end) : m_data(NULL) {
  int i;
  int len;
...
inline idStr::idStr(const char ch) : m_data(NULL) {
  EnsureAlloced(2);
...
inline idStr::idStr(const float num) : m_data(NULL) {
  char text[32];
  int len;
...
inline idStr::idStr(const int num) : m_data(NULL) {
  char text[32];
  int len;
...
inline idStr::idStr(const unsigned num) : m_data(NULL) {
  char text[32];
  int len;
...
inline void idStr::operator=(const char* text) {
  int len;
...
inline int idStr::icmpn(const idStr& text, int n) const {
  assert(m_data);
  assert(text.m_data);
...
inline int idStr::icmp(const idStr& text) const {
  assert(c_str());
  assert(text.c_str());
...
inline int idStr::cmp(const idStr& text) const {
  assert(c_str());
  assert(text.c_str());
...
inline int idStr::cmpn(const idStr& text, int n) const {
  assert(c_str());
  assert(text.c_str());
