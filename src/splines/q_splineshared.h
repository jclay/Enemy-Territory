...
typedef const idVec3&
    vec3_c;  // for passing vectors as const function arguments

class angles_t;              // for defining angle vectors
typedef angles_t& angles_p;  // for passing angles as function arguments
typedef const angles_t&
    angles_c;  // for passing angles as const function arguments

class mat3_t;            // for defining matrices
typedef mat3_t& mat3_p;  // for passing matrices as function arguments
typedef const mat3_t&
    mat3_c;  // for passing matrices as const function arguments
...
void VectorNormalizeFast(
    vec3_p v);  // does NOT return vector length, uses rsqrt approximation
...
void VectorRotate3Origin(vec3_c vIn,
                         vec3_c vRotation,
                         vec3_c vOrigin,
                         vec3_p out);
...
void RotatePointAroundVector(vec3_p dst,
                             vec3_c dir,
                             vec3_c point,
                             float degrees);
...
void MatrixInverseMultiply(mat3_c in1,
                           mat3_c in2,
                           mat3_p out);  // in2 is transposed during multiply
void MatrixTransformVector(vec3_c in, mat3_c matrix, vec3_p out);
void MatrixProjectVector(
    vec3_c in,
    mat3_c matrix,
    vec3_p out);  // Places the vector into a new coordinate system.
...
// int			Com_Filter( const char *filter, const char *name, int
// casesensitive ); const char *Com_StringContains( const char *str1, const char
// *str2, int casesensitive );
...
qboolean GetVectorForMapEntityKey(const mapEntity_t* ent,
                                  const char* key,
                                  idVec3& vec);
