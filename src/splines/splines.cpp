...
qboolean getCameraInfo(int camNum,
                       int time,
                       float* origin,
                       float* angles,
                       float* fov) {
...
void glLabeledPoint(idVec3& color,
                    idVec3& point,
                    float size,
                    const char* label) {
  qglColor3fv(color);
...
// extern void D_DebugLine( const idVec3 &color, const idVec3 &start, const
// idVec3 &end );

void debugLine(idVec3& color,
               float x,
               float y,
               float z,
               float x2,
  ...
  for (int i = 0; i < events.Num(); i++) {
    if (time >= startTime + events[i]->getTime() &&
      ...
      if (events[i]->getType() == idCameraEvent::EVENT_TARGET) {
        ...
        // Com_Printf("Triggered event switch to target:
        // %s\n",events[i]->getParam());
  ...
  for (i = 0; i < events.Num(); i++) {
    ...
    switch (events[i]->getType()) {
      ...
      case idCameraEvent::EVENT_WAIT: {
        ...
        cameraPosition->addVelocity(
            events[i]->getTime(), (int)(atof(events[i]->getParam()) * 1000), 0);

        startTime = (long int)(events[i]->getTime() +
                               atof(events[i]->getParam()) * 1000);
        long endTime = startTime + 1000;
        speed = 0;
        while (startTime <= endTime) {
          cameraPosition->addVelocity(startTime, loopTime, speed);
      ...
      case idCameraEvent::EVENT_SPEED: {
          /*
                                          // take the average delay between up
             to the next five segments float adjust =
             atof(events[i]->getParam()); int index = events[i]->getSegment();
                                          total = 0;
                                          count = 0;
             ...
             cameraSpline.getSegmentTime(j + 1) -
             cameraSpline.getSegmentTime(j); count++;
                                          ...
                                          // now propogate that difference out
             to each segment for (j = index; j < cameraSpline.numSegments();
             j++) { cameraSpline.addSegmentTime(j, additive); additive +=
             newTotal;
                                          }
                                          break;
          */
