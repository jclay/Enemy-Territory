...
void AAS_InitSettings(void) {
  ...
  aassettings.sv_maxbarrier =
      49;  //-0.8 + (0.5 * aassettings.sv_gravity * (aassettings.sv_jumpvel /
           //aassettings.sv_gravity) * (aassettings.sv_jumpvel /
           //aassettings.sv_gravity));
           // done.
...
int AAS_AgainstLadder(vec3_t origin, int ms_areanum) {
  ...
  for (i = 0; i < area->numfaces; i++) {
    ...
    if (abs(DotProduct(plane->normal, origin) - plane->dist) < 7) {
      // RF, if hanging on to the edge of a ladder, we have to account for
      // bounding box touching
...
int AAS_OnGround(vec3_t origin, int presencetype, int passent) {
  ...
  trace = AAS_Trace(origin, mins, maxs, end, passent,
...
// Changes Globals:		-
//===========================================================================
void AAS_Accelerate(vec3_t velocity,
                    float frametime,
                    vec3_t wishdir,
                    float wishspeed,
                    float accel) {
...
// Changes Globals:		-
//===========================================================================
void AAS_ApplyFriction(vec3_t vel,
                       float friction,
                       float stopspeed,
                       float frametime) {
...
//
// Parameter:				origin			: origin to
// start with
//								presencetype
//: presence type to start with 								velocity			: velocity to
//start with 								cmdmove			: client command movement 								cmdframes
//: number of frame cmdmove is valid 								maxframes		: maximum number of
//predicted frames 								frametime		: duration of one predicted frame
//								stopevent
//: events that stop the prediction 						stopareanum		: stop as soon as
//entered this area
...
int AAS_PredictClientMovement(struct aas_clientmove_s* move,
  ...
  if (visualize) {
// These debugging tools are not currently available in bspc. Mad Doctor I,
// 1/27/2003.
#ifndef BSPC
  ...
  trace = AAS_Trace(org, mins, maxs, org, entnum,
                    (CONTENTS_SOLID | CONTENTS_PLAYERCLIP) & ~CONTENTS_BODY);
  ...
  for (n = 0; n < maxframes; n++) {
    ...
    if (cmdframes < 0) {
      ...
      for (i = 0; i < ax; i++)
      {
              ...
              if (velchange > sv_maxacceleration) velchange =
      sv_maxacceleration; else if (velchange < -sv_maxacceleration) velchange =
      -sv_maxacceleration; newvel = frame_test_vel[i] + velchange;
              //
              if (frame_test_vel[i] <= maxvel && newvel > maxvel)
      frame_test_vel[i] = maxvel; else if (frame_test_vel[i] >= -maxvel &&
      newvel < -maxvel) frame_test_vel[i] = -maxvel; else frame_test_vel[i] =
      newvel; } //end for
      */
    }  // end if
       // if (crouch)
    ...
    do {
      ...
      trace =
          AAS_Trace(org, mins, maxs, end, entnum,
                    (CONTENTS_SOLID | CONTENTS_PLAYERCLIP) & ~CONTENTS_BODY);
      //
      ...
      if (stopevent & SE_ENTERAREA) {
        ...
        for (i = 0; i < numareas; i++) {
          if (areas[i] == stopareanum) {
            ...
            move->presencetype =
                (*aasworld).areasettings[areas[i]].presencetype;
            move->endcontents = 0;
            move->time = n * frametime;
            move->frames = n;
            return qtrue;
      ...
      if (trace.fraction < 1.0) {
        ...
        if (stopevent & SE_HITGROUNDAREA) {
          if (DotProduct(plane->normal, up) > sv_maxsteepness) {
            ...
            start[2] += 0.5;
            if ((stopareanum < 0 && AAS_PointAreaNum(start)) ||
              ...
              move->presencetype =
                  (*aasworld).areasettings[stopareanum].presencetype;
              move->endcontents = 0;
              move->time = n * frametime;
              move->frames = n;
              return qtrue;
        ...
        if (plane->normal[2] == 0 && (jump_frame < 0 || n - jump_frame > 2)) {
          ...
          start[2] += sv_maxstep;
          // steptrace = AAS_TraceClientBBox(start, stepend, PRESENCE_NORMAL,
          // entnum);
          steptrace = AAS_Trace(
              start, mins, maxs, stepend, entnum,
              (CONTENTS_SOLID | CONTENTS_PLAYERCLIP) & ~CONTENTS_BODY);
          //
            ...
            if (delta) {
              ...
              */
              if (delta > 40) {
                ...
                if (areanum) {
                  move->presencetype =
                      (*aasworld).areasettings[areanum].presencetype;
                }
                move->endcontents = 0;
                move->time = n * frametime;
                move->frames = n;
    ...
    if (onground) {
      ...
      gaptrace =
          AAS_Trace(start, mins, maxs, end, -1,
                    (CONTENTS_SOLID | CONTENTS_PLAYERCLIP) & ~CONTENTS_BODY);
      // if solid is found the bot cannot walk any further and will not fall
      // into a gap
      if (!gaptrace.startsolid) {
        ...
        if (gaptrace.endpos[2] < org[2] - aassettings.sv_maxstep - 1) {
          if (!(AAS_PointContents(end) &
                (CONTENTS_WATER |
                 CONTENTS_SLIME))) {  //----(SA)	modified since slime is
                                      //no longer deadly
                                      //					if (!(AAS_PointContents(end) &
                                      //CONTENTS_WATER))
    ...
    if (stopevent & SE_TOUCHJUMPPAD) {
      if ((*aasworld).areasettings[AAS_PointAreaNum(org)].contents &
  ...
  move->stopevent = SE_NONE;
  move->presencetype =
      aasworld->areasettings ? aasworld->areasettings[areanum].presencetype : 0;
  move->endcontents = 0;
  move->time = n * frametime;
  move->frames = n;
  //
...
void AAS_TestMovementPrediction(int entnum, vec3_t origin, vec3_t dir) {
  ...
  AAS_PredictClientMovement(&move, entnum, origin, PRESENCE_NORMAL, qtrue,
                            velocity, cmdmove, 13, 13, 0.1, SE_HITGROUND, 0,
...
//								start
//: start position of jump 								end		: end position of jump 								*speed	:
//returned speed for jump
// Returns:					qfalse if too high or too far
// from start to end Changes Globals:		-
//===========================================================================
