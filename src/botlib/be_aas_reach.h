...
#define AAS_AreaCrouch(areanum)                                        \
  ((!(aasworld->areasettings[areanum].presencetype & PRESENCE_NORMAL)) \
       ? qtrue                                                         \
       : qfalse)

// returns true if a player can swim in this area
// int AAS_AreaSwim(int areanum);
#define AAS_AreaSwim(areanum) \
  ((aasworld->areasettings[areanum].areaflags & AREA_LIQUID) ? qtrue : qfalse)
