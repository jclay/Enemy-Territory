...
#define AREA_TEAM_FLAGS                                           \
  (AREA_TEAM_AXIS | AREA_TEAM_ALLIES | AREA_TEAM_AXIS_DISGUISED | \
   AREA_TEAM_ALLIES_DISGUISED | AREA_AVOID_AXIS | AREA_AVOID_ALLIES)
...
typedef struct aas_areasettings_s {
  ...
  // Ridah, add a ground steepness stat, so we can avoid terrain when we can
  // take a close-by flat route
...
typedef struct aas_node_s {
  int planenum;
  int children[2];  // child nodes of this node, or convex areas as leaves when
