...
void AAS_DrawPlaneCross(vec3_t point,
  ...
  start1[n0] =
      (dist - (start1[n1] * normal[n1] + start1[n2] * normal[n2])) / normal[n0];
  end1[n0] =
      (dist - (end1[n1] * normal[n1] + end1[n2] * normal[n2])) / normal[n0];
  start2[n0] =
      (dist - (start2[n1] * normal[n1] + start2[n2] * normal[n2])) / normal[n0];
  end2[n0] =
...
void AAS_ShowReachability(aas_reachability_t* reach) {
  ...
  if (reach->traveltype == TRAVEL_JUMP ||
    ...
    AAS_HorizontalVelocityForJump(aassettings.sv_jumpvel, reach->start,
                                  reach->end, &speed);
    //
