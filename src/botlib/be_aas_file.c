...
void AAS_SwapAASData(void) {
  ...
  for (i = 0; i < (*aasworld).numareasettings; i++) {
    ...
    (*aasworld).areasettings[i].groundsteepness =
        LittleFloat((*aasworld).areasettings[i].groundsteepness);
  }  // end for
     // area reachability
...
// Changes Globals:		-
//===========================================================================
char* AAS_LoadAASLump(fileHandle_t fp,
                      int offset,
                      int length,
                      int* lastoffset) {
  char* buf;
...
int AAS_LoadAASFile(char* filename) {
  ...
  if (header.version != AASVERSION) {
    ...
    return BLERR_WRONGAASFILEVERSION;
     ...
     //
     // RF, checksum of -1 always passes, hack to fix commercial maps without
     // having to distribute new bsps
  nocrc = 0;
  ...
  (*aasworld).bboxes =
      (aas_bbox_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).vertexes =
      (aas_vertex_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).planes =
      (aas_plane_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).edges =
      (aas_edge_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).edgeindex =
      (aas_edgeindex_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).faces =
      (aas_face_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).faceindex =
      (aas_faceindex_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).areas =
      (aas_area_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).areasettings =
      (aas_areasettings_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).reachability =
      (aas_reachability_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).nodes =
      (aas_node_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).portals =
      (aas_portal_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).portalindex =
      (aas_portalindex_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
  ...
  (*aasworld).clusters =
      (aas_cluster_t*)AAS_LoadAASLump(fp, offset, length, &lastoffset);
...
int AAS_WriteAASLump(fileHandle_t fp,
                     aas_header_t* h,
                     int lumpnum,
                     void* data,
                     int length) {
