...
// NOTE: the moveflags MFL_ONGROUND, MFL_TELEPORTED and MFL_WATERJUMP must be
// set outside the movement code
...
// Ridah, provide a means of resetting the avoidreach, so if a bot stops moving,
...
==================
*/
int BotFirstReachabilityArea(vec3_t origin,
                             int* areas,
                             int numareas,
                             qboolean distCheck) {
  int i, best = 0;
  ...
  for (i = 0; i < numareas; i++) {
    if (AAS_AreaReachability(areas[i])) {
      ...
      if (distCheck) {
        ...
        if (dist < bestDist) {
          tr = AAS_Trace(origin, NULL, NULL, center, -1,
...
// float f;
#define BOTAREA_JIGGLE_DIST \
  256  // 32	// OUCH, hack for MP_BEACH which has lots of voids (mostly in
       // water)
  ...
  /*
          ...
          for (f=0.1; f<=1.0; f+=0.45) {
                  ...
                  if (numareas > 0) bestarea = BotFirstReachabilityArea(origin,
     areas, numareas); if (bestarea) return bestarea;
          }
  */
  return 0;
  /*
          ...
          for (z = 1; z >= -1; z -= 1)
          {
                  for (x = 1; x >= -1; x -= 1)
                  {
                          ...
                          {
                                  VectorCopy(origin, end);
                                  // Ridah, increased this for Wolf larger
     bounding boxes end[0] += x * 16;//8; end[1] += y * 16;//8; end[2] += z *
     24;//12; numareas = AAS_TraceAreas(origin, end, areas, points, 10); for (j
     = 0; j < numareas; j++)
                                  {
                                          if (AAS_AreaReachability(areas[j]))
                                          {
                                                  VectorSubtract(points[j],
     origin, v); dist = VectorLength(v); if (dist < bestdist)
                                                  {
                                                          bestareanum =
     areas[j]; bestdist = dist; } //end if } //end if if (!firstareanum)
     firstareanum = areas[j]; } //end for } //end for } //end for if
     (bestareanum) return bestareanum; } //end for return firstareanum;
  */
...
again:
  ...
  if (AAS_AreaDoNotEnterLarge(areanum) ||
      AAS_AreaDoNotEnterLarge(goal->areanum)) {
  ...
  for (reachnum = AAS_NextAreaReachability(areanum, 0); reachnum;
    ...
    for (i = 0; i < MAX_AVOIDREACH; i++) {
      if (avoidreach[i] == reachnum && avoidreachtimes[i] >= AAS_Time()) {
       ...
       //  RF, if this is a "useAvoidPass" then we should only accept avoidreach
       //  reachabilities
    if ((!useAvoidPass && i != MAX_AVOIDREACH &&
         avoidreachtries[i] > AVOIDREACH_TRIES) ||
        (useAvoidPass &&
         !(i != MAX_AVOIDREACH && avoidreachtries[i] > AVOIDREACH_TRIES))) {
#ifdef DEBUG
      if (bot_developer) {
    ...
    // get the travel time
    t = AAS_AreaTravelTimeToGoalArea(reach.areanum, reach.end, goal->areanum,
                                     travelflags);
    ...
    //		if (AAS_AreaTravelTimeToGoalArea(areanum, reach.start,
    //goal->areanum, travelflags) + reach.traveltime == t) 			continue;

    // add the travel time towards the area
    //  Ridah, not sure why this was disabled, but it causes looped links in the
    //  route-cache
    ...
    //		if (reach.areanum != goal->areanum && AAS_IsEntityInArea(
    //entnum, goal->entitynum, reach.areanum )) { 			t += 50;
    //		}
    ...
    if (!besttime || t < besttime) {
     ...
     //
     //  RF, if we didnt find a route, then try again only looking through
     //  avoidreach reachabilities
...
// Changes Globals:		-
//===========================================================================
int BotAddToTarget(vec3_t start,
                   vec3_t end,
                   float maxdist,
                   float* dist,
                   vec3_t target) {
...
int BotMovementViewTarget(int movestate,
  ...
  while (reachnum && dist < lookahead) {
    ...
    reachnum = BotGetReachabilityToGoal(
        reach.end, reach.areanum, -1, ms->lastgoalareanum, lastareanum,
...
int BotPredictVisiblePosition(vec3_t origin,
  ...
  for (i = 0; i < 20 && (areanum != goal->areanum); i++) {
    //
    reachnum = BotGetReachabilityToGoal(
        end, areanum, -1, lastgoalareanum, lastareanum, avoidreach,
        avoidreachtimes, avoidreachtries, goal, travelflags, travelflags);
    if (!reachnum) {
      return qfalse;
    }
    AAS_ReachabilityFromNum(reachnum, &reach);
  ...
  for (dist = 8; dist <= 100; dist += 8) {
    ...
    if (!trace.startsolid) {
      ...
      if (trace.endpos[2] < startz - sv_maxstep - 8) {
        ...
        //CONTENTS_WATER) break; if a gap is found slow down
...
int BotWalkInDirection(bot_movestate_t* ms, vec3_t dir, float speed, int type) {
  ...
  if (ms->moveflags & MFL_ONGROUND) {
    ...
    if (type & MOVE_JUMP) {
      ...
      cmdframes = 2;
      stopevent =
          SE_HITGROUNDDAMAGE | SE_ENTERWATER | SE_ENTERSLIME | SE_ENTERLAVA;
    }  // end else
    ...
    if (move.stopevent &
        (SE_ENTERLAVA | SE_HITGROUNDDAMAGE)) {  //----(SA)	modified since
                                                //slime is no longer deadly
                                                //		if (move.stopevent &
      //(SE_ENTERSLIME|SE_ENTERLAVA|SE_HITGROUNDDAMAGE))
      // botimport.Print(PRT_MESSAGE, "client %d: would be hurt ", ms->client);
      // if (move.stopevent & SE_ENTERSLIME) botimport.Print(PRT_MESSAGE,
      // "slime\n"); if (move.stopevent & SE_ENTERLAVA)
      // botimport.Print(PRT_MESSAGE, "lava\n"); if (move.stopevent &
      // SE_HITGROUNDDAMAGE) botimport.Print(PRT_MESSAGE, "hitground\n");
      return qfalse;
    }  // end if
       // if ground was hit
...
// Changes Globals:		-
//===========================================================================
void BotCheckBlocked(bot_movestate_t* ms,
                     vec3_t dir,
                     int checkbottom,
...
bot_moveresult_t BotTravel_Walk(bot_movestate_t* ms,
  ...
  if (dist < 32) {
    ...
    dist = VectorNormalize(hordir);
    /*
                    // if we are really close to the line, then move towards the
       center of the reach area VectorCopy( reach->start, v1 ); VectorCopy(
       reach->end, v2 ); VectorCopy( ms->origin, p ); v1[2] = 0; v2[2] = 0; p[2]
       = 0; if (DistanceFromVectorSquared( p, v1, v2 ) < 4) { if
       (!AAS_AreaWaypoint( reach->areanum, p )) AAS_AreaCenter( reach->areanum,
       p ); if (VectorDistance( ms->origin, p ) > 32) { VectorSubtract( p,
       ms->origin, hordir ); hordir[2] = 0; dist = VectorNormalize(hordir);
                    }
                    }
    */
  } else {
    // botimport.Print(PRT_MESSAGE, "BotTravel_Walk: NOT in range of reach
    // (%i)\n", reach->areanum);
  }  // end if
  ...
  //
  // botimport.Print(PRT_MESSAGE, "\nBotTravel_Walk: srcarea %i, rcharea %i, org
  // %s, reachorg %s\n", ms->areanum, reach->areanum, vtosf(ms->origin),
  // vtosf(reach->start) );
  //
  return result;
...
bot_moveresult_t BotFinishTravel_BarrierJump(bot_movestate_t* ms,
  ...
  if (ms->velocity[2] < 250) {
    // Ridah, extend the end point a bit, so we strive to get over the ledge
    // more
    vec3_t end;
...
bot_moveresult_t BotTravel_WalkOffLedge(bot_movestate_t* ms,
  ...
  // Ridah, tweaked this
	...
	if ( dist < 48 ) {
		...
		hordir[2] = 0;
#else
  if ((dist < 72) && (DotProduct(dir, hordir) <
                      0)) {  // walk in the direction of start -> end
                             // hordir[0] = reach->end[0] - ms->origin[0];
                             // hordir[1] = reach->end[1] - ms->origin[1];
                             // VectorNormalize( dir );
...
// Changes Globals:		-
//===========================================================================
int BotAirControl(vec3_t origin,
                  vec3_t velocity,
                  vec3_t goal,
                  vec3_t dir,
                  float* speed) {
...
bot_moveresult_t BotTravel_Ladder(bot_movestate_t* ms,
  ...
  if (reach->start[2] > reach->end[2]) {
    if (!(ms->moveflags & MFL_ONGROUND)) {
      ...
      if (dist >= 3.1) {  // vertical ladder -> ladder reachability has length
                          // 3, dont inverse in this case
      ...
      // RF, disabled this check, if we're not on ground, then it shouldn't be a
      // problem, and the ladder flag is unreliable
      // if (ms->moveflags & MFL_AGAINSTLADDER) {
      // botimport.Print(PRT_MESSAGE, "against ladder, descending\n");
      EA_MoveBack(
          ms->client);  // only move back if we are touching the ladder brush
      ...
      if (dist < 64) {  // within range, go for the end
        // botimport.Print(PRT_MESSAGE, "found top, moving towards ladder
        // edge\n");
        VectorSubtract(reach->end, reach->start, dir);
        // make sure the horizontal movement is large anough
        dir[2] = 0;
        ...
        Vector2Angles(viewdir, result.ideal_viewangles);
        result.flags |= MOVERESULT_MOVEMENTVIEW;
        // if we are still on ground, then start moving backwards until we are
        // in air
        if ((dist < 4) && (ms->moveflags & MFL_ONGROUND)) {
          // botimport.Print(PRT_MESSAGE, "close to edge, backing in
          // slowly..\n");
    ...
    if ((ms->moveflags & MFL_AGAINSTLADDER)
      ...
      if (dist < 16) {  // within range, go for the end
        // botimport.Print(PRT_MESSAGE, "found base, moving towards ladder
        // top\n");
        VectorSubtract(reach->end, ms->origin, dir);
        // make sure the horizontal movement is large anough
        VectorCopy(dir, hordir);
...
// Changes Globals:		-
//===========================================================================
void BotFuncBobStartEnd(aas_reachability_t* reach,
                        vec3_t start,
                        vec3_t end,
                        vec3_t origin) {
...
int GrappleState(bot_movestate_t *ms, aas_reachability_t *reach)
{
        ...
        {
                grapplemodelindex =
AAS_IndexFromModel("models/weapons/grapple/hook/tris.md2"); } //end if for (i =
AAS_NextEntity(0); i; i = AAS_NextEntity(i))
        {
...
bot_moveresult_t BotTravel_RocketJump(bot_movestate_t* ms,
  ...
  /*
          ...
          {
  //		botimport.Print(PRT_MESSAGE, "between jump start and run start
  point\n"); hordir[0] = reach->end[0] - ms->origin[0]; hordir[1] =
  reach->end[1] - ms->origin[1]; hordir[2] = 0; VectorNormalize(hordir);
                  //elemantary action jump
                  if (dist1 < 24) EA_Jump(ms->client);
                  else if (dist1 < 32) EA_DelayedJump(ms->client);
                  EA_Attack(ms->client);
...
void BotMoveToGoal(bot_moveresult_t* result,
      ...
      if (modelnum >= 0 && modelnum < MAX_MODELS) {
        ...
        if (modeltype == MODELTYPE_FUNC_PLAT) {
          ...
          if (reach.traveltype != TRAVEL_ELEVATOR ||
            ...
            reachnum = AAS_NextModelReachability(0, modelnum);
            if (reachnum) {
              // botimport.Print(PRT_MESSAGE, "client %d: accidentally ended up
              // on func_plat\n", ms->client);
              AAS_ReachabilityFromNum(reachnum, &reach);
  ...
  if (ms->moveflags & (MFL_ONGROUND | MFL_SWIMMING | MFL_AGAINSTLADDER)) {
      ...
      if (!(AAS_TravelFlagForType(reach.traveltype) & travelflags)) {
        ...
        if ((result->flags & MOVERESULT_ONTOPOF_FUNCBOB) ||
          ...
          ms->reachability_time = AAS_Time() + 5;
           ...
           // if the bot was going for an elevator and reached the reachability
           // area
        if (ms->areanum == reach.areanum ||
        ...
        if (bot_developer) {
          if (ms->reachability_time < AAS_Time()) {
            botimport.Print(PRT_MESSAGE, "client %d: reachability timeout in ",
                            ms->client);
        ...
        if (ms->lastgoalareanum != goal->areanum ||
            ...
            //@TODO.  The hardcoded distance here should actually be tied to
            //speed.  As it was, 20 was too big for
            // slow moving walking Nazis.
            //						((ms->lasttime >
            //(AAS_Time()-0.5)) && (VectorDistance(ms->origin, ms->lastorigin) <
            //20*(AAS_Time()-ms->lasttime))))
            ((ms->lasttime > (AAS_Time() - 0.5)) &&
             (VectorDistance(ms->origin, ms->lastorigin) <
              5 * (AAS_Time() - ms->lasttime)))) {
          reachnum = 0;
    ...
    if (!reachnum) {
      ...
      reachnum = BotGetReachabilityToGoal(
          ms->origin, ms->areanum, ms->entitynum, ms->lastgoalareanum,
          ms->lastareanum, ms->avoidreach, ms->avoidreachtimes,
          ms->avoidreachtries, goal, travelflags, travelflags);
      ...
      if (reachnum) {
        ...
        ms->reachability_time =
            AAS_Time() +
            (float)(0.01 * (AAS_AreaTravelTime(ms->areanum, ms->origin,
                                               reach.start) *
                            2) +
...
#ifdef AVOIDREACH
        if (reach.traveltype !=
            TRAVEL_LADDER) {  // don't avoid ladders unless we were unable to
                              // reach them in time
    ...
    for (i = numareas - 1; i >= 0; i--) {
      if (AAS_AreaJumpPad(areas[i])) {
        // botimport.Print(PRT_MESSAGE, "client %d used a jumppad without
        // knowing, area %d\n", ms->client, areas[i]);
        foundjumppad = qtrue;
        lastreachnum = BotGetReachabilityToGoal(
            end, areas[i], ms->entitynum, ms->lastgoalareanum, ms->lastareanum,
            ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries, goal,
            travelflags, TFL_JUMPPAD);
        if (lastreachnum) {
    ...
    if (bot_developer) {
      ...
      if (foundjumppad && !ms->lastreachnum) {
       ...
       //
    if (ms->lastreachnum) {
      // botimport.Print(PRT_MESSAGE, "%s: NOT onground, swimming or against
      // ladder\n", ClientName(ms->entitynum-1));
      AAS_ReachabilityFromNum(ms->lastreachnum, &reach);
      result->traveltype = reach.traveltype;
#ifdef DEBUG
      ...
      if (bot_developer) {
        if (result->failure) {
          botimport.Print(PRT_MESSAGE, "client %d: movement failure in finish ",
                          ms->client);
          AAS_PrintTravelType(reach.traveltype);
          botimport.Print(PRT_MESSAGE, "\n");
        }  // end if
  ...
  /*
          ...
          if (reachnum > 0 && !(result->flags &
     (MOVERESULT_MOVEMENTVIEW|MOVERESULT_SWIMVIEW))) { vec3_t dir; int
     ftraveltime, freachnum;

                  AAS_ReachabilityFromNum( reachnum, &reach);
                  if (reach.areanum != goal->areanum) {
                          if (AAS_AreaRouteToGoalArea( reach.areanum, reach.end,
     goal->areanum, travelflags, &ftraveltime, &freachnum )) {
                                          AAS_ReachabilityFromNum( freachnum,
     &reach); VectorSubtract( reach.end, ms->origin, dir ); VectorNormalize( dir
     ); vectoangles( dir, result->ideal_viewangles ); result->flags |=
     MOVERESULT_FUTUREVIEW;
                                          }
                  } else {
