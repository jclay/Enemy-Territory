...
#define MAX_VISIBLE_AREAS \
  1024  // going over this limit will result in excessive memory usage, try and
        // keep RANGE low enough so this limit won't be reached
#define MAX_LOCALTRAVELTIME \
  60  // use this to tweak memory usage (reduces parent count, increases local
      // count (and cpu usage) - find a balance)
...
typedef struct {
  unsigned short int reachable_index;  // reachability index (from this area's
                                       // first reachability link in the world)
                                       // to head for to get to the destination
...
typedef struct {
  ...
  int startVisibleParents;  // list of other parents that we can see (used for
                            // fast hide/retreat checks)
...
typedef struct {
  unsigned short int*
      areaChildIndexes;  // each aas area that is part of the Route-Table has a
                         // pointer here to their position in the list of
                         // children
  ...
  //	int							numLocalRoutes;
  //	aas_rt_route_t				*localRoutes;		// the
  //list of routes to all other local areas

  //	int							numParentRoutes;
  //	unsigned char				*parentRoutes;		//
  //reachability to each other parent, as an offset from our first reachability

  int numParentLinks;
  aas_rt_parent_link_t*
      parentLinks;  // links from each child to the parent's it belongs to

  //	int numParentTravelTimes; 	unsigned short int
  //*parentTravelTimes;	// travel times between all parent areas

  //	int							numRouteIndexes;
  //	unsigned short int			*routeIndexes;		// each
  //parentLink has a list within here, which
...
typedef struct {
  unsigned short int
      numvisible;  // number of areas that are visible and within range
  unsigned short int
      visible[MAX_VISIBLE_AREAS];  // list of area indexes of visible and within
                                   // range areas
...
typedef struct aas_parent_link_s {
  ...
  unsigned short int childindex;  // our index in the parent's list of children
  unsigned short int*
      routeindexes;  // for this parent link, list the children that fall under
                     // that parent, and their associated indexes in our
                     // localroutes table
...
typedef struct {
  ...
  unsigned short int* visibleParents;  // list of other parents that we can see
                                       // (used for fast hide/retreat checks)
} aas_area_parent_t;
