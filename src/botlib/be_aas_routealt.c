...
int AAS_AlternativeRouteGoals(vec3_t start,
  ...
  for (i = 1; i < (*aasworld).numareas; i++) {
    ...
    // if the travel time from the area to the goal is greater than the shortest
    // goal travel time
  ...
  for (i = 1; i < (*aasworld).numareas; i++) {
    ...
    for (j = 0; j < numclusterareas; j++) {
      ...
      if (dist < bestdist) {
       ...
       //  make sure the route to the destination isn't in the same direction as
       //  the route to the source
    if (!AAS_AreaRouteToGoalArea(bestareanum,
                                 (*aasworld).areawaypoints[bestareanum],
                                 goalareanum, travelflags, &time, &reachnum)) {
      continue;
    }
    a1 = (*aasworld).reachability[reachnum].areanum;
    if (!AAS_AreaRouteToGoalArea(bestareanum,
                                 (*aasworld).areawaypoints[bestareanum],
                                 startareanum, travelflags, &time, &reachnum)) {
      continue;
    }
    ...
    VectorCopy((*aasworld).areawaypoints[bestareanum],
    ...
    //
    /*#ifdef DEBUG
                    botimport.Print(PRT_MESSAGE, "alternative route goal area
    %d, numclusterareas = %d\n", bestareanum, numclusterareas); if (color)
                    {
                            AAS_DrawPermanentCross((*aasworld).areas[bestareanum].center,
    10, color); } //end if
