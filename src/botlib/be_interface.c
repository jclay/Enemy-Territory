...
int Export_BotLibLoadMap(const char* mapname) {
  ...
  if (!mapname) {
    ...
    // don't init the heap if no aas loaded, causes "SV_Bot_HunkAlloc: Alloc
    // with marks already set"
    if ((*aasworld).loaded) {
      AAS_InitAASLinkHeap();
      AAS_EnableAllAreas();
    }
    ...
    memset((*aasworld).arealinkedentities, 0,
           (*aasworld).numareas * sizeof(aas_link_t*));
    memset((*aasworld).entities, 0,
...
int BotExportTest(int parm0, char* parm1, vec3_t parm2, vec3_t parm3) {
  ...
  if (bot_testhidepos) {
    ...
    if (parm0 & 1) {
      ...
      botimport.Print(PRT_MESSAGE,
                      "new enemy position %2.1f %2.1f %2.1f area %d\n",
                      origin[0], origin[1], origin[2], newarea);
    }  // end if

    AAS_ClearShownPolygons();
    AAS_ClearShownDebugLines();
    hideposarea = AAS_NearestHideArea(
        0, origin, AAS_PointAreaNum(origin), 0, botlibglobals.goalorigin,
        botlibglobals.goalareanum, TFL_DEFAULT, 99999, NULL);
  ...
  if (bot_debug == 9) {
    ...
    this_success = AAS_PredictClientMovement(
        &move, 0, origin, -1, qfalse, forward, dest, -1, 40, 0.05,
        SE_ENTERAREA | SE_HITGROUNDDAMAGE | SE_HITENT | SE_HITGROUNDAREA |
            SE_STUCK | SE_GAP,
        botlibglobals.goalareanum, qtrue);
  ...
  botimport.Print(PRT_MESSAGE, "\rtravel time to goal (%d) = %d  ",
                  ...
                  AAS_AreaTravelTimeToGoalArea(
                      newarea, origin, botlibglobals.goalareanum, TFL_DEFAULT));
  if (newarea != area) {
    ...
    botimport.Print(
        ...
        botlibglobals.goalareanum,
        AAS_AreaTravelTimeToGoalArea(newarea, origin, botlibglobals.goalareanum,
  ...
  if (parm0 & 2) {
    ...
    reachnum = BotGetReachabilityToGoal(origin, newarea, -1, lastgoalareanum,
                                        lastareanum, avoidreach,
                                        avoidreachtimes, avoidreachtries, &goal,
                                        TFL_DEFAULT | TFL_FUNCBOB, TFL_DEFAULT);
