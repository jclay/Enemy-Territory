...
void AAS_UpdateStringIndexes(int numconfigstrings, char* configstrings[]) {
  ...
  for (i = 0; i < numconfigstrings; i++) {
    if (configstrings[i]) {
      // if ((*aasworld).configstrings[i])
      // FreeMemory((*aasworld).configstrings[i]);
...
int AAS_Setup(void) {
  ...
  // LibVarSet("forceclustering", "1");			//force clustering
  // calculation LibVarSet("forcereachability", "1");		//force
