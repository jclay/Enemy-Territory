...
int AAS_UpdateEntity(int entnum, bot_entitystate_t* state) {
  ...
  //of changing the aas_entityinfo_t and bot_entitystate_t structures.
  ...
  if (ent->i.solid == SOLID_BSP) {
    ...
    // AAS_BSPModelMinsMaxsOrigin(ent->i.modelindex, ent->i.angles, ent->i.mins,
    // ent->i.maxs, NULL);
  ...
  if (relink) {
    ...
    if (entnum != ENTITYNUM_WORLD) {
      ...
      ent->areas =
          AAS_LinkEntityClientBBox(absmins, absmaxs, entnum, PRESENCE_NORMAL);
...
void AAS_EntityInfo(int entnum, aas_entityinfo_t* info) {
  // Gordon: lets not spam this message making it impossible to see anything on
  // the console
  ...
  if (!(*defaultaasworld).initialized) {
    if (!debug_msg_done) {
      ...
      botimport.Print(PRT_FATAL,
                      "AAS_EntityInfo: (*defaultaasworld) not initialized\n");
  ...
  if (entnum < 0 || entnum >= (*defaultaasworld).maxentities) {
    ...
    if (!(entnum >= (*defaultaasworld).maxentities &&
          entnum < (*defaultaasworld).maxentities + NUM_BOTGAMEENTITIES)) {
  ...
  memcpy(info, &(*defaultaasworld).entities[entnum].i,
...
// Ridah, used to find out if there is an entity touching the given area, if so,
// try and avoid it
/*
============
AAS_EntityInArea
...
areas_again:
  ...
  //
  if (mover && !changed) {  // map must not be compiled with MOVER flags
                            // enabled, so redo the old way
    mover = qfalse;
    goto areas_again;
  }
}
