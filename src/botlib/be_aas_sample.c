...
void AAS_InitAASLinkHeap(void) {
  ...
  if (!(*aasworld).linkheap) {
    ...
    memset((*aasworld).linkheap, 0,
           (*aasworld).linkheapsize * sizeof(aas_link_t));
  }
...
#endif  // AAS_SAMPLE_DEBUG
  /*
                  // RF (HACK), if we failed, move us to the other side of the
     closest plane if ((recursion < 10) && closestPlane) { dist = closestDist;
                          node = closestNode;
                          plane = closestPlane;
                          if (dist > 0) {
                                  VectorMA( point, -(dist+1), plane->normal,
     point ); } else { VectorMA( point, -(dist-1), plane->normal, point );
                          }
                          // take the opposite side since we have moved the
     point there now if (dist <= 0) nodenum = node->children[0]; else nodenum =
     node->children[1];
                          //
                          goto nodesearch;
...
// Changes Globals:		-
//===========================================================================
qboolean AAS_AreaEntityCollision(int areanum,
                                 vec3_t start,
                                 vec3_t end,
                                 int presencetype,
                                 int passent,
...
// Changes Globals:		-
//===========================================================================
aas_trace_t AAS_TraceClientBBox(vec3_t start,
                                vec3_t end,
                                int presencetype,
                                int passent) {
  ...
  while (1) {
    ...
    if (nodenum < 0) {
      ...
      /*			//if can't enter the area because it hasn't got
      the right presence type if
      (!((*aasworld).areasettings[-nodenum].presencetype & presencetype))
                              {
                                      //if the start point is still the initial
      start point
                                      //NOTE: no need for epsilons because the
      points will be
                                      //exactly the same when they're both the
      start point if (tstack_p->start[0] == start[0] && tstack_p->start[1] ==
      start[1] && tstack_p->start[2] == start[2])
                                      {
                                              trace.startsolid = qtrue;
                                              trace.fraction = 0.0;
                                              // Gordon: NOTE, uninitialized
      var: v1 VectorSubtract(end, start, v1); } //end if else
                                      {
                                              trace.startsolid = qfalse;
                                              VectorSubtract(end, start, v1);
                                              VectorSubtract(tstack_p->start,
      start, v2); trace.fraction = VectorLength(v2) / VectorNormalize(v1);
                                              VectorMA(tstack_p->start, -0.125,
      v1, tstack_p->start); } //end else VectorCopy(tstack_p->start,
      trace.endpos); trace.ent = ENTITYNUM_NONE; trace.area = -nodenum;
      //				VectorSubtract(end, start, v1);
                                      trace.planenum = tstack_p->planenum;
                                      //always take the plane with normal facing
      towards the trace start plane = &(*aasworld).planes[trace.planenum]; if
      (DotProduct(v1, plane->normal) > 0) trace.planenum ^= 1; return trace; }
      //end if else
      */
      {
        if (passent >= 0) {
          if (AAS_AreaEntityCollision(-nodenum, tstack_p->start, tstack_p->end,
    ...
    switch (plane->type) { /*FIXME: wtf doesn't this work? obviously the axial
...
int AAS_TraceAreas(vec3_t start,
  ...
  while (1) {
    ...
    switch (plane->type) { /*FIXME: wtf doesn't this work? obviously the node
                              planes aren't always facing positive!!!
                                  //check for axial planes
                                  case PLANE_X:
                                  {
                                          front = cur_start[0] - plane->dist;
...
//===========================================================================
qboolean AAS_InsideFace(aas_face_t* face,
                        vec3_t pnormal,
                        vec3_t point,
                        float epsilon) {
...
#define AAS_BoxOnPlaneSide(absmins, absmaxs, p)                                \
  (((p)->type < 3) ? (((p)->dist <= (absmins)[(p)->type])                      \
                          ? (1)                                                \
...
// Changes Globals:		-
//===========================================================================
aas_link_t* AAS_LinkEntityClientBBox(vec3_t absmins,
                                     vec3_t absmaxs,
                                     int entnum,
                                     int presencetype) {
  vec3_t mins, maxs;
...
typedef struct {
  ...
  unsigned short areas[BBOXAREASCACHE_MAXAREAS];  // we can used shorts since
                                                  // AAS_MAX_AREAS < 65536
#else
...
int AAS_BBoxAreasCheckCache(vec3_t absmins,
                            vec3_t absmaxs,
                            int* areas,
                            int maxareas) {
  int i;
  ...
  for (i = 0, cache = bboxAreasCache; i < NUM_BBOXAREASCACHE; i++, cache++) {
    if (VectorCompare(absmins, cache->absmins) &&
        VectorCompare(absmaxs, cache->absmaxs)) {
      // found a match
      break;
    }
  }
...
void AAS_BBoxAreasAddToCache(vec3_t absmins,
                             vec3_t absmaxs,
                             int* areas,
                             int numareas) {
  int i;
