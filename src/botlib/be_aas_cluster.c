...
int AAS_FloodClusterAreasUsingReachabilities(int clusternum) {
  ...
  for (i = 1; i < (*aasworld).numareas; i++) {
    ...
    for (j = 0; j < (*aasworld).areasettings[i].numreachableareas; j++) {
      ...
      areanum =
          (*aasworld)
              .reachability[(*aasworld).areasettings[i].firstreachablearea + j]
              .areanum;
      // if this area is a cluster portal
...
(*aasworld).vertexes[edge->v[j]]) - plane2->dist < -0.01) return qtrue; } //end
for } //end for for (i = 0; i < face2->numedges; i++)
        {
...
(*aasworld).vertexes[edge->v[j]]) - plane1->dist < -0.01) return qtrue; } //end
for } //end for
...
(*aasworld).vertexes[edge2->v[0]], edgevec2); if (side1)
VectorInverse(edgevec1); if (side2) VectorInverse(edgevec2);
        //
        ...
        for (i = 0; i < 2; i++)
        {
                if (DotProduct((*aasworld).vertexes[edge1->v[i]], normal2) -
dist2 < -0.01) return qfalse; } //end for for (i = 0; i < 2; i++)
        {
                if (DotProduct((*aasworld).vertexes[edge2->v[i]], normal1) -
dist1 < -0.01) return qfalse; } //end for return qtrue; } //end of the function
AAS_NonConvexEdges
//===========================================================================
//
// Parameter:				-
...
qboolean AAS_CanMergeFaces(int *facenums, int numfaces, int planenum)
{
        ...
        for (i = 0; i < numfaces; i++)
        {
                ...
                {
                        edgenum1 = (*aasworld).edgeindex[face1->firstedge +
en1]; side1 = (edgenum1 < 0) ^ (face1->planenum != planenum); edgenum1 =
abs(edgenum1); edge1 = &(*aasworld).edges[edgenum1];
                        ...
                        for (s = 0; s < numfaces; s++)
                        {
                                ...
                                {
                                        if (edgenum1 ==
abs((*aasworld).edgeindex[otherface->firstedge + ens])) break; } //end for if
(ens != otherface->numedges) break; } //end for
...
void AAS_FloodCluster_r(int areanum, int clusternum)
{
        ...
        //if ((*aasworld).areasettings[areanum].contents &
AREACONTENTS_CLUSTERPORTAL) return;
        //
        area = &(*aasworld).areas[areanum];
        //use area faces to flood into adjacent areas
        for (i = 0; i < area->numfaces; i++)
        {
                face =
&(*aasworld).faces[abs((*aasworld).faceindex[area->firstface + i])];
                //
                ...
                //if the area is a portal
                if ((*aasworld).areasettings[otherareanum].contents &
AREACONTENTS_CLUSTERPORTAL) continue;
        ...
        //use the reachabilities to flood into other areas
        for (i = 0; i < (*aasworld).areasettings[areanum].numreachableareas;
i++)
        {
                otherareanum = (*aasworld).reachability[
                                        (*aasworld).areasettings[areanum].firstreachablearea
+ i].areanum; if (!otherareanum)
                {
                        continue;
                        AAS_Error("reachability %d has zero area\n",
(*aasworld).areasettings[areanum].firstreachablearea + i); } //end if
                //if the area is a portal
                if ((*aasworld).areasettings[otherareanum].contents &
AREACONTENTS_CLUSTERPORTAL) continue;
                //if the area is already marked
...
void AAS_RemoveTeleporterPortals(void)
{
        ...
        for (i = 1; i < (*aasworld).numareas; i++)
        {
                for (j = 0; j < (*aasworld).areasettings[i].numreachableareas;
j++)
                {
                        areanum =
(*aasworld).reachability[(*aasworld).areasettings[i].firstreachablearea +
j].areanum; if
((*aasworld).reachability[(*aasworld).areasettings[i].firstreachablearea +
...
~AREACONTENTS_CLUSTERPORTAL; break; } //end if } //end for } //end for } //end
of the function AAS_RemoveTeleporterPortals
//===========================================================================
//
// Parameter:				-
// Returns:					-
...
AREACONTENTS_CLUSTERPORTAL)) continue;
                ...
                for (j = 0; j < area->numfaces; j++)
                {
                        facenum = abs((*aasworld).faceindex[area->firstface +
j]); face = &(*aasworld).faces[facenum];
                        //
                        if (face->frontarea != i) otherareanum =
face->frontarea; else otherareanum = face->backarea;
                        //
                        if (!otherareanum) continue;
                        //
                        if ((*aasworld).areasettings[otherareanum].contents &
AREACONTENTS_CLUSTERPORTAL)
                        ...
                        //check if all adjacent non-portal areas have a cluster
set for (k = 0; k < area->numfaces; k++)
                        {
                                facenum =
abs((*aasworld).faceindex[area->firstface + k]); face =
&(*aasworld).faces[facenum];
                                //
                                if (face->frontarea != i) otherareanum =
face->frontarea; else otherareanum = face->backarea;
                                //
                                ...
                                //
                                if
((*aasworld).areasettings[otherareanum].contents & AREACONTENTS_CLUSTERPORTAL)
                                {
                                        continue;
                                } //end if
                                //
                                if
(!(*aasworld).areasettings[otherareanum].cluster) break; } //end for
                        //if all adjacent non-portal areas have a cluster set
then the portal
                        //didn't seal a cluster
                        if (k >= area->numfaces)
                        {
                                (*aasworld).areasettings[i].contents &=
~AREACONTENTS_CLUSTERPORTAL; nonclosingportals++;
...
void AAS_RemoveNotClusterClosingPortals(void)
{
        int i, j, facenum, otherareanum, nonclosingportals,
numseperatedclusters; aas_area_t *area; aas_face_t *face;
...
void AAS_AddTeleporterPortals(void)
{
        ...
        for (ent = entities; ent; ent = ent->next)
        {
                ...
                if (classname && !strcmp(classname, "misc_teleporter"))
                {
                        ...
                        {
                                botimport.Print(PRT_ERROR, "teleporter (%s)
without origin\n", target); continue; } //end if
                        //
                        target = AAS_ValueForBSPEpairKey(ent, "target");
                        if (!target)
                        {
                                botimport.Print(PRT_ERROR, "teleporter (%s)
without target\n", target); continue; } //end if for (dest = entities; dest;
dest = dest->next)
                        {
                                classname = AAS_ValueForBSPEpairKey(dest,
"classname"); if (classname && !strcmp(classname, "misc_teleporter_dest"))
                                {
                                        targetname =
AAS_ValueForBSPEpairKey(dest, "targetname"); if (targetname &&
!strcmp(targetname, target))
                                        {
                                                break;
                                        } //end if
                                } //end if
                        ...
                        if (!dest)
                        {
                                botimport.Print(PRT_ERROR, "teleporter without
destination (%s)\n", target); continue; } //end if if
(!AAS_VectorForBSPEpairKey(dest, "origin", destorigin))
                        {
                                botimport.Print(PRT_ERROR, "teleporter
destination (%s) without origin\n", target); continue; } //end if destorigin[2]
+= 24; //just for q2e1m2, the dork has put the telepads in the ground
                        VectorCopy(destorigin, end);
                        end[2] -= 100;
                        trace = AAS_TraceClientBBox(destorigin, end,
PRESENCE_CROUCH, -1); if (trace.startsolid)
                        {
                                botimport.Print(PRT_ERROR, "teleporter
destination (%s) in solid\n", target); continue; } //end if
...
|= AREACONTENTS_CLUSTERPORTAL | AREACONTENTS_TELEPORTAL; } //end for
                        //
                        for (link = areas; link; link = link->next_area)
                        {
                                if (!AAS_AreaGrounded(link->areanum)) continue;
                                //find a non-portal area adjacent to the portal
area and flood
                                //the cluster from there
                                ...
                                {
                                        facenum =
abs((*aasworld).faceindex[area->firstface + j]); face =
&(*aasworld).faces[facenum];
                                        //
                                        if (face->frontarea != link->areanum)
otherareanum = face->frontarea; else otherareanum = face->backarea;
                                        //
                                        if (!otherareanum) continue;
                                        //
                                        if
...
continue;
                                        //add the teleporter portal mark
                                        (*aasworld).areasettings[link->areanum].contents
&= ~(AREACONTENTS_CLUSTERPORTAL | AREACONTENTS_TELEPORTAL); } //end for } //end
if } //end if } //end for AAS_FreeBSPEntities(entities); } //end of the function
...
j++)
                {
                        if
((*aasworld).reachability[(*aasworld).areasettings[i].firstreachablearea +
j].traveltype != TRAVEL_TELEPORT) continue; areanum =
(*aasworld).reachability[(*aasworld).areasettings[i].firstreachablearea +
j].areanum;
                        (*aasworld).areasettings[areanum].contents |=
AREACONTENTS_CLUSTERPORTAL; } //end for } //end for } //end of the function
AAS_AddTeleporterPortals*/
//===========================================================================
//
// Parameter:				-
...
void AAS_InitClustering(void) {
  ...
  if ((*aasworld).numclusters >= 1) {
    ...
    (*aasworld).clusterTeamTravelFlags =
        (int*)GetClearedMemory((*aasworld).numclusters * sizeof(int));
  ...
  (*aasworld).clusterTeamTravelFlags =
      (int*)GetClearedMemory(AAS_MAX_CLUSTERS * sizeof(int));
  //
