...
fielddef_t weaponinfo_fields[] = {
    ...
    {"ammoamount", WEAPON_OFS(ammoamount),
     FT_INT},                                      // ammo amount used per shot
    ...
    {"spinup", WEAPON_OFS(spinup),
     FT_FLOAT},  // time it takes before first shot
...
structdef_t projectileinfo_struct = {sizeof(projectileinfo_t),
...
int BotValidWeaponNumber(int weaponnum) {
  ...
  if ((g_singleplayer && weaponnum < 0) || (!g_singleplayer && weaponnum < 0) ||
...
weaponconfig_t* LoadWeaponConfig(char* filename) {
  ...
  while (PC_ReadToken(source, &token)) {
    if (!strcmp(token.string, "weaponinfo")) {
      ...
      memcpy(&wc->weaponinfo[weaponinfo.number], &weaponinfo,
             sizeof(weaponinfo_t));
      ...
      memset(&wc->projectileinfo[wc->numprojectiles], 0,
  ...
  for (i = 0; i < wc->numweapons; i++) {
    ...
    for (j = 0; j < wc->numprojectiles; j++) {
      if (!strcmp(wc->projectileinfo[j].name, wc->weaponinfo[i].projectile)) {
        memcpy(&wc->weaponinfo[i].proj, &wc->projectileinfo[j],
               sizeof(projectileinfo_t));
        break;
      }  // end if
    }    // end for
