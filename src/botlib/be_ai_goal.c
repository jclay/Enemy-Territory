...
void BotUpdateEntityItems(void) {
  ...
  for (ent = AAS_NextEntity(0); ent; ent = AAS_NextEntity(ent)) {
    ...
    for (li = levelitems; li; li = li->next) {
      ...
      if (li->entitynum) {
        ...
        if (VectorLength(dir) < 30) {
          ...
          // Log_Write("found item %s entity",
          // ic->iteminfo[li->iteminfo].classname);
          break;
        }  // end if
           // else botimport.Print(PRT_MESSAGE, "item %s has no attached
    ...
    // get the item goal area and goal origin
    li->goalareanum = AAS_BestReachableArea(
        li->origin, ic->iteminfo[i].mins, ic->iteminfo[i].maxs, li->goalorigin);
    //
      ...
      if (t > 0 && t < maxtime) {
        ...
        if (weight > bestweight) {
          ...
          if (ltg && !li->timeout) {
            ...
            t = AAS_AreaTravelTimeToGoalArea(li->goalareanum, li->goalorigin,
                                             ltg->areanum, travelflags);
          }  // end if
