...
#define GROUNDSTEEPNESS_TIMESCALE \
  1  // this is the maximum scale, 1 being the usual for a flat ground
...
// Ridah, routing memory calls go here, so we can change between Hunk/Zone
// easily
...
void AAS_CalculateClusterTeamFlags(int clusternum) {
  ...
  for (i = 1; i < aasworld->numareas; i++) {
    if (aasworld->areasettings[i].cluster == clusternum) {
      aasworld->clusterTeamTravelFlags[clusternum] |=
          AAS_TeamTravelFlagsForAreaFlags(aasworld->areasettings[i].areaflags);
    }
  }
}
//===========================================================================
...
int AAS_EnableRoutingArea(int areanum, int enable) {
  ...
  if ((enable & 1) || (enable < 0)) {
    // clear all flags
    bitflag = AREA_AVOID | AREA_DISABLED | AREA_TEAM_AXIS | AREA_TEAM_ALLIES |
              AREA_TEAM_AXIS_DISGUISED | AREA_TEAM_ALLIES_DISGUISED;
  } else if (enable & 0x10) {
    bitflag = AREA_AVOID;
  } else if (enable & 0x20) {
...
void AAS_CreateReversedReachability(void) {
  ...
  for (i = 1; i < aasworld->numareas; i++) {
    ...
    for (n = 0; n < settings->numreachableareas; n++) {
      ...
      if (settings->firstreachablearea < 0 ||
          settings->firstreachablearea >= (*aasworld).reachabilitysize) {
      ...
      if ((reach->areanum < 0 ||
           (reach->areanum >= aasworld->reachabilitysize))) {
...
float AAS_AreaGroundSteepnessScale(int areanum) {
  return (1.0 + aasworld->areasettings[areanum].groundsteepness *
                    (float)(GROUNDSTEEPNESS_TIMESCALE - 1));
}
//===========================================================================
//
// Parameter:				-
...
unsigned short int AAS_AreaTravelTime(int areanum, vec3_t start, vec3_t end) {
  ...
  dist = VectorDistance(
      start, end);  // * AAS_AreaGroundSteepnessScale(areanum); // Gordon:
                    // useless as it returns 1 all the time...

  if (AAS_AreaCrouch(areanum)) {
    dist *= DISTANCEFACTOR_CROUCH;  // if crouch only area
    /*	} else if( AAS_AreaSwim(areanum)) { // Gordon: again, uselss as it's a
       multiply by 1 dist *= DISTANCEFACTOR_SWIM; //if swim area */
  } else {
...
void AAS_CalculateAreaTravelTimes(void) {
  ...
  for (i = 0; i < aasworld->numareas; i++) {
    ...
    //
    size += settings->numreachableareas * revreach->numlinks *
...
void AAS_InitPortalMaxTravelTimes(void) {
  ...
  aasworld->portalmaxtraveltimes =
      (int*)AAS_RoutingGetMemory(aasworld->numportals * sizeof(int));
...
void AAS_InitClusterAreaCache(void) {
  ...
  ptr = (char*)AAS_RoutingGetMemory(aasworld->numclusters *
...
void AAS_WriteRouteCache(void) {
  ...
  buf = (byte*)GetClearedMemory(
      aasworld->numareas * 2 *
      sizeof(byte));  // in case it ends up bigger than the decompressedvis,
  ...
  routecacheheader.areacrc = CRC_ProcessString(
      (unsigned char*)aasworld->areas, sizeof(aas_area_t) * aasworld->numareas);
  routecacheheader.clustercrc =
      CRC_ProcessString((unsigned char*)aasworld->clusters,
                        sizeof(aas_cluster_t) * aasworld->numclusters);
  routecacheheader.reachcrc = CRC_ProcessString(
      (unsigned char*)aasworld->reachability,
      sizeof(aas_reachability_t) * aasworld->reachabilitysize);
  ...
  for (i = 0; i < aasworld->numareas; i++) {
    ...
    AAS_DecompressVis(aasworld->areavisibility[i], aasworld->numareas,
                      aasworld->decompressedvis);
    size = AAS_CompressVis(aasworld->decompressedvis, aasworld->numareas, buf);
    botimport.FS_Write(&size, sizeof(int), fp);
    botimport.FS_Write(buf, size, fp);
  }
  // write the waypoints
  botimport.FS_Write(aasworld->areawaypoints,
                     sizeof(vec3_t) * aasworld->numareas, fp);
  //
...
aas_routingcache_t* AAS_ReadCache(fileHandle_t fp) {
  ...
  cache = (aas_routingcache_t*)AAS_RoutingGetMemory(size);
  cache->size = size;
  botimport.FS_Read((unsigned char*)cache + sizeof(size), size - sizeof(size),
                    fp);
  ...
  //sizeof(aas_routingcache_t) - sizeof(unsigned short) + 		(size -
...
int AAS_ReadRouteCache(void) {
  ...
  for (i = 0; i < routecacheheader.numareacache; i++) {
    ...
    aasworld->clusterareacache[cache->cluster][clusterareanum] = cache;
     ...
     //  read the visareas
  aasworld->areavisibility =
      (byte**)GetClearedMemory(aasworld->numareas * sizeof(byte*));
  aasworld->decompressedvis =
      (byte*)GetClearedMemory(aasworld->numareas * sizeof(byte));
  for (i = 0; i < aasworld->numareas; i++) {
    botimport.FS_Read(&size, sizeof(size), fp);
  ...
  aasworld->areawaypoints =
      (vec3_t*)GetClearedMemory(aasworld->numareas * sizeof(vec3_t));
  botimport.FS_Read(aasworld->areawaypoints,
                    aasworld->numareas * sizeof(vec3_t), fp);
  //
  botimport.FS_FCloseFile(fp);
  return qtrue;
...
void AAS_InitRouting(void) {
  ...
  max_routingcachesize =
      1024 * (int)LibVarValue("max_routingcache", DEFAULT_MAX_ROUTINGCACHESIZE);
  ...
  if (LibVarGetValue("bot_norcd")) {
    ...
    if (!AAS_ReadRouteCache()) {
      aasworld->initialized =
          qtrue;  // Hack, so routing can compute traveltimes
...
void AAS_UpdateAreaRoutingCache(aas_routingcache_t* areacache) {
  ...
  //	memset(aasworld->areaupdate, 0, aasworld->numareas *
  ...
  while (updateliststart) {
    ...
    for (i = 0, revlink = revreach->first; revlink;
      ...
      if (aasworld->areasettings[reach->areanum].areaflags & AREA_AVOID) {
        t += 1000;
      } else if ((aasworld->areasettings[reach->areanum].areaflags &
                  AREA_AVOID_AXIS) &&
                 (areacache->travelflags & TFL_TEAM_AXIS)) {
        t +=
            200;  // + (curupdate->areatraveltimes[i] + reach->traveltime) * 30;
      } else if ((aasworld->areasettings[reach->areanum].areaflags &
                  AREA_AVOID_ALLIES) &&
      ...
      if (aasworld->areatraveltimes[nextareanum] &&
        ...
        nextupdate->areatraveltimes =
            aasworld
                ->areatraveltimes[nextareanum]
                                 [linknum - aasworld->areasettings[nextareanum]
                                                .firstreachablearea];
...
// Changes Globals:		-
//===========================================================================
aas_routingcache_t* AAS_GetAreaRoutingCache(int clusternum,
                                            int areanum,
                                            int travelflags,
  ...
  if (!cache) {
    ...
    if (!forceUpdate &&
        (aasworld->frameroutingupdates > max_frameroutingupdates)) {
      return NULL;
    }  // end if
...
#endif  // ROUTING_DEBUG
        // clear the routing update fields
  //	memset(aasworld->portalupdate, 0, (aasworld->numportals+1) *
  //sizeof(aas_routingupdate_t));
  //
...
int AAS_AreaRouteToGoalArea(int areanum,
  ...
  if (AAS_AreaDoNotEnterLarge(areanum) ||
      AAS_AreaDoNotEnterLarge(goalareanum)) {
  ...
  } else if (clusternum > 0 &&
             goalclusternum <
                 0) {  // check if the goalarea is a portal of the area cluster
  ...
  if (clusternum > 0 && goalclusternum > 0 && clusternum == goalclusternum) {
    areacache =
        AAS_GetAreaRoutingCache(clusternum, goalareanum, travelflags, qfalse);
    // RF, note that the routing cache might be NULL now since we are
    // restricting the updates per frame, hopefully rejected cache's will be
    // requested again when things have settled down
    if (!areacache) {
      return qfalse;
    }
  ...
  for (i = 0; i < cluster->numportals; i++, pPortalnum++) {
    ...
    areacache = AAS_GetAreaRoutingCache(clusternum, portal->areanum,
                                        travelflags, qfalse);
    ...
    //		to be more specific we don't know which reachability is used to
    //travel 		into the portal area when coming from the current area
    t += aasworld->portalmaxtraveltimes[portalnum];
    //
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaTravelTimeToGoalArea(int areanum,
                                 vec3_t origin,
                                 int goalareanum,
                                 int travelflags) {
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaTravelTimeToGoalAreaCheckLoop(int areanum,
                                          vec3_t origin,
                                          int goalareanum,
                                          int travelflags,
                                          int loopareanum) {
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaReachabilityToGoalArea(int areanum,
                                   vec3_t origin,
                                   int goalareanum,
                                   int travelflags) {
  int traveltime, reachnum;
...
// Changes Globals:		-
//===========================================================================
int AAS_RandomGoalArea(int areanum,
                       int travelflags,
                       int* goalareanum,
                       vec3_t goalorigin) {
  int i, n, t;
  ...
  for (i = 0; i < aasworld->numareas; i++) {
    ...
    if (AAS_AreaReachability(n)) {
      t = AAS_AreaTravelTimeToGoalArea(areanum, aasworld->areas[areanum].center,
                                       n, travelflags);
      ...
      if (t > 0) {
        ...
        if (!AAS_PointAreaNum(start)) {
          Log_Write("area %d center %f %f %f in solid?", n, start[0], start[1],
                    start[2]);
        }
...
int AAS_AreaVisible(int srcarea, int destarea) {
  if (!aasworld->areavisibility) {
    //		botimport.Print(PRT_MESSAGE, "AAS_AreaVisible: no vis data
    //available, returning qtrue\n" );
    return qtrue;
  }
  if (srcarea != aasworld->decompressedvisarea) {
    ...
    AAS_DecompressVis(aasworld->areavisibility[srcarea], aasworld->numareas,
                      aasworld->decompressedvis);
...
void AAS_CreateVisibility(qboolean waypointsOnly) {
  ...
  aasworld->areawaypoints =
      (vec3_t*)GetClearedMemory(numAreas * sizeof(vec3_t));
  ...
  for (i = 1; i < numAreas; i++) {
    ...
    if (trace.startsolid && trace.ent < ENTITYNUM_WORLD) {
      trace = AAS_Trace(
          aasworld->areas[i].center, mins, maxs, endpos, trace.ent,
          CONTENTS_SOLID | CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP);
    }
    if (!trace.startsolid && trace.fraction < 1 &&
        AAS_PointAreaNum(trace.endpos) == i) {
  ...
  for (i = 1; i < numAreas; i++) {
    ...
    for (j = 1; j < numAreas; j++) {
      ...
      trace = AAS_Trace(aasworld->areawaypoints[i], NULL, NULL,
                        aasworld->areawaypoints[j], -1, CONTENTS_SOLID);
      if (trace.startsolid && trace.ent < ENTITYNUM_WORLD) {
        trace = AAS_Trace(
            aasworld->areas[i].center, mins, maxs, endpos, trace.ent,
            CONTENTS_SOLID | CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP);
  ...
  botimport.Print(PRT_MESSAGE,
                  "AAS_CreateVisibility: compressed vis size = %i\n",
                  totalsize);
}
//===========================================================================
//
...
extern void ProjectPointOntoVector(vec3_t point,
                                   vec3_t vStart,
                                   vec3_t vEnd,
                                   vec3_t vProj);
...
#define MAX_HIDEAREA_LOOPS 3000
  ...
  if (!aasworld->hidetraveltimes) {
    aasworld->hidetraveltimes = (unsigned short int*)GetClearedMemory(
        aasworld->numareas * sizeof(unsigned short int));
  } else {
    memset(aasworld->hidetraveltimes, 0,
           aasworld->numareas * sizeof(unsigned short int));
  }  // end else
     //
  ...
  bestarea = 0;
  if (enemyareanum) {
    enemytraveltime = AAS_AreaTravelTimeToGoalArea(areanum, origin,
                                                   enemyareanum, travelflags);
  }
  VectorSubtract(enemyorigin, origin, enemyVec);
  enemytraveldist = VectorNormalize(enemyVec);
  startVisible = botimport.BotVisibleFromPos(enemyorigin, enemynum, origin,
                                             srcnum, qfalse);
  //
  ...
  while (updateliststart) {
    ...
    for (i = 0; i < numreach; i++, reach++) {
      ...
      aasworld->hidetraveltimes[nextareanum] = t;
      // if the bestarea is this area, then it must be a longer route, so ignore
      // it
      ...
      if (enemytraveldist > 32 &&
          DotProduct(v2, enemyVec) > enemytraveldist / 2) {
        continue;
      }
      //
      ...
      //
      // if we weren't visible when starting, make sure we don't move into their
      // view
      if (enemyareanum && !startVisible &&
          AAS_AreaVisible(enemyareanum, nextareanum)) {
        continue;
        // t += 1000;
      }
      //
      if (!besttraveltime || besttraveltime > t) {
        ...
        if (aasworld->areavisibility[nextareanum]) {
          ...
          if (!AAS_AreaVisible(
                  enemyareanum,
                  nextareanum)) {  // now last of all, check that this area is a
                                   // safe hiding spot
            if ((aasworld->visCache[nextareanum] == 2) ||
                (!aasworld->visCache[nextareanum] &&
                 !botimport.BotVisibleFromPos(
                     enemyorigin, enemynum,
                     aasworld->areawaypoints[nextareanum], srcnum, qfalse))) {
        ...
        if (loopCount++ > MAX_HIDEAREA_LOOPS) {
          // botimport.Print(PRT_MESSAGE, "AAS_NearestHideArea: exceeded max
          // loops, aborting\n" );
          continue;
        }
        //
        // otherwise, add this to the list so we check is reachables
        // disabled, this should only store the raw traveltime, not the adjusted
        // time
        // aasworld->hidetraveltimes[nextareanum] = t;
     ...
     // botimport.Print(PRT_MESSAGE, "AAS_NearestHideArea: hidearea: %i, %i
     // loops\n", bestarea, count );
  return bestarea;
...
// Changes Globals:		-
//===========================================================================
int AAS_FindAttackSpotWithinRange(int srcnum,
                                  int rangenum,
                                  int enemynum,
                                  float rangedist,
                                  int travelflags,
...
#define MAX_ATTACKAREA_LOOPS 200
  static float lastTime;
  //
  // RF, currently doesn't work with multiple AAS worlds, so only enable for the
  // default world
  ...
  while (updateliststart) {
    ...
    for (i = 0; i < numreach; i++, reach++) {
      ...
      if (count++ > MAX_ATTACKAREA_LOOPS) {
        // botimport.Print(PRT_MESSAGE, "AAS_FindAttackSpotWithinRange: exceeded
        // max loops, aborting\n" );
        if (bestarea) {
      ...
      if (Distance(rangeorg, aasworld->areawaypoints[nextareanum]) >
          rangedist) {
        continue;
      }
      //
      // find the traveltime from srcnum
      srctraveltime = AAS_AreaTravelTimeToGoalArea(srcarea, srcorg, nextareanum,
                                                   travelflags);
      // do this test now, so we can reject the route if it starts out too long
      ...
      if (aasworld->areavisibility[nextareanum]) {
        ...
        if (AAS_AreaVisible(enemyarea,
                            nextareanum)) {  // now last of all, check that this
                                             // area is a good attacking spot
          if ((aasworld->visCache[nextareanum] == 2) ||
              (!aasworld->visCache[nextareanum] &&
               (count += 10) &&  // we are about to use lots of CPU time
               botimport.BotCheckAttackAtPos(
                   srcnum, enemynum, aasworld->areawaypoints[nextareanum],
                   qfalse, qfalse))) {
            aasworld->visCache[nextareanum] = 2;
...
// Changes Globals:		-
//===========================================================================
qboolean AAS_GetRouteFirstVisPos(vec3_t srcpos,
                                 vec3_t destpos,
                                 int travelflags,
                                 vec3_t retpos) {
...
#define MAX_GETROUTE_VISPOS_LOOPS 200
  ...
  // SRCPOS: enemy
  // DESTPOS: self
  // RETPOS: first area that is visible from destpos, in route from srcpos to
  // destpos
  ...
  while ((loops++ < MAX_GETROUTE_VISPOS_LOOPS) &&
         AAS_AreaRouteToGoalArea(travarea, travpos, destarea, travelflags,
                                 &ftraveltime, &freachnum)) {
...
// Changes Globals:		-
//===========================================================================
int AAS_ListAreasInRange(vec3_t srcpos,
                         int srcarea,
                         float range,
                         int travelflags,
                         vec3_t* outareas,
  ...
  while (updateliststart) {
    ...
    for (i = 0; i < numreach; i++, reach++) {
      ...
      //
      // if (aasworld->areasettings[reach->areanum].areaflags & AREA_DISABLED)
      // continue; number of the area the reachability leads to
      nextareanum = reach->areanum;
      // if we've already been to this area
      if (aasworld->hidetraveltimes[nextareanum]) {
        continue;
...
// Changes Globals:		-
//===========================================================================
int AAS_AvoidDangerArea(vec3_t srcpos,
                        int srcarea,
                        vec3_t dangerpos,
                        int dangerarea,
                        float range,
  ...
  while (updateliststart) {
    ...
    for (i = 0; i < numreach; i++, reach++) {
      ...
      // How far is it
      dangerDistance =
          Distance(dangerpos, aasworld->areawaypoints[nextareanum]);
      ...
      // Mad Doctor I, 11/3/2002.  The inlist field seems to not be inited
      // properly for this function. It causes certain routes to be excluded
      // unnecessarily, so I'm trying to do without it. Note that the
      // hidetraveltimes array seems to cut off duplicates already.
      //			if (!nextupdate->inlist)
      {
        // add the new update to the end of the list
        nextupdate->next = NULL;
...
void AAS_DangerPQInit() {
  ...
  // a quicky, slow-ass PQ.  This needs to be replaced with a heap
  // implementation.

  // Init the distanceFromDanger array if needed
  if (!aasworld->PQ_accumulator) {
    // Get memory for this array the safe way.
    aasworld->PQ_accumulator = (unsigned short int*)GetClearedMemory(
        aasworld->numareas * sizeof(unsigned short int));
...
void AAS_DangerPQChange(
  ...
  //@TEST. To test that the retreat algorithm is going to work, I've implemented
  // a quicky, slow-ass PQ.  This needs to be replaced with a heap
  // implementation. NOTHING NEEDS TO BE DONE HERE FOR THE SLOW-ASS PQ.
}
//
// AAS_DangerPQChange
//
...
void AAS_CalculateDangerZones(
    ...
    // What is the furthest danger range we care about? (Everything further is
    // safe)
    float dangerRange,
  ...
  // Local Variables ////////////////////////////////////////////////////////
  // Generic index used to loop through danger zones (and later, the
  // reachabilities)
  int i;
  ...
  while (!AAS_DangerPQEmpty()) {
    ...
    reach = &aasworld->reachability[aasworld->areasettings[currentArea]
                                        .firstreachablearea];
    ...
    for (i = 0; i < numreach; i++, reach++) {
      ...
      if (dangerDistance < aasworld->distanceFromDanger[nextareanum]) {
        ...
        }  // if (aasworld->distanceFromDanger[nextareanum] ==
           // definitelySafe)... Otherwise, insert the neighbor into the PQ.
        else {
...
int AAS_Retreat(
  ...
  if (!aasworld->distanceFromDanger) {
    // Get memory for this array the safe way.
    aasworld->distanceFromDanger = (unsigned short int*)GetClearedMemory(
        aasworld->numareas * sizeof(unsigned short int));

  }  // if (!aasworld->distanceFromDanger) ...

  // Set all the values in the distanceFromDanger array to a safe value
  memset(aasworld->distanceFromDanger, 0xFF,
         aasworld->numareas * sizeof(unsigned short int));
  ...
  AAS_CalculateDangerZones(dangerSpots, dangerSpotCount, dangerRange,
                           definitelySafe);
  ...
  while (updateliststart) {
    ...
    for (i = 0; i < numreach; i++, reach++) {
      ...
      // If it's safe from dangerpos
      if (aasworld->areavisibility[nextareanum] && (sourceDistance > range) &&
...
void AAS_InitTeamDeath(void) {
  ...
  aasworld->teamDeathTime =
      GetClearedMemory(sizeof(int) * aasworld->numareas * 2);
  if (aasworld->teamDeathCount) {
    FreeMemory(aasworld->teamDeathCount);
  }
  aasworld->teamDeathCount =
      GetClearedMemory(sizeof(byte) * aasworld->numareas * 2);
...
#define TEAM_DEATH_AVOID_COUNT_SCALE \
  0.5  // so if there are 12 players, then if count reaches (12 * scale) then
       // AVOID
...
===================
*/
void AAS_RecordTeamDeathArea(vec3_t srcpos,
                             int srcarea,
                             int team,
                             int teamCount,
                             int travelflags) {
  ...
  while (updateliststart) {
    ...
    for (i = 0; i < numreach; i++, reach++) {
      ...
      //
      // if it's too far from srcpos, ignore
      if (Distance(curupdate->start, reach->end) +
              (float)curupdate->tmptraveltime >
          TEAM_DEATH_RANGE) {
        continue;
      }
      //
      ...
      if (!aasworld->teamDeathAvoid[k]) {
        if (aasworld->teamDeathCount[k] >
            (int)(TEAM_DEATH_AVOID_COUNT_SCALE * teamCount)) {
          // avoid this area
      ...
      VectorCopy(reach->end, nextupdate->start);
      // calc the distance
      nextupdate->tmptraveltime = (float)curupdate->tmptraveltime +
                                  Distance(curupdate->start, reach->end);
      // if this update is not in the list yet
      if (!nextupdate->inlist) {
        // add the new update to the end of the list
        nextupdate->next = NULL;
