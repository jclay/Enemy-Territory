...
// this must be enabled for the route-tables to work, but it's not fully
// operational yet
...
void* AAS_RT_GetClearedMemory(unsigned long size) {
  ...
  // Ryan - 01102k, need to use this, since the routetable calculations use up a
  // lot of memory this will be a non-issue once we transfer the remnants of the
  // routetable over to the aasworld
  // ptr = malloc (size);
  // memset (ptr, 0, size);
...
//===========================================================================
// return the number of unassigned areas that are in the given area's visible
// list
//
// Parameter:				-
// Returns:					-
// Changes Globals:		-
//===========================================================================
int AAS_RT_GetValidVisibleAreasCount(
    aas_area_buildlocalinfo_t* localinfo,
    aas_area_childlocaldata_t** childlocaldata) {
  int i, cnt;
...
int AAS_AreaRouteToGoalArea(int areanum,
                            vec3_t origin,
                            int goalareanum,
                            int travelflags,
                            int* traveltime,
...
void AAS_RT_CalcTravelTimesToGoalArea(int goalarea) {
  int i;
  // TTimo: unused
  //	static int tfl = TFL_DEFAULT &
  //~(TFL_JUMPPAD|TFL_ROCKETJUMP|TFL_BFGJUMP|TFL_GRAPPLEHOOK|TFL_DOUBLEJUMP|TFL_RAMPJUMP|TFL_STRAFEJUMP|TFL_LAVA);
  ////----(SA)	modified since slime is no longer deadly 	static int tfl =
  //TFL_DEFAULT &
  //~(TFL_JUMPPAD|TFL_ROCKETJUMP|TFL_BFGJUMP|TFL_GRAPPLEHOOK|TFL_DOUBLEJUMP|TFL_RAMPJUMP|TFL_STRAFEJUMP|TFL_SLIME|TFL_LAVA);
  aas_rt_route_t* rt;
  int reach, travel;
  ...
  for (i = 0; i < childcount; i++) {
    ...
    if (AAS_AreaRouteToGoalArea(
            filtered_areas[i], (*aasworld).areas[filtered_areas[i]].center,
            goalarea, ~RTB_BADTRAVELFLAGS, &travel, &reach)) {
...
//
// FIXME: this isn't fully operational yet, for some reason not all routes are
// found
//
// Parameter:				-
// Returns:					-
...
// Changes Globals:		-
//===========================================================================
void AAS_RT_AddParentLink(aas_area_childlocaldata_t* child,
                          int parentindex,
                          int childindex) {
  ...
  child->parentlink =
      (aas_parent_link_t*)AAS_RT_GetClearedMemory(sizeof(aas_parent_link_t));
...
void AAS_RT_WriteRouteTable() {
  ...
  botimport.FS_Write(
      (*aasworld).routetable->children,
      (*aasworld).routetable->numChildren * sizeof(aas_rt_child_t), fp);

  // parents
  botimport.FS_Write(&(*aasworld).routetable->numParents, sizeof(int), fp);
  botimport.FS_Write(
      (*aasworld).routetable->parents,
      (*aasworld).routetable->numParents * sizeof(aas_rt_parent_t), fp);

  // parentChildren
  botimport.FS_Write(&(*aasworld).routetable->numParentChildren, sizeof(int),
                     fp);
  botimport.FS_Write(
      (*aasworld).routetable->parentChildren,
      (*aasworld).routetable->numParentChildren * sizeof(unsigned short int),
      fp);

  // visibleParents
  botimport.FS_Write(&(*aasworld).routetable->numVisibleParents, sizeof(int),
                     fp);
  botimport.FS_Write(
      (*aasworld).routetable->visibleParents,
      (*aasworld).routetable->numVisibleParents * sizeof(unsigned short int),
      fp);

  // parentLinks
  botimport.FS_Write(&(*aasworld).routetable->numParentLinks, sizeof(int), fp);
  botimport.FS_Write(
      (*aasworld).routetable->parentLinks,
      (*aasworld).routetable->numParentLinks * sizeof(aas_rt_parent_link_t),
      fp);
...
//===========================================================================
//	reads the given file, and creates the structures required for the
//route-table system
//
// Parameter:				-
// Returns:					qtrue if succesful, qfalse if
// not Changes Globals:		-
//===========================================================================
...
qboolean AAS_RT_ReadRouteTable(fileHandle_t fp) {
  ...
  routetable->children = (aas_rt_child_t*)AAS_RT_GetClearedMemory(
      routetable->numChildren * sizeof(aas_rt_child_t));
  botimport.FS_Read(routetable->children,
                    routetable->numChildren * sizeof(aas_rt_child_t), fp);
  ...
  routetable->parents = (aas_rt_parent_t*)AAS_RT_GetClearedMemory(
      routetable->numParents * sizeof(aas_rt_parent_t));
  botimport.FS_Read(routetable->parents,
                    routetable->numParents * sizeof(aas_rt_parent_t), fp);
  ...
  routetable->parentChildren = (unsigned short int*)AAS_RT_GetClearedMemory(
      routetable->numParentChildren * sizeof(unsigned short int));
  botimport.FS_Read(routetable->parentChildren,
                    routetable->numParentChildren * sizeof(unsigned short int),
                    fp);
  ...
  routetable->visibleParents = (unsigned short int*)AAS_RT_GetClearedMemory(
      routetable->numVisibleParents * sizeof(unsigned short int));
  botimport.FS_Read(routetable->visibleParents,
                    routetable->numVisibleParents * sizeof(unsigned short int),
                    fp);
  ...
  routetable->parentLinks = (aas_rt_parent_link_t*)AAS_RT_GetClearedMemory(
      routetable->numParentLinks * sizeof(aas_rt_parent_link_t));
  botimport.FS_Read(routetable->parentLinks,
                    routetable->numParentLinks * sizeof(aas_parent_link_t), fp);
  ...
  routetable->areaChildIndexes = (unsigned short int*)AAS_RT_GetClearedMemory(
...
void AAS_RT_BuildRouteTable(void) {
  ...
  filtered_areas = (unsigned short int*)AAS_RT_GetClearedMemory(
      (*aasworld).numareas * sizeof(unsigned short int));
  rev_filtered_areas = (unsigned short int*)AAS_RT_GetClearedMemory(
      (*aasworld).numareas * sizeof(unsigned short int));
  ...
  for (i = 0; i < (*aasworld).numareas; i++) {
    ...
    if (!(srcsettings->areaflags &
          (AREA_GROUNDED | AREA_LIQUID | AREA_LADDER))) {
      continue;
    }
#endif
  ...
  filteredroutetable = (aas_rt_route_t**)AAS_RT_GetClearedMemory(
      childcount * sizeof(aas_rt_route_t*));
  for (i = 0; i < childcount; i++)
    filteredroutetable[i] = (aas_rt_route_t*)AAS_RT_GetClearedMemory(
        childcount * sizeof(aas_rt_route_t));
  ...
  area_localinfos = (aas_area_buildlocalinfo_t**)AAS_RT_GetClearedMemory(
      childcount * sizeof(aas_area_buildlocalinfo_t*));
  ...
  for (i = 0; i < childcount; i++) {
    ...
    area_localinfos[i] = (aas_area_buildlocalinfo_t*)AAS_RT_GetClearedMemory(
        sizeof(aas_area_buildlocalinfo_t));
    ...
    for (j = 0; j < childcount; j++) {
      ...
      if (localinfo->numvisible >= MAX_VISIBLE_AREAS) {
        botimport.Print(
            PRT_MESSAGE,
            "MAX_VISIBLE_AREAS exceeded, lower MAX_VISIBLE_RANGE\n");
        break;
      }
    }
  }
  ...
  area_childlocaldata = (aas_area_childlocaldata_t**)AAS_RT_GetClearedMemory(
      childcount * sizeof(aas_area_childlocaldata_t*));

  for (i = 0; i < childcount; i++) {
    area_childlocaldata[i] =
        (aas_area_childlocaldata_t*)AAS_RT_GetClearedMemory(
            sizeof(aas_area_childlocaldata_t));
  ...
  while (1) {
    ...
    for (i = 0; i < childcount; i++) {
      ...
      cnt = AAS_RT_GetValidVisibleAreasCount(area_localinfos[i],
                                             area_childlocaldata);
    ...
    // look through this area's list of visible areas, and pick the one with the
    // most VALID visible areas
    ...
    for (i = 0; i < localinfo->numvisible; i++) {
      ...
      cnt = AAS_RT_GetValidVisibleAreasCount(
          area_localinfos[localinfo->visible[i]], area_childlocaldata);
    ...
    area_parents[num_parents] =
        (aas_area_parent_t*)AAS_RT_GetClearedMemory(sizeof(aas_area_parent_t));
    thisparent = area_parents[num_parents];

    thisparent->areanum = filtered_areas[bestparent];
    thisparent->children = (unsigned short int*)AAS_RT_GetClearedMemory(
        (localinfo->numvisible + 1) * sizeof(unsigned short int));
    ...
    thisparent->children[thisparent->numchildren++] =
        filtered_areas[bestparent];

    // loop around all the parent's visible list, and make them children if
    // they're aren't already assigned to a parent
    for (i = 0; i < localinfo->numvisible; i++) {
      ...
      if (child->areanum != thisparent->areanum) {
        AAS_RT_AddParentLink(child, num_parents, thisparent->numchildren);
        thisparent->children[thisparent->numchildren++] =
            filtered_areas[localinfo->visible[i]];
      }
    }
    ...
    for (i = 0; i < thisparent->numchildren; i++) {
      child =
          area_childlocaldata[-1 + rev_filtered_areas[thisparent->children[i]]];
      localinfo =
          area_localinfos[-1 + rev_filtered_areas[thisparent->children[i]]];

      child->parentlink->routeindexes =
          (unsigned short int*)AAS_RT_GetClearedMemory(
              thisparent->numchildren * sizeof(unsigned short int));
      ...
      for (j = 0; j < thisparent->numchildren; j++) {
        ...
        for (k = 0; k < localinfo->numvisible; k++) {
          if (thisparent->children[j] ==
              filtered_areas[localinfo->visible[k]]) {  // found a match
        ...
        if (k ==
            localinfo->numvisible) {  // didn't find it, so add it to our list
          if (localinfo->numvisible >= MAX_VISIBLE_AREAS) {
            botimport.Print(
                PRT_MESSAGE,
                "MAX_VISIBLE_AREAS exceeded, lower MAX_VISIBLE_RANGE\n");
          } else {
            localinfo->visible[localinfo->numvisible] =
                -1 + rev_filtered_areas[thisparent->children[j]];
            child->parentlink->routeindexes[j] =
                (unsigned short int)localinfo->numvisible;
  ...
  // place all the visible areas from each child, into their childlocaldata
  // route-table
  for (i = 0; i < childcount; i++) {
    localinfo = area_localinfos[i];
    ...
    child->localroutes = (aas_rt_route_t*)AAS_RT_GetClearedMemory(
        localinfo->numvisible * sizeof(aas_rt_route_t));

    for (j = 0; j < localinfo->numvisible; j++) {
      child->localroutes[j] = filteredroutetable[i][localinfo->visible[j]];
    }

    child->parentroutes = (aas_rt_route_t*)AAS_RT_GetClearedMemory(
        num_parents * sizeof(aas_rt_route_t));

    for (j = 0; j < num_parents; j++) {
      child->parentroutes[j] =
          filteredroutetable[i]
                            [-1 + rev_filtered_areas[area_parents[j]->areanum]];
    }
  }

  // build the visibleParents lists
  visibleParents = (unsigned short int*)AAS_RT_GetClearedMemory(
      num_parents * sizeof(unsigned short int));
  for (i = 0; i < num_parents; i++) {
    ...
    for (j = 0; j < num_parents; j++) {
      ...
      if (!AAS_inPVS((*aasworld).areas[area_parents[i]->areanum].center,
                     (*aasworld).areas[area_parents[j]->areanum].center)) {
        continue;
      }
    ...
    area_parents[i]->visibleParents =
        (unsigned short int*)AAS_RT_GetClearedMemory(
            area_parents[i]->numVisibleParents * sizeof(unsigned short int));
    memcpy(area_parents[i]->visibleParents, visibleParents,
           area_parents[i]->numVisibleParents * sizeof(unsigned short int));
  }
  AAS_RT_FreeMemory(visibleParents);

  // before we free the main childlocaldata, go through and assign the
  // aas_area's to their appropriate childlocaldata
  //	this would require modification of the aas_area_t structure, so for now,
  //we'll just place them in a global array, for external reference

  //	aasworld->routetable->area_childlocaldata_list =
  //(aas_area_childlocaldata_t **) AAS_RT_GetClearedMemory( (*aasworld).numareas
  //* sizeof(aas_area_childlocaldata_t *) ); 	for (i=0; i<childcount; i++)
  //	{
  //		aasworld->routetable->area_childlocaldata_list[filtered_areas[i]]
  //= area_childlocaldata[i];
  //	}

  // copy the list of parents to a global structure for now (should eventually
  // go into the (*aasworld) structure
  //	aasworld->routetable->area_parents_global = (aas_area_parent_t **)
  //AAS_RT_GetClearedMemory( num_parents * sizeof(aas_area_parent_t *) );
  //	memcpy( aasworld->routetable->area_parents_global, area_parents,
  //num_parents * sizeof(aas_area_parent_t *) );
  ...
  // Convert the data into the correct format
  {
    ...
    int localRoutesCount, parentRoutesCount, parentChildrenCount,
        visibleParentsCount, parentLinkCount, routeIndexesCount;
    ...
    // areaChildIndexes
    rt->areaChildIndexes = (unsigned short int*)AAS_RT_GetClearedMemory(
        (*aasworld).numareas * sizeof(unsigned short int));
    for (i = 0; i < childcount; i++) {
    ...
    rt->children = (aas_rt_child_t*)AAS_RT_GetClearedMemory(
        rt->numChildren * sizeof(aas_rt_child_t));
    child = rt->children;
    ...
    rt->parents = (aas_rt_parent_t*)AAS_RT_GetClearedMemory(
        rt->numParents * sizeof(aas_rt_parent_t));
    parent = rt->parents;
    ...
    rt->parentChildren = (unsigned short int*)AAS_RT_GetClearedMemory(
        parentChildrenCount * sizeof(unsigned short int));
    ...
    rt->visibleParents = (unsigned short int*)AAS_RT_GetClearedMemory(
        rt->numVisibleParents * sizeof(unsigned short int));
    ...
    rt->parentLinks = (aas_rt_parent_link_t*)AAS_RT_GetClearedMemory(
        parentLinkCount * sizeof(aas_rt_parent_link_t));
...
#define GETROUTE_NUMROUTES 64
  static aas_rt_route_t
      routes[GETROUTE_NUMROUTES];  // cycle through these, so we don't overlap
  ...
  static int tfl =
      TFL_DEFAULT &
      ~(TFL_JUMPPAD | TFL_ROCKETJUMP | TFL_BFGJUMP | TFL_GRAPPLEHOOK |
        TFL_DOUBLEJUMP | TFL_RAMPJUMP | TFL_STRAFEJUMP |
        TFL_LAVA);  //----(SA)	modified since slime is no longer deadly
                    //	static int tfl = TFL_DEFAULT &
  //~(TFL_JUMPPAD|TFL_ROCKETJUMP|TFL_BFGJUMP|TFL_GRAPPLEHOOK|TFL_DOUBLEJUMP|TFL_RAMPJUMP|TFL_STRAFEJUMP|TFL_SLIME|TFL_LAVA);
  ...
  if (AAS_AreaRouteToGoalArea(srcnum, origin, destnum, tfl, &traveltime,
                              &reach)) {
...
#define MAX_RT_AVOID_REACH 1
  ...
  AAS_ShowAreaPolygons(destnum, 4, qtrue);
  {
    ...
    reachnum = BotGetReachabilityToGoal(
        srcpos, srcnum, -1, lastgoalareanum, lastareanum, avoidreach,
        avoidreachtimes, avoidreachtries, &goal, TFL_DEFAULT | TFL_FUNCBOB,
        TFL_DEFAULT | TFL_FUNCBOB);
...
=================
*/
qboolean AAS_RT_GetHidePos(vec3_t srcpos,
                           int srcnum,
                           int srcarea,
                           vec3_t destpos,
                           int destnum,
...
int AAS_RT_GetReachabilityIndex(int areanum, int reachIndex) {
  //	return (*aasworld).areasettings[areanum].firstreachablearea +
  //reachIndex;
  return reachIndex;
}
