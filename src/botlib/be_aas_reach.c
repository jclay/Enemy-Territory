...
// #define INSIDEUNITS_WALKEND					0.2
// // new
...
// Changes Globals:		-
//===========================================================================
int AAS_BestReachableArea(vec3_t origin,
                          vec3_t mins,
                          vec3_t maxs,
                          vec3_t goalorigin) {
  int areanum, i, j, k, l;
  ...
  if (areanum) {
    ...
    if (!trace.startsolid) {
      ...
      start[0], start[1], start[2], areanum); AAS_DrawPermanentCross(start, 4,
      LINECOLOR_RED); } //end if botimport.Print(PRT_MESSAGE,
...
float AAS_MaxJumpHeight(float sv_jumpvel) {
  ...
  return 0.5 * sv_gravity * (sv_jumpvel / sv_gravity) *
         (sv_jumpvel / sv_gravity);
    ...
    for (j = 0; j < area2->numfaces; j++) {
      ...
      if (face1num == face2num) {
        ...
        if (AAS_PointContents(start) &
          ...
          // if (!(AAS_PointContents(start) & MASK_WATER)) lreach->traveltime +=
          // 500; link the reachability
...
int AAS_Reachability_EqualFloorHeight(int area1num, int area2num) {
  ...
  for (i = 0; i < area1->numfaces; i++) {
    face1 =
        &(*aasworld).faces[abs((*aasworld).faceindex[area1->firstface + i])];
    ...
    for (j = 0; j < area2->numfaces; j++) {
      ...
      for (edgenum1 = 0; edgenum1 < face1->numedges; edgenum1++) {
        for (edgenum2 = 0; edgenum2 < face2->numedges; edgenum2++) {
          ...
          // NOTE: disabled because it disables reachabilities to very small
          // areas if (AAS_PointAreaNum(end) != area2num) continue; get the
          // longest lowest edge
...
int AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(int area1num,
  ...
  for (i = 0; i < area1->numfaces; i++) {
    ...
    for (k = 0; k < groundface1->numedges; k++) {
     ...
     //~~~~~~~~~~~~~~~~~~
     //         ---------
     //         |          step height and low water up to the step ->
     //         TRAVEL_WALK
     //--------|
     //
  ...
  if (water_foundreach) {
    ...
    if ((*aasworld).areasettings[AAS_PointAreaNum(testpoint)].areaflags &
      ...
      if (water_bestdist < aassettings.sv_maxwaterjump + 24) {
        // waterjumping from or towards a crouch only area is not possible in
        // Quake2
        if (((*aasworld).areasettings[area1num].presencetype &
          ...
          VectorMA(water_bestend, INSIDEUNITS_WATERJUMP, water_bestnormal,
                   lreach->end);
  ...
  if (ground_foundreach) {
    if (ground_bestdist < 0) {
      if (ground_bestdist > -aassettings.sv_maxstep) {
        ...
        //INSIDEUNITS_WALKEND, ground_bestnormal, lreach->end);
        VectorMA(ground_bestend, INSIDEUNITS_WALKOFFLEDGEEND, ground_bestnormal,
                 lreach->end);
      ...
      if (!trace.startsolid && trace.fraction >= 1.0) {
        ...
        if (AAS_PointAreaNum(trace.endpos) == area2num) {
          ...
          lreach->traveltime =
              STARTWALKOFFLEDGE_TIME +
              Q_fabs(ground_bestdist) * 50 / aassettings.sv_gravity;
...
float AAS_ClosestEdgePoints(vec3_t v1,
                            vec3_t v2,
                            vec3_t v3,
                            vec3_t v4,
                            aas_plane_t* plane1,
...
int AAS_Reachability_Jump(int area1num, int area2num) {
  ...
  if (aasworld->areasettings[area1num].areaflags & AREA_JUMPSRC) {
    if (jumplinks[area1num].destarea == area2num) {
      ...
      height = dir[2];
      dir[2] = 0;
      lreach->traveltime = STARTJUMP_TIME + VectorDistance(beststart, bestend) *
                                                240 /
                                                aassettings.sv_maxwalkvelocity;
      //
    ...
    for (j = 0; j < area2->numfaces; j++) {
      ...
      for (k = 0; k < face1->numedges; k++) {
        ...
        for (l = 0; l < face2->numedges; l++) {
          ...
          //
          bestdist =
              AAS_ClosestEdgePoints(v1, v2, v3, v4, plane1, plane2, beststart,
                                    bestend, beststart2, bestend2, bestdist);
        }  // end for
      }    // end for
    }      // end for
  }        // end for
  ...
  if (bestdist > 4 && bestdist < maxjumpdistance) {
    ...
    //area1num, area2num, bestdist); if the fall would damage the bot
    //
    if (AAS_HorizontalVelocityForJump(0, beststart, bestend, &speed)) {
      ...
      // calculate this speed (the jump is not possible) then there's no jump
      // reachability created
      if (!AAS_HorizontalVelocityForJump(sv_jumpvel, beststart, bestend,
                                         &speed)) {
        return qfalse;
      }
    ...
    if (trace.fraction < 1) {
      ...
      if (DotProduct(plane->normal, up) >= 0.7) {
        if (!(AAS_PointContents(trace.endpos) &
              CONTENTS_LAVA)) {  //----(SA)	modified since slime is no
                                 //longer deadly
                                 //				if (!(AAS_PointContents(trace.endpos) &
    ...
    //		if (move.stopevent & (SE_ENTERSLIME|SE_ENTERLAVA)) return
    //qfalse; the end position should be in area2, also test a little bit back
    ...
    if (traveltype == TRAVEL_WALKOFFLEDGE && height > VectorLength(dir)) {
      lreach->traveltime =
          STARTWALKOFFLEDGE_TIME + height * 50 / aassettings.sv_gravity;
    } else {
      lreach->traveltime = STARTJUMP_TIME + VectorDistance(bestend, beststart) *
                                                240 /
                                                aassettings.sv_maxwalkvelocity;
    }  // end if
       //
...
int AAS_Reachability_Ladder(int area1num, int area2num) {
  ...
  if (ladderface1 && ladderface2) {
    ...
    if (ladderface1vertical && (ladderface2->faceflags & FACE_GROUND)) {
      ...
      reach_ladder++;
      // create a new reachability link
      //  RF, this should be a ladder reachability, since we usually climb down
      //  ladders in Wolf (falling is possibly dangerous)
      lreach = AAS_AllocReachability();
      if (!lreach) {
        return qfalse;
      }
    ...
    if (ladderface1vertical) {
                                                                                      ...
                                                                                      | AREACONTENTS_LAVA))
      {
              for (i = 20; i <= 120; i += 20)
              {
                      ...
                      //trace without entity collision
                      trace = AAS_TraceClientBBox(start, end, PRESENCE_NORMAL,
      -1);
                      //
                      ...
                      if ((*aasworld).areasettings[area2num].contents &
  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
    ...
    if (!strcmp(classname, "trigger_multiple")) {
      ...
      if (!AAS_ValueForBSPEpairKey(ent, "target", target, MAX_EPAIRKEY)) {
        botimport.Print(
            PRT_ERROR, "trigger_multiple at %1.0f %1.0f %1.0f without target\n",
      ...
      if (!AAS_ValueForBSPEpairKey(ent, "target", target, MAX_EPAIRKEY)) {
        botimport.Print(
            PRT_ERROR, "trigger_teleport at %1.0f %1.0f %1.0f without target\n",
            origin[0], origin[1], origin[2]);
        continue;
      }  // end if
    }    // end if
    ...
    if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num)) {
      destorigin[2] +=
          24;  // just for q2e1m2, the dork has put the telepads in the ground
...
aas_lreachability_t* AAS_FindFaceReachabilities(vec3_t* facepoints,
  ...
  for (i = 1; i < (*aasworld).numareas; i++) {
    ...
    for (j = 0; j < area->numfaces; j++) {
      ...
      for (k = 0; k < face->numedges; k++) {
        ...
        for (l = 0; l < numpoints; l++) {
          ...
          dist =
              AAS_ClosestEdgePoints(v1, v2, v3, v4, faceplane, plane, beststart,
                                    bestend, beststart2, bestend2, bestdist);
          if (dist < bestdist) {
            bestfacenum = facenum;
    ...
    testpoint[2] = 0;
    testpoint[2] =
        (bestfaceplane->dist - DotProduct(bestfaceplane->normal, testpoint)) /
...
void AAS_Reachability_FuncBobbing(void) {
  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
    ...
    //
    Log_Write(
        "funcbob model %d, start = {%1.1f, %1.1f, %1.1f} end = {%1.1f, %1.1f, "
        "%1.1f}\n",
    ...
    */
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (i == 0) {
        ...
        firstendreach = AAS_FindFaceReachabilities(start_edgeverts, 4,
                                                   &start_plane, qfalse);
      }  // end else
         //
      ...
      for (startreach = firststartreach; startreach;
        ...
        //
        // trace = AAS_TraceClientBBox(startreach->start, move_start_top,
...
void AAS_Reachability_JumpPad(void) {
  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
    ...
    // if there is a horizontal velocity check for a reachability without air
    // control
      ...
      for (i = 0; i < area2->numfaces; i++) {
        ...
        if (ret && speed < 150) {
          ...
          // if (hordist < 1.6 * facecenter[2] - areastart[2])
          {
            ...
            //
            AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL,
                                      qfalse, velocity, cmdmove, 30, 30, 0.1,
...
int AAS_Reachability_Grapple(int area1num, int area2num) {
  ...
  for (i = 0; i < area2->numfaces; i++) {
    ...
    // the grapple won't stick to the sky and the grapple point should be near
    // the AAS wall
    ...
    if ((*aasworld).areasettings[areanum].contents &
        AREACONTENTS_LAVA) {  //----(SA)	modified since slime is no
                              //longer deadly
      //		if ((*aasworld).areasettings[areanum].contents &
...
void AAS_SetWeaponJumpAreaFlags(void) {
  ...
  //
  weaponjumpareas = 0;
  jumplinks = (aas_jumplink_t*)GetClearedMemory(aasworld->numareas *
                                                sizeof(aas_jumplink_t));
  //
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
    if (!AAS_ValueForBSPEpairKey(ent, "classname", classname, MAX_EPAIRKEY)) {
      continue;
    ...
    if (!strcmp(classname, "bot_jump_source")) {
      ...
      for (dent = AAS_NextBSPEntity(0); dent; dent = AAS_NextBSPEntity(dent)) {
        if (!AAS_ValueForBSPEpairKey(dent, "targetname", targetname,
                                     MAX_EPAIRKEY)) {
          continue;
        }
      ...
      if (!dent) {
        botimport.Print(
            PRT_MESSAGE,
            "WARNING: %s doesn't have a matching bot_jump_dest (target = %s)\n",
            classname, target);
        continue;
      }
      // success
      ...
      if (AAS_VectorForBSPEpairKey(ent, "origin", origin)) {
        ...
        if (!areanum) {
          botimport.Print(PRT_MESSAGE,
                          "%s in void area at (%1.1f %1.1f %1.1f)\n", classname,
                          origin[0], origin[1], origin[2]);
          continue;
        }
      } else {
      ...
      // find dest area
      if (!AAS_ValueForBSPEpairKey(dent, "classname", classname,
                                   MAX_EPAIRKEY)) {
        continue;
      }
      if (AAS_VectorForBSPEpairKey(dent, "origin", destorigin)) {
        ...
        if (!(spawnflags & 1)) {
          if (!AAS_DropToFloor(destorigin, mins, maxs)) {
            botimport.Print(PRT_MESSAGE, "%s in solid at (%1.1f %1.1f %1.1f)\n",
                            classname, destorigin[0], destorigin[1],
                            destorigin[2]);
          }  // end if
        }    // end if
           // areanum = AAS_PointAreaNum(origin);
        ...
        if (!destareanum) {
          botimport.Print(PRT_MESSAGE,
                          "%s in void area at (%1.1f %1.1f %1.1f)\n", classname,
                          destorigin[0], destorigin[1], destorigin[2]);
          continue;
        }
      } else {
        botimport.Print(PRT_MESSAGE, "%s has no origin (%s)\n", classname,
    ...
    /*
                            ...
                            !strcmp(classname, "item_invulnerability"))
                    {
                            ...
                            {
                                    spawnflags = 0;
                                    AAS_IntForBSPEpairKey(ent, "spawnflags",
       &spawnflags);
                                    //if not a stationary item
                                    if (!(spawnflags & 1))
                                    {
                                            if (!AAS_DropToFloor(origin, mins,
       maxs))
                                            {
                                                    botimport.Print(PRT_MESSAGE,
       "%s in solid at (%1.1f %1.1f %1.1f)\n", classname, origin[0], origin[1],
       origin[2]); } //end if } //end if
                                    //areanum = AAS_PointAreaNum(origin);
                                    areanum = AAS_BestReachableArea(origin,
       mins, maxs, origin);
                                    //the bot may rocket jump towards this area
                                    (*aasworld).areasettings[areanum].areaflags
       |= AREA_WEAPONJUMP;
                                    //
                                    if (!AAS_AreaGrounded(areanum))
       botimport.Print(PRT_MESSAGE, "area not grounded\n");
  ...
  /*
     ...
     AREA_WEAPONJUMP; weaponjumpareas++; } //end if } //end for
  */
      ...
      if (ret && speed < 270) {
        ...
        // if (hordist < 1.6 * (facecenter[2] - areastart[2]))
        {
          ...
          */
          //
          AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL,
                                    qtrue, velocity, cmdmove, 30, 30, 0.1,
                                    SE_ENTERWATER | SE_ENTERSLIME |
                                        SE_ENTERLAVA | SE_HITGROUNDDAMAGE |
        ...
        for (l = 0; l < face2->numedges; l++) {
          ...
          if (abs(edge1num) == abs(edge2num)) {
            ...
            if (!AAS_AreaGrounded(reachareanum) &&
              ...
              // Log_Write("area %d, reach area %d: not grounded and not
              // swim\r\n", areanum, reachareanum);
              break;
            }  // end if
               //
            if ((*aasworld).areasettings[reachareanum].contents &
                AREACONTENTS_LAVA) {  //----(SA)	modified since slime is
                                      //no longer deadly
                                      //						if
              //((*aasworld).areasettings[reachareanum].contents &
              //(AREACONTENTS_SLIME | AREACONTENTS_LAVA)) Log_Write("area %d,
              // reach area %d: lava or slime\r\n", areanum, reachareanum);
              break;
            }  // end if
            lreach = AAS_AllocReachability();
            if (!lreach) {
            ...
            lreach->traveltime =
                STARTWALKOFFLEDGE_TIME +
                Q_fabs(mid[2] - trace.endpos[2]) * 50 / aassettings.sv_gravity;
...
int AAS_ContinueInitReachability(float time) {
  ...
  // loop over the areas
  for (i = (*aasworld).reachabilityareas; i < (*aasworld).numareas && i < todo;
       i++) {
    ...
    for (j = 1; j < (*aasworld).numareas; j++) {
      ...
      // never create reachabilities from teleporter or jumppad areas to regular
      // areas
      if ((*aasworld).areasettings[i].contents &
    ...
    //
    if ((*aasworld).reachabilityareas * 1000 / (*aasworld).numareas >
        lastpercentage) {
      break;
    }
  }  // end for
  ...
  if ((*aasworld).reachabilityareas == (*aasworld).numareas) {
    ...
    lastpercentage =
        (*aasworld).reachabilityareas * 1000 / (*aasworld).numareas;
