...
void AAS_Optimize(void) {
  ...
  for (i = 0; i < (*aasworld).reachabilitysize; i++) {
    ...
    // NOTE: for TRAVEL_JUMPPAD the facenum is the Z velocity and the edgenum is
    // the hor velocity
...
void AAS_RemoveNonReachabilityAlloc(optimized_t* optimized) {
  optimized->areas =
      (aas_area_t*)GetClearedMemory((*aasworld).numareas * sizeof(aas_area_t));
  //
  optimized->areasettings = (aas_areasettings_t*)GetClearedMemory(
      (*aasworld).numareas * sizeof(aas_areasettings_t));
  //
  optimized->reachability = (aas_reachability_t*)GetClearedMemory(
      (*aasworld).reachabilitysize * sizeof(aas_reachability_t));
  optimized->reachabilitysize = (*aasworld).reachabilitysize;
  /*	//
          optimized->nodes = (aas_node_t *)
     GetClearedMemory((*aasworld).numnodes * sizeof(aas_node_t));
          optimized->numnodes = (*aasworld).numnodes;
          //
          optimized->portals = (aas_portals_t *)
     GetClearedMemory((*aasworld).numportals * sizeof(aas_portal_t));
          optimized->numportals = (*aasworld).numportals;
          //
          optimized->clusters = (aas_cluster_t *)
     GetClearedMemory((*aasworld).numclusters * sizeof(aas_cluster_t));
  ...
  */                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     //
  ...
  optimized->arearemap =
      (int*)GetClearedMemory((*aasworld).numareas * sizeof(int));
  optimized->removedareas =
      (int*)GetClearedMemory((*aasworld).numareas * sizeof(int));
  optimized->reachabilityremap =
      (int*)GetClearedMemory((*aasworld).reachabilitysize * sizeof(int));
}  // end of the function AAS_OptimizeAlloc
//===========================================================================
//
// Parameter:				-
...
void AAS_RemoveNonReachability(void) {
  ...
  for (i = 1; i < aasworld->reachabilitysize; i++) {
    ...
    optimized.reachability[validreach].areanum =
        optimized.arearemap[optimized.reachability[validreach].areanum];
    //
    validreach++;
  }
  ...
  for (i = 1; i < numoriginalareas; i++) {
    ...
    optimized.areasettings[j].firstreachablearea =
        optimized
            .reachabilityremap[aasworld->areasettings[i].firstreachablearea];
    ...
    for (j = 0; j < 2; j++) {
      if (aasworld->nodes[i].children[j] < 0) {
        if (optimized.removedareas[-aasworld->nodes[i].children[j]]) {
          ...
          aasworld->nodes[i].children[j] =
              -optimized.arearemap[-aasworld->nodes[i].children[j]];
        }
      }
    }
  }
  ...
  for (i = 0; i < aasworld->numportals; i++) {
    aasworld->portals[i].areanum =
        optimized.arearemap[aasworld->portals[i].areanum];
  }
  ...
  botimport.Print(PRT_MESSAGE,
                  "%i non-reachability areas removed, %i remain.\n", removed,
                  valid);
}  // end of the function AAS_Optimize
//===========================================================================
//
...
void AAS_RemoveNonGrounded(void) {
  ...
  for (i = 1; i < (*aasworld).numareas; i++) {
    ...
    if (optimized.areakeep[i] ||
        (aasworld->areasettings[i].areaflags & (AREA_GROUNDED | AREA_LADDER))) {
      optimized.arearemap[i] = ++valid;
      // copy it to the optimized areas
      optimized.areas[valid] = (*aasworld).areas[i];
      optimized.areas[valid].areanum = valid;
