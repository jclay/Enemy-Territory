...
// Hack variable for deciding which kind of texture rectangle thing to do (for
// some reason it acts different on radeon! It's against the spec!).
...
void CheckErrors(const char* inFile, int inLine) {
  ...
  if (err != AGL_NO_ERROR) {
    ...
    Com_Error(ERR_FATAL, "aglGetError: %s at %s line %d", aglErrorString(err),
              inFile, inLine);
  }
}

Boolean IsRadeon(void) {
  if (strstr(glConfig.renderer_string, "Radeon") ||
      strstr(glConfig.renderer_string, "R-200")) {
    return true;
  } else {
    return false;
  }
...
void GLimp_InitGamma(void) {
...

  {
    if (1) {
      ...
      cgErr = CGGetDisplayTransferByTable(
          NULL, 256, cgGamma->red, cgGamma->green, cgGamma->blue, &count);
      if (cgErr) {
...
void GLimp_AglDescribe_f(void) {
  ...
  VID_Printf(PRINT_ALL,
             "red:%i green:%i blue:%i alpha:%i depth:%i stencil:%i\n", r, g, b,
             a, depth, stencil);
...
Q3DEF void GLimp_SetGameGamma(void) {
...

  {
    ...
    cgErr = CGSetDisplayTransferByTable(NULL, 256, sys_gl.gameGamma.red,
                                        sys_gl.gameGamma.green,
                                        sys_gl.gameGamma.blue);
...
static void GLW_InitTextureCompression(void) {
  ...
  newer_tc =
      (strstr(glConfig.extensions_string, "ARB_texture_compression") &&
       strstr(glConfig.extensions_string, "EXT_texture_compression_s3tc"))
          ? qtrue
          : qfalse;
...
#endif
  {
    ...
    // some form of supported texture compression is avaiable, so see if the
      ...
      if (old_tc && newer_tc) {
        ...
        if (r_ext_preferred_tc_method->integer == TC_S3TC) {
          ...
          if (old_tc) {
            ...
            VID_Printf(PRINT_ALL,
                       "...preferred tc method, GL_S3_s3tc not available\n");
            VID_Printf(
                PRINT_ALL,
                ".....falling back to GL_EXT_texture_compression_s3tc\n");
            glConfig.textureCompression = TC_S3TC_DXT;
          }
        } else {
          ...
          if (newer_tc) {
            ...
            VID_Printf(PRINT_ALL,
                       "...preferred tc method, "
                       "GL_EXT_texture_compression_s3tc not available\n");
...
static void GLimp_Extensions(void) {
...
#if MAC_JKJA
    Cvar_Set("r_ext_texture_filter_anisotropic_avail",
             va("%f", glConfig.maxTextureFilterAnisotropy));
    if (r_ext_texture_filter_anisotropic->value >
        glConfig.maxTextureFilterAnisotropy) {
      Cvar_Set("r_ext_texture_filter_anisotropic",
               va("%f", glConfig.maxTextureFilterAnisotropy));
    }
#else
  ...
  if (strstr(glConfig.extensions_string, "GL_ARB_multitexture")) {
    if (r_ext_multitexture->integer) {
      ...
      if (qglActiveTextureARB) {
        qglGetIntegerv(GL_MAX_ACTIVE_TEXTURES_ARB,
                       (long*)&glConfig.maxActiveTextures);
  ...
  // LBO - the PC JK2 code checks for GL_EXT_point_parameters, but Apple
  // supports the ARB version which is identical.
  ...
  if (IsRadeon() /*&& gSystemVersion >= 0x0922*/) {
    GLenum err;
    aglSetInteger(sys_gl.context, ATI_FSAA_SAMPLES,
                  (const long*)&r_ati_fsaa_samples->integer);
...
Q3DEF void GLimp_Init(void) {
  ...
  cvar_t* lastValidRenderer =
      Cvar_Get("r_lastValidRenderer", "(uninitialized)", CVAR_ARCHIVE);
  ...
  r_ext_transform_hint =
      Cvar_Get("r_ext_transform_hint", "1", CVAR_LATCH | CVAR_ARCHIVE);

  // Default FSAA options to off
  r_ati_fsaa_samples = Cvar_Get("r_ati_fsaa_samples", "1", CVAR_ARCHIVE);
  r_nv_fsaa_samples =
      Cvar_Get("r_nv_fsaa_samples", "0", CVAR_LATCH | CVAR_ARCHIVE);
  r_nv_quincunx = Cvar_Get("r_nv_quincunx", "0", CVAR_ARCHIVE);
#if MAC_Q3_OLDSTUFF
  r_ext_preferred_tc_method =
      Cvar_Get("r_ext_preferred_tc_method", "0", CVAR_ARCHIVE | CVAR_LATCH);
  r_ext_texture_filter_anisotropic =
      Cvar_Get("r_ext_texture_filter_anisotropic", "0", CVAR_ARCHIVE);
#endif
...
#if MAC_JK2 || \
    MAC_JKJA  // LBO - starting with JK2, these are pointers to const strings
  glConfig.vendor_string = (const char*)qglGetString(GL_VENDOR);
  glConfig.renderer_string = (const char*)qglGetString(GL_RENDERER);
  glConfig.version_string = (const char*)qglGetString(GL_VERSION);
  glConfig.extensions_string = (const char*)qglGetString(GL_EXTENSIONS);
#else
  Q_strncpyz(glConfig.vendor_string, (const char*)qglGetString(GL_VENDOR),
             sizeof(glConfig.vendor_string));
  Q_strncpyz(glConfig.renderer_string, (const char*)qglGetString(GL_RENDERER),
             sizeof(glConfig.renderer_string));
  Q_strncpyz(glConfig.version_string, (const char*)qglGetString(GL_VERSION),
  ...
  if (Q_stricmp(lastValidRenderer->string, glConfig.renderer_string)) {
    ...
    if (strstr(buf, "kyro")) {
      Cvar_Set("r_ext_texture_filter_anisotropic",
               "0");  // KYROs have it avail, but suck at it!
      Cvar_Set("r_ext_preferred_tc_method",
               "1");  //(Use DXT1 instead of DXT5 - same quality but much better
                      //performance on KYRO)
    }
    ...
    if ((glConfig.textureCompression == TC_NONE) ||
        (glConfig.maxActiveTextures < 2) || (glConfig.maxTextureSize <= 512)) {
...
Q3DEF void GLimp_EndFrame(void) {
  ...
  if (r_swapInterval->modified) {
    ...
    aglSetInteger(sys_gl.context, AGL_SWAP_INTERVAL,
                  (long*)&r_swapInterval->integer);
  }

  if (r_ati_fsaa_samples->modified) {  // DAJ added changed for ati fsaa
    r_ati_fsaa_samples->modified = qfalse;
    if (IsRadeon() /*&& gSystemVersion >= 0x0922*/) {
      if (r_ati_fsaa_samples->integer == 1 ||
          r_ati_fsaa_samples->integer == 2 ||
          r_ati_fsaa_samples->integer == 4) {
        GLenum err;
        ...
        VID_Printf(PRINT_ALL,
                   "Valid ATI FSAA samples are 1, 2, 4.  Resetting to 1\n");
  ...
  if (sys_gl.systemGammas) {
    {
      ...
      CGSetDisplayTransferByTable(NULL, 256, cgGamma->red, cgGamma->green,
                                  cgGamma->blue);
...
qboolean GLimp_ChangeMode(int mode) {
  ...
  if (!_aglSetGameContext(glConfig.vidWidth, glConfig.vidHeight, colorDepth,
                          zDepth, r_stencilbits->integer, gRefreshRate,
                          sys_gl.displayID, !r_fullscreen->integer,
                          fsaaSamples)) {
    //		return qfalse;
...
Q3DEF qboolean GLimp_SpawnRenderThread(void (*function)(void)) {
  ...
  if (pthread_create(&renderThreadHandle, NULL, GLimp_RenderThreadWrapper,
                     NULL)) {
    return qfalse;
  }
...
Q3DEF qboolean GLimp_SpawnRenderThread(void (*function)(void)) {
  ...
  status = MPCreateTask(GLimp_RenderThreadWrapper, (void*)NULL, 0, NULL, 0, 0,
                        kNilOptions, &renderThreadTask);
  if (status != noErr) {
