...
#define AGL_SAMPLE_BUFFERS_ARB \
  55                       /* number of multi sample buffers               */
...
AGLContext gAGLContext;  // The currently active drawing context, either primary
                         // or secondary
AGLContext gAGLContext_Primary;  // Primary context, used in non-SMP situations
                                 // or in the main thread
AGLContext
    gAGLContext_Secondary;  // Secondary context, used mainly for an SMP thread
AGLPixelFormat gPixelFormat;

//=====================================================================
// Local Variables

static WindowRef sBlankingWindow;  // Used only for AGL_FULLSCREEN cases, to
                                   // blank out the desktop
static WindowRef
    sGLWindow;  // In non-AGL_FULLSCREEN cases, contains the GL drawable window
...
static EventHandlerUPP
    sWindowEventHandlerUPP;  // carbon window constrain event handler
...
OSStatus MySetWindowContentColor(WindowRef inWindow, RGBColor const* inRGB) {
  ...
  // LBO - the following routine requires 8.5 in non-Carbon builds (8.1 in
  // Carbon)
...
//===============================================================================
static pascal OSStatus appWindowEventHandler(EventHandlerCallRef myHandler,
                                             EventRef event,
                                             void* userData) {
...
void InstallCarbonWindowEvents(WindowRef inWindow) {
  ...
  status = InstallWindowEventHandler(inWindow, sWindowEventHandlerUPP, 1, list,
                                     0, &sEventHandlerRef);
  if (status != noErr) {
    SysBeep(0);
  }
...
//  _aglSetGameContext
//
//  Given an already chosen AGL context and monitor, this routine will take care
//  of the bs involved with choosing a properly-sized window (on non-fullscreen
//  devices) and it will activate a fullscreen context if desired for those
//  devices that support that option.
// ÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑ
GLboolean _aglSetGameContext(short inWidth,
  ...
  // If our monitor is set to 8-bit under MacOS 8 or 9, we have to change that
  // for OpenGL to find a valid context.
  if (sSystemVersion < 0x01000) {
    ...
    if (curDepth == 8) {
      ...
      if (err) {
        MacWarning(
            "This game requires a color monitor that supports thousands or "
            "millions of colors");
      }
    }
  }
  ...
  if (sSystemVersion < 0x01000) {
    ...
    // fullscreen. It's deprecated and buggy, so we must set it up the "wrong"
    // way to get agl fullscreen to work, otherwise the system will freak out
    // and crash badly in the aglChoosePixelFormat call below.
    attrib[3] = AGL_ACCELERATED;
    gPixelFormat = aglChoosePixelFormat(&device, 1, attrib);
  } else {
    // Since we're trying for AGL_FULLSCREEN, we must set the first 2 params to
    // 0
  ...
  // This is pretty dumb, but on OSX it can be mutually exclusive due to a bug
  // in OpenGL
  ...
  // Burp the agl error handler. We could have encountered errors above and
  // worked around them so we need to toss any errors currently pending.
  glErr = aglGetError();

  // If it is a fullscreen renderer and we want to run fullscreen, set up a
  // fullscreen context. We don't use DrawSprocket for fullscreen contexts (e.g.
  // 3dfx) because a) it's not strictly necessary and b) the aglSetFullScreen
  // call can confuse DrawSprocket and cause the Finder to mangle the desktop
  // icons The downside of this is that there are no gamma fades around the res
  // switch when we're running on 3dfx hardware - yet.
  if (result && !inWindow) {
    OSStatus status;
    Rect desktopRect;
    CGrafPtr oldPort;
    ...
    {
      status = CreateNewWindow(kDocumentWindowClass, winAttrib, &desktopRect,
                               &sBlankingWindow);
      if (status) {
...
#else
    sBlankingWindow = NewCWindow(NULL, &desktopRect, "\p", false, noGrowDocProc,
                                 (WindowRef)-1, false, 0);
    if (!sBlankingWindow) {
      return GL_FALSE;
    }
    ...
    ok = aglSetFullScreen(gAGLContext, glWidth, glHeight, glFrequency,
                          glMonitor);
    ...
    if (!ok) {
      MacFatalError(
          "aglSetFullScreen failed. Try specifying a different refresh rate.");
      return GL_FALSE;
    }
    ...
    if (!inWindow) {
      ...
      //			if (!gDrawSprocket->SwitchToResolution (inWidth,
      //inHeight, inDepth, inFreq, 1, true)) return GL_FALSE;
      gDrawSprocket->ConfigureGameContext(inWidth, inHeight, inDepth, inFreq,
                                          1);
      ...
      gDrawSprocket->GetGameResolution(&outWidth, &outHeight, &outDepth,
                                       &outFreq, &outPageCount);
#if MAC_Q3
#if MAC_JKJA
      ...
      // Center the window
      sGLWindowRect.left =
          (((*device)->gdRect.right - (*device)->gdRect.left) - glWidth) / 2;
      sGLWindowRect.top =
          (((*device)->gdRect.bottom - (*device)->gdRect.top) - glHeight) / 2;
    ...
    if (!inWindow) {
      ...
      //			DSpContext_LocalToGlobal (gameContext, (Point
      //*)&sGLWindowRect);
...
#if TARGET_API_MAC_CARBON
    {
      ...
      if (inWindow) {
        CreateNewWindow(kDocumentWindowClass, winAttrib, &sGLWindowRect,
                        &sGLWindow);
      } else if (sSystemVersion < 0x01000) {
        //				CreateNewWindow (kPlainWindowClass,
        //winAttrib, &sGLWindowRect, &sGLWindow); 				CreateNewWindow
        //(kDocumentWindowClass, winAttrib, &sGLWindowRect, &sGLWindow);
        // Always create a window on classic
        sGLWindow = NewCWindow(NULL, &sGLWindowRect, winTitle, 0, windowType,
                               (WindowRef)-1, 0, 0);
        if (sGLWindow == NULL) {
          return GL_FALSE;
        }
    ...
    if (sGLWindow) {
      ...
      //			status = MySetWindowContentColor (sGLWindow,
      //&sRGBBlack);
      ...
      if (sCurrentAGLPort == NULL) {
        //				ReportError
        //("DSpContext_GetFrontBuffer() had an error.");
        return NULL;
      }

      // there is a problem in Mac OS X GM CoreGraphics that may not size the
      // port pixmap correctly this will check the vertical sizes and offset if
      // required to fix the problem this will not center ports that are smaller
      // then a particular resolution
      {
        long deltaV, deltaH;
        Rect portBounds;
        ...
        deltaV = (portBounds.bottom - portBounds.top) -
                 (pixBounds.bottom - pixBounds.top) +
        ...
        if (deltaV || deltaH) {
          ...
          // set origin to account for CG offset and if requested drawable
          // smaller than screen rez
...
#undef MoveWindow
    ...
    result = aglSetFullScreen(gAGLContext, glWidth, glHeight, glFrequency,
                              glMonitor);

    // That tanked, try a different frequency
    if (!result) {
      result =
          aglSetFullScreen(gAGLContext, glWidth, glHeight, 60.0, glMonitor);
    }
    if (!result) {
      result =
          aglSetFullScreen(gAGLContext, glWidth, glHeight, 75.0, glMonitor);
    }
  }
#if TARGET_API_MAC_CARBON
  else {
