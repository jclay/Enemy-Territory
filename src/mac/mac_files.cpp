...
void GetHFSPathFromPosix(const char* inPath, char* outPath) {
  ...
  theFileName = CFStringCreateWithCString(CFAllocatorGetDefault(), inPath,
                                          kCFStringEncodingMacRoman);
  if (!theFileName) {
    goto bail;
  }
  theFileURL = CFURLCreateWithFileSystemPath(
      CFAllocatorGetDefault(), theFileName, kCFURLPOSIXPathStyle, false);
  if (!theFileURL) {
    goto bail;
  }
  ...
  CFStringGetCString(newPathString, outPath, MAX_OSPATH,
                     kCFStringEncodingMacRoman);
...
void GetPosixPathFromHFS(const char* inPath, char* outPath) {
  ...
  theFileURL = CFURLCreateWithFileSystemPath(
      CFAllocatorGetDefault(), theFileName, kCFURLHFSPathStyle, false);
  if (!theFileURL) {
    goto bail;
  }
...
FILE* mac_fopen(const char* filename, const char* open_mode) {
  if (strstr(filename, ".cfg") || strstr(filename, ".log") ||
      strstr(filename, ".txt")) {
    _ftype = text_type;
...
OSErr GetFullPathFromSpec(const FSSpec* spec, char* fullpath) {
  ...
  // case, we create an FSRef for the parent directory, convert that to a full
  // path and append the filename to that.
  if (err == fnfErr) {
    ...
    if (err == noErr) {
      theParentURL =
          CFURLCreateFromFSRef(CFAllocatorGetDefault(), paramBlock.newRef);
      theFileName = CFStringCreateWithBytes(CFAllocatorGetDefault(),
                                            spec->name + 1, spec->name[0],
                                            kCFStringEncodingMacRoman, false);
      theFileURL = CFURLCreateWithFileSystemPathRelativeToBase(
          CFAllocatorGetDefault(), theFileName,
#if USE_POSIX_PATH
...
#endif
          theParentURL);
      successful = CFURLGetFileSystemRepresentation(
          theFileURL, true, (UInt8*)fullpath, MAX_OSPATH);
      CFRelease(theFileURL);
      CFRelease(theFileName);
...
OSErr GetApplicationPackageFSSpecFromBundle(FSSpecPtr theFSSpecPtr) {
  ...
  return FSGetCatalogInfo(&myBundleRef, kFSCatInfoNone, NULL, NULL,
                          theFSSpecPtr, NULL);
}
...
void Sys_Mkdir(const char* inPath) {
  ...
  if (result != 0 /*&& errno == ENOENT*/) {
    ...
    // Remove trailing delimiter if it exists, we'll use null terminator
    // instead.
...
char* Sys_DefaultHomePath(void) {
  ...
  if (1) {  //(gSystemVersion >= 0x1000)
    ...
    err = FindFolder(kUserDomain, kApplicationSupportFolderType, kCreateFolder,
                     &refNum, &dirID);
...
char* Sys_DefaultBasePath(void) {
  // In older versions of the Q3 engine, it doesn't differentiate between the
  // install path and the user's home directory. Instead, it calls the
  // installation path the "base path". In this case, we point the install to
  // the home directory and fake up the CD path to point to the install path,
  // since we never use the CD path anyway.
...
#if MAC_MOHAA
Q3DEF char** Sys_ListFiles(const char* directory,
...
#else
    listCopy =
        (char**)Z_Malloc((nfiles + 1) * sizeof(*listCopy), TAG_FILESYS, qfalse);
#endif
    for (i = 0; i < nfiles; i++) {
      listCopy[i] = list[i];
    }
...
#else
  listCopy =
      (char**)Z_Malloc((nfiles + 1) * sizeof(*listCopy), TAG_LISTFILES, qfalse);
#endif
  for (i = 0; i < nfiles; i++) {
    listCopy[i] = list[i];
  }
...
#if MAC_JKJA
qboolean Sys_FileOutOfDate(LPCSTR psFinalFileName /* dest */,
                           LPCSTR psDataFileName /* src */) {
  return qfalse;
}

qboolean Sys_CopyFile(LPCSTR lpExistingFileName,
