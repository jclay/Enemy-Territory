...
This file is part of the Wolfenstein: Enemy Territory GPL Source Code (Â“Wolf ET
Source CodeÂ”).
...
static OSStatus MyFindBestContextOnDisplayID(
    DSpContextAttributesPtr inDesiredAttrib,
  ...
  if ((void*)DSpFindBestContextOnDisplayID !=
      (void*)kUnresolvedCFragSymbolAddress) {
    return ::DSpFindBestContextOnDisplayID(inDesiredAttrib, outContext,
                                           inDisplayID);
  }
#endif
  ...
  while (status == noErr) {
    ...
    if (status == noErr) {
      ...
      if (inDesiredAttrib->pageCount == 1) {
        ...
        if ((attrib.backBufferDepthMask &
             inDesiredAttrib->backBufferDepthMask) &&
            (attrib.displayDepthMask & inDesiredAttrib->displayDepthMask)) {
          score =
              score +
              (((attrib.backBufferBestDepth &
                 inDesiredAttrib->backBufferDepthMask) &&
                (attrib.displayBestDepth & inDesiredAttrib->displayBestDepth))
                   ? kScoreDepthExact
                   : kScoreDepthOK);
...
CDrawSprocket::CDrawSprocket(DisplayIDType inPreferredMonitor) {
  ...
  if ((Ptr)::DSpContext_Queue != (Ptr)kUnresolvedCFragSymbolAddress) {
    // My Voodoo monitor (secondary) crashes hard when a 32-bit,
    // single-buffered, 1024x768 game context is reserved and activated,
    // followed by a 16-bit, single-buffered 640x480 movie context being queued
    // and switched. The actual crash is in PaintRect() in SwitchToContext().
    // Not using the queueing fixes the crash. No problem with the Ati Rage 128
    // monitor. There also was no problem when the game context was 16-bit. I
    // suspect a 3dfx driver bug. I added an option to Centipede to disable fast
    // res switching and turn mHasQueue back off.
    // - jb, 3/27/01
    mHasQueue = true;
  }
  ...
  if ((Ptr)kUnresolvedCFragSymbolAddress != (Ptr)DSpGetVersion) {
    ...
    if ((versionDSp.majorRev == 0x01) && (versionDSp.minorAndBugRev < 0x99)) {
      ...
      if ((Gestalt(gestaltSystemVersion, (SInt32*)&response) == noErr) &&
        ...
        MacWarning(
            "This version of DrawSprocket is unstable under MacOS X. Proceed "
            "with caution.");
      }
    }
  }
...
//------------------------------------------------------------------------------------
//		Make sure that when you hit a breakpoint, you don't have a blank
//screen
//------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------
//		IsValidResolution indicates whether or not the currently
//selected 		display has a valid context available for the desired width, height,
//and depth. 		Frequency info is optional.  When we iterate through the available
//contexts, 		we assume that DrawSprocket will return them in order.  Since this
//should be 		the case, we can look for the first context whose width & height is
//EQUAL to 		or GREATER than the width and height we're requesting.
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::IsValidResolution(short inWidth,
                                         short inHeight,
                                         short inDepth,
                                         Fixed inFrequency) {
  ...
  // Iterate through each successive context looking for the first one which
  // matches our search criteria the best
  while (status == noErr) {
    ...
    if (status == noErr && theAttributes.displayWidth >= inWidth &&
...
//------------------------------------------------------------------------------------
//
//      Returns the context that best matches the specified attributes.
//      Restricts
//		the context to the monitor specified by mDisplayID, unless the
//user is unable 		or unwilling to choose a monitor.
//
//		Returns in outAttributes attributes that describe the requested
//attributes.
//
...
//
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::FindBestContext(short inWidth,
                                       short inHeight,
                                       short inDepth,
                                       Fixed inFrequency,
                                       short inPageCount,
  ...
  // if we couldn't get a context for the specified display or no display was
  // specifed, try to get one for any display
  if (context == NULL) {
...
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::FindFirstContext(DSpContextReference* outContext,
                                        DSpContextAttributes* outAttributes) {
  OSStatus status;
...
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::FindNextContext(DSpContextReference* ioContext,
                                       DSpContextAttributes* outAttributes) {
  OSStatus status;
...
//------------------------------------------------------------------------------------
// Sets up a DrawSprocket context, must be called before any DrawSprocket
// context has been made active. If we're using DSp 1.7 or later, this will also
// queue the context so that we can switch to it quickly.
//
// If this is called and a movie context has already been set, it returns false
// and does nothing further.
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::ConfigureMovieContext(short inWidth,
                                             short inHeight,
                                             short inDepth,
                                             Fixed inFrequency,
                                             short inPageCount) {
  ...
  if (!FindBestContext(inWidth, inHeight, inDepth, inFrequency, inPageCount,
                       &newContext, &desiredAttributes)) {
    // For some reason, we've failed miserably.  Gotta punt...
    //		FadeGammaUp(true);
    //		char text[64];
    //		sprintf(text, "[%dx%d, %d bit, %d buffer(s)]", mMovieWidth,
    //mMovieHeight, mMovieDepth, inPageCount);
    return false;
  } else {
...
// queue the context so that we can switch to it quickly.
//
// If this is called and a game context has already been set, it returns false
// and does nothing further.
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::ConfigureGameContext(short inWidth,
                                            short inHeight,
                                            short inDepth,
                                            Fixed inFrequency,
                                            short inPageCount) {
  ...
  if (!mGameContext) {
    ...
    if (!FindBestContext(inWidth, inHeight, inDepth, inFrequency, inPageCount,
      ...
      //			FadeGammaUp(true);
      //			char text[64];
      //			sprintf(text, "[%dx%d, %d bit, %d buffer(s)]",
      //mMovieWidth, mMovieHeight, mMovieDepth, inPageCount);
      return false;
    } else {
      // Set this flag so that updates aren't synched with the vertical retrace
      desiredAttributes.contextOptions |= kDSpContextOption_DontSyncVBL;
...
// and does nothing further.
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::ConfigureAltGameContext(short inWidth,
                                               short inHeight,
                                               short inDepth,
                                               Fixed inFrequency,
                                               short inPageCount) {
...
//------------------------------------------------------------------------------------
//
//	Switch to the specified context, using Queue/Switch if available. - jb,
//3/24/01
//
//	Some things about DrawSprocket that are apparently undocumented but seem
//to be true:
//
//	- Each time you call DSpFindBestContextOnDisplayID(),
//DSpGetFirstContext(), 		DSpGetNextContext(), or DSpFindBestContext(), you get a
//different context 		reference, i.e. if you call DSpGetFirstContext() three times
//in a row you 		get three different references. (Is it leaking memory each time?
//I wouldn't 		doubt it.) If you ever call DSpContext_Release() on a context
//reference, it is 		invalid forever after that and can't be reserved again.
//
//	- Only one context can ever be reserved at a time. Would have been nice
//if that 		was mentioned in the docs.
//
//	- There must be a reserved context when you call the gamma fade
//functions or 		your Mac will crash hard.
//
//	- If you have queued or reserved any contexts, you must release them
//before 		exiting your application to prevent hard crashes and/or corruption on
//quit.
//
//	- If you intend to open dialog boxes or the like while a context is
//active, you 		should ensure that your application reserves and activates its
//largest 		context before any other context. Otherwise the visible region can be
//too 		small and interfere with the drawing of the dialog box.
//
//	- If you reserve a context but don't activate it, and then activate a
//different 		context, you can be pretty sure desktop icons will get moved all
//over the 		place.
//
//	YMMV. - jb, 3/25/01
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::SwitchToContext(DSpContextReference inContext,
                                       DSpContextAttributes* inAttrib,
                                       Boolean inDoFade) {
  ...
  if (mContext) {
    ...
    // Voodoo3 monitor is selected. It also prevents a problem with the
    // InputSprocket configure dialog appearing partially offscreen on the 3dfx
    // monitor. The reason is probably complicated... - jb, 3/25/01
    SetGDevice(GetMainDevice());
#endif
    ...
    if (mHasQueue) {
      ...
      if (killedContext) {
        ...
        *killedContext = NULL;
        ...
        Boolean result = FindBestContext(
            killedAttrib->displayWidth, killedAttrib->displayHeight,
            killedAttrib->displayBestDepth, killedAttrib->frequency,
            killedAttrib->pageCount, killedContext, &dontCareAttrib);
      }
    }
  }
  ...
  {  // TTimo - variables and labels playing nasty
     // Immediately set the port to that of the DSp drawing buffer - phs,
     // 10/24/99
     // Ê(I'm pretty sure that not only is this needed, but it's correct.  We
     // want to
...
//------------------------------------------------------------------------------------
// Switches to the preconfigured movie context. If we're already there, does
// nothing. If the movie context is NULL, returns failure. This code assumes
// that we're already looking at the game context, and that the movie context
...
// ¥ SwitchToGameContext
//------------------------------------------------------------------------------------
// Switches to the preconfigured game context. If we're already there, does
// nothing. If the game context is NULL, returns failure. The game context must
// already be reserved.
//------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------
// Switches to the preconfigured alt game context. If we're already there, does
// nothing. If the game context is NULL, returns failure. The alt game context
// must already be reserved. Assumes that the regular game context is also
// reserved. movie context is optional.
//------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::SwitchToResolution(short inWidth,
                                          short inHeight,
                                          short inDepth,
                                          Fixed inFrequency,
                                          short inPageCount,
  ...
  // If we've previously switched resolutions, we should have saved off our
  // context attributes.  Check the parameters sent in against what's been saved
  // to see if we've already got the correct context
  if (mContext && mAttributes.displayWidth == inWidth &&
  ...
  // First, release any existing context.  We can't get ANY context info from a
  // display with an active context (this is a stupid limitation of
  // DrawSprocket)
  OSStatus status;
  if (mContext) {
    status = ::DSpContext_SetState(mContext, kDSpContextState_Inactive);
  ...
  if (!FindBestContext(inWidth, inHeight, inDepth, inFrequency, inPageCount,
    ...
    FadeGammaUp(true);
    char text[64];
    sprintf(text, "[%dx%d, %d bit, %d buffer(s)]", inWidth, inHeight, inDepth,
            inPageCount);
    // LBO - FIXME
...
/*
                        ...
                        ::DSpContext_GetAttributes(mContext,
   &desiredAttributes); if (desiredAttributes.pageCount == 3)
...
*/
    ...
    if ((status == noErr) && (!mUseDefaultContext)) {
      // If we didn't call DSpFindBestContext, and instead iterated through all
      // available contexts to find one that was appropriate, we may have found
      // a context whose width or height is too big.  Therefore, we need to
      // compensate by calling PortSize() & MovePortTo()
      CGrafPtr port = GetDrawingBuffer();
      if (port) {
...
#if MAC_DEVELOPMENT
    // Make sure we didn't get any errors besides kDSpConfirmSwitchWarning -
    // phs, 11/9/99
...
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::GetResolution(short* outWidth,
                                     short* outHeight,
                                     short* outDepth,
                                     Fixed* outFrequency,
                                     short* outPageCount) {
...
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::GetGameResolution(short* outWidth,
                                         short* outHeight,
                                         short* outDepth,
                                         Fixed* outFrequency,
                                         short* outPageCount) {
...
DisplayIDType CDrawSprocket::GetDisplayID(void) {
  //	DisplayIDType displayID;
  //	if (mContext && (::DSpContext_GetDisplayID(mContext, &displayID) ==
  //noErr)) 		return displayID;
...
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::SetCLUTEntries(ColorSpec* inColorSpec,
                                      unsigned short inStartingEntry,
                                      unsigned short inEntryCount) {
  if (mContext) {
    OSErr err = DSpContext_SetCLUTEntries(mContext, inColorSpec,
                                          inStartingEntry, inEntryCount);
    if (err == noErr) {
      return true;
    }
  }
...
//------------------------------------------------------------------------------------
//		Set one CLUT entry of the on-screen draw context, 16-bit RGB
//values
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::SetRGBIndex(UInt16 red,
                                   UInt16 green,
                                   UInt16 blue,
...
//------------------------------------------------------------------------------------
//		Set a range of CLUT entries of the on-screen draw context, 8-bit
//RGB values
//------------------------------------------------------------------------------------
Boolean CDrawSprocket::SetRGBEntries_8(UInt8 const* rgbArray,
                                       unsigned short inStartingEntry,
                                       unsigned short inEntryCount) {
  if (mContext) {
    ColorSpec* colorSpec;
    ...
    // Set the color table
    // LBO - Debugging DSp views inEntryCount as inLastEntry?!? So we subtract
    // one for the 256 case...
    OSErr err = DSpContext_SetCLUTEntries(mContext, colorSpec, inStartingEntry,
                                          inEntryCount - 1);
...
Boolean CDrawSprocket::FadeGammaUp(Boolean inFadeAllScreens) {
  ...
  if (mGammaFadedOut) {
    MacAssert(mContext);
    if (mContext) {
      ::DSpContext_FadeGammaIn(inFadeAllScreens ? kDSpEveryContext : mContext,
                               NULL);
      mGammaFadedOut = false;
      result = true;
    }
...
Boolean CDrawSprocket::FadeGammaDown(Boolean inFadeAllScreens) {
  ...
  if (!mGammaFadedOut) {
    MacAssert(mContext);
    if (mContext) {
      ::DSpContext_FadeGammaOut(inFadeAllScreens ? kDSpEveryContext : mContext,
                                NULL);
      mGammaFadedOut = true;
      alreadyHere = 0;
      result = true;
...
Boolean CDrawSprocket::FadeGammaUpImmediate(Boolean inFadeAllScreens) {
  ...
  if (mGammaFadedOut) {
    if (mContext) {
      MacAssert(mContext);
      ::DSpContext_FadeGamma(inFadeAllScreens ? kDSpEveryContext : mContext,
                             100, NULL);
      mGammaFadedOut = false;
      result = true;
    }
...
Boolean CDrawSprocket::FadeGammaDownImmediate(Boolean inFadeAllScreens) {
  ...
  if (!mGammaFadedOut) {
    if (mContext) {
      MacAssert(mContext);
      ::DSpContext_FadeGamma(inFadeAllScreens ? kDSpEveryContext : mContext, 0,
                             NULL);
      mGammaFadedOut = true;
      result = true;
    }
...
CGrafPtr CDrawSprocket::GetBackBuffer(void) {
  if (mContext && mDoubleBuffered) {
    CGrafPtr port = NULL;
    if (::DSpContext_GetBackBuffer(mContext, kDSpBufferKind_Normal, &port) ==
        noErr) {
      return port;
    }
  }
...
//------------------------------------------------------------------------------------
//		Return the CGrafPtr to draw into.  Normally this is the back
//buffer, but when 		drawing directly to the screen, it will be the front buffer.
//------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------
//		Get the rowBytes (pitch) of the currently active context's back
//buffer
//------------------------------------------------------------------------------------
...
#else
    pitch =
        (*(GetPortPixMap(gDrawSprocket->GetBackBuffer())))->rowBytes & 0x3fff;
#endif
  }
...
//------------------------------------------------------------------------------------
//		Copies a gworld to the current drawing buffer. Makes 2
//assumptions:
//		1. the gworld pixmap is locked, 2. the buffers are identical in
//dimension
//------------------------------------------------------------------------------------
void CDrawSprocket::CopyGWorld(GWorldPtr theGWorld, Rect* blitRect) {
  ...
  CopyBits((BitMapPtr)*pixmap, GetPortBitMapForCopyBits(dstPort), blitRect,
           blitRect, srcCopy, NULL);
  ...
  if (mDoubleBuffered) {
    //		if (fast)
    {
      ...
      CopyBits(GetPortBitMapForCopyBits(srcPort),
               GetPortBitMapForCopyBits(dstPort), &srcBounds, &dstBounds,
               srcCopy, NULL);
