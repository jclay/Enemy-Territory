...
void GetDialogItemTextAppearance(DialogRef inDialog,
...
#if MAC_Q3_MP
// Call this from SV_SpawnServer() in sv_init.c(pp), right after the call to
// SV_InitGameProgs();
...
//instructions to call CFM routines
//
UInt32 sGlueCode[6] = {0x3D800000, 0x618C0000, 0x800C0000,
                       0x804C0004, 0x7C0903A6, 0x4E800420};

void* MachOFunctionPointerForCFMFunctionPointer(void* cfmfp) {
  UInt32* mfp = (UInt32*)NewPtr(
      sizeof(sGlueCode));  //	Must later dispose of allocated memory
...
attempts to treat the copied file as a zip file, unzipping it to the temp
directory. This should only be called when looking for a Mach-O .bundle DLL.
===============
*/
...
Boolean Sys_UnpackDLL(const char* name) {
  ...
  // so that it's not found and used the next time through (which might mark it
  // as unpure).
  Mac_GetOSPath(kUserDomain, kTemporaryFolderType, macFolderPath);
  sprintf(zipPath, "%s%s%s", macFolderPath, kPathSep, kMacGameDir);
  Sys_Mkdir(zipPath);
  sprintf(zipPath, "%s%s%s%s%s", macFolderPath, kPathSep, kMacGameDir, kPathSep,
          name);
  ...
  // LBO - now try and unzip the file
  {
    ...
    for (i = 0; i < zipInfo.number_entry; i++) {
      ...
      err = unzGetCurrentFileInfo(zipFile, &file_info, newFileName,
                                  sizeof(newFileName), NULL, 0, NULL, 0);
      ...
      sprintf(newFilePath, "%s%s%s%s%s", macFolderPath, kPathSep, kMacGameDir,
              kPathSep, newFileName);
...
tryMachO: {
  ...
  sprintf(temp, "%s%s%s%s%s.bundle", macFolderPath, kPathSep, kMacGameDir,
          kPathSep, name2);

  dllSpec->bundleURL =
      CFURLCreateWithBytes(kCFAllocatorDefault, (UInt8*)temp, strlen(temp),
  ...
  dllEntry = (dllEntryPtr)CFBundleGetFunctionPointerForName(dllSpec->bundleRef,
                                                            CFSTR("dllEntry"));
  if (dllEntry == NULL) {
    if (dllSpec->bundleRef) {
      CFRelease(dllSpec->bundleRef);
  ...
  *entryPoint = (vmMainPtr)CFBundleGetFunctionPointerForName(dllSpec->bundleRef,
                                                             CFSTR("vmMain"));
  if (*entryPoint == NULL) {
    if (dllSpec->bundleRef) {
      CFRelease(dllSpec->bundleRef);
...
tryAppPackage:
  ...
  // when we found in the legacy_mp_bin, keep going. We'll make sure the
  // checksum matches the hardcoded value
  ...
  // Look for the bundle inside the app package
  {
    ...
    libString = CFStringCreateWithCString(kCFAllocatorDefault, name2,
                                          kCFStringEncodingUTF8);

    // get a URL to our shared library from the "Resources" directory of the
    // bundle
    dllSpec->bundleURL =
        CFBundleCopyResourceURL(refMainBundle, libString, NULL, NULL);
    if (!dllSpec->bundleURL) {
      err = fnfErr;
      goto cantLoad;
    }
    ...
    if (legacy_mp_bin) {
      ...
      cfpath =
          CFURLCopyFileSystemPath(dllSpec->bundleURL, kCFURLPOSIXPathStyle);
      if (!CFStringGetCString(cfpath, path, sizeof(path),
                              kCFStringEncodingASCII)) {
        Com_Printf("failed to get the OS path to the bundle\n");
        goto cantLoad;
      }
      strncat(path, va("/Contents/MacOS/%s_mac", name),
              sizeof(path) - strlen(path) - 64);
      checksum = FS_ChecksumOSPath(path);
      // Com_Printf( "%s %X\n", path, checksum );
      //  it doesn't hurt to always do the load and checksum, makes the setup
      //  easier we only enforce the check when pureness is required however
      if (FS_IsPure()) {
    ...
    // get the actual bundle for the library
    dllSpec->bundleRef =
        CFBundleCreate(kCFAllocatorDefault, dllSpec->bundleURL);
    if (!dllSpec->bundleRef) {
      if (dllSpec->bundleURL) {
        CFRelease(dllSpec->bundleURL);
      }
    ...
    dllEntry = (dllEntryPtr)CFBundleGetFunctionPointerForName(
        dllSpec->bundleRef, CFSTR("dllEntry"));
    if (dllEntry == NULL) {
    ...
    // Bind "vmMain" so we can call into it later as well
    *entryPoint = (vmMainPtr)CFBundleGetFunctionPointerForName(
        dllSpec->bundleRef, CFSTR("vmMain"));
    if (*entryPoint == NULL) {
...
void* Sys_GetGameAPI(void* parms) {
  ...
  if (AmIBundled()) {
    ...
    // bundle
    refMainBundleURL =
        CFBundleCopyResourceURL(refMainBundle, CFSTR(kMacGameLib), NULL, NULL);
    if (!refMainBundleURL) {
      err = fnfErr;
      goto cantLoad;
    }
    ...
    err = FSGetCatalogInfo(&myBundleRef, kFSCatInfoNone, NULL, NULL, &SLSpec,
                           NULL);
  } else {
    char gameDir2[1024];
  ...
  err = GetDiskFragment(&SLSpec, 0, kCFragGoesToEOF, NULL, kPrivateCFragCopy,
                        &game_connID, (Ptr*)NULL, symName);
...
cantLoad:
  if (err == cfragNoClientMemErr) {
    Sys_Error("Can't load %s. Give the game additional memory and try again.",
              kMacGameLib);
  } else {
...
void* Sys_GetUIAPI(void) {
  ...
  if (AmIBundled()) {
    ...
    // bundle
    refMainBundleURL =
        CFBundleCopyResourceURL(refMainBundle, CFSTR(kMacUILib), NULL, NULL);
    if (!refMainBundleURL) {
      err = fnfErr;
      goto cantLoad;
    }
...
void* Sys_GetCGameAPI(void) {
  ...
  if (AmIBundled()) {
    ...
    // bundle
    refMainBundleURL =
        CFBundleCopyResourceURL(refMainBundle, CFSTR(kMacCGameLib), NULL, NULL);
    if (!refMainBundleURL) {
      err = fnfErr;
      goto cantLoad;
    }
...
char* Sys_GetClipboardData(void) {
  ...
  if ((status == noErr) && (byteCount < 1023)) {
    ...
    status =
        GetScrapFlavorData(currentScrap, 'TEXT', &byteCount, (void*)buffer);
  }
  return buffer;
}
...
void Sys_SetClipboard(const char* contents) {
  ...
  status = PutScrapFlavor(currentScrap, 'TEXT', kScrapFlavorMaskNone, byteCount,
                          (void*)contents);
}
...
void Sys_OpenURL(const char* url, qboolean doexit) {
  ...
  if (err == noErr) {
...
#endif
    {
      ...
      endSel = strlen(url);
      err =
          ICLaunchURL(inst, "\p", (char*)url, strlen(url), &startSel, &endSel);
    }
    ICStop(inst);
  }
...
void Sys_EndProfiling(void) {
  ...
  sprintf((char*)pstring + 1, "%s:profile.txt",
...
void Sys_Error(const char* format, ...) {
  ...
  // If we're using DSp on OSX, we need to pause the context so we can see
  // dialogs
  if (r_fullscreen) {
    GLimp_pause();
  ...
  StandardAlert(kAlertStopAlert, briefMsg, (StringPtr)errString, &param,
                &itemHit);
...
qboolean LoadRegistryInfo(qboolean user,
                          const char* pszName,
                          void* pvBuf,
                          long* plSize) {
  // NOT IMPLMENTED
...
qboolean SaveRegistryInfo(qboolean user,
                          const char* pszName,
                          void* pvBuf,
                          long lSize,
                          qboolean bString) {
...
void SetDialogControlValue(DialogRef inDialog,
                           DialogItemIndex inItem,
                           SInt16 inValue) {
  ControlRef controlRef;
...
//
//	Returns the text of a static/edit text item assuming the Appearance
//Manager.
//===============================================================================
...
static char* GetCommandLineParameters(const AppleEvent* appleEvt,
                                      AEKeyword keyWord) {
  OSErr theError;
  DescType typeCode;
  Size dataSize;
  ...
  if ((theError = AESizeOfParam(appleEvt, keyWord, &typeCode, &dataSize)) ==
          noErr &&
      dataSize > 0) {
    ...
    if ((theError = AEGetParamPtr(appleEvt, keyWord, typeChar, &typeCode,
                                  theCommandLine, dataSize, &actualSize)) !=
            noErr ||
        dataSize != actualSize) {
...
static pascal OSErr RunAppleEventHandler(const AppleEvent* appleEvt,
                                         AppleEvent* reply,
                                         long refcon) {
#pragma unused(reply, refcon)
  // Parameters pass in with an 'orun' event. 'CLin' seems to be the defacto
  // standard, as seen in Unreal Tournament, so we use it too. The normal
  // behavior of 'orun' is to open an empty document, but that doesn't make much
  // sense here!
...
#if MAC_Q3_MP
static pascal OSErr OMExecuteAppleEventHandler(const AppleEvent* appleEvt,
                                               AppleEvent* reply,
                                               long refcon) {
#pragma unused(reply, refcon)
...
static OSErr InitAppleEvents(void) {
  ...
  err = AEInstallEventHandler(kCoreEventClass, kAEOpenApplication,
                              NewAEEventHandlerUPP(RunAppleEventHandler), 0,
                              false);
  if (err != noErr) {
    goto bail;
  }
  ...
  err = AEInstallEventHandler(kCoreEventClass, kAEQuitApplication,
                              NewAEEventHandlerUPP(QuitAppleEventHandler), 0,
                              false);
  if (err != noErr) {
    goto bail;
  }
...
#if MAC_Q3_MP
  err = AEInstallEventHandler(kOMEventClass, kOMExecuteCommands,
                              NewAEEventHandlerUPP(OMExecuteAppleEventHandler),
                              0, false);
  if (err != noErr) {
    goto bail;
  }
...
void InitMacStuff(void) {
  ...
  // We do this early because it relies on grabbing the first apple event from
  // the Finder.
...
Boolean OptionDialog(void) {
  ...
  // Disable the "choose monitor" button if we've only got one to pick from
  {
    ...
    for (currentMonitor = GetDeviceList(); currentMonitor != NULL;
         currentMonitor = GetNextDevice(currentMonitor)) {
      totalMonitors++;
    }
...
char* ReadCommandLineParms(void) {
  ...
  // wait for the first AppleEvent to come through, but time out after 1/2
  // second
  timeoutTicks = TickCount() + 30;
  while (!WaitNextEvent(highLevelEventMask, &event, 0x7fffffff, NULL) &&
         TickCount() < timeoutTicks) {
  };
  ...
  if (GRCheckFileForCmd())
  {
          ...
          if (GRHasProperty( 'Exec' ))
          {
                  ...
                  if ( !buf )
                  {
                          Sys_Error ("Could not launch via GameRanger, %s length
%d", grName, len);
                  }
                  buf[len] = 0;
                  fread( buf, len, 1, f );
...
int main(int argc, const char* argv[]) {
  ...
  switch (macPrefs.monitorFrequency) {
    case kMenuRefreshAuto:
      gRefreshRate = 0;
      break;
    case kMenuRefresh60:
      gRefreshRate = kFrequency60;
      break;
    case kMenuRefresh67:
      gRefreshRate = kFrequency67;
      break;
    case kMenuRefresh70:
      gRefreshRate = kFrequency70;
      break;
    case kMenuRefresh75:
      gRefreshRate = kFrequency75;
      break;
    case kMenuRefresh80:
      gRefreshRate = kFrequency80;
      break;
    case kMenuRefresh85:
      gRefreshRate = kFrequency85;
      break;
    case kMenuRefresh90:
      gRefreshRate = kFrequency90;
      break;
    case kMenuRefresh99:
      gRefreshRate = kFrequency99;
      break;
    case kMenuRefresh100:
      gRefreshRate = kFrequency100;
      break;
    case kMenuRefresh120:
      gRefreshRate = kFrequency120;
      break;
    case kMenuRefresh124:
      gRefreshRate = kFrequency124;
      break;
    default:
      gRefreshRate = 0;
      break;
  ...
  // Write the changes back out now, in case we bail before the app quits
  // properly
  SavePrefs(&macPrefs);
...
#ifndef DEDICATED
  // LBO - 1.11a patch. Use carbon events when running as a dedicated server.
  // This prevents all the hitching warnings when resizing the windows, etc.
