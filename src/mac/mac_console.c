...
OSStatus InitMLTE(void) {
  TXNMacOSPreferredFontDescription
      defaults;  // fontID, pointSize, encoding, and fontStyle
  ...
  options =
      0L;  // kTXNWantMoviesMask | kTXNWantSoundMask | kTXNWantGraphicsMask;
...
OSStatus AttachMLTEToWindow(WindowRef inWindow, TXNObject* outObject) {
  ...
  frameOptions = kTXNWantVScrollBarMask | kTXNNoKeyboardSyncMask |
                 kTXNAlwaysWrapAtViewEdgeMask;

  GetMLTEBoundsFromWindow(inWindow, &textRect);

  status = TXNNewObject(NULL, inWindow, &textRect, frameOptions,
  ...
  if (status == noErr) {
    if (*outObject != NULL) {
      ...
      status = TXNSetTXNObjectControls(
          *outObject, false, sizeof(controlTags) / sizeof(TXNControlTag),
...
void SetMLTECarbonEvents(WindowRef inWindow, TXNObject inTextObject) {
  ...
  CFStringRef keys[] = {
      kTXNTextHandlerKey, kTXNWindowEventHandlerKey, kTXNCommandTargetKey,
      //		kTXNCommandUpdateKey
  };
  EventTargetRef eventTargets[] = {
      GetWindowEventTarget(inWindow), GetWindowEventTarget(inWindow),
  ...
  carbonEventInfo.fDictionary = CFDictionaryCreate(
      kCFAllocatorDefault, (const void**)&keys, (const void**)&eventTargets,
      sizeof(keys) / sizeof(CFStringRef), &kCFCopyStringDictionaryKeyCallBacks,
      NULL);

  controlData[0].uValue = (UInt32)&carbonEventInfo;

  // Tell MLTE to install its Carbon handlers
  status = TXNSetTXNObjectControls(inTextObject, false,
...
OSStatus AttachEditableMLTEToWindow(WindowRef inWindow, TXNObject* outObject) {
  ...
  frameOptions = kTXNSingleLineOnlyMask | kTXNNoKeyboardSyncMask |
                 kTXNAlwaysWrapAtViewEdgeMask;
...
void SetMLTEFontInfo(TXNObject inTextObject,
                     Str255 inFontName,
                     short inFontSize) {
...
void GetEditableMLTEText(TXNObject inTextObject,
                         char* outText,
                         int inBufferSize) {
  ...
  status = TXNGetDataEncoded(inTextObject,     // TXNObject iTXNObject,
                             kTXNStartOffset,  // TXNOffset iStartOffset,
                             kTXNEndOffset,    // TXNOffset iEndOffset,
                             &textHandle, kTXNTextData);
  ...
  // adds a garbage character to the end. We manually truncate it to prevent
  // this.
...
void SetEditableMLTEText(TXNObject inTextObject, char* inText) {
  ...
  status = TXNSetData(inTextObject, kTXNTextData, (void*)inText, strlen(inText),
...
void UpdateMLTEFocus(Boolean forceUpdate) {
  ...
  if ((sMLTEActive == lastActive) && (lastFocus == sCurrentFocus) &&
      !forceUpdate) {
    willDraw = false;
  }
  ...
  // MLTE draws scrollbars differently on 9 and 10. Namely, on 10 it draws in
...
void UpdateConsoleWindow(void) {
  ...
  DrawThemeEditTextFrame(&textRect,
                         sMLTEActive ? kThemeStateActive : kThemeStateInactive);
...
static pascal OSStatus consoleEventHandler(EventHandlerCallRef handlerChain,
                                           EventRef event,
                                           void* userData) {
  UInt32 event_kind;
  UInt32 event_class;
  ...
  if (event_class == kEventClassKeyboard) {
    ...
    (void)GetEventParameter(event, kEventParamKeyMacCharCodes, typeChar, NULL,
    ...
    switch (event_kind) {
      ...
      case kEventRawKeyRepeat: {
        ...
        if (charCode == 0x0d) {
          GetEditableMLTEText(s_wcd.editTextObject, inputBuffer,
                              sizeof(inputBuffer));
          ...
          kg.historyEditLines[kg.nextHistoryLine % COMMAND_HISTORY] =
              kg.g_consoleField;
          kg.nextHistoryLine++;
    ...
    switch (event_kind) {
      ...
      case kEventWindowResizeCompleted:
        ...
        TXNSetFrameBounds(s_wcd.textObject, textRect.top, textRect.left,
                          textRect.bottom, textRect.right, 0);
        GetEditableMLTEBoundsFromWindow(s_wcd.hWnd, &textRect);
        TXNSetFrameBounds(s_wcd.editTextObject, textRect.top, textRect.left,
                          textRect.bottom, textRect.right, 0);
        err = noErr;
        break;
    }
    ...
    if (event_kind == kEventMouseMoved) {
      ...
      (void)GetEventParameter(event, kEventParamMouseLocation, typeQDPoint,
    ...
    (void)GetEventParameter(event, kEventParamDirectObject, typeHICommand, NULL,
                            sizeof(cmd), NULL, &cmd);

    switch (event_kind) {
      case kEventCommandProcess:
        if ((sCurrentFocus == kMLTEStatic) &&
            (cmd.commandID == kHICommandCopy)) {
...
void Sys_CreateConsole(void) {
  ...
  windowBounds.left =
      (((*device)->gdRect.right - (*device)->gdRect.left) - kConsole_Width) / 2;
  windowBounds.top =
      (((*device)->gdRect.bottom - (*device)->gdRect.top) - kConsole_Height) /
      2;
  windowBounds.right = windowBounds.left + kConsole_Width;
  windowBounds.bottom = windowBounds.top + kConsole_Height;

  status = CreateNewWindow(kDocumentWindowClass, kWindowResizableAttribute,
                           &windowBounds, &s_wcd.hWnd);
  if (status != noErr) {
    return;
  }
  ...
  InstallWindowEventHandler(s_wcd.hWnd, NewEventHandlerUPP(consoleEventHandler),
                            sizeof(consoleEventList) / sizeof(EventTypeSpec),
                            consoleEventList, 0, NULL);
  ...
  // MLTE bug: LBO - we set all the MLTE object paramters after both have been
  // created because MLTE on 9 will use the parameters for the first one in the
  // creation of the second if we set them individually above.
...
void Sys_ShowConsole(int visLevel, qboolean quitOnClose) {
  ...
  switch (visLevel) {
    ...
    case 1:
      ...
      //			SendMessage( s_wcd.hwndBuffer, EM_LINESCROLL, 0,
      //0xffff );
      break;
    default:
...
void Sys_Print(const char* text) {
  if (s_wcd.hWnd) {
    ...
    while ((c = *curText) != 0) {
      if (Q_IsColorString(curText)) {
        ...
        if (runLength) {
          TXNSetData(s_wcd.textObject, kTXNTextData, startText, runLength,
        ...
        switch (color) {
#if MAC_MOHAA
            // LBO - our console background text is black, so we print black
            // text in the default console text color.
          case 0:
            SetMLTEFontColor(s_wcd.textObject, &sTextDefaultColor);
            break;
#else
          case 0:
            SetMLTEFontColor(s_wcd.textObject, &sRGBBlack);
            break;
#endif
          case 1:
            SetMLTEFontColor(s_wcd.textObject, &sRGBRed);
            break;
          case 2:
            SetMLTEFontColor(s_wcd.textObject, &sRGBGreen);
            break;
          case 3:
            SetMLTEFontColor(s_wcd.textObject, &sRGBYellow);
            break;
          case 4:
            SetMLTEFontColor(s_wcd.textObject, &sRGBBlue);
            break;
          case 5:
            SetMLTEFontColor(s_wcd.textObject, &sRGBCyan);
            break;
          case 6:
            SetMLTEFontColor(s_wcd.textObject, &sRGBMagenta);
            break;
          case 7:
            SetMLTEFontColor(s_wcd.textObject, &sRGBWhite);
            break;
          default:
            SetMLTEFontColor(s_wcd.textObject, &sTextDefaultColor);
            break;
        ...
        // LBO - weirdness. Sometimes there's a 0x19 after a color change byte
        // pair. MLTE draws it as a box, so we just skip it now.
        if (*curText == 0x19) {
          curText++;
        }
        startText = curText;
    ...
    if (s_wcd.visLevel == 1) {
      ...
      // unfocused MLTE object, the hilight state of the unfocused selection
      // will suddenly render as if it were active. By resetting the focus after
      // such an action, the hilight state does not change.
      UpdateMLTEFocus(false);
  ...
  if (!sConsoleFile) {
    ...
    //		Com_sprintf( buffer, sizeof(buffer), "%s:console.log",
    //basedir->string );
    if (AmIBundled()) {
