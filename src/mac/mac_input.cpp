...
void Sys_InitInput(void) {
  ...
  in_macMouseDivider = Cvar_Get("in_macMouseDivider", "163",
                                CVAR_ARCHIVE);  // FIXME: why this constant
...
static int sMouseButtonLookup[MAX_MOUSE_BUTTONS] = {A_MOUSE1,
                                                    A_MOUSE2,
                                                    A_MOUSE3,
                                                    A_MOUSE4,
                                                    A_MOUSE5,
  ...
  if (in_mouse->integer != 0) {
    ...
    for (i = 0; i < MAX_MOUSE_BUTTONS; i++) {
      ...
      if (isPressed != sLastButton[i]) {
        ...
        // 1.04.1 patch
        if ((q3_button == A_MOUSE1) && (macPrefs.button2fake) &&
            IsPressed(kControlKey)) {
          q3_button = A_MOUSE2;
        }
...
int joyDirection[4] = {A_CURSOR_UP, A_CURSOR_DOWN, A_CURSOR_LEFT,
                       A_CURSOR_RIGHT};
