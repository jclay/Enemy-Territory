...
=================
*/
pascal void NotifyProc(void* contextPtr,
                       OTEventCode code,
                       OTResult result,
                       void* cookie) {
  switch (code) {
...
struct InetAddress
{
                ...
                InetHost		fHost;			// Host address
in net byte order UInt8			fUnused[8];		// Traditional
unused bytes
};
...
void Sys_InitNetworking(void) {
  ...
  inet_services = OTOpenInternetServicesInContext(kDefaultInternetServicesPath,
                                                  0, &err, NULL);
...
qboolean Sys_GetPacket(netadr_t* net_from, msg_t* net_message) {
  ...
  net_from->type = NA_IP;
  // the game wants them in network order, for some reason we're getting them in
  // host order
...
qboolean Sys_IsLANAddress(netadr_t adr) {
  ...
  if (num_interfaces) {
    ...
    for (i = 0; i < num_interfaces; i++) {
      if ((sys_inetInfo[i].fAddress & sys_inetInfo[i].fNetmask) ==
          (ip & sys_inetInfo[i].fNetmask)) {
        return qtrue;
      }
    }
    return qfalse;
