...
class CDrawSprocket {
 private:
  ...
  DisplayIDType
      mDisplayID;  // Try to use this screen ID when switching contexts

  Boolean mGammaFadedOut;
  Boolean mPickedMonitor;  // true if the user has chosen a display already

  short mXOffset;  // Used if we have to center the screen output manually in a
                   // context
  ...
  bool mInWindow;  // True if we're drawing into a window instead of a
                   // fullscreen context
 ...
 public:
  ...
  virtual Boolean IsValidResolution(short inWidth,
                                    short inHeight,
                                    short inDepth,
                                    Fixed inFrequency = 0);

  virtual Boolean FindBestContext(short inWidth,
                                  short inHeight,
                                  short inDepth,
                                  Fixed inFrequency,
                                  short inPageCount,
  ...
  virtual Boolean FindFirstContext(DSpContextReference* outContext,
                                   DSpContextAttributes* outAttributes);
  virtual Boolean FindNextContext(DSpContextReference* outContext,
                                  DSpContextAttributes* outAttributes);
  ...
  virtual Boolean ConfigureMovieContext(short inWidth,
                                        short inHeight,
                                        short inDepth,
                                        Fixed inFrequency,
                                        short inPageCount);
  virtual Boolean ConfigureGameContext(short inWidth,
                                       short inHeight,
                                       short inDepth,
                                       Fixed inFrequency,
                                       short inPageCount);
  virtual Boolean ConfigureAltGameContext(short inWidth,
                                          short inHeight,
                                          short inDepth,
                                          Fixed inFrequency,
                                          short inPageCount);

  virtual Boolean SwitchToContext(DSpContextReference inContext,
  ...
  virtual Boolean SwitchToResolution(short inWidth,
                                     short inHeight,
                                     short inDepth,
                                     Fixed inFrequency,
                                     short inPageCount,
                                     Boolean inDoFade);
  virtual Boolean GetResolution(short* outWidth,
                                short* outHeight,
                                short* outDepth,
                                Fixed* outFrequency,
                                short* outPageCount);
  virtual Boolean GetGameResolution(short* outWidth,
                                    short* outHeight,
                                    short* outDepth,
                                    Fixed* outFrequency,
                                    short* outPageCount);
  ...
  virtual Boolean SetCLUTEntries(ColorSpec* inColorSpec,
                                 unsigned short inStartingEntry,
                                 unsigned short inEntryCount);
  virtual Boolean SetRGBIndex(UInt16 red,
                              UInt16 green,
                              UInt16 blue,
                              unsigned short inIndex);
  virtual Boolean SetRGBEntries_8(UInt8 const* rgbArray,
                                  unsigned short inStartingEntry,
                                  unsigned short inEntryCount);
  ...
  CGrafPtr
  GetDrawingBuffer();  // Returns the buffer to draw into (back normally) - KLC.
  void CopyGWorld(
      GWorldPtr theGWorld,
      Rect* blitRect);  // Copies a gworld to the current drawing buffer
  void SwapBuffers();   // Blits entire screen - KLC.
  void ClearBackBuffer(RGBColor* inColor);  // Sets the back buffer to specified
                                            // color - phs, 8/19/99
