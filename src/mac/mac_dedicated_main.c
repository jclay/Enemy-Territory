...
//  ¥ Sys_UnpackDLL
// ÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑ
//  Tries to find a named DLL in the pk3 hierarchy. If found, it copies it, and
//  then attempts to treat the copied file as a zip file, unzipping it to the
//  temp directory. This should only be called when looking for a Mach-O .bundle
//  DLL.
...
Boolean Sys_UnpackDLL(const char* name) {
  ...
  // We create our temp copy of the DLL zip file outside the Quake3 folder
  // hierarchy so that it's not found and used the next time through (which
  // might mark it as unpure).
...
tryAppPackage:
  ...
  // Look for the bundle inside the app package
  {
    ...
    bundleURL =
        CFURLCreateFromFileSystemRepresentation(NULL, path, strlen(path), true);
...
void* Sys_GetGameAPI(void* parms) {
  ...
  // get a URL to our shared library from the "Resources" directory of the
  // bundle
  refMainBundleURL = CFBundleCopyResourceURL(
      refMainBundle, CFSTR(kMacGameLib ".bundle"), NULL, NULL);
  if (!refMainBundleURL) {
    err = fnfErr;
    goto cantLoad;
  }
  ...
  GetGameAPI = (getGameAPIPtr)CFBundleGetFunctionPointerForName(
      gameBundleRef, CFSTR("GetGameAPI"));
  if (GetGameAPI == NULL) {
    goto cantLoad;
  }
...
int main(int argc, char** argv) {
  ...
  *cmdline = 0;
  ...
  while (1) {
    ...
    //		if ( g_wv.isMinimized || ( com_dedicated &&
    //com_dedicated->integer ) ) { ¥¥¥			Sleep( 5 );
    //		}
