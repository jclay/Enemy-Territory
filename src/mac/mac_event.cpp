...
#else
int vkeyToQuakeKey[256] = {
    //			0 or 8		1 or 9			2 or A
    //3 or B			4 or C			5 or D
    //6 or E			7 or F
    /*0x00*/ A_CAP_A,
    A_CAP_S,
    A_CAP_D,
...
Boolean GenerateCharEvent(int inModifiers, int inKeyCode) {
  // This routine returns true if the modifier/key combo should generate an
  // additional SE_CHAR event for the Q3 core. We use it to block combos of keys
  // that would otherwise print an extraneous character, like Cmd-V (it would
  // print a 'v').
...
void Mac_GenerateKeyEvent(int inTime,
                          int inModifiers,
                          int inCharCode,
                          int inKeyCode,
                          Boolean inIsKeyDown) {
...
void DoKeyDown(EventRecord* event) {
  ...
  Mac_GenerateKeyEvent(Sys_MsecForMacEvent(), event->modifiers, charCode,
                       keyCode, true);
}
...
static void Sys_ModifierEvents(int modifiers) {
...
#if !MAC_Q3_OLDSTUFF
  if ((cl_paused->integer == 0 && prevPause == 1) ||
      (fullscreen == qfalse && prevFullscreen == qtrue)) {
    changed = modifiers;
  } else
#endif
  {
  ...
  for (i = 0; keys[i].bit != -1; i++) {
    ...
    if (changed & keys[i].bit) {
      Sys_QueEvent(Sys_MsecForMacEvent(), SE_KEY, keys[i].keyCode,
                   !!(modifiers & keys[i].bit), 0, NULL);
    }
  }
...
void Sys_SendKeyEvents(void) {
  ...
  {
    // MoviesTask can be expensive. Only call Update() every 20 ticks at most. -
    // jb, 6/29/00
...
static pascal OSStatus appEventHandler(EventHandlerCallRef handlerChain,
                                       EventRef event,
                                       void* userData) {
  UInt32 event_kind;
  UInt32 event_class;
  ...
  //	(void)GetEventParameter (event, kEventParamWindowRef, typeWindowRef,
  //NULL, sizeof (WindowRef), NULL, &eventWindow);
  ...
  if (event_class == kEventClassKeyboard) {
    ...
    // Therefore, if the console window is in front, pass any keyboard events
    // on.
    ...
    (void)GetEventParameter(event, kEventParamKeyCode, typeUInt32, NULL,
                            sizeof(keyCode), NULL, &keyCode);
    (void)GetEventParameter(event, kEventParamKeyModifiers, typeUInt32, NULL,
    ...
    switch (event_kind) {
      ...
      case kEventRawKeyRepeat:
        Mac_GenerateKeyEvent(eventTimeInMsecs, modifiers, charCode, keyCode,
                             true);
...
static pascal OSStatus serviceEventHandler(EventHandlerCallRef handlerChain,
                                           EventRef inEvent,
                                           void* userData) {
  OSStatus err = eventNotHandledErr;
  ...
  GetEventParameter(inEvent, kEventParamServiceMessageName, typeCFStringRef,
                    NULL, sizeof(CFStringRef), NULL, &message);
  GetEventParameter(inEvent, kEventParamScrapRef, typeScrapRef, NULL,
                    sizeof(ScrapRef), NULL, &specificScrap);

  status = GetScrapFlavorSize(specificScrap, 'TEXT', &byteCount);
  if (status == noErr) {
    // Allocate room for the string plus the trailing NULL and set it all to
    // zero
...
void Carbon_InstallTimer(void) {
  ...
  InstallEventLoopTimer(GetCurrentEventLoop(), NULL,
                        kEventDurationMillisecond * 1, sTimerUPP, NULL,
                        &sTimerRef);
}
...
void Carbon_InstallServiceEvents(void) {
  ...
  InstallApplicationEventHandler(NewEventHandlerUPP(serviceEventHandler), 1,
                                 serviceEventList, 0, NULL);
}
