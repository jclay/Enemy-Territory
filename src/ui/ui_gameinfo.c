...
static void UI_LoadArenasFromFile(char* filename) {
  /*	int				len;
     ...
     allowed is %i", filename, len, MAX_ARENAS_TEXT ) ); trap_FS_FCloseFile( f
     ); return;
          }
  ...
  while (trap_PC_ReadToken(handle, &token)) {
    if (*token.string == '}') {
      ...
      /*} else if( !Q_stricmp( token.string, "objectives" ) ) {
              if( !PC_String_Parse( handle,
         &uiInfo.mapList[uiInfo.mapCount].objectives ) ) { trap_Print( va(
...
void UI_LoadArenas(void) {
  ...
  */
  ...
  // reeally should be using PC_ parsing stuff for this too eek, no ;
  // either.....
  ...
  /*		str = Info_ValueForKey( ui_arenaInfos[n], "Timelimit" );
     ...
     ); else uiInfo.mapList[uiInfo.mapCount].Timelimit = 0;

                  // set axis respawn time
                  str = Info_ValueForKey( ui_arenaInfos[n], "AxisRespawnTime" );
                  if ( *str )
                          uiInfo.mapList[uiInfo.mapCount].AxisRespawnTime =
     atoi( str ); else uiInfo.mapList[uiInfo.mapCount].AxisRespawnTime = 0;

                  // set allied respawn time
                  str = Info_ValueForKey( ui_arenaInfos[n], "AlliedRespawnTime"
     ); if ( *str ) uiInfo.mapList[uiInfo.mapCount].AlliedRespawnTime = atoi(
     str ); else uiInfo.mapList[uiInfo.mapCount].AlliedRespawnTime = 0;
                  // -NERVE - SMF
...
!trap_PC_ReadToken( handle, &token ) ) {
                        ...
                        if( *token.string != '{' ) {
                                ...
                                trap_Print( va( S_COLOR_RED "unexpected token
'%s' inside: %s\n", token.string, filename ) ); trap_PC_FreeSource( handle );
                                return;
                        }
...
GT_WOLF_LMS);
                        ...
                        if( !PC_String_Parse( handle,
&campaignList[*campaignCount].maps ) ) { trap_Print( va( S_COLOR_RED "unexpected
end of file inside: %s\n", filename ) ); trap_PC_FreeSource( handle ); return;
                        }
                }
        }
...
static void UI_LoadCampaignsFromFile(const char* filename) {
  ...
  while (trap_PC_ReadToken(handle, &token)) {
    if (*token.string == '}') {
      ...
      uiInfo.campaignList[uiInfo.campaignCount].mapTC[1][0] = token.floatvalue +
      ...
      uiInfo.campaignList[uiInfo.campaignCount].mapTC[1][1] = token.floatvalue +
...
void UI_LoadCampaigns(void) {
  ...
  for (i = 0; i < numdirs && uiInfo.campaignCount < MAX_CAMPAIGNS;
    ...
    // &uiInfo.campaignCount, MAX_CAMPAIGNS ); UI_LinkCampaignsToArenas();
  }
  //	trap_DPrint( va( "%i campaigns parsed\n", ui_numCampaigns ) ); // JPW
  //NERVE pulled per atvi req
  if (UI_OutOfMemory()) {
    trap_Print(S_COLOR_YELLOW
               "WARNING: not anough memory in pool to load all campaigns\n");
  }
  ...
  // now use the initial nextCampaignShortName to find the next one, etc etc for
  // single player campaigns rain - don't let i go above the maximum number of
  // campaigns
...
static void UI_LoadBotsFromFile(char* filename) {
  ...
  ui_numBots += UI_ParseInfos(buf, MAX_BOTS - ui_numBots,
                              &ui_botInfos[ui_numBots], MAX_BOTS);
}
