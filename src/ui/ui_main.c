...
static const char* UI_SelectedMap(qboolean singlePlayer,
                                  int index,
                                  int* actual);
...
static playerType_t playerTypes[] = {
    ...
    {"player_window_engineer",
     PT_KNIFE | PT_PISTOL | PT_LIGHTONLY | PT_GRENADES},
    {"player_window_lieutenant",
     PT_KNIFE | PT_PISTOL | PT_RIFLE | PT_EXPLOSIVES},
...
"mp_weapon",	4,	"ui/assets/weapon_panzerfaust.tga",
"stand_panzer",			"stand_panzer"			}, { WP_FLAMETHROWER,		"Flamethrower",				"mp_weapon",	6,	"ui/assets/weapon_flamethrower.tga",	"stand_machinegun",		"stand_machinegun"		},
...
"firing_pistolB_1",		"stand_pistolB"			},
        ...
        { WP_KAR98,				"Kar98", "mp_weapon",	2,
"ui/assets/weapon_kar98.tga",			"stand_rifle",
"stand_rifle"			}, { WP_CARBINE,			"M1
Garand",                "mp_weapon",	2,	"ui/assets/weapon_carbine.tga",
"stand_rifle",			"stand_rifle"			},

        { WP_FG42,				"FG42",
"mp_weapon",	7,	"ui/assets/weapon_fg42.tga",
"stand_rifle",			"stand_rifle"			}, { WP_GARAND,
"M1 Garand",				"mp_weapon",	8,
"ui/assets/weapon_carbine.tga",			"stand_rifle",
"stand_rifle"			}, { WP_MOBILE_MG42,		"Mobile MG42",
"mp_weapon",	9,	"ui/assets/weapon_mg42.tga",
"stand_rifle",			"stand_rifle"			},
...
"firing_pistolB_1",		"stand_pistolB"			},
        ...
        { WP_K43,				"K43",
"mp_weapon",	8,	"ui/assets/weapon_kar98.tga",
"stand_rifle",			"stand_rifle"			},
...
int Text_Height_Ext(const char* text,
                    float scale,
                    int limit,
                    fontInfo_t* font) {
  int len, count;
  ...
  if (text) {
    ...
    while (s && *s && count < len) {
      if (Q_IsColorString(s)) {
        ...
        glyph = &font->glyphs[(
            unsigned char)*s];  // NERVE - SMF - this needs to be an unsigned
                                // cast for localization
        if (max < glyph->height) {
          max = glyph->height;
        }
        s++;
    ...
    while (s && *s && count < len) {
      if (Q_IsColorString(s)) {
        ...
        if (*s == '\n') {
          if (!totalheight) {
            totalheight +=
                5;  // 5 is the vertical spacing that autowrap painting uses
          }
          totalheight += max;
          max = 0;
        } else {
...
void Text_PaintCharExt(float x,
                       float y,
                       float w,
                       float h,
                       float scalex,
    ...
    while (s && *s && count < len) {
      ...
      if (Q_IsColorString(s)) {
        ...
        if (style == ITEM_TEXTSTYLE_SHADOWED ||
          ...
          Text_PaintCharExt(x + (glyph->pitch * scalex) + ofs, y - yadj + ofs,
...
char* Text_AutoWrap_Paint_Chunk(float x,
                                float y,
                                int width,
                                float scale,
                                vec4_t color,
  ...
  if (text) {
    ...
    while (s && *s && count < len) {
      ...
      glyph = &font->glyphs[index];  // NERVE - SMF - this needs to be an
                                     // unsigned cast for localization
      if (Q_IsColorString(s)) {
        if (*(s + 1) == COLOR_NULL) {
          memcpy(&newColor[0], &color[0], sizeof(vec4_t));
        } else {
        ...
        if (x + (glyph->xSkip * useScale) + adjust > wrap_x) {
          if (wrap_point) {
            return wrap_point +
                   1;  // the next char to be printed after line wrap
          }
...
// parameters
int Count_Text_AutoWrap_Paint(float x,
                              float y,
                              int width,
                              float scale,
                              vec4_t color,
...
void Text_AutoWrap_Paint(float x,
                         float y,
                         int width,
                         int height,
                         float scale,
...
void Text_PaintWithCursor(float x,
                          float y,
                          float scale,
                          vec4_t color,
                          const char* text,
  ...
  if (text) {
    ...
    while (s && *s && count < len) {
      glyph = &font->glyphs[(
          unsigned char)*s];  // NERVE - SMF - this needs to be an unsigned cast
                              // for localization
      ...
      /*if ( Q_IsColorString( s ) ) {
              memcpy( newColor, g_color_table[ColorIndex(*(s+1))], sizeof(
      newColor ) ); newColor[3] = color[3]; trap_R_SetColor( newColor ); s += 2;
              continue;
      } else*/
      {
        ...
        // uiDC.Assets.textFont.glyphs[text[i]].imageWidth, scale *
...
static void Text_Paint_Limit(float* maxX,
                             float x,
                             float y,
                             float scale,
                             vec4_t color,
...
void UI_LoadMenus(const char* menuFile, qboolean reset) {
  ...
  if (!handle) {
    ...
    if (!handle) {
      trap_Error(S_COLOR_RED
                 "default menu file not found: ui_mp/menus.txt, unable to "
                 "continue!\n");
    }
  }
...
static void UI_DrawHandicap(rectDef_t* rect,
                            float scale,
                            vec4_t color,
                            int textStyle) {
  int i, h;
...
static void UI_DrawClanName(rectDef_t* rect,
                            float scale,
                            vec4_t color,
                            int textStyle) {
...
static void UI_DrawGameType(rectDef_t* rect,
                            float scale,
                            vec4_t color,
                            int textStyle) {
  Text_Paint(rect->x, rect->y, scale, color,
...
/*static void UI_DrawNetGameType(rectDef_t *rect, float scale, vec4_t color, int
textStyle) { if (ui_netGameType.integer < 0 || ui_netGameType.integer >
uiInfo.numGameTypes) { trap_Cvar_Set("ui_netGameType", "0");
                trap_Cvar_Set("ui_actualNetGameType", "0");
        }
  Text_Paint(rect->x, rect->y, scale, color,
uiInfo.gameTypes[ui_netGameType.integer].gameType , 0, 0, textStyle);
}*/

/*static void UI_DrawJoinGameType(rectDef_t *rect, float scale, vec4_t color,
int textStyle) { if (ui_joinGameType.integer < 0 || ui_joinGameType.integer >
uiInfo.numJoinGameTypes) { trap_Cvar_Set("ui_joinGameType", "0");
        }
  Text_Paint(rect->x, rect->y, scale, color, trap_TranslateString(
uiInfo.joinGameTypes[ui_joinGameType.integer].gameType ), 0, 0, textStyle);
}*/
...
/*static void UI_DrawSkill(rectDef_t *rect, float scale, vec4_t color, int
textStyle) { int i; i = trap_Cvar_VariableValue( "g_spSkill" ); if (i < 1 || i >
numSkillLevels) { i = 1;
  }
  Text_Paint(rect->x, rect->y, scale, color, skillLevels[i-1],0, 0, textStyle);
}*/

static void UI_DrawTeamName(rectDef_t* rect,
                            float scale,
                            vec4_t color,
                            qboolean blue,
                            int textStyle) {
...
static void UI_DrawTeamMember(rectDef_t* rect,
                              float scale,
                              vec4_t color,
                              qboolean blue,
                              int num,
...
void UI_DrawMapPreview(rectDef_t* rect,
                       float scale,
                       vec4_t color,
                       qboolean net) {
  ...
  if (uiInfo.mapList[map].levelShot > 0) {
          UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h,
  uiInfo.mapList[map].levelShot); } else { UI_DrawHandlePic( rect->x, rect->y,
  rect->w, rect->h, trap_R_RegisterShaderNoMip("levelshots/unknownmap"));
  }*/
}
...
static void UI_DrawMapCinematic(rectDef_t* rect,
                                float scale,
                                vec4_t color,
                                qboolean net) {
  int map = (net) ? ui_currentNetMap.integer : ui_currentMap.integer;
  ...
  if (game == GT_WOLF_CAMPAIGN) {
    ...
    /*		if( uiInfo.campaignList[map].campaignCinematic >= -1 ) {
       ...
       uiInfo.campaignList[map].campaignShortName), 0, 0, 0, 0, (CIN_loop |
       CIN_silent) );
                            }
                            if( uiInfo.campaignList[map].campaignCinematic >= 0
       ) { trap_CIN_RunCinematic( uiInfo.campaignList[map].campaignCinematic );
                                    trap_CIN_SetExtents(
       uiInfo.campaignList[map].campaignCinematic, rect->x, rect->y, rect->w,
       rect->h ); trap_CIN_DrawCinematic(
...
static void UI_DrawCampaignPreview(rectDef_t* rect,
                                   float scale,
                                   vec4_t color,
                                   qboolean net) {
  int campaign =
...
static void UI_DrawCampaignCinematic(rectDef_t* rect,
                                     float scale,
                                     vec4_t color,
                                     qboolean net) {
  int campaign =
...
static void UI_DrawCampaignName(rectDef_t* rect,
                                float scale,
                                vec4_t color,
                                int textStyle,
                                qboolean net) {
...
void UI_DrawCampaignDescription(rectDef_t* rect,
                                float scale,
                                vec4_t color,
                                float text_x,
                                float text_y,
...
void UI_DrawGametypeDescription(rectDef_t* rect,
                                float scale,
                                vec4_t color,
                                float text_x,
                                float text_y,
...
static void UI_DrawCampaignMapDescription(rectDef_t* rect,
                                          float scale,
                                          vec4_t color,
                                          float text_x,
                                          float text_y,
...
static void UI_DrawCampaignMapPreview(rectDef_t* rect,
                                      float scale,
                                      vec4_t color,
                                      qboolean net,
                                      int map) {
...
static void UI_DrawMissionBriefingTitle(rectDef_t* rect,
                                        float scale,
                                        vec4_t color,
                                        int textStyle) {
  char buffer[64];
...
static void UI_DrawMissionBriefingObjectives(rectDef_t* rect,
                                             float scale,
                                             vec4_t color,
                                             float text_x,
                                             float text_y,
...
static void UI_DrawPlayerModel(rectDef_t* rect) {
  ...
  if (updateModel) {  // NERVE - SMF - TEMPORARY
    ...
    UI_PlayerInfo_SetInfo(&info, LEGS_IDLE, TORSO_STAND, viewangles, moveangles,
                          -1, qfalse);
...
textStyle) { if (ui_netSource.integer < 0 || ui_netSource.integer >
numNetSources ) { ui_netSource.integer = 0;
        }
        Text_Paint(rect->x, rect->y, scale, color, trap_TranslateString(
va("Source: %s", netSources[ui_netSource.integer]) ), 0, 0, textStyle);
}*/
...
/*static void UI_DrawNetMapPreview(rectDef_t *rect, float scale, vec4_t color) {
        ...
        if (uiInfo.serverStatus.currentServerPreview > 0) {
                UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h,
uiInfo.serverStatus.currentServerPreview); } else { UI_DrawHandlePic( rect->x,
rect->y, rect->w, rect->h, trap_R_RegisterShaderNoMip("levelshots/unknownmap"));
        }
}*/

/*static void UI_DrawNetMapCinematic(rectDef_t *rect, float scale, vec4_t color)
{ if (ui_currentNetMap.integer < 0 || ui_currentNetMap.integer >
uiInfo.mapCount) { ui_currentNetMap.integer = 0;
                trap_Cvar_Set("ui_currentNetMap", "0");
        }
...
static void UI_DrawNetFilter(rectDef_t* rect,
                             float scale,
                             vec4_t color,
                             int textStyle) {
...
/*static void UI_DrawTier(rectDef_t *rect, float scale, vec4_t color, int
textStyle) { int i; i = trap_Cvar_VariableValue( "ui_currentTier" ); if (i < 0
|| i >= uiInfo.tierCount) { i = 0;
  }
  Text_Paint(rect->x, rect->y, scale, color, va("Tier: %s",
uiInfo.tierList[i].tierName),0, 0, textStyle);
}
...
static void UI_DrawTierMapName(rectDef_t *rect, float scale, vec4_t color, int
textStyle) { int i, j; i = trap_Cvar_VariableValue( "ui_currentTier" ); if (i <
0 || i >= uiInfo.tierCount) { i = 0;
  }
...
static void UI_DrawTierGameType(rectDef_t *rect, float scale, vec4_t color, int
textStyle) { int i, j; i = trap_Cvar_VariableValue( "ui_currentTier" ); if (i <
0 || i >= uiInfo.tierCount) { i = 0;
  }
        j = trap_Cvar_VariableValue("ui_currentMap");
  ...
  Text_Paint(rect->x, rect->y, scale, color,
uiInfo.gameTypes[uiInfo.tierList[i].gameTypes[j]].gameType , 0, 0, textStyle);
}*/
...
vec3_origin, WP_MP40, qfalse ); UI_RegisterClientModelname( &info2, model);
...
static void UI_DrawAllMapsSelection(rectDef_t* rect,
                                    float scale,
                                    vec4_t color,
                                    int textStyle,
                                    qboolean net) {}

static void UI_DrawOpponentName(rectDef_t* rect,
                                float scale,
                                vec4_t color,
                                int textStyle) {
  Text_Paint(rect->x, rect->y, scale, color,
  ...
  switch (ownerDraw) {
    ...
    case UI_REDTEAM5:
      ...
      /*		case UI_NETSOURCE:
         ...
         ui_netSource.integer > uiInfo.numJoinGameTypes) { ui_netSource.integer
         = 0;
                              }
                              s = va("Source: %s",
         netSources[ui_netSource.integer]); break;*/
    case UI_NETFILTER:
      if (ui_serverFilterType.integer < 0 ||
          ui_serverFilterType.integer > numServerFilters) {
        ui_serverFilterType.integer = 0;
...
static void UI_DrawBotName(rectDef_t* rect,
                           float scale,
                           vec4_t color,
                           int textStyle) {}
...
static void UI_DrawRedBlue(rectDef_t* rect,
                           float scale,
                           vec4_t color,
                           int textStyle) {
  Text_Paint(rect->x, rect->y, scale, color,
...
static void UI_BuildPlayerList() {
  ...
  for (n = 0; n < count; n++) {
    ...
    if (info[0]) {
      ...
      uiInfo.playerRefereeStatus[uiInfo.playerCount] =
          atoi(Info_ValueForKey(info, "ref"));
      uiInfo.playerCount++;
...
static void UI_DrawSelectedPlayer(rectDef_t* rect,
                                  float scale,
                                  vec4_t color,
                                  int textStyle) {
...
static void UI_DrawServerRefreshDate(rectDef_t* rect,
                                     float scale,
                                     vec4_t color,
                                     int textStyle) {
  ...
  if (uiInfo.serverStatus.refreshActive) {
    ...
    if (serverCount >= 0) {
      Text_Paint(rect->x, rect->y, scale, newColor,
                 va(trap_TranslateString(
                        "Getting info for %d servers (ESC to cancel)"),
                    serverCount),
                 0, 0, textStyle);
    } else {
      Text_Paint(
          rect->x, rect->y, scale, newColor,
          trap_TranslateString("Waiting for response from Master Server"), 0, 0,
          textStyle);
    }
  } else {
    char buff[64];
...
static void UI_DrawKeyBindStatus(rectDef_t* rect,
                                 float scale,
                                 vec4_t color,
                                 int textStyle,
                                 float text_x,
...
// FIXME: table drive
//
static void UI_OwnerDraw(float x,
                         float y,
                         float w,
                         float h,
                         float text_x,
  ...
  switch (ownerDraw) {
    ...
    case UI_GAMETYPE:
      ...
      break;
      /*		case UI_NETGAMETYPE:
                              UI_DrawNetGameType(&rect, scale, color,
         textStyle); break;*/
      /*		case UI_JOINGAMETYPE:
                              UI_DrawJoinGameType(&rect, scale, color,
         textStyle); break;*/
    case UI_MAPPREVIEW:
    ...
    case UI_NETMAPPREVIEW:
      ...
      /*		case UI_MAP_TIMETOBEAT:
                              UI_DrawMapTimeToBeat(&rect, scale, color,
         textStyle); break;*/
    ...
    case UI_OPPONENTLOGO_NAME:
      ...
      break;
      /*		case UI_TIER_MAPNAME:
                              UI_DrawTierMapName(&rect, scale, color,
         textStyle); break; case UI_TIER_GAMETYPE: UI_DrawTierGameType(&rect,
         scale, color, textStyle); break;*/
    case UI_ALLMAPS_SELECTION:
...
static qboolean UI_GameType_HandleKey(int flags,
                                      float* special,
                                      int key,
                                      qboolean resetMap) {
  if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER ||
...
ui_joinGameType.integer)); UI_BuildServerDisplayList(qtrue); return qtrue;
        }
        return qfalse;
}*/
...
static qboolean UI_TeamName_HandleKey(int flags,
                                      float* special,
                                      int key,
                                      qboolean blue) {
  if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER ||
...
static qboolean UI_TeamMember_HandleKey(int flags,
                                        float* special,
                                        int key,
                                        qboolean blue,
                                        int num) {
...
static qboolean UI_SelectedPlayer_HandleKey(int flags,
                                            float* special,
                                            int key) {
  if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER ||
      key == K_KP_ENTER) {
...
static qboolean UI_OwnerDrawHandleKey(int ownerDraw,
                                      int flags,
                                      float* special,
                                      int key) {
  switch (ownerDraw) {
    ...
    if (dirptr[0] && Q_stricmp(dirptr, ".") && Q_stricmp(dirptr, "..")) {
      ...
      /*if( uiInfo.profileCount == 1 ) {
              ...
              for( j = 0; j < Menu_Count(); j++ ) {
                      Menu_SetFeederSelection( Menu_Get(j), FEEDER_PROFILES, 0,
      NULL );
              }
      }*/
  ...
  if (uiInfo.savegameCount) {
    ...
    for (i = 0; i < uiInfo.savegameCount; i++) {
      ...
      if (!Q_strncmp(sgname, "current",
                     7)) {  // ignore current.svg since it has special uses and
...
static void UI_LoadDemos() {
  ...
  uiInfo.demoCount =
      trap_FS_GetFileList("demos", demoExt, demolist, sizeof(demolist));
...
void UI_RunMenuScript(char** args) {
  ...
  if (String_Parse(args, &name)) {
    if (Q_stricmp(name, "StartServer") == 0) {
      ...
      Menu_SetFeederSelection(NULL, FEEDER_ALLMAPS, ui_currentNetMap.integer,
                              NULL);
      ...
      UI_StartServerRefresh(
          uiInfo.serverStatus.numDisplayServers
              ? qfalse
              : qtrue);  // if we don't have any valid servers, it's kinda safe
                         // to assume we would like to get a full new list
      UI_BuildServerDisplayList(qtrue);
      ...
      if (trap_Cvar_VariableValue("ui_serverBrowser") == 1) {
        ...
        if (strlen(s) && strcmp(s, "localhost")) {
          ...
          uiInfo.serverStatus.numDisplayServers =
              1;  // this is ugly, have to force a non zero display server count
                  // to emit the query
          UI_BuildServerStatus(qtrue);
        } else {
          // we can't close the menu from here, it's not open yet .. (that's the
          // onOpen script)
      ...
      // query server and display the URL buttons if the error happened during a
      // server connection situation
      s = UI_Cvar_VariableString("com_errorDiagnoseIP");
      ...
      // if we are running a local server, make sure we kill it cleanly for
      // other clients
      ...
      trap_Cmd_ExecuteText(
          EXEC_APPEND, va("callvote warmupdamage %d\n",
      ...
      trap_Cmd_ExecuteText(
          EXEC_APPEND, va("ref warmupdamage %d\n",
                          (int)trap_Cvar_VariableValue("ui_voteWarmupDamage")));
      ...
      sizeof(cl_profile.string) ); Q_CleanStr( cl_profile.string );
      Q_CleanDirName( cl_profile.string );

      trap_Cvar_Set( "cl_profile", cl_profile.string );
      if( trap_FS_FOpenFile( va( "profiles/%s/profile.dat", cl_profile.string ),
      &f, FS_WRITE ) >= 0 ) { trap_FS_Write( va( "\"%s\"", ui_profile.string ),
      strlen(ui_profile.string) + 2, f ); trap_FS_FCloseFile( f );
      }*/
      ...
      if (Q_stricmp(buff, cl_profile.string)) {
        if (!Q_stricmp(buff, cl_defaultProfile.string)) {
          // if deleting the default profile, set the default to the current
          // active profile
          fileHandle_t f;
      ...
      // this is the only one that effectively triggers the URL, after the
      // disclaimers are done with we use ui_finalURL as an auxiliary variable
      // to gather URLs from various sources
      ...
      // TODO: if dumped because of cl_allowdownload problem, toggle on first
      // (we don't have appropriate support for this yet)
    ...
    // ydnar
      ...
      int ui_r_allowextensions =
          trap_Cvar_VariableValue("ui_r_allowextensions");
      ...
      int ui_profile_mousePitch =
          trap_Cvar_VariableValue("ui_profile_mousePitch");
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ping > /*=*/0 || ui_netSource.integer == AS_FAVORITES) {
      ...
      serverFilters[ui_serverFilterType.integer].basedir) != 0) {
                      trap_LAN_MarkServerVisible(ui_netSource.integer, i,
      qfalse); continue;
              }
      }*/
...
static void UI_BuildServerStatus(qboolean force) {
  ...
  if (!force) {
    ...
    // the URL buttons are used in the two menus, serverinfo_popmenu and
    // error_popmenu_diagnose
    menu = Menus_FindByName("serverinfo_popmenu");
    if (menu) {
...
const char* UI_FeederItemText(float feederID,
                              int index,
                              int column,
                              qhandle_t* handles,
                              int* numhandles) {
  ...
  *numhandles = 0;
  if (feederID == FEEDER_HEADS) {
    ...
    if (index == 0) {
      ...
      return va("Pixelformat: color(%d-bits) Z(%d-bits) stencil(%d-bits)",
...
void UI_FeederSelection(float feederID, int index) {
  ...
  if (feederID == FEEDER_HEADS) {
    ...
    if (feederID == FEEDER_ALLMAPS && game != GT_WOLF_CAMPAIGN) {
      ...
      /*			trap_Cvar_Set( "ui_userTimelimit", va( "%d",
         uiInfo.mapList[ui_currentMap.integer].Timelimit ) ); trap_Cvar_Set(
         "ui_userAxisRespawnTime", va( "%d",
         uiInfo.mapList[ui_currentMap.integer].AxisRespawnTime ) );
                              trap_Cvar_Set( "ui_userAlliedRespawnTime", va(
         "%d", uiInfo.mapList[ui_currentMap.integer].AlliedRespawnTime ) );*/
    }
    // -NERVE - SMF
    ...
    if (feederID == FEEDER_MAPS) {
      ...
      //trap_CIN_PlayCinematic(va("%s.roq",
      //uiInfo.mapList[ui_currentMap.integer].mapLoadName), 0, 0, 0, 0,
      //(CIN_loop | CIN_silent) );
      //			UI_LoadBestScores(uiInfo.mapList[ui_currentMap.integer].mapLoadName,
      //uiInfo.gameTypes[ui_gameType.integer].gtEnum);
      //			trap_Cvar_Set("ui_opponentModel",
      //uiInfo.mapList[ui_currentMap.integer].opponentName); 			updateOpponentModel
      //= qtrue;
    } else {
      ...
      //trap_CIN_PlayCinematic(va("%s.roq",
      //uiInfo.mapList[ui_currentNetMap.integer].mapLoadName), 0, 0, 0, 0,
      //(CIN_loop | CIN_silent) );
    }
  } else if (feederID == FEEDER_CAMPAIGNS || feederID == FEEDER_ALLCAMPAIGNS) {
    ...
    if (feederID == FEEDER_CAMPAIGNS) {
      ...
      //= trap_CIN_PlayCinematic(va("%s.roq",
      //uiInfo.campaignList[ui_currentCampaign.integer].campaignShortName), 0,
      ...
      /*			if
         (uiInfo.campaignList[ui_currentCampaign.integer].campaignCinematic < 0)
         { uiInfo.campaignList[ui_currentCampaign.integer].campaignCinematic =
         -2;
                              }*/
    ...
    /*		if( uiInfo.serverStatus.currentServerCinematic >= 0 ) {
       ...
       trap_CIN_PlayCinematic(va("%s.roq", mapName), 0, 0, 0, 0, (CIN_loop |
       CIN_silent) );
                    }*/
  } else if (feederID == FEEDER_SERVERSTATUS) {
    //
...
extern void Item_ListBox_MouseEnter(itemDef_t* item,
                                    float x,
                                    float y,
                                    qboolean click);
qboolean UI_FeederSelectionClick(itemDef_t* item) {
...
= NULL; if (!String_Parse(p, &uiInfo.teamList[uiInfo.teamCount].teamMembers[i]))
{ return qfalse;
                                }
                        }
...
static qboolean Character_Parse(char **p) {
  ...
  while ( 1 ) {
        ...
        if (token[0] == '{') {
          ...
          if (!String_Parse(p,
&uiInfo.characterList[uiInfo.characterCount].name) || !String_Parse(p,
&tempStr)) { return qfalse;
          }
...
static qboolean Alias_Parse(char **p) {
  ...
  while ( 1 ) {
        ...
        if (token[0] == '{') {
          // three tokens per line, character name, bot alias, and preferred
action a - all purpose, d - defense, o - offense if (!String_Parse(p,
...
static int UI_PlayCinematic(const char* name,
                            float x,
                            float y,
                            float w,
                            float h) {
...
i++,dirptr+=dirlen+1)
        {
                ...
                numfiles = trap_FS_GetFileList( va("models/players/%s",dirptr),
"tga", filelist, 2048 ); fileptr  = filelist; for (j=0; j<numfiles &&
...
void _UI_KeyEvent(int key, qboolean down) {
  ...
  if (Menu_Count() > 0) {
    ...
    if (menu) {
      ...
      //!Menus_AnyFullScreenVisible()) { 				Menus_CloseAll(); 			} else {
    ...
    switch (menu) {
      ...
      case UIMENU_MAIN:
        ...
        Menus_ActivateByName(
            "backgroundmusic",
            qtrue);  // Arnout: not nice, but best way to do it - putting the
                     // music in it's own menudef makes sure it doesn't get
                     // restarted every time you reach the main menu
        ...
        // see why.  I think this is bustified in q3ta NOTE TTimo - I'm not sure
        // Q_stricmp is useful to anything anymore show_bug.cgi?id=507 TTimo -
        // improved and tweaked that area a whole bunch
        if ((*buf) && (Q_stricmp(buf, ";"))) {
          trap_Cvar_Set("ui_connecting", "0");
          if (!Q_stricmpn(buf, "Invalid password", 16)) {
            trap_Cvar_Set("com_errorMessage",
                          trap_TranslateString(buf));  // NERVE - SMF
            Menus_ActivateByName("popupPassword", qtrue);
          } else if (strlen(buf) > 5 &&
                     !Q_stricmpn(buf, "ET://", 5)) {  // fretn
            ...
            // text printing limitations force us to keep it all in a single
            // message NOTE: this works thanks to flip flop in
            // UI_Cvar_VariableString
...
void UI_DrawConnectScreen(qboolean overlay) {
  ...
  if (!overlay) {
    ...
    //"sound/music/level_load.wav", "", 1000 ); 			playingMusic = qtrue;
    //		}
  }
  ...
  ITEM_TEXTSTYLE_SHADOWEDMORE); } else { strcpy(text, va( trap_TranslateString(
  "Connecting to %s" ), cstate.servername)); Text_PaintCenter(centerPoint,
  yStart + 48, scale, colorWhite,text , ITEM_TEXTSTYLE_SHADOWEDMORE);
          }
  ...
  Info_ValueForKey( cstate.updateInfoString, "motd" ), 0);
          ...
          // DHM - Nerve :: This now accepts strings up to 256 chars long, and
  will break them up into multiple lines.
          //					They are also now printed in
  Yellow for readability. if ( cstate.connState < CA_CONNECTED ) { char	*s; char
  ps[60]; int		i, len, index = 0, yPrint = yStart + 210; qboolean
  neednewline = qfalse;
                  ...
                  for ( i = 0; i < len; i++, index++ ) {
                          ...
                          // if out of temp buffer room OR end of string OR it
  is time to linebreak & we've found a space if ( (index >= 58) || (i ==
  ...
  trap_TranslateString( "Awaiting challenge...%i" ), cstate.connectPacketCount);
                  break;
          case CA_CONNECTED:
                          if (*downloadName) {
                                  UI_DisplayDownloadInfo( downloadName,
  centerPoint, yStart, scale ); return;
                          }
...
void UI_RegisterCvars(void) {
  ...
  BG_setCrosshair(cg_crosshairColor.string, uiInfo.xhairColor,
                  cg_crosshairAlpha.value, "cg_crosshairColor");
  BG_setCrosshair(cg_crosshairColorAlt.string, uiInfo.xhairColorAlt,
                  cg_crosshairAlphaAlt.value, "cg_crosshairColorAlt");
}
    ...
    if (cv->vmCvar) {
      ...
      if (cv->modificationCount != cv->vmCvar->modificationCount) {
        ...
        if (cv->vmCvar == &cg_crosshairColor ||
            cv->vmCvar == &cg_crosshairAlpha) {
          BG_setCrosshair(cg_crosshairColor.string, uiInfo.xhairColor,
                          cg_crosshairAlpha.value, "cg_crosshairColor");
        }

        if (cv->vmCvar == &cg_crosshairColorAlt ||
            cv->vmCvar == &cg_crosshairAlphaAlt) {
          BG_setCrosshair(cg_crosshairColorAlt.string, uiInfo.xhairColorAlt,
                          cg_crosshairAlphaAlt.value, "cg_crosshairColorAlt");
        }
      }
...
static void UI_StopServerRefresh(void) {
  ...
  if (count - uiInfo.serverStatus.numDisplayServers > 0) {
    // TTimo - used to be about cl_maxping filtering, that was Q3 legacy, RTCW
    // browser has much more filtering options
    Com_Printf(
