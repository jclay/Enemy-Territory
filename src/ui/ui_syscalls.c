...
void trap_R_AddCoronaToScene(const vec3_t org,
                             ...
                             int id,
                             qboolean visible) {
  syscall(UI_R_ADDCORONATOSCENE, org, PASSFLOAT(r), PASSFLOAT(g), PASSFLOAT(b),
...
void trap_R_DrawRotatedPic(float x,
                           ...
                           qhandle_t hShader,
                           float angle) {
  syscall(UI_R_DRAWROTATEDPIC, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w),
          PASSFLOAT(h), PASSFLOAT(s1), PASSFLOAT(t1), PASSFLOAT(s2),
...
int trap_CM_LerpTag(orientation_t* tag,
                    ...
                    const char* tagName,
                    int startIndex) {
  return syscall(UI_CM_LERPTAG, tag, refent, tagName,
                 0);  // NEFVE - SMF - fixed
}
...
void trap_S_FadeBackgroundTrack(
  ...
  syscall(
      UI_S_FADESTREAMINGSOUND, PASSFLOAT(targetvol), time,
