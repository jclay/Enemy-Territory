...
#define WINDOW_IGNORE_HUDALPHA \
  0x01000000  // window will apply cg_hudAlpha value to colors unless this flag
              // is set
#define WINDOW_DRAWALWAYSONTOP 0x02000000
#define WINDOW_MODAL \
  0x04000000  // window is modal, the window to go back to is stored in a stack
...
typedef struct itemDef_s {
  ...
  const char*
      onAccept;  // NERVE - SMF - run when the users presses the enter key
  ...
  //		the way it works, we could have multiple items in a menu with
  //the same hotkey 		so in the mission pack, we search through all the menu items
  //to find the one that is applicable to this key press 		so the item has to
  //store both the hotkey and the command to execute
  int hotkey;
  const char* onKey;
  // END - TAT 9/16/2002
  ...
  struct itemDef_s*
      toolTipData;  // OSP - Tag an item to this item for auto-help popups
...
typedef struct {
  ...
  // START - TAT 9/16/2002
  // should we search through all the items to find the hotkey instead of using
  // the onKey array?
...
typedef struct {
  ...
  void (*drawTextWithCursor)(float x,
                             float y,
                             float scale,
                             vec4_t color,
                             const char* text,
  ...
  qboolean (*ownerDrawHandleKey)(int ownerDraw,
                                 int flags,
                                 float* special,
                                 int key);
  int (*feederCount)(float feederID);
  const char* (*feederItemText)(float feederID,
                                int index,
                                int column,
                                qhandle_t* handles,
                                int* numhandles);
  ...
  void (*feederAddItem)(float feederID,
                        const char* name,
                        int index);              // NERVE - SMF
  ...
  void (*startBackgroundTrack)(const char* intro,
                               const char* loop,
                               int fadeupTime);
...
void SetupRotatedThing(polyVert_t* verts,
                       vec2_t org,
                       float w,
                       float h,
                       vec_t angle);
