...
=================
LerpColor
        lerp and clamp each component of <a> and <b> into <c> by the fraction
<t>
=================
*/
...
int Menu_ItemsMatchingGroup(menuDef_t* menu, const char* name) {
  ...
  pdest = strstr(name, "*");  // allow wildcard strings (ex.  "hide nb_*" would
...
itemDef_t* Menu_GetMatchingItemByNumber(menuDef_t* menu,
                                        int index,
                                        const char* name) {
  int i;
  int count = 0;
    ...
    if (menu->window.flags & WINDOW_MODAL) {
      if (modalMenuCount <= 0) {
        Com_Printf(S_COLOR_YELLOW
                   "WARNING: tried closing a modal window with an empty modal "
                   "stack!\n");
      } else {
        modalMenuCount--;
        // if modal doesn't have a parent, the stack item may be NULL .. just go
        // back to the main menu then
        if (modalMenuStack[modalMenuCount]) {
          Menus_ActivateByName(
              modalMenuStack[modalMenuCount]->window.name,
              qfalse);  // don't try to push the one we are opening to the stack
        }
      }
    }
  }
      ...
      case 2:
        ...
        if (!Q_stricmp(cvar, "UIProfileIsActiveProfile")) {
          ...
          if (trap_FS_FOpenFile(
                  va("profiles/%s/profile.dat", ui_profileCleanedStr), &f,
                  FS_READ) >= 0) {
            alreadyExists = qtrue;
          ...
          /*} else if( !Q_stricmpn( cvar, "voteflags", 9 ) ) {
                  ...
                  trap_Cvar_VariableStringBuffer( "cg_ui_voteFlags", info,
             sizeof(info) );
...
void Menu_TransitionItemByName(menuDef_t* menu,
                               const char* p,
                               rectDef_t rectFrom,
                               rectDef_t rectTo,
                               int time,
...
void Script_Transition(itemDef_t* item, qboolean* bAbort, char** args) {
  ...
  if (String_Parse(args, &name)) {
    if (Rect_Parse(args, &rectFrom) && Rect_Parse(args, &rectTo) &&
      ...
      Menu_TransitionItemByName(item->parent, name, rectFrom, rectTo, time,
                                amt);
    }
  }
}

void Menu_OrbitItemByName(menuDef_t* menu,
                          const char* p,
                          float x,
                          float y,
                          float cx,
...
#ifndef _DEBUG
        Com_Printf(
            "^3WARNING: profile.pid found for profile '%s' - not executing "
            "%s\n",
...
void Script_SetEditFocus(itemDef_t* item, qboolean* bAbort, char** args) {
  ...
  if (String_Parse(args, &name)) {
    ...
    if (editItem && (editItem->type == ITEM_TYPE_EDITFIELD ||
      ...
      // the stupidest idea ever, let's just override the console, every ui
      // element, user choice, etc nuking this
...
commandDef_t commandList[] = {
    ...
    {"conditionalopen",
     &Script_ConditionalOpen},  // DHM - Nerve:: cvar menu menu
                                ...
                                // second if false
    {"conditionalscript", &Script_ConditionalScript},  // as conditonalopen, but
...
qboolean Item_TextField_HandleKey(itemDef_t* item, int key) {
  ...
  if (item->cvar) {
    ...
    // Gordon: make sure our cursorpos doesn't go oob, windows doesn't like
    // negative memory copy operations :)
    ...
    if (key & K_CHAR_FLAG) {
      ...
      if (key == K_HOME ||
          key == K_KP_HOME) {  // || ( tolower(key) == 'a' && trap_Key_IsDown(
                               // K_CTRL ) ) ) {
        item->cursorPos = 0;
        editPtr->paintOffset = 0;
        return qtrue;
...
static void Scroll_ListBox_ThumbFunc(void* p) {
  ...
  if (DC->realTime > si->nextScrollTime) {
    // need to scroll which is done by simulating a click to the item
    // this is done a bit sideways as the autoscroll "knows" that the item is a
    // listbox so it calls it directly Arnout: clear doubleclicktime though!
    ...
    if (key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2 ||
      ...
      if (editDef) {
        ...
        if (Rect_ContainsPoint(&testRect, DC->cursorx, DC->cursory)) {
          ...
          value *= (editDef->maxVal - editDef->minVal);
          // vm fuckage
          // value = (((float)(DC->cursorx - x)/ SLIDER_WIDTH) *
...
itemDef_t* Menu_SetNextCursorItem(menuDef_t* menu) {
  ...
  while (menu->cursorItem < menu->itemCount) {
    menu->cursorItem++;
    if (menu->cursorItem >=
        menu->itemCount) {  // (SA) had a problem 'tabbing' in dialogs with only
                            // one possible button
      if (!wrapped) {
        wrapped = qtrue;
        menu->cursorItem = 0;
...
void Menus_Activate(menuDef_t* menu) {
  ...
  // ydnar: set open time (note dc time may be 0, in which case refresh code
  // sets this)
  ...
  if (menu) {
    ...
    for (i = 0; i < menuCount; i++) {
      if (Menu_OverActiveItem(&Menus[i], DC->cursorx, DC->cursory)) {
        //				Menu_RunCloseScript(menu);
        //// NERVE - SMF - why do we close the calling menu instead of just
        //removing the focus? 				menu->window.flags &= ~(WINDOW_HASFOCUS |
        //WINDOW_VISIBLE | WINDOW_MOUSEOVER);
...
void Menu_HandleKey(menuDef_t* menu, int key, qboolean down) {
  ...
  // START - TAT 9/16/2002
  // we need to check and see if we're supposed to loop through the items to
  // find the key press func
  if (!menu->itemHotkeyMode) {
    ...
    // we're using the item hotkey mode, so we want to loop through all the
    // items in this menu
    for (i = 0; i < menu->itemCount; i++) {
      it = menu->items[i];
      ...
      if (item) {
        if (item->type == ITEM_TYPE_TEXT) {
          ...
          if (Rect_ContainsPoint(&item->window.rect, DC->cursorx,
            ...
            // NERVE - SMF - reset scroll offset so we can see what we're
            // editing
            if (editPtr) {
              editPtr->paintOffset = 0;
            }
            ...
            // see elsewhere for venomous comment about this particular piece of
            // "functionality"
            //%	DC->setOverstrikeMode(qtrue);
          }
        } else {
...
void Item_Text_Paint(itemDef_t* item) {
  ...
  /*
     ...
     DC->Assets.shadowX, item->textRect.y + DC->Assets.shadowY, item->textscale,
...
void Item_TextField_Paint(itemDef_t* item) {
  ...
  // TTimo
  // text length control
  // if the edit field goes beyond the available width, drop some characters at
  // the beginning of the string and apply some offseting
  // FIXME: we could cache the text length and offseting, but given the low
  // count of edit fields, I abstained for now
  // FIXME: this won't handle going back into the line of the editfield to the
  // hidden area start of text painting: item->textRect.x + item->textRect.w +
  // offset our window limit: item->window.rect.x + item->window.rect.w
  field_offset = -1;
  do {
    field_offset++;
  ...
  if (field_offset) {
    // we had to take out some chars to make it fit in, there is an additional
    // screen offset to compute
...
void Item_CheckBox_Paint(itemDef_t* item) {
  ...
  if (item->text) {
    ...
    if (hasMultiText) {
      ...
      DC->drawText(
          item->textRect.x + item->textRect.w + 8 + item->window.rect.h + 4,
          item->textRect.y, item->textscale, colour, Item_Multi_Setting(item),
          0, 0, item->textStyle);
    }
  } else {
    if (item->type == ITEM_TYPE_TRICHECKBOX && value == 2) {
...
void Item_YesNo_Paint(itemDef_t* item) {
  ...
  if (item->text) {
    Item_Text_Paint(item);
    DC->drawText(
        item->textRect.x + item->textRect.w + 8, item->textRect.y,
        item->textscale, newColor,
        (value != 0) ? DC->translateString("Yes") : DC->translateString("No"),
        0, 0, item->textStyle);
  } else {
...
static bind_t g_bindings[] = {
    ...
    /*	{"+scores",         -1,				-1, -1, -1},
            ...
            {"+mlook",			'/',			-1, -1, -1},
    //	{"centerview",		K_END,			-1, -1, -1},	// this
    is an exploit nowadays
            {"+zoom",			'z',            -1, -1, -1},
    ...
    -1},
            {"weaplastused",	-1,				-1, -1,
    -1},//----(SA)	added
            {"weapnextinbank",	-1,				-1, -1,
    -1},//----(SA)	added
            {"weapprevinbank",	-1,				-1, -1,
    -1},//----(SA)	added
        ...
        if (listPtr->numColumns > 0) {
          ...
          for (j = 0; j < listPtr->numColumns; j++) {
            ...
            if (numOptionalImages > 0) {
              ...
              // DC->drawHandlePic( x + 4 + listPtr->columnInfo[j].pos, y - 1 +
              // listPtr->elementHeight / 2, listPtr->columnInfo[j].width,
              // listPtr->columnInfo[j].width, optionalImage);
            } else if (text) {
              DC->drawText(
                  x + 4 + listPtr->columnInfo[j].pos + item->textalignx,
...
void Menu_SetFeederSelection(menuDef_t* menu,
                             int feeder,
                             int index,
                             const char* name) {
  if (menu == NULL) {
...
menuDef_t* Menus_ActivateByName(const char* p, qboolean modalStack) {
  ...
  for (i = 0; i < menuCount; i++) {
    if (Q_stricmp(Menus[i].window.name, p) == 0) {
      ...
      break;  // Arnout: found it, don't continue searching as we might unfocus
              // the menu we just activated again.
    } else {
      Menus[i].window.flags &= ~(WINDOW_HASFOCUS | WINDOW_MOUSEOVER);
    }
  }
...
qboolean MenuParse_itemDef(itemDef_t* item, int handle) {
  ...
  if (menu->itemCount < MAX_MENUITEMS) {
    ...
    // START - TAT 9/16/2002
    // If we are storing the hotkeys in the items, we have a little problem, in
    // that
    //		people check with the menu to see if we have a hotkey (see
    //UI_CheckExecKey) 		So we sort of need to stuff the hotkey back into the menu
    //for that to work 		only do this at all if we're using the item hotkey mode
    //		NOTE:  we couldn't do this earlier because the menu wasn't set,
    //and I don't know 		what would happen if we tried to set the menu before the
    //parse had succeeded...
...
qboolean BG_RectContainsPoint(float x,
                              float y,
                              float w,
                              float h,
                              float px,
...
qboolean BG_PanelButton_EditClick(panel_button_t* button, int key) {
  if (key == K_MOUSE1) {
    ...
    if (key & K_CHAR_FLAG) {
      ...
      // Gordon: FIXME: have this work with all our stuff (use data[x] to store
      // cursorpos etc)
      /*			if ( key == K_DEL || key == K_KP_DEL ) {
                                      if ( item->cursorPos < len ) {
                                              memmove( buff + item->cursorPos,
         buff + item->cursorPos + 1, len - item->cursorPos);
                                              DC->setCVar(item->cvar, buff);
                                      }
                                      return qtrue;
                              }*/
      ...
      /*			if ( key == K_RIGHTARROW || key ==
         K_KP_RIGHTARROW )
                              {
                                      if (editPtr->maxPaintChars &&
         item->cursorPos >= editPtr->paintOffset + editPtr->maxPaintChars &&
         item->cursorPos < len) { item->cursorPos++; editPtr->paintOffset++;
                                              return qtrue;
                                      }
                                      if (item->cursorPos < len) {
                                              item->cursorPos++;
         ...
         editPtr->paintOffset) { editPtr->paintOffset--;
                              ...
                              if ( key == K_HOME || key == K_KP_HOME) {// || (
         tolower(key) == 'a' && trap_Key_IsDown( K_CTRL ) ) ) { item->cursorPos
         = 0; editPtr->paintOffset = 0; return qtrue;
                              }

                              if ( key == K_END || key == K_KP_END)  {// (
         tolower(key) == 'e' && trap_Key_IsDown( K_CTRL ) ) ) { item->cursorPos
         = len; if(item->cursorPos > editPtr->maxPaintChars) {
                                              editPtr->paintOffset = len -
...
qboolean BG_PanelButtonsKeyEvent(int key,
                                 qboolean down,
                                 panel_button_t** buttons) {
...
void BG_FitTextToWidth_Ext(char* instr,
                           float scale,
                           float w,
                           int size,
                           fontInfo_t* font) {
