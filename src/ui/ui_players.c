...
tryagain:
  ...
  // NERVE - SMF - multiplayer only hack to show correct panzerfaust and venom
  // barrel
  if (weaponNum == WP_PANZERFAUST) {
    pi->weaponModel =
        trap_R_RegisterModel("models/multiplayer/panzerfaust/multi_pf.md3");
    return;
  }
  // -NERVE - SMF
...
//----(SA) added
        ...
        if (	currentAnim == TORSO_RAISE || currentAnim == TORSO_RAISE2 ||
                        currentAnim == TORSO_RAISE3 || currentAnim ==
TORSO_RAISE4 || currentAnim == TORSO_RAISE5) { UI_SetTorsoAnim( pi, TORSO_STAND
); return;
        }
//----(SA) end
}
...
static void UI_LegsSequencing( playerInfo_t *pi ) {
        ...
        if ( pi->legsAnimationTimer > 0 ) {
                if ( currentAnim == LEGS_JUMP ) {
                        jumpHeight = JUMP_HEIGHT * sin( M_PI * ( UI_TIMER_JUMP -
pi->legsAnimationTimer ) / UI_TIMER_JUMP );
                }
                return;
        }
...
/*
// TTimo: unused
static void UI_SetLerpFrameAnimation( playerInfo_t *ci, lerpFrame_t *lf, int
newAnimation ) { animation_t	*anim;

        lf->animationNumber = newAnimation;
        newAnimation &= ~ANIM_TOGGLEBIT;

        if ( newAnimation < 0 || newAnimation >= MAX_ANIMATIONS ) {
                trap_Error( va("Bad animation number (UI_SLFA): %i",
newAnimation) );
        }
...
initial lerp } else { lf->frameTime = lf->oldFrameTime + anim->frameLerp;
                }
                ...
                if ( f >= anim->numFrames ) {
                        ...
                        if ( anim->loopFrames ) {
                                ...
                                // can immediately transition to another
sequence lf->frameTime = dp_realtime;
                        }
                }
...
/*
// TTimo: unused
static void UI_PlayerAnimation( playerInfo_t *pi, int *legsOld, int *legs, float
*legsBackLerp, int *torsoOld, int *torso, float *torsoBackLerp ) {
...
) { UI_RunLerpFrame( pi, &pi->legs, LEGS_TURN ); } else { UI_RunLerpFrame( pi,
&pi->legs, pi->legsAnim );
        }
...
==================
*/
static void UI_SwingAngles(float destination,
...
===============
*/
static void UI_PlayerAngles(playerInfo_t* pi,
                            vec3_t legs[3],
                            vec3_t torso[3],
                            vec3_t head[3]) {
...
===============
*/
static void UI_PlayerFloatSprite(playerInfo_t* pi,
                                 vec3_t origin,
                                 qhandle_t shader) {
  refEntity_t ent;
  ...
  if (pi->pendingWeapon != -1 && dp_realtime > pi->weaponTimer) {
    ...
    if (pi->currentWeapon != pi->weapon) {
      trap_S_StartLocalSound(
          trap_S_RegisterSound("sound/weapons/change.wav", qfalse), CHAN_LOCAL);
    }
  }
...
==========================
*/
static qboolean UI_RegisterClientSkin(playerInfo_t* pi,
  ...
  Com_sprintf(filename, sizeof(filename), "models/players/%s/body_%s.skin",
              modelName, skinName);  // NERVE - SMF - make this work with wolf
...
============
*/
static qboolean AnimParseAnimConfig(playerInfo_t* animModelInfo,
                                    const char* filename,
                                    const char* input) {
  char *text_p, *token;
  ...
  while (1) {
    ...
    if (!Q_stricmp(token, "footsteps")) {
      ...
      if (!Q_stricmp(token, "default") || !Q_stricmp(token, "normal")) {
        ...
        //				BG_AnimParseError( "Bad footsteps parm
        //'%s'\n", token );
      }
      continue;
  ...
  for (i = 0; (animModelInfo->version > 1) || (i < MAX_ANIMATIONS); i++) {
    ...
    if (animModelInfo->version >
        1) {  // includes animation names at start of each line
      ...
      Q_strncpyz(animations[i].name, animStrings[i],
                 sizeof(animations[i].name));
      // convert to all lower case
      Q_strlwr(animations[i].name);
    }
    ...
    if (!animModelInfo
             ->isSkeletal) {  // skeletal models dont require adjustment
  ...
  if (token && token[0]) {
    if (animModelInfo->version < 2 || !Q_stricmp(token, "HEADFRAMES")) {
      ...
      for (i = 0; i < MAX_HEAD_ANIMS; i++) {
        ...
        // modify according to last frame of the main animations, since the head
        // is totally seperate
      ...
      if (i != MAX_HEAD_ANIMS) {
        //				BG_AnimParseError( "Incorrect number of
        //head frames" );
        return qfalse;
      }
    }
...
static qboolean UI_ParseAnimationFile(const char* filename, playerInfo_t* pi) {
  ...
  /*
     ...
     animations[LEGS_WALKCR].firstFrame - animations[TORSO_GESTURE].firstFrame;
                  }
                  if ( i >= LEGS_WALKCR ) {
                          animations[i].firstFrame -= skip;
                  }
...
qboolean UI_RegisterClientModelname(playerInfo_t* pi,
  ...
  //		Q_strncpyz( skinName, "bluesoldier1", sizeof( skinName ) );
  //// NERVE - SMF - make this work with wolf - TESTING!!!
  //	}
  //	else {
  //		Q_strncpyz( skinName, "redsoldier1", sizeof( skinName ) );
  //// NERVE - SMF - make this work with wolf - TESTING!!!
  //	}
  ...
  Com_sprintf(filename, sizeof(filename), "models/players/%s/body.mds",
              modelName);  // NERVE - SMF - make this work with wolf
  ...
  if (backpack) {
    pi->backpackModel =
        trap_R_RegisterModel(va("models/players/%s/%s", modelName, backpack));
  }

  if (helmet) {
    pi->helmetModel =
        trap_R_RegisterModel(va("models/players/%s/%s", modelName, helmet));
  }
  ...
  // load the animations
  //----(SA) changing name of config file to avoid backwards or alternate
  //compatibility confustion 	Com_sprintf( filename, sizeof( filename ),
  //"models/players/%s/animation.cfg", modelName );
  Com_sprintf(filename, sizeof(filename), "models/players/%s/wolfanim.cfg",
              modelName);
  //----(SA) end
  if (!UI_ParseAnimationFile(filename,
                             pi)) {  // NERVE - SMF - make this work with wolf
