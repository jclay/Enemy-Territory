...
// Time mouse was reset, we ignore the first 50ms of the mouse to allow settling
// of events
...
**   in-game it would probably be better to use KeyCode (i.e. hardware key
*codes)
**   you would still need the KeySyms in some cases, such as for the console and
*all entry textboxes
**     (cause there's nothing worse than a qwerty mapping on a french keyboard)
**
** you can turn on some debugging and verbose of the keyboard code with #define
*KBD_DBG
******************************************************************************/
...
static char* XLateKey(XKeyEvent* ev, int* key) {
  ...
  *key = 0;
  ...
  if (!in_shiftedkeys->integer) {
    ...
    ev->state = 0;
    XLookupRet =
        XLookupString(ev, shiftlessbuf, sizeof shiftlessbuf, &keysym, 0);

#ifdef KBD_DBG
    ri.Printf(PRINT_ALL,
              "XLookupString ret (shiftless): %d buf: %s keysym: %x\n",
      ...
      if (XLookupRet == 0) {
        ...
        if (in_shiftedkeys->integer) {
          ...
          *key = buf[0];
          ...
          // if ctrl is pressed, the keys are not between 'A' and 'Z', for
          // instance ctrl-z == 26 ^Z ^C etc. see show_bug.cgi?id=19
...
static void install_grabs(void) {
  ...
  XGrabPointer(dpy, win,  // bk010108 - do this earlier?
               False, MOUSE_MASK, GrabModeAsync, GrabModeAsync, win, None,
               CurrentTime);
  ...
  if (in_dgamouse->value) {
    ...
    if (!XF86DGAQueryVersion(dpy, &MajorVersion, &MinorVersion)) {
      ...
      if (!XF86DGADirectVideo(dpy, DefaultScreen(dpy), XF86DGADirectMouse)) {
        ri.Printf(PRINT_ALL,
                  "XF86DGADirectVideo failed despite advertised "
                  "XF86DGAQueryVersion - broken drivers?");
        ri.Cvar_Set("in_dgamouse", "0");
      } else {
    ...
    switch (event.type) {
      ...
      case ButtonPress:
        ...
        if (event.xbutton.button == 4) {
          ...
          // NOTE TTimo there seems to be a weird mapping for K_MOUSE1 K_MOUSE2
          // K_MOUSE3 ..
          b = -1;
...
static void GLW_GenDefaultLists(void) {
  ...
  fontInfo =
      XLoadQueryFont(dpy, "-*-helvetica-medium-r-normal-*-12-*-*-*-p-*-*-*");
  if (fontInfo == NULL) {
  ...
  for (i = firstrow; i <= lastrow; i++) {
    ...
    qglXUseXFont(fontInfo->fid, firstbitmap, last - first + 1,
                 gl_NormalFontBase + firstbitmap);
    firstbitmap += 256;
  }
...
void GLimp_SetGamma(unsigned char red[256],
  ...
  // NOTE TTimo we get the gamma value from cvar, because we can't work with the
  // s_gammatable
...
void GLimp_Shutdown(void) {
  ...
  if (dpy) {
    ...
    // NOTE TTimo opening/closing the display should be necessary only once per
    // run
...
static void GLW_InitExtensions(void) {
  ...
  if (Q_stristr(glConfig.extensions_string, "GL_ARB_multitexture")) {
    if (r_ext_multitexture->value) {
      qglMultiTexCoord2fARB = (PFNGLMULTITEXCOORD2FARBPROC)dlsym(
          glw_state.OpenGLLib, "glMultiTexCoord2fARB");
      qglActiveTextureARB = (PFNGLACTIVETEXTUREARBPROC)dlsym(
          glw_state.OpenGLLib, "glActiveTextureARB");
      qglClientActiveTextureARB = (PFNGLCLIENTACTIVETEXTUREARBPROC)dlsym(
          glw_state.OpenGLLib, "glClientActiveTextureARB");
  ...
  if (Q_stristr(glConfig.extensions_string, "GL_EXT_compiled_vertex_array")) {
    if (r_ext_compiled_vertex_array->value) {
      ...
      qglLockArraysEXT = (void(APIENTRY*)(int, int))dlsym(glw_state.OpenGLLib,
                                                          "glLockArraysEXT");
      qglUnlockArraysEXT = (void(APIENTRY*)(void))dlsym(glw_state.OpenGLLib,
                                                        "glUnlockArraysEXT");
...
#define GAMMA_MINMINOR 0
  ...
  if (vidmode_ext) {
    if (vidmode_MajorVersion < GAMMA_MINMAJOR ||
        (vidmode_MajorVersion == GAMMA_MINMAJOR &&
         vidmode_MinorVersion < GAMMA_MINMINOR)) {
      ri.Printf(PRINT_ALL,
                "XF86 Gamma extension not supported in this version\n");
      return;
    }
...
void IN_Init(void) {
  ...
  joy_threshold = Cvar_Get("joy_threshold", "0.15",
                           CVAR_ARCHIVE);  // FIXME: in_joythreshold
