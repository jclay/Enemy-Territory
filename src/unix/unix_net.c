...
qboolean Sys_IsLANAddress(netadr_t adr) {
  ...
  if (num_interfaces) {
    ...
    for (i = 0; i < num_interfaces; i++) {
      //			Com_Printf( "%d: %08lx/%08lx ", i, netint[i].ip,
      //netint[i].mask );
  ...
  // choose which comparison to use based on the class of the address being
  // tested any local adresses of a different class than the address being
  // tested will fail based on the first byte
  ...
  if ((adr.ip[0] & 0xc0) == 0x80) {
    for (i = 0; i < numIP; i++) {
      ...
      if (adr.ip[0] == 172 && localIP[i][0] == 172 &&
          (adr.ip[1] & 0xf0) == 16 && (localIP[i][1] & 0xf0) == 16) {
        return qtrue;
      }
    }
    return qfalse;
  ...
  for (i = 0; i < numIP; i++) {
    ...
    if (adr.ip[0] == 192 && localIP[i][0] == 192 && adr.ip[1] == 168 &&
        localIP[i][1] == 168) {
      return qtrue;
    }
  }
...
void Sys_ShowIP(void) {
  ...
  for (i = 0; i < numIP; i++) {
    Com_Printf("IP: %i.%i.%i.%i\n", localIP[i][0], localIP[i][1], localIP[i][2],
               localIP[i][3]);
  }
}
...
*/
#ifdef MACOS_X
// Don't do a forward mapping from the hostname of the machine to the IP.  The
// reason is that we might have obtained an IP address from DHCP and there might
// not be any name registered for the machine.  On Mac OS X, the machine name
// defaults to 'localhost' and NetInfo has 127.0.0.1 listed for this name.
// Instead, we want to get a list of all the IP network interfaces on the
// machine. This code adapted from OmniNetworking.
...
void NET_GetLocalAddress(void) {
  ...
  if (ioctl(interfaceSocket, SIOCGIFCONF, &ifc) != 0) {
    Com_Printf(
        "NET_GetLocalAddress: Unable to get list of network interfaces, errno "
        "= %d\n",
        errno);
    return;
  }
  ...
  while ((char*)linkInterface < &ifc.ifc_buf[ifc.ifc_len]) {
    ...
    // The ioctl returns both the entries having the address (AF_INET) and the
    // link layer entries (AF_LINK).  The AF_LINK entry has the link layer
    // address which contains the interface type.  This is the only way I can
    // see to get this information.  We cannot assume that we will get bot an
    // AF_LINK and AF_INET entry since the interface may not be configured.  For
    // example, if you have a 10Mb port on the motherboard and a 100Mb card, you
    // may not configure the motherboard port.
      ...
      while ((char*)inetInterface < &ifc.ifc_buf[ifc.ifc_len]) {
        if (inetInterface->ifr_addr.sa_family == AF_INET &&
          ...
          if (sdl->sdl_type != IFT_LOOP) {
            ...
            strncpy(ifr.ifr_name, inetInterface->ifr_name,
                    sizeof(ifr.ifr_name));
            if (ioctl(interfaceSocket, OSIOCGIFADDR, (caddr_t)&ifr) < 0) {
              Com_Printf(
                  "NET_GetLocalAddress: Unable to get local address for "
                  "interface '%s', errno = %d\n",
              ...
              Com_Printf("IP: %i.%i.%i.%i (%s)\n", localIP[numIP][0],
                         localIP[numIP][1], localIP[numIP][2],
                         localIP[numIP][3], inetInterface->ifr_name);
              numIP++;
            }
          }

          // We will assume that there is only one AF_INET entry per AF_LINK
          // entry. What happens when we have an interface that has multiple IP
          // addresses, or can that even happen? break;
        }
