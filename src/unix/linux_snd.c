...
qboolean SNDDMA_Init(void) {
  ...
  if (rc < 0) {
    perror(snddevice->string);
    Com_Printf("Could not set %s to stereo=%d", snddevice->string,
               dma.channels);
    close(audio_fd);
    return 0;
  }
  ...
  /*
          if (!dma.buffer)
                  dma.buffer = (unsigned char *) mmap(NULL, info.fragstotal
                          * info.fragsize, PROT_WRITE|PROT_READ,
  ...
  */
  ...
  if (dma.buffer == NULL) {
    ...
    //    Com_Printf("trying mmap PROT_WRITE (with associated better
    //    compatibility / less performance code)\n");
    dma.buffer =
    ...
    if (dma.buffer == MAP_FAILED)
      dma.buffer = NULL;
    // NOTE TTimo could add a variable to force using regular memset on systems
