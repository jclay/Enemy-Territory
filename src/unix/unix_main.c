...
// console then it won't look good
//   so we provide tty_Clear and tty_Show to be called before and after a stdout
//   or stderr output
// =============================================================
...
char* Sys_ConsoleInput(void) {
  ...
  if (ttycon && ttycon->value) {
    avail = read(0, &key, 1);
    if (avail != -1) {
      // NOTE: testing a lot of values .. seems it's the only way to get it to
      // work everywhere
      ...
      if ((key) && (key) < ' ') {
        ...
        if (key == '\t') {
          ...
          //   cursor doesn't reflect actual length of the string that's sent
          //   back
...
NOTE T.Ray:
fqpath param added 2/15/02 by T.Ray - Sys_LoadDll is only called in vm.c at this
time fqpath will be empty if dll not loaded, otherwise will hold fully qualified
path of dll module loaded fqpath buffersize must be at least MAX_QPATH+1 bytes
long
=================
*/
...
this is used when we are loading a mod and realize we don't have the DLL in the
standard path
=================
*/
qboolean CopyDLLForMod(char** p_fn,
                       const char* gamedir,
                       const char* pwdpath,
                       const char* homepath,
                       const char* basepath,
  ...
  if (strlen(gamedir) && Q_stricmp(gamedir, BASEGAME)) {
    ...
    // NOTE TTimo: we don't attempt to validate version-wise, it could be a
    // problem (acceptable tradeoff I say, should not cause major issues)
#ifndef NDEBUG
    ...
    // fortunately FS_BuildOSPath does a flip flop, we have 'from path' in fn
    // and 'to path' in *p_fn
    *p_fn = FS_BuildOSPath(homepath, gamedir, fname);
    // copy to destination
    FS_CopyFile(fn, *p_fn);
    if (access(*p_fn, R_OK) ==
        -1) {  // could do with FS_CopyFile returning a boolean actually
...
#define Q_RTLD RTLD_NOW
  ...
  // we need to pass this to FS_ExtractFromPakFile so that it checksums the
  // right file NOTE: if something fails (not found, or file operation failed),
  // we will ERR_FATAL (in the checksum itself, we only ERR_DROP)
#ifndef NDEBUG
  ...
  if (access(fn, R_OK) == -1) {
    ...
    if (access(fn, R_OK) == 0) {
      ...
      if (Q_stricmp(Cvar_VariableString("version"), lastVersion->string)) {
        ...
        if (access(fn, R_OK) == -1) {
          // we may be dealing with a media-only mod, check wether we can find
          // 'reference' DLLs and copy them over
          if (!CopyDLLForMod(&fn, gamedir, pwdpath, homepath, basepath,
                             fname)) {
            Com_Error(ERR_FATAL,
                      "Sys_LoadDll(%s) failed, no corresponding .so file found "
                      "in fs_homepath or fs_basepath\n",
                      name);
          }
        }
      }
      // the .so in fs_homepath is valid version-wise ..
      // FS_CL_ExtractFromPakFile will have to decide wether it's valid pk3-wise
      // later
    } else {
      fn = FS_BuildOSPath(basepath, gamedir, fname);
      if (access(fn, R_OK) == -1) {
  ...
  // NOTE #2: we may have found a file in fs_basepath, and if the checksum is
  // wrong, FS_Extract will write in fs_homepath
...
UGLY HACK:
  ...
  but system doesn't replace the current process, which leads to a situation
like: wolf.x86--spawned_process.x86 in the case of auto-update for instance,
this will cause write access denied on wolf.x86:
  wolf-beta/2002-March/000079.html
  we hack the implementation here, if there are no space in the command line,
assume it's a straight process and use execl otherwise, use system .. The clean
solution would be Sys_StartProcess and Sys_StartProcess_Args..
==================
*/
void Sys_DoStartProcess(char* cmdline) {
  switch (fork()) {
  ...
  if (access(fn, X_OK) == -1) {
    ...
    if (access(fn, X_OK) == -1) {
      ...
      if (access(fn, X_OK) == -1) {
        Com_DPrintf("%s not found\n", fn);
        Com_Printf(
            "Can't find script '%s' to open requested URL (use +set developer "
            "1 for more verbosity)\n",
            fname);
        // we won't quit
        return;
      }
