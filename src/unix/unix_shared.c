...
disable with in_subframe 0
 ...
 I didn't find much info in the XWindow documentation about the wrapping
   we clamp sys_timeBase*1000 to unsigned long, that gives us the current origin
for xtime the computation will still work if xtime wraps (at ~49 days period
since the Epoch) after we set sys_timeBase
...
//	Com_Printf("xtime: %p\n", xtime);
  ...
  // printf("delta: %d\n", test);
  if (test < 0 ||
      test > 30) {  // in normal conditions I've never seen this go above
    return time;
  }
...
// char **Sys_ListFiles( const char *directory, const char *extension, int
// *numfiles, qboolean wantsubs )
  ...
  while ((d = readdir(fdir)) != NULL) {
    ...
    if (*extension) {
      if (strlen(d->d_name) < strlen(extension) ||
          Q_stricmp(d->d_name + strlen(d->d_name) - strlen(extension),
                    extension)) {
